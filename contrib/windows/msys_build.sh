...
if ! [ -e julia-installer.exe ]; then
  f=julia-0.3-latest-win$bits.exe
  echo "Downloading $f"
  $curlflags -O http://s3.amazonaws.com/julialang/bin/winnt/x$archsuffix/0.3/$f
  echo "Extracting $f"
...
if [ -n "$USEMSVC" ]; then
  ...
  echo 'override SUITESPARSE_VER = 4.2.1' >> Make.user
  make -C deps SuiteSparse-4.2.1/Makefile
  touch deps/SuiteSparse-4.2.1/UMFPACK/Lib/libumfpack.a
