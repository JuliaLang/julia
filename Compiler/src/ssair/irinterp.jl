...
function enqueue_succ!(scanner::BBScanner, succ::Int)
    succ in scanner.scanned && return true  # already visited
    push!(scanner.bb_ip, succ)
    return false   # newly enqueued
end

# Process the terminator and add the successors to `bb_ip`. Returns whether a backedge was seen.
function process_terminator!(@nospecialize(stmt), bb::Int, scanner::BBScanner)
    if isa(stmt, ReturnNode)
        return false
    elseif isa(stmt, GotoNode)
        return enqueue_succ!(scanner, stmt.label)
    elseif isa(stmt, GotoIfNot)
        backedge = enqueue_succ!(scanner, stmt.dest)
        backedge |= enqueue_succ!(scanner, bb+1)
        return backedge
    elseif isa(stmt, EnterNode)
        dest = stmt.catch_dest
        backedge = false
        if dest ≠ 0
            backedge |= enqueue_succ!(scanner, dest)
        end
        backedge |= enqueue_succ!(scanner, bb+1)
        return backedge
    else
        return enqueue_succ!(scanner, bb+1)
    end
end
