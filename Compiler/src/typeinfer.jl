...
function typeinf_ext_toplevel(methods::Vector{Any}, worlds::Vector{UInt}, trim::Bool)
    ...
    for this_world in reverse(sort!(worlds))
        ...
        while !isempty(tocompile)
            ...
            elseif isa(def, Method) && ccall(:jl_get_module_infer, Cint, (Any,), def.module) == 0
                src = retrieve_code_info(mi, get_inference_world(interp))
            else
            ...
            if src isa CodeInfo
                ...
                # but for the purpose of native compile, we always want them put there (or
                # to go ahead and reuse the ones already there, if there was some unintended
                # duplication of that).
                preexisting = ccall(:jl_get_ci_equiv, Any, (Any, Int32), callee, 0)
                if preexisting === nothing
                    code_cache(interp)[mi] = callee
                else
                    callee = preexisting::CodeInstance
                end
