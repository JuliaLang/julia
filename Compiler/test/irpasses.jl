...
let ir = Base.code_ircode(; optimize_until="slot2ssa (ERRONOUS PASS NAME)") do
        v = try
        catch
        end
        v
...
let ir = first(only(Base.code_ircode(f_with_early_try_catch_exit, (); optimize_until="compact (ERRONOUS PASS NAME)")))
