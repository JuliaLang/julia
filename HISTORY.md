...
- The package environment (active project, `LOAD_PATH`, `DEPOT_PATH`) is now propagated when adding _local_ workers
...
- An _exception stack_ is maintained on each task to make exception handling
...
- `readuntil` now does _not_ include the delimiter in its result, matching the
...
- `@sync` now waits only for _lexically_ enclosed (i.e. visible directly in the source
...
- Expressions `x^-n` where `n` is an _integer literal_ now correspond to `inv(x)^n`.
...
- `readline`, `readlines` and `eachline` return lines without line endings by default.
  You _must_ use `readline(s, chomp=false)`, etc. to get the old behavior where
...
- The REPL now supports something called _prompt pasting_ ([#17599]).
...
- Incremental precompilation of modules: call `VERSION >= v"0.4.0-dev+6521" && __precompile__()` at the top of a
  ...
  - See manual section on `Module initialization and precompilation` (under `Modules`) for
    ...
    time. Modules that are _not_ precompilable should call `__precompile__(false)`.
