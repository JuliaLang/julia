...
const history_keymap = AnyDict(
    "^R" => (s::MIState,o...)->(repl_history_hook!(s)),
    "^S" => (s::MIState,o...)->(repl_history_hook!(s)),
    # C/M-n/p
...
function repl_history_hook!(mistate::MIState)
    cancel_beep(mistate)
    termbuf = TerminalBuffer(IOBuffer())
    term = terminal(Base.active_repl.mistate)
    mimode = mode(mistate)
    mistate.mode_state[mimode] =
        deactivate(mimode, state(mistate), termbuf, term)
    result = histsearch(mimode.hist.history, term, mimode.prompt_prefix)
