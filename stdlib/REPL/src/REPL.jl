    ...
    while true
        ...
        try
            ...
            if lasterr !== nothing
                ...
                vtype = typeof(value)
                if startswith(String(nameof(vtype)), '#')
                    vtype = supertype(vtype)
                end
                vtname = nameof(vtype)
                '\n' in String(vtname) && (vtname = Symbol(replace(String(vtname), '\n' => ' ')))
                push!(backend.hist.histfile,
                      HistUpdate(backend.hist.session, vtname, false, eval_duration))
            end
            break
        catch err
            if lasterr !== nothing
            ...
            etname = nameof(errtype)
            '\n' in String(etname) && (etname = Symbol(replace(String(etname), '\n' => ' ')))
            push!(backend.hist.histfile,
                  HistUpdate(backend.hist.session, etname, true, time() - eval_start))
        end
    end
    Base.sigatomic_end()
    nothing
