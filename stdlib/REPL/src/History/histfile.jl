...
function update!(hist::HistoryFile)
    (; file, records) = hist
    # If the file has grown since the last read, we need to trigger a
    # synchronisation of the stream state. This can be done with `fseek`,
    # but that can't easily be called from Julia. Instead, we can use `filesize`
    # to detect when we need to do this, and then use `peek` to trigger the
    # synchronisation. This relies on undocumented implementation details,
    ...
    try
        ...
        histindex = if isempty(hist.records)
            sizehint!(hist.records, length(bytes) ÷ 64)
            0
        else
        ...
        while true
            ...
            while pos < length(bytes) && bytes[pos] == UInt8('#')
                ...
                if isstrmatch(bytes, metastart, "id:")
                    ...
                    if slashpos < valend
                        sessionval = tryparse(UInt64, StringView(@view bytes[valstart:slashpos-1]), base=62)
                        sidval = tryparse(UInt32, StringView(@view bytes[slashpos+1:valend-1]))
                ...
                elseif isstrmatch(bytes, metastart, "mode:")
                    ...
                    elseif all(>(0x5a), view(bytes, valstart:valend-1))
                        ...
                        Symbol(lowercase(StringView(@view bytes[valstart:valend-1])))
                    end
                elseif isstrmatch(bytes, metastart, "time:")
                    valend = min(valend, valstart + ncodeunits("0000-00-00 00:00:00"))
                    timestr = StringView(@view bytes[valstart:valend-1])
