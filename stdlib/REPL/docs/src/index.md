...
It is possible to get an interface which is similar to the IPython REPL and the Mathematica notebook with numbered input prompts and output prefixes. This is done by calling `REPL.numbered_mode!()`. If you want to have this enabled on startup, add
...
atreplinit() do repl
    ...
    REPL.numbered_mode!(repl)
end
```

to your `startup.jl` file. In numbered mode the variable `Out[n]` (where `n` is an integer) can be used to refer to earlier results:
