...
# Content in this file was originally extracted from BinaryProvider.jl (MIT), via Pkg.

module PlatformEngines

using SHA, Downloads, Tar, TOML
using ArgTools: FileSpec
using ..ArtifactDownloads: pkg_server, depots1, can_fancyprint, stderr_f, atomic_toml_write
...
A handler is only invoked if `occursin(urlscheme, url)` is true (where `url` is the URL
currently being downloaded.)

`f` must be a function that takes three input arguments `(url, pkgserver, err)`, where `url` is the
URL currently being downloaded, `pkgserver = ArtifactDownloads.pkg_server()` the current package server, and
