...
using Test, ArtifactDownloads, ArtifactDownloads.PlatformEngines, Base.BinaryPlatforms, SHA, Sockets
...
@testset "Authentication Header Hooks" begin
    ...
    function test_server_dir(url, server, expected_directory::AbstractString)
        observed = PlatformEngines.get_server_dir(url, server)
        expected = joinpath(ArtifactDownloads.depots1(), "servers", expected_directory)
        ...
        @test startswith(observed, ArtifactDownloads.depots1())
        @test startswith(observed, joinpath(ArtifactDownloads.depots1(), "servers"))
    end
