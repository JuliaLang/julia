...
let buf = IOBuffer()
    ...
    @test String(take!(buf)) == "  \e[3memph\e[23m"
end
...
let word = "Markdown" # disable underline when wrapping lines
    ...
    @test endswith(lines[begin], StyledStrings.ANSI_STYLE_CODES.end_italics)
    @test startswith(lines[begin+1], ' '^Markdown.margin * StyledStrings.ANSI_STYLE_CODES.start_italics)
end
