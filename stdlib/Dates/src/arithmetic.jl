...
function (+)(dt::DateTime, y::Union{Year,Years})
...
function (+)(dt::Date, y::Union{Year,Years})
    oy, m, d = yearmonthday(dt); ny = oy + value(y); ld = daysinmonth(ny, m)
...
function (+)(dt::DateTime, z::Union{Month,Months})
...
function (+)(dt::Date, z::Union{Month,Months})
    y,m,d = yearmonthday(dt)
    ny = yearwrap(y, m, value(z))
    mm = monthwrap(m, value(z)); ld = daysinmonth(ny, mm)
...
(+)(x::Date, y::Union{Quarter,Quarters}) = x + Month(y)
(-)(x::Date, y::Union{Quarter,Quarters}) = x - Month(y)
(+)(x::DateTime, y::Union{Quarter,Quarters}) = x + Month(y)
(-)(x::DateTime, y::Union{Quarter,Quarters}) = x - Month(y)
(+)(x::Date, y::Union{Week,Weeks}) = return Date(UTD(value(x) + 7 * value(y)))
(-)(x::Date, y::Union{Week,Weeks}) = return Date(UTD(value(x) - 7 * value(y)))
(+)(x::Date, y::Union{Day,Days})  = return Date(UTD(value(x) + value(y)))
(-)(x::Date, y::Union{Day,Days})  = return Date(UTD(value(x) - value(y)))
