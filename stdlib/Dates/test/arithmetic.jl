...
@testset "Vectorized arithmetic" begin
    ...
    @testset "deprecated" begin
        @test_deprecated "use `x .- y` instead" Dates.Date(2010, 1, 1) - t1
        @test_deprecated "use `x .- y` instead" t1 - Dates.Date(2010, 1, 1)
        @test_deprecated "use `x .- y` instead" Dates.DateTime(2009, 1, 1) - t3
        @test_deprecated "use `x .- y` instead" t3 - Dates.DateTime(2009, 1, 1)
        @test_deprecated "use `x .+ y` instead" Dates.Day(1) + t1
        @test_deprecated "use `x .+ y` instead" Dates.Hour(1) + t3
        @test_deprecated "use `x .+ y` instead" t1 + Dates.Day(1)
        @test_deprecated "use `x .+ y` instead" t3 + Dates.Hour(1)
        @test_deprecated "use `x .+ y` instead" t5 + Dates.Hour(1)
        @test_deprecated "use `x .+ y` instead" (Dates.Month(1) + Dates.Day(1)) + t1
        @test_deprecated "use `x .+ y` instead" (Dates.Hour(1) + Dates.Minute(1)) + t3
