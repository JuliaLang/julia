...
temp_pkg_dir() do
    ...
    begin
        ...
        msg = read(`$(Base.julia_cmd()) --startup-file=no -e 'redirect_stderr(STDOUT); using Pkg; Pkg.build("BuildFail")'`, String)
    ...
    let package = "Example"
        ...
        msg = read(ignorestatus(`$(Base.julia_cmd()) --startup-file=no -e "redirect_stderr(STDOUT); using Pkg; Pkg.build(\"$package\")"`), String)
    ...
    let package = "Example"
        ...
        msg = read(ignorestatus(`$(Base.julia_cmd()) --startup-file=no -e
            "redirect_stderr(STDOUT); using Example,Pkg; Pkg.update(\"$package\")"`), String)
    ...
    let package = "StartupFile"
        ...
        withenv((Sys.iswindows() ? "USERPROFILE" : "HOME") => home) do
            code = "redirect_stderr(STDOUT); using Pkg; Pkg.build(\"$package\")"
            ...
            code = "redirect_stderr(STDOUT); using Pkg; Pkg.test(\"$package\")"
    ...
    let package = "Output"
        ...
        code = "using Pkg; Pkg.build(\"$package\")"
        msg = run(pipeline(
