...
function chol!(F::Factor{Tv}, A::Sparse{Tv}; shift::Real=0.0) where Tv
    # Makes it an LLt
...
"""
    chol!(F::Factor, A; shift = 0.0) -> CHOLMOD.Factor
...
!!! note
    ...
    as appropriate.
"""
chol!(F::Factor, A::Union{SparseMatrixCSC{T},
    ...
    chol!(F, Sparse(A); shift = shift)

function chol(A::Sparse; shift::Real=0.0,
    perm::AbstractVector{SuiteSparse_long}=SuiteSparse_long[])
    ...
    # Compute the numerical factorization
    chol!(F, A; shift = shift)

    return F
end

"""
    chol(A; shift = 0.0, perm = Int[]) -> CHOLMOD.Factor
...
`F = chol(A)` is most frequently used to solve systems of equations with `F\\b`,
...
"""
chol(A::Union{SparseMatrixCSC{T}, SparseMatrixCSC{Complex{T}},
    Symmetric{T,SparseMatrixCSC{T,SuiteSparse_long}},
    Hermitian{Complex{T},SparseMatrixCSC{Complex{T},SuiteSparse_long}},
    Hermitian{T,SparseMatrixCSC{T,SuiteSparse_long}}};
    kws...) where {T<:Real} = chol(Sparse(A); kws...)
