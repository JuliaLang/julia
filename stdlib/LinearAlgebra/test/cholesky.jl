...
@testset "core functionality" begin
    ...
    for eltya in (Float32, Float64, ComplexF32, ComplexF64, BigFloat, Int)
        ...
        @testset "throw for non-square input" begin
            A = rand(eltya, 2, 3)
            @test_throws DimensionMismatch LinearAlgebra.legacychol!(A)
        ...
        @test all(x -> x ≈ √apos, chol(apos).factors)
        @test_throws PosDefException LinearAlgebra.legacychol(-one(eltya))

        # Test chol with Symmetric/Hermitian upper/lower
        apds  = Symmetric(apd)
        ...
        @test Matrix(chol(S).UL) ≈ [2 -1; 0 sqrt(eltya(3))] / sqrt(eltya(2))
        ...
        if eltya <: BlasFloat
            ...
            @testset "generic chol!" begin
                ...
                @test Matrix(chol(A).L) ≈ Matrix(invoke(LinearAlgebra._chol!, Tuple{AbstractMatrix, Type{LowerTriangular}}, copy(A), LowerTriangular)[1])
                @test Matrix(chol(A).U) ≈ Matrix(invoke(LinearAlgebra._chol!, Tuple{AbstractMatrix, Type{UpperTriangular}}, copy(A), UpperTriangular)[1])
            end
        end
    end
end
...
@testset "cholesky up- and downdates" begin
    ...
    for uplo in (:U, :L)
        ...
        BcB = AcA + v*v'
        BcB = (BcB + BcB')/2
        F = chol(Hermitian(AcA, uplo))
        G = chol(Hermitian(BcB, uplo))
...
@testset "issue #13243, unexpected nans in complex chol" begin
    ...
    chol(Hermitian(apd, :L), Val(true)) \ b
    r = factorize(apd).U
    E = abs.(apd - r'*r)
...
@testset "handling of non-Hermitian" begin
    ...
    C = complex.(R, R)
    for A in (R, C)
        @test !LinearAlgebra.issuccess(chol(A))
        @test !LinearAlgebra.issuccess(chol!(copy(A)))
        @test_throws PosDefException LinearAlgebra.legacychol(A)
        @test_throws PosDefException LinearAlgebra.legacychol!(copy(A))
    end
end

@testset "fail for non-BLAS element types" begin
    @test_throws ArgumentError chol!(Hermitian(rand(Float16, 5,5)), Val(true))
end
