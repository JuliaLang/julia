...
@testset "adjoint and transpose MemoryLayout" begin
    A = [1.0 2; 3 4]
    @test Base.MemoryLayout(A') == Base.DenseRowMajor()
    @test Base.MemoryLayout(transpose(A)) == Base.DenseRowMajor()
    B = [1.0+im 2; 3 4]
    @test Base.MemoryLayout(B') == LinearAlgebra.ConjLayout(Base.DenseRowMajor())
    @test Base.MemoryLayout(transpose(B)) == Base.DenseRowMajor()
    VA = view(A, 1:1, 1:1)
    @test Base.MemoryLayout(VA') == Base.RowMajor()
    @test Base.MemoryLayout(transpose(VA)) == Base.RowMajor()
    VB = view(B, 1:1, 1:1)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(Base.RowMajor())
    @test Base.MemoryLayout(transpose(VB)) == Base.RowMajor()
    VA = view(A, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VA') == Base.StridedLayout()
    @test Base.MemoryLayout(transpose(VA)) == Base.StridedLayout()
    VB = view(B, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(Base.StridedLayout())
    @test Base.MemoryLayout(transpose(VB)) == Base.StridedLayout()
    VA2 = view(A, [1,2], :)
    @test Base.MemoryLayout(VA2') == Base.UnknownLayout()
    @test Base.MemoryLayout(transpose(VA2)) == Base.UnknownLayout()
    VB2 = view(B, [1,2], :)
    @test Base.MemoryLayout(VB2') == Base.UnknownLayout()
    @test Base.MemoryLayout(transpose(VB2)) == Base.UnknownLayout()
    VAc = view(A', 1:1, 1:1)
    @test Base.MemoryLayout(VAc) == Base.RowMajor()
    VAt = view(transpose(A), 1:1, 1:1)
    @test Base.MemoryLayout(VAt) == Base.RowMajor()
    VBc = view(B', 1:1, 1:1)
    @test Base.MemoryLayout(VBc) == LinearAlgebra.ConjLayout(Base.RowMajor())
    VBt = view(transpose(B), 1:1, 1:1)
    @test Base.MemoryLayout(VBt) == Base.RowMajor()
end
