...
@testset "adjoint and transpose MemoryLayout" begin
    A = [1.0 2; 3 4]
    @test Base.MemoryLayout(A') == Base.DenseRowMajor{Float64}()
    @test Base.MemoryLayout(transpose(A)) == Base.DenseRowMajor{Float64}()
    B = [1.0+im 2; 3 4]
    @test Base.MemoryLayout(B') == LinearAlgebra.ConjLayout(Base.DenseRowMajor{ComplexF64}())
    @test Base.MemoryLayout(transpose(B)) == Base.DenseRowMajor{ComplexF64}()
    VA = view(A, 1:1, 1:1)
    @test Base.MemoryLayout(VA') == Base.DenseRowsStridedColumns{Float64}()
    @test Base.MemoryLayout(transpose(VA)) == Base.DenseRowsStridedColumns{Float64}()
    VB = view(B, 1:1, 1:1)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(Base.DenseRowsStridedColumns{ComplexF64}())
    @test Base.MemoryLayout(transpose(VB)) == Base.DenseRowsStridedColumns{ComplexF64}()
    VA = view(A, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VA') == Base.StridedLayout{Float64}()
    @test Base.MemoryLayout(transpose(VA)) == Base.StridedLayout{Float64}()
    VB = view(B, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(Base.StridedLayout{ComplexF64}())
    @test Base.MemoryLayout(transpose(VB)) == Base.StridedLayout{ComplexF64}()
    VA2 = view(A, [1,2], :)
    @test Base.MemoryLayout(VA2') == Base.UnknownLayout{Float64}()
    @test Base.MemoryLayout(transpose(VA2)) == Base.UnknownLayout{Float64}()
    VB2 = view(B, [1,2], :)
    @test Base.MemoryLayout(VB2') == Base.UnknownLayout{ComplexF64}()
    @test Base.MemoryLayout(transpose(VB2)) == Base.UnknownLayout{ComplexF64}()
    VAc = view(A', 1:1, 1:1)
    @test Base.MemoryLayout(VAc) == Base.DenseRowsStridedColumns{Float64}()
    VAt = view(transpose(A), 1:1, 1:1)
    @test Base.MemoryLayout(VAt) == Base.DenseRowsStridedColumns{Float64}()
    VBc = view(B', 1:1, 1:1)
    @test Base.MemoryLayout(VBc) == LinearAlgebra.ConjLayout(Base.DenseRowsStridedColumns{ComplexF64}())
    VBt = view(transpose(B), 1:1, 1:1)
    @test Base.MemoryLayout(VBt) == Base.DenseRowsStridedColumns{ComplexF64}()
end
