...
@testset "adjoint and transpose MemoryLayout" begin
    ...
    VA = view(A, 1:1, 1:1)
    @test Base.MemoryLayout(VA') == Base.RowMajor{Float64}()
    @test Base.MemoryLayout(transpose(VA)) == Base.RowMajor{Float64}()
    VB = view(B, 1:1, 1:1)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(Base.RowMajor{ComplexF64}())
    @test Base.MemoryLayout(transpose(VB)) == Base.RowMajor{ComplexF64}()
    ...
    VAc = view(A', 1:1, 1:1)
    @test Base.MemoryLayout(VAc) == Base.RowMajor{Float64}()
    VAt = view(transpose(A), 1:1, 1:1)
    @test Base.MemoryLayout(VAt) == Base.RowMajor{Float64}()
    VBc = view(B', 1:1, 1:1)
    @test Base.MemoryLayout(VBc) == LinearAlgebra.ConjLayout(Base.RowMajor{ComplexF64}())
    VBt = view(transpose(B), 1:1, 1:1)
    @test Base.MemoryLayout(VBt) == Base.RowMajor{ComplexF64}()
end
