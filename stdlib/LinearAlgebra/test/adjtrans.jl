...
@testset "adjoint and transpose MemoryLayout" begin
    A = [1.0 2; 3 4]
    @test Base.MemoryLayout(A') == LinearAlgebra.DenseRowMajor{Float64}()
    @test Base.MemoryLayout(transpose(A)) == LinearAlgebra.DenseRowMajor{Float64}()
    B = [1.0+im 2; 3 4]
    @test Base.MemoryLayout(B') == LinearAlgebra.ConjLayout(LinearAlgebra.DenseRowMajor{ComplexF64}())
    @test Base.MemoryLayout(transpose(B)) == LinearAlgebra.DenseRowMajor{ComplexF64}()
    VA = view(A, 1:1, 1:1)
    @test Base.MemoryLayout(VA') == LinearAlgebra.DenseRowsStridedColumns{Float64}()
    @test Base.MemoryLayout(transpose(VA)) == LinearAlgebra.DenseRowsStridedColumns{Float64}()
    VB = view(B, 1:1, 1:1)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(LinearAlgebra.DenseRowsStridedColumns{ComplexF64}())
    @test Base.MemoryLayout(transpose(VB)) == LinearAlgebra.DenseRowsStridedColumns{ComplexF64}()
    VA = view(A, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VA') == LinearAlgebra.StridedLayout{Float64}()
    @test Base.MemoryLayout(transpose(VA)) == LinearAlgebra.StridedLayout{Float64}()
    VB = view(B, 1:2:2, 1:2:2)
    @test Base.MemoryLayout(VB') == LinearAlgebra.ConjLayout(LinearAlgebra.StridedLayout{ComplexF64}())
    @test Base.MemoryLayout(transpose(VB)) == LinearAlgebra.StridedLayout{ComplexF64}()
    VA2 = view(A, [1,2], :)
    @test Base.MemoryLayout(VA2') == LinearAlgebra.UnknownLayout{Float64}()
    @test Base.MemoryLayout(transpose(VA2)) == LinearAlgebra.UnknownLayout{Float64}()
    VB2 = view(B, [1,2], :)
    @test Base.MemoryLayout(VB2') == LinearAlgebra.UnknownLayout{ComplexF64}()
    @test Base.MemoryLayout(transpose(VB2)) == LinearAlgebra.UnknownLayout{ComplexF64}()
    VAc = view(A', 1:1, 1:1)
    @test Base.MemoryLayout(VAc) == LinearAlgebra.DenseRowsStridedColumns{Float64}()
    VAt = view(transpose(A), 1:1, 1:1)
    @test Base.MemoryLayout(VAt) == LinearAlgebra.DenseRowsStridedColumns{Float64}()
    VBc = view(B', 1:1, 1:1)
    @test Base.MemoryLayout(VBc) == LinearAlgebra.ConjLayout(LinearAlgebra.DenseRowsStridedColumns{ComplexF64}())
    VBt = view(transpose(B), 1:1, 1:1)
    @test Base.MemoryLayout(VBt) == LinearAlgebra.DenseRowsStridedColumns{ComplexF64}()
end
