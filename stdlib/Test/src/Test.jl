...
export @test, @test_throws, @test_broken, @test_skip,
    @test_stream, @test_output, @test_warn, @test_nowarn,
...
function do_test_throws(result::ExecutionResult, orig_expr, extype)
    if isa(result, Threw)
        ...
        elseif isa(extype, Exception) || !isa(exc, Exception)
            ...
            success = contains_output(exc, extype)
            exc = repr(exc)
...
# Test for output messages (deprecated)

contains_output(output, s::AbstractString) = occursin(s, output)
contains_output(output, s::Regex) = occursin(s, output)
contains_output(output, s::Function) = s(output)
contains_output(output, S::Union{AbstractArray,Tuple}) = all(s -> contains_output(output, s), S)

"""
    @test_stream stream msg expr

Test whether evaluating `expr` results in output on the stream that contains
...
tuple or array, checks that the screen output contains/matches each item in `msg`.
Returns the result of evaluating `expr`.

[@test_output](@ref) and [@test_warn](@ref) provide utility wrappers for this
macro for `stdout` and `stderr`.
"""
macro test_stream(stream, msg, expr)
    stream === :stdout ? redirect = redirect_stdout :
        stream === :stderr ? redirect = redirect_stderr :
        stream === :stdin ? redirect = redirect_stdin :
        redirect = nothing
    quote
        let fname = tempname()
            try
                #XXX why not read from the stream directly?
                ret = open(fname, "w") do f
                    isnothing($redirect) ? $(esc(expr)) :
                        $redirect(f) do
                            $(esc(expr))
                        end
                end
                @test contains_output(read(fname, String), $(esc(msg)))
                ret
            finally
                rm(fname, force=true)
            end
...
"""
    @test_output msg expr

Test whether evaluating `expr` results in [`stdout`](@ref) output that contains
the `msg` string or matches the `msg` regular expression.  If `msg` is
a boolean function, tests whether `msg(output)` returns `true`.  If `msg` is a
tuple or array, checks that the screen output contains/matches each item in `msg`.
Returns the result of evaluating `expr`.
...
julia> hello() = println("Hello Julia.")
hello (generic function with 1 method)

julia> @test_output "Hello Julia." hello()

julia> @test_output(s->s[1:5]=="Hello", hello())

julia> @test_output ["Hello", "Julia"] hello()
```

Note: For testing log statements, it is recommended to use [`@test_logs`](@ref)
instead.
"""
macro test_output(msg, expr)
    :(@test_stream stdout $(esc(msg)) $(esc(expr)))
end
...
julia> mywarning() = println(stderr, "WARNING: something is wrong.")
mywarning (generic function with 1 method)

julia> @test_warn "WARNING: something is wrong." mywarning()

julia> @test_warn(s->s[1:7]=="WARNING", mywarning())
...
macro test_warn(msg, expr)
    :(@test_stream stderr $(esc(msg)) $(esc(expr)))
end
