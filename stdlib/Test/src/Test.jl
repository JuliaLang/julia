...
function do_broken_test_throws(result::ExecutionResult, @nospecialize(orig_expr), extype, pattern=nothing, context=nothing)
    ...
    function format_extype(extype, pattern)
        if pattern !== nothing
            pattern_str = isa(pattern, AbstractString) ? repr(pattern) :
                         isa(pattern, Function) ? "< match function >" :
                         string(pattern)
    ...
    # Check if the test would have passed (i.e., the right exception was thrown)
    success = false
    if isa(result, Threw)
        exc = result.exception

        if pattern !== nothing
            # Three-argument form: check type + pattern
            if isa(extype, Type) && isa(exc, extype)
                exc_msg = sprint(showerror, exc)
    ...
    if success
        ...
        testres = Error(:test_unbroken, orig_expr, format_extype(extype, pattern), nothing, result.source, context_str)
    else
