...
macro test(ex, kws...)
    ...
    result = quote
        if $(length(skip) > 0 && esc(skip[1]))
            record(get_testset(), Broken(:skipped, $ex, __source__))
        else
            if $(length(broken) > 0 && esc(broken[1]))
                do_broken_test($result, $ex, __source__)
            else
                do_test($result, $ex)
            end
        end
...
macro test_broken(ex, kws...)
    ...
    ex = Expr(:inert, ex)
    return :(do_broken_test($result, $ex, __source__))
end
...
macro test_skip(ex, kws...)
    ...
    return :(record(get_testset(), $testres, __source__))
end
...
function do_broken_test(result::ExecutionResult, orig_expr, source::LineNumberNode)
    testres = Broken(:test, orig_expr, source)
