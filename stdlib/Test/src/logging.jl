...
macro test_deprecated(exs...)
    ...
    for (i, e) in enumerate(exs)
        ...
        elseif e isa Expr && e.head === :call
            ...
            expression !== nothing && throw(ArgumentError("""`@test_deprecated` expects at most one expression.
                               Usage: `@test_deprecated [pattern] expr_to_run [broken=cond] [skip=cond]`"""))
            expression = e
        elseif i == 1 && (e isa Regex || (e isa Expr && e.head !== :call))
            # First non-keyword argument that's a Regex or non-call expression is the pattern
            pattern = e
        else
    ...
    # The structure changed due to skip handling, update line locations
    res.args[2].args[2].args[3].args[2].args[2].args[2] = __source__
    res.args[2].args[2].args[3].args[3].args[2].args[2].args[2] = __source__
    res
end
