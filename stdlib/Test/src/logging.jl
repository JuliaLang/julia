...
If you want to test the absence of specific messages such as warnings, you need
to pass no additional arguments to `@test_logs` and set the `min_level` accordingly.
For example, to test the absence of warnings and higher-level loggging output, you
can use something along the following lines.

    using Test, Logging

    foo_info() = begin @info "Some information"; 2 end
    @test_logs min_level=Logging.Warn foo_info() # passes

    foo_warn() = begin @warn "Some warning"; 3 end
    @test_logs min_level=Logging.Warn foo_warn() # fails

If you want to test the absence of warnings (or error messages) in [`stderr`](@ref)
that may not be generated by `@warn`, you can use [`@test_nowarn`](@ref).
"""
macro test_logs(exs...)
