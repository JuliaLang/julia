...
Note: `@test_logs` cannot test for log statements which are handled by a local logger.
In this case, or if you want to test for the presence or absence of messages to
[`stdout`](@ref) or [`stderr`](@ref) which are not generated by a log statement, see
[`@test_output`](@ref), [`@test_warn`](@ref), and [`@test_nowarn`](@ref).
"""
macro test_logs(exs...)
