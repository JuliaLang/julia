...
`Xoshiro` supports "jumping ahead" via [`Random.jump`](@ref), with a number of steps
equal to `2^128` or `2^192`, where one step represents consuming a `UInt64` value.
Note that the period of Xoshiro is `2^256-1`; it follows that it's possible to create
`2^128` (resp. `2^64`) non-overlapping subsequences when jumping by `2^128` (resp.
`2^192`) steps.
...
function jump!(rng::Union{Xoshiro, TaskLocalRNG}; by::Real=NaN)
    isnan(by) && (by = 2.0^128)
    if by == 2.0^128
        jump_128!(rng)
    elseif by == 2.0^192
