...
`trand` is meant as a thread safe wrapper for rand(). If called with no arguments
it relies on `ThreadRNG[]`, a threaded array of `MersenneTwister` objects generated by
