...
seed!(rng::SeedHasher, seeder::AbstractRNG) = seed!(rng, rand(seeder, UInt64, 8))
seed!(rng::SeedHasher, ::Nothing) = seed!(rng, RandomDevice())

function seed!(rng::SeedHasher, seed)
    # typically, no more than 256 bytes will be needed, so use
...
@noinline function rehash!(rng::SeedHasher)
    ...
    # also hash the counter, just for the unlikely case where the hash of rng.bytes
    # is equal to rng.bytes (i.e. rng.bytes is a "fixed point"); with the current setting,
    # if this happens, it means we found a collision (practically impossible)
...
This function is used by the [`SeedHasher`](@ref) RNG in order to produce
random bytes.
...
if `n != m`, then `hash_seed(n, cn) != hash_seed(m, cm)`.
...
seed!(r::MersenneTwister, seed) = initstate!(r, rand(SeedHasher(seed), UInt32, 24), seed)
