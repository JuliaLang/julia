            ...
            for taskid in taskids
                ...
                if length(threadids) == 0
                    ...
                    for threadid in threadids
                        printstyled(io, " Thread $threadid ($(Threads.threadpool(threadid))) "; bold=true, color=Base.info_color())
        ...
        elseif groupby == [:thread, :task]
            ...
            for threadid in threadids
                ...
                if length(taskids) == 0
                    ...
                    nl = length(taskids) > 1 ? "\n" : ""
                    printstyled(io, "Thread $threadid ($(Threads.threadpool(threadid)))$nl"; bold=true, color=Base.info_color())
                    for taskid in taskids
