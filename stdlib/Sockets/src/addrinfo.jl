...
function uv_getaddrinfocb(req::Ptr{Cvoid}, status::Cint, addrinfo::Ptr{Cvoid})
    ...
    if data != C_NULL
        ...
        if status != 0 || addrinfo == C_NULL
            Base.unyielding_schedule(t, _UVError("getaddrinfocb", status))
        else
            ...
            Base.unyielding_schedule(t, addrs)
        end
    else
...
function uv_getnameinfocb(req::Ptr{Cvoid}, status::Cint, hostname::Cstring, service::Cstring)
    ...
    if data != C_NULL
        ...
        uv_req_set_data(req, C_NULL)
        if status != 0
            Base.unyielding_schedule(t, _UVError("getnameinfocb", status))
        else
            Base.unyielding_schedule(t, unsafe_string(hostname))
        end
    else
        # no owner for this req, safe to just free it
        Libc.free(req)
