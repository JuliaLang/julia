...
function should_send_whole_type(s, t::DataType)
    ...
    if isdefined(tn, :mt)
        ...
        isanonfunction = mod === Main && # only Main
            ...
            (!isdefined(mod, name) || t != typeof(Core.getglobal(mod, name))) # XXX: 95% accurate test for this being an inner function
...
function deserialize_module(s::AbstractSerializer)
    ...
    if isa(mkey, Tuple)
        ...
        for i = 2:length(mkey)
            m = Core.getglobal(m, mkey[i])::Module
        end
    else
        ...
        mname = deserialize(s)
        while mname !== ()
            m = Core.getglobal(m, mname)::Module
            mname = deserialize(s)
        end
    end
    return m
