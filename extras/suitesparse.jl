...
function _jl_cholmod_common_finalizer(x::Vector{Ptr{Void}})
    st = ccall((:cholmod_finish, libcholmod), Int32, (Ptr{Void},), x[1])
...
function show(io, cm::CholmodCommon)
    st = ccall((:cholmod_print_common, libcholmod), Int32,
...
function nnz(cso::CholmodSparseOut)
    ccall((:cholmod_nnz, libcholmod), Int32,
...
function SparseMatrixCSC{Tv,Ti}(cso::CholmodSparseOut{Tv,Ti})
    ...
    st = ccall((:jl_cholmod_sparse_copy_out, libsuitesparse_wrapper), Int32,
