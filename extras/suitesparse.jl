...
function _jl_cholmod_common_finalizer(x::Vector{Ptr{Void}})
    st = ccall((:cholmod_finish, libcholmod), Int, (Ptr{Void},), x[1])
...
type CholmodCommon
    ...
    function CholmodCommon()
        ...
        ccall((:jl_cholmod_common, libsuitesparse_wrapper), Void,
              (Ptr{Void},), pt)
        st = ccall((:cholmod_start, libcholmod), Int, (Ptr{Void}, ), pt[1])
...
function show(io, cm::CholmodCommon)
    st = ccall((:cholmod_print_common, libcholmod), Int,
...
function nnz(cso::CholmodSparseOut)
    ccall((:cholmod_nnz, libcholmod), Int,
...
function SparseMatrixCSC{Tv,Ti}(cso::CholmodSparseOut{Tv,Ti})
    ...
    st = ccall((:jl_cholmod_sparse_copy_out, libsuitesparse_wrapper), Int,
