...
("Iterable Collections","Base","unique","unique(itr[, dim])
...
("Associative Collections","Base","get","get(f::Function, collection, key)
...
("Set-Like Collections","Base","Set","Set([itr])
...
("Set-Like Collections","Base","IntSet","IntSet([itr])
...
("Strings","Base","bytestring","bytestring(::Ptr{Uint8}[, length])
...
("Strings","Base","normalize_string","normalize_string(s, normalform::Symbol)
...
("Arrays","Base","bitbroadcast","bitbroadcast(f, As...)
...
("Numerical Integration","Base","quadgk","quadgk(f, a, b, c...; reltol=sqrt(eps), abstol=0, maxevals=10^7, order=7, norm=vecnorm)
...
("Parallel Computing","Base","@parallel","@parallel()
...
("Shared Arrays (Experimental, UNIX-only feature)","Base","SharedArray","SharedArray(T::Type, dims::NTuple; init=false, pids=Int[])
...
("Shared Arrays (Experimental, UNIX-only feature)","Base","procs","procs(S::SharedArray)

   Get the vector of processes that have mapped the shared array

"),

("Shared Arrays (Experimental, UNIX-only feature)","Base","sdata","sdata(S::SharedArray)

   Returns the actual \"Array\" object backing \"S\"

"),

("Shared Arrays (Experimental, UNIX-only feature)","Base","indexpids","indexpids(S::SharedArray)
...
("Linear Algebra","Base","eig","eig(A,[permute=true,][scale=true]) -> D, V
...
("Linear Algebra","Base","eigvecs","eigvecs(A, [eigvals,][permute=true,][scale=true])
...
("Linear Algebra","Base","eigfact","eigfact(A,[permute=true,][scale=true])
...
("Linear Algebra","Base","norm","norm(A[, p])
   ...
   Use \"vecnorm()\" to compute the Frobenius norm.

"),

("Linear Algebra","Base","vecnorm","vecnorm(A[, p])
...
("Linear Algebra","Base","blkdiag","blkdiag(A...)
...
("Linear Algebra","Base","eigs","eigs(A; nev=6, which=\"LM\", tol=0.0, maxiter=1000, sigma=nothing, ritzvec=true, op_part=:real, v0=zeros((0, ))) -> (d[, v], nconv, niter, nmult, resid)

   \"eigs\" computes eigenvalues \"d\" of \"A\" using Lanczos or
   Arnoldi iterations for real symmetric or general nonsymmetric
   matrices respectively. The following keyword arguments are
   supported:
