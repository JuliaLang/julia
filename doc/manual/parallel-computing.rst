...
example, ``ssh`` clusters are also implemented using a :class:`ClusterManager`::
...
where :func:`launch_ssh_workers` is responsible for instantiating new
Julia processes and :func:`manage_ssh_workers` provides a means to manage
those processes, e.g. for sending interrupt signals. New processes can
then be added at runtime using :func:`addprocs`::

    addprocs(5, cman=LocalManager())

which specifies a number of processes to add and a :class:`ClusterManager` to
