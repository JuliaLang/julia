...
The argument type tuple here must be written as ``(Cstring,)``,
...
| ``T*`` [#f-T]_                    |                 |                      | ``Ref{T}``                        |
+-----------------------------------+-----------------+----------------------+-----------------------------------+
| ``char*`` or ``char[]`` [#f-str]_ | ``CHARACTER*N`` |                      | ``Cstring`` if NUL-terminated, or |
|                                   |                 |                      | ``Ptr{UInt8}`` if not             |
+-----------------------------------+-----------------+----------------------+-----------------------------------+
...
- For string arguments (``char*``) the Julia type should be ``Cstring`` (if
  NUL-terminated data is expected) or either ``Ptr{Cchar}`` or ``Ptr{UInt8}``
  otherwise, as described above.  It should *not* be ``ASCIIString``.

- Similarly, for array arguments (``T[]`` or ``T*``), the Julia type should
  again be ``Ptr{T}``, not ``Vector{T}``.
...
- For ``wchar_t*`` arguments, the Julia type should be ``Cwstring`` (if the
  C routine expects a NUL-terminated string) or ``Ptr{Cwchar_t}`` otherwise.
  ...
  the width of ``Cwchar_t``); this conversion will be called
...
.. [#f_auto_conversion]
   ...
   Note that the primary fallback method for ``cconvert`` is::
