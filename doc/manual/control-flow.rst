...
:func:`& <&>` and :func:`| <|>`. These are normal functions, which happen to support
...
For example, the :func:`sqrt` function throws a ``DomainError()`` if applied to a
negative real value:
...
defined only for nonnegative numbers could be written to :func:`throw` a ``DomainError``
...
The :func:`error` function is used to produce an ``ErrorException`` that
...
just finishing normally), :func:`close(f) <close>` will be executed. If
...
function) is designated as a :data:`Task`, it becomes possible to interrupt
it by switching to another :data:`Task`. The original :data:`Task` can later be
...
To consume values, first the producer is wrapped in a :data:`Task`,
...
A :data:`Task` can be used as an iterable object in a ``for`` loop, in which
...
:func:`istaskdone(t) <istaskdone>` queries whether a task has exited.
:func:`istaskstarted(t) <istaskstarted>` queries whether a task has run yet.
...
implement :func:`wait`; for example, given a ``Process`` object, :func:`wait` will
...
In all of these cases, :func:`wait` ultimately operates on a :data:`Condition`
object, which is in charge of queueing and restarting tasks. When a task
calls :func:`wait` on a :data:`Condition`, the task is marked as non-runnable, added
...
A task created explicitly by calling :data:`Task` is initially not known to the
...
waiting for any events. This is done by calling :func:`schedule(task) <schedule>`, or using
the :func:`@schedule` or :func:`@async` macros (see :ref:`man-parallel-computing` for
more details).
