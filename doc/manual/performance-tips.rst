...
A type stable function means that the type of the output
cannot vary depending on the *values* of the inputs. When possible, it helps to
ensure that a function is type stable. Consider the following definition::
...
either of two types. Since Julia can't predict the return type of this function at
...
This behavior is allowed, and may be desirable in
...
values of any type, such as cell arrays (arrays of type ``Array{Any}``).
But, if you're using one of these structures and happen to know the
type of an element, it helps to share this knowledge with the compiler::
...
For the same reason, as mentioned in :ref:`man-parametric-types`
a function that accepts real vectors should be defined as
...
Note that, in contrast, there is no difference between these two signatures::

    function f(x::Real)
    function f{T <: Real}(x::T)

In both cases, the full type of ``x`` is accessible at compile time.

Declare concrete type in fields
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
When working with :ref:`man-composite-types`, it is best to declare
fields with a concrete type
...
Avoid "abstract container type" fields
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The same best practices that apply in the previous section
also work for container types::
...
your code.  Consequently, in addition to the allocation itself,
it's very likely that the code generated for your function is far
from optimal. You can also use the profiler or start Julia with the ``--track-allocation=user`` option as explained in :ref:`man-track-allocation`.


In this particular case, the problem lies in type instability
    julia> function f(n)
               s = zero(Float64)
               for i = 1:n
                   s += i/2
...
such cases, consider using one of the tools below to diagnose problems, or write a
...
.. _man-code-warntype:
Examine your code with :obj:`@code_warntype`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
For instance, use a list comprehension rather than the function map:
    ...
    julia> @time  [x^2 for x in 1:10^7];
...
You can find more information about :obj:`@simd` on
`this Intel website<https://software.intel.com/en-us/articles/vectorization-in-julia>`_
