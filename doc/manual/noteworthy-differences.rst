...
- Macros start with ``@``, and are not as seamlessly integrated into the language, consequently macro usage is not as widespread as in Common Lisp. A form of hygiene for macros is supported by the language (see :ref:`man-macros`). Because of the different syntax, there is no equivalent to ``&body``.
...
- Symbols do not belong to any package, and do not contain any values. ``M.var`` evaluates the symbol ``var`` in the module ``M``.
