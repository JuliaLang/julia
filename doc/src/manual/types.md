...
Inside a `typegroup` block, type names are collected from all `struct` and `abstract type`
definitions, and field types referencing these names are resolved after all types are defined.

All struct kinds are supported inside typegroup blocks:
