...
| `Base.islinearstrided(::Type{<:A})`             |     `Base.is_contiguous(A)`          | Return `true` to declare that the array additionally has evenly spaced elements in column-major order. Implies `Base.isstrided`. |
| `Base.is_contiguous(::Type{<:A})`             |     `false`                            | Return `true` to declare that the array additionally has the same memory layout as an `Array`. Implies `Base.islinearstrided` and provides default `strides` and `Base.elsize` definitions. |
