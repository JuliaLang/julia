...
1. No code reload / cache invalidation is performed after changes are made to the source files themselves,
   (including by [`OldPkg.update`](@ref)), and no cleanup is done after [`OldPkg.rm`](@ref)
...
line flag is passed to [`OldPkg.build`](@ref) to disable automatic precompilation triggering when installing,
