...
that can even optimize across this boundary, but Julia does not yet support this. In the future,
however, it may do so, to achieve even greater performance gains.)
...
`clock` takes no arguments and returns an [`Int32`](@ref). One common mistake is forgetting that a 1-tuple must be
...
C or Fortran function indicates them. And if an error occurs it is propagated to the Julia caller as an exception. This is especially
...
is permitted). Now, suppose that we have a 1-d array `A` of values in Julia that we want to sort
