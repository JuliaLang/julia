...
- There are two different packages named `Priv` that the application uses. It uses a private package, which is a root dependency, and a public one, which is an indirect dependency through `Pub`. These are differentiated by the uuid, and have different deps:
...
**The paths map** of a project environment is also determined by searching through the manifest files that are present in the load path. The path to the folder for a given package, identified by `uuid` and `Name`, is determined by these rules (in order):

1. If the directory contains a Project file with a matching `uuid` and `Name`, then either:
  - It has a `path` entry, which will be used (relative to the Project file directory).
...
2. If the there is a project file and matching manifest, and the manifest contains a stanza matching `uuid`
  ...
  - If it has a `git-tree-sha1` entry, compute a deterministic hash function of `uuid` and `git-tree-sha1`—call it `slug`—and look for a directory named `packages/Name/$slug` in each directory in the Julia `DEPOT_PATH` global array. Use the first such directory that exists.

If any of these result in success, the path to the source code entry point will be either that result,
the relative path from that result plus `src/Name.jl`, or fail.
...
Additionally, a UUID will be assigned to each entry as follows: For a given package found inside the folder `Name`...

1. If `Name/Project.toml` exists and has a `uuid` entry, then `uuid` is that value.
2. If `Name/Project.toml` exists and but does *not* have a top-level UUID entry, `uuid` is a dummy UUID generated by hashing the canonical (real) path to `Name/Project.toml`.
