...
* **Correctness:** Type declarations can be useful if your function only returns correct results for certain argument types.  For example, if we omitted argument types and wrote ` function fib(n) ... end`, then `fib(1.5)` would silently give us the nonsensical answer `1.0`.
