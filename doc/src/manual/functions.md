...
You can declare the types of function arguments by appending `::TypeName`, as usual for [Type Declarations](@ref) in Julia.
...
* **Correctness:** Type declarations can be useful if your function only returns correct results for certain argument types.  For example, if we omitted argument types and wrote `fib(n) = n ≤ 2 ? 1 : fib(n-1) + fib(n-2)`, then `fib(1.5)` would silently give us the nonsensical answer `1`.
* **Clarity:** Type declarations can serve as a form of documentation, indicating what argument types were expected by the implementation.

However, it is a **common mistake to overly restrict the argument types**, which can unnecessarily limit the applicability of the function and prevent it from being re-used in circumstances you did not anticipate.    For example, the `fib(n::Integer)` function above works equally well for `Int` arguments (machine integers) and `BigInt` arbitrary-precision integers (see [BigFloats and BigInts](@ref)), which is especially useful because Fibonacci numbers grow exponentially rapidly and will quickly overflow any fixed-precision type like `Int` (see [Overflow behavior](@ref)).  If we had declared our function as `fib(n::Int)`, however, the application to `BigFloat` would have been prevented for no reason.   In general, yo should use the **most general applicable abstract types** for arguments, and **when in doubt, omit the argument type**.  You can always add argument-type specifications later if they become necessary, and you *don't sacrifice performance* by omitting the type.
