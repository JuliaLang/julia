...
1. Compute the element type `R` of the result matrix as `promote_type(argument_types...)`,
   where `argument_types` is computed from `eltype` applied to each input array.
2. Build the output matrix as `similar(R, dims)`, where `dims` are the desired dimensions of the output array.

For a more specific example, a generic square-matrix multiply might look like:
...
function matmul(a::AbstractMatrix, b::AbstractMatrix)
    ...
    for j in axes(b, 2), i in axes(a, 1)
        ## we declare `ab::R` to make the type of `ab` constant in the loop,
