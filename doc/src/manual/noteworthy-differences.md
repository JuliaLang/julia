...
## Noteworthy differences from Python
  ...
  * In particular, it is idiomatic in Julia to split the text of a large module into multiple files, without introducing a new module (namespace) per file. The files are reassembled inside a single module in a main file via `include`. While the equivalent in Python (using `exec`) is considered harmful (because silent clobbering of prior definitions can lead to subtle bugs), Julia prints a warning message if redefinition occurs. 
