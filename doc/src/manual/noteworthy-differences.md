...
For users coming to Julia from R, these are some noteworthy differences:
  ...
  * Be aware that Julia ignores trailing commas. This can cause confusion among R users when indexing
    into arrays. For example, `df[1,]` in R would return the first row; in Julia, however, the comma
    is ignored, so `df[1,] == df[1]`, and will return the first element. To extract a row, be sure
    to use `:`, as in `df[1,:]`.
