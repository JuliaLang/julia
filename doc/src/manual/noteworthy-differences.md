...
## Noteworthy differences from Python
  ...
  * In Python, the majority of values can be used in logical contexts (e.g, for `if "a":` the following block is executed, for `if "":` not). In Julia, you need explicit conversion to `Bool` (e.g. `if "a"` throws an exception).
