...
taken with respect to the working directory. If `$JULIA_PKGDIR` is not set, then
...
history file. If `$JULIA_HISTORY` is not set, then `Base.find_hist_file()`
defaults to
```
$HOME/.julia_history
```
...
interpreted as an unsigned 64-bit integer and gives, in nanoseconds, the amount
of time after which spinning threads should sleep.
...
package provides a high-level interface with much of the same functionality: see
the section on [Interacting With Julia](@ref).
...
with garbage-collection debugging (that is, if `WITH_GC_DEBUG_ENV` is set to `1`
...
*Note:* This environment variable only has an effect if Julia was compiled with
garbage-collection debugging (that is, if `WITH_GC_DEBUG_ENV` is set to `1` in
...
*Note:* This environment variable only has an effect if Julia was compiled with
JIT profiling support, using either
