...
the configuration file directory. Then Julia searches for a `juliarc.jl` file
at
...
`/bin/julia`, a `DATAROOTDIR` of `../share`, and a `SYSCONFDIR` of `../etc`
will have `JULIA_HOME` set to `/bin`, a source-file search path of
...
[`Base.LOAD_PATH`](@ref). (In Unix-like systems, the path separator is `:`; in
Windows systems, the path separator is `;`.) The `Base.LOAD_PATH` variable is
where [`Base.require`](@ref) and `Base.load_in_path()` look for
code; it defaults to the absolute paths
...
`_pkgroot()` defaults to

```
$HOME/.julia
```

Then the repository location [`Base.Pkg.dir`](@ref) for a given
Julia version is
...
The absolute path `Base.find_hist_file()` of the REPL's history file. If
`$JULIA_HISTORY` is not set, then `Base.find_hist_file()` defaults to
...
[`Base.Pkg.resolve`](@ref) will devote to attempting
...
*   Intel's [VTune™
    Amplifier](https://software.intel.com/en-us/intel-vtune-amplifier-xe)
    (`USE_INTEL_JITEVENTS` set to `1` in the build configuration), or
*   [OProfile](http://oprofile.sourceforge.net/news/) (`USE_OPROFILE_JITEVENTS`
    set to `1` in the build configuration).
