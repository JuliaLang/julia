...
**Choosing the shard count.** The shard count is the larger of two terms. A *memory* term, `weight / weight_per_shard`, splits large modules finely enough that peak working memory stays roughly `concurrency * weight_per_shard` regardless of module size (`weight` is a codegen-cost estimate; see `compute_module_info` in `aotcompile.cpp`); it ignores the core count, so a module shards the same way on every machine. A *core-fill* term then ramps the count up toward the available concurrency, so a module too small to trip the memory term still isn't emitted as a single serial shard that leaves cores idle. Under a jobserver (see below) that "available concurrency" is the free-token count probed when the module is partitioned: near zero mid-precompile, where the machine is already saturated and the ramp stays coarse, and near the full machine at the tail, where one remaining package fans out across the idle cores. The exact rule and behavior by size are tabulated below.
...
where `F` is the free jobserver-token count when the module is partitioned — roughly 1 mid-precompile (machine saturated by other packages) and roughly the core count at the tail. The `weight / 125,000` term is the `weight_per_shard / 4` minimum shard size, so a shard runs ~500,000 weight when cores are scarce and ~125,000 when they are free. For a machine with `C` cores:
