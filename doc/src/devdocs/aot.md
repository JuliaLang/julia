...
**Choosing the shard count.** The shard count is the larger of two terms. A *memory* term, `weight / weight_per_shard`, splits large modules finely enough that peak working memory stays roughly `concurrency * weight_per_shard` regardless of module size (`weight` is a codegen-cost estimate; see `compute_module_info` in `aotcompile.cpp`). A *core-fill* term, `min(C, weight / min_shard)`, then ramps the count up toward `C`, the concurrency ceiling chosen below, so a module too small to trip the memory term still isn't emitted as a single serial shard that leaves cores idle. Whichever term wins, the result is finally capped at one shard per 100 module globals: a module with little content has nothing to gain from splitting. The count is **static**: it scales with the concurrency ceiling but does not depend on how many cores are momentarily free. Under a jobserver (see below) the *elastic pool* handles that at runtime — a worker that can only get a few tokens still produces the full shard count but compiles a few shards at a time, draining the rest in waves, while a worker alone at the precompile tail runs them all at once. The exact rule and behavior by size are tabulated below.

**Choosing the thread count.** The thread count aims for all effective cores, and is finally capped to the shard count — there is no point having more threads than shards, and this is also how the shard count's global-count cap reaches concurrency. How the "all cores" aim is bounded depends on who owns the machine. For a *single-process build* — the system image, or a lone `PackageCompiler` run — the process owns the whole machine, so concurrency is bounded by a *memory budget* rather than the core count alone: peak codegen memory grows with the number of concurrent shards, so the thread count is capped at roughly `codegen_memory_budget / per_shard_codegen_memory`. That budget is sized from the machine's total RAM (capped by any cgroup or container limit), leaving headroom for the build process, the GC, and the OS. For a *parallel package precompile*, concurrency is instead coordinated across all worker processes by a jobserver, described next.

**Summary by module size.** The full rule, with the default 500,000 weight target, is

```
shards = min(globals / 100,  max(weight / 500,000,  min(C, weight / 125,000)))
```

where `C` is the **concurrency ceiling** from the previous paragraph — not the raw core count. On a jobserver `C` is the effective core count, but for a single-process build it is the memory-bounded thread count, so the same module splits into fewer shards on a machine with many cores but little RAM. The `weight / 125,000` term is the `weight_per_shard / 4` minimum shard size, so core-fill splits a module small relative to `C` into ~125,000-weight shards, while a module large enough for the memory term to win converges on ~500,000 per shard. The layout is fixed up front; the jobserver throttles *concurrency* at runtime, not the shard count. Ignoring the global-count cap:

| Module `text` weight | Shards |
| :--- | :--- |
| below 125,000 | 1 (single shard, no round-trip) |
| 125,000 – 500,000 | `weight / 125,000` (1–3) |
| 500,000 – `C`×125,000 | `weight / 125,000` (~one per 125k) |
| `C`×125,000 – `C`×500,000 | ≈ `C` (one per concurrent thread) |
| above `C`×500,000 | `weight / 500,000` (more than `C`; run in waves) |

For example, Makie's `text` module (`weight`≈3.1M) on a 32-core precompile worker splits into `3.1M / 125,000` ≈ 24 shards. Compiled alone at the tail of a precompile it runs all 24 at once; while the machine is busy with other packages the jobserver lets it run only a few at a time and drains the rest in waves. The same module in a single-process build on a 32-core machine with only 16 GiB of RAM has `C` ≈ 10 rather than 32, so it splits into ~10 shards and runs them all at once.

During parallel package precompilation, the orchestrating process shares a single CPU-thread budget across all worker subprocesses via a named-semaphore jobserver (whose name is passed to workers through the `JULIA_PRECOMPILE_JOBSERVER` environment variable, and whose size defaults to one more than the number of effective CPU threads but is overridable via [`JULIA_PRECOMPILE_THREADS`](@ref JULIA_PRECOMPILE_THREADS)). Each worker holds one baseline token while it is CPU-active, and during its imaging phase sizes its thread pool elastically against the shared budget: it starts with the baseline thread plus whatever extra tokens are immediately available, keeps polling for tokens freed by sibling workers while it still has uncompiled shards, and returns each token to the pool as soon as the thread holding it runs out of work. Because a worker's main thread sleeps while its imaging threads run, that baseline token doubles as the imaging phase's first codegen thread rather than being counted twice. A lone worker thus expands to all available cores, workers that start while the machine is busy grow into cores as they free up, and concurrent workers share the pool without oversubscribing the machine. An explicit `JULIA_IMAGE_THREADS` setting takes precedence over the jobserver, unless `JULIA_PRECOMPILE_THREADS` is also set, in which case the jobserver takes precedence instead and `JULIA_IMAGE_THREADS` is ignored.

Setting [`JULIA_IMAGE_THREADS`](@ref JULIA_IMAGE_THREADS) explicitly fixes *both* quantities: the shard count is set to the requested thread count rather than computed, so `JULIA_IMAGE_THREADS=1` emits the module whole as a single shard with no partitioning or serialize/deserialize round-trip.

The defaults behind the shard-sizing terms — the per-shard weight target (`JULIA_IMAGE_PARTITION_WEIGHT`), the core-fill minimum shard size (`JULIA_IMAGE_MIN_SHARD_WEIGHT`), and the per-weight codegen-memory estimate (`JULIA_IMAGE_CODEGEN_BYTES_PER_WEIGHT`) — are intentionally-undocumented environment variables read in `aotcompile.cpp`. Setting [`JULIA_IMAGE_TIMINGS`](@ref JULIA_IMAGE_TIMINGS) reports the chosen shard count with the two terms that produced it, the per-stage cost of each shard, and the shard occupancy and straggler ratio over the codegen window.
