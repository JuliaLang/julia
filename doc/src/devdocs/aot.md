...
**Summary by module size.** With default settings — a per-shard weight target of 500,000 and, under a jobserver, a core-fill cap of 4 — a parallel package precompile splits a module's `text` as follows. (`weight` is the codegen-cost estimate from `compute_module_info`, and "cores" is the worker's effective concurrency.)
...
| Medium | 500,000 – 2,000,000 | 4 (the core-fill cap) | Held at the cap: the worker shares a machine already saturated by other packages, so more shards would only add per-shard overhead without buying parallelism. |
| Large | 2,000,000 – roughly (cores × 500,000) | `weight / 500,000` (about one per core) | The memory term takes over from the cap; the shards roughly fill the available cores and generally compile together once the worker holds enough jobserver tokens. |
| Extra-large | above roughly (cores × 500,000) | `weight / 500,000` (more than the thread count) | Shards outnumber the concurrent threads and drain from the queue in waves, holding peak working memory near `concurrency × 500,000` however large the module grows. |

A single-process build (the system image or a lone `PackageCompiler` run) has no jobserver cap, so the core-fill term ramps toward the memory-bounded core count instead of stopping at 4: medium and large modules are split into correspondingly more shards, up to what the memory budget allows. In every case the thread count is additionally capped at one per 100 module globals, so a module with few globals shards and parallelizes less than its weight alone would suggest.
...
The defaults behind the shard-sizing terms — the per-shard weight target, the per-weight codegen-memory estimate, and the core-fill cap — are governed by intentionally-undocumented `JULIA_IMAGE_*` environment variables read in `aotcompile.cpp`; the per-stage cost of each shard can be inspected with [`JULIA_IMAGE_TIMINGS`](@ref JULIA_IMAGE_TIMINGS).
