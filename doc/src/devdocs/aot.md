...
```
shards = max(weight / 500,000,  min(C, weight / 125,000))
```

where `C` is the machine's effective core count. The `weight / 125,000` term is the `weight_per_shard / 4` minimum shard size, so a shard carries ~500,000 weight on a module small relative to the core count and ~125,000 on a larger one. The layout is fixed up front; the jobserver throttles *concurrency* at runtime, not the shard count. For a machine with `C` cores:
...
| `C`×125,000 – `C`×500,000 | ≈ `C` (one per core) |
| above `C`×500,000 | `weight / 500,000` (more than cores; run in waves) |

For example, Makie's `text` module (`weight`≈3.1M) on a 32-core machine splits into `3.1M / 125,000` ≈ 24 shards. Compiled alone at the tail of a precompile it runs all 24 at once; while the machine is busy with other packages the jobserver lets it run only a few at a time and drains the rest in waves. A single-process build (the system image or a lone `PackageCompiler` run) owns the whole machine and runs the full width. In every case the thread count is additionally capped at one per 100 module globals, so a module with few globals shards and parallelizes less than its weight alone would suggest.
