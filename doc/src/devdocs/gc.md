...
Generational collection is implemented through sticky bits: objects are only pushed to the mark-stack, and therefore traced, if their mark-bits have not been set. Every object that survives a collection is promoted to the oldest generation by the sweep that follows the mark phase: a quick sweep sets the age bit of every marked object and leaves its mark-bit set, so these objects aren't traced during a subsequent mark phase. The young generation therefore consists exactly of the objects allocated since the previous collection. A full sweep, however, resets the mark-bits of all objects, so all of them are traced in a subsequent collection.
