...
types exist in lowered form:
  ...
  * `SlotNumber`
    ...
    they are converted to temporary `TypedSlot` object. This object has an additional `typ`
    field as well as the `id` field. Note that `TypedSlot` only appears in an unoptimized
    lowered form that is scheduled for optimization, and it never appears elsewhere.
