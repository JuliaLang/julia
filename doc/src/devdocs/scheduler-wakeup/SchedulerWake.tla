...
(* wakes one worker per insert, gated on searcher accounting.                *)
(*                                                                           *)
...
vars == <<st, pc, spin, nspin, nrun, queue, inject, prewake, handoffs>>
...
PreaccountWake ==
    ...
    /\ UNCHANGED <<st, pc, spin, nrun, queue, inject, handoffs>>
...
CommitWake ==
    \E t \in prewake :
        ...
        /\ spin' = [spin EXCEPT ![t] = TRUE]
        /\ prewake' = {}
        /\ UNCHANGED <<nspin, queue, inject, handoffs>>
...
RollbackWake ==
    \E t \in prewake :
        ...
        /\ UNCHANGED <<st, pc, spin, nrun, queue, inject>>

RollbackHandoff ==
    \E p \in handoffs :
        /\ handoffs' = handoffs \ {p}
        /\ Wakeup(p, queue[p])
        /\ UNCHANGED <<queue, inject, prewake>>
