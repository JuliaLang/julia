...
For example, `subtype` is sensitive to the number arguments, but `type_morespecific` may not be.
...
which allows you to test whether argument tuple type `a` is more specific than tuple type `b`.
