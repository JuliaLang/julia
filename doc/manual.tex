...
Julia has all the standard Matlab-derived control flow constructs, plus a few additional constructs borrowed from other languages:
\begin{itemize}
\item Conditional evaluation: \verb|if|, \verb|else|, \verb|ifelse|, \verb|&&|, \texttt{||}, \verb|?:|.
\item Repeated evaluation: \verb|while|, \verb|for|.
\item Exceptions: \verb|try|, \verb|catch|, \verb|throw|, \verb|error|.
\end{itemize}
In addition to these constructs, Julia has symmetric continuations, a powerful non-local control flow construct similar to continuations.
In fact, both exception handling and cooperative multitasking are implemented using coroutines.
However, everyday Julia programming requires no direct usage of this powerful feature.

\subsection{Conditional Evaluation}\sec{conditional-evaluation}
\subsection{Repeated Evaluation}\sec{repeated-evaluation}
\subsection{Exceptions}\sec{exceptions}
...
probably clear that two functions can both have arguments called \texttt{x}
without the two \texttt{x}s referring to the same thing. There are
...
constructs are: function bodies (as introduced by any of the syntaxes for
functions), for loops, while loops, try blocks, catch blocks, let blocks,
and type declarations.

Next, there are constructs that introduce new variables into scopes. When
a variable is introduced into a scope, it is also inherited by all inner
scopes unless one of the inner scopes explicitly overrides it. These
...
\item A declaration \verb|global x| makes \texttt{x} in the current scope
and inner scopes refer to the global variable of that name.
\item An assignment \verb|x = y| introduces a new local variable \texttt{x}
only if \texttt{x} is neither declared global nor explicitly introduced as local
by any enclosing scope.
\end{itemize}

In the following example, there is only one \texttt{x} assigned both inside
and outside a loop:
\begin{verbatim}
  function foo(n)
      x = 0
...
In the following example, the loop has a separate \texttt{x} and the function
always returns zero:
\begin{verbatim}
  function foo(n)
      x = 0
...
In the following example, an \texttt{x} exists only inside the loop, and
the function encounters an undefined variable error on its last line
(unless there is a global variable \texttt{x}):
\begin{verbatim}
  function foo(n)
      for i = 1:n
          x = i
...
A variable that is not assigned to and not introduced defaults to global,
so this function would actually return the value of the global \texttt{x}
if there is such a variable.

Observe, however, that the only way to assign to a global variable inside
another scope is to explicitly declare the variable as global, since
otherwise the assignment would introduce a new local. This rule works out
well in practice, since the vast majority of variables assigned inside
functions are intended to be local variables.
...
As you can see, in the first case \texttt{y} only exists inside the loop.
...
this declaration would be necessary to modify a global variable.
