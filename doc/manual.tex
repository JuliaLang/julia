...
They can also be created anonymously:
\begin{verbatim}
  julia> x -> x^2 + 2x - 1
  #<closure>
\end{verbatim}
...
That's all well and good if a named function effecting precisely the transform one wants already exists, to pass as the first argument to \verb|map|.
Often, however, a named function doing exactly what one wants does not exist, and in such cases, the anonymous function construct allows easy creation of a single-use function:
\begin{verbatim}
  julia> map(x -> x^2 + 2x - 1, [1.2,3.5,1.7])
  {2.8399999999999999,18.25,5.2899999999999991}
\end{verbatim}
