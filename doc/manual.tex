...
\begin{verbatim}
  ...
  13.0
\end{verbatim}
Numeric literal coefficients conflict with two numeric literal syntaxes:
...
\begin{verbatim}
  ...
  +(Tensor{T,N},Number)
\end{verbatim}
Multiple dispatch together with the flexible parametric type system, which allows us to express which values methods may be applied to, give Julia its ability to abstractly express high-level algorithms decoupled from implementation details, yet generate efficient specialized code at run time.
...
\begin{verbatim}
  ...
  10.0
\end{verbatim}
% To suppress the warning, the intersection method must be defined first.
...
\end{verbatim}
In this rather contrived example, the \verb|echo| program prints the string \verb|"hello"| and that output is captured into the variable \verb|$greeting|.
...
if the file name the variable \verb|$file| contains characters that are specially interpreted by the shell, they will be.
...
There are two approaches one can take to address this: escape all the shell metacharacters in the the string \verb|$file| before interpolating it, or avoid using the shell by calling a lower level function like \verb|exec|.
...
The more paranoid programmers will wrap arguments in single quotes within the backticks, writing \verb|`sort -n '$file'`|.
However, this is ugly, annoying, and doesn't cover all the cases.

Julia takes a different approach:
...
We retain the historical backtick notation, however in Julia it doesn't run a command, but rather creates a \verb|Cmd| object:
\begin{verbatim}
  ...
  Cmd(exec=("echo",("hello",)),
      pipes=HashTable(),
      pipeline=Set([`echo hello`]),
      pid=0,
      status=ProcessNotRun(),)
\end{verbatim}
The \verb|Cmd| object is a structure storing what command to run as well as metadata about the resulting process and related processes.
For convenience, however, it is shown in the friendlier input format.
The most obvious thing to do with a \verb|Cmd| object is to run it:
\begin{verbatim}
  julia> run(`echo hello`)
  hello
  true
\end{verbatim}
The \verb|hello| part above is the output of the echo command, while the \verb|true| part is the value retuned by the \verb|run| function, indicating that all the subprocesses spawned by \verb|run| succeeded.
Here we see one advantage of spawning processes directly:
the program has direct access to the return status of each spawned child program, thereby avoiding the overhead of starting a shell and providing more direct control and feedback.
...
The Julia backtick syntax understands both shell argument quoting \emph{and} how arguments were interpolated.
...
When printing the \verb|Cmd| object, it quotes the command so that if the output is evaluated, it would create and equivalent command.
...
\end{verbatim}
If a collection is interpolated as part of a larger command argument, it emulates the \verb|{a,b,c}| shell syntax:
\begin{verbatim}
...
\end{verbatim}
If multiple collections are interpolated into a single argument, it produces the cartesian product of their concatenations, again emulating the shell's \verb|{a,b,c}| syntax:
\begin{verbatim}
  julia> exts = ["aux","log"]
  ["aux","log"]
...
Especially when call other programming languages, it may become necessary to use spaces, escapes or quotes.
...
The \verb|$a| is interpolated, however, because the single quote are closed and opened again;
...
This may seem quite complex, but it is carefully designed so that you can basically just cut and paste commands from the shell and expect backticks to do the right thing.

Unlike the shell, other than escapes, quotes, and spaces, no other characters are treated specially.
A pipe is always just a pipe:
\begin{verbatim}
  julia> run(`echo | sort`)
  | sort
  true
\end{verbatim}
To do complex shell-like programming, you use Julia's operators instead:
\begin{verbatim}
...
\begin{verbatim}
  julia> run(`echo world` & `echo hello`)
  world
  hello
  true
\end{verbatim}
You can even pipe the output from these programs running in parallel to another program simply as well:
\begin{verbatim}
  ...
  true
\end{verbatim}
Now we're just going to showboat for a bit (apologies for excessive Perl):
\begin{verbatim}
  ...
  julia> dup = `perl -pe '$|=1; $_=".$_"; warn $_; sleep 1'`
  `perl -pe '$|=1; $_=".$_"; warn $_; sleep 1'`
  ...
  .....0
\end{verbatim}
The last one never terminates because the \verb|dup| process consumes and duplicates its own output forever.
