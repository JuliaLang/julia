...
In UTF-8, for example, all of the ASCII characters are one byte, exactly identical to their traditional ASCII encoding; all other characters, however, take between two and four bytes to represent.
...
Worse still, some bytes are not even the beginning of a character at all---they're right in the middle of a character, i.e. what is known as a continuation character.
Fortunately, however, UTF-8 is carefully designed so that you can immediately tell which category the byte at an index falls into.
...
If you provide a string abstraction that makes strings behave like functions from indices to Unicode characters, then you have to chose one of the following:
\begin{enumerate}
...
This allows fast random access, and reduces the memory inflation of the UTF-32 approach, but still requires transcoding almost every string on input and output.
...
\item Represent strings by some clever, but relatively complex data structure that allows strings to remain encoded, but somehow makes indexing the $k$th character look like it's faster then $\O(n)$, when amortized over many operations.
...
For that matter, option 2 is not a solution, per se, but rather a pragmatic compromise that gets you 90\% of the way (unless you happen to want to process hieroglyphs).
Frankly, none of these solutions is good.

\subsection{Strings As Partial Functions on Indices}

Our approach is to change the abstraction of what a string is instead of choosing any of the above.
Rather than conceptualizing a string as a function from indices to characters, we're conceptualize a string as a \emph{partial function} from indices to characters.
What does this mean?
It means that for some indices no character value is returned:
...
Why does this modified abstraction help?
