...
In purer object-oriented languages, such as Smalltalk or Ruby, every value is an object and need not be of a record type, but functions are still bundled with those object and called methods.

In Julia, since record types are the most common form of concrete type, they are simply introduced with the \verb|type| keyword, and include a list of fields with optional type annotations:
% , followed optionally by a series of definitions of constructor functions for the new type:
\begin{verbatim}
  type Foo
    bar::Int32
    baz::Float64
...
\end{verbatim}
The new type, \verb|Foo|, is a first-class Julia object, and in addition to being a type, it behaves like a function\,---\,one that constructs new \verb|Foo| objects from their individual pieces:
\begin{verbatim}
  julia> foo = Foo(2,1.5)
  Foo(2,1.5)
  ...
  1.5
\end{verbatim}
As you can see here, the fields of a \verb|Foo| object like \verb|foo| are accessed using the traditional dot (``\verb|.|'') notation.
...
\end{verbatim}
The language tries to convert the values to the appropriate types if it can.
Julia knows how to convert many values to other types, and promote different types to a common super type (see \Section{conversion-and-promotion}).
Floating point values are converted to integers by truncation so the value \verb|ok| create has \verb|ok.bar == 1|.
...
The declaration block of a new record type can also include explicitly declared constructor methods for the type.
...
\end{verbatim}
Inside of a \verb|type| block, the local variable \verb|new| is bound to the raw constructor function for the type being declared:
...
When constructor methods \emph{are} provided, the \verb|new| function is not exposed so that the type declaration controls all the ways to make a new object of the declared type.

With this new definition, we can try constructing some \verb|Foo| objects again:
\begin{verbatim}
  julia> Foo(1,2.5)
  Foo(1,2.5)
...
\end{verbatim}
The explicit constructor for \verb|Foo| objects now always calls \verb|round| on its first argument:
in the first call to the \verb|Foo| constructor, 1 is left alone by the \verb|round| function, while in the second, 1.5 is rounded to 2.0, which is in turn converted to the \verb|Int32| value, 2, and assigned into the \verb|bar| slot of a new \verb|Foo| object.
...
In the last usage, the second argument cannot be converted from string to floating point, just as before.
...
This is of great utility in defining specific behaviors for type constructors:
when used as a function to construct \verb|Foo| objects, \verb|Foo| is a function like any other Julia function, and can have as many different methods defined on it as are necessary to specify the desired behavior.
We will cover examples of using multiple methods to construct new objects of record type in that section.

One final note:
record types can have abstract supertypes.
As before, these are declared when the type is declared, using the \verb|<:| operator immediately after the name of the new type:
\begin{verbatim}
  abstract SuperFoo
...
\end{verbatim}
Here, \verb|SuperFoo| is a subtype of \verb|Any| and \verb|Foo| is a subtype of \verb|SuperFoo|.
This is of little use if \verb|Foo| is the only concrete subtype of \verb|SuperFoo|, since there's only one choice of implementation for the \verb|SuperFoo| abstract type.
However, we also could define an alternate implementation of the \verb|SuperFoo| type:
\begin{verbatim}
  type AltFoo <: SuperFoo
    shazzam::String
  end
\end{verbatim}
This new type, \verb|AltFoo|, has a completely different implememtation from \verb|Foo|, but is also a type of \verb|SuperFoo|, and either \verb|Foo| or \verb|AltFoo| objects can be used anywhere a \verb|SuperFoo| type of object is needed.
