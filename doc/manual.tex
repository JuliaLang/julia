...
the comparisons are performed in whatever type the arguments are promoted to, which is generally the smallest type by which they can all be faithfully represented.
...
There are a few notable high-level features about Julia's approach to strings:
\begin{enumerate}
...
This for allows efficient indexing into strings by the byte index of an encoded representation rather than by character index, which cannot be implemented efficiently and simply in most encodings.
...
They are not all yet assigned intelligible meanings, nor are they necessarily interpretable by applications, but all such values are considered to be valid Unicode.
...
\begin{verbatim}
  ...
  '.'
\end{verbatim}
Like all other indexing in Julia, it is 1-based, not 0-based.
Using an index less than 1 or more than \verb|end| will raise an error:
\begin{verbatim}
  julia> str[0]
...
\begin{verbatim}
  ...
  "lo, wo"
\end{verbatim}
Note the distinction between \verb|str[6]| and \verb|str[6:6]|:
\begin{verbatim}
  julia> str[6]
  ','
  ...
  SubString
\end{verbatim}
The former is a single character value of type \verb|Char|, while the latter is a string value of type \verb|SubString| (we'll get into the nitty-gritty details later).
...
Because there are many different ways a sequence of characters can be represented, Julia has many different string types, each with a different underlying implementation.
...
Not having a single canonical string implementation provides significant flexibility.
For example, Julia performs string concatenation using a simple but effective ropes implementation:
\begin{verbatim}
