...
In fact, the primitive bits types are all defined in Julia itself:
\begin{verbatim}
  bitstype 8 Bool
...
Accordingly, boolean values, although they really only need a single bit, cannot be declared to be any smaller then eight bits.
...
Abstract types allow us to easily talk about any type that is an integer, without restricting an algorithm to any specific integer type.
...
there are other subtypes of both \verb|Number| and \verb|Real|, which will be introduced in later sections.
