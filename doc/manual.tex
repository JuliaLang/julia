...
\subsection{Arithmetic and Bitwise Operators}
...
Almost every Julia operator also comes in an update form.
The update form of \verb|+| is the \verb|+=| operator:
\begin{verbatim}
  julia> x = 1
  1
...
\begin{itemize}
  \item \verb|div(a,b)| --- integral division, rounded towards zero.
  \item \verb|rem(a,b)| --- remainder, satisfies \verb|a == b*div(a,b) + rem(a,b)|, implying that sign matches \verb|a|.
  \item \verb|fld(a,b)| --- integral division, rounded towards $-\infty$.
  \item \verb|mod(a,b)| --- modulus, satisfies \verb|a == b*fld(a,b) + mod(a,b)|, implying that sign matches \verb|b|.
  \item \verb|a % b| --- equivalent to \verb|mod(a,b)|.
  \item \verb|gcd(a,b...)| --- greatest common divisor of \verb|a|, \verb|b|... with sign matching \verb|a|.
  \item \verb|lcm(a,b...)| --- least common multiple of \verb|a|, \verb|b|... with sign matching \verb|a|.
  \item \verb|sign(x)| --- indicate the sign of \verb|x| using $-1$, $0$, $+1$.
  \item \verb|signbit(x)| --- indicate the sign bit of \verb|x| using $-1$, $+1$.\footnote{For IEEE floating point values, this has precise specified semantics.
  ...
  \item \verb|copysign(x,y)| --- a value with the magnitude of \verb|a| and the sign of \verb|b|.
  \item \verb|sqrt(x)| --- the square root of \verb|x|.
  \item \verb|exp(x)| --- the natural exponential function $e^x$ at \verb|x|.
  \item \verb|x^y| --- \verb|x| raised to the exponent \verb|y|.
...
Note that no space may come between a numeric literal coefficient and the identifier or parenthesized expression that it multiplies.
...
Here are the situations where such syntactic conflicts arise:
\begin{itemize}
...
\begin{verbatim}
  ...
  2.6925824035672519
\end{verbatim}
The full gamut of mathematical functions are defined for complex numbers as well:
\begin{verbatim}
  julia> sqrt(im)
...
\begin{verbatim}
  ...
  1//3
\end{verbatim}
This reduced form of a ratio of integers is unique, so equality of rational values can be checked by testing for equality of the numerator and denominator.
...
\begin{verbatim}
  ...
  3
\end{verbatim}
This is not usually necessary, however, since the standard arithmetic and comparison operations are defined for rational values:
\begin{verbatim}
  julia> 2//3 == 6//9
  true
...
\begin{verbatim}
  ...
  0.75
\end{verbatim}
All such conversions obey the following identity for all integral numerator and denominator values:
\begin{verbatim}
...
\begin{verbatim}
  ...
  2
\end{verbatim}
Like condition expressions used in \verb|if|, \verb|elseif| or ternary operator, the operands of \verb|&&| or \texttt{||} must be boolean values (\verb|true| or \verb|false|).
...
As in C and most other imperative or functional languages, the \verb|return| keyword causes the function to return from any point, optionally providing an expression whose value is returned: % TODO: make return value actually optional.
\begin{verbatim}
  function g(x,y)
    return x * y
    x + y
...
In Julia, most operators are just functions with support for special syntax.\footnote{The exceptions are operators with special evaluation semantics like \texttt{\&\&} and \texttt{||} which cause short-circuit evaluation.
These operators cannot be defined as normal functions because their operands are not evaluated as normal function arguments.}
...
Since operators like \verb|+| are just Julia functions, you can add new methods for them.
To add a new method to the \verb|+| operator, you define a new method using the name of the operator where the name of the function goes.
The addition method for \verb|Complex| numbers is defined as:
\begin{verbatim}
