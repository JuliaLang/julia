...
In Julia, most operators are just functions with support for special syntax.\footnote{The exceptions are operators with special semantics like \texttt{\&\&} and \texttt{||} which cause short-circuit evaluation.}
As such, they have an alternate purely ``functional'' form:
\begin{verbatim}
  julia> 1 + 2 + 3
  6
...
\end{verbatim}
Other than special syntax support, operators are no different than other functions.
When using an operator as a function, to be safe you can always wrap it in parentheses like \verb|(+)| above, but that is only necessary for those operators that have a unary form: \verb|+|, \verb|-|, \verb|~|, and \verb|!|.
