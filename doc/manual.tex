...
Julia maintains the ease and expressiveness of Matlab for high-level numerical computing but transcends these general programming limitations.
...
finite numbers are compared bitwise, positive infinite values are equal to each other, negative infinite values are equal to each other, and \verb|NaN| values are unequal to everything including each other.
For comparisons where \verb|NaN != NaN| is undesirable, the function \verb|isequal| is also provided, which considers \verb|NaN| to be equal to \verb|NaN|:
\begin{verbatim}
