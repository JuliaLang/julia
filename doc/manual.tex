...
Julia supports the standard Matlab-derived control flow constructs, plus a few additional constructs borrowed from other programming languages:
\begin{itemize}
...
Here is the general anatomy of the \verb|if|-\verb|esleif|-\verb|else| conditional evaluation syntax:
\begin{verbatim}
  if x < y
...
\end{verbatim}
The syntax and semantics are identical to Matlab, and are precisely what you'd expect:
\begin{verbatim}
  julia> x = 1; y = 2;
  ...
  x is greater than y
\end{verbatim}
The \verb|elseif| and \verb|else| blocks are optional, and as many \verb|elsif| blocks as desired can be used.
...
% The essential conditional evaluation keyword is \verb|if|, used with the same syntax as Matlab:
% \begin{verbatim}
%   julia> if true
...
%          end
% \end{verbatim}
% The first time the expression is evaluated, it prints a line, while the second time it does not.
% In these cases, the condition expressions are the literal booleans \verb|true| and \verb|false|, but in general, since this is not actually conditional in practice, expressions which evaluate to \verb|true| or \verb|false| are used instead:
% \begin{verbatim}
%   julia> if x < y
%            println("$x is less than $y")
%          end
...
%          end
% \end{verbatim}
% Again, in the first case a line is printed\,---\,``x is less than y''\,---\,in the second case, it is not.
% Typically, one wants to 

% \paragraph{Short-Circuit Evaluation}\sec{short-circuit-evaluation}

% \paragraph{Repeated Evaluation}\sec{repeated-evaluation}

% \paragraph{Exceptions}\sec{exceptions}

% \paragraph{Coroutines}\sec{coroutines}
