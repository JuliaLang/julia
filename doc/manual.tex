...
static type systems, where every value must have a fully specified type, computable before the execution of a program, and dynamic type systems, where nothing is known about the types of values flowing through a piece of code until run-time, when the values themselves are available.
Object orientation and generic programming inject some flexibility into the world of statically type languages by allowing code to be written without the exact type of values being known at compile time.
...
only by explicitly checking types, or because certain types don't support certain operations at run-time, are the types of any values ever restricted.
...
The upshot of the balance struck between dynamic and static typing is that one can write many useful Julia programs without ever explicitly mentioning a type.
When additional power is needed, however, it is easy to gradually introduce types into previously ``untyped'' code.
Doing so will typically increase both the performance and robustness of these systems.

To describe Julia's type system in the lingo of types, it is dynamic, nominative, parametric and dependent.
...
the only type of a value is its actual type.
Talking about the static type of a value in Julia is meaningless.
...
In particular, all types are subtypes of the abstract default type, \verb|Any|.%
...
Type parameters may be completely omitted when they need not be explicitly referenced.
...
In formal type system literature, \verb|Any| is commonly called Top.
