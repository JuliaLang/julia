...
\end{itemize}
Although there are many implementations of the \verb|String| interface, we do not delve into the details of these implementations here:
...
To reduce the need for these explicit calls to \verb|strcat|, Julia allows interpolation into string literals using \verb|$|, as in Perl:
\begin{verbatim}
  julia> "$greet, $who.\n"
  "Hello, world.\n"
\end{verbatim}
...
\begin{verbatim}
  bar(a,b,x...) = (a,b,x)
\end{verbatim}
The variables \verb|a| and \verb|b| are bound to the first two argument values as usual, and the variable \verb|x| is bound to an iterable collection of the zero or more values passed to \verb|bar|:
\begin{verbatim}
  julia> bar(1,2)
  (1,2,())
  ...
  (1,2,(3,4,5,6))
\end{verbatim}
In these cases, \verb|x| is bound to a tuple of the trailing values passed to \verb|bar|.

On the other hand, it is often handy to ``splice'' the values contained in an iterable collection into a call to a function.
To do this, one also uses ``\verb|...|'' placed after and argument in a function call:
\begin{verbatim}
  julia> x = (3,4)
  (3,4)
...
\begin{verbatim}
  ...
  5050
\end{verbatim}
Note that in this last example, the range object, \verb|1:100|, is never materialized in memory as a vector, rather it is a computed vector-like object, and the most general definition of the \verb|+| operator iterates over its collection of arguments, summing them into an accumulator like this:
\begin{verbatim}
  function +(a, b, c, xs...)
      accum = a + b + c
      for x = xs
...
\end{verbatim}
Therefore, the \verb|+(x...)| call is actually quite efficient in terms of both memory usage and speed.
...
None of the examples given here provide any type annotations on their arguments, meaning that they are applicable to any argument types.
...
Julia has a nominative, parametric, dependent type system:
...
Parametric typing allows one to concisely an elegantly define and talk about a type like \verb|List|, without caring or specifying what type of objects the list holds\,---\,for many list algorithms, the type of values is unimportant.
In situations where the type of objects contained in a list is of importance, one can simply write \verb|List{Int32}|, for example, to specify that a list contains only 32-bit integers.
The nominative type hierarchy prevents one type from being used in the place of another when they happen to have similar structure, allowing distinctions to be drawn between an \verb|Int32| and a \verb|Char|, even though they have the same underlying implementation as a 32-bit integer values.

There are some high-level aspects of Julia's type system that are worth mentioning up front:
\begin{itemize}

\item Like Lisp, Perl, Python, Lua, Ruby, and Matlab but unlike C or Java, only values, not variables, have types.
There is no meaningful concept of a compile-time type versus a run-time type:
the only type of a value is its actual run-time type.
Variables have no type: they are just names bound to values.

\item The type hierarchy, is not flat:
abstract types may have many subtypes and implementations.
Unlike traditional object-oriented (OO) programming languages, however, all concrete type implementations are final:
one implementation may never be a subtype of another implementation.

\item Julia supports multiple inheritance:
a type may explicitly declare that it is a subtype of many abstract types.
The absence of implementation inheritance removes traditional data representation problems encountered in the presence of multiple inheritance.%
\footnote{The other problems classically caused by multiple inheritance are method dispatch ambiguities and instabilities introduced when different methods for the same function are dispatched on non-disjoint types, sharing common descendant subtypes.
Such problems are prevented by a simple and sensible rule described in \Section{method-ambiguities}.}

\end{itemize}
In general, the Julia type system is designed to be powerful and expressive, but unobtrusive and intuitive to use.
Much useful Julia programming can be done without ever explicitly using types.
When additional power is needed, however, it is easy to gradually introduce types into previously typeless programs.
Doing so will typically increase both the performance and robustness of these systems.
