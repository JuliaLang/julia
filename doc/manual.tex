...
Julia supports a broad range of numeric types together with the full complement of arithmetic and bitwise operations on them.
...
\begin{itemize}
  ...
  For other numeric types, this is essentially just a test of whether \texttt{x < 0}.}
...
\end{itemize}
In addition all the standard trigonometric functions are defined:
...
Since Julia allows numeric literals as juxtaposed coefficients, this binding alone suffices to give a convenient input syntax for complex numbers:
\begin{verbatim}
  julia> 1 + 2im
  1 + 2im
\end{verbatim}
...
\end{verbatim}
The promotion mechanism ensures that combinations of operands of different types simply work:
\begin{verbatim}
  julia> (1 + 2im) + 0.5im
  1.0 + 2.5im
  ...
  julia> (1 + 2//3im) + (0.5 + 1/3im)
  1.5 + 1.0im
\end{verbatim}
Note that \verb|1/3im| parses as \verb|1/3*im|, which, since division and multiplication have equal precedence, is equivalent to \verb|(1/3)*im| rather than \verb|1/(3im)|.
