...
.. function:: eltype(type)
   ...
   Determine the type of the elements generated by iterating a collection of the given ``type``\ . For associative collection types, this will be a ``Pair{keytype,valuetype}`` type. The definition ``eltype(x) = eltype(typeof(x))`` is provided for convenience so that instances can be passed instead of types. However the form that accepts a type argument should be defined for new types.
...
.. function:: collect(collection)
   ...
   Return an array of all items in a collection. For associative collections, returns an array of Pair{keytype,valtype}(key, value).
