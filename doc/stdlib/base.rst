...
.. function:: Set(element...)
   ...
   Construct a ``Set`` with the given elements or an empty set.
   Should be used instead of ``IntSet`` for sparse integer sets, or for sets of arbitrary objects.

.. function:: IntSet(element...)

   Construct a sorted set of the integers with the given elements, or an empty set. Implemented as a bit string, and therefore designed for dense integer sets. Only non-negative integers can be stored. If the set will be sparse (for example holding a single very large integer), use ``Set`` instead.
