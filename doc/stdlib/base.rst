...
.. function:: convert(T, x)
   ...
   If "T" is a "FloatingPoint" or "Rational" type, then it will return the closest value to "x" representable by "T".
...
.. function:: pipe(command; stdin, stdout, stderr, append=false)
   ...
   **Examples**:     * "run(pipe(``dothings``, stdout="out.txt",       stderr="errs.txt"))"
