...
.. function:: set_rounding(T, mode)
   ...
   Set the rounding mode of floating point type "T", controlling the rounding of basic arithmetic functions ("+()", "-()", "*()", "/()" and "sqrt()") and type conversion.

   Note that this may affect other types, for instance changing the rounding mode of "Float64" will change the rounding mode of "Float32". See "get_rounding" for available modes
