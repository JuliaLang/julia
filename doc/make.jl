...
s = readstring(joinpath(@__DIR__, "..", "NEWS.md"))
content, links = split(s, "<!--- generated by NEWS-update.jl: -->")
d = Dict()
...
for (k, v) in d
    content = replace(content, k, string(k, "(", v, ")"))
end
write(joinpath(@__DIR__, "src", "NEWS.md"), content)
