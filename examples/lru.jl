...
mutable struct UnboundedLRU{K,V} <: LRU{K,V}
    ...
    UnboundedLRU{K,V}() where {K,V} = new(Dict{Any,Any}(), similar(Vector{CacheItem}(uninitialized, 1), 0))
end
...
mutable struct BoundedLRU{K,V} <: LRU{K,V}
    ...
    BoundedLRU{K,V}(m) where {K,V} = new(Dict{Any,Any}(), similar(Vector{CacheItem}(uninitialized, 1), 0), m)
