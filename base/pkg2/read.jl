...
function available(names=readdir("METADATA"))
    pkgs = Dict{ByteString,Dict{VersionNumber,Available},Unordered}()
    for pkg in names
...
function installed(avail::Dict=available())
    pkgs = Dict{ByteString,(VersionNumber,Bool),Unordered}()
    for pkg in readdir()
        isinstalled(pkg) || continue
        ap = get(avail,pkg,Dict{VersionNumber,Available,Unordered}())
...
function fixed(avail::Dict=available(), inst::Dict=installed(avail), julia_version::VersionNumber=VERSION)
    pkgs = Dict{ByteString,Fixed,Unordered}()
...
function free(inst::Dict=installed())
    pkgs = Dict{ByteString,VersionNumber,Unordered}()
    for (pkg,(ver,fix)) in inst
        fix && continue
        pkgs[pkg] = ver
    end
