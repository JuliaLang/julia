...
findfirst(
    pred::Fix2{<:Union{typeof(isequal),typeof(==)},<:Union{Int8,UInt8}},
    a::ByteArray
...
function _searchindex(
    ...
    if isempty(t)
        return 1 <= i <= nextind(s, lastindex(s)) ? i : throw(BoundsError(s, i))
    end
...
_searchindex(s::AbstractString, t::AbstractChar, i::Integer) =
...
function _rsearchindex(
    ...
    while true
        ...
        ii = prevind(s, i)
        a = Iterators.Stateful(trest)
        b = Iterators.Stateful(Iterators.reverse(pairs(SubString(s, 1, ii))))
