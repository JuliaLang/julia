...
(`NamedTuple{(:label, :value), Tuple{Symbol, Any}}`) with the wrapped character.
...
AnnotatedChar(s::S, annotations::Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}})
```
...
struct AnnotatedChar{C <: AbstractChar} <: AbstractChar
    char::C
    annotations::Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}
end
...
AnnotatedChar(c::AbstractChar, annots::Vector) =
    AnnotatedChar(c, Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}(annots))

AnnotatedChar(c::AbstractChar, annots) =
    AnnotatedChar(c, collect(NamedTuple{(:label, :value), Tuple{Symbol, Any}}, annots))
...
AnnotatedChar(c::AnnotatedChar, annots::Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}) =
    AnnotatedChar(c.char, vcat(c.annotations, Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}(annots)))
...
convert(::Type{AnnotatedChar{C}}, c::C) where { C <: AbstractChar } =
    AnnotatedChar{C}(c, Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}())
...
function getindex(s::AnnotatedString, i::Integer)
    ...
    @inbounds if isvalid(s, i)
        AnnotatedChar(s.string[i], NamedTuple{(:label, :value), Tuple{Symbol, Any}}[(; label, value) for (; label, value) in annotations(s, i)])
    else
...
function _annotate!(annlist::Vector{RegionAnnotation}, region::UnitRange{Int}, label::Symbol, @nospecialize(value::Any))
    if value === nothing
        ...
        push!(annlist, (; region, label, value))
    end
end
...
annotate!(c::AnnotatedChar, label::Symbol, @nospecialize(val::Any)) =
    (push!(c.annotations, (; label, val)); c)
...
"""
    annotations(chr::AnnotatedChar) -> Vector{NamedTuple{(:label, :value), Tuple{Symbol, Any}}}
