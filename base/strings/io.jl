...
Create a string representation of any value using the 2-argument `show(io, x)` function, which aims to produce a string that is parseable Julia code where possible. i.e.
`eval(Meta.parse(repr(x))) == x` should hold true.
