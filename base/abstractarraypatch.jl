...
function copyto_unaliased!(deststyle::IndexStyle, dest::AbstractArray, srcstyle::IndexStyle, src::AbstractArray)
    ...
    if deststyle isa IndexLinear
        ...
        if srcstyle isa IndexLinear
            L2Ccopyto!(dest, src) # this manually expanded version seems faster
            #i = firstindex(src) - 1
            #@inbounds @simd for J in view(eachindex(dest), 1:length(src))
            #    dest[J] = src[i += 1]
            #end
        else
...
@eval @inline function L2Ccopyto!(dest::AbstractArray, src::AbstractArray)
    ...
    @inbounds for J in simd_outer_range(iter)
        ...
        i = zero(n)
        while i < n
            dest[simd_index(iter, J, i)] = src[i + Δi]
            i += 1
