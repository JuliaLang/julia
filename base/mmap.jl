...
for use in `Mmap.mmap`. Used by [`SharedArray`](@ref) for creating shared memory arrays.
"""
type Anonymous <: IO
    name::AbstractString
    readonly::Bool
...
"""
    Mmap.mmap(io, type::Type{Array{T,N}}, dims, offset; grow::Bool=true, shared::Bool=true)
...
"""
    Mmap.mmap(io, BitArray, dims, offset::Int64=position(io))
