...
function create_match_context()
    ...
    jit_stack = ccall(
        (:pcre2_jit_stack_create_8, PCRE_LIB),
        Ptr{Cvoid},
        (Cint, Cint, Ptr{Cvoid}),
    ...
    ctx = ccall((:pcre2_match_context_create_8, PCRE_LIB), Ptr{Cvoid}, (Ptr{Cvoid},), C_NULL)
    ccall(
        (:pcre2_jit_stack_assign_8, PCRE_LIB),
        Cvoid,
...
const COMPILE_MASK = ANCHORED | CASELESS | DOLLAR_ENDONLY | DOTALL | ENDANCHORED | EXTENDED |
                     FIRSTLINE | MULTILINE | NEWLINE_ANY | NEWLINE_ANYCRLF | NEWLINE_CR |
                     NEWLINE_CRLF | NEWLINE_LF | NO_AUTO_CAPTURE | NO_START_OPTIMIZE |
                     NO_UTF_CHECK | UNGREEDY | UTF | UCP

const EXECUTE_MASK = NEWLINE_ANY | NEWLINE_ANYCRLF | NEWLINE_CR | NEWLINE_CRLF | NEWLINE_LF |
                     NOTBOL | NOTEMPTY | NOTEMPTY_ATSTART | NOTEOL | NO_START_OPTIMIZE |
...
function info(regex::Ptr{Cvoid}, what::Integer, ::Type{T}) where T
    ...
    if ret != 0
        error(ret == ERROR_NULL ? "NULL regex object" :
              ...
              ret == ERROR_BADOPTION ? "invalid option flags" : "unknown error $ret")
    end
    buf[]
end

function ovec_length(match_data)
    n = ccall((:pcre2_get_ovector_count_8, PCRE_LIB), UInt32, (Ptr{Cvoid},), match_data)
    return 2 * n
end

function ovec_ptr(match_data)
    ptr = ccall(
        (:pcre2_get_ovector_pointer_8, PCRE_LIB),
        Ptr{Csize_t},
        (Ptr{Cvoid},),
        match_data
    )
...
free_re(re) = ccall((:pcre2_code_free_8, PCRE_LIB), Cvoid, (Ptr{Cvoid},), re)
...
function err_message(errno)
    ...
    ccall(
        (:pcre2_get_error_message_8, PCRE_LIB),
        Cvoid,
        (Int32, Ptr{UInt8}, Csize_t),
        errno,
        buffer,
        sizeof(buffer)
    )
...
function create_match_data(re)
    ccall(
        (:pcre2_match_data_create_from_pattern_8, PCRE_LIB),
        Ptr{Cvoid},
        (Ptr{Cvoid}, Ptr{Cvoid}),
        re,
        C_NULL
...
function substring_number_from_name(re, name)
    ccall(
        (:pcre2_substring_number_from_name_8, PCRE_LIB),
        Cint,
        (Ptr{Cvoid}, Cstring),
        re,
        name
...
function substring_length_bynumber(match_data, number)
    s = RefValue{Csize_t}()
    rc = ccall(
        (:pcre2_substring_length_bynumber_8, PCRE_LIB),
        Cint,
...
function substring_copy_bynumber(match_data, number, buf, buf_size)
    ...
    rc = ccall(
        (:pcre2_substring_copy_bynumber_8, PCRE_LIB),
        Cint,
