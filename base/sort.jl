...
import .Base: sort, sort!, issorted, sortperm, to_indices
...
for s in [:searchsortedfirst, :searchsortedlast, :searchsorted]
    @eval begin
        $s(v::AbstractVector, x, o::Ordering) =
            (inds = axes(v, 1); $s(v, x, first(inds), last(inds), o))
        $s(
           v::AbstractVector,
           x;
           lt = isless,
...
function sort!(v::AbstractVector, lo::Int, hi::Int, a::PartialQuickSort{Int}, o::Ordering)
    @inbounds while lo < hi
        hi - lo <= SMALL_THRESHOLD && return sort!(v, lo, hi, SMALL_ALGORITHM, o)
        j = partition!(v, lo, hi, o)
        if j >= a.k
...
function sort!(
    ...
    lo::Int,
    hi::Int,
    a::PartialQuickSort{T},
    o::Ordering
) where T <: OrdinalRange
    @inbounds while lo < hi
        hi - lo <= SMALL_THRESHOLD && return sort!(v, lo, hi, SMALL_ALGORITHM, o)
        j = partition!(v, lo, hi, o)
...
partialsortperm(v::AbstractVector, k::Union{Integer,OrdinalRange}; kwargs...) =
    partialsortperm!(
        similar(Vector{eltype(k)}, axes(v, 1)),
        v,
        k;
...
function nans2left!(
    v::AbstractVector,
    o::Ordering,
    lo::Int = first(axes(v, 1)),
    hi::Int = last(axes(v, 1))
)
    i = lo
...
function fpsort!(v::AbstractVector, a::Algorithm, o::Ordering)
    ...
    @inbounds while true
        while i <= j && issignleft(o, v[i])
            i += 1
        end
        while i <= j && !issignleft(o, v[j])
            j -= 1
        end
        i <= j || break
        v[i], v[j] = v[j], v[i]
...
sort!(v::Vector{Int}, a::Algorithm, o::Perm{<:DirectOrdering,<:Vector{<:Floats}}) =
    fpsort!(v, a, o)
