...
~{I<:Integer}(x::I)  = box(I,not_int(unbox(I,x)))
(&){I<:Integer} (x::I, y::I) = box(I,and_int(unbox(I,x),unbox(I,y)))
|{I<:Integer}   (x::I, y::I)  = box(I,or_int(unbox(I,x),unbox(I,y)))
($){I<:Integer} (x::I, y::I)  = box(I,xor_int(unbox(I,x),unbox(I,y)))

<<{I<:Integer}  (x::I, y::Int32) = box(I,shl_int(unbox(I,x),unbox(Int32,y)))
>>{S<:Signed}   (x::S, y::Int32) = box(S,ashr_int(unbox(S,x),unbox(Int32,y)))
>>{U<:Unsigned} (x::U, y::Int32) = box(U,lshr_int(unbox(U,x),unbox(Int32,y)))
>>>{I<:Integer}(x::I, y::Int32) = box(I,lshr_int(unbox(I,x),unbox(Int32,y)))

bswap(x::Int8) = x
bswap(x::Uint8) = x
bswap{I<:Integer}(x::I) = box(I,bswap_int(unbox(I,x)))

count_ones{I<:Integer}    (x::I) = int(box(I,ctpop_int(unbox(I,x))))
leading_zeros{I<:Integer} (x::I) = int(box(I,ctlz_int(unbox(I,x))))
trailing_zeros{I<:Integer}(x::I) = int(box(I,cttz_int(unbox(I,x))))
...
<{S<:Signed}   (x::S, y::S) = slt_int(unbox(S,x),unbox(S,y))
<{U<:Unsigned} (x::U, y::U) = ult_int(unbox(U,x),unbox(U,y))
<={S<:Signed}  (x::S, y::S) = sle_int(unbox(S,x),unbox(S,y))
<={U<:Unsigned}(x::U, y::U) = ule_int(unbox(U,x),unbox(U,y))
