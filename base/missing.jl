...
function mapreduce(f, op, itr::SkipMissing{<:AbstractArray}; kw...)
    if eltype(itr.x) >: Missing && isempty(kw)
        return _mapreduce(f, op, IndexStyle(itr.x), itr)
    end
    return mapreduce(f, op, itr.x; kw...)
end
