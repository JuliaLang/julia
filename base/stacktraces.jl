...
function ==(a::StackFrame, b::StackFrame)
    return a.line == b.line &&
           a.from_c == b.from_c &&
           a.func == b.func && a.file == b.file && a.inlined == b.inlined # excluding linfo and pointer
end
...
function lookup(pointer::Ptr{Cvoid})
    ...
    isempty(infos) && return [StackFrame(
        empty_sym,
        empty_sym,
        -1,
        nothing,
...
"""
function stacktrace(
    trace::Vector{<:Union{Base.InterpreterIP,Ptr{Cvoid}}},
    c_funcs::Bool = false
)
    ...
    !isempty(stack) &&
    startswith(string(stack[1].func), "jlcall_stacktrace") && popfirst!(stack)
    stack
end
...
is_top_level_frame(f::StackFrame) =
    ...
    if frame.linfo === nothing
        ...
        elseif frame.func === top_level_scope_sym
            ...
            color = get(io, :color, false) && get(io, :backtrace, false) ?
                    Base.stackframe_function_color() : :nothing
...
function show(io::IO, frame::StackFrame; full_path::Bool = false)
    ...
    if frame.file !== empty_sym
        ...
        Base.with_output_color(
            get(io, :color, false) && get(io, :backtrace, false) ?
            Base.stackframe_lineinfo_color() : :nothing,
            io
        ) do io
            print(io, file_info, ":")
            if frame.line >= 0
