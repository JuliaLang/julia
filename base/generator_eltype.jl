...
function eltype(::Type{Generator{A, Fix1{typeof(getindex), B}}}) where {A, B}
    if B <: Type
        ...
        # TODO: add special-casing for the case when `B <: AbstractArray`? It
        # could make sense to do this because arrays support multiple index
        # types, not all of which are necessarily covered by `keytype`.
