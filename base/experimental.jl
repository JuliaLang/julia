...
macro optlevel(n::Int, ex)
    if Base.is_function_def(Base.unwrap_macrocalls(ex))
...
"""
    Experimental.set_optlevel!(m::Method, n::Int)

Set the optimization level for a specific method. `n` must be between 0 and 3,
or -1 to inherit from the method's module.

See also [`Experimental.@optlevel`](@ref).
"""
function set_optlevel!(m::Method, n::Int)
    (0 <= n <= 3 || n == -1) || throw(ArgumentError(LazyString("optlevel must be between 0 and 3, or -1 to inherit, got ", n)))
    ccall(:jl_set_method_optlevel, Cvoid, (Any, Cint), m, n)
end
...
"""
    Experimental.set_compile!(m::Method, n::Int)

Set the compilation mode for a specific method. Valid values are 0 (off), 1 (on),
2 (all), 3 (min), or -1 to inherit from the method's module.

See also [`Experimental.@compiler_options`](@ref).
"""
function set_compile!(m::Method, n::Int)
    (0 <= n <= 3 || n == -1) || throw(ArgumentError(LazyString("compile must be between 0 and 3, or -1 to inherit, got ", n)))
    ccall(:jl_set_method_compile, Cvoid, (Any, Cint), m, n)
end
...
"""
    Experimental.set_infer!(m::Method, n::Int)

Set inference for a specific method. `n` should be 0 (off), 1 (on),
or -1 to inherit from the method's module.

See also [`Experimental.@compiler_options`](@ref).
"""
function set_infer!(m::Method, n::Int)
    (n == 0 || n == 1 || n == -1) || throw(ArgumentError(LazyString("infer must be 0, 1, or -1 to inherit, got ", n)))
    ccall(:jl_set_method_infer, Cvoid, (Any, Cint), m, n)
end
...
"""
    Experimental.set_max_methods!(m::Method, n::Int)

Set the max_methods for a specific method. `n` must be between 1 and 4,
or -1 to inherit from the method's module.

See also [`Experimental.@compiler_options`](@ref).
"""
function set_max_methods!(m::Method, n::Int)
    (1 <= n <= 4 || n == -1) || throw(ArgumentError(LazyString("max_methods must be between 1 and 4, or -1 to inherit, got ", n)))
    ccall(:jl_set_method_max_methods, Cvoid, (Any, Cint), m, n)
end
...
macro compiler_options(args...)
    ...
    if length(args) >= 2
        last_arg = args[end]
        if last_arg isa Expr && Base.is_function_def(Base.unwrap_macrocalls(last_arg))
            fdef = last_arg
    ...
    for ex in option_args
        if isa(ex, Expr) && ex.head === :(=) && length(ex.args) == 2
            if ex.args[1] === :optimize
                push!(metas, Expr(:optlevel, ex.args[2]::Int))
