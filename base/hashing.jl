...
utf8units(s::AbstractString) = UTF8Units(s)
utf8units(s::String) = codeunits(s)
...
# Backward compatibility wrapper for pointer-based interface
@assume_effects :terminates_globally function hash_bytes(
        ptr::Ptr{UInt8},
        n::Int,
        seed::UInt64,
    ...
    # Create an unsafe array view of the pointer data
    data = unsafe_wrap(Array, ptr, n)
    return hash_bytes(data, seed, secret)
end
