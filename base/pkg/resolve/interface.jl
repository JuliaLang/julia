...
type Interface
    ...
    function Interface(reqs::Requires, deps::Dict{UTF8String,Dict{VersionNumber,Available}})

        # generate pkgs
        pkgs = sort!(UTF8String[Set{UTF8String}(keys(deps))...])

        np = length(pkgs)

        # generate pdict
        pdict = (UTF8String=>Int)[ pkgs[i] => i for i = 1:np ]
...
function compute_output_dict(sol::Vector{Int}, interface::Interface)
    ...
    want = Dict{UTF8String,VersionNumber}()
    for p0 = 1:np
        p = pkgs[p0]
        s = sol[p0]
        if s != spp[p0]
...
function greedysolver(interface::Interface)
    ...
    staged = Set{UTF8String}(keys(reqs))
    seen = copy(staged)
