...
function accumulate_pairwise!(op::Op, result::AbstractVector, v::AbstractVector) where Op except DimensionMismatch
    li = LinearIndices(v)
...
```
"""
function accumulate(op, A; dims::Union{Nothing,Integer}=nothing, kw...) except ArgumentError
...
```
"""
function accumulate!(op, B, A; dims::Union{Integer, Nothing} = nothing, kw...) except ArgumentError
    nt = values(kw)
    if isempty(kw)
...
function _accumulate!(op, B, A, dims::Nothing, init::Union{Nothing, Some}) except ArgumentError
...
function _accumulate!(op, B, A, dims::Integer, init::Union{Nothing, Some}) except Union{ArgumentError, DimensionMismatch}
...
function _accumulate1!(op, B, v1, A::AbstractVector, dim::Integer) except Union{ArgumentError, DimensionMismatch}
