...
In general, `Future` is expected to be short-lived and have its allocation elided.
"""
struct Future{T}
...
function doworkloop(interp::AbstractInterpreter, sv::AbsIntState)
    ...
    if !completed
        @assert length(tasks) >= prev "Task did not complete, but also did not create any child tasks"
        push!(tasks, task)
    end
