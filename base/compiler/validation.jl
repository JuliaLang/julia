    ...
    for x in c.code
        if isa(x, Expr)
            ...
            elseif head === :call || head === :invoke || head === :gc_preserve_end || head === :meta ||
                ...
                head === :throw_undef_if_not || head === :code_coverage_effect || head === :detach ||
...
function is_valid_rvalue(@nospecialize(x))
    ...
    if isa(x, Expr) && x.head in (:new, :splatnew, :the_exception, :isdefined, :call, :invoke, :foreigncall, :cfunction, :gc_preserve_begin, :copyast, :syncregion)
        return true
    end
    return false
end
