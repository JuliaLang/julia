...
function construct_ssa!(ci::CodeInfo, ir::IRCode, domtree::DomTree,
    ...
    @zone JOPT_idf for (idx, slot) in Iterators.enumerate(defuses)
        ...
        @zone JOPT_liveness (live = compute_live_ins(cfg, slot))
    ...
    @zone JOPT_SSARename while !isempty(worklist)
    ...
    @zone JOPT_domsort ir = domsort_ssa!(ir, domtree)
    return ir
end
