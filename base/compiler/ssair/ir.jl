...
#=
                if any(x->x===0, blocks[i].preds) || any(x->x===0, blocks[i].succs)
                    @assert false
                end
=#
            end
...
function kill_edge!(compact, active_bb, from, to, duplicate = false)
...
function process_node!(compact::IncrementalCompact, result::Vector{Any},
    ...
    elseif isa(stmt, GotoIfNot) && compact.allow_cfg_transforms
        ...
        if stmt.dest == active_bb+1
            kill_edge!(compact, active_bb, active_bb, active_bb+1, true)
            # Destination and fall-through are the same - drop the branch.
            ssa_rename[idx] = result[result_idx] = nothing
