...
function ssa_inlining_pass!(ir::IRCode, state::InliningState, propagate_inbounds::Bool)
    ...
    @zone JOPT_InliningAnalysis todo = assemble_inline_todo!(ir, state)
    isempty(todo) && return ir
    # Do the actual inlining for every call we identified
    @zone JOPT_InliningExecution ir = batch_inline!(ir, todo, propagate_inbounds, OptimizationParams(state.interp))
    return ir
end
