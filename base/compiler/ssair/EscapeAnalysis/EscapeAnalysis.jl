...
import Core.Compiler: # Core.Compiler specific definitions
    ...
    buffer_builtin_common_typecheck, bufferset_typecheck,
...
function escape_foreigncall!(astate::AnalysisState, pc::Int, args::Vector{Any})
    ...
    if isa(nn, Symbol)
        ...
        elseif is_array_isassigned(nn)
            ...
            return
        elseif is_buffer_isassigned(nn)
            escape_buffer_isassigned!(astate, pc, args)
            return
        end
...
function escape_buffer_isassigned!(astate::AnalysisState, pc::Int, args::Vector{Any})
    if !buffer_isassigned_nothrow(args, astate.ir)
        add_thrown_escapes!(astate, pc, args)
    end
    add_liveness_changes!(astate, pc, args, 6)
end
...
function buffer_isassigned_nothrow(args::Vector{Any}, src::IRCode)
    ...
    #     :jl_buffer_isassigned, Cint, svec(Any,Csize_t), 0, :ccall)
    #     return false
    # end
    length(args) ≥ 7 || return false
    argextype(args[6], src) ⊑ Buffer || return false
    argextype(args[7], src) ⊑ Csize_t || return false
    return true
end
