...
function abstract_iteration(interp::AbstractInterpreter, @nospecialize(itft), @nospecialize(itertype), sv::Union{InferenceState, IRCode})
    ...
    stateordonet === Bottom && return AbstractIterationResult(Any[Bottom], AbstractIterationInfo(CallMeta[CallMeta(Bottom, info)], true))
...
function abstract_invoke(interp::AbstractInterpreter, (; fargs, argtypes)::ArgInfo, si::StmtInfo, sv::InferenceState)
    ...
    if types === Bottom || types === Any || !(unwrapped isa DataType)
        return CallMeta(Bottom, NoCallInfo(EFFECTS_THROWS))
    end
...
function abstract_call_known(interp::AbstractInterpreter, @nospecialize(f),
    ...
    elseif isa(f, Core.OpaqueClosure)
        ...
        return CallMeta(typeof(f).parameters[2], NoCallInfo())
    elseif f === TypeVar
        ...
        return CallMeta(typevar_tfunc(𝕃ᵢ, n, lb_var, ub_var), NoCallInfo(EFFECTS_UNKNOWN))
    elseif f === UnionAll
    ...
    elseif la == 3 && istopfunction(f, :(>:))
        ...
        return CallMeta(abstract_call_known(interp, <:, ArgInfo(fargs, argtypes), si, sv, max_methods).rt, NoCallInfo(EFFECTS_TOTAL))
    elseif la == 2 &&
...
function abstract_call(interp::AbstractInterpreter, arginfo::ArgInfo, si::StmtInfo,
    ...
    if f === nothing
        ...
        elseif hasintersect(wft, Core.OpaqueClosure)
            ...
            if isa(uft, DataType)
                return CallMeta(rewrap_unionall(uft.parameters[2], wft), NoCallInfo())
            end
            return CallMeta(Any, NoCallInfo())
        end
...
function abstract_eval_statement_expr(interp::AbstractInterpreter, e::Expr, vtypes::Union{VarTable, Nothing},
    ...
    if ehead === :call
        ...
        if argtypes === nothing
            ...
            (; rt, info) = abstract_call(interp, arginfo, si, sv)
