...
# Package loading, precompilation, and cache handling.
...
package_slug(uuid::UUID, p::Int=5) = slug(_crc32c(uuid), p)
...
function extension_parent_name(M::Module)
    triggers = @lock require_lock get(EXT_PRIMED, PkgId(moduleroot(M)), nothing)
    return triggers === nothing ? nothing : first(triggers).name
end
...
function run_package_callbacks(modkey::PkgId, env::Union{Nothing, ExplicitEnv, ImplicitEnv}=nothing)
    if env === nothing
        prime_extensions(modkey)
    else
        prime_extensions(env, modkey)
    end
...
function prime_extensions(pkg::PkgId)
    ...
    _, root = specenv
    for i in eachindex(stack.roots)
        if stack.roots[i] == root
            return prime_extensions(stack.envs[i], pkg)
        end
    end
    root == Sys.STDLIB && return prime_extensions(stdlib_env(), pkg)
    return
end

insert_extension_triggers(pkg::PkgId) = prime_extensions(pkg) # Revise compatibility

function prime_extensions(env::ExplicitEnv, parent::PkgId)
    get(env.names, parent.uuid, nothing) == parent.name || return
    for (ext, trigger_uuids) in get(Dict{String, Vector{UUID}}, env.extensions, parent.uuid)
        triggers = PkgId[PkgId(uuid, env.names[uuid]) for uuid in trigger_uuids]
        _prime_extension(parent, ext, triggers)
    end
end

function prime_extensions(env::ImplicitEnv, parent::PkgId)
    pkg = implicit_env_pkg(env, parent.name)
    pkg === nothing && return
    pkg.uuid == parent.uuid || return
    for (ext, triggers) in pkg.extensions
        _prime_extension(parent, ext, triggers)
    end
end

function _prime_extension(parent::PkgId, ext::String, triggers::Vector{PkgId})
    id = PkgId(uuid5(parent.uuid::UUID, ext), ext)
    if haskey(EXT_PRIMED, id) || haskey(loaded_modules, id)
        return
    end
    ...
    for trigger_id in triggers
        ...
        if !haskey(loaded_modules, trigger_id) || haskey(package_locks, trigger_id)
        ...
        @label next_path for path_to_try in paths::Vector{String}
            ...
            try # any exit from here (goto, break, continue, return) will end_loading
                ...
                for modkey in newdeps
                    stalecheck ? run_package_callbacks(modkey) : prime_extensions(modkey)
                end
            end
        end
    end
...
function __require(into::Module, mod::Symbol)
    ...
    @lock require_lock begin
    # Frozen workers reuse one stack; ordinary package top-level code may mutate its environment.
    if _env_frozen()
...
function find_unsuitable_manifests_versions(stack::EnvironmentStack=current_env_stack())
    unsuitable = String[]
    dev = String[]
    for env in stack.envs
        env isa ExplicitEnv || continue
        manifest_file = env.manifest_path
        manifest_file === nothing && continue
        manifest_version = env.manifest_julia_version
        @label check begin
            manifest_version isa String || break check
            manifest_version = VersionNumber(manifest_version)
            thispatch(manifest_version) != thispatch(VERSION) && break check
            isempty(manifest_version.prerelease) != isempty(VERSION.prerelease) && break check
            isempty(manifest_version.prerelease) && continue
            manifest_version.prerelease[1] != VERSION.prerelease[1] && break check
            VERSION.prerelease[1] == "DEV" && push!(dev, manifest_file)
            continue
        end
        push!(unsuitable, string(manifest_file, " (v", manifest_version, ")"))
    end
    return unsuitable, dev
end
...
function _cachepath_in_depots(cachepath::Union{Nothing, String}, depots::Vector{String})
    cachepath === nothing && return false
    return any(depot -> startswith(cachepath, joinpath(depot, "")), depots)
end
...
function require_stdlib(package_uuidkey::PkgId, ext::Union{Nothing, String}, from::Module)
    ...
    try
        ...
        if ext isa String
            ...
            if from_m === from
                ...
                from_stdlib = _cachepath_in_depots(cachepath, depot_path)
            end
        end
        if from_stdlib
            spec = reify_stdlib(env, this_uuidkey, package_uuidkey)
            set_pkgorigin_version_path(this_uuidkey, spec.path)
            newm = _require_search_from_serialized(this_uuidkey, spec, UInt128(0), false; DEPOT_PATH=depot_path)
        end
    finally
    ...
    if newm isa Module
        # After successfully loading, notify downstream consumers
        run_package_callbacks(this_uuidkey, env)
    else
...
function create_expr_cache(pkg::PkgId, input::PkgLoadSpec, output::String, output_o::Union{Nothing, String},
    ...
    if triggers !== nothing
        parentid = triggers[1]
        @lock require_lock begin
            for env in current_env_stack().envs
                env isa ImplicitEnv || continue
                parent = implicit_env_pkg(env, parentid.name)
                parent === nothing && continue
                parent.uuid == parentid.uuid || continue
                parent.project_file === nothing || push!(load_path, parent.project_file)
            end
        end
    end
    path_sep = Sys.iswindows() ? ';' : ':'
