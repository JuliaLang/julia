...
function compilecache_freshest_path(pkg::PkgId;
    ...
    for build_id in try_build_ids
        for path_to_try in cachepaths
            staledeps = stale_cachefile(pkg, build_id, sourcepath, path_to_try; ignore_loaded, requested_flags=flags, verify_checksums)
            if staledeps === true
                continue
            end
            ...
            for dep in staledeps
                ...
                for modpath_to_try in modpaths::Vector{String}
                    ...
                    if get!(() -> stale_cachefile(modkey, modbuild_id, modpath, modpath_to_try; ignore_loaded, requested_flags=flags, verify_checksums) === true,
...
function _parse_cache_header(f::IO, cachefile::AbstractString)
    ...
    return modules, (includes, srcfiles, requires), required_modules, srctextpos, prefs_blob, clone_targets, flags
end

function parse_cache_header(f::IO, cachefile::AbstractString)
    modules, (includes, srcfiles, requires), required_modules,
        srctextpos, prefs_blob, clone_targets, flags = _parse_cache_header(f, cachefile)
    ...
    return modules, (includes, includes_srcfiles, requires), required_modules, srctextpos, prefs_blob, clone_targets, flags
end
...
@constprop :none function stale_cachefile(modpath::String, cachefile::String; ignore_loaded::Bool = false, requested_flags::CacheFlags=CacheFlags(), reasons=nothing, verify_checksums::Bool=true)
    return stale_cachefile(PkgId(""), UInt128(0), modpath, cachefile; ignore_loaded, requested_flags, reasons, verify_checksums)
end
@constprop :none function stale_cachefile(modkey::PkgId, build_id::UInt128, modpath::String, cachefile::String;
    ...
    try
        ...
        modules, (includes, _, requires), required_modules, srctextpos, prefs_blob, clone_targets, actual_flags = parse_cache_header(io, cachefile)
        if isempty(modules)
