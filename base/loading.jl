        ...
        if triggers !== nothing
            ...
            if parent_project_file !== nothing
                ...
                if parentproj == parentid
                    mby_ext = project_file_load_spec(parent_project_file, pkg)
                    mby_ext === nothing || return mby_ext
                end
            end
        else
...
function project_get_syntax_version(d::Dict)
    ...
    # However, it avoids suprises from moving over scripts and REPL code to packages
    if sv === nothing
        sv = VERSION
...
@constprop :none function stale_cachefile(modspec::PkgLoadSpec, cachefile::String; ignore_loaded::Bool = false, requested_flags::CacheFlags=CacheFlags(), reasons=nothing)
    return stale_cachefile(PkgId(""), UInt128(0), modspec, modsver, cachefile; ignore_loaded, requested_flags, reasons)
end
