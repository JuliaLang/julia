...
function fl_parse(text::Union{Core.SimpleVector,String},
                  filename::String, lineno, offset, options, versionctx)
    ...
    # versionctx is ignored as jl_fl_parse isn't multi-version aware.
    ccall(:jl_fl_parse, Any, (Ptr{UInt8}, Csize_t, Any, Csize_t, Csize_t, Any),
          text, text_len, filename, lineno, offset, options)
end

function fl_parse(text::AbstractString, filename::AbstractString, lineno, offset, options, versionctx)
    fl_parse(String(text), String(filename), lineno, offset, options, versionctx)
end
