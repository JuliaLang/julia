...
mutable struct LockMonitor
    ...
    global function mkpidlock(at::String, pid::Cint, open_exclusive=open_exclusive; stale_age::Real=0, refresh::Real=stale_age/2, kwopts...)
        local lock
...
function open_exclusive(path::String;
    ...
    while true
        ...
        file = tryopen_exclusive(path, mode)
        file === nothing || return file
        sleep(poll_interval) # sleep for a bit before trying again
        if stale_age > 0 && stale_pidfile(path, stale_age)
