...
if Base.is_primary_base_module
    ...
    let
        ...
        print_time = (mod, t) -> (print(rpad(string(mod) * "  ", maxlen + 3, "─")); Base.time_print(t *
                                                                                                    10^9); println())
...
let
    tot_time_userimg = @elapsed (Base.isfile("userimg.jl") && Base.include(
        Main,
        "userimg.jl"
    ))
    ...
    print(" \n")
    print("Base: ─────── ")
    Base.time_print(tot_time_base * 10^9)
    print(" ")
    show(IOContext(stdout, :compact => true), (tot_time_base / tot_time) * 100)
    println("%")
    print("Stdlibs: ──── ")
    Base.time_print(Base.tot_time_stdlib[] * 10^9)
    print(" ")
    show(IOContext(stdout, :compact => true), (Base.tot_time_stdlib[] / tot_time) * 100)
    println("%")
    if isfile("userimg.jl")
        print("Userimg: ──── ")
        Base.time_print(tot_time_userimg * 10^9)
        print(" ")
        show(IOContext(stdout, :compact => true), (tot_time_userimg / tot_time) * 100)
        println("%")
    end
end
