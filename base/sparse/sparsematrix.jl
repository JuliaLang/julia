...
maximum{T}(A::SparseMatrixCSC{T}) =
    isempty(A) ? error("maximum: argument is empty") : reducedim(Base.scalarmax,A,(1,2),typemin(T))
...
minimum{T}(A::SparseMatrixCSC{T}) =
    isempty(A) ? error("minimum: argument is empty") : reducedim(Base.scalarmin,A,(1,2),typemax(T))
