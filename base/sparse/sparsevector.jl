...
# Define an alias for a view of a whole column of a SparseMatrix. Many methods can be written for the
# union of such a view and a SparseVector so we define an alias for such a union as well
const SparseColumnView{T}  = SubArray{T,1,<:SparseMatrix,Tuple{Base.Slice{Base.OneTo{Int}},Int},false}
...
_sparsesimilar(S::SparseVector, ::Type{TvNew}, ::Type{TiNew}, dims::Dims{2}) where {TvNew,TiNew} =
    SparseMatrix(dims..., ones(TiNew, last(dims)+1), similar(S.nzind, TiNew), similar(S.nzval, TvNew))
...
# specify a SparseVector result and storage space if the dims specify a SparseMatrix result.
...
# convert SparseMatrix to SparseVector
function convert(::Type{SparseVector{Tv,Ti}}, s::SparseMatrix{Tv,Ti}) where {Tv,Ti<:Integer}
    size(s, 2) == 1 || throw(ArgumentError("The input argument must have a single-column."))
    SparseVector(s.m, s.rowval, s.nzval)
end

convert(::Type{SparseVector{Tv}}, s::SparseMatrix{Tv,Ti}) where {Tv,Ti} =
    convert(SparseVector{Tv,Ti}, s)

convert(::Type{SparseVector}, s::SparseMatrix{Tv,Ti}) where {Tv,Ti} =
    convert(SparseVector{Tv,Ti}, s)
...
function copyto!(A::SparseVector, B::SparseMatrix)
    prep_sparsevec_copy_dest!(A, length(B), nnz(B))
...
copyto!(A::SparseMatrix, B::SparseVector{TvB,TiB}) where {TvB,TiB} =
    copyto!(A, SparseMatrix{TvB,TiB}(B.n, 1, TiB[1, length(B.nzind)+1], B.nzind, B.nzval))
...
# Column slices
function getindex(x::SparseMatrix, ::Colon, j::Integer)
    checkbounds(x, :, j)
...
function getindex(x::SparseMatrix, I::AbstractUnitRange, j::Integer)
    checkbounds(x, I, j)
...
# In the general case, we piggy back upon SparseMatrix's optimized solution
@inline function getindex(A::SparseMatrix, I::AbstractVector, J::Integer)
    M = A[I, [J]]
    SparseVector(M.m, M.rowval, M.nzval)
end

# Row slices
getindex(A::SparseMatrix, i::Integer, ::Colon) = A[i, 1:end]
function Base.getindex(A::SparseMatrix{Tv,Ti}, i::Integer, J::AbstractVector) where {Tv,Ti}
    checkbounds(A, i, J)
    nJ = length(J)
    ...
    # adapted from SparseMatrix's sorted_bsearch_A
    ptrI = 1
    @inbounds for j = 1:nJ
        col = J[j]
        rowI = i
...
getindex(A::SparseMatrix, I::AbstractVector{Bool}) = _logical_index(A, I) # Ambiguities
getindex(A::SparseMatrix, I::AbstractArray{Bool}) = _logical_index(A, I)
function _logical_index(A::SparseMatrix{Tv}, I::AbstractArray{Bool}) where Tv
    checkbounds(A, I)
    n = sum(I)
    nnzB = min(n, nnz(A))
...
getindex(A::SparseMatrix, ::Colon) = A[1:end]

function getindex(A::SparseMatrix{Tv}, I::AbstractUnitRange) where Tv
    checkbounds(A, I)
    szA = size(A)
    nA = szA[1]*szA[2]
    colptrA = A.colptr
...
function getindex(A::SparseMatrix{Tv,Ti}, I::AbstractVector) where {Tv,Ti}
    szA = size(A)
    nA = szA[1]*szA[2]
    colptrA = A.colptr
    rowvalA = A.rowval
...
@inline function getindex(x::AbstractSparseVector{Tv,Ti}, I::AbstractVector) where {Tv,Ti}
    # SparseMatrix has a nicely optimized routine for this; punt
    S = SparseMatrix(x.n, 1, Ti[1,length(x.nzind)+1], x.nzind, x.nzval)
    S[I, 1]
end

function getindex(x::AbstractSparseVector{Tv,Ti}, I::AbstractArray) where {Tv,Ti}
    # punt to SparseMatrix
    S = SparseMatrix(x.n, 1, Ti[1,length(x.nzind)+1], x.nzind, x.nzval)
    S[I]
end
...
function convert(::Type{SparseMatrix{Tv,Ti}}, x::AbstractSparseVector) where {Tv,Ti}
    n = length(x)
    xnzind = nonzeroinds(x)
    xnzval = nonzeros(x)
    m = length(xnzind)
    ...
    SparseMatrix(n, 1, colptr, rowval, nzval)
end

convert(::Type{SparseMatrix{Tv}}, x::AbstractSparseVector{<:Any,Ti}) where {Tv,Ti} =
    convert(SparseMatrix{Tv,Ti}, x)

convert(::Type{SparseMatrix}, x::AbstractSparseVector{Tv,Ti}) where {Tv,Ti} =
    convert(SparseMatrix{Tv,Ti}, x)
...
# sparse/special/dense matrix/vector types concatenate to SparseMatrixs, instead
...
function _absspvec_hcat(X::AbstractSparseVector{Tv,Ti}...) where {Tv,Ti}
    ...
    colptr[n+1] = roff
    SparseMatrix{Tv,Ti}(m, n, colptr, nzrow, nzval)
end
...
# sparse/special/dense matrix/vector types concatenate to SparseMatrixs (because
...
const _SparseArrays = Union{SparseVector, SparseMatrix, RowVector{<:Any,<:SparseVector}, Adjoint{<:Any,<:SparseVector}, Transpose{<:Any,<:SparseVector}}
...
function cat(catdims, Xin::_SparseConcatGroup...)
    X = map(x -> SparseMatrix(issparse(x) ? x : sparse(x)), Xin)
...
function hvcat(rows::Tuple{Vararg{Int}}, X::_SparseConcatGroup...)
    ...
    tmp_rows = Vector{SparseMatrix}(uninitialized, nbr)
    k = 0
...
promote_to_array_type(A::Tuple{Vararg{Union{_SparseConcatGroup,UniformScaling}}}) = SparseMatrix
promote_to_array_type(A::Tuple{Vararg{Union{_DenseConcatGroup,UniformScaling}}}) = Matrix
promote_to_arrays_(n::Int, ::Type{SparseMatrix}, J::UniformScaling) = sparse(J, n, n)
...
function densemv(A::SparseMatrix, x::AbstractSparseVector; trans::Char='N')
...
mul!(y::StridedVector{Ty}, A::SparseMatrix, x::AbstractSparseVector{Tx}) where {Tx,Ty} =
    mul!(one(Tx), A, x, zero(Ty), y)

function mul!(α::Number, A::SparseMatrix, x::AbstractSparseVector, β::Number, y::StridedVector)
    m, n = size(A)
    length(x) == n && length(y) == m || throw(DimensionMismatch())
    m == 0 && return y
    if β != one(β)
...
mul!(y::StridedVector{Ty}, transA::Transpose{<:Any,<:SparseMatrix}, x::AbstractSparseVector{Tx}) where {Tx,Ty} =
    (A = transA.parent; mul!(one(Tx), Transpose(A), x, zero(Ty), y))

mul!(α::Number, transA::Transpose{<:Any,<:SparseMatrix}, x::AbstractSparseVector, β::Number, y::StridedVector) =
    (A = transA.parent; _At_or_Ac_mul_B!(*, α, A, x, β, y))

mul!(y::StridedVector{Ty}, adjA::Adjoint{<:Any,<:SparseMatrix}, x::AbstractSparseVector{Tx}) where {Tx,Ty} =
    (A = adjA.parent; mul!(one(Tx), Adjoint(A), x, zero(Ty), y))

mul!(α::Number, adjA::Adjoint{<:Any,<:SparseMatrix}, x::AbstractSparseVector, β::Number, y::StridedVector) =
    (A = adjA.parent; _At_or_Ac_mul_B!(dot, α, A, x, β, y))

function _At_or_Ac_mul_B!(tfun::Function,
                          α::Number, A::SparseMatrix, x::AbstractSparseVector,
...
function *(A::SparseMatrix, x::AbstractSparseVector)
    y = densemv(A, x)
    initcap = min(nnz(A), size(A,1))
    _dense2sparsevec(y, initcap)
end

*(transA::Transpose{<:Any,<:SparseMatrix}, x::AbstractSparseVector) =
    (A = transA.parent; _At_or_Ac_mul_B(*, A, x))

*(adjA::Adjoint{<:Any,<:SparseMatrix}, x::AbstractSparseVector) =
    (A = adjA.parent; _At_or_Ac_mul_B(dot, A, x))

function _At_or_Ac_mul_B(tfun::Function, A::SparseMatrix{TvA,TiA}, x::AbstractSparseVector{TvX,TiX}) where {TvA,TiA,TvX,TiX}
    m, n = size(A)
    length(x) == m || throw(DimensionMismatch())
...
function _fillnonzero!(arr::SparseMatrix{Tv, Ti}, val) where {Tv,Ti}
    m, n = size(arr)
...
import Base.fill!
function fill!(A::Union{SparseVector, SparseMatrix}, x)
    T = eltype(A)
    xT = convert(T, x)
    if xT == zero(T)
        fill!(A.nzval, xT)
