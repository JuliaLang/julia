...
function show_tuple_as_call(io::IO, name::Symbol, sig::Type; demangle=false, kwargs=nothing, argnames=nothing, qualified=false, hasfirst=true)
    ...
    n = 1
    if hasfirst
        show_signature_function(env_io, sig[n], demangle, "", false, qualified)
        n += 1
    end
    first = true
