...
# Limited IO buffer, used in array and REPL printing
...
function showerror(io::IO, e::LimitIOException)
    print(io, "$LimitIOException: aborted printing after attempting to print more than $(format_bytes(e.maxbytes)) within a `LimitIO`.")
end

displaysize(io::LimitIO) = displaysize(io.io)

function write(io::LimitIO, v::UInt8)
...
# Semantically, we only need to override `write`, but we also
# override `unsafe_write` for performance.
function unsafe_write(limiter::LimitIO, p::Ptr{UInt8}, nb::UInt)
