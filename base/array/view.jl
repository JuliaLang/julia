...
v2a{T,n}(v::ArrayView{T,n}, I::NTuple{n,Int}) = v2a(v.strides, v.offset, I)
v2a{T,n}(v::ArrayView{T,n}, i::Int) = v2a(v.size, v.strides, v.offset, i)
...
function ArrayView{T,n,m}(a::Array{T,m}, R::NTuple{n,Ranges})
    ...
    strides = ntuple(n) do k
        offset += prod*(first(R[k])-1)
        stride = prod*step(R[k])
        prod *= size(a,k)
        return stride
    end
    ArrayView(a,sz,strides,offset)
end
