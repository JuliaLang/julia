...
Get a lower bound on the number of threads (across all thread pools or within the specified
thread pool) available to the Julia process >= [`threadid()`](@ref), with atomic-acquire semantics.
"""
