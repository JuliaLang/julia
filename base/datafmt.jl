    ...
    for retry in 1:2
        try
            ...
            elseif get(optsd, :ignore_invalid_chars, false)
                sbuff = convert(typeof(sbuff), sbuff.data, "")
            else
                rethrow(ex)
            end
            offset_handler = (dims === nothing) ? DLMOffsets(sbuff) : DLMStore(T, dims, has_header, sbuff, auto, eol)
...
dlm_parse(s::ByteString, eol::Char, dlm::Char, qchar::Char, cchar::Char, ign_adj_dlm::Bool, allow_quote::Bool, allow_comments::Bool, skipstart::Int, skipblanks::Bool, dh::DLMHandler) =  begin
