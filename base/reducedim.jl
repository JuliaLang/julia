...
reduced_index(i::AbstractUnitRange) except ArgumentError =
    throw(ArgumentError(
"""
...
function _check_valid_region(region) except ArgumentError
    for d in region
...
function check_reducedims(R, A) except DimensionMismatch
...
#
function findminmax!(f, op, Rval, Rind, A::AbstractArray{T,N}) where {T,N} except DimensionMismatch
...
function _findmin(f, A, region::D) where {D} except ArgumentError
...
function _findmax(f, A, region::D) where {D} except ArgumentError
    ri = reduced_indices0(A, region)
    if isempty(A)
        if prod(map(length, reduced_indices(A, region))) != 0
            throw(ArgumentError("collection slices must be non-empty"))?
