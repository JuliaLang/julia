...
const BACKGROUND_PRECOMPILE = BackgroundPrecompileState(nothing, false, false, IOBuffer(), nothing, nothing, nothing, nothing, ReentrantLock(), Threads.Condition(), Threads.Condition(), Channel{Int32}[])

# IO wrapper that notifies on writes and delegates displaysize to the real IO
struct NotifyingIO{T<:IO} <: IO
    io::IOBuffer
    lock::ReentrantLock
...
Base.write(nio::NotifyingIO, x::UInt8) = begin
    n = @lock nio.lock write(nio.io, x)
    @lock nio.cond notify(nio.cond)
    return n
end
Base.unsafe_write(nio::NotifyingIO, p::Ptr{UInt8}, n::UInt) = begin
    m = @lock nio.lock unsafe_write(nio.io, p, n)
    @lock nio.cond notify(nio.cond)
    return m
end
Base.displaysize(nio::NotifyingIO) = displaysize(nio.real_io)::Tuple{Int,Int}
...
function monitor_background_precompile(io::IO = stderr, detachable::Bool = true)
    ...
    local task = @lock BACKGROUND_PRECOMPILE.lock BACKGROUND_PRECOMPILE.task
    ...
    # Wait for completion while showing new output as it arrives
    return try
        while !exit_requested[] && !cancel_requested[] && !interrupt_requested[]
            # Wait for output to become available or task to complete
            @lock BACKGROUND_PRECOMPILE.output_available wait(BACKGROUND_PRECOMPILE.output_available)

            # Show new output if available, check for completion
            completed = @lock BACKGROUND_PRECOMPILE.lock begin
        ...
        if cancel_requested[]
            ...
            # Don't show subprocess output when canceling with 'c'
            print(io, "\e[J")
        ...
        # (interrupt_requested causes do_precompile to stop starting new workers),
        # then show all buffered output including worker reports after SIGINT
        if interrupt_requested[]
            ...
            wait(task; throw=false)
            close(escalation)
            # Clear partial progress bar output
            print(io, "\e[J")
            # Show all buffered output (includes worker reports after SIGINT)
            @lock BACKGROUND_PRECOMPILE.lock begin
                current_size = output_buf.size
                if current_size > last_pos
                    seek(output_buf, last_pos)
        ...
        # Flush any remaining output after task completes
        @lock BACKGROUND_PRECOMPILE.lock begin
            current_size = output_buf.size
            if current_size > last_pos
                seek(output_buf, last_pos)
        ...
        BACKGROUND_PRECOMPILE.task = Threads.@spawn :samepool begin
            try
                ...
                notifying_io = NotifyingIO(BACKGROUND_PRECOMPILE.output_buffer,
                                          ...
                                          io.io)
                wrapper_io = IOContext(notifying_io, io)
                ...
                @lock BACKGROUND_PRECOMPILE.lock begin
                    ...
                    BACKGROUND_PRECOMPILE.result = "Background precompilation completed successfully"
                end
            catch e
                @lock BACKGROUND_PRECOMPILE.lock begin
...
function do_precompile(pkgs::Union{Vector{String}, Vector{PkgId}},
    ...
    function should_stop(interrupted)
        local ir, cr
        @lock BACKGROUND_PRECOMPILE.lock begin
            ir = BACKGROUND_PRECOMPILE.interrupt_requested
            cr = BACKGROUND_PRECOMPILE.cancel_requested
        end
        if ir || cr
            interrupted[] = interrupted[] || ir
            if !interrupted_or_done[]
                interrupted_or_done[] = true
                foreach(notify, values(was_processed))
                notify(first_started)
            end
            return true
        end
        return interrupted_or_done[]
    end
    # Create a per-process channel registered in signal_channels, closed when compilecache finishes
            ...
            while !printloop_should_exit[]
                @lock print_lock begin
                    ...
                    str_ = sprint(; context=logio) do iostr
                        ...
                        bar.max = n_total - n_already_precomp[]
                        termwidth = (displaysize(logio)::Tuple{Int,Int})[2]
                        if !final_loop_local
                            tip_width = isempty(keyboard_tip) ? 0 : textwidth(keyboard_tip) + 1
                            bar_termwidth = termwidth - tip_width
                            s = sprint(io -> show_progress(io, bar; termwidth=bar_termwidth, carriagereturn=false); context=logio)
                            if !isempty(keyboard_tip)
                                s = string(s, " ", keyboard_tip)
                            end
                        ...
                        for pkg_config in pkg_queue_show
                            ...
                            elseif was_recompiled[pkg_config]
                                ...
                                loaded || Base.errormonitor(Threads.@spawn :samepool begin # keep successful deps visible for short period
                                    sleep(1);
                    ...
                    i += 1
                    if printloop_should_exit[]
                        print(logio, str_)
                    else
                        # Write display + cursor reposition as a single atomic write so
                        # the monitor never sees partial (un-repositioned) output
                        print(logio, str_, ansi_moveup(n_print_rows), ansi_movecol1)
                    end
                end
                wait(t)
            end
        ...
        for config in configs
            ...
            task = Threads.@spawn :samepool try
                ...
                if !circular && is_stale
                    ...
                    t_monitor = Threads.@spawn :samepool _precompilepkgs_monitor_std(pkg_config, std_pipe,
    ...
    if !strict && !requested_errs && !interrupted[]
    ...
    if ndeps > 0 || !isempty(failed_deps)
        ...
        elseif interrupted[]
            @lock print_lock begin
                println(logio, sprint(context=logio) do iostr
                    if fancyprint
                        printpkgstyle(iostr, :Precompiling, "interrupted.")
                    end
                    n_failed = length(failed_deps)
                    print(iostr, "  $(ndeps) dependenc$(ndeps == 1 ? "y" : "ies") precompiled, ",
                          color_string("$(n_failed)", Base.error_color()),
