...
function print_matrix(
    ...
    if m > screenheight
        rowsA = [rowsA[(0:halfheight-1).+firstindex(rowsA)]; rowsA[(end -
                                                                    div(
            screenheight - 1,
            2
        ) + 1):end]]
    ...
    if n > maxpossiblecols
        colsA = [colsA[(0:maxpossiblecols-1).+firstindex(colsA)]; colsA[(end -
                                                                         maxpossiblecols + 1):end]]
    end
    ...
    if m <= screenheight # rows fit vertically on screen
        if n <= length(A) # rows and cols fit so just print whole matrix in one piece
            ...
            c = screenwidth - sum(map(sum, Ralign)) - (length(Ralign) - 1) * sepsize -
                length(hdots)
            ...
            for i in rowsA
                ...
                print(
                    io,
                    (i - first(rowsA)) % hmod == 0 ? hdots : repeat(" ", length(hdots))
                )
        ...
        if n <= length(A) # rows don't fit, cols do, so only vertical ellipsis
            for i in rowsA
                ...
                print_matrix_row(io, X, A, i, colsA, sep)
                print(io, i == last(rowsA) ? post : postsp)
                if i != rowsA[end] || i == rowsA[halfheight]
                    println(io)
                end
...
_show_empty(io::IO, X::Array{T}) where {T} =
...
function show(io::IO, X::AbstractArray)
    ...
    io = IOContext(io, :typeinfo => eltype(X))
    isempty(X) ? _show_empty(io, X) : _show_nonempty(io, X, prefix)
end
...
function typeinfo_prefix(io::IO, X)
    ...
    if X isa AbstractDict
        if eltype_X == eltype_ctx ||
           !isempty(X) && isconcretetype(keytype(X)) && isconcretetype(valtype(X))
