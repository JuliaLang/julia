...
function copy!{R,S}(B::Matrix{R}, ir_dest::Range1{Int}, jr_dest::Range1{Int}, A::StridedMatrix{S}, ir_src::Range1{Int}, jr_src::Range1{Int})
    if length(ir_dest) != length(ir_src) || length(jr_dest) != length(jr_src)
        error("copy!: source and destination must have same size")
    end
...
function copy_transpose!{R,S}(B::Matrix{R}, ir_dest::Range1{Int}, jr_dest::Range1{Int}, A::StridedMatrix{S}, ir_src::Range1{Int}, jr_src::Range1{Int})
    if length(ir_dest) != length(jr_src) || length(jr_dest) != length(ir_src)
        error("copy_transpose!: source and destination must have same size")
    end
    checkbounds(B, ir_dest, jr_dest)
    checkbounds(A, ir_src, jr_src)
    idest = first(ir_dest)
...
function one{T}(x::AbstractMatrix{T})
    m,n = size(x)
    if m != n; error("one: multiplicative identity defined only for square matrices"); end;
    eye(T, m)
end
...
function setindex!{T}(A::Array{T}, X::Array{T}, I::Range1{Int})
    if length(X) != length(I); error("setindex!: argument dimensions must match"); end
...
function setindex!{T<:Real}(A::Array, x, i::Real, J::AbstractVector{T})
    ...
    if !isa(x,AbstractArray)
        ...
        X = x
        if length(X) != length(J); error("setindex!: argument dimensions must match"); end
        count = 1
        for j in J
            A[(j-1)*m + i] = X[count]
            count += 1
...
function setindex!{T}(A::Array{T}, X::Array{T}, I::Range1{Int}, J::Range1{Int})
    ...
    if length(X) != nel ||
        ...
        error("setindex!: argument dimensions must match")
    end
    if length(I) == size(A,1)
...
function pop!(a::Vector)
    if isempty(a)
        error("pop!: array must not be empty")
    end
    item = a[end]
...
function shift!(a::Vector)
    if isempty(a)
        error("shift!: array must not be empty")
    end
    item = a[1]
...
function complex{S<:Real,T<:Real}(A::Array{S}, B::Array{T})
    if size(A) != size(B); error("complex: argument dimensions must match"); end
...
function findmax(a)
    if isempty(a)
        error("findmax: array must not be empty")
    end
    m = a[1]
    mi = 1
    for i=2:length(a)
...
function findmin(a)
    if isempty(a)
        error("findmin: array must not be empty")
    end
    m = a[1]
    mi = 1
    for i=2:length(a)
...
function transpose!{T<:Number}(B::Matrix{T}, A::Matrix{T})
    m, n = size(A)
    if size(B) != (n,m)
        error("transpose!: input and output must have same size")
    end
