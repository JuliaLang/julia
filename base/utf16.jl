...
function convert(::Type{UTF16String}, data::Vector{Uint16})
    ...
    if data[end] == 0 # already NULL-terminated
        UTF16String(data)
    else
        d = Array(Uint16, length(data) + 1)
        d[end] = 0 # NULL terminate
        UTF16String(copy!(d,1, data,1, length(data)))
    end
end
...
function convert(T::Type{UTF16String}, bytes::Array{Uint8})
    ...
    if data[1] == 0xfeff        # native byte order
        ...
        d[end] = 0 # NULL terminate
        convert(T, copy!(d,1, data,2, length(data)-1))
    elseif data[1] == 0xfffe    # byte-swapped
        ...
        d = Array(Uint16, length(data) + 1)
        d[end] = 0 # NULL terminate     
        convert(T, copy!(d,1, data,1, length(data))) # assume native byte order
    end
end
