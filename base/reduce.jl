...
[`isequal`](:func:`isequal`) to one of the values generated by iterating over the
collection. Some collections need a slightly different definition. For example,
[`Dict`](:obj:`Dict`)s look for `(key,value)` pairs; to test for the presence of
a key in a dictionary, use [`haskey`](:func:`haskey`) or `k in keys(dict)`.
