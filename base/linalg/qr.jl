...
function _qr(A::Union{Number,AbstractMatrix}, ::Val{false}; full::Bool = false)
    ...
    return (!full ? Array(Q) : A_mul_B!2(Q, Matrix{eltype(Q)}(I, sQf1, sQf1))), R
end
function _qr(A::Union{Number, AbstractMatrix}, ::Val{true}; full::Bool = false)
    F = qrfact(A, Val(true))
    Q, R, p = getq(F), F[:R]::Matrix{eltype(F)}, F[:p]::Vector{BlasInt}
    sQf1 = size(Q.factors, 1)
    return (!full ? Array(Q) : A_mul_B!2(Q, Matrix{eltype(Q)}(I, sQf1, sQf1))), R, p
end
...
convert(::Type{Matrix}, A::AbstractQ{T}) where {T} = A_mul_B!2(A, Matrix{T}(I, size(A.factors, 1), min(size(A.factors)...)))
...
function getindex(A::AbstractQ, i::Integer, j::Integer)
    ...
    y[j] = 1
    return dot(x, A_mul_B!2(A, y))
end

## Multiplication by Q
### QB
A_mul_B!2(A::QRCompactWYQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasFloat, S<:StridedMatrix} =
    LAPACK.gemqrt!('L','N',A.factors,A.T,B)
A_mul_B!2(A::QRPackedQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasFloat, S<:StridedMatrix} =
    LAPACK.ormqr!('L','N',A.factors,A.τ,B)
function A_mul_B!2(A::QRPackedQ, B::AbstractVecOrMat)
...
### QcB
Ac_mul_B!2(A::QRCompactWYQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasReal,S<:StridedMatrix} =
    LAPACK.gemqrt!('L','T',A.factors,A.T,B)
Ac_mul_B!2(A::QRCompactWYQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasComplex,S<:StridedMatrix} =
    LAPACK.gemqrt!('L','C',A.factors,A.T,B)
Ac_mul_B!2(A::QRPackedQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasReal,S<:StridedMatrix} =
    LAPACK.ormqr!('L','T',A.factors,A.τ,B)
Ac_mul_B!2(A::QRPackedQ{T,S}, B::StridedVecOrMat{T}) where {T<:BlasComplex,S<:StridedMatrix} =
    LAPACK.ormqr!('L','C',A.factors,A.τ,B)
function Ac_mul_B!2(A::QRPackedQ, B::AbstractVecOrMat)
    mA, nA = size(A.factors)
    mB, nB = size(B,1), size(B,2)
    if mA != mB
        throw(DimensionMismatch("matrix A has dimensions ($mA,$nA) but B has dimensions ($mB, $nB)"))
...
function Ac_mul_B(Q::AbstractQ, B::StridedVecOrMat)
    ...
    return Ac_mul_B!2(convert(AbstractMatrix{TQB}, Q), copy_oftype(B, TQB))
end
...
for (f, f!2) in ((:A_mul_Bc, :A_mul_B!2),
    ...
    @eval begin
        function ($f)(Q::AbstractQ, B::StridedVecOrMat)
            TQB = promote_type(eltype(Q), eltype(B))
            Bc = similar(B, TQB, (size(B, 2), size(B, 1)))
            adjoint!(Bc, B)
            return ($f!2)(convert(AbstractMatrix{TQB}, Q), Bc)
        end
    end
end

### AQ
A_mul_B!1(A::StridedVecOrMat{T}, B::QRCompactWYQ{T,S}) where {T<:BlasFloat,S<:StridedMatrix} =
    LAPACK.gemqrt!('R','N', B.factors, B.T, A)
A_mul_B!1(A::StridedVecOrMat{T}, B::QRPackedQ{T,S}) where {T<:BlasFloat,S<:StridedMatrix} =
    LAPACK.ormqr!('R', 'N', B.factors, B.τ, A)
function A_mul_B!1(A::StridedMatrix,Q::QRPackedQ)
...
function (*)(A::StridedMatrix, Q::AbstractQ)
    ...
    return A_mul_B!1(copy_oftype(A, TAQ), convert(AbstractMatrix{TAQ}, Q))
end

### AQc
A_mul_Bc!1(A::StridedVecOrMat{T}, B::QRCompactWYQ{T}) where {T<:BlasReal} = LAPACK.gemqrt!('R','T',B.factors,B.T,A)
A_mul_Bc!1(A::StridedVecOrMat{T}, B::QRCompactWYQ{T}) where {T<:BlasComplex} = LAPACK.gemqrt!('R','C',B.factors,B.T,A)
A_mul_Bc!1(A::StridedVecOrMat{T}, B::QRPackedQ{T}) where {T<:BlasReal} = LAPACK.ormqr!('R','T',B.factors,B.τ,A)
A_mul_Bc!1(A::StridedVecOrMat{T}, B::QRPackedQ{T}) where {T<:BlasComplex} = LAPACK.ormqr!('R','C',B.factors,B.τ,A)
function A_mul_Bc!1(A::StridedMatrix,Q::QRPackedQ)
    mQ, nQ = size(Q.factors)
    mA, nA = size(A,1), size(A,2)
    if nA != mQ
        throw(DimensionMismatch("matrix A has dimensions ($mA,$nA) but matrix Q has dimensions ($mQ, $nQ)"))
...
function A_mul_Bc(A::StridedMatrix, B::AbstractQ)
    ...
    if size(A,2) == size(B.factors, 1)
        ...
        copy!(AA, A)
        return A_mul_Bc!1(AA, BB)
    elseif size(A,2) == size(B.factors,2)
        return A_mul_Bc!1([A zeros(TAB, size(A, 1), size(B.factors, 1) - size(B.factors, 2))], BB)
    else
...
### AcQ/AcQc
for (f, f!1) in ((:Ac_mul_B, :A_mul_B!1),
                 (:Ac_mul_Bc, :A_mul_Bc!1))
    @eval begin
        function ($f)(A::StridedVecOrMat, Q::AbstractQ)
            TAQ = promote_type(eltype(A), eltype(Q))
            Ac = similar(A, TAQ, (size(A, 2), size(A, 1)))
            adjoint!(Ac, A)
            return ($f!1)(Ac, convert(AbstractMatrix{TAQ}, Q))
        end
    end
end

A_ldiv_B!2(A::QRCompactWY{T}, b::StridedVector{T}) where {T<:BlasFloat} = (A_ldiv_B!2(UpperTriangular(A[:R]), view(Ac_mul_B!2(A[:Q], b), 1:size(A, 2))); b)
A_ldiv_B!2(A::QRCompactWY{T}, B::StridedMatrix{T}) where {T<:BlasFloat} = (A_ldiv_B!2(UpperTriangular(A[:R]), view(Ac_mul_B!2(A[:Q], B), 1:size(A, 2), 1:size(B, 2))); B)

# Julia implementation similar to xgelsy
function A_ldiv_B!2(A::QRPivoted{T}, B::StridedMatrix{T}, rcond::Real) where T<:BlasFloat
    mA, nA = size(A.factors)
    nr = min(mA,nA)
    nrhs = size(B, 2)
    if nr == 0
    ...
    A_ldiv_B!2(UpperTriangular(C[1:rnk,1:rnk]),view(Ac_mul_B!2(getq(A),view(B, 1:mA, 1:nrhs)),1:rnk,1:nrhs))
    B[rnk+1:end,:] = zero(T)
    LAPACK.ormrz!('L', eltype(B)<:Complex ? 'C' : 'T', C, τ, view(B,1:nA,1:nrhs))
    B[1:nA,:] = view(B, 1:nA, :)[invperm(A[:p]::Vector{BlasInt}),:]
    return B, rnk
end
A_ldiv_B!2(A::QRPivoted{T}, B::StridedVector{T}) where {T<:BlasFloat} =
    vec(A_ldiv_B!2(A,reshape(B,length(B),1)))
A_ldiv_B!2(A::QRPivoted{T}, B::StridedVecOrMat{T}) where {T<:BlasFloat} =
    A_ldiv_B!2(A, B, min(size(A)...)*eps(real(float(one(eltype(B))))))[1]
function A_ldiv_B!2(A::QR{T}, B::StridedMatrix{T}) where T
    m, n = size(A)
    minmn = min(m,n)
    mB, nB = size(B)
    Ac_mul_B!2(A[:Q], view(B, 1:m, :))
    R = A[:R]
    @inbounds begin
        ...
        Base.A_ldiv_B!2(UpperTriangular(view(R, :, 1:minmn)), view(B, 1:minmn, :))
...
A_ldiv_B!2(A::QR, B::StridedVector) = A_ldiv_B!2(A, reshape(B, length(B), 1))[:]
function A_ldiv_B!2(A::QRPivoted, b::StridedVector)
    A_ldiv_B!2(QR(A.factors,A.τ), b)
    b[1:size(A.factors, 2)] = view(b, 1:size(A.factors, 2))[invperm(A.jpvt)]
    b
end
function A_ldiv_B!2(A::QRPivoted, B::StridedMatrix)
    A_ldiv_B!2(QR(A.factors, A.τ), B)
    B[1:size(A.factors, 2),:] = view(B, 1:size(A.factors, 2), :)[invperm(A.jpvt),:]
    B
end
