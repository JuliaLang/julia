...
function getindex{T,S}(C::Cholesky{T,S}, d::Symbol)
    d == :U && return UpperTriangularSymbol(C.uplo) == d ? C.factors : C.factors')
    d == :L && return LowerTriangularSymbol(C.uplo) == d ? C.factors : C.factors')
    d == :UL && returnSymbol(C.uplo) == :U ? UpperTriangular(C.factors) : LowerTriangular(C.factors)
    throw(KeyError(d))
end
