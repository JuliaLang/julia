...
"""
    infer_eltype(collection)

Determine the type of the elements generated by iterating the specified collection
using type inference. This can be useful when `eltype` fails to return a concrete
type which could be possible to infer.

Note that since the function uses type inference to determine the type, it works on
a best-effort basis, which means that it is not guaranteed that it will return the
same result for the same collection when executed at different points of the program.
...
function infer_eltype(itr)
    T1, T2 = eltype(itr), Base.@default_eltype_tight(itr)
