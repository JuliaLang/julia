...
struct Format{S, T}
    ...
    arguments::Int  # Arguments required
end
...
function fmtfallback(buf, pos, arg, spec::Spec{T}) where {T}
    ...
    n2 = n + (expform ? 4 : 0) + (prec > 0 || hash) + (neg || (plus | space)) +
        (isf && pt >= n ? prec + 1 : 0)
    @show expform, pt, prec, n, n2, width
...
function format(io::IO, f::Format, args...) # => Nothing
    length(args) == f.arguments || argmismatch(length(args), length(f.formats))
