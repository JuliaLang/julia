...
function gen_d(flags::ASCIIString, width::Int, precision::Int, c::Char)
    ...
    prefix = ""
    if lowercase(c)=='o'
        f = '#' in flags ? :decode_0ct : :decode_oct
        fn = :(($f)($x))
    elseif c=='x'
        '#' in flags && (prefix = "0x")
        fn = :(decode_hex($x))
...
function gen_f(flags::ASCIIString, width::Int, precision::Int, c::Char)
    ...
    push!(blk.args, :((neg, pt, len) = fix_dec($x,$precision)))
    # calculate padding
    padding = nothing
...
function gen_e(flags::ASCIIString, width::Int, precision::Int, c::Char)
    ...
    push!(blk.args, :((neg, pt, len) = ini_dec($x,$ndigits)))
...
macro handle_zero(ex)
    quote
        if $(esc(ex)) == 0
            DIGITS[1] = '0'
            return ($(esc(:neg)), int32(1), int32(1))
        end
    end
end
...
function ini_dec(x::SmallFloatingPoint, n::Int)
    if x == 0.0
        ...
        return signbit(x), int32(1), int32(1)
    else
