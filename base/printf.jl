    ...
    for x in parse(s)
        if isa(x, AbstractString)
            ...
            f = c == 'f' ? gen_f :
                ...
                c == 'a' ? gen_a :
                c == 'g' ? gen_g :
                c == 'c' ? gen_c : c == 's' ? gen_s : c == 'p' ? gen_p : gen_d
...
function special_handler(flags::String, width::Int)
    ...
    pos = '+' in flags ? "+" : ' ' in flags ? " " : ""
    abn = quote
        isnan($x) ? $(pad("NaN", width)) :
        $x < 0 ? $(pad("-Inf", width)) : $(pad("$(pos)Inf", width))
    end
...
function gen_f(flags::String, width::Int, precision::Int, c::Char)
    ...
    push!(
        blk.args,
        :((do_out, args) = fix_dec(out, $x, $flags, $width, $precision, $c, buf))
    )
    ifblk = Expr(:if, :do_out, Expr(:block))
    push!(blk.args, ifblk)
    blk = ifblk.args[2]
...
function gen_e(flags::String, width::Int, precision::Int, c::Char, inside_g::Bool = false)
    ...
    push!(
        blk.args,
        :((do_out, args) = ini_dec(out, $x, $ndigits, $flags, $width, $precision, $c, buf))
    )
...
function gen_a(flags::String, width::Int, precision::Int, c::Char)
    ...
    if precision < 0
        push!(
            blk.args,
            :((do_out, args) = $fn(out, $x, $flags, $width, $precision, $c, buf))
        )
    else
        ndigits = min(precision + 1, length(Grisu.getbuf()) - 1)
        push!(
            blk.args,
            :((do_out, args) = $fn(out, $x, $ndigits, $flags, $width, $precision, $c, buf))
        )
    end
    push!(blk.args, :(digits = buf))
    ifblk = Expr(:if, :do_out, Expr(:block))
...
function gen_g(flags::String, width::Int, precision::Int, c::Char)
    ...
    push!(
        fblk.args,
        :((do_out, args) = fix_dec(out, $x, $flags, $width, fprec, $c - 1, buf))
    )
    ...
    if '+' in flags || ' ' in flags
        ...
        push!(blk.args, :(if neg
                width -= 1
            end))
    end
    push!(blk.args, :(if width >= 1
            padding = width
        end))
    # print space padding
...
decode_oct(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, decode_oct(d, digits))
decode_0ct(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, decode_0ct(d, digits))
decode_dec(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, decode_dec(d, digits))
decode_hex(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, decode_hex(d, digits))
decode_HEX(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, decode_HEX(d, digits))
fix_dec(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, fix_dec(d, precision, digits))
ini_dec(out, d, ndigits::Int, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, ini_dec(d, ndigits, digits))
ini_hex(out, d, ndigits::Int, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, ini_hex(d, ndigits, digits))
ini_HEX(out, d, ndigits::Int, flags::String, width::Int, precision::Int, c::Char, digits) =
    (true, ini_HEX(d, ndigits, digits))
ini_hex(out, d, flags::String, width::Int, precision::Int, c::Char, digits) =
...
ini_hex(x::Real, n::Int, symbols::AbstractArray{UInt8,1}, digits) =
    ini_hex(float(x), n, symbols, digits)
ini_hex(x::Real, symbols::AbstractArray{UInt8,1}, digits) =
...
#BigFloat
fix_dec(out, d::BigFloat, flags::String, width::Int, precision::Int, c::Char, digits) =
...
ini_hex(out, d::BigFloat, flags::String, width::Int, precision::Int, c::Char, digits) =
    bigfloat_printf(out, d, flags, width, precision, c, digits)
ini_HEX(out, d::BigFloat, flags::String, width::Int, precision::Int, c::Char, digits) =
    bigfloat_printf(out, d, flags, width, precision, c, digits)
function bigfloat_printf(
    out,
    d::BigFloat,
    flags::String,
    width::Int,
...
is_str_expr(ex) =
    ...
    (ex.head == :string ||
     (ex.head == :macrocall &&
      isa(ex.args[1], Symbol) && endswith(string(ex.args[1]), "str")))
