...
function map!(f, A::Union(BitArray,StridedArray))
    for i=1:length(A)
        A[i] = f(A[i])
    end
    return A
...
function map!(f, dest::BitArray, A::Union(StridedArray,BitArray))
    for i=1:length(A)
        dest[i] = f(A[i])
    end
    return dest
...
function map!(f, dest::BitArray, A::Union(StridedArray,BitArray), B::Union(StridedArray,BitArray))
    for i=1:length(A)
        dest[i] = f(A[i], B[i])
    end
    return dest
...
function map!(f, dest::BitArray, A::Union(StridedArray,BitArray), B::Number)
    for i=1:length(A)
        dest[i] = f(A[i], B)
    end
    return dest
...
function map!(f, dest::BitArray, A::Number, B::Union(StridedArray,BitArray))
    for i=1:length(B)
        dest[i] = f(A, B[i])
    end
    return dest
...
function map!(f, dest::BitArray, As::Union(StridedArray,BitArray)...)
    n = length(As[1])
    i = 1
    ith = a->a[i]
    for i=1:n
