...
function Float64(x::UInt128)
    ...
    if n <= 53
        ...
        y &= flipbits(UInt64(trailing_zeros(x) == (n-54))) # fix last bit to round to even
    end
    d = ((n+1022) % UInt64) << 52
...
function Float32(x::UInt128)
    ...
    if n <= 24
        ...
        y &= flipbits(UInt32(trailing_zeros(x) == (n-25))) # fix last bit to round to even
    end
    d = ((n+126) % UInt32) << 23
...
function Float32(val::Float16)
    ...
    if exp == 0
        if sig == 0
            ...
            sign = sign << 31
            exp = (-14 - n_bit + 127) << 23
            sig = ((sig & flipbits(bit)) << n_bit) << (23 - 10)
            ret = sign | exp | sig
        end
    elseif exp == 0x1f
        if sig == 0  # Inf
...
Base.iszero(x::Float16) = reinterpret(UInt16, x) & flipbits(sign_mask(Float16)) == 0x0000
