...
ccall(
    ...
    quote
        (f::typeof(Typeof))(x) =
            ($(_expr(:meta, :nospecialize, :x)); isa(x, Type) ? Type{x} : typeof(x))
    end
)
...
struct InexactError <: Exception
    ...
    T # Type
    val
    InexactError(f::Symbol, @nospecialize(T), @nospecialize(val)) =
        (@_noinline_meta; new(f, T, val))
end
...
mutable struct WeakRef
    ...
    WeakRef(@nospecialize(v)) =
        ccall(:jl_gc_new_weakref_th, Ref{WeakRef}, (Ptr{Cvoid}, Any), getptls(), v)
end
...
_new(typ::Symbol, argty::Symbol) =
    eval(Core, :($typ(@nospecialize n::$argty) = $(Expr(:new, typ, :n))))
_new(:GotoNode, :Int)
...
eval(
    Core,
    :(LineNumberNode(l::Int, @nospecialize(f)) = $(Expr(:new, :LineNumberNode, :l, :f)))
)
...
eval(
    Core,
    :(PhiNode(edges::Array{Any,1}, values::Array{Any,1}) =
        $(Expr(:new, :PhiNode, :edges, :values)))
)
...
eval(
    Core,
    :(LineInfoNode(@nospecialize(method), file::Symbol, line::Int, inlined_at::Int) =
        $(Expr(:new, :LineInfoNode, :method, :file, :line, :inlined_at)))
)

Module(name::Symbol = :anonymous, std_imports::Bool = true) =
...
Array{T,1}(::UndefInitializer, d::NTuple{1,Int}) where {T} =
    Array{T,1}(undef, getfield(d, 1))
Array{T,2}(::UndefInitializer, d::NTuple{2,Int}) where {T} =
    Array{T,2}(undef, getfield(d, 1), getfield(d, 2))
Array{T,3}(::UndefInitializer, d::NTuple{3,Int}) where {T} =
...
eval(
    Core,
    :(NamedTuple{names,T}(args::T) where {names,T<:Tuple} =
        $(Expr(:splatnew, :(NamedTuple{names,T}), :args)))
)

# constructors for built-in types

import .Intrinsics: eq_int,
                    trunc_int,
                    lshr_int,
                    sub_int,
                    shl_int,
...
function is_top_bit_set(x)
    @_inline_meta
    eq_int(
        trunc_int(UInt8, lshr_int(x, sub_int(shl_int(sizeof(x), 3), 1))),
        trunc_int(UInt8, 1)
    )
end
