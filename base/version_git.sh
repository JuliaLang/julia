...
fork_master_distance=$(git rev-list HEAD ^"$(echo $origin)release-0.5" | wc -l | sed -e 's/[^[:digit:]]//g')
fork_master_timestamp=$(git show -s $(git merge-base HEAD $(echo $origin)release-0.5) --format=format:"%ct")
