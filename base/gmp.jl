...
function __init__()
    try
        if version().major != VERSION.major || bits_per_limb() != BITS_PER_LIMB
            ...
            msg(
                "The dynamically loaded GMP library (v\"$(version())\" with __gmp_bits_per_limb == $(bits_per_limb()))\n",
    ...
    try
        ccall(
            (:__gmp_set_alloc_overflow_function, :libgmp),
            Cvoid,
            (Ptr{Cvoid},),
...
sizeinbase(a::BigInt, b) =
    Int(ccall((:__gmpz_sizeinbase, :libgmp), Csize_t, (mpz_t, Cint), a, b))

for (op, nbits) in (
    :add => :(BITS_PER_LIMB * (1 + max(abs(a.size), abs(b.size)))),
    ...
    @eval begin
        $op!(x::BigInt, a::BigInt, b::BigInt) =
            (ccall($(gmpz(op)), Cvoid, (mpz_t, mpz_t, mpz_t), x, a, b); x)
...
for op in (:add_ui, :sub_ui, :mul_ui, :mul_2exp, :fdiv_q_2exp, :pow_ui, :bin_ui)
    ...
    @eval begin
        $op!(x::BigInt, a::BigInt, b) =
            (ccall($(gmpz(op)), Cvoid, (mpz_t, mpz_t, Culong), x, a, b); x)
        $op(a::BigInt, b) = $op!(BigInt(), a, b)
        $op!(x::BigInt, b) = $op!(x, x, b)
    end
end

ui_sub!(x::BigInt, a, b::BigInt) =
    (ccall((:__gmpz_ui_sub, :libgmp), Cvoid, (mpz_t, Culong, mpz_t), x, a, b); x)
...
mul_si!(x::BigInt, a::BigInt, b) =
    (ccall((:__gmpz_mul_si, :libgmp), Cvoid, (mpz_t, mpz_t, Clong), x, a, b); x)
...
mpn_popcount(d::Ptr{Limb}, s::Integer) =
...
function tryparse_internal(
    ...
    bstr = startpos == firstindex(s) && endpos == lastindex(s) ? String(s) :
           String(SubString(s, startpos, endpos))

    sgn, base, i = Base.parseint_preamble(
        true,
        Int(base_),
        bstr,
        firstindex(bstr),
...
function Float64(x::BigInt, ::RoundingMode{:Nearest})
    ...
    elseif Limb == UInt32 && xsize == 2
        ...
        if Limb == UInt64
            ...
            y += n > (precision(Float64) - 32) ? 0 :
                 (unsafe_load(x.d, xsize - 2) >> (10 + n))
        end
...
function Float32(x::BigInt, ::RoundingMode{:Nearest})
    ...
    elseif xsize == 1
        ...
        y += (n > precision(Float32) ? 0 :
              unsafe_load(x.d, xsize - 1) >> (BITS_PER_LIMB - (25 - n))) % UInt32
...
for (fJ, fC) in ((:+, :add), (:*, :mul), (:&, :and), (:|, :ior), (:xor, :xor))
    ...
    @eval begin
        ...
        ($fJ)(a::BigInt, b::BigInt, c::BigInt, d::BigInt) =
            MPZ.$fC!(MPZ.$fC!(MPZ.$fC(a, b), c), d)
...
function bigint_pow(x::BigInt, y::Integer)
    if y < 0
        throw(DomainError(y, "`y` cannot be negative."))
    end
    @noinline throw1(y) =
