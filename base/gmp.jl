...
using .Base.JLLAdapters: get_libgmp

version() = VersionNumber(unsafe_string(unsafe_load(cglobal((:__gmp_version, get_libgmp()), Ptr{Cchar}))))
bits_per_limb() = Int(unsafe_load(cglobal((:__gmp_bits_per_limb, get_libgmp()), Cint)))
...
mutable struct BigInt <: Signed
    ...
    function BigInt(; nbits::Integer=0)
        ...
        finalizer(cglobal((:__gmpz_clear, get_libgmp())), b)
        return b
    end
end
...
function __init__()
    # Resuscitate GMP.ONE object
    ONE.alloc, ONE.size, ONE.d = 1, 1, pointer(_ONE)
end

function libgmp_init()
    try
        ...
        ccall((:__gmp_set_memory_functions, get_libgmp()), Cvoid,
    ...
    try
        ccall((:__gmp_set_alloc_overflow_function, get_libgmp()), Cvoid,
              (Ptr{Cvoid},),
...
using ..GMP: BigInt, Limb, BITS_PER_LIMB, get_libgmp

const mpz_t = Ref{BigInt}
const bitcnt_t = Culong

gmpz(op::Symbol) = (Symbol(:__gmpz_, op), get_libgmp())

init!(x::BigInt) = (ccall((:__gmpz_init, get_libgmp()), Cvoid, (mpz_t,), x); x)
init2!(x::BigInt, a) = (ccall((:__gmpz_init2, get_libgmp()), Cvoid, (mpz_t, bitcnt_t), x, a); x)

realloc2!(x, a) = (ccall((:__gmpz_realloc2, get_libgmp()), Cvoid, (mpz_t, bitcnt_t), x, a); x)
realloc2(a) = realloc2!(BigInt(), a)

sizeinbase(a::BigInt, b) = Int(ccall((:__gmpz_sizeinbase, get_libgmp()), Csize_t, (mpz_t, Cint), a, b))
...
invert!(x::BigInt, a::BigInt, b::BigInt) =
    ccall((:__gmpz_invert, get_libgmp()), Cint, (mpz_t, mpz_t, mpz_t), x, a, b)
...
ui_sub!(x::BigInt, a, b::BigInt) = (ccall((:__gmpz_ui_sub, get_libgmp()), Cvoid, (mpz_t, Culong, mpz_t), x, a, b); x)
...
mul_si!(x::BigInt, a::BigInt, b) = (ccall((:__gmpz_mul_si, get_libgmp()), Cvoid, (mpz_t, mpz_t, Clong), x, a, b); x)
...
popcount(a::BigInt) = Int(signed(ccall((:__gmpz_popcount, get_libgmp()), Culong, (mpz_t,), a)))

mpn_popcount(d::Ptr{Limb}, s::Integer) = Int(ccall((:__gmpn_popcount, get_libgmp()), Culong, (Ptr{Limb}, Csize_t), d, s))
mpn_popcount(a::BigInt) = mpn_popcount(a.d, abs(a.size))

function tdiv_qr!(x::BigInt, y::BigInt, a::BigInt, b::BigInt)
    ccall((:__gmpz_tdiv_qr, get_libgmp()), Cvoid, (mpz_t, mpz_t, mpz_t, mpz_t), x, y, a, b)
    x, y
end
tdiv_qr(a::BigInt, b::BigInt) = tdiv_qr!(BigInt(), BigInt(), a, b)

powm!(x::BigInt, a::BigInt, b::BigInt, c::BigInt) =
    (ccall((:__gmpz_powm, get_libgmp()), Cvoid, (mpz_t, mpz_t, mpz_t, mpz_t), x, a, b, c); x)
...
function gcdext!(x::BigInt, y::BigInt, z::BigInt, a::BigInt, b::BigInt)
    ccall((:__gmpz_gcdext, get_libgmp()), Cvoid, (mpz_t, mpz_t, mpz_t, mpz_t, mpz_t), x, y, z, a, b)
    x, y, z
end
gcdext(a::BigInt, b::BigInt) = gcdext!(BigInt(), BigInt(), BigInt(), a, b)

cmp(a::BigInt, b::BigInt) = Int(ccall((:__gmpz_cmp, get_libgmp()), Cint, (mpz_t, mpz_t), a, b))
cmp_si(a::BigInt, b) = Int(ccall((:__gmpz_cmp_si, get_libgmp()), Cint, (mpz_t, Clong), a, b))
cmp_ui(a::BigInt, b) = Int(ccall((:__gmpz_cmp_ui, get_libgmp()), Cint, (mpz_t, Culong), a, b))
cmp_d(a::BigInt, b) = Int(ccall((:__gmpz_cmp_d, get_libgmp()), Cint, (mpz_t, Cdouble), a, b))

mpn_cmp(a::Ptr{Limb}, b::Ptr{Limb}, c) = ccall((:__gmpn_cmp, get_libgmp()), Cint, (Ptr{Limb}, Ptr{Limb}, Clong), a, b, c)
mpn_cmp(a::BigInt, b::BigInt, c) = mpn_cmp(a.d, b.d, c)

get_str!(x, a, b::BigInt) = (ccall((:__gmpz_get_str,get_libgmp()), Ptr{Cchar}, (Ptr{Cchar}, Cint, mpz_t), x, a, b); x)
set_str!(x::BigInt, a, b) = Int(ccall((:__gmpz_set_str, get_libgmp()), Cint, (mpz_t, Ptr{UInt8}, Cint), x, a, b))
get_d(a::BigInt) = ccall((:__gmpz_get_d, get_libgmp()), Cdouble, (mpz_t,), a)
...
function export!(a::AbstractVector{T}, n::BigInt; order::Integer=-1, nails::Integer=0, endian::Integer=0) where {T<:Base.BitInteger}
    ...
    count = Ref{Csize_t}()
    ccall((:__gmpz_export, get_libgmp()), Ptr{T}, (Ptr{T}, Ref{Csize_t}, Cint, Csize_t, Cint, Csize_t, mpz_t),
        a, count, order, sizeof(T), endian, nails, n)
    @assert count[] ≤ length(a)
    return a, Int(count[])
end

limbs_write!(x::BigInt, a) = ccall((:__gmpz_limbs_write, get_libgmp()), Ptr{Limb}, (mpz_t, Clong), x, a)
limbs_finish!(x::BigInt, a) = ccall((:__gmpz_limbs_finish, get_libgmp()), Cvoid, (mpz_t, Clong), x, a)
import!(x::BigInt, a, b, c, d, e, f) = ccall((:__gmpz_import, get_libgmp()), Cvoid,
    (mpz_t, Csize_t, Cint, Csize_t, Cint, Csize_t, Ptr{Cvoid}), x, a, b, c, d, e, f)

setbit!(x, a) = (ccall((:__gmpz_setbit, get_libgmp()), Cvoid, (mpz_t, bitcnt_t), x, a); x)
tstbit(a::BigInt, b) = ccall((:__gmpz_tstbit, get_libgmp()), Cint, (mpz_t, bitcnt_t), a, b) % Bool
...
import ..GMP: BigInt, MPZ, Limb, isneg, get_libgmp

gmpq(op::Symbol) = (Symbol(:__gmpq_, op), get_libgmp())
...
function Rational{BigInt}(num::BigInt, den::BigInt)
    ...
    ccall((:__gmpq_canonicalize, get_libgmp()), Cvoid, (mpq_t,), xq)
...
function set!(z::Rational{BigInt}, x::Rational{BigInt})
    zq = _MPQ(z)
    ccall((:__gmpq_set, get_libgmp()), Cvoid, (mpq_t, mpq_t), zq, _MPQ(x))
    return sync_rational!(zq)
end

function set_z!(z::Rational{BigInt}, x::BigInt)
    zq = _MPQ(z)
    ccall((:__gmpq_set_z, get_libgmp()), Cvoid, (mpq_t, MPZ.mpz_t), zq, x)
    return sync_rational!(zq)
end
...
function add!(z::Rational{BigInt}, x::Rational{BigInt}, y::Rational{BigInt})
    ...
    zq = _MPQ(z)
    ccall((:__gmpq_add, get_libgmp()), Cvoid,
...
function sub!(z::Rational{BigInt}, x::Rational{BigInt}, y::Rational{BigInt})
    ...
    zq = _MPQ(z)
    ccall((:__gmpq_sub, get_libgmp()), Cvoid,
          (mpq_t,mpq_t,mpq_t), zq, _MPQ(x), _MPQ(y))
    return sync_rational!(zq)
end
...
function mul!(z::Rational{BigInt}, x::Rational{BigInt}, y::Rational{BigInt})
    ...
    zq = _MPQ(z)
    ccall((:__gmpq_mul, get_libgmp()), Cvoid,
          (mpq_t,mpq_t,mpq_t), zq, _MPQ(x), _MPQ(y))
    return sync_rational!(zq)
end
...
function div!(z::Rational{BigInt}, x::Rational{BigInt}, y::Rational{BigInt})
    ...
    zq = _MPQ(z)
    ccall((:__gmpq_div, get_libgmp()), Cvoid,
          (mpq_t,mpq_t,mpq_t), zq, _MPQ(x), _MPQ(y))
    return sync_rational!(zq)
end
...
function Base.cmp(x::Rational{BigInt}, y::Rational{BigInt})
    Int(ccall((:__gmpq_cmp, get_libgmp()), Cint, (mpq_t, mpq_t), _MPQ(x), _MPQ(y)))
end
