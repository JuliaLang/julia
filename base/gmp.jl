...
function Base.parse(::Type{BigInt}, s::AbstractString; base::Union{Nothing,Integer}=nothing)
    Base.Parse.check_valid_base(base, true) || return nothing
...
function parse_bigint(s::AbstractString, base::Integer, raise)
    ...
    c, sgn, base, state = Base.Parse.parseint_preamble(s, #=signed=# true, base)
    Base.Parse.check_valid_base(base, raise) || return nothing
