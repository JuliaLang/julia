...
# the cancellation point that the annotation emits establishes a reset region
# that stays published across the foreign call, so an asynchronous
# cancellation (e.g. ^C) can unwind out of a long-running GMP computation at
# `CANCEL_REQUEST_SAFE`. GMP 6.3.0 is audited for this: its core has no
...
for (op, nbits) in (:add => :(BITS_PER_LIMB*(1 + max(abs(a.size), abs(b.size)))),
    ...
    @eval begin
        $op!(x::BigInt, a::BigInt, b::BigInt) =
            (@ccall(reset_safe=true, libgmp.$fname(x::mpz_t, a::mpz_t, b::mpz_t)::Cvoid); x)
...
invert!(x::BigInt, a::BigInt, b::BigInt) =
    @ccall reset_safe=true libgmp.__gmpz_invert(x::mpz_t, a::mpz_t, b::mpz_t)::Cint
...
for op in (:add_ui, :sub_ui, :mul_ui, :mul_2exp, :fdiv_q_2exp, :pow_ui, :bin_ui)
    ...
    @eval begin
        $op!(x::BigInt, a::BigInt, b) =
            (@ccall(reset_safe=true, libgmp.$fname(x::mpz_t, a::mpz_t, b::Culong)::Cvoid); x)
        $op(a::BigInt, b) = $op!(BigInt(), a, b)
        $op!(x::BigInt, b) = $op!(x, x, b)
    end
end

ui_sub!(x::BigInt, a, b::BigInt) = (@ccall(reset_safe=true, libgmp.__gmpz_ui_sub(x::mpz_t, a::Culong, b::mpz_t)::Cvoid); x)
...
mul_si!(x::BigInt, a::BigInt, b) = (@ccall(reset_safe=true, libgmp.__gmpz_mul_si(x::mpz_t, a::mpz_t, b::Clong)::Cvoid); x)
...
for op in (:neg, :com, :sqrt, :set)
    ...
    @eval begin
        $op!(x::BigInt, a::BigInt) =
            (@ccall(reset_safe=true, libgmp.$fname(x::mpz_t, a::mpz_t)::Cvoid); x)
...
fac_ui!(x::BigInt, a) = (@ccall(reset_safe=true, libgmp.__gmpz_fac_ui(x::mpz_t, a::Culong)::Cvoid); x)
...
powm!(x::BigInt, a::BigInt, b::BigInt, c::BigInt) =
    (@ccall(reset_safe=true, libgmp.__gmpz_powm(x::mpz_t, a::mpz_t, b::mpz_t, c::mpz_t)::Cvoid); x)
...
get_str!(x, a, b::BigInt) = (@ccall(reset_safe=true, libgmp.__gmpz_get_str(x::Ptr{Cchar}, a::Cint, b::mpz_t)::Ptr{Cchar}); x)
set_str!(x::BigInt, a, b) = Int(@ccall(reset_safe=true, libgmp.__gmpz_set_str(x::mpz_t, a::Ptr{UInt8}, b::Cint)::Cint))
