...
struct PermutedDimsArray{T,N,perm,iperm,AA<:AbstractArray} <: AbstractArray{T,N}
    ...
    function PermutedDimsArray{T,N,perm,iperm,AA}(data::AA) where {
        T,
        N,
        perm,
        iperm,
        ...
        isperm(perm) || throw(ArgumentError(string(
            perm,
            " is not a valid permutation of dimensions 1:",
            N
        )))
        all(map(d -> iperm[perm[d]] == d, 1:N)) || throw(ArgumentError(string(
            perm,
            " and ",
            iperm,
            " must be inverses"
...
function PermutedDimsArray(data::AbstractArray{T,N}, perm) where {T,N}
    length(perm) == N || throw(ArgumentError(string(
        perm,
        " is not a valid permutation of dimensions 1:",
        N
    )))
...
Base.unsafe_convert(::Type{Ptr{T}}, A::PermutedDimsArray{T}) where {T} =
...
# or a linear index?
Base.pointer(A::PermutedDimsArray, i::Integer) =
...
@inline function Base.getindex(
    A::PermutedDimsArray{T,N,perm,iperm},
    I::Vararg{Int,N}
