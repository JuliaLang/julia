...
const ViewIndex = Union{Real, ArrayLike}
...
function SubArray(parent::ArrayLike, indices::Tuple)
    @_inline_meta
...
```
"""
parentindices(a::ArrayLike) = map(OneTo, size(a))
...
_trimmedindex(i::ArrayLike) = oftype(i, reshape(eachindex(IndexLinear(), i), axes(i)))
...
_maybe_reshape_parent(A::ArrayLike, ::NTuple{1, Bool}) = reshape(A, Val(1))
...
_maybe_reshape_parent(A::ArrayLike, ::NTuple{N, Bool}) where {N} = reshape(A, Val(N))
"""
    view(A, inds...)
...
```
"""
function view(A::ArrayLike, I::Vararg{Any,N}) where {N}
    @_inline_meta
...
function unsafe_view(A::ArrayLike, I::Vararg{ViewIndex,N}) where {N}
    @_inline_meta
    SubArray(A, I)
end
...
reindex(idxs::Tuple{ArrayLike{1}, Vararg{Any}}, subidxs::Tuple{Any, Vararg{Any}}) =
...
reindex(idxs::Tuple{ArrayLike{2}, Vararg{Any}}, subidxs::Tuple{Any, Any, Vararg{Any}}) =
    (@_propagate_inbounds_meta; (idxs[1][subidxs[1], subidxs[2]], reindex(tail(idxs), tail(tail(subidxs)))...))

# In general, we index N-dimensional parent arrays with N indices
@generated function reindex(idxs::Tuple{ArrayLike{N}, Vararg{Any}}, subidxs::Tuple{Vararg{Any}}) where {N}
...
compute_stride1(parent::ArrayLike, I::NTuple{N,Any}) where {N} =
...
compute_offset1(parent::ArrayLike{1}, stride1::Integer, I::Tuple{AbstractRange}) =
...
_indices_sub() = ()
function _indices_sub(i1::ArrayLike, I...)
    @_inline_meta
