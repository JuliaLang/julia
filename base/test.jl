...
Pass(test_type, orig_expr, expr, value) = Pass(test_type, orig_expr, expr, value, "")
...
Fail(test_type, orig_expr, expr, value) = Fail(test_type, orig_expr, expr, value, "")
...
Error(test_type, orig_expr, expr, value) = Error(test_type, orig_expr, expr, value, "")
...
`false`, and an `Error` `Result` if it could not be evaluated. An
optional message can be included to give context to the conditions
being tested.
"""
macro test(ex, message::AbstractString = "")
    ...
    :(do_test($result, $orig_ex, $message))
end
...
function do_test(result::ExecutionResult, orig_expr, message::AbstractString)
    ...
    if isa(result, Returned)
        ...
        testres = if isa(value, Bool)
            value ? Pass(:test, orig_expr, expr, value, message) :
                    Fail(:test, orig_expr, expr, value, message)
        else
            # If the result is non-Boolean, this counts as an Error
            Error(:test_nonbool, orig_expr, value, nothing, message)
        end
    else
        # The predicate couldn't be evaluated without throwing an
        # exception, so that is an Error and not a Fail
        @assert isa(result, Threw)
        testres = Error(:test_error, orig_expr, result.exception, result.backtrace, message)
    end
...
"""
macro test_throws(extype, ex, message::AbstractString = "")
    orig_ex = Expr(:quote,ex)
    result = quote
        try
            Returned($(esc(ex)))
    ...
    Base.remove_linenums!(result)
    :(do_test_throws($result, $orig_ex, $(esc(extype)), $message))
end

# An internal function, called by the code generated by @test_throws
# to evaluate and catch the thrown exception - if it exists
function do_test_throws(result::ExecutionResult, orig_expr, extype, message::AbstractString)
    if isa(result, Threw)
        # Check the right type of exception was thrown
        if isa(result.exception, extype)
            testres = Pass(:test_throws, orig_expr, extype, result.exception, message)
        else
            testres = Fail(:test_throws_wrong, orig_expr, extype, result.exception, message)
        end
    else
        testres = Fail(:test_throws_nothing, orig_expr, extype, nothing, message)
    end
    record(get_testset(), testres)
end
