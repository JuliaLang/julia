...
# @test_fail_expected - check if the expression evaluates to false
"""
...
macro test_fail_expected(ex)
    ...
    :(do_test($result, $orig_ex, fail_expected=true))
end
...
function do_test(result::ExecutionResult, orig_expr; fail_expected::Bool=false)
    ...
    if isa(result, Returned)
        ...
        testres = if isa(value, Bool)
            ...
            # a false value Passes
                value ? Fail(:test, orig_expr, result.data, value) :
                        Pass(:test, orig_expr, result.data, value)
            end
        else
