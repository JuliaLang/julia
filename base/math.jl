...
function ldexp(x::T, e::Integer) where T<:IEEEFloat
    ...
    if k > 0 # normal case
        xu = (xu & flipbits(exponent_mask(T))) | (rem(k, uinttype(T)) << significand_bits(T))
...
function exponent(x::T) where T<:IEEEFloat
    ...
    xs = reinterpret(Unsigned, x) & flipbits(sign_mask(T))
...
function significand(x::T) where T<:IEEEFloat
    ...
    xs = xu & flipbits(sign_mask(T))
    xs >= exponent_mask(T) && return x # NaN or Inf
    if xs <= (flipbits(exponent_mask(T)) & flipbits(sign_mask(T))) # x is subnormal
        xs == 0 && return x # +-0
        m = unsigned(leading_zeros(xs) - exponent_bits(T))
        xs <<= m
        xu = xs | (xu & sign_mask(T))
    end
    xu = (xu & flipbits(exponent_mask(T))) | exponent_one(T)
    return reinterpret(T, xu)
end
...
function frexp(x::T) where T<:IEEEFloat
    ...
    k -= (exponent_bias(T) - 1)
    xu = (xu & flipbits(exponent_mask(T))) | exponent_half(T)
