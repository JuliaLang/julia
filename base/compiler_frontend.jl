...
#
# Look up compiler frontend for `versionctx` which should be a version number,
# module to look up the frontend in, or `nothing` to get the default compiler
# for this version of julia.
function compiler_frontend(mod::Union{Nothing,Module} = nothing)
    m = mod !== nothing && isdefined(mod, _frontend_var_name) ? mod : Base
...
function _parse_hook(code::Union{AbstractString,Core.SimpleVector}, filename::String, first_line::Int,
                     offset::Int, rule::Symbol, versionctx::Union{Nothing,Module,VersionNumber})
