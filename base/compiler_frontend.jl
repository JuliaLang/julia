...
* `version` - Use the default system compiler at syntax `version`
...
function compiler_frontend(mod::Module)
    if isdefined(mod, _frontend_var_name)
        return unwrap_frontend(getglobal(mod, _frontend_var_name))::AbstractCompilerFrontend
    else
        return compiler_frontend()
    end
end
