...
@noinline __throw_rational_argerror(T) =
...
Rational{T}(x::Rational) where {T<:Integer} =
    Rational{T}(convert(T, x.num), convert(T, x.den))
Rational{T}(x::Integer) where {T<:Integer} = Rational{T}(convert(T, x), convert(T, 1))

Rational(x::Rational) = x

Bool(x::Rational) = x == 0 ? false : x == 1 ? true : throw(InexactError(:Bool, Bool, x)) # to resolve ambiguity
(::Type{T})(x::Rational) where {T<:Integer} =
    (isinteger(x) ? convert(T, x.num) : throw(InexactError(nameof(T), T, x)))
...
rationalize(::Type{T}, x::AbstractFloat; tol::Real = eps(x)) where {T<:Integer} =
...
for (op, chop) in ((:+, :checked_add), (:-, :checked_sub), (:rem, :rem), (:mod, :mod))
    @eval begin
        function ($op)(x::Rational, y::Rational)
            xd, yd = divgcd(x.den, y.den)
            Rational(
                ($chop)(checked_mul(x.num, yd), checked_mul(y.num, xd)),
                checked_mul(x.den, yd)
            )
        end
    end
end
...
<(x::Rational, y::Rational) =
    x.den == y.den ? x.num < y.num : widemul(x.num, y.den) < widemul(x.den, y.num)
<=(x::Rational, y::Rational) =
    x.den == y.den ? x.num <= y.num : widemul(x.num, y.den) <= widemul(x.den, y.num)
...
for rel in (:<, :<=, :cmp)
    for (Tx, Ty) in ((Rational, AbstractFloat), (AbstractFloat, Rational))
        @eval function ($rel)(x::$Tx, y::$Ty)
            if isnan(x)
                $(rel == :cmp ? :(return isnan(y) ? 0 : 1) : :(return false))
            end
            if isnan(y)
                $(rel == :cmp ? :(return -1) : :(return false))
            end
...
round(::Type{T}, x::Rational, r::RoundingMode = RoundNearest) where {T} =
...
function _round_rational(::Type{T}, x::Rational{Tr}, ::RoundingMode{:Nearest}) where {T,Tr}
    ...
    if abs(r) >= abs((denominator(x) - copysign(Tr(4), numerator(x)) + one(Tr) +
                      iseven(q)) >> 1 + copysign(Tr(2), numerator(x)))
...
function _round_rational(
    ...
    q, r = divrem(numerator(x), denominator(x))
    s = q
    if abs(r) >= abs((denominator(x) - copysign(Tr(4), numerator(x)) + one(Tr)) >> 1 +
                     copysign(Tr(2), numerator(x)))
        s += copysign(one(Tr), numerator(x))
    end
    convert(T, s)
end
...
function _round_rational(
    ...
    s = q
    if abs(r) >= abs((denominator(x) - copysign(Tr(4), numerator(x)) + one(Tr) +
                      (numerator(x) < 0)) >> 1 + copysign(Tr(2), numerator(x)))
        s += copysign(one(Tr), numerator(x))
    end
    convert(T, s)
end
