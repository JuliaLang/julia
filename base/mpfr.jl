...
type MPFRFloat{N} <: FloatingPoint
    mpfr::mpfr_struct
    function MPFRFloat()
        if N < 2
            error("Invalid precision")
        end
        z = mpfr_struct(convert(Clong, 0), convert(Cint, 0), convert(Clong, 0), C_NULL)
        ccall((:mpfr_init2,:libmpfr), Void, (Ptr{mpfr_struct}, Int), &z, N)
        b = new(z)
...
function MPFRFloat(x::MPFRFloat)
    ...
    ccall((:mpfr_set, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
for (fJ, fC) in ((:si,:Int), (:ui,:Uint), (:d,:Float64))
    @eval begin
        function MPFRFloat(x::($fC))
            z = MPFRFloat{DEFAULT_PRECISION[end]}()
            ccall(($(string(:mpfr_set_,fJ)), :libmpfr), Int32, (Ptr{mpfr_struct}, ($fC), Int32), &(z.mpfr), x, ROUNDING_MODE[end])
            return z
        end
    end
end
...
function MPFRFloat(x::BigFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_set_f, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{Void}, Int32), &(z.mpfr), x.mpf, ROUNDING_MODE[end])   
    return z
end

function MPFRFloat(x::String, base::Int)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    err = ccall((:mpfr_set_str, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{Uint8}, Int32, Int32), &(z.mpfr), x, base, ROUNDING_MODE[end])
...
convert(::Type{Int64}, x::MPFRFloat) = integer_valued(x) ?
    ccall((:mpfr_get_si,:libmpfr), Int64, (Ptr{mpfr_struct}, Int32), &(x.mpfr), ROUNDING_MODE[end]) :
    throw(InexactError())
convert(::Type{Int32}, x::MPFRFloat) = int32(convert(Int64, x))
convert(::Type{Uint64}, x::MPFRFloat) = integer_valued(x) ?
    ccall((:mpfr_get_ui,:libmpfr), Uint64, (Ptr{mpfr_struct}, Int32), &(x.mpfr), ROUNDING_MODE[end]) :
    throw(InexactError())
...
convert(::Type{Float64}, x::MPFRFloat) = ccall((:mpfr_get_d,:libmpfr), Float64, (Ptr{mpfr_struct},), &(x.mpfr))
convert(::Type{Float32}, x::MPFRFloat) = ccall((:mpfr_get_flt,:libmpfr), Float32, (Ptr{mpfr_struct},), &(x.mpfr))
...
for (fJ, fC) in ((:+,:add), (:-,:sub), (:*,:mul), (:/,:div), (:^, :pow))
    @eval begin 
        function ($fJ)(x::MPFRFloat, y::MPFRFloat)
            z = MPFRFloat{DEFAULT_PRECISION[end]}()
            ccall(($(string(:mpfr_,fC)),:libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), &(y.mpfr), ROUNDING_MODE[end])
            return z
        end
    end
end
...
function -(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_neg, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function cmp(x::MPFRFloat, y::MPFRFloat)
    ccall((:mpfr_cmp, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}), &(x.mpfr), &(y.mpfr))
end

function sqrt(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_sqrt, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    if isnan(z)
        throw(DomainError())
    end
    return z
...
for f in (:ceil, :floor, :trunc)
    @eval begin
        function ($f)(x::MPFRFloat)
            z = MPFRFloat{DEFAULT_PRECISION[end]}()
            ccall(($(string(:mpfr_,f)), :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}), &(z.mpfr), &(x.mpfr))
            return z
        end
    end
end
...
function ^(x::MPFRFloat, y::Uint)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_pow_ui, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Uint, Int32), &(z.mpfr), &(x.mpfr), y, ROUNDING_MODE[end])
    return z
end

function ^(x::MPFRFloat, y::Int)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_pow_si, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int, Int32), &(z.mpfr), &(x.mpfr), y, ROUNDING_MODE[end])
    return z
end
...
function exp(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_exp, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function exp2(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_exp2, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function exp10(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_exp10, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function besselj0(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_j0, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function besselj1(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_j1, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
function besselj(nu::Integer, x::MPFRFloat)
    n = int64(nu)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_jn, :libmpfr), Int32, (Ptr{mpfr_struct}, Int64, Ptr{mpfr_struct}, Int32), &(z.mpfr), n, &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function bessely0(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_y0, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function bessely1(x::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_y1, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
function bessely(nu::Integer, x::MPFRFloat)
    n = int64(nu)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_yn, :libmpfr), Int32, (Ptr{mpfr_struct}, Int64, Ptr{mpfr_struct}, Int32), &(z.mpfr), n, &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
function factorial(x::MPFRFloat)
    ...
    ui = uint64(x)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_fac_ui, :libmpfr), Int32, (Ptr{mpfr_struct}, Uint64, Int32), &(z.mpfr), ui, ROUNDING_MODE[end])
    return z
end
...
function log(x::MPFRFloat)
    ...
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_log, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
function log2(x::MPFRFloat)
    ...
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_log2, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end
...
function log10(x::MPFRFloat)
    ...
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_log10, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return z
end

function max(x::MPFRFloat, y::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_max, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), &(y.mpfr), ROUNDING_MODE[end])
    return z
end

function min(x::MPFRFloat, y::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_min, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), &(y.mpfr), ROUNDING_MODE[end])
    return z
end
...
function modf(x::MPFRFloat)
    ...
    ccall((:mpfr_modf, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(zint.mpfr), &(zfloat.mpfr), &(x.mpfr), ROUNDING_MODE[end])
    return (zfloat, zint)
end

function rem(x::MPFRFloat, y::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_remainder, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), &(y.mpfr), ROUNDING_MODE[end])
    return z
end
...
#         (Ptr{mpfr_struct}, Ptr{Void}, Uint, Int32), 
...
for f in (:sin,:cos,:tan,:sec,:csc,:cot,:acos,:asin,:atan,
    ...
    @eval begin
        function ($f)(x::MPFRFloat)
            z = MPFRFloat{DEFAULT_PRECISION[end]}()
            ccall(($(string(:mpfr_,f)), :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), ROUNDING_MODE[end])
            if isnan(z)
                throw(DomainError())
            end
            return z
...
# Utility functions
==(x::MPFRFloat, y::MPFRFloat) = ccall((:mpfr_equal_p, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}), &(x.mpfr), &(y.mpfr)) != 0
<=(x::MPFRFloat, y::MPFRFloat) = ccall((:mpfr_lessequal_p, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}), &(x.mpfr), &(y.mpfr)) != 0
>=(x::MPFRFloat, y::MPFRFloat) = ccall((:mpfr_greaterequal_p, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}), &(x.mpfr), &(y.mpfr)) != 0
...
function copysign(x::MPFRFloat, y::MPFRFloat)
    z = MPFRFloat{DEFAULT_PRECISION[end]}()
    ccall((:mpfr_copysign, :libmpfr), Int32, (Ptr{mpfr_struct}, Ptr{mpfr_struct}, Ptr{mpfr_struct}, Int32), &(z.mpfr), &(x.mpfr), &(y.mpfr), ROUNDING_MODE[end])
    return z
end
...
function exponent(x::MPFRFloat)
    ...
    return ccall((:mpfr_get_exp, :libmpfr), Int, (Ptr{mpfr_struct},), &(x.mpfr)) - 1
end

function integer_valued(x::MPFRFloat)
    return ccall((:mpfr_integer_p, :libmpfr), Int32, (Ptr{mpfr_struct},), &(x.mpfr)) != 0
end

function iround(x::MPFRFloat)
    fits = ccall((:mpfr_fits_slong_p, :libmpfr), Int32, (Ptr{mpfr_struct}, Int32), &(x.mpfr), ROUNDING_MODE[end])
    if fits != 0
...
function isinf(x::MPFRFloat)
    return ccall((:mpfr_inf_p, :libmpfr), Int32, (Ptr{mpfr_struct},), &(x.mpfr)) != 0
end

function isnan(x::MPFRFloat)
    return ccall((:mpfr_nan_p, :libmpfr), Int32, (Ptr{mpfr_struct},), &(x.mpfr)) != 0
end

function nextfloat(x::MPFRFloat)
    z = MPFRFloat(x)
   ccall((:mpfr_nextabove, :libmpfr), Int32, (Ptr{mpfr_struct},), &(z.mpfr)) != 0
   return z
end

function prevfloat(x::MPFRFloat)
    z = MPFRFloat(x)
   ccall((:mpfr_nextbelow, :libmpfr), Int32, (Ptr{mpfr_struct},), &(z.mpfr)) != 0
   return z
end
...
function round(x::MPFRFloat, prec::Int)
    ...
    z = MPFRFloat(x)
    ccall((:mpfr_prec_round, :libmpfr), Int32, (Ptr{mpfr_struct}, Int, Int32), &(z.mpfr), prec, ROUNDING_MODE[end])
    return z
end
...
function string(x::MPFRFloat)
    ...
    for i = 1:2
        z = Array(Uint8, lng)
        lng = ccall((:mpfr_snprintf,:libmpfr), Int32, (Ptr{Uint8}, Uint, Ptr{Uint8}, Ptr{mpfr_struct}...), z, lng, "%.Re", &(x.mpfr))
        if lng < 128 || i == 2
