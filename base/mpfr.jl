...
import Base.GMP: ClongMax, CulongMax, CdoubleMax, Limb, MPZ, OutputMPZ
using Base: GMP
...
function convert(::Type{BigFloat}, x::BigInt)
    z = BigFloat()
    ccall((:mpfr_set_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, ROUNDING_MODE[end])
    return z
end
...
for (fJ, fC) in ((:+,:add), (:*,:mul))
    @eval begin
        ...
        function ($fJ)(x::BigFloat, c::BigInt)
            z = BigFloat()
            ccall(($(string(:mpfr_,fC,:_z)), :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &c, ROUNDING_MODE[end])
            return z
        end
...
function -(c::BigInt, x::BigFloat)
    z = BigFloat()
    ccall((:mpfr_z_sub, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}, Ptr{BigFloat}, Int32), &z, &c, &x, ROUNDING_MODE[end])
    return z
end
...
function div(x::BigFloat, c::BigInt)
    z = BigFloat()
    ccall((:mpfr_div_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &c, to_mpfr(RoundToZero))
    ccall((:mpfr_trunc, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}), &z, &z)
    return z
end
...
function ^(x::BigFloat, y::BigInt)
    z = BigFloat()
    ccall((:mpfr_pow_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{BigFloat}, Ptr{MPZ}, Int32), &z, &x, &y, ROUNDING_MODE[end])
    return z
end
...
function cmp(x::BigFloat, y::BigInt)
    ...
    ccall((:mpfr_cmp_z, :libmpfr), Int32, (Ptr{BigFloat}, Ptr{MPZ}), &x, &y)
end
