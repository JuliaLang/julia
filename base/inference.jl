...
function abstract_call_gf_by_type(f::ANY, atype::ANY, sv::InferenceState)
    ...
    fullmatch = !isempty(applicable) && (argtype <: (applicable[end][3]::Method).sig) # probably the last item will be the full-match, if any is, so try it first
    for (m::SimpleVector) in applicable
        ...
        splitunions = 1 < countunionsplit(sigtuple.parameters) * length(m) <= sv.params.MAX_UNION_SPLITTING * 2
        if splitunions
            ...
            rt = abstract_call_method(method, f, sig, m[2]::SimpleVector, sv)
            rettype = tmerge(rettype, rt)
            rettype === Any && break
        end
    end
