...
function structinfo_constant(ctx::AllocOptContext, @nospecialize(v), vt::DataType)
    nf = fieldcount(vt)
    if vt <: Tuple
        si = StructInfo(Vector{Any}(nf), Symbol[], vt, false, false)
    else
        si = StructInfo(Vector{Any}(nf), fieldnames(vt), vt, false, false)
    end
    for i in 1:nf
        if isdefined(v, i)
...
function structinfo_new(ctx::AllocOptContext, ex::Expr, vt::DataType)
    nf = fieldcount(vt)
    si = StructInfo(Vector{Any}(nf), fieldnames(vt), vt, vt.mutable, true)
                    ...
                    if has_def
                        if si.isnew && !(exprtype(fld_def, ctx.sv.src,
                            ...
                            check_ex = Expr(:call, check_new_field_type, fld_def,
                                            maybe_quoting(fieldtype(si.typ, fld_idx)))
                            push!(exprs, check_ex)
...
const enable_verify_valueinfo = Array{Bool,0}()
