...
function show(io::IO, t::AbstractDict{K,V}) where V where K
    ...
    # show in a Julia-syntax-like form: UDict(k=>v, ...)
...
`UDict{K,V}()` constructs a hash table with keys of type `K` and values of type `V`.
Keys are compared with [`isequal`](@ref) and hashed with [`hash`](@ref).

Given a single iterable argument, constructs a [`UDict`](@ref) whose key-value pairs
...
```
"""
mutable struct UDict{K,V} <: AbstractDict{K,V}
    slots::Array{UInt8,1}
    keys::Array{K,1}
    vals::Array{V,1}
    ndel::Int
...
UDict{K,V}(p::Pair) where {K,V} = setindex!(UDict{K,V}(), p.second, p.first)
function UDict{K,V}(ps::Pair...) where V where K
    h = UDict{K,V}()
...
# Note the constructors of WeakKeyUDict mirror these here, keep in sync.
UDict() = UDict{Any,Any}()
UDict(kv::Tuple{}) = UDict()
copy(d::UDict) = UDict(d)

const AnyUDict = UDict{Any,Any}

UDict(ps::Pair{K,V}...) where {K,V} = UDict{K,V}(ps)
UDict(ps::Pair...)                  = UDict(ps)
...
empty(a::AbstractDict, ::Type{K}, ::Type{V}) where {K, V} = UDict{K, V}()

hashindex(key, sz) = (((hash(key)::UInt % Int) & (sz-1)) + 1)::Int

@propagate_inbounds isslotempty(h::UDict, i::Int) = h.slots[i] == 0x0
@propagate_inbounds isslotfilled(h::UDict, i::Int) = h.slots[i] == 0x1
@propagate_inbounds isslotmissing(h::UDict, i::Int) = h.slots[i] == 0x2

function rehash!(h::UDict{K,V}, newsz = length(h.keys)) where V where K
    olds = h.slots
    oldk = h.keys
    oldv = h.vals
    sz = length(olds)
...
function sizehint!(d::UDict{T}, newsz) where T
...
julia> A
Base.UDict{String, Int64}()
```
"""
function empty!(h::UDict{K,V}) where V where K
    fill!(h.slots, 0x0)
    sz = length(h.slots)
    empty!(h.keys)
    empty!(h.vals)
...
function ht_keyindex(h::UDict{K,V}, key) where V where K
    sz = length(h.keys)
    iter = 0
    maxprobe = h.maxprobe
...
function ht_keyindex2!(h::UDict{K,V}, key) where V where K
    age0 = h.age
    sz = length(h.keys)
    iter = 0
    maxprobe = h.maxprobe
...
@propagate_inbounds function _setindex!(h::UDict, v, key, index)
    h.slots[index] = 0x1
    h.keys[index] = key
    h.vals[index] = v
    h.count += 1
...
function setindex!(h::UDict{K,V}, v0, key0) where V where K
    key = convert(K, key0)
    if !isequal(key, key0)
...
function setindex!(h::UDict{K,V}, v0, key::K) where V where K
    v = convert(V, v0)
...
# Examples
```jldoctest
julia> d = Base.UDict("a"=>1, "b"=>2, "c"=>3);
...
julia> d
Base.UDict{String, Int64} with 4 entries:
  "c" => 3
  "b" => 2
  "a" => 1
  "d" => 4
...
# Examples
```jldoctest
julia> squares = Base.UDict{Int, Int}();
...
julia> squares
Base.UDict{Int64, Int64} with 1 entry:
  2 => 4
```
"""
get!(f::Function, collection, key)

function get!(default::Callable, h::UDict{K,V}, key0) where V where K
    key = convert(K, key0)
    if !isequal(key, key0)
        throw(ArgumentError("$(limitrepr(key0)) is not a valid key for type $K"))
    end
...
function get!(default::Callable, h::UDict{K,V}, key::K) where V where K
    index = ht_keyindex2!(h, key)
...
function getindex(h::UDict{K,V}, key) where V where K
...
function get(h::UDict{K,V}, key, default) where V where K
    index = ht_keyindex(h, key)
...
function get(default::Callable, h::UDict{K,V}, key) where V where K
    index = ht_keyindex(h, key)
    @inbounds return (index < 0) ? default() : h.vals[index]::V
end
...
# Examples
```jldoctest
julia> D = Base.UDict('a'=>2, 'b'=>3)
Base.UDict{Char, Int64} with 2 entries:
  'a' => 2
  'b' => 3
...
```
"""
haskey(h::UDict, key) = (ht_keyindex(h, key) >= 0)
in(key, v::KeySet{<:Any, <:UDict}) = (ht_keyindex(v.dict, key) >= 0)
...
```
"""
function getkey(h::UDict{K,V}, key, default) where V where K
    index = ht_keyindex(h, key)
    @inbounds return (index<0) ? default : h.keys[index]::K
end

function _pop!(h::UDict, index)
    @inbounds val = h.vals[index]
    _delete!(h, index)
    return val
end

function pop!(h::UDict, key)
    index = ht_keyindex(h, key)
...
function pop!(h::UDict, key, default)
    index = ht_keyindex(h, key)
...
function _delete!(h::UDict{K,V}, index) where {K,V}
...
julia> delete!(d, "b")
Base.UDict{String, Int64} with 1 entry:
  "a" => 1
...
function skip_deleted(h::UDict, i)
    L = length(h.slots)
    for i = i:L
...
function skip_deleted_floor!(h::UDict)
...
@propagate_inbounds _iterate(t::UDict{K,V}, i) where {K,V} = i == 0 ? nothing : (Pair{K,V}(t.keys[i],t.vals[i]), i == typemax(Int) ? 0 : i+1)
@propagate_inbounds function iterate(t::UDict)
    _iterate(t, skip_deleted_floor!(t))
end
@propagate_inbounds iterate(t::UDict, i) = _iterate(t, skip_deleted(t, i))

isempty(t::UDict) = (t.count == 0)
length(t::UDict) = t.count

@propagate_inbounds function Base.iterate(v::T, i::Int = v.dict.idxfloor) where T <: Union{KeySet{<:Any, <:UDict}, ValueIterator{<:UDict}}
    i == 0 && return nothing
...
function filter!(pred, h::UDict{K,V}) where {K,V}
    h.count == 0 && return h
...
function reduce(::typeof(merge), items::Vector{<:UDict})
    K = mapreduce(keytype, promote_type, items)
    V = mapreduce(valtype, promote_type, items)
    return reduce(merge!, items; init=UDict{K,V}())
end

function map!(f, iter::ValueIterator{<:UDict})
    dict = iter.dict
    vals = dict.vals
...
_similar_for(c::UDict, ::Type{Pair{K,V}}, itr, isz) where {K, V} = empty(c, K, V)
