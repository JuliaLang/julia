...
############################
# Hacky weak object-id dict copy-pasted-adapted from Dict
#
# WeakObjectIdDict
#
# TODO:
...
type WeakObjectIdDict{K,V} <: Associative{K,V}
    ...
    function WeakObjectIdDict()
        n = 16
        new(zeros(Uint8,n), Array(WeakRef,n), Array(V,n), 0, 0, identity)
    end
    function WeakObjectIdDict(ks, vs)
        # TODO: eventually replace with a call to WeakObjectIdDict(zip(ks,vs))
        n = min(length(ks), length(vs))
        h = WeakObjectIdDict{K,V}()
        for i=1:n
            h[ks[i]] = vs[i]
        end
        return h
    end
    function WeakObjectIdDict(kv)
        h = WeakObjectIdDict{K,V}()
        for (k,v) in kv
            h[k] = v
        end
...
WeakObjectIdDict() = WeakObjectIdDict{Any,Any}()

WeakObjectIdDict{K,V}(ks::AbstractArray{K}, vs::AbstractArray{V}) = WeakObjectIdDict{K,V}(ks,vs)
WeakObjectIdDict(ks, vs) = WeakObjectIdDict{Any,Any}(ks, vs)

# # conversion between WeakObjectIdDict types
# function convert{K,V}(::Type{WeakObjectIdDict{K,V}},d::WeakObjectIdDict)
#     h = WeakObjectIdDict{K,V}()
#     for (k,v) in d
#         ck = convert(K,k)
#         if !haskey(h,ck)
...
#     return h
# end
# convert{K,V}(::Type{WeakObjectIdDict{K,V}},d::WeakObjectIdDict{K,V}) = d

# syntax entry points
WeakObjectIdDict{K,V}(ks::(K...), vs::(V...)) = WeakObjectIdDict{K  ,V  }(ks, vs)
WeakObjectIdDict{K  }(ks::(K...), vs::Tuple ) = WeakObjectIdDict{K  ,Any}(ks, vs)
WeakObjectIdDict{V  }(ks::Tuple , vs::(V...)) = WeakObjectIdDict{Any,V  }(ks, vs)

WeakObjectIdDict{K,V}(kv::AbstractArray{(K,V)}) = WeakObjectIdDict{K,V}(kv)
WeakObjectIdDict{K,V}(kv::Associative{K,V}) = WeakObjectIdDict{K,V}(kv)

similar{K,V}(d::WeakObjectIdDict{K,V}) = (K=>V)[]

function serialize(s, t::WeakObjectIdDict)
    serialize_type(s, typeof(t))
    write(s, int32(length(t)))
    for (k,v) in t
        serialize(s, k)
...
function deserialize{K,V}(s, T::Type{WeakObjectIdDict{K,V}})
    n = read(s, Int32)
    t = T(); sizehint(t, n)
    for i = 1:n
        k = deserialize(s)
...
isslotfilled(h::WeakObjectIdDict, i::Int) = h.slots[i] == 0x1
isslotmissing(h::WeakObjectIdDict, i::Int) = h.slots[i] == 0x2

function rehash{K,V}(h::WeakObjectIdDict{K,V}, newsz)
    olds = h.slots
    oldv = h.vals
    sz = length(olds)
    newsz = _tablesz(newsz)
    indexnothing_old = hashindex(h, nothing, sz)
    wnothing = WeakRef(nothing)
    # check whether nothing is a key
    if haskey(h, nothing)
        error("Rehashing for a WeakObjectIdDict with 'nothing' as key is not yet implemented")
    end
...
function sizehint(d::WeakObjectIdDict, newsz)
    oldsz = length(d.slots)
    if newsz <= oldsz
        # todo: shrink
        # be careful: rehash() assumes everything fits. it was only designed
...
function empty!{K,V}(h::WeakObjectIdDict{K,V})
    fill!(h.slots, 0x0)
    sz = length(h.slots)
    h.keys = Array(WeakRef, sz)
    h.vals = Array(V, sz)
    h.ndel = 0
    h.count = 0
    return h
...
function ht_keyindex{K,V}(h::WeakObjectIdDict{K,V}, key)
    ...
    while true
        ...
        index = (index & (sz-1)) + 1 # is this equal to modulo index+=1?
        iter+=1
        iter > maxprobe && break
    end
...
# and the key would be inserted at pos
# This version is for use by setindex! and get!
function ht_keyindex2{K,V}(h::WeakObjectIdDict{K,V}, key)
    sz = numslots(h)
    iter = 0
    maxprobe = max(16, sz>>6)
    index = hashindex(h, key, sz)
...
function setindex!{K,V}(h::WeakObjectIdDict{K,V}, v0, key0)
    assert(isa(key0, K))
    if ~isimmutable(key0)
        if is(h.deleter, identity)
            h.deleter = x->weak_key_delete!(h, x)
        end
        finalizer(key0, h.deleter)
    end
...
function get!{K,V}(h::WeakObjectIdDict{K,V}, key0, default)
    assert(isa(key0, K))
    if ~isimmutable(key0)
        if is(h.deleter, identity)
            h.deleter = x->weak_key_delete!(h, x)
...
function get!{K,V}(default::Function, h::WeakObjectIdDict{K,V}, key0)
    assert(isa(key0, K))
    if ~isimmutable(key0)
        if is(h.deleter, identity)
            h.deleter = x->weak_key_delete!(h, x)
...
# NOTE: this macro is specific to WeakObjectIdDict, not Associative, and should
#       therefore not be exported as-is: it's for internal use only.
macro get!(h, key0, default)
    quote
        K, V = eltype($(esc(h)))
        assert( isa($(esc(key0)), K) )
        idx = ht_keyindex2($(esc(h)), $(esc(key0)))
        if idx < 0
            idx = -idx
            v = convert(V, $(esc(default)))
            _setindex!($(esc(h)), v,  $(esc(key0)), idx)
        else
            @inbounds v = $(esc(h)).vals[idx]
        end
        v
...
function getindex{K,V}(h::WeakObjectIdDict{K,V}, key)
    index = ht_keyindex(h, key)
    return (index<0) ? throw(KeyError(key)) : h.vals[index]::V
end

function get{K,V}(h::WeakObjectIdDict{K,V}, key, deflt)
    index = ht_keyindex(h, key)
    return (index<0) ? deflt : h.vals[index]::V
end

function get{K,V}(deflt::Function, h::WeakObjectIdDict{K,V}, key)
    index = ht_keyindex(h, key)
    return (index<0) ? deflt() : h.vals[index]::V
end

haskey(h::WeakObjectIdDict, key) = (ht_keyindex(h, key) >= 0)
in{T<:WeakObjectIdDict}(key, v::KeyIterator{T}) = (ht_keyindex(v.dict, key) >= 0)

function getkey{K,V}(h::WeakObjectIdDict{K,V}, key, deflt)
    index = ht_keyindex(h, key)
    return (index<0) ? deflt : h.keys[index].value
end

function _pop!(h::WeakObjectIdDict, index)
    val = h.vals[index]
    _delete!(h, index)
    return val
end

function pop!(h::WeakObjectIdDict, key)
    index = ht_keyindex(h, key)
    index > 0 ? _pop!(h, index) : throw(KeyError(key))
end

function pop!(h::WeakObjectIdDict, key, default)
    index = ht_keyindex(h, key)
    index > 0 ? _pop!(h, index) : default
end

function _delete!(h::WeakObjectIdDict, index)
    h.slots[index] = 0x2
    ccall(:jl_arrayunset, Void, (Any, Uint), h.keys, index-1)
    ccall(:jl_arrayunset, Void, (Any, Uint), h.vals, index-1)
    h.ndel += 1
...
function delete!(h::WeakObjectIdDict, key)
    index = ht_keyindex(h, key)
    if index > 0; _delete!(h, index); end
    h
end

function skip_deleted(h::WeakObjectIdDict, i)
    L = length(h.slots)
    while i<=L && !isslotfilled(h,i)
        i += 1
    end
...
start(t::WeakObjectIdDict) = skip_deleted(t, 1)
done(t::WeakObjectIdDict, i) = done(t.vals, i)
next(t::WeakObjectIdDict, i) = ((getkey(t,i), t.vals[i]), skip_deleted(t,i+1))

isempty(t::WeakObjectIdDict) = (t.count == 0)
length(t::WeakObjectIdDict) = t.count

next{T<:WeakObjectIdDict}(v::KeyIterator{T}, i) = (v.dict.keys[i], skip_deleted(v.dict,i+1))
next{T<:WeakObjectIdDict}(v::ValueIterator{T}, i) = (v.dict.vals[i], skip_deleted(v.dict,i+1))
