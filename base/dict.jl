...
Makes an attempt to preallocate enough space to accommodate `sizehint` elements,
if provided.
...
mutable struct Dict{K,V} <: AbstractDict{K,V}
    ...
    function Dict{K,V}(;sizehint=0) where V where K
        n = _tablesz(K, sizehint)
...
function Dict{K,V}(kv) where V where K
    h = Dict{K,V}(sizehint = haslength(kv) ? Int(length(kv))::Int : 0)
    for (k,v) in kv
        h[k] = v
    end
    return h
...
Dict(; sizehint = 0) = Dict{Any,Any}(; sizehint)
Dict(kv::Tuple{}) = Dict()
copy(d::Dict) = Dict(d)
