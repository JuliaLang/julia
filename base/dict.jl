...
`EgalDict{K,V}()` constructs a hash table based `object_id`-hash with keys
of type `K` and values of type `V`.

Given a single iterable argument, constructs a [`EgalDict`](@ref) whose key-value
...
struct EgalDict{K,V} <: HashDict{K,V}
    ...
    EgalDict{K,V}() where {K, V} =
        new(BaseDict{K,V}())
    EgalDict{K,V}(h::EgalDict{K,V}) where {K, V} =
        new(BaseDict{K,V}(h.ht))
end
