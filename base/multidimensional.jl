...
@inline index_shape_dim(A, dim, i::AbstractArray, I...) = (i_s_d(i)..., index_shape_dim(A, dim+1, I...)...)
@inline index_shape_dim(A, dim, i::AbstractArray{Bool}, I...) = (sum(i), index_shape_dim(A, dim+1, I...)...)
@inline index_shape_dim{N}(A, dim, i::AbstractArray{CartesianIndex{N}}, I...) = (i_s_d(i)..., index_shape_dim(A, dim+N, I...)...)
i_s_d(I) = i_s_d(indicesbehavior(I), I)
i_s_d(::IndicesStartAt1, I) = size(I)
i_s_d(::IndicesBehavior, I) = indices(I)
...
@generated function unique{T,N}(A::AbstractArray{T,N}, dim::Int)
    quote
        ...
        hashes = allocate_for(inds->zeros(UInt, inds), A, dim)
        ...
        uniquerow = allocate_for(Array{Int}, A, dim)
        ...
        collided = allocate_for(falses, A, dim)
        @inbounds begin
        ...
        if any(collided)
            nowcollided = allocate_for(BitArray, A, dim)
            while any(collided)
