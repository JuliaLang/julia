...
@inline function check_valid_base(base, raise::Bool)
    base === nothing && return true
    if 2 <= base <= 62
        return true
    end
...
function parse(::Type{T}, s::AbstractString; base::Union{Nothing,Integer} = nothing)::T where {T<:Integer}
    check_valid_base(base, true)
    _parse(T, s, (base===nothing ? 0 : Int(base)), true)
end

function parse(::Type{T}, s::AbstractChar; base::Integer=10)::T where {T<:Integer}
    check_valid_base(base, false)
    _parse(T, s, Int(base), true)
end

function tryparse(::Type{T}, s::AbstractString;
                  base::Union{Nothing,Integer} = nothing)::Union{Nothing, T} where {T<:Integer}
    check_valid_base(base, true) || return nothing
    _parse(T, s, (base===nothing ? 0 : Int(base)), false)
end

function tryparse(::Type{T}, s::AbstractChar;
                  base::Integer=10)::Union{Nothing, T} where {T<:Integer}
    check_valid_base(base, false) || return nothing
    _parse(T, s, Int(base), false)
end
...
function tryparse_internal_int(::Type{T}, s, base::Int, raise::Bool) where T<:Integer
    ...
    check_valid_base(base, raise) || return nothing
...
function parse(::Type{T}, s::AbstractString)::T where {T<:Real}
     _parse_float(T, s, true)
end
function tryparse(::Type{T}, s::AbstractString)::Union{Nothing, T} where {T<:Real}
...
tryparse_internal_float(::Type{Float64}, s::String)            = ccall_try_substrtod(s, 0, sizeof(s))
tryparse_internal_float(::Type{Float64}, s::SubString{String}) = ccall_try_substrtod(s, 0, s.ncodeunits)

function ccall_try_substrtod(s, startpos::Int, n::Int)
    hasvalue, val = ccall(:jl_try_substrtod, Tuple{Bool, Float64},
                      (Ptr{UInt8},Csize_t,Csize_t), s, startpos, n)
    hasvalue ? val : nothing
end

tryparse_internal_float(::Type{Float32}, s::String)            = ccall_try_substrtof(s, 0, sizeof(s))
tryparse_internal_float(::Type{Float32}, s::SubString{String}) = ccall_try_substrtof(s, 0, s.ncodeunits)
function ccall_try_substrtof(s, startpos::Int, n::Int)
    hasvalue, val = ccall(:jl_try_substrtof, Tuple{Bool, Float32},
                      (Ptr{UInt8},Csize_t,Csize_t), s, startpos, n)
    hasvalue ? val : nothing
end

tryparse_internal_float(::Type{Float16}, s::Union{String, SubString{String}})::Union{Float32, Nothing} = Float16(tryparse_internal_float(Float32, s))

###############################
# string to complex functions #
###############################

function parse(::Type{Complex{T}}, s::AbstractString)::Complex{T} where {T<:Real}
    _parse_complex(Complex{T}, s, true)
end
function tryparse(::Type{Complex{T}}, s::AbstractString)::Union{Nothing, Complex{T}} where {T<:Real}
...
function tryparse_internal_complex(::Type{Complex{T}}, s::Union{String,SubString{String}}, raise::Bool) where {T<:Real}
