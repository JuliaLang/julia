...
function tryparse_internal(
    ...
    m::T = base == 10 ? div(typemax(T) - T(9), T(10)) :
           base == 16 ? div(typemax(T) - T(15), T(16)) : div(typemax(T) - base + 1, base)
    n::T = 0
    a::Int = base <= 36 ? 10 : 36
    _0 = UInt32('0')
    _9 = UInt32('9')
    ...
    while n <= m
        ...
        d::T = _0 <= _c <= _9 ? _c - _0 :
               _A <= _c <= _Z ? _c - _A + UInt32(10) : _a <= _c <= _z ? _c - _a + a : base
        if d >= base
...
function tryparse_internal(
    ::Type{Bool},
    sbuff::Union{String,SubString{String}},
    startpos::Int,
    endpos::Int,
    base::Integer,
    raise::Bool
...
function tryparse(
    ...
    tryparse_internal(
        ...
        firstindex(s),
        lastindex(s),
        base === nothing ? 0 : check_valid_base(base),
        false
    )
end
...
@noinline _parse_failure(
    T,
    s::AbstractString,
    startpos = firstindex(s),
    endpos = lastindex(s)
...
tryparse_internal(
    ...
    endpos::Int,
    raise::Bool
) where T <: Integer = tryparse_internal(T, s, startpos, endpos, 10, raise)
