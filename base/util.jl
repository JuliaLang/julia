...
"""
    Base.runtests(tests=["all"]; ncores=ceil(Int, Sys.CPU_LOGICAL_CORES / 2),
...
"""
function runtests(tests = ["all"]; ncores = ceil(Int, Sys.CPU_LOGICAL_CORES / 2),
                  exit_on_error=false,
    ...
    ENV2 = copy(ENV)
    ENV2["JULIA_CPU_LOGICAL_CORES"] = "$ncores"
    try
