...
function _redirect_io_cglobal(handle::Union{LibuvStream, IOStream, RawFD}, unix_fd::Int)
    c_sym = unix_fd == 0 ? cglobal(:jl_uv_stdin, Ptr{Cvoid}) :
            ...
            C_NULL
    c_sym == C_NULL || unsafe_store!(c_sym, handle isa RawFD ? handle : handle.handle)
    nothing
end
...
function (f::RedirectStdStream)(::DevNull)
    ...
    _redirect_io_cglobal(RawFD(f.unix_fd), f.unix_fd)
