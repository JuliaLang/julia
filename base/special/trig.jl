...
### sincos methods
@noinline sincos_domain_error(x) =
    throw(DomainError(x, "sincos(x) is only defined for finite x."))
...
@inline sincos_kernel(y::Union{Float32,Float64,DoubleFloat32,DoubleFloat64}) =
    (sin_kernel(y), cos_kernel(y))
...
arc_p(t::Float32) = t * @horner(t, 1.6666586697f-01, -4.2743422091f-02, -8.6563630030f-03)
...
atan_p(z::Float32, w::Float32) =
    z * @horner(w, 3.3333328366f-01, 1.4253635705f-01, 6.1687607318f-02)
...
@inline ACOS_CORRECT_LOWWORD(::Type{Float32}, x) =
    reinterpret(Float32, (reinterpret(UInt32, x) & 0xfffff000))
@inline ACOS_CORRECT_LOWWORD(::Type{Float64}, x) =
    reinterpret(Float64, (reinterpret(UInt64, x) >> 32) << 32)
...
cosc(x::Complex{<:AbstractFloat}) =
    x == 0 ? zero(x) : oftype(x, (cospi(x) - sinpi(x) / (pi * x)) / x)
cosc(x::Complex) = cosc(float(x))
cosc(x::Real) = x == 0 || isinf(x) ? zero(x) : (cospi(x) - sinpi(x) / (pi * x)) / x

for (finv, f, finvh, fh, finvd, fd, fn) in (
    (:sec, :cos, :sech, :cosh, :secd, :cosd, "secant"),
...
for (tfa, tfainv, hfa, hfainv, fn) in (
    (:asec, :acos, :asech, :acosh, "secant"),
    ...
    @eval begin
        @doc """
                 $($tname)(x)
             Compute the inverse $($fn) of `x`, where the output is in radians. """ ($tfa)(y::Number) =
            ($tfainv)(inv(y))
        @doc """
                 $($hname)(x)
             Compute the inverse hyperbolic $($fn) of `x`. """ ($hfa)(y::Number) =
            ($hfainv)(inv(y))
    end
end
...
for (fd, f, fn) in (
    ...
    @eval begin
        @doc """
             ...
             Compute the inverse $($fn) of `x`, where the output is in degrees. """ ($fd)(y) =
            rad2deg(($f)(y))
    end
end
