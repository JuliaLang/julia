...
"""
    sinc(x; [normalized::Bool=true])

Compute ``\\sin(\\pi x) / (\\pi x)`` if ``x \\neq 0``, and ``1`` if ``x = 0`` (equal to the
[`sincpi`](@ref) function).  If `normalized` is `false`, the factors of ``\\pi`` are
omitted (equal to the [`sincu`](@ref) function).

The normalization is with respect to integration over all real values of ``x``:
```math
\\int_{-\\infty}^{\\infty} \\mathrm{sincpi}(x) \\, \\mathrm{d}x = 1,
```
or, if `normalized` is `false`,
```math
\\int_{-\\infty}^{\\infty} \\mathrm{sincu}(x) \\, \\mathrm{d}x = \\pi.
```

See also [`cosc`](@ref), the derivative of this function, or [`sincpi`](@ref),
[`coscpi`](@ref), [`sincu`](@ref), or [`coscu`](@ref) for different normalizations.

!!! compat "Julia 1.11"
    The `normalized` keyword argument requires at least Julia 1.11.
"""
sinc(x; normalized::Bool=true) = normalized ? sincpi(x) : sincu(x)

"""
    cosc(x; [normalized::Bool=true])

Compute the derivative of `sinc(x)`: ``\\cos(\\pi x) / x - \\sin(\\pi x) / (\\pi x^2)`` if
``x \\neq 0``, and ``0`` if ``x = 0``.  If `normalized` is `true` (the default), this is the
derivative of [`sincpi`](@ref); if `normalized` is `false`, the factors of ``\\pi`` are
omitted, and this is the derivative of [`sincu`](@ref).

!!! compat "Julia 1.11"
    The `normalized` keyword argument requires at least Julia 1.11.
"""
cosc(x; normalized::Bool=true) = normalized ? coscpi(x) : coscu(x)

"""
    sincpi(x)

Compute the normalized ``\\mathrm{sinc}`` function ``\\sin(\\pi x) / (\\pi x)`` if ``x \\neq
0``, and ``1`` if ``x = 0``.
...
See also [`coscpi`](@ref), the derivative of this function, or [`sinc`](@ref),
[`cosc`](@ref), [`sincu`](@ref), or [`coscu`](@ref) for different normalizations.

!!! compat "Julia 1.11"
    This method was added in Julia 1.11.
"""
sincpi(x::Number) = _sincpi(float(x))
sincpi(x::Integer) = iszero(x) ? one(x) : zero(x)
_sincpi(x::Number) = iszero(x) ? one(x) : isinf_real(x) ? zero(x) : sinpi(x)/(pi*x)
@inline _sincpi(x::Union{Float64,ComplexF64}) =
    fastabs(x) < 0.001 ? evalpoly(x^2, (1.0, -1.6449340668482264, 0.8117424252833535)) :
    isinf_real(x) ? zero(x) : sinpi(x)/(pi*x)
@inline _sincpi(x::Union{Float32,ComplexF32}) =
    fastabs(x) < 0.05f0 ? evalpoly(x^2, (1.0f0, -1.6449342f0, 0.81174254f0)) :
    isinf_real(x) ? zero(x) : sinpi(x)/(pi*x)
_sincpi(x::Float16) = Float16(_sincpi(Float32(x)))
_sincpi(x::ComplexF16) = ComplexF16(_sincpi(ComplexF32(x)))

"""
    coscpi(x)

Compute ``\\cos(\\pi x) / x - \\sin(\\pi x) / (\\pi x^2)`` if ``x \\neq 0``, and ``0`` if
``x = 0``. This is the derivative of [`sincpi`](@ref).

See also [`sinc`](@ref), [`cosc`](@ref), [`sincu`](@ref), or [`coscu`](@ref) for different
normalizations.

!!! compat "Julia 1.11"
    This method was added in Julia 1.11.
"""
coscpi(x::Number) = _coscpi(float(x))
function _coscpi(x::Number)
    # naive coscpi formula is susceptible to catastrophic
...
_coscpi(x::Union{Float64,ComplexF64}) =
    ...
    isinf_real(x) ? zero(x) : ((s,c)=sincospi(x); ((pi*x)*c-s)/((pi*x)*x))
_coscpi(x::Union{Float32,ComplexF32}) =
    fastabs(x) < 0.26f0 ? x*evalpoly(x^2, (-3.289868f0, 3.2469697f0, -1.144511f0, 0.20918278f0)) :
    isinf_real(x) ? zero(x) : ((s,c)=sincospi(x); ((pi*x)*c-s)/((pi*x)*x))
_coscpi(x::Float16) = Float16(_coscpi(Float32(x)))
_coscpi(x::ComplexF16) = ComplexF16(_coscpi(ComplexF32(x)))

"""
    sincu(x)

Compute the un-normalized ``\\mathrm{sinc}`` function ``\\sin(x) / x`` if ``x \\neq
0``, and ``1`` if ``x = 0``.
...
See also [`coscu`](@ref), the derivative of this function, or [`sinc`](@ref),
[`cosc`](@ref), [`sincpi`](@ref), or [`coscpi`](@ref) for different normalizations.
...
Compute ``\\cos(x) / x - \\sin(x) / x^2`` if ``x \\neq 0``, and ``0`` if ``x = 0``. This is
the derivative of [`sincu`](@ref).

See also [`sinc`](@ref), [`cosc`](@ref), [`sincpi`](@ref), or [`coscpi`](@ref) for different
normalizations.

!!! compat "Julia 1.11"
    This method was added in Julia 1.11.
"""
coscu(x::Number) = _coscu(float(x))
