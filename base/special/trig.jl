...
@inline function sin_kernel(y::DoubleFloat64)
    y² = y.hi*y.hi
    y⁴ = y²*y²
    r  = evalpoly(y², (DS2, DS3, DS4)) + y²*y⁴*evalpoly(y², (DS5, DS6))
    y³ = y²*y.hi
...
@inline function sin_kernel(y::DoubleFloat32)
    ...
    z = y.hi*y.hi
    w = z*z
    r = evalpoly(z, (-0.00019839334836096632, 2.718311493989822e-6))
    s = z*y.hi
    Float32((y.hi + s*evalpoly(z, (S1, S2))) + s*w*r)
end
...
@inline function cos_kernel(y::DoubleFloat64)
    y² = y.hi*y.hi
    y⁴ = y²*y²
    r  = y²*evalpoly(y², (DC1, DC2, DC3)) + y⁴*y⁴*evalpoly(y², (DC4, DC5, DC6))
    half_y² = 0.5*y²
    w  = 1.0-half_y²
...
@inline function cos_kernel(y::DoubleFloat32)
    ...
    y² = y.hi*y.hi
    y⁴ = y²*y²
    r = evalpoly(y², (-0.001388676377460993, 2.439044879627741e-5))
...
@inline function tan_kernel(y::DoubleFloat64, k)
    ...
    r = evalpoly(y⁴, (1.33333333333201242699e-01,   # T2
    ...
    v = y² * evalpoly(y⁴,(5.39682539762260521377e-02,  # T3
...
@inline function tan_kernel(y::DoubleFloat32, k)
    ...
    y² = y.hi*y.hi
    r  = evalpoly(y², (0.00297435743359967304927, 0.00946564784943673166728))
    t  = evalpoly(y², (0.0533812378445670393523, 0.0245283181166547278873))
    y⁴ = y²*y²
    y³ = y²*y.hi
    u  = evalpoly(y², (0.333331395030791399758, 0.133392002712976742718))
...
arc_p(t::Float64) = t*evalpoly(t, (1.66666666666666657415e-01,
...
arc_q(z::Float64) = evalpoly(z,(1.0,
                                -2.40339491173441421878e+00,
                                2.02094576023350569471e+00,
                                -6.88283971605453293030e-01,
                                7.70381505559019352791e-02))

arc_p(t::Float32) = t*evalpoly(t, (1.6666586697f-01,
                                   -4.2743422091f-02,
                                   -8.6563630030f-03))

arc_q(t::Float32) = evalpoly(t, (1.0f0, -7.0662963390f-01))
...
atan_p(z::Float64, w::Float64) = z*evalpoly(w, (3.33333333333329318027e-01,
                                                1.42857142725034663711e-01,
                                                9.09088713343650656196e-02,
                                                6.66107313738753120669e-02,
                                                4.97687799461593236017e-02,
                                                1.62858201153657823623e-02))
atan_q(w::Float64) = w*evalpoly(w, (-1.99999999998764832476e-01,
                                    -1.11111104054623557880e-01,
                                    -7.69187620504482999495e-02,
                                    -5.83357013379057348645e-02,
                                    -3.65315727442169155270e-02))
atan_p(z::Float32, w::Float32) = z*evalpoly(w, (3.3333328366f-01,  1.4253635705f-01, 6.1687607318f-02))
atan_q(w::Float32) = w*evalpoly(w, (-1.9999158382f-01, -1.0648017377f-01))
