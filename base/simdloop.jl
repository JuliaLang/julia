...
# NB: the `@inbounds` is semantic, not an optimization: `i` is generated by the
# `@simd` macro expansion in `0:simd_inner_length(r,j)-1`, so it is in bounds by
# construction, and a checked path here would burden every `@simd` loop in the
# system with a per-iteration range check (blocking or bloating vectorization).
