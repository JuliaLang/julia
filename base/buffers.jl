...
function Buffer{T}(::UndefInitializer, n::Int) where {T}
    ccall(:jl_new_buffer, Any, (Any, UInt), Buffer{T}, n)
end
...
@eval setindex!(b::Buffer{T}, x, i::Int) where {T} = Core.bufset($(Expr(:boundscheck)), b, convert(T, x)::T, i)
eltype(::Type{<:Buffer{T}}) where {T} = T
elsize(@nospecialize T::Type{<:Buffer}) = aligned_sizeof(eltype(T))
length(b::Buffer) = Core.buflen(b)
