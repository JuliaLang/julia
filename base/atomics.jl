    ...
    if typ <: Union{Integer, Ptr}
        @eval atomic_cas!(x::Atomic{V}, cmp::V, new::V) where {V <: $typ} =
            llvmcall($"""
                     ...
                     ret $lt %rv
                     """, V, Tuple{Ptr{V}, V, V},
                     unsafe_convert(Ptr{V}, x), cmp, new)
    else
        ...
        if typ <: Union{Integer, Ptr}
            @eval $fn(x::Atomic{V}, v::V) where {V <: $typ} =
                llvmcall($"""
                         ...
                         ret $lt %rv
                         """, V, Tuple{Ptr{V}, V}, unsafe_convert(Ptr{V}, x), v)
        else
            rmwop === :xchg || continue
