...
$(eval $(call git-external,libtracyclient,LIBTRACYCLIENT,,,$(SRCCACHE)))
...
$(LIBTRACYCLIENT_SRCCACHE)/cmake-patch-applied: $(LIBTRACYCLIENT_SRCCACHE)/source-extracted
ifneq ($(OS),WINNT)
	echo "target_compile_definitions(TracyClient PUBLIC __STDC_FORMAT_MACROS)" >> $(LIBTRACYCLIENT_SRCCACHE)/CMakeLists.txt
else
	echo "target_compile_definitions(TracyClient PUBLIC WINVER=0x0602 _WIN32_WINNT=0x0602)" >> $(LIBTRACYCLIENT_SRCCACHE)/CMakeLists.txt
endif
	echo 1 > $@

$(LIBTRACYCLIENT_SRCCACHE)/libTracyClient-freebsd-elfw.patch-applied: $(LIBTRACYCLIENT_SRCCACHE)/cmake-patch-applied
	cd $(LIBTRACYCLIENT_SRCCACHE) && \
		patch -p1 -f < $(SRCDIR)/patches/libTracyClient-freebsd-elfw.patch
	echo 1 > $@

$(LIBTRACYCLIENT_SRCCACHE)/libTracyClient-no-crash-handler.patch-applied: $(LIBTRACYCLIENT_SRCCACHE)/libTracyClient-freebsd-elfw.patch-applied
	cd $(LIBTRACYCLIENT_SRCCACHE) && \
		patch -p1 -f < $(SRCDIR)/patches/libTracyClient-no-crash-handler.patch
	echo 1 > $@

$(LIBTRACYCLIENT_BUILDDIR)/build-configured: $(LIBTRACYCLIENT_SRCCACHE)/libTracyClient-no-crash-handler.patch-applied
	mkdir -p $(dir $@)
	cd $(dir $@) && \
		$(CMAKE) $(LIBTRACYCLIENT_SRCCACHE) $(CMAKE_GENERATOR_COMMAND) $(CMAKE_COMMON) $(LIBTRACYCLIENT_CMAKE) \
...
extract-libtracyclient: $(LIBTRACYCLIENT_SRCCACHE)/source-extracted
