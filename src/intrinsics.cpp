...
static Constant *julia_const_to_llvm(jl_codectx_t &ctx, const void *ptr, jl_datatype_t *bt)
{
    ...
    if (bt == jl_bool_type)
        return ConstantInt::get(getInt8Ty(ctx), (*(const uint8_t*)ptr) ? 1 : 0);
    ...
    if (lt->isFloatTy()) {
        ...
        return ConstantFP::get(getLLVMContext(ctx),
    ...
    if (lt->isFloatingPointTy() || lt->isIntegerTy() || lt->isPointerTy()) {
        ...
        if (lt->isPointerTy()) {
            Type *Ty = IntegerType::get(getLLVMContext(ctx), 8 * nb);
    ...
    for (size_t i = 0; i < nf; i++) {
        ...
        unsigned llvm_idx = isa<StructType>(lt) ? convert_struct_offset(getLLVMModule(ctx)->getDataLayout(), lt, offs) : i;
        ...
        if (jl_is_uniontype(ft)) {
            ...
            Type *AlignmentType = IntegerType::get(getLLVMContext(ctx), 8 * al);
            ...
            while (remainder--) {
                ...
                if (active_sz > 0) {
                    ...
                    APInt Elem(8, byte);
                    active_sz -= 1;
                    fld = ConstantInt::get(getInt8Ty(ctx), Elem);
                }
                else {
                    fld = UndefValue::get(getInt8Ty(ctx));
                }
                ov += 1;
                fields.push_back(fld);
            }
            fields.push_back(ConstantInt::get(getInt8Ty(ctx), sel));
        }
        else {
...
{
    if (e == jl_true)
        return ConstantInt::get(getInt8Ty(ctx), 1);
    if (e == jl_false)
        return ConstantInt::get(getInt8Ty(ctx), 0);
...
static Value *emit_unboxed_coercion(jl_codectx_t &ctx, Type *to, Value *unboxed)
{
    ...
    if (ty == getInt1Ty(ctx) && to == getInt8Ty(ctx)) {
        // bools may be stored internally as int8
        unboxed = ctx.builder.CreateZExt(unboxed, getInt8Ty(ctx));
    }
    else if (ty == getInt8Ty(ctx) && to == getInt1Ty(ctx)) {
        // bools may be stored internally as int8
        unboxed = ctx.builder.CreateTrunc(unboxed, getInt1Ty(ctx));
    }
    else if (ty == getVoidTy(ctx) || DL.getTypeSizeInBits(ty) != DL.getTypeSizeInBits(to)) {
...
static Value *emit_unbox(jl_codectx_t &ctx, Type *to, const jl_cgval_t &x, jl_value_t *jt, Value *dest, MDNode *tbaa_dest, bool isVolatile)
{
    ...
    if (jt == (jl_value_t*)jl_bool_type || to == getInt1Ty(ctx)) {
        Instruction *unbox_load = tbaa_decorate(x.tbaa, ctx.builder.CreateLoad(getInt8Ty(ctx), maybe_bitcast(ctx, p, getInt8PtrTy(ctx))));
        if (jt == (jl_value_t*)jl_bool_type)
            unbox_load->setMetadata(LLVMContext::MD_range, MDNode::get(getLLVMContext(ctx), {
                ConstantAsMetadata::get(ConstantInt::get(getInt8Ty(ctx), 0)),
                ConstantAsMetadata::get(ConstantInt::get(getInt8Ty(ctx), 2)) }));
        Value *unboxed;
        if (to == getInt1Ty(ctx))
            unboxed = ctx.builder.CreateTrunc(unbox_load, getInt1Ty(ctx));
        else
            unboxed = unbox_load; // `to` must be getInt8Ty(ctx)
        if (!dest)
            return unboxed;
        Type *dest_ty = unboxed->getType()->getPointerTo();
        if (dest->getType() != dest_ty)
...
static jl_cgval_t generic_bitcast(jl_codectx_t &ctx, const jl_cgval_t *argv)
{
    ...
    Type *llvmt = bitstype_to_llvm(bt, getLLVMContext(ctx));
    int nb = jl_datatype_size(bt);
    ...
    if (!jl_is_primitivetype(v.typ) || jl_datatype_size(v.typ) != nb) {
        ...
        if (!jl_is_datatype(v.typ) || jl_datatype_size(v.typ) != nb) {
            if (isboxed) {
                ...
                error_unless(ctx,
                        ctx.builder.CreateICmpEQ(size, ConstantInt::get(getInt32Ty(ctx), nb)),
    ...
    if (v.ispointer() && vx == NULL) {
        ...
        if (isboxed)
            vxt = llvmt;
        auto storage_type = vxt == getInt1Ty(ctx) ? getInt8Ty(ctx) : vxt;
    ...
    vxt = vx->getType();
    if (vxt != llvmt) {
        if (llvmt == getInt1Ty(ctx))
            vx = ctx.builder.CreateTrunc(vx, llvmt);
        else if (vxt == getInt1Ty(ctx) && llvmt == getInt8Ty(ctx))
        ...
        const jl_cgval_t *argv, bool toint, bool fromint)
{
    ...
    Type *to = bitstype_to_llvm(jlto, getLLVMContext(ctx));
    Type *vt = bitstype_to_llvm(v.typ, getLLVMContext(ctx));
    if (toint)
        to = INTT(to);
    else
        to = FLOATT(to);
...
static jl_cgval_t emit_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    Value *idx = emit_unbox(ctx, getSizeTy(getLLVMContext(ctx)), i, (jl_value_t*)jl_long_type);
    Value *im1 = ctx.builder.CreateSub(idx, ConstantInt::get(getSizeTy(getLLVMContext(ctx)), 1));
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        im1 = ctx.builder.CreateMul(im1, ConstantInt::get(getSizeTy(getLLVMContext(ctx)),
                    LLT_ALIGN(size, jl_datatype_align(ety))));
        Value *thePtr = emit_unbox(ctx, getInt8PtrTy(ctx), e, e.typ);
        thePtr = ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx), emit_bitcast(ctx, thePtr, getInt8PtrTy(ctx)), im1);
...
static jl_cgval_t emit_pointerset(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (ety == (jl_value_t*)jl_any_type) {
        ...
        thePtr = emit_unbox(ctx, getSizePtrTy(getLLVMContext(ctx)), e, e.typ);
        Instruction *store = ctx.builder.CreateAlignedStore(
          ctx.builder.CreatePtrToInt(emit_pointer_from_objref(ctx, boxed(ctx, x)), getSizeTy(getLLVMContext(ctx))),
            ctx.builder.CreateInBoundsGEP(getSizeTy(getLLVMContext(ctx)), thePtr, im1), Align(align_nb));
        tbaa_decorate(ctx.tbaa().tbaa_data, store);
    }
    else if (!jl_isbits(ety)) {
        thePtr = emit_unbox(ctx, getInt8PtrTy(ctx), e, e.typ);
        uint64_t size = jl_datatype_size(ety);
        im1 = ctx.builder.CreateMul(im1, ConstantInt::get(getSizeTy(getLLVMContext(ctx)),
                    LLT_ALIGN(size, jl_datatype_align(ety))));
        emit_memcpy(ctx, ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx), thePtr, im1), nullptr, x, size, align_nb);
    }
    else {
        bool isboxed;
        Type *ptrty = julia_type_to_llvm(ctx, ety, &isboxed);
        ...
        if (!type_is_ghost(ptrty)) {
            ...
            typed_store(ctx, thePtr, im1, x, jl_cgval_t(ctx), ety, ctx.tbaa().tbaa_data, nullptr, nullptr, isboxed,
...
static jl_cgval_t emit_atomic_pointerref(jl_codectx_t &ctx, jl_cgval_t *argv)
{
    ...
    if (!jl_isbits(ety)) {
        ...
        Value *strct = emit_allocobj(ctx, size,
                                     literal_pointer_val(ctx, ety));
        Value *thePtr = emit_unbox(ctx, getInt8PtrTy(ctx), e, e.typ);
        Type *loadT = Type::getIntNTy(getLLVMContext(ctx), nb * 8);
...
static jl_cgval_t emit_atomic_pointerop(jl_codectx_t &ctx, intrinsic f, const jl_cgval_t *argv, int nargs, const jl_cgval_t *modifyop)
{
    ...
    const jl_cgval_t undefval(getLLVMContext(ctx));
    const jl_cgval_t &e = argv[0];
...
static Value *emit_checked_srem_int(jl_codectx_t &ctx, Value *x, Value *den)
{
    ...
    BasicBlock *m1BB = BasicBlock::Create(getLLVMContext(ctx), "minus1", ctx.f);
    BasicBlock *okBB = BasicBlock::Create(getLLVMContext(ctx), "oksrem", ctx.f);
    BasicBlock *cont = BasicBlock::Create(getLLVMContext(ctx), "after_srem", ctx.f);
...
static jl_cgval_t emit_ifelse(jl_codectx_t &ctx, jl_cgval_t c, jl_cgval_t x, jl_cgval_t y, jl_value_t *rt_hint)
{
    ...
    if (t1 == jl_bottom_type && t2 == jl_bottom_type)
        return jl_cgval_t(ctx); // undefined
        ...
        if (x_tindex || y_tindex) {
            ...
            if (!x_ptr && !y_ptr) { // both ghost
                ...
                if (ifelse_tbaa == NULL) {
                    ...
                    auto *OffsetNode = ConstantAsMetadata::get(ConstantInt::get(getInt64Ty(ctx), 0));
                    Metadata *Ops[] = {ctx.tbaa().tbaa_root, ctx.tbaa().tbaa_root, OffsetNode};
                    ifelse_tbaa = MDNode::get(getLLVMContext(ctx), Ops);
                }
            }
            Value *tindex;
            if (!x_tindex && x.constant) {
                x_tindex = ConstantInt::get(getInt8Ty(ctx), 0x80 | get_box_tindex((jl_datatype_t*)jl_typeof(x.constant), rt_hint));
            }
            if (!y_tindex && y.constant) {
                y_tindex = ConstantInt::get(getInt8Ty(ctx), 0x80 | get_box_tindex((jl_datatype_t*)jl_typeof(y.constant), rt_hint));
            }
            if (x_tindex && y_tindex) {
                tindex = ctx.builder.CreateSelect(isfalse, y_tindex, x_tindex);
            }
            else {
                PHINode *ret = PHINode::Create(getInt8Ty(ctx), 2);
                BasicBlock *post = BasicBlock::Create(getLLVMContext(ctx), "post", ctx.f);
                BasicBlock *compute = BasicBlock::Create(getLLVMContext(ctx), "compute_tindex", ctx.f);
                ...
                tindex = ctx.builder.CreateOr(tindex, ConstantInt::get(getInt8Ty(ctx), 0x80));
...
static jl_cgval_t emit_intrinsic(jl_codectx_t &ctx, intrinsic f, jl_value_t **args, size_t nargs)
{
    ...
    case not_int: {
        ...
        if (!jl_is_primitivetype(x.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        Type *xt = INTT(bitstype_to_llvm(x.typ, getLLVMContext(ctx)));
        Value *from = emit_unbox(ctx, xt, x, x.typ);
        Value *ans;
        if (x.typ == (jl_value_t*)jl_bool_type)
            ans = ctx.builder.CreateXor(from, ConstantInt::get(getInt8Ty(ctx), 1, true));
        else
    ...
    case have_fma: {
        ...
        FunctionCallee intr = getLLVMModule(ctx)->getOrInsertFunction(intr_name, getInt1Ty(ctx));
    ...
    default: {
        ...
        if (!jl_is_primitivetype(xinfo.typ))
            return emit_runtime_call(ctx, f, argv, nargs);
        Type *xtyp = bitstype_to_llvm(xinfo.typ, getLLVMContext(ctx));
        if (float_func()[f])
            xtyp = FLOATT(xtyp);
        else
            xtyp = INTT(xtyp);
        ...
        if (f == shl_int || f == lshr_int || f == ashr_int) {
            if (!jl_is_primitivetype(argv[1].typ))
                return emit_runtime_call(ctx, f, argv, nargs);
            argt[1] = INTT(bitstype_to_llvm(argv[1].typ, getLLVMContext(ctx)));
        }
        else {
        ...
        if (newtyp == (jl_value_t*)jl_bool_type && r->getType() != getInt1Ty(ctx))
                                     ...
                                     jl_datatype_t **newtyp, jl_value_t *xtyp)
{
    ...
    case add_ptr: {
        return ctx.builder.CreatePtrToInt(
            ctx.builder.CreateGEP(getInt8Ty(ctx),
                emit_inttoptr(ctx, x, getInt8PtrTy(ctx)), y), t);
    ...
    case sub_ptr: {
        return ctx.builder.CreatePtrToInt(
            ctx.builder.CreateGEP(getInt8Ty(ctx),
                emit_inttoptr(ctx, x, getInt8PtrTy(ctx)), ctx.builder.CreateNeg(y)), t);
    ...
    case fma_float: {
        ...
        FunctionCallee fmaintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::fma, makeArrayRef(t));
    ...
    case checked_umul_int: {
        ...
        FunctionCallee intr = Intrinsic::getDeclaration(getLLVMModule(ctx), intr_id, makeArrayRef(t));
        ...
        Value *obyte = ctx.builder.CreateZExt(obit, getInt8Ty(ctx));
    ...
    case bswap_int: {
        FunctionCallee bswapintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::bswap, makeArrayRef(t));
        return ctx.builder.CreateCall(bswapintr, x);
    }
    case ctpop_int: {
        FunctionCallee ctpopintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::ctpop, makeArrayRef(t));
        return ctx.builder.CreateCall(ctpopintr, x);
    }
    case ctlz_int: {
        FunctionCallee ctlz = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::ctlz, makeArrayRef(t));
        y = ConstantInt::get(getInt1Ty(ctx), 0);
        return ctx.builder.CreateCall(ctlz, {x, y});
    }
    case cttz_int: {
        FunctionCallee cttz = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::cttz, makeArrayRef(t));
        y = ConstantInt::get(getInt1Ty(ctx), 0);
        return ctx.builder.CreateCall(cttz, {x, y});
    }

    case abs_float: {
        FunctionCallee absintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::fabs, makeArrayRef(t));
        return ctx.builder.CreateCall(absintr, x);
    }
    case copysign_float: {
        FunctionCallee copyintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::copysign, makeArrayRef(t));
    ...
    case ceil_llvm: {
        FunctionCallee ceilintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::ceil, makeArrayRef(t));
        return ctx.builder.CreateCall(ceilintr, x);
    }
    case floor_llvm: {
        FunctionCallee floorintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::floor, makeArrayRef(t));
        return ctx.builder.CreateCall(floorintr, x);
    }
    case trunc_llvm: {
        FunctionCallee truncintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::trunc, makeArrayRef(t));
        return ctx.builder.CreateCall(truncintr, x);
    }
    case rint_llvm: {
        FunctionCallee rintintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::rint, makeArrayRef(t));
        return ctx.builder.CreateCall(rintintr, x);
    }
    case sqrt_llvm: {
        FunctionCallee sqrtintr = Intrinsic::getDeclaration(getLLVMModule(ctx), Intrinsic::sqrt, makeArrayRef(t));
