...
static Value *emit_unbox(Type *to, const jl_cgval_t &x, jl_value_t *jt, Value *dest, bool volatile_store)
{
    ...
    if (dest) {
        ...
        alignment = 0;
        builder.CreateMemCpy(dest, p, jl_datatype_size(jt), alignment, volatile_store, x.tbaa, tbaa_stack);
        return NULL;
    }
    else {
        Instruction *load;
...
static jl_cgval_t emit_pointerref(jl_cgval_t *argv, jl_codectx_t *ctx)
{
    ...
    return typed_load(thePtr, im1, ety, ctx, tbaa_data, nullptr, nullptr, true, align_nb);
}
