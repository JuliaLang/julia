...
#define ADD_I(name, nargs)                                                     \
    do {                                                                       \
        Function *func = Function::Create(                                     \
                FunctionType::get(T_prjlvalue, args##nargs, false),            \
...
#undef ALIAS
    ...
    // float_func[copysign_float] = false; // this is actually an integer
    // operation
...
static Constant *julia_const_to_llvm(void *ptr, jl_value_t *bt)
{
    ...
    if (jl_is_cpointer_type(bt))
        return ConstantExpr::getIntToPtr(
                ...
                julia_type_to_llvm(bt));
    if (jl_is_primitivetype(bt)) {
        int nb = jl_datatype_size(bt);
        // TODO: non-power-of-2 size datatypes may not be interpreted correctly
        ...
        case 4: {
            ...
            if (bt == (jl_value_t *)jl_float32_type)
                return ConstantFP::get(
                        jl_LLVMContext,
                        LLVM_FP(APFloat::IEEEsingle, APInt(32, data32)));
        ...
        case 8: {
            ...
            if (bt == (jl_value_t *)jl_float64_type)
                return ConstantFP::get(
                        jl_LLVMContext,
                        LLVM_FP(APFloat::IEEEdouble, APInt(64, data64)));
        ...
        default:
            ...
            // where 16-byte alignment requirement == (8-byte typetag) %
            // (uint64_t ArrayRef access)
            if (nb % 16 != 0) {
            ...
            return ConstantInt::get(
                    IntegerType::get(jl_LLVMContext, 8 * nb), val);
        }
    }
        ...
        bool volatile_store)
{
    ...
    if (x.isghost) {
        ...
        // emit_error("emit_unbox: a type mismatch error in occurred during
        // codegen", ctx);
    ...
    Value *unboxed = NULL;
    if (to == T_int1)
        unboxed = builder.CreateTrunc(
                tbaa_decorate(x.tbaa, builder.CreateLoad(p)), T_int1);
    else if (jt == (jl_value_t *)jl_bool_type)
        unboxed = builder.CreateZExt(
                builder.CreateTrunc(
                        tbaa_decorate(x.tbaa, builder.CreateLoad(p)), T_int1),
                to);
    if (unboxed) {
    ...
    if (dest) {
        ...
        builder.CreateMemCpy(
                dest, p, jl_datatype_size(jt), alignment, volatile_store, tbaa);
        return NULL;
    }
    else {
        Instruction *load;
...
static jl_cgval_t emit_runtime_call(
        JL_I::intrinsic f,
        const jl_cgval_t *argv,
        size_t nargs,
        jl_codectx_t *ctx)
{
...
// put a bits type tag on some value (despite the name, this doesn't necessarily
// actually change anything about the value however)
static jl_cgval_t generic_bitcast(const jl_cgval_t *argv, jl_codectx_t *ctx)
{
    ...
    if (!jl_is_primitivetype(v.typ) || jl_datatype_size(v.typ) != nb) {
        ...
        if (!jl_is_primitivetype(v.typ)) {
            if (isboxed) {
                ...
                error_unless(
                        isbits,
                        "bitcast: expected primitive type value for second "
                        "argument",
                        ctx);
            }
            else {
            ...
            if (isboxed) {
                ...
                error_unless(
                        builder.CreateICmpEQ(
                                size, ConstantInt::get(T_int32, nb)),
                        "bitcast: argument size does not match size of target "
                        "type",
                        ctx);
            }
            else {
    ...
    if (v.ispointer() && vx == NULL) {
        ...
        vx = tbaa_decorate(
                v.tbaa,
                builder.CreateLoad(data_pointer(
                        v,
                        ctx,
...
static jl_cgval_t generic_cast(
        intrinsic f,
        Value *(*generic)(Type *, Value *, jl_codectx_t *),
        const jl_cgval_t *argv,
        jl_codectx_t *ctx,
        bool toint,
        bool fromint)
...
static jl_cgval_t emit_pointerref(jl_cgval_t *argv, jl_codectx_t *ctx)
{
    ...
    if (!jl_isbits(ety)) {
        ...
        if (!jl_is_structtype(ety) || jl_is_array_type(ety) ||
            !jl_is_leaf_type(ety)) {
        ...
        Value *strct = emit_allocobj(
                ctx, size, literal_pointer_val((jl_value_t *)ety));
        im1 = builder.CreateMul(
                im1,
                ConstantInt::get(
                        T_size, LLT_ALIGN(size, jl_datatype_align(ety))));
    ...
    if (!jl_isbits(ety) && ety != (jl_value_t *)jl_any_type) {
        ...
        im1 = builder.CreateMul(
                ...
                ConstantInt::get(
                        T_size, LLT_ALIGN(size, jl_datatype_align(ety))));
        builder.CreateMemCpy(
                builder.CreateGEP(thePtr, im1),
                data_pointer(x, ctx, T_pint8),
                size,
                align_nb);
    }
    else {
...
static Value *emit_checked_srem_int(Value *x, Value *den, jl_codectx_t *ctx)
{
    Type *t = den->getType();
    raise_exception_unless(
            builder.CreateICmpNE(den, ConstantInt::get(t, 0)),
    ...
    builder.CreateCondBr(
            builder.CreateICmpEQ(den, ConstantInt::get(t, -1, true)),
            m1BB,
            okBB);
...
struct math_builder {
        ...
        : old_fmf(builder.getFastMathFlags())
    {
        if (jl_options.fast_math != JL_OPTIONS_FAST_MATH_OFF &&
            (always_fast || jl_options.fast_math == JL_OPTIONS_FAST_MATH_ON)) {
            FastMathFlags fmf;
    ...
    ~math_builder() { builder.setFastMathFlags(old_fmf); }
};

static Value *emit_untyped_intrinsic(
        intrinsic f,
        Value **argvalues,
        size_t nargs,
        jl_codectx_t *ctx,
...
static jl_cgval_t
emit_intrinsic(intrinsic f, jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    if (expected_nargs && expected_nargs != nargs) {
        jl_errorf(
                "intrinsic #%d %s: wrong number of arguments",
                f,
                jl_intrinsic_name((int)f));
    }
    ...
    case arraylen:
        return mark_julia_type(
                emit_arraylen(argv[0], args[1], ctx), false, jl_long_type, ctx);
    case pointerref:
    ...
    case select_value: {
        Value *isfalse = emit_condition(
                argv[0], "select_value", ctx); // emit the first argument
        ...
        if (!isboxed) {
            ...
            ifelse_result = builder.CreateSelect(
                    isfalse, emit_unbox(llt1, y, t1), emit_unbox(llt1, x, t1));
        }
        else {
            ifelse_result =
                    builder.CreateSelect(isfalse, boxed(y, ctx), boxed(x, ctx));
        }
    ...
    default: {
        ...
        Value *r = emit_untyped_intrinsic(
                f, argvalues, nargs, ctx, (jl_datatype_t **)&newtyp, xinfo.typ);
        if (r->getType() == T_int1)
        ...
        jl_value_t *xtyp)
{
    ...
    case srem_int:
        return builder.CreateSRem(x, y);
    case urem_int:
        return builder.CreateURem(x, y);

    // Implements IEEE negate. See issue #7868
    case neg_float:
        return math_builder(ctx)().CreateFSub(ConstantFP::get(t, -0.0), x);
    case neg_float_fast:
        return math_builder(ctx, true)().CreateFNeg(x);
    case add_float:
        return math_builder(ctx)().CreateFAdd(x, y);
    case sub_float:
        return math_builder(ctx)().CreateFSub(x, y);
    case mul_float:
        return math_builder(ctx)().CreateFMul(x, y);
    case div_float:
        return math_builder(ctx)().CreateFDiv(x, y);
    case rem_float:
        return math_builder(ctx)().CreateFRem(x, y);
    case add_float_fast:
        return math_builder(ctx, true)().CreateFAdd(x, y);
    case sub_float_fast:
        return math_builder(ctx, true)().CreateFSub(x, y);
    case mul_float_fast:
        return math_builder(ctx, true)().CreateFMul(x, y);
    case div_float_fast:
        return math_builder(ctx, true)().CreateFDiv(x, y);
    case rem_float_fast:
        return math_builder(ctx, true)().CreateFRem(x, y);
    case fma_float: {
        assert(y->getType() == x->getType());
        assert(z->getType() == y->getType());
        Value *fmaintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::fma, makeArrayRef(t));
        return builder.CreateCall(fmaintr, { x, y, z });
    }
    case muladd_float: {
        assert(y->getType() == x->getType());
        assert(z->getType() == y->getType());
        Value *muladdintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::fmuladd, makeArrayRef(t));
    ...
    case checked_umul_int: {
                                               ...
                                               : (f == checked_usub_int
                                                          ...
                                                          : (f == checked_smul_int
                                                                     ...
                                                                     : Intrinsic::
                                                                               umul_with_overflow)))));
        Value *intr =
                Intrinsic::getDeclaration(jl_Module, intr_id, makeArrayRef(t));
        ...
        *newtyp = tuptyp;
        ...
        tupval =
                builder.CreateInsertValue(tupval, obyte, ArrayRef<unsigned>(1));
        return tupval;
    }

    case checked_sdiv_int: {
        Value *typemin = builder.CreateShl(
                ConstantInt::get(t, 1), t->getPrimitiveSizeInBits() - 1);
        raise_exception_unless(
                builder.CreateAnd(
                        builder.CreateICmpNE(y, ConstantInt::get(t, 0)),
                        builder.CreateOr(
                                builder.CreateICmpNE(
                                        y, ConstantInt::get(t, -1, true)),
    ...
    case ult_int:
        *newtyp = jl_bool_type;
        return builder.CreateICmpULT(x, y);
    case sle_int:
        *newtyp = jl_bool_type;
        return builder.CreateICmpSLE(x, y);
    case ule_int:
        *newtyp = jl_bool_type;
        return builder.CreateICmpULE(x, y);

    case eq_float:
        *newtyp = jl_bool_type;
        return math_builder(ctx)().CreateFCmpOEQ(x, y);
    case ne_float:
        *newtyp = jl_bool_type;
        return math_builder(ctx)().CreateFCmpUNE(x, y);
    case lt_float:
        *newtyp = jl_bool_type;
        return math_builder(ctx)().CreateFCmpOLT(x, y);
    case le_float:
        *newtyp = jl_bool_type;
        return math_builder(ctx)().CreateFCmpOLE(x, y);

    case eq_float_fast:
        *newtyp = jl_bool_type;
        return math_builder(ctx, true)().CreateFCmpOEQ(x, y);
    case ne_float_fast:
        *newtyp = jl_bool_type;
        return math_builder(ctx, true)().CreateFCmpUNE(x, y);
    case lt_float_fast:
        *newtyp = jl_bool_type;
        return math_builder(ctx, true)().CreateFCmpOLT(x, y);
    case le_float_fast:
        *newtyp = jl_bool_type;
        return math_builder(ctx, true)().CreateFCmpOLE(x, y);
    ...
    case shl_int:
        return builder.CreateSelect(
                builder.CreateICmpUGE(
                        y,
                        ConstantInt::get(
                                y->getType(), t->getPrimitiveSizeInBits())),
                ConstantInt::get(t, 0),
    ...
    case ashr_int:
        return builder.CreateSelect(
                ...
                builder.CreateAShr(
                        x,
                        ConstantInt::get(t, t->getPrimitiveSizeInBits() - 1)),
                builder.CreateAShr(x, uint_cnvt(t, y)));

    case bswap_int: {
        Value *bswapintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::bswap, makeArrayRef(t));
        return builder.CreateCall(bswapintr, x);
    }
    case ctpop_int: {
        Value *ctpopintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::ctpop, makeArrayRef(t));
        return builder.CreateCall(ctpopintr, x);
    }
    case ctlz_int: {
        Value *ctlz = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::ctlz, makeArrayRef(t));
        y = ConstantInt::get(T_int1, 0);
        return builder.CreateCall(ctlz, { x, y });
    }
    case cttz_int: {
        Value *cttz = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::cttz, makeArrayRef(t));
        y = ConstantInt::get(T_int1, 0);
        return builder.CreateCall(cttz, { x, y });
    }

    case abs_float: {
        Value *absintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::fabs, makeArrayRef(t));
    ...
    case flipsign_int: {
        ...
        if (cy) {
            ...
            return iy.isNonNegative()
                           ? x
                           : builder.CreateSub(ConstantInt::get(t, 0), x);
        }
        Value *tmp = builder.CreateAShr(
                y,
                ConstantInt::get(t, cast<IntegerType>(t)->getBitWidth() - 1));
        return builder.CreateXor(builder.CreateAdd(x, tmp), tmp);
    }
    case ceil_llvm: {
        Value *ceilintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::ceil, makeArrayRef(t));
        return builder.CreateCall(ceilintr, x);
    }
    case floor_llvm: {
        Value *floorintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::floor, makeArrayRef(t));
        return builder.CreateCall(floorintr, x);
    }
    case trunc_llvm: {
        Value *truncintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::trunc, makeArrayRef(t));
        return builder.CreateCall(truncintr, x);
    }
    case rint_llvm: {
        Value *rintintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::rint, makeArrayRef(t));
        return builder.CreateCall(rintintr, x);
    }
    case sqrt_llvm:
        raise_exception_unless(
                builder.CreateFCmpUGE(x, ConstantFP::get(t, 0.0)),
                literal_pointer_val(jl_domain_exception),
                ctx);
    // fall-through
    case sqrt_llvm_fast: {
        Value *sqrtintr = Intrinsic::getDeclaration(
                jl_Module, Intrinsic::sqrt, makeArrayRef(t));
...
#define BOX_F(ct, jl_ct)                                                       \
    box_##ct##_func = boxfunc_llvm(                                            \
            ft1arg(T_prjlvalue, T_##jl_ct), "jl_box_" #ct, &jl_box_##ct, m);

#define SBOX_F(ct, jl_ct)                                                      \
    BOX_F(ct, jl_ct);                                                          \
    box_##ct##_func->addAttribute(1, Attribute::SExt);
#define UBOX_F(ct, jl_ct)                                                      \
    BOX_F(ct, jl_ct);                                                          \
    box_##ct##_func->addAttribute(1, Attribute::ZExt);

template <typename T>
static Function *
boxfunc_llvm(FunctionType *ft, const std::string &cname, T *addr, Module *m)
{
    Function *f = Function::Create(ft, Function::ExternalLinkage, cname, m);
