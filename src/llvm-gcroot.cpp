...
void JuliaGCAllocator::collapseRedundantRoots()
{
    for (BasicBlock::iterator I = gcframe->getParent()->begin(), E(gcframe); I != E;) {
        ...
        if (callInst && callInst->getCalledValue() == gcroot_func) {
            // see if a root is only used briefly for `store -> load -> store other` pattern
            // or `store, store other`
            // such that the first store can be trivially replaced with just "other" and
            // delete the chain
            ...
            bool       variable_slot = true; // whether this gc-root is only used as a
                                       // variable-slot; e.g. whether theLoad is theValue
    ...
    for (std::map<BasicBlock *, std::map<frame_register, liveness::id>>::iterator
        ...
        for (unsigned arg_offset = 0; arg_offset < arg_n; ++arg_offset) {
            ...
            if (inuse_reg == live_reg->second.end())
                continue;
// TODO: optimize here better when not live in inuse_reg->second, by ascertaining liveness
// at the instruction level for this bb
#endif
...
void JuliaGCAllocator::allocate_frame()
{
    ...
    for (BasicBlock::iterator I = gcframe->getParent()->begin(), E(gcframe); I != E;) {
        ...
        if (callInst && callInst->getCalledValue() == jlcall_frame_func) {
            ...
            // the jlcall frame should have been passed to exactly one call (the jlcall) --
            // find its basic-block
            for (User::use_iterator use = callInst->use_begin(), usee = callInst->use_end();
                 use != usee; ++use) {
#ifdef LLVM35
                User *user = use->getUser();
     ...
     *         if inst matches store-inst # todo: or inst matches "gc-store-inst" (for
     * stores to non-stack-slots)
     *             if inst->operand(0) matches "a call to make-jlcall-frame" (or gep
     * thereof)
     *                 def = <inst, arg-offset>
    ...
    for (std::map<BasicBlock *, std::map<frame_register, liveness::id>>::iterator
        ...
        assert(
            &*bb !=
            &F.getEntryBlock()); // only undef variables should live-out from the entry bb
     ...
     *                 # need to add live value from successor to current block, unless it
     * was already marked
     *                 inuse-list[def] |= live
     *                 changes += 1
     ...
     *                          inuse-list[def] &= ~live
     *                     else
     *                          # not a true kill due to recursion -- the kill happened
     * before this assign in this BB, so it is still live
     *                     changes -= 1
     *     # if the live list changed, make sure all predecessors are in the queue to be
     * reanalyzed
     *     if changes == 0
     *         continue
     *     for pred in predecessors(bb)
 ...
 */
    ...
    if (argSpaceSize + maxDepth == 0) {
        ...
        gcframe->setOperand(
            0, ConstantInt::get(T_int32, 2 + argSpaceSize +
                                             maxDepth)); // fix up the size of the gc frame
        if (tempSlot)
            tempSlot->setOperand(
                1,
                ConstantInt::get(
                    T_int32, 2 + argSpaceSize)); // fix up the offset to the temp slot space
        ...
        builder.SetInsertPoint(&*(++BasicBlock::iterator(
            last_gcframe_inst))); // set insert *before* point, e.g. after the gcframe
        DebugLoc noDbg;
