...
static void tbaa_decorate_gcframe(Instruction *inst, std::set<Instruction *> &visited,
                                  MDNode *tbaa_gcframe)
{
    if (visited.find(inst) != visited.end()) return;
    visited.insert(inst);
#ifdef LLVM35
...
public:
    JuliaGCAllocator(CallInst *ptlsStates, Type *T_pjlvalue, MDNode *tbaa)
        : F(*ptlsStates->getParent()->getParent()), M(*F.getParent()),
          T_int1(Type::getInt1Ty(F.getContext())), T_int8(Type::getInt8Ty(F.getContext())),
          T_int32(Type::getInt32Ty(F.getContext())),
          T_int64(Type::getInt64Ty(F.getContext())),
          V_null(Constant::getNullValue(T_pjlvalue)), ptlsStates(ptlsStates),
          ...
          gcroot_flush_func(M.getFunction("julia.gcroot_flush")), tbaa_gcframe(tbaa)
    {
/* Algorithm sketch:
...
private:
    ...
    bool
    record_usage(CallInst *callInst,
                 std::map<BasicBlock *, std::map<frame_register, liveness::id>> &bb_uses,
                 std::map<BasicBlock *, SmallBitVector> &regs_used, unsigned &offset,
                 bool commit = true);
    unsigned
    find_space_for(CallInst *callInst,
                   std::map<BasicBlock *, std::map<frame_register, liveness::id>> &bb_uses,
...
Instruction *JuliaGCAllocator::get_pgcstack(Instruction *ptlsStates)
{
    Constant *offset = ConstantInt::getSigned(T_int32, offsetof(jl_tls_states_t, pgcstack) /
                                                           sizeof(void *));
    return GetElementPtrInst::Create(LLVM37_param(NULL) ptlsStates,
                                     ArrayRef<Value *>(offset), "jl_pgcstack");
}
        ...
        if (GetElementPtrInst *gepInst = dyn_cast<GetElementPtrInst>(ptr)) {
            if (gepInst->getNumIndices() == 1) {
                ...
                if (frame.first && frame.first->getCalledValue() == jlcall_frame_func)
                    frame.second =
                        cast<ConstantInt>(gepInst->idx_begin()->get())->getZExtValue();
                else
                    frame.first = NULL;
            }
        }
...
void JuliaGCAllocator::collapseRedundantRoots()
{
    for (BasicBlock::iterator I = gcframe->getParent()->begin(), E(gcframe); I != E;) {
        ...
        if (callInst && callInst->getCalledValue() == gcroot_func) {
            // see if a root is only used briefly for `store -> load -> store other`
            // pattern or `store, store other`
            // such that the first store can be trivially replaced with just "other"
            // and delete the chain
            // or if is used for store, but the value is never needed
            StoreInst *theStore      = NULL;
            unsigned   n_stores      = 0;
            bool       variable_slot = true; // whether this gc-root is only used as a
                                             // variable-slot; e.g. whether theLoad is
                                             // theValue
            LoadInst *theLoad = NULL;
            for (User::use_iterator use = callInst->use_begin(), usee = callInst->use_end();
                 use != usee;) {
#ifdef LLVM35
                ...
                if (StoreInst *storeInst = dyn_cast<StoreInst>(user)) {
                    ...
                    if (!theValue->hasOneUse()) { // not just the store
                        variable_slot = false; // this gc-root is used as more than just a
                                               // variable-slot (aka phi node)
                    }
                    n_stores++;
                }
            ...
            if (n_stores == 0) continue;

            if (theLoad == NULL) {
                // this gc-root is never loaded from, so we don't need it as a variable
                // location
                // delete any stores to this gc-root that would be keeping an
                // otherwise-unused value alive
                for (User::use_iterator use  = callInst->use_begin(),
                                        usee = callInst->use_end();
                     use != usee;) {
#ifdef LLVM35
                    User *user = use->getUser();
#else
            ...
            if ((theLoad != NULL && variable_slot) ||
                ...
                if (theValue->hasNUses(
                        theLoad ? 1
                                : 2)) { // only uses are theStore and theLoad and theOther
                    // check if this value is only used for a store to another gcroot
                    User::use_iterator value_use = theValue->use_begin();
                    if (theLoad && *value_use == theStore) ++value_use;
#ifdef LLVM35
                    ...
                    if (theOther && OperandNo != StoreInst::getPointerOperandIndex()) {
                        ...
                        frame_register gcroot_other_gep =
                            get_gcroot(theOther->getPointerOperand());
                        CallInst *gcroot_other = gcroot_other_gep.first;
                        // it could be a gcroot...
                        if (gcroot_other && gcroot_other->getCalledValue() == gcroot_func &&
                            theStore != NULL) {
                            // need to make sure there aren't any other uses of gcroot_other
                            // (including gckill)
                            // between the initial store and the replacement store
                            // TODO: do this better once we have liveness information for
                            // locals?
                            ...
                            while (patternMatchSuccess) {
                                ...
                                for (Instruction::op_iterator op   = inst->op_begin(),
                                    ...
                                    if (op->get() == gcroot_other) {
                                        ...
                                        break; // fail: gcroot_other had another reference,
                                               // can't
...
// iterate the basicblock forward, if it's a simple branch
...
#else
                                    succ_iterator SI = succ_begin(current),
                                                  E  = succ_end(current);
                        ...
                        // ...or it could be a jlcall frame
                        else if (gcroot_other &&
                                 gcroot_other->getCalledValue() == jlcall_frame_func) {
                            ...
                            // locals?
                            if (theStore != NULL &&
                                theOther->getParent() == theStore->getParent()) {
                                // unsigned arg_offset = gcroot_other_gep->second;
                                // frame_register def(gcroot_other, arg_offset);
                                // std::map<frame_register, liveness::id> &inuse_list =
                                // bb_uses[theOther->getParent()];
                                // std::map<frame_register, liveness::id>::iterator
                                // inuse_reg =
                                // inuse_list.find(def);
                                patternMatchSuccess = true;
                            }
                        ...
                        if (patternMatchSuccess) {
                            // do the gcroot merge -- replace gcroot with gcroot_other in
                            // all
                            // the store operations for this gcroot
                            // so that theOther, theLoad, and this gcroot are no longer
                            // needed
                            ...
                            if (theStore) {
                                theStore->setOperand(StoreInst::getPointerOperandIndex(),
                                                     gcroot_other);
                            }
                            else {
                                for (User::use_iterator use  = callInst->use_begin(),
                                    ...
                                    StoreInst *theStore = cast<StoreInst>(user);
                                    theStore->setOperand(
                                        StoreInst::getPointerOperandIndex(), gcroot_other);
                                }
                            }
                            callInst->eraseFromParent();
                        }
...
bool JuliaGCAllocator::record_usage(
    CallInst *callInst,
    std::map<BasicBlock *, std::map<frame_register, liveness::id>> &bb_uses,
    std::map<BasicBlock *, SmallBitVector> &regs_used, unsigned &offset, bool commit)
{
    ...
    for (std::map<BasicBlock *, std::map<frame_register, liveness::id>>::iterator
             live_reg = bb_uses.begin(),
             e        = bb_uses.end();
         live_reg != e; ++live_reg) {
        BasicBlock *    bb   = live_reg->first;
        SmallBitVector &regs = regs_used[bb];
        if (offset + arg_n > regs.size()) regs.resize(offset + arg_n);
        for (unsigned arg_offset = 0; arg_offset < arg_n; ++arg_offset) {
            frame_register def(callInst, arg_offset);
            std::map<frame_register, liveness::id>::iterator inuse_reg =
                live_reg->second.find(def);
            if (inuse_reg == live_reg->second.end()) continue;
// TODO: optimize here better when not live in inuse_reg->second, by
// ascertaining liveness at the instruction level for this bb
#endif
    ...
    if (commit) {
        ...
        // update the offset argument to point to the next open register beyond
        // index
...
unsigned JuliaGCAllocator::find_space_for(
    CallInst *callInst,
    std::map<BasicBlock *, std::map<frame_register, liveness::id>> &bb_uses,
    std::map<BasicBlock *, SmallBitVector> &regs_used)
{
...
void JuliaGCAllocator::rearrangeRoots()
{
    for (auto BB = F.begin(), E(F.end()); BB != E; BB++) {
        ...
        if (!isa<ReturnInst>(terminst) && !isa<UnreachableInst>(terminst)) continue;
        ...
        for (auto I = BB->rbegin(), E(BB->rend()); I != E; ++I) {
            ...
            Instruction *inst = &*I;
            if (isa<CallInst>(inst)) break;
            if (LoadInst *loadInst = dyn_cast<LoadInst>(inst)) {
                CallInst *loadAddr = dyn_cast<CallInst>(loadInst->getPointerOperand());
                if (loadAddr && loadAddr->getCalledValue() == gcroot_func) break;
                continue;
            }
            if (StoreInst *storeInst = dyn_cast<StoreInst>(inst)) {
                CallInst *storeAddr = dyn_cast<CallInst>(storeInst->getPointerOperand());
...
void JuliaGCAllocator::allocate_frame()
{
    ...
    for (BasicBlock::iterator I = gcframe->getParent()->begin(), E(gcframe); I != E;) {
        ...
        if (callInst && callInst->getCalledValue() == jlcall_frame_func) {
            ...
            // the jlcall frame should have been passed to exactly one call (the
            // jlcall) -- find its basic-block
            for (User::use_iterator use = callInst->use_begin(), usee = callInst->use_end();
                 use != usee; ++use) {
#ifdef LLVM35
                User *user = use->getUser();
     ...
     *         if inst matches store-inst # todo: or inst matches "gc-store-inst"
     * (for stores to non-stack-slots)
     *             if inst->operand(0) matches "a call to make-jlcall-frame" (or
     * gep thereof)
     *                 def = <inst, arg-offset>
    ...
    for (std::map<BasicBlock *, std::map<frame_register, liveness::id>>::iterator
                ...
                if (CallInst *callInst = def.first) {
                    if (callInst->getCalledValue() == jlcall_frame_func) {
                        ...
                        if (inuse_reg != inuse_list.end() &&
                            inuse_reg->second == liveness::kill) {
        ...
        if (live_out == 0) continue;
        assert(&*bb != &F.getEntryBlock()); // only undef variables should live-out
                                            // from the entry bb
     ...
     *             if (not assign in op) and not (inuse-list[def] contains live or
     * assign)
     *                 # need to add live value from successor to current block,
     ...
     *                          inuse-list[def] &= ~live
     *                     else
     *                          # not a true kill due to recursion -- the kill
     * happened before this assign in this BB, so it is still live
     *                     changes -= 1
     *     # if the live list changed, make sure all predecessors are in the queue
     * to be reanalyzed
     *     if changes == 0
     *         continue
     *     for pred in predecessors(bb)
    ...
    while (!bb_queue.empty()) {
        ...
        for (succ_iterator SI = succ_begin(bb), SE = succ_end(bb); SI != SE; ++SI) {
            ...
            for (std::map<frame_register, liveness::id>::iterator reg = succ_uses.begin(),
                                                                  rege = succ_uses.end();
                 reg != rege; ++reg) {
    ...
    for (; !frames.empty(); frames.pop()) {
        ...
        if (frame_offset + arg_n > maxDepth) maxDepth = frame_offset + arg_n;
    }
            ...
            // delete any StoreInst to a gcframe slot that isn't live
                ...
                if (CallInst *gcroot = def.first) {
                    if (gcroot->getCalledValue() == jlcall_frame_func) {
                        std::map<frame_register, liveness::id> &inuse_list =
                            bb_uses[storeInst->getParent()];
                        std::map<frame_register, liveness::id>::iterator inuse_reg =
                            inuse_list.find(def);
                        if (inuse_reg == inuse_list.end()) storeInst->eraseFromParent();
                    }
                }
            }
        }
    ...
    if (frame_offsets.empty()) {
        ...
        tempSlot = GetElementPtrInst::Create(
            LLVM37_param(NULL) gcframe, ArrayRef<Value *>(ConstantInt::get(T_int32, 2)));
#ifdef JL_DEBUG_BUILD
        tempSlot->setName("temproots");
#endif
        tempSlot->insertAfter(gcframe);
        if (last_gcframe_inst == gcframe) last_gcframe_inst = tempSlot;

        // finalize all of the jlcall frames by replacing all of the frames with the
        // appropriate gep(tempslot)
        for (std::map<CallInst *, unsigned>::iterator frame = frame_offsets.begin(),
                                                      framee = frame_offsets.end();
             frame != framee; ++frame) {
            CallInst *gcroot = frame->first;
            tbaa_decorate_gcframe(gcroot, tbaa_gcframe);
            Value *            offset[1] = {ConstantInt::get(T_int32, frame->second)};
            GetElementPtrInst *gep = GetElementPtrInst::Create(LLVM37_param(NULL) tempSlot,
                                                               makeArrayRef(offset));
 ...
 */
    ...
    for (BasicBlock::iterator I = gcframe->getParent()->begin(), E(gcframe); I != E;) {
        ...
        if (CallInst *callInst = dyn_cast<CallInst>(inst)) {
            if (callInst->getCalledValue() == gcroot_func) {
                ...
                Instruction *argTempi = GetElementPtrInst::Create(
                    LLVM37_param(NULL) gcframe,
                    ArrayRef<Value *>(ConstantInt::get(T_int32, offset)));
                ...
                if (md) {
                    ...
                    for (User::use_iterator use  = mdValue->use_begin(),
                                            usee = mdValue->use_end();
                         use != usee;) {
                        // need to recreate the dbg_declare accordingly -- sadly llvm can't
                        // handle this in RAUW
                        User *user = use->getUser();
                        ++use;
                        if (CallInst *dbg = dyn_cast<CallInst>(user)) {
                            ...
                            if (called &&
                                called->getIntrinsicID() == Intrinsic::dbg_declare) {
                                DILocalVariable *dinfo = cast<DILocalVariable>(
                                    cast<MetadataAsValue>(dbg->getOperand(1))
                                        ->getMetadata());
                                DIExpression *expr = cast<DIExpression>(
                                    cast<MetadataAsValue>(dbg->getOperand(2))
                                        ->getMetadata());
    ...
    if (argSpaceSize + maxDepth == 0) {
        ...
        if (maxDepth > 0) {
            BitCastInst *tempSlot_i8 = new BitCastInst(
                tempSlot, PointerType::get(T_int8, 0), "", last_gcframe_inst);
            Type *    argsT[2] = {tempSlot_i8->getType(), T_int32};
            Function *memset =
                Intrinsic::getDeclaration(&M, Intrinsic::memset, makeArrayRef(argsT));
            Value *args[5] = {
                tempSlot_i8,                                                // dest
        ...
        gcframe->setOperand(
            0, ConstantInt::get(T_int32,
                                2 + argSpaceSize +
                                    maxDepth)); // fix up the size of the gc frame
        if (tempSlot)
            tempSlot->setOperand(
                1, ConstantInt::get(
                       T_int32,
                       2 + argSpaceSize)); // fix up the offset to the temp slot space
        ...
        builder.SetInsertPoint(
            &*(++BasicBlock::iterator(last_gcframe_inst))); // set insert *before*
                                                            // point, e.g. after the
                                                            // gcframe
        DebugLoc noDbg;
        builder.SetCurrentDebugLocation(noDbg);

        Instruction *inst = builder.CreateStore(
            ConstantInt::get(T_size, (argSpaceSize + maxDepth) << 1),
        ...
        inst = builder.CreateStore(
            builder.CreateLoad(builder.Insert(get_pgcstack(ptlsStates))),
...
struct LowerGCFrame : public FunctionPass {
    ...
    LowerGCFrame(MDNode *_tbaa_gcframe = nullptr)
        : FunctionPass(ID), tbaa_gcframe(_tbaa_gcframe)
    {
    }
    ...
    std::map<BasicBlock *, std::map<frame_register, liveness::id>> &bb_uses)
{
    for (std::map<BasicBlock *, std::map<frame_register, liveness::id>>::iterator
        ...
        for (std::map<frame_register, liveness::id>::iterator
                 regs  = live_reg->second.begin(),
                 regse = live_reg->second.end();
             regs != regse; ++regs) {
