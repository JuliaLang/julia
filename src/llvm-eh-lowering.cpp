                                            ...
                                            AllocaInst *eh_data) const
{
    ...
    for (auto &info: arg_infos) {
        ...
        if (info.arg_cls == ArgInfo::Spill) {
            ...
            auto val = call->getOperand(info.orig_opno);
            // LLVM optimization can make the field type and the value type disagree
            field_addr = builder.CreateBitCast(field_addr, val->getType()->getPointerTo());
                                    ...
                                    ArrayRef<Value*> eh_args, StructType *alloca_ty) const
{
    ...
    for (uint32_t i = 0; i < nargs; i++) {
        ...
        if (info.arg_cls == ArgInfo::Spill) {
            // LLVM might change the type on the caller side when it doesn't affect the ABI
