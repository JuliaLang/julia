...
JL_DLLEXPORT const char *jl_filename =
    "no file"; // need to update jl_critical_error if this is TLS
...
JL_DLLEXPORT jl_module_t *jl_new_main_module(void)
{
    ...
    if (ptls->current_module != jl_main_module && ptls->current_module != NULL &&
        jl_main_module != NULL)
...
void jl_module_run_initializer(jl_module_t *m)
{
    ...
    JL_CATCH {
        if (jl_initerror_type == NULL) {
            ...
            jl_rethrow_other(
                jl_new_struct(jl_initerror_type, m->name, ptls->exception_in_transit));
        }
    }
}
...
jl_value_t *jl_eval_module_expr(jl_expr_t *ex)
{
    ...
    if (parent_module == jl_main_module) jl_module_export(jl_main_module, name);
    ...
    if (outermost == NULL) outermost                          = newm;
...
int jl_has_intrinsics(jl_lambda_info_t *li, jl_value_t *v, jl_module_t *m)
{
    ...
    if (jl_array_len(e->args) == 0) return 0;
    if (e->head == toplevel_sym || e->head == copyast_sym) return 0;
    jl_value_t *e0 = jl_exprarg(e, 0);
    if (e->head == call_sym) {
        jl_value_t *sv = jl_static_eval(e0, NULL, m, li, li != NULL, 0);
        if (sv && jl_typeis(sv, jl_intrinsic_type)) return 1;
    }
    if (0 && e->head == assign_sym &&
        jl_is_ssavalue(e0)) { // code branch needed for *very-linear-mode*, but
    ...
    for (i = 0; i < jl_array_len(e->args); i++) {
        ...
        if (jl_is_expr(a) && jl_has_intrinsics(li, a, m)) return 1;
    }
    return 0;
}
...
static int jl_eval_with_compiler_p(jl_lambda_info_t *li, jl_array_t *body, int compileloops,
                                   jl_module_t *m)
{
    size_t i, maxlabl = 0;
...
static jl_module_t *eval_import_path_(jl_array_t *args, int retrying)
{
    ...
    if (var != dot_sym) {
        ...
        while (1) {
            ...
            if (!jl_is_symbol(var))
                jl_type_error("import or using", (jl_value_t *)jl_sym_type,
                              (jl_value_t *)var);
            i++;
            if (var != dot_sym) {
    ...
    while (1) {
        ...
        if (retrying && require_func) {
            jl_printf(JL_STDERR, "WARNING: requiring \"%s\" in module \"%s\" did not "
                                 "define a corresponding module.\n",
                      jl_symbol_name(var), jl_symbol_name(ptls->current_module->name));
            return NULL;
        }
        else {
    ...
    for (; i < jl_array_len(args) - 1; i++) {
        ...
        if (!jl_is_module(m)) jl_errorf("invalid import statement");
    }
    return m;
}
...
int jl_is_toplevel_only_expr(jl_value_t *e)
{
    return jl_is_expr(e) &&
           (((jl_expr_t *)e)->head == module_sym ||
            ...
            ((jl_expr_t *)e)->head == import_sym || ((jl_expr_t *)e)->head == using_sym ||
            ((jl_expr_t *)e)->head == export_sym || ((jl_expr_t *)e)->head == thunk_sym ||
...
jl_value_t *jl_toplevel_eval_flex(jl_value_t *e, int fast, int expanded)
{
    ...
    if (ex->head == error_sym || ex->head == jl_incomplete_sym) {
        ...
        if (!jl_is_symbol(name)) jl_error("syntax: malformed \"importall\" statement");
        m = (jl_module_t *)jl_eval_global_var(m, name);
        if (!jl_is_module(m))
            jl_errorf("invalid %s statement: name exists but does not refer to a module",
                      jl_symbol_name(ex->head));
        ...
        if (m == NULL) return jl_nothing;
        jl_sym_t *name = (jl_sym_t *)jl_array_ptr_ref(ex->args, jl_array_len(ex->args) - 1);
        if (!jl_is_symbol(name)) jl_error("syntax: malformed \"using\" statement");
        ...
        if (m == NULL) return jl_nothing;
        jl_sym_t *name = (jl_sym_t *)jl_array_ptr_ref(ex->args, jl_array_len(ex->args) - 1);
        if (!jl_is_symbol(name)) jl_error("syntax: malformed \"import\" statement");
        ...
        for (size_t i = 0; i < jl_array_len(ex->args); i++) {
            ...
            if (!jl_is_symbol(name)) jl_error("syntax: malformed \"export\" statement");
    ...
    if (!expanded && ex->head != body_sym && ex->head != thunk_sym &&
        ex->head != return_sym && ex->head != method_sym && ex->head != toplevel_sym) {
        // not yet expanded
    ...
    if (head == thunk_sym) {
        ...
        ewc = jl_eval_with_compiler_p(thk, (jl_array_t *)thk->code, fast,
                                      ptls->current_module);
    }
    else {
        if (head &&
            jl_eval_expr_with_compiler_p((jl_value_t *)ex, fast, ptls->current_module)) {
...
{
    jl_array_t *ary = jl_array_cconvert_cstring((jl_array_t *)(jl_data_ptr(str)[0]));
    JL_GC_PUSH1(&ary);
...
static int svec_contains(jl_svec_t *svec, jl_value_t *x)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (e == x || type_contains(e, x)) return 1;
    }
    return 0;
}
...
{
    if (ty == x) return 1;
    if (jl_is_uniontype(ty)) return svec_contains((jl_svec_t *)jl_fieldref(ty, 0), x);
    if (jl_is_datatype(ty)) return svec_contains(((jl_datatype_t *)ty)->parameters, x);
    return 0;
}
...
void jl_check_static_parameter_conflicts(jl_method_t *m, jl_svec_t *t)
{
    ...
    for (size_t i = 0; i < jl_svec_len(t); i++) {
        for (size_t j = 0; j < nvars; j++) {
            ...
            if (jl_is_typevar(tv)) {
                if ((jl_sym_t *)jl_array_ptr_ref(li->slotnames, j) ==
                    ((jl_tvar_t *)tv)->name) {
                    jl_printf(JL_STDERR, "WARNING: local variable %s conflicts with a "
...
{
    if (jl_is_datatype(a)) {
        if (got_tuple1) return (jl_datatype_t *)a;
        if (jl_is_tuple_type(a)) {
            if (jl_nparams(a) < 1) return NULL;
        ...
        for (i = 1; i < jl_svec_len(ts); i++) {
            ...
            if (ti == NULL || ti->name != dt->name) return NULL;
        }
        return dt;
    }
    return NULL;
...
JL_DLLEXPORT void jl_method_def(jl_svec_t *argdata, jl_lambda_info_t *f,
                                jl_value_t *isstaged)
{
    ...
    if (!jl_is_lambda_info(f)) {
        ...
        // the result is that the closure variables get interpolated directly into
        // the AST
    ...
    if (jl_is_tuple_type(argtypes) && jl_nparams(argtypes) > 0 &&
        !jl_is_type(jl_tparam0(argtypes)))
        ...
        if (!jl_is_type(elt) && !jl_is_typevar(elt)) {
            ...
            if (argname == unused_sym)
                jl_exceptionf(jl_argumenterror_type,
                              "invalid type for argument number %d in "
                              "method definition for %s at %s:%d",
    ...
    for (size_t i = 0; i < jl_svec_len(tvars); i++) {
        ...
        if (!jl_is_typevar(tv))
            jl_type_error_rt(jl_symbol_name(name), "method definition",
                             (jl_value_t *)jl_tvar_type, tv);
        if (!ishidden && !type_contains((jl_value_t *)argtypes, tv)) {
            jl_printf(JL_STDERR,
                      "WARNING: static parameter %s does not occur in signature for %s",
                      jl_symbol_name(((jl_tvar_t *)tv)->name), jl_symbol_name(name));
            print_func_loc(JL_STDERR, m);
    ...
    if (jl_newmeth_tracer) jl_call_tracer(jl_newmeth_tracer, (jl_value_t *)m);
