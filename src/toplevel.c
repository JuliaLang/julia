...
int jl_has_intrinsics(jl_lambda_info_t *li, jl_value_t *v, jl_module_t *m)
{
    ...
    if (0 && e->head == assign_sym &&
        jl_is_ssavalue(
            e0)) { // code branch needed for *very-linear-mode*, but not desirable otherwise
                                   ...
                                   jl_module_t *m)
{
    ...
    for (i = 0; i < jl_array_len(body); i++) {
        ...
        if (jl_has_intrinsics(li, stmt, m))
            return 1;
    }
    return 0;
}
    ...
    for (i = 0; i < na; i++) {
        ...
        if (!jl_is_type(elt) && !jl_is_typevar(elt)) {
            ...
            if (argname == unused_sym)
                jl_exceptionf(jl_argumenterror_type, "invalid type for argument number %d "
                                                     "in method definition for %s at %s:%d",
