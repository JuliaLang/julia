...
// A region is contiguous storage for up to DEFAULT_REGION_PG_COUNT naturally
// aligned GC_PAGE_SZ pages
...
#ifdef _OS_WINDOWS_
    char *mem =
        (char *)VirtualAlloc(NULL, alloc_size + GC_PAGE_SZ, MEM_RESERVE, PAGE_READWRITE);
    if (mem == NULL) return NULL;
#else
    if (GC_PAGE_SZ > jl_page_size) alloc_size += GC_PAGE_SZ;
...
static void jl_gc_alloc_region(region_t *region)
{
    ...
    char *       mem         = NULL;
    while (1) {
        if (__likely((mem = jl_gc_try_alloc_region(pg_cnt)))) break;
    ...
    VirtualAlloc(region->meta, pg_cnt * sizeof(jl_gc_pagemeta_t), MEM_COMMIT,
                 PAGE_READWRITE);
#endif
}
...
NOINLINE void *jl_gc_alloc_page(void)
{
    ...
    while (region_i < REGION_COUNT) {
        ...
        if (region->pages == NULL) jl_gc_alloc_region(region);
        for (i = region->lb; i < region->pg_cnt / 32; i++) {
            if (~region->allocmap[i]) break;
        }
    ...
    if (region->lb < i) region->lb = i;
    if (region->ub < i) region->ub = i;
...
void jl_gc_free_page(void *p)
{
    ...
    if (GC_PAGE_SZ < jl_page_size) {
        ...
        p              = (void *)((uintptr_t)region->pages[pg_idx].data &
                     ~(jl_page_size - 1)); // round down to the nearest page
        pg_idx = page_index(region, p);
...
#endif
no_decommit:
    if (region->lb > pg_idx / 32) region->lb = pg_idx / 32;
    current_pg_count--;
}
