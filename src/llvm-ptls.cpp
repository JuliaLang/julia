...
struct LowerPTLS : public ModulePass {
    ...
    LowerPTLS(bool _imaging_mode = false)
        : ModulePass(ID), imaging_mode(_imaging_mode)
    {
    }
...
static void
ensure_global(const char *name, Type *t, Module &M, bool dllimport = false)
{
    if (M.getNamedValue(name))
        return;
    GlobalVariable *proto = new GlobalVariable(
            M, t, false, GlobalVariable::ExternalLinkage, NULL, name);
#ifdef _OS_WINDOWS_
...
static Instruction *emit_ptls_tp(
        LLVMContext &ctx,
        Value *offset,
        Type *T_ppjlvalue,
        Instruction *insertBefore)
{
    ...
    if (offset) {
        ...
        auto tp = InlineAsm::get(
                FunctionType::get(T_pint8, args, false),
                dyn_asm_str,
        ...
        auto tp = InlineAsm::get(
                FunctionType::get(T_pint8, false),
                const_asm_str.c_str(),
    ...
    return new BitCastInst(
            tls, PointerType::get(T_ppjlvalue, 0), "ptls", insertBefore);
...
// the add for now.
    ...
    if (!offset) {
        auto T_size =
                (sizeof(size_t) == 8 ? Type::getInt64Ty(ctx)
                                     : Type::getInt32Ty(ctx));
        offset = ConstantInt::getSigned(T_size, jl_tls_offset);
    }
    auto tp = InlineAsm::get(
            FunctionType::get(T_pint8, false), asm_str, "=r", false);
    Value *tls = CallInst::Create(tp, "thread_ptr", insertBefore);
    tls = GetElementPtrInst::Create(
        ...
        MDNode *tbaa_const)
{
    ...
    if (imaging_mode) {
        ...
        if (jl_tls_elf_support) {
            GlobalVariable *OffsetGV =
                    cast<GlobalVariable>(M.getNamedValue("jl_tls_offset.val"));
            // if (offset != 0)
            //     ptls = tp + offset;
            // else
            //     ptls = getter();
            ...
            auto phi = PHINode::Create(
                    PointerType::get(T_ppjlvalue, 0), 2, "", ptlsStates);
        ...
        ptlsStates->replaceAllUsesWith(
                emit_ptls_tp(ctx, nullptr, T_ppjlvalue, ptlsStates));
        ptlsStates->eraseFromParent();
    }
    else {
        setCallPtlsAttrs(ptlsStates);
