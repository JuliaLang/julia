...
// cause this process to exit with WEXITSTATUS(signo), after waiting to finish all julia, C,
// and C++ cleanup
...
JL_DLLEXPORT void jl_atexit_hook(int exitcode) JL_NOTSAFEPOINT_ENTER
{
    ...
    if (loop != NULL) {
        ...
        if (ct) {
            while (item) {
                ...
                JL_CATCH {
                    ...
                    uv_unref(item->h);
                    jl_printf((JL_STREAM *)STDERR_FILENO,
                              "error during exit cleanup: close: ");
                    jl_static_show((JL_STREAM *)STDERR_FILENO, jl_current_exception(ct));
                    jl_printf((JL_STREAM *)STDERR_FILENO, "\n");
                    jlbacktrace(); // written to STDERR_FILENO
                    item = next_shutdown_queue_item(item);
        ...
        jl_wake_libuv(); // set the async pending flag, so that future calls are immediate
                         // no-ops on other threads we would like to guarantee this, but
                         // cannot currently, so there is still a small race window that
                         // needs to be fixed in libuv
    }
    if (loop != NULL) {
    ...
    if (ct)
        jl_safepoint_suspend_all_threads(
            ct); // Destroy other threads, so that they don't segfault
    if (ct)
...
static void *init_stdio_handle(const char *stdio, uv_os_fd_t fd, int readable)
{
    ...
    if ((err = uv_dup(fd, &fd)))
        jl_errorf("error initializing %s in uv_dup: %s (%s %d)", stdio, uv_strerror(err),
                  uv_err_name(err), err);
    assert(fd != -1); // This avoids a bug in clang's static analyzer, if an error did not
                      // occur, fd != -1
...
static void restore_fp_env(void)
{
    ...
    if (!jl_options.handle_signals && jl_atomic_load_relaxed(&jl_n_threads) > 1) {
        jl_error(
            "Cannot use `--handle-signals=no` with multiple threads (JULIA_NUM_THREADS > 1).\n"
...
static NOINLINE void _finish_jl_init_(jl_image_buf_t sysimage, jl_ptls_t ptls,
                                      jl_task_t *ct)
{
...
JL_DLLEXPORT void jl_init_(jl_image_buf_t sysimage)
{
    ...
    jl_libjulia_internal_handle =
        jl_find_dynamic_library_by_addr(&jl_load_dynamic_library, /* throw_err */ 1);
    ...
    jl_ntdll_handle = jl_dlopen(
        "ntdll.dll", JL_RTLD_NOLOAD); // bypass julia's pathchecking for system dlls
...
#pragma GCC diagnostic ignored "-Wdangling-pointer"
    ...
    // warning: this changes `jl_current_task`, so be careful not to call that from this
    // function
