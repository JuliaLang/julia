...
static void jl_uv_exitcleanup_add(uv_handle_t *handle, struct uv_shutdown_queue *queue)
{
    struct uv_shutdown_queue_item *item =
        (struct uv_shutdown_queue_item *)malloc(sizeof(struct uv_shutdown_queue_item));
    item->h                            = handle;
    item->next                         = NULL;
...
JL_DLLEXPORT void jl_atexit_hook(int exitcode)
{
    ...
    if (jl_options.code_coverage) jl_write_coverage_data();
    if (jl_options.malloc_log) jl_write_malloc_log();
    if (jl_base_module) {
...
static void *init_stdio_handle(uv_file fd, int readable)
{
    ...
    // On windows however, libuv objects remember streams by their HANDLE, so this
    // is
    // unnecessary.
    fd = dup(fd);
#endif
...
// dup the descriptor with a new one pointing at the bit bucket ...
        ...
        dup2(open("/dev/null", O_RDWR,
                  S_IRUSR | S_IWUSR /* 0600 */ | S_IRGRP | S_IROTH /* 0644 */),
             fd);
#endif
    ...
    case UV_NAMED_PIPE:
        ...
        if (uv_pipe_init(jl_io_loop, (uv_pipe_t *)handle,
...
{ // compute an absolute path location, so
  // that chdir doesn't change the file
  // reference
#ifndef _OS_WINDOWS_
    ...
    if (!out) {
        if (in[0] == PATHSEPSTRING[0]) {
            ...
            if (uv_cwd(path, &path_size)) {
                jl_error("fatal error: unexpected error while retrieving current "
                         "working directory");
            }
...
{   // this function resolves the paths in jl_options to
    // absolute file locations as needed
    ...
    // `free()` function
    // on the original pointer for each `char*` you've inserted into `jl_options`,
    // after
    // calling `julia_init()`
    ...
    if (jl_options.julia_home) jl_options.julia_home = abspath(jl_options.julia_home);
    free(free_path);
    free_path = NULL;
    if (jl_options.image_file) {
        ...
        if (jl_options.image_file) jl_options.image_file = abspath(jl_options.image_file);
        if (free_path) {
            free(free_path);
            free_path = NULL;
        }
    }
    if (jl_options.outputo) jl_options.outputo         = abspath(jl_options.outputo);
    if (jl_options.outputji) jl_options.outputji       = abspath(jl_options.outputji);
    if (jl_options.outputbc) jl_options.outputbc       = abspath(jl_options.outputbc);
    if (jl_options.machinefile) jl_options.machinefile = abspath(jl_options.machinefile);
    if (jl_options.load) jl_options.load               = abspath(jl_options.load);
}
...
void _julia_init(JL_IMAGE_SEARCH rel)
{
    ...
    if (jl_options.cpu_target == NULL) jl_options.cpu_target = "native";
    ...
    if (jl_dbghelp)
        hSymRefreshModuleList =
            (BOOL(WINAPI *)(HANDLE))jl_dlsym(jl_dbghelp, "SymRefreshModuleList");
#endif
    ...
    if ((jl_options.outputo || jl_options.outputbc) &&
        ...
        jl_error("cannot generate code-coverage or track allocation information "
                 "while generating a .o or .bc output file");
    }
    ...
    for (i = 1; i < jl_core_module->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->value && jl_is_datatype(b->value)) {
                ...
                if (tt->name->mt) tt->name->mt->module = jl_core_module;
            }
        }
    }
...
static void julia_save(void)
{
    if (!jl_generating_output()) return;
    ...
    if (jl_options.incremental) {
        if (jl_options.outputji)
            if (jl_save_incremental(jl_options.outputji, worklist)) jl_exit(1);
        ...
        ios_t *s                                         = NULL;
        if (jl_options.outputo || jl_options.outputbc) s = jl_create_system_image();
        ...
        if (jl_options.outputo || jl_options.outputbc)
            jl_dump_native(jl_options.outputbc, jl_options.outputo, (const char *)s->buf,
                           (size_t)s->size);
    }
    JL_GC_POP();
}
