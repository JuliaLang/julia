...
JL_DLLEXPORT const char *__asan_default_options()
{
    ...
    // FIXME: enable LSAN after fixing leaks & defining
    // __lsan_default_suppressions(),
    //        or defining __lsan_default_options = exitcode=0 once publicly
    //        available
...
void jl_init_stack_limits(int ismaster)
{
    ...
    ptls->stack_hi =
            (char *)tib[1]; // Stack Base / Bottom of stack (high address)
    ptls->stack_lo =
            (char *)tib[2]; // Stack Limit / Ceiling of stack (low address)
#else
...
static void
jl_uv_exitcleanup_add(uv_handle_t *handle, struct uv_shutdown_queue *queue)
{
    struct uv_shutdown_queue_item *item =
            (struct uv_shutdown_queue_item *)malloc(
                    sizeof(struct uv_shutdown_queue_item));
    item->h = handle;
    item->next = NULL;
    if (queue->last)
...
static void *init_stdio_handle(uv_file fd, int readable)
{
    ...
    jl_uv_file_t *file;
#ifndef _OS_WINDOWS_
    // Duplicate the file descriptor so we can later dup it over if we want to
    // redirect
    // STDIO without having to worry about closing the associated libuv object.
    // This also helps limit the impact other libraries can cause on our file
    // handle.
    // On windows however, libuv objects remember streams by their HANDLE, so
    // this is
    // unnecessary.
    fd = dup(fd);
#else
    ...
    case UV_TTY:
        ...
        if (uv_tty_init(jl_io_loop, (uv_tty_t *)handle, fd, readable)) {
            jl_errorf(
                    "error initializing stdio in uv_tty_init (%d, %d)",
                    fd,
                    type);
        }
...
// dup the descriptor with a new one pointing at the bit bucket ...
    ...
    case UV_NAMED_PIPE:
        ...
        if (uv_pipe_init(
                    jl_io_loop,
                    (uv_pipe_t *)handle,
                    (readable ? UV_PIPE_READABLE : UV_PIPE_WRITABLE))) {
            jl_errorf(
                    "error initializing stdio in uv_pipe_init (%d, %d)",
                    fd,
                    type);
        }
        if (uv_pipe_open((uv_pipe_t *)handle, fd)) {
            jl_errorf(
                    "error initializing stdio in uv_pipe_open (%d, %d)",
                    fd,
                    type);
        }
    ...
    case UV_TCP:
        ...
        if (uv_tcp_init(jl_io_loop, (uv_tcp_t *)handle)) {
            jl_errorf(
                    "error initializing stdio in uv_tcp_init (%d, %d)",
                    fd,
                    type);
        }
        if (uv_tcp_open((uv_tcp_t *)handle, fd)) {
            jl_errorf(
                    "error initializing stdio in uv_tcp_open (%d, %d)",
                    fd,
                    type);
        }
    ...
    default:
        jl_errorf(
                "this type of handle for stdio is not yet supported (%d, %d)",
                fd,
                type);
        break;
    }
...
char jl_using_intel_jitevents; // Non-zero if running under Intel VTune
                               // Amplifier
#endif
...
{ // compute an absolute path location, so that chdir doesn't change the file
  // reference
#ifndef _OS_WINDOWS_
    ...
    if (!out) {
        if (in[0] == PATHSEPSTRING[0]) {
            ...
            if (uv_cwd(path, &path_size)) {
                jl_error("fatal error: unexpected error while retrieving "
                         "current working directory");
            }
            if (path_size + len + 2 >= PATH_MAX) {
                jl_error(
                        "fatal error: current working directory path too long");
            }
    ...
    if (n <= 0) {
        jl_error("fatal error: jl_options.image_file path too long or "
...
{ // this function resolves the paths in jl_options to absolute file locations
  // as needed
    // and it replaces the pointers to `julia_home`, `julia_bin`, `image_file`,
    // and output file paths
    // it may fail, print an error, and exit(1) if any of these paths are longer
    // than PATH_MAX
    //
    // note: if you care about lost memory, you should call the appropriate
    // `free()` function
    // on the original pointer for each `char*` you've inserted into
    // `jl_options`, after
    // calling `julia_init()`
...
void _julia_init(JL_IMAGE_SEARCH rel)
{
    ...
    jl_io_loop = uv_default_loop(); // this loop will internal events (spawning
                                    // process etc.),
    ...
    jl_arr_xtralloc_limit =
            total_mem / 100; // Extra allocation limited to 1% of total RAM
    jl_find_stack_bottom();
    jl_dl_handle = jl_load_dynamic_library(NULL, JL_RTLD_DEFAULT);
#ifdef _OS_WINDOWS_
    jl_ntdll_handle = jl_dlopen(
            "ntdll.dll", 0); // bypass julia's pathchecking for system dlls
    ...
    if (jl_dbghelp)
        hSymRefreshModuleList = (BOOL(WINAPI *)(HANDLE))jl_dlsym(
                jl_dbghelp, "SymRefreshModuleList");
#else
    ...
    init_stdio();
    // libuv stdio cleanup depends on jl_init_tasks() because JL_TRY is used in
    // jl_atexit_hook()

    if ((jl_options.outputo || jl_options.outputbc) &&
        (jl_options.code_coverage || jl_options.malloc_log)) {
        jl_error("cannot generate code-coverage or track allocation "
                 "information while generating a .o or .bc output file");
    }
...
void jl_get_builtin_hooks(void)
{
    ...
    jl_diverror_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("DivideError"));
    jl_domain_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("DomainError"));
    jl_overflow_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("OverflowError"));
    jl_inexact_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("InexactError"));
    jl_undefref_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("UndefRefError"));
    jl_undefvarerror_type = (jl_datatype_t *)core("UndefVarError");
    jl_interrupt_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("InterruptException"));
    jl_boundserror_type = (jl_datatype_t *)core("BoundsError");
    jl_memory_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("OutOfMemoryError"));
    ...
    jl_segv_exception =
            jl_new_struct_uninit((jl_datatype_t *)core("SegmentationFault"));
#endif
...
void jl_get_builtins(void)
{
    jl_builtin_throw = core("throw");
    jl_builtin_is = core("===");
    jl_builtin_typeof = core("typeof");
    jl_builtin_sizeof = core("sizeof");
    jl_builtin_issubtype = core("issubtype");
    jl_builtin_isa = core("isa");
    jl_builtin_typeassert = core("typeassert");
    jl_builtin__apply = core("_apply");
    jl_builtin_isdefined = core("isdefined");
    jl_builtin_nfields = core("nfields");
    jl_builtin_tuple = core("tuple");
    jl_builtin_svec = core("svec");
    jl_builtin_getfield = core("getfield");
    jl_builtin_setfield = core("setfield!");
    jl_builtin_fieldtype = core("fieldtype");
    jl_builtin_arrayref = core("arrayref");
    jl_builtin_arrayset = core("arrayset");
    jl_builtin_arraysize = core("arraysize");
    jl_builtin_apply_type = core("apply_type");
    jl_builtin_applicable = core("applicable");
    jl_builtin_invoke = core("invoke");
    jl_builtin__expr = core("_expr");
}
