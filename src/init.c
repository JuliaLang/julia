    ...
    if (!out) {
        if (in[0] == PATHSEPSTRING[0]) {
            ...
            if (uv_cwd(path, &path_size)) {
                jl_error("fatal error: unexpected error while retrieving current working "
                         "directory");
            }
...
{   // this function resolves the paths in jl_options to absolute file locations as needed
    // and it replaces the pointers to `julia_home`, `julia_bin`, `image_file`, and output
    // file paths
    // it may fail, print an error, and exit(1) if any of these paths are longer than
    // PATH_MAX
    //
...
void _julia_init(JL_IMAGE_SEARCH rel)
{
    ...
    if ((jl_options.outputo || jl_options.outputbc) &&
        ...
        jl_error("cannot generate code-coverage or track allocation information while "
                 "generating a .o or .bc output file");
    }
