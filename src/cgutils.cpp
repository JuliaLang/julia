                             ...
                             Value *ptindex = nullptr, MDNode *tbaa_ptindex = nullptr)
{
    ...
    if (Order == AtomicOrdering::NotAtomic && !isboxed && (!aliasscope && elty->isAggregateType() && !jl_is_genericmemoryref_type(jltype))) {
        ...
        return mark_julia_slot(val, jltype, nullptr, result_tbaa, std::move(roots));
    }
    Type *realelty = elty;
...
using isother_store_function_type = std::function<void(const jl_cgval_t &)>;
static std::pair<BasicBlock*, BasicBlock*> emit_isother_store(jl_codectx_t &ctx,
        const jl_cgval_t &rhs, jl_value_t *loadtype, jl_value_t *singleton,
        ...
        isother_store_function_type &&emit_store_b)
{
    ...
    //    emit_store_a(mark_julia_const(ctx, singleton));
    //    return {};
    //}
    ...
    ctx.builder.SetInsertPoint(isaBB);
    emit_store_a(ghostValue(ctx, singleton));
    ...
    {
        jl_cgval_t asload = update_julia_type(ctx, rhs, loadtype);
        if (asload.typ != jl_bottom_type)
            emit_store_b(asload);
    }
    ctx.builder.CreateBr(doneBB);
...
static void emit_isother_store(jl_codectx_t &ctx,
        Value *ptr, const jl_cgval_t &rhs, jl_value_t *loadtype, jl_value_t *singleton,
        MDNode *tbaa, isother_store_function_type &&emit_store_b)
{
    auto emit_store_a = [&](const jl_cgval_t &) {
    ...
    emit_isother_store(ctx, rhs, loadtype, singleton, emit_store_a, std::move(emit_store_b));
}
        ...
        Value *ptindex = nullptr, MDNode *tbaa_ptindex = nullptr)
{
    ...
    if (op == StoreKind::Set || (Order == AtomicOrdering::NotAtomic && op == StoreKind::Swap)) {
        ...
        if (is_union) {
            ...
            auto do_store = [&](const jl_cgval_t &rhs) {
                Value *r_local = r;
                if (!r_local) {
            ...
            if (is_other)
                emit_isother_store(ctx, ptr, rhs, loadtype, ((jl_uniontype_t*)jltype)->a, tbaa, do_store);
            else
                do_store(rhs);
        }
    }
    ...
    if (parent != NULL && tracked_pointers && (!isboxed || !type_is_permalloc(rhs.typ))) {
        ...
        if (is_other) {
            ...
            emit_isother_store(ctx, rhs, loadtype, ((jl_uniontype_t*)jltype)->a,
            ...
            for (unsigned i = 0; i < na; i++) {
                ...
                if (kind == JL_FIELDKIND_ISPTR) {
                    ...
                    if (type_is_ghost(fty))
                        continue;
                    auto do_store = [&](const jl_cgval_t &fval_info) {
                        ...
                        if (field_promotable) {
                            ...
                            split_value_into(ctx,
                                (!roots.empty() && fval_info.inline_roots.empty() ? value_to_pointer(ctx, fval_info) : fval_info),
                                align_src, dest, align_dst, jl_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_stack), false);
                            if (!roots.empty()) {
                    ...
                    if (is_other) {
                        auto do_store_a = [&](const jl_cgval_t &) { /* already stored null during initialization */ };
                        auto [FromA, FromB] = emit_isother_store(ctx, fval_info, loadtype, ((jl_uniontype_t*)jtype)->a, do_store_a, do_store);
                        if (init_as_value && fval == nullptr)
