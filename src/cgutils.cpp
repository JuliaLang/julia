        ...
        Value *ptindex = nullptr, MDNode *tbaa_ptindex = nullptr)
{
    ...
    // Emit pre-write barrier before any store.
    // The pre-barrier receives the NEW value being stored, same as the post-barrier.
    // For op == StoreKind::Modify, we don't know r yet. Will be emitted later.
    if (parent != NULL && tracked_pointers && op != StoreKind::Modify)
