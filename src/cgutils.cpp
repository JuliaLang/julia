...
static Value *decay_derived(Value *V)
{
    ...
    if (cast<PointerType>(T)->getAddressSpace() == AddressSpace::Derived)
        return V;
    // Once llvm deletes pointer element types, we won't need it here any more
    // either.
    Type *NewT = PointerType::get(
            cast<PointerType>(T)->getElementType(), AddressSpace::Derived);
...
static Value *mark_callee_rooted(Value *V)
{
    ...
    return builder.CreateAddrSpaceCast(
            V, PointerType::get(T_jlvalue, AddressSpace::CalleeRooted));
}
...
#define JL_FEAT_REQUIRE(ctx, feature)                                          \
    if (!JL_FEAT_TEST(ctx, feature))                                           \
        jl_errorf(                                                             \
                "%s for %s:%d requires the " #feature                          \
                " language feature, which is disabled",                        \
                __FUNCTION__,                                                  \
                (ctx)->file.str().c_str(),                                     \
                *(ctx)->line);
...
template <int N>
static inline void
_hook_call(jl_value_t *hook, std::array<jl_value_t *, N> args)
{
    jl_value_t **argv;
...
static Value *stringConstPtr(IRBuilder<> &builder, const std::string &txt)
{
    ...
    StringMap<GlobalVariable *>::iterator pooledval =
            stringConstants
                    .insert(std::pair<StringRef, GlobalVariable *>(ctxt, NULL))
                    .first;
    ...
    if (imaging_mode) {
        if (pooledval->second == NULL) {
            ...
            GlobalVariable *gv = get_pointer_to_constant(
                    ...
                    *shadow_output);
            pooledval->second = gv;
            jl_ExecutionEngine->addGlobalMapping(
                    gv, (void *)(uintptr_t)pooledtxt.data());
        }
        ...
        Value *v = ConstantExpr::getIntToPtr(
                ConstantInt::get(T_size, (uintptr_t)pooledtxt.data()), T_pint8);
        return v;
    }
}
...
static DIType *
julia_type_to_di(jl_value_t *jt, DIBuilder *dbuilder, bool isboxed = false)
{
    if (isboxed)
    ...
    if (jl_is_abstracttype(jt) || !jl_is_datatype(jt) || jl_is_array_type(jt) ||
        ...
        jt == (jl_value_t *)jl_simplevector_type ||
        jt == (jl_value_t *)jl_datatype_type ||
    ...
    if (jl_is_primitivetype(jt)) {
        ...
        for (unsigned i = 0; i < ntypes; i++)
            Elements.push_back(julia_type_to_di(
                    jl_svecref(jst->types, i), dbuilder, false));
        dbuilder->replaceArrays(
                ct, dbuilder->getOrCreateArray(ArrayRef<Metadata *>(Elements)));
        return ct;
    }
    else {
        assert(jl_is_datatype(jt));
        jdt->ditype = dbuilder->createTypedef(
                jl_pvalue_dillvmt,
                jl_symbol_name(jdt->name->name),
                NULL,
                0,
...
static Value *emit_pointer_from_objref(Value *V)
{
    ...
    CallInst *Call =
            builder.CreateCall(prepare_call(pointer_from_objref_func), V);
...
// and thus, this should typically be avoided in new API's
    ...
    return ConstantExpr::getPointerBitCastOrAddrSpaceCast(
            ConstantExpr::getIntToPtr(
                    ConstantInt::get(T_int64, (uint64_t)p), T_pjlvalue),
            t);
#else
    return ConstantExpr::getPointerBitCastOrAddrSpaceCast(
            ConstantExpr::getIntToPtr(
                    ConstantInt::get(T_int32, (uint32_t)p), T_pjlvalue),
            t);
#endif
}
...
static Value *
julia_pgv(const char *prefix, jl_sym_t *name, jl_module_t *mod, void *addr)
{
    // emit a GlobalVariable for a jl_value_t, using the prefix, name, and
    // module to
...
{
    // emit a pointer to a jl_value_t* which will allow it to be valid across
    // reloading code
    ...
    if (jl_is_datatype(p)) {
        ...
        return maybe_decay_untracked(
                julia_pgv("+", addr->name->name, addr->name->module, p));
    }
    if (jl_is_method(p)) {
    ...
    if (jl_is_method_instance(p)) {
        ...
        if (jl_is_method(linfo->def.method))
            return maybe_decay_untracked(julia_pgv(
                    "-",
                    linfo->def.method->name,
                    linfo->def.method->module,
                    p));
    }
    if (jl_is_symbol(p)) {
...
{
    // emit a pointer to any jl_value_t which will be valid across reloading
    // code
    if (p == NULL)
        return V_null;
    if (!imaging_mode)
...
// bitcast a value, but preserve its address space when dealing with pointer
// types
static Value *emit_bitcast(Value *v, Type *jl_value)
{
    if (isa<PointerType>(jl_value) &&
        v->getType()->getPointerAddressSpace() !=
                jl_value->getPointerAddressSpace()) {
        // Cast to the proper address space
        Type *jl_value_addr = PointerType::get(
                cast<PointerType>(jl_value)->getElementType(),
                v->getType()->getPointerAddressSpace());
...
static Value *julia_binding_gv(Value *bv)
{
    Value *offset = ConstantInt::get(
            T_size, offsetof(jl_binding_t, value) / sizeof(size_t));
...
static Type *
julia_struct_to_llvm(jl_value_t *jt, jl_unionall_t *ua_env, bool *isboxed);
...
// (which is conservatively approximated here by asking whether the types of any
// of the
...
{
    if (dt->layout || dt->struct_decl || jl_is_primitivetype(dt) ||
        jl_isbits(dt))
        return true;
    if (ua) {
...
static Type *
julia_struct_to_llvm(jl_value_t *jt, jl_unionall_t *ua, bool *isboxed)
{
    // this function converts a Julia Type into the equivalent LLVM struct
    // use this where C-compatible (unboxed) structs are desired
    // use julia_type_to_llvm directly when you want to preserve Julia's type
    // semantics
    if (isboxed)
        *isboxed = false;
    if (jt == (jl_value_t *)jl_bottom_type)
    ...
    if ((isTuple || jl_is_structtype(jt)) && !jl_is_array_type(jt)) {
        ...
        if (jst->struct_decl == NULL) {
            ...
            if (!isTuple) {
                structdecl = StructType::create(
                        jl_LLVMContext, jl_symbol_name(jst->name->name));
            ...
            for (i = 0; i < ntypes; i++) {
                ...
                if (jst->layout)
                    ...
                    isptr = jl_isbits(ty) && jl_is_leaf_type(ty) &&
            ...
            if (allghost) {
                assert(jst->layout ==
                       NULL); // otherwise should have been caught above
                ...
                if (isarray && lasttype != T_int1 && !type_is_ghost(lasttype)) {
                    if (isvector &&
                        jl_special_vector_alignment(ntypes, jlasttype) != 0)
                        jst->struct_decl = VectorType::get(lasttype, ntypes);
                    else
                        jst->struct_decl = ArrayType::get(lasttype, ntypes);
                }
                else {
                    jst->struct_decl = StructType::get(
                            jl_LLVMContext,
                            ArrayRef<Type *>(&latypes[0], ntypes));
                }
            }
#ifndef JL_NDEBUG
            // If LLVM and Julia disagree about alignment, much trouble ensues,
            // so check it!
            if (jst->layout) {
                const DataLayout &DL =
#if JL_LLVM_VERSION >= 40000
...
#endif
                unsigned llvm_alignment =
                        DL.getABITypeAlignment((Type *)jst->struct_decl);
                ...
                // TODO: Fix alignment calculation in LLVM, as well as in the GC
...
static bool deserves_sret(jl_value_t *dt, Type *T)
{
    ...
    return (size_t)jl_datatype_size(dt) > sizeof(void *) &&
           !T->isFloatingPointTy() && !T->isVectorTy();
}
        ...
        unsigned &counter)
{
    ...
    if (jl_is_uniontype(ty)) {
        bool allunbox =
                for_each_uniontype_small(f, ((jl_uniontype_t *)ty)->a, counter);
        allunbox &=
                for_each_uniontype_small(f, ((jl_uniontype_t *)ty)->b, counter);
        return allunbox;
    }
...
static Value *emit_nthptr_addr(Value *v, ssize_t n, bool gctracked = true)
{
    return builder.CreateGEP(
            emit_bitcast(gctracked ? decay_derived(v) : v, T_pprjlvalue),
...
{
    return builder.CreateGEP(
            emit_bitcast(gctracked ? decay_derived(v) : v, T_pprjlvalue), idx);
}
...
static Value *emit_nthptr_recast(
        Value *v, Value *idx, MDNode *tbaa, Type *ptype, bool gctracked = true)
{
    // p = (jl_value_t**)v; *(ptype)&p[n]
    Value *vptr = emit_nthptr_addr(v, idx, gctracked);
    return tbaa_decorate(
            tbaa, builder.CreateLoad(emit_bitcast(vptr, ptype), false));
}

static Value *emit_nthptr_recast(
        Value *v, ssize_t n, MDNode *tbaa, Type *ptype, bool gctracked = true)
{
    // p = (jl_value_t**)v; *(ptype)&p[n]
...
{
    ssize_t offset =
            (sizeof(jl_taggedvalue_t) - offsetof(jl_taggedvalue_t, type)) /
            sizeof(jl_value_t *);
    return emit_nthptr_addr(p, -offset);
}

static Value *
boxed(const jl_cgval_t &v, jl_codectx_t *ctx, bool gcooted = true);
static Value *boxed(const jl_cgval_t &v, jl_codectx_t *ctx, jl_value_t *type) =
        delete; // C++11 (temporary to prevent rebase error)
...
{
    assert(tt != NULL && !isa<AllocaInst>(tt) &&
           "expected a conditionally boxed value");
...
static jl_cgval_t emit_typeof(const jl_cgval_t &p, jl_codectx_t *ctx)
{
    ...
    if (p.TIndex) {
        Value *tindex =
                builder.CreateAnd(p.TIndex, ConstantInt::get(T_int8, 0x7f));
        Value *pdatatype;
        unsigned counter;
        counter = 0;
        bool allunboxed = for_each_uniontype_small(
                [&](unsigned idx, jl_datatype_t *jt) {}, p.typ, counter);
        if (allunboxed)
            ...
            pdatatype = emit_bitcast(
                    emit_typeptr_addr(builder.CreateLoad(p.gcroot)),
                    T_ppjlvalue);
        }
        counter = 0;
        for_each_uniontype_small(
                [&](unsigned idx, jl_datatype_t *jt) {
                    Value *cmp = builder.CreateICmpEQ(
                            tindex, ConstantInt::get(T_int8, idx));
                    pdatatype = builder.CreateSelect(
                            cmp,
                            decay_derived(emit_bitcast(
                                    literal_pointer_val_slot((jl_value_t *)jt),
                                    T_ppjlvalue)),
        ...
        Value *datatype;
        if (allunboxed) {
            datatype = tbaa_decorate(
                    tbaa_const,
                    builder.CreateLoad(maybe_decay_untracked(pdatatype)));
        }
        else {
            datatype = maybe_decay_untracked(mask_gc_bits(
                    tbaa_decorate(tbaa_tag, builder.CreateLoad(pdatatype))));
        }
        return mark_julia_type(
                datatype, true, jl_datatype_type, ctx, /*needsroot*/ false);
    }
...
static Value *emit_datatype_types(Value *dt)
{
    return tbaa_decorate(
            tbaa_const,
            builder.CreateLoad(emit_bitcast(
                    builder.CreateGEP(
                            emit_bitcast(decay_derived(dt), T_pint8),
                            ConstantInt::get(
                                    T_size, offsetof(jl_datatype_t, types))),
                    T_ppjlvalue)));
}

static Value *emit_datatype_nfields(Value *dt)
{
    Value *nf = tbaa_decorate(
            tbaa_const,
            builder.CreateLoad(tbaa_decorate(
                    tbaa_const,
                    builder.CreateLoad(emit_bitcast(
                            builder.CreateGEP(
                                    emit_bitcast(decay_derived(dt), T_pint8),
...
{
    Value *size = tbaa_decorate(
            tbaa_const,
            builder.CreateLoad(emit_bitcast(
                    builder.CreateGEP(
                            emit_bitcast(decay_derived(dt), T_pint8),
                            ConstantInt::get(
                                    T_size, offsetof(jl_datatype_t, size))),
                    T_pint32)));
    return size;
}
...
{
    Value *mutabl = tbaa_decorate(
            tbaa_const,
            builder.CreateLoad(builder.CreateGEP(
                    emit_bitcast(decay_derived(dt), T_pint8),
                    ConstantInt::get(
                            T_size, offsetof(jl_datatype_t, mutabl)))));
    return builder.CreateTrunc(mutabl, T_int1);
}

static Value *emit_datatype_abstract(Value *dt)
{
    Value *abstract = tbaa_decorate(
            tbaa_const,
            builder.CreateLoad(builder.CreateGEP(
                    emit_bitcast(decay_derived(dt), T_pint8),
                    ConstantInt::get(
                            T_size, offsetof(jl_datatype_t, abstract)))));
    return builder.CreateTrunc(abstract, T_int1);
}

static Value *emit_datatype_isbitstype(Value *dt)
{
    Value *immut = builder.CreateXor(
            emit_datatype_mutabl(dt), ConstantInt::get(T_int1, -1));
    Value *nofields = builder.CreateICmpEQ(
            emit_datatype_nfields(dt), ConstantInt::get(T_size, 0));
    Value *isbitstype = builder.CreateAnd(
            immut,
            builder.CreateAnd(
                    nofields,
                    builder.CreateXor(
                            builder.CreateAnd(
                                    emit_datatype_abstract(dt),
                                    builder.CreateICmpSGT(
                                            emit_datatype_size(dt),
...
static void emit_error(const std::string &txt, jl_codectx_t *ctx)
{
    ...
    BasicBlock *cont =
            BasicBlock::Create(jl_LLVMContext, "after_error", ctx->f);
...
static void
raise_exception(Value *exc, jl_codectx_t *ctx, BasicBlock *contBB = nullptr)
{
    if (JL_HOOK_TEST(ctx->params, raise_exception)) {
        ...
        builder.CreateCall(
                prepare_call(jlthrow_func), { mark_callee_rooted(exc) });
    }
    builder.CreateUnreachable();
    if (!contBB) {
...
static void raise_exception_if(Value *cond, Value *exc, jl_codectx_t *ctx)
{
    raise_exception_unless(
            builder.CreateXor(cond, ConstantInt::get(T_int1, -1)), exc, ctx);
}
...
static inline void
maybe_mark_argument_dereferenceable(Argument *A, jl_value_t *jt)
{
    if (!jl_is_leaf_type(jt)) {
        return;
    }
...
maybe_mark_load_dereferenceable(Instruction *LI, bool can_be_null, size_t size)
{
    if (!size) {
        return LI;
    }
    ...
    LI->setMetadata(
            can_be_null ? "dereferenceable_or_null" : "dereferenceable",
            MDNode::get(jl_LLVMContext, OPs));
    return LI;
}

static inline Instruction *maybe_mark_load_dereferenceable(
        Instruction *LI, bool can_be_null, jl_value_t *jt)
{
    if (!jl_is_leaf_type(jt)) {
        return LI;
    }
...
{
    raise_exception_unless(
            builder.CreateICmpNE(v, Constant::getNullValue(v->getType())),
        ...
        jl_codectx_t *ctx)
{
    ...
    builder.CreateCall(
            prepare_call(jltypeerror_func),
            { fname_val,
              msg_val,
              type,
              mark_callee_rooted(boxed(x, ctx, false)) });
}

static std::pair<Value *, bool> emit_isa(
        const jl_cgval_t &x,
        jl_value_t *type,
        const std::string *msg,
        jl_codectx_t *ctx)
{
    ...
    if (known_isa) {
        if (!*known_isa && msg) {
            ...
            builder.CreateUnreachable();
            BasicBlock *failBB =
                    BasicBlock::Create(jl_LLVMContext, "fail", ctx->f);
            builder.SetInsertPoint(failBB);
        }
    ...
    if (jl_has_intersect_type_not_kind(type)) {
        ...
        return std::make_pair(
                builder.CreateICmpNE(
                        builder.CreateCall(
                                prepare_call(jlisa_func), { vx, vtyp }),
                        ConstantInt::get(T_int32, 0)),
                false);
    }
    ...
    if (jl_is_leaf_type(type)) {
        if (x.TIndex) {
            ...
            if (tindex > 0) {
                // optimize more when we know that this is a split union-type
                // where tindex = 0 is invalid
                Value *xtindex = builder.CreateAnd(
                        x.TIndex, ConstantInt::get(T_int8, 0x7f));
                return std::make_pair(
                        builder.CreateICmpEQ(
                                xtindex, ConstantInt::get(T_int8, tindex)),
                        false);
            }
            else {
                // test for (x.TIndex == 0x80 && typeof(x.V) == type)
                Value *isboxed = builder.CreateICmpEQ(
                        x.TIndex, ConstantInt::get(T_int8, 0x80));
                BasicBlock *currBB = builder.GetInsertBlock();
                BasicBlock *isaBB =
                        BasicBlock::Create(jl_LLVMContext, "isa", ctx->f);
                BasicBlock *postBB =
                        BasicBlock::Create(jl_LLVMContext, "post_isa", ctx->f);
                builder.CreateCondBr(isboxed, isaBB, postBB);
                builder.SetInsertPoint(isaBB);
                Value *istype_boxed = builder.CreateICmpEQ(
                        emit_typeof(x.V),
    ...
    return std::make_pair(
            builder.CreateICmpNE(
                    builder.CreateCall(
                            prepare_call(jlsubtype_func),
                            { vxt, literal_pointer_val(type) }),
                    ConstantInt::get(T_int32, 0)),
            false);
}

static void emit_typecheck(
        const jl_cgval_t &x,
        jl_value_t *type,
        const std::string &msg,
        jl_codectx_t *ctx)
...
static void
emit_leafcheck(Value *typ, const std::string &msg, jl_codectx_t *ctx)
{
    assert(typ->getType() == T_pjlvalue);
    emit_typecheck(
            mark_julia_type(typ, true, jl_any_type, ctx, false),
            (jl_value_t *)jl_datatype_type,
            msg,
            ctx);
    Value *isleaf;
    isleaf = builder.CreateConstInBoundsGEP1_32(
            T_int8,
            emit_bitcast(decay_derived(typ), T_pint8),
            offsetof(jl_datatype_t, isleaftype));
...
static Value *emit_bounds_check(
        const jl_cgval_t &ainfo,
        jl_value_t *ty,
        Value *i,
        Value *len,
        jl_codectx_t *ctx)
{
    ...
    if (bounds_check_enabled(ctx)) {
        ...
        if (!ty) { // jl_value_t** tuple (e.g. the vararg)
            builder.CreateCall(
                    prepare_call(jlvboundserror_func), { ainfo.V, len, i });
        }
        else if (ainfo.isboxed) { // jl_datatype_t or boxed jl_value_t
            builder.CreateCall(
                    prepare_call(jlboundserror_func),
                    { mark_callee_rooted(boxed(ainfo, ctx)), i });
        }
            ...
            builder.CreateCall(
                    prepare_call(jluboundserror_func),
...
// might be stricter than the Julia alignment for jltype, return the alignment
// of jltype.
// Otherwise return the given alignment.
//
// Parameter ptr should be the pointer argument for the LoadInst or StoreInst.
// It is currently unused, but might be used in the future for a more precise
// answer.
static unsigned
julia_alignment(Value * /*ptr*/, jl_value_t *jltype, unsigned alignment)
{
    if (!alignment) {
...
static jl_cgval_t typed_load(
        Value *ptr,
        Value *idx_0based,
        jl_value_t *jltype,
        jl_codectx_t *ctx,
        MDNode *tbaa,
        bool maybe_null_if_boxed = true,
        unsigned alignment = 0)
{
    ...
    Value *elt;
    // TODO: can only lazy load if we can create a gc root for ptr for the
    // lifetime of elt
    // if (elty->isAggregateType() && tbaa == tbaa_immut && !alignment) { // can
    // lazy load on demand, no copy needed
    //    elt = data;
    //}
    // else {
    Instruction *load = builder.CreateAlignedLoad(
            data,
            isboxed ? alignment : julia_alignment(data, jltype, alignment),
            false);
    if (isboxed)
...
static void typed_store(
        Value *ptr,
        Value *idx_0based,
        const jl_cgval_t &rhs,
        jl_value_t *jltype,
        jl_codectx_t *ctx,
        MDNode *tbaa,
        ...
        unsigned alignment = 0,
        bool root_box =
                true) // if the value to store needs a box, should we root it ?
{
    bool isboxed;
    Type *elty = julia_type_to_llvm(jltype, &isboxed);
    if (type_is_ghost(elty))
    ...
    Instruction *store = builder.CreateAlignedStore(
            r,
            builder.CreateGEP(data, idx_0based),
...
static Value *julia_bool(Value *cond)
{
    return builder.CreateSelect(
            cond, literal_pointer_val(jl_true), literal_pointer_val(jl_false));
}
...
static Value *
data_pointer(const jl_cgval_t &x, jl_codectx_t *ctx, Type *astype = T_ppjlvalue)
{
    Value *data = x.V;
    if (x.constant) {
...
static bool emit_getfield_unknownidx(
        jl_cgval_t *ret,
        const jl_cgval_t &strct,
        Value *idx,
        jl_datatype_t *stt,
        jl_codectx_t *ctx)
{
    ...
    if (strct.ispointer()) { // boxed or stack
        if (is_datatype_all_pointers(stt)) {
            ...
            *ret = mark_julia_type(
                    ...
                    jl_any_type,
                    ctx,
                    strct.gcroot || !strct.isimmutable);
            return true;
        }
            ...
            if (!stt->mutabl) {
                ...
                Value *addr = builder.CreateGEP(
                        emit_bitcast(
                                decay_derived(ptr), PointerType::get(fty, 0)),
                        idx);
            ...
            *ret = typed_load(ptr, idx, jt, ctx, strct.tbaa, false);
            ...
            Value *fld = builder.CreateCall(
                    prepare_call(jlgetnthfieldchecked_func),
                    { boxed(strct, ctx), idx });
        ...
        if (sizeof(void *) != sizeof(int))
            idx0 = builder.CreateTrunc(
                    idx0, T_int32); // llvm3.3 requires this, harmless elsewhere
...
static jl_cgval_t emit_getfield_knownidx(
        const jl_cgval_t &strct,
        unsigned idx,
        jl_datatype_t *jt,
        jl_codectx_t *ctx)
    ...
    if (strct.ispointer()) {
        ...
        if (isboxed) {
            ...
            if (VectorType *vlt = dyn_cast<VectorType>(lt)) {
                // doesn't have the struct wrapper, so this must have been a
                // VecElement
        ...
        if (jl_field_isptr(jt, idx)) {
            ...
            Instruction *Load = maybe_mark_load_dereferenceable(
                    ...
                    maybe_null,
                    jl_field_type(jt, idx));
            Value *fldv = tbaa_decorate(strct.tbaa, Load);
            if (maybe_null)
                null_pointer_check(fldv, ctx);
            return mark_julia_type(
                    fldv, true, jfty, ctx, strct.gcroot || !strct.isimmutable);
        }
        else if (!jt->mutabl) {
            // just compute the pointer and let user load it when necessary
        ...
        if (strct.V->getType()->isVectorTy()) {
            fldv = builder.CreateExtractElement(
                    strct.V, ConstantInt::get(T_int32, idx));
        }
        else {
...
static Value *emit_n_varargs(jl_codectx_t *ctx)
{
    ...
    Value *valen = builder.CreateSub(
            (Value *)ctx->argCount, ConstantInt::get(T_int32, nreq));
#ifdef _P64
...
static void maybe_alloc_arrayvar(int s, jl_codectx_t *ctx)
{
    ...
    if (arraytype_constshape(jt)) {
        ...
        if (isboxed)
            elt = T_prjlvalue;
        // CreateAlloca is OK here because maybe_alloc_arrayvar is only called
        // in the prologue setup
...
static Value *
emit_arraysize(const jl_cgval_t &tinfo, Value *dim, jl_codectx_t *ctx)
{
    Value *t = boxed(tinfo, ctx);
    int o = offsetof(jl_array_t, nrows) / sizeof(void *) - 1;
    MDNode *tbaa =
            arraytype_constshape(tinfo.typ) ? tbaa_const : tbaa_arraysize;
    return emit_nthptr_recast(
            t,
            builder.CreateAdd(dim, ConstantInt::get(dim->getType(), o)),
            tbaa,
            T_psize);
}
...
static Value *
emit_arraysize(const jl_cgval_t &tinfo, int dim, jl_codectx_t *ctx)
{
...
static Value *emit_arraylen_prim(const jl_cgval_t &tinfo, jl_codectx_t *ctx)
{
    ...
    if (jl_is_long(p1)) {
        ...
        Value *alen = jl_Module->getOrInsertFunction(
                "jl_array_len_", ft); // TODO: move to codegen init block
...
static Value *
emit_arraylen(const jl_cgval_t &tinfo, jl_value_t *ex, jl_codectx_t *ctx)
{
...
static Value *
emit_arrayptr(const jl_cgval_t &tinfo, jl_codectx_t *ctx, bool isboxed = false)
{
    Value *t = boxed(tinfo, ctx);
    Value *addr = builder.CreateStructGEP(
            nullptr,
...
static Value *emit_arrayptr(
        const jl_cgval_t &tinfo,
        jl_value_t *ex,
        jl_codectx_t *ctx,
        bool isboxed = false)
{
...
static Value *emit_arraysize(
        const jl_cgval_t &tinfo, jl_value_t *ex, int dim, jl_codectx_t *ctx)
{
    jl_arrayvar_t *av = arrayvar_for(ex, ctx);
...
static void
assign_arrayvar(jl_arrayvar_t &av, const jl_cgval_t &ainfo, jl_codectx_t *ctx)
{
    tbaa_decorate(
            tbaa_arrayptr,
            builder.CreateStore(
                    emit_bitcast(
                            emit_arrayptr(ainfo, ctx),
...
// Returns the size of the array represented by `tinfo` for the given dimension
// `dim` if
// `dim` is a valid dimension, otherwise returns constant one.
static Value *emit_arraysize_for_unsafe_dim(
        const jl_cgval_t &tinfo,
        jl_value_t *ex,
        size_t dim,
        size_t nd,
        jl_codectx_t *ctx)
{
    return dim > nd ? ConstantInt::get(T_size, 1)
                    : emit_arraysize(tinfo, ex, dim, ctx);
}
        ...
        if (k < nidxs - 1) {
            ...
            if (bc) {
                ...
                builder.CreateCondBr(
                        builder.CreateICmpULT(ii, d), okBB, failBB);
    ...
    if (bc) {
        ...
        if (linear_indexing) {
            ...
            if (nidxs > 1) {
                ...
                BasicBlock *partidx =
                        BasicBlock::Create(jl_LLVMContext, "partlinidx");
                BasicBlock *partidxwarn =
                        BasicBlock::Create(jl_LLVMContext, "partlinidxwarn");
                Value *d = emit_arraysize_for_unsafe_dim(
                        ainfo, ex, nidxs, nd, ctx);
                builder.CreateCondBr(
                        builder.CreateICmpULT(ii, d), endBB, partidx);
                ...
                builder.CreateCondBr(
                        builder.CreateICmpULT(i, alen), partidxwarn, failBB);
                ...
                builder.CreateCall(
                        prepare_call(jldepwarnpi_func),
                        ConstantInt::get(T_size, nidxs));
                builder.CreateBr(endBB);
            }
            else {
                Value *alen = emit_arraylen(ainfo, ex, ctx);
                builder.CreateCondBr(
                        builder.CreateICmpULT(i, alen), endBB, failBB);
            }
        }
        else {
            // Compare the last index of the access against the last dimension
            // of
            // the accessed array, i.e. `if !(last_index < last_dimension) goto
            // error`.
            assert(nd >= 0);
            Value *last_index = ii;
            Value *last_dimension =
                    emit_arraysize_for_unsafe_dim(ainfo, ex, nidxs, nd, ctx);
            builder.CreateCondBr(
                    builder.CreateICmpULT(last_index, last_dimension),
                    endBB,
                    failBB);
        }
        ...
        // CreateAlloca is OK here since we are on an error branch
        Value *tmp =
                builder.CreateAlloca(T_size, ConstantInt::get(T_size, nidxs));
        for (size_t k = 0; k < nidxs; k++) {
            builder.CreateStore(
                    idxs[k],
                    builder.CreateGEP(tmp, ConstantInt::get(T_size, k)));
        }
        builder.CreateCall(
                prepare_call(jlboundserrorv_func),
                { mark_callee_rooted(a),
                  tmp,
                  ConstantInt::get(T_size, nidxs) });
        builder.CreateUnreachable();
...
static void init_bits_value(
        Value *newv,
        Value *v,
        MDNode *tbaa,
        unsigned alignment = sizeof(
                void *)) // min alignment in julia's gc is pointer-aligned
{
    ...
    tbaa_decorate(
            ...
            builder.CreateAlignedStore(
                    v,
                    emit_bitcast(newv, PointerType::get(v->getType(), 0)),
                    alignment));
}

static void init_bits_cgval(
        Value *newv, const jl_cgval_t &v, MDNode *tbaa, jl_codectx_t *ctx)
{
    // newv should already be tagged
    if (v.ispointer()) {
        builder.CreateMemCpy(
                newv,
                data_pointer(v, ctx, T_pint8),
                jl_datatype_size(v.typ),
                sizeof(void *));
    }
    else {
...
static jl_value_t *static_constant_instance(Constant *constant, jl_value_t *jt)
{
    ...
    if (cint != NULL) {
        ...
        return jl_new_bits(
                jt, const_cast<uint64_t *>(cint->getValue().getRawData()));
    }
    ...
    if (ce != NULL) {
        if (ce->isCast()) {
            return static_constant_instance(
                    dyn_cast<Constant>(ce->getOperand(0)), jt);
        }
    }
    ...
    for (size_t i = 0; i < nargs; i++) {
        tupleargs[i] = static_constant_instance(
                constant->getAggregateElement(i), jl_tparam(jt, i));
    }
...
static Value *
_boxed_special(const jl_cgval_t &vinfo, Type *t, jl_codectx_t *ctx)
{
    ...
    if (ctx->linfo && jl_is_method(ctx->linfo->def.method) &&
        !vinfo.ispointer()) { // don't bother codegen pre-boxing for toplevel
    ...
    if (jb == jl_int8_type)
        ...
        box = builder.CreateCall(
                prepare_call(box_float32_func), as_value(t, vinfo));
    // if (jb == jl_float64_type)
    //  box = builder.CreateCall(box_float64_func, as_value(t, vinfo);
    // for Float64, fall through to generic case below, to inline alloc & init
    // of Float64 box. cheap, I know.
...
static Value *box_union(
        const jl_cgval_t &vinfo, jl_codectx_t *ctx, const SmallBitVector &skip)
{
    ...
    //   br post_box_union
    // post_box_union:
    //   box = phi [ box1, box_union_1 ], [ box2, box_union_2 ], [ vinfo,
    //   box_union_isboxed ]
    //   ...
    Value *tindex = vinfo.TIndex;
    BasicBlock *defaultBB =
            BasicBlock::Create(jl_LLVMContext, "box_union_isboxed", ctx->f);
    SwitchInst *switchInst = builder.CreateSwitch(tindex, defaultBB);
    BasicBlock *postBB =
            BasicBlock::Create(jl_LLVMContext, "post_box_union", ctx->f);
    builder.SetInsertPoint(postBB);
    ...
    for_each_uniontype_small(
            [&](unsigned idx, jl_datatype_t *jt) {
                ...
                BasicBlock *tempBB =
                        BasicBlock::Create(jl_LLVMContext, "box_union", ctx->f);
                ...
                if (type_is_ghost(t)) {
                    ...
                    jl_cgval_t vinfo_r =
                            jl_cgval_t(vinfo, (jl_value_t *)jt, NULL);
                    box = _boxed_special(vinfo_r, t, ctx);
                    if (!box) {
                        box = emit_allocobj(
                                ctx,
                                jl_datatype_size(jt),
                                literal_pointer_val((jl_value_t *)jt));
                        init_bits_cgval(
                                box,
                                vinfo_r,
                                jl_is_mutable(jt) ? tbaa_mutab : tbaa_immut,
                                ctx);
                    }
                }
    ...
    if (skip.size() > 0 && skip[0]) {
        ...
        // We're guaranteed here that Load(.gcroot) == .V, because we have
        // determined
        // that this union is a boxed value, rather than an interior pointer of
        // some sort
...
static Value *boxed(const jl_cgval_t &vinfo, jl_codectx_t *ctx, bool gcrooted)
{
    ...
    if (vinfo.isboxed) {
        ...
        // We're guaranteed here that Load(.gcroot) == .V, because we have
        // determined
        // that this value is a box, so if it has a gcroot, that's where the
        // value is.
    ...
    if (vinfo.TIndex) {
        ...
        assert(jl_isbits(jt) && jl_is_leaf_type(jt) &&
               "This type shouldn't have been unboxed.");
        Type *t = julia_type_to_llvm(jt);
        assert(!type_is_ghost(t)); // ghost values should have been handled by
                                   // vinfo.constant above!
...
// copy src to dest, if src is isbits. if skip is true, the value of dest is
// undefined
static void emit_unionmove(
        Value *dest,
        const jl_cgval_t &src,
        Value *skip,
        bool isVolatile,
        MDNode *tbaa,
        jl_codectx_t *ctx)
    ...
    if (jl_is_leaf_type(src.typ) || src.constant) {
        ...
        if (jl_isbits(typ)) {
            if (!src.ispointer() || src.constant) {
                ...
                if (skip) // copy dest -> dest to simulate an undef value /
                          // conditional copy
                    src_ptr = builder.CreateSelect(skip, dest, src_ptr);
                unsigned nb = jl_datatype_size(typ);
                unsigned alignment = 0;
                builder.CreateMemCpy(
                        dest, src_ptr, nb, alignment, isVolatile, tbaa);
            }
        }
    }
    else if (src.TIndex) {
        Value *tindex =
                builder.CreateAnd(src.TIndex, ConstantInt::get(T_int8, 0x7f));
        if (skip)
            tindex = builder.CreateSelect(
                    skip, ConstantInt::get(T_int8, 0), tindex);
        Value *src_ptr = data_pointer(src, ctx, T_pint8);
        if (dest->getType() != T_pint8)
            dest = emit_bitcast(dest, T_pint8);
        BasicBlock *defaultBB =
                BasicBlock::Create(jl_LLVMContext, "union_move_skip", ctx->f);
        SwitchInst *switchInst = builder.CreateSwitch(tindex, defaultBB);
        BasicBlock *postBB =
                BasicBlock::Create(jl_LLVMContext, "post_union_move", ctx->f);
        unsigned counter = 0;
        bool allunboxed = for_each_uniontype_small(
                [&](unsigned idx, jl_datatype_t *jt) {
                    unsigned nb = jl_datatype_size(jt);
                    unsigned alignment = 0;
                    BasicBlock *tempBB = BasicBlock::Create(
                            jl_LLVMContext, "union_move", ctx->f);
                    builder.SetInsertPoint(tempBB);
                    switchInst->addCase(ConstantInt::get(T_int8, idx), tempBB);
                    if (nb > 0)
                        builder.CreateMemCpy(
        ...
        if (!skip && allunboxed && (src.V == NULL || isa<AllocaInst>(src.V))) {
            Function *trap_func = Intrinsic::getDeclaration(
                    ctx->f->getParent(), Intrinsic::trap);
            builder.CreateCall(trap_func);
            builder.CreateUnreachable();
        }
        else {
        ...
        if (skip)
            copy_bytes = builder.CreateSelect(
                    skip,
                    ConstantInt::get(copy_bytes->getType(), 0),
                    copy_bytes);
        builder.CreateMemCpy(
                dest,
...
static void emit_cpointercheck(
        const jl_cgval_t &x, const std::string &msg, jl_codectx_t *ctx)
{
    Value *t = emit_typeof_boxed(x, ctx);
    emit_typecheck(
            mark_julia_type(t, true, jl_any_type, ctx, false),
            (jl_value_t *)jl_datatype_type,
            msg,
            ctx);

    Value *istype = builder.CreateICmpEQ(
            mark_callee_rooted(emit_datatype_name(t)),
            mark_callee_rooted(
    ...
    emit_type_error(
            x, literal_pointer_val((jl_value_t *)jl_pointer_type), msg, ctx);
    builder.CreateUnreachable();
...
static Value *emit_allocobj(jl_codectx_t *ctx, size_t static_size, Value *jt)
{
    ...
    if (offset < 0) {
        Value *args[] = {
            ptls_ptr, ConstantInt::get(T_size, static_size + sizeof(void *))
        };
        v = builder.CreateCall(
                prepare_call(jlalloc_big_func), ArrayRef<Value *>(args, 2));
    }
    else {
        Value *pool_offs = ConstantInt::get(T_int32, offset);
        Value *args[] = {
            ptls_ptr, pool_offs, ConstantInt::get(T_int32, osize)
        };
        v = builder.CreateCall(
                prepare_call(jlalloc_pool_func), ArrayRef<Value *>(args, 3));
    }
    tbaa_decorate(
            tbaa_tag,
            builder.CreateStore(
                    maybe_decay_untracked(jt), emit_typeptr_addr(v)));
    return v;
}
...
static void emit_write_barrier(jl_codectx_t *ctx, Value *parent, Value *ptr)
{
    ...
    Value *parent_old_marked =
            builder.CreateICmpEQ(parent_bits, ConstantInt::get(T_size, 3));

    BasicBlock *cont = BasicBlock::Create(jl_LLVMContext, "cont");
    BasicBlock *barrier_may_trigger =
            BasicBlock::Create(jl_LLVMContext, "wb_may_trigger", ctx->f);
    BasicBlock *barrier_trigger =
            BasicBlock::Create(jl_LLVMContext, "wb_trigger", ctx->f);
    builder.CreateCondBr(parent_old_marked, barrier_may_trigger, cont);

    builder.SetInsertPoint(barrier_may_trigger);
    Value *ptr_mark_bit = builder.CreateAnd(
            tbaa_decorate(
                    tbaa_tag,
                    builder.CreateLoad(
                            emit_bitcast(emit_typeptr_addr(ptr), T_psize))),
            1);
    Value *ptr_not_marked =
            builder.CreateICmpEQ(ptr_mark_bit, ConstantInt::get(T_size, 0));
    builder.CreateCondBr(ptr_not_marked, barrier_trigger, cont);
    builder.SetInsertPoint(barrier_trigger);
    builder.CreateCall(
            prepare_call(queuerootfun),
            maybe_decay_untracked(emit_bitcast(parent, T_prjlvalue)));
...
static void
emit_checked_write_barrier(jl_codectx_t *ctx, Value *parent, Value *ptr)
{
    BasicBlock *cont;
    Value *not_null = builder.CreateICmpNE(
            mark_callee_rooted(ptr), mark_callee_rooted(V_null));
    BasicBlock *if_not_null =
            BasicBlock::Create(jl_LLVMContext, "wb_not_null", ctx->f);
        ...
        bool wb)
{
    if (sty->mutabl || !checked) {
        ...
        Value *addr = builder.CreateGEP(
                data_pointer(strct, ctx, T_pint8),
                ConstantInt::get(T_size, jl_field_offset(sty, idx0)));
        jl_value_t *jfty = jl_svecref(sty->types, idx0);
        if (jl_field_isptr(sty, idx0)) {
            Value *r = maybe_decay_untracked(
                    boxed(rhs, ctx, false)); // don't need a temporary gcroot
                                             // since it'll be rooted by strct
            tbaa_decorate(
                    strct.tbaa,
                    builder.CreateStore(r, emit_bitcast(addr, T_pprjlvalue)));
            if (wb && strct.isboxed)
            ...
            typed_store(
                    ...
                    ctx,
                    strct.tbaa,
                    data_pointer(strct, ctx, T_pjlvalue),
                    align);
        }
    }
    else {
...
static jl_cgval_t emit_new_struct(
        jl_value_t *ty, size_t nargs, jl_value_t **args, jl_codectx_t *ctx)
{
    ...
    if (nf > 0) {
        if (jl_isbits(sty)) {
            ...
            // whether we should perform the initialization with the struct as a
            // IR value
            // or instead initialize the stack buffer with stores
            bool init_as_value = false;
            if (lt->isVectorTy() || jl_is_vecelement_type(ty) ||
            ...
            for (size_t i = 0; i < na; i++) {
                ...
                if (!type_is_ghost(fty)) {
                    ...
                    if (!init_as_value) {
                        ...
                        dest = builder.CreateConstInBoundsGEP2_32(
                                lt, strct, 0, i);
                    }
                    ...
                    if (init_as_value) {
                        if (lt->isVectorTy())
                            strct = builder.CreateInsertElement(
                                    strct,
                                    fval,
                                    ConstantInt::get(T_int32, idx));
                        else if (jl_is_vecelement_type(ty))
                            strct = fval; // VecElement type comes unwrapped in
                                          // LLVM.
                        else if (lt->isAggregateType())
                            strct = builder.CreateInsertValue(
                                    strct, fval, ArrayRef<unsigned>(&idx, 1));
                        else
                            assert(false);
                    }
                }
        ...
        Value *strct = emit_allocobj(
                ctx,
                jl_datatype_size(sty),
...
static Value *emit_exc_in_transit(jl_codectx_t *ctx)
{
    ...
    Constant *offset = ConstantInt::getSigned(
            T_int32,
...
static void emit_signal_fence(void)
{
    ...
    builder.CreateCall(InlineAsm::get(
            FunctionType::get(T_void, false), "", "~{memory}", true));
#else
...
{
    Value *ptls =
            emit_bitcast(ctx->ptlsStates, PointerType::get(T_sigatomic, 0));
    Constant *offset = ConstantInt::getSigned(
            T_int32,
            offsetof(jl_tls_states_t, defer_signal) / sizeof(sig_atomic_t));
    return builder.CreateGEP(
            ptls, ArrayRef<Value *>(offset), "jl_defer_signal");
}
...
static int compare_cgparams(const jl_cgparams_t *a, const jl_cgparams_t *b)
{
    return (a->cached == b->cached) &&
           // language features
           (a->runtime == b->runtime) && (a->exceptions == b->exceptions) &&
