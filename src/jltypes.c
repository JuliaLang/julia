                                     ...
                                     cenv_t *eqc, variance_t var)
{
    ...
    if (a == (jl_value_t *)jl_any_type || a == jl_ANY_flag)
        return b;
    if (b == (jl_value_t *)jl_any_type || b == jl_ANY_flag)
        return a;
    // tuple
...
static int solve_tvar_constraints(cenv_t *env, cenv_t *soln, jl_value_t **tvs, int ntv)
{
    ...
    while (1) {
        ...
        // 3. given T, let S´ = intersect(all S s.t. (T=S) or (S=T) ∈ env). add (T=S´) to
        // soln.
        for (int i = 0; i < env->n; i += 2) {
...
static int type_eqv__(jl_value_t *a, jl_value_t *b, int distinguish_tctor)
{
    if (a == b)
        return 1;
    if (distinguish_tctor && jl_is_typector(a) != jl_is_typector(b))
        return 0;
    if (jl_is_typector(a))
        a = (jl_value_t *)((jl_typector_t *)a)->body;
    if (jl_is_typector(b))
...
static int typekey_compare(jl_datatype_t *tt, jl_value_t **key, size_t n)
{
    size_t j;
    if (tt == NULL)
        return -1; // place NULLs at end to allow padding for fast growing
...
static int jl_tuple_morespecific(jl_datatype_t *cdt, jl_datatype_t *pdt, int invariant)
{
    ...
    while (1) {
        ...
        */
        if (cseq && pseq)
            return clenr >= plenr || some_morespecific;
        ci++;
        pi++;
    }
    return 0;
                               ...
                               int morespecific, int invariant)
{
    ...
    if (!invariant && parent == (jl_value_t *)jl_any_type)
        return jl_true;
    if (child == (jl_value_t *)jl_any_type)
        return jl_false;
