...
static int has_free_typevars(jl_value_t *v, jl_typeenv_t *env)
{
    ...
    if (jl_is_unionall(v)) {
        ...
        return has_free_typevars(ua->var->lb, env) ||
               has_free_typevars(ua->var->ub, env) ||
...
static void
find_free_typevars(jl_value_t *v, jl_typeenv_t *env, jl_array_t *out)
{
    if (jl_typeis(v, jl_tvar_type)) {
        if (v == jl_ANY_flag)
            return;
...
static int jl_has_bound_typevars(jl_value_t *v, jl_typeenv_t *env)
{
    ...
    if (jl_is_datatype(v)) {
        if (!((jl_datatype_t *)v)->hasfreetypevars &&
            !(env && env->var == (jl_tvar_t *)jl_ANY_flag))
            return 0;
        size_t i;
        for (i = 0; i < jl_nparams(v); i++) {
...
static int
_jl_has_typevar_from_ua(jl_value_t *t, jl_unionall_t *ua, jl_typeenv_t *prev)
{
...
static void
flatten_type_union(jl_value_t **types, size_t n, jl_value_t **out, size_t *idx)
{
    size_t i;
    for (i = 0; i < n; i++) {
        jl_value_t *e = types[i];
...
JL_DLLEXPORT jl_value_t *jl_type_union(jl_value_t **ts, size_t n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!(jl_is_type(pi) || jl_is_typevar(pi)) || jl_is_vararg_type(pi))
            jl_type_error_rt(
                    "Union", "parameter", (jl_value_t *)jl_type_type, pi);
    }
    if (n == 1)
        return ts[0];
...
jl_new_typevar(jl_sym_t *name, jl_value_t *lb, jl_value_t *ub)
{
    if ((lb != jl_bottom_type && !jl_is_type(lb) && !jl_is_typevar(lb)) ||
        ...
        jl_type_error_rt(
                "TypeVar", "lower bound", (jl_value_t *)jl_type_type, lb);
    if ((ub != (jl_value_t *)jl_any_type && !jl_is_type(ub) &&
         !jl_is_typevar(ub)) ||
        jl_is_vararg_type(ub))
        jl_type_error_rt(
                "TypeVar", "upper bound", (jl_value_t *)jl_type_type, ub);
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_tvar_t *tv =
            (jl_tvar_t *)jl_gc_alloc(ptls, sizeof(jl_tvar_t), jl_tvar_type);
    tv->name = name;
    tv->lb = lb;
    tv->ub = ub;
    return tv;
...
{
    if (jl_is_uniontype(type) || jl_is_unionall(type))
        return 1;
    ...
    for (j = 0; j < n; j++) {
        ...
        if (tj != kj) {
            ...
            if (dk->uid != dt->uid) {
                ...
                int cmp = typekey_compare(
                        dt, jl_svec_data(dk->parameters), jl_nparams(dk));
                if (cmp != 0)
                    return cmp;
            }
        }
...
static int dt_compare(const void *ap, const void *bp)
{
    ...
    if (a == NULL)
        return 1;
    return typekey_compare(
            b, jl_svec_data(a->parameters), jl_svec_len(a->parameters));
}
...
static int typekey_eq(jl_datatype_t *tt, jl_value_t **key, size_t n)
{
    ...
    if (tt->name == jl_type_typename) {
        ...
        return (kj == tj ||
                (jl_typeof(tj) == jl_typeof(kj) && jl_types_equal(tj, kj)));
    }
    for (j = 0; j < n; j++) {
        jl_value_t *kj = key[j], *tj = jl_svecref(tt->parameters, j);
...
static ssize_t
lookup_type_idx(jl_typename_t *tn, jl_value_t **key, size_t n, int ordered)
{
    if (n == 0)
        return -1;
    if (ordered) {
        ...
        /*
          When a module is replaced, the new versions of its types are different
          but
          cannot be distinguished by typekey_compare, since the TypeNames can
          only
          be distinguished by addresses, which don't have a reliable order. So
          we
          need to allow sequences of typekey_compare-equal types in the ordered
          cache.
        */
...
static jl_value_t *lookup_type(jl_typename_t *tn, jl_value_t **key, size_t n)
{
    ...
    jl_value_t *t =
            (idx < 0) ? NULL
                      : jl_svecref(ord ? tn->cache : tn->linearcache, idx);
    return t;
}
...
jl_value_t *jl_cache_type_(jl_datatype_t *type)
{
    if (is_cacheable(type)) {
        ...
        int ord = is_typekey_ordered(
                jl_svec_data(type->parameters), jl_svec_len(type->parameters));
        ssize_t idx = lookup_type_idx(
                type->name,
                jl_svec_data(type->parameters),
...
static jl_value_t *inst_datatype(
        jl_datatype_t *dt,
        jl_svec_t *p,
        jl_value_t **iparams,
        size_t ntp,
...
jl_value_t *jl_apply_type(jl_value_t *tc, jl_value_t **params, size_t n)
{
    ...
    size_t i;
    if (n > 1) {
        // detect common case of applying a wrapper, where we know that all
        // parameters will
        // end up as direct parameters of a certain datatype, which can be
        // optimized.
        ...
        if (jl_is_datatype(u) && n == jl_nparams((jl_datatype_t *)u) &&
            ...
            return inst_datatype(
                    (jl_datatype_t *)u, NULL, params, n, cacheable, NULL);
        }
    }
    JL_GC_PUSH1(&tc);
    jl_value_t *tc0 = tc;
    for (i = 0; i < n; i++) {
        ...
        if (!jl_has_free_typevars(ua->var->lb) &&
            !jl_has_free_typevars(ua->var->ub) &&
            ...
            // if this is a wrapper, let check_datatype_parameters give the
            // error
            if (!iswrapper)
                jl_type_error_rt(
                        "Type",
...
jl_apply_type2(jl_value_t *tc, jl_value_t *p1, jl_value_t *p2)
{
    jl_value_t **args;
...
static jl_value_t *inst_type_w_(
        jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack, int check);
static jl_svec_t *
inst_all(jl_svec_t *p, jl_typeenv_t *env, jl_typestack_t *stack, int check);

JL_DLLEXPORT jl_value_t *
jl_instantiate_unionall(jl_unionall_t *u, jl_value_t *p)
{
...
static jl_value_t *lookup_type_stack(
        jl_typestack_t *stack,
        jl_datatype_t *tt,
        size_t ntp,
        jl_value_t **iparams)
...
void jl_precompute_memoized_dt(jl_datatype_t *dt)
{
    ...
    dt->isleaftype = !dt->abstract ||
                     (jl_type_type != NULL && dt->name == jl_type_typename);
    for (i = 0; i < l; i++) {
        ...
        if (dt->isleaftype)
            dt->isleaftype =
                    (istuple ? (jl_is_leaf_type(p) || p == jl_bottom_type)
                             : !dt->hasfreetypevars);
    }
}

static void
check_datatype_parameters(jl_typename_t *tn, jl_value_t **params, size_t np)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if (!within_typevar(params[i], bounds[2 * i], bounds[2 * i + 1])) {
            // TODO: pass a new version of `tv` containing the instantiated
            // bounds
            jl_type_error_rt(
...
// convert `Vararg{X, Y} where T` to `Vararg{X where T, Y}` where T doesn't
// occur free in Y
        ...
        jl_typeenv_t *env)
{
    ...
    if (istuple) {
        ...
        ndt->super = (jl_datatype_t *)inst_type_w_(
                (jl_value_t *)dt->super, env, stack, 1);
...
// This is the environment needed for instantiating a type's supertype and field
// types.
static jl_value_t *inst_datatype_env(
        jl_value_t *dt,
        jl_svec_t *p,
        jl_value_t **iparams,
        size_t ntp,
        ...
        int c)
{
    if (jl_is_datatype(dt))
        return inst_datatype_inner(
                (jl_datatype_t *)dt, p, iparams, ntp, cacheable, stack, env);
    assert(jl_is_unionall(dt));
    jl_unionall_t *ua = (jl_unionall_t *)dt;
    jl_typeenv_t e = { ua->var, iparams[c], env };
    return inst_datatype_env(
            ua->body, p, iparams, ntp, cacheable, stack, &e, c + 1);
}
...
{
    return inst_datatype_env(
            dt->name->wrapper, p, iparams, ntp, cacheable, stack, NULL, 0);
}

static jl_tupletype_t *
jl_apply_tuple_type_v_(jl_value_t **p, size_t np, jl_svec_t *params)
{
    int cacheable = 1;
    ...
    jl_datatype_t *ndt = (jl_datatype_t *)inst_datatype(
            jl_anytuple_type, params, p, np, cacheable, NULL);
    return ndt;
}

JL_DLLEXPORT jl_tupletype_t *jl_apply_tuple_type(jl_svec_t *params)
{
    return jl_apply_tuple_type_v_(
            jl_svec_data(params), jl_svec_len(params), params);
}
...
{
    return (jl_datatype_t *)inst_datatype(
            jl_anytuple_type, p, jl_svec_data(p), jl_svec_len(p), 1, NULL);
}

jl_datatype_t *jl_inst_concrete_tupletype_v(jl_value_t **p, size_t np)
{
    return (jl_datatype_t *)inst_datatype(
            jl_anytuple_type, NULL, p, np, 1, NULL);
}
...
inst_all(jl_svec_t *p, jl_typeenv_t *env, jl_typestack_t *stack, int check)
{
    ...
    for (i = 0; i < lp; i++) {
        jl_svecset(
                np,
                i,
                (jl_value_t *)inst_type_w_(
                        jl_svecref(p, i), env, stack, check));
    }
    JL_GC_POP();
    return np;
}

static jl_value_t *inst_tuple_w_(
        jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack, int check)
{
    ...
    jl_value_t *result = inst_datatype(
            (jl_datatype_t *)tt, ip_heap, iparams, ntp, cacheable, stack);
    JL_GC_POP();
    return result;
}

static jl_value_t *
inst_type_w_(jl_value_t *t, jl_typeenv_t *env, jl_typestack_t *stack, int check)
{
    size_t i;
    if (jl_is_typevar(t)) {
        jl_typeenv_t *e = env;
        while (e != NULL) {
            if (e->var == (jl_tvar_t *)t) {
                ...
                // if (check && !jl_is_typevar(val) && !within_typevar(val,
                // (jl_tvar_t*)t)) {
                //    jl_type_error_rt("type parameter",
                //                     jl_symbol_name(((jl_tvar_t*)t)->name), t,
                //                     val);
                //}
                return val;
            }
    ...
    if (jl_is_unionall(t)) {
        ...
        jl_typeenv_t newenv = {
            ua->var, (jl_value_t *)((jl_unionall_t *)res)->var, env
        };
    ...
    jl_value_t *result = inst_datatype(
            (jl_datatype_t *)tt, NULL, iparams, ntp, cacheable, stack);
    JL_GC_POP();
    return result;
}

jl_value_t *instantiate_with(
        jl_value_t *t,
        jl_value_t **env,
        size_t n,
        jl_typeenv_t *te,
...
static jl_value_t *_jl_instantiate_type_in_env(
        jl_value_t *ty,
        jl_unionall_t *env,
        jl_value_t **vals,
        jl_typeenv_t *prev)
{
    ...
    if (jl_is_unionall(env->body))
        return _jl_instantiate_type_in_env(
                ty, (jl_unionall_t *)env->body, vals + 1, &en);
    else
        return inst_type_w_(ty, &en, NULL, 1);
}

JL_DLLEXPORT jl_value_t *jl_instantiate_type_in_env(
        jl_value_t *ty, jl_unionall_t *env, jl_value_t **vals)
{
    jl_value_t *typ = ty;
...
void jl_reinstantiate_inner_types(jl_datatype_t *t) // can throw!
{
    ...
    for (j = 0; j < partial_inst.len; j++) {
        ...
        ndt->super = (jl_datatype_t *)instantiate_with(
                (jl_value_t *)t->super, env, n, NULL, &top);
        jl_gc_wb(ndt, ndt->super);
    }
        ...
        for (j = 0; j < partial_inst.len; j++) {
            ...
            for (k = 0; k < jl_svec_len(t->types); k++) {
                jl_svecset(
                        ...
                        k,
                        instantiate_with(
                                jl_svecref(t->types, k), env, n, NULL, &top));
            }
...
void jl_init_types(void)
{
    ...
    jl_emptysvec =
            (jl_svec_t *)jl_gc_permobj(sizeof(void *), jl_simplevector_type);
    jl_svec_set_len_unsafe(jl_emptysvec, 0);

    jl_any_type = (jl_datatype_t *)jl_new_abstracttype(
            (jl_value_t *)jl_symbol("Any"), core, NULL, jl_emptysvec);
    jl_any_type->super = jl_any_type;
    jl_type_type = (jl_unionall_t *)jl_new_abstracttype(
            (jl_value_t *)jl_symbol("Type"), core, jl_any_type, jl_emptysvec);
    ...
    jl_datatype_type->name->names = jl_perm_symsvec(
            16,
            "name",
            "super",
            "parameters",
    ...
    jl_typename_type->name->mt =
            jl_new_method_table(jl_typename_type->name->name, core);
    jl_typename_type->super = jl_any_type;
    jl_typename_type->parameters = jl_emptysvec;
    jl_typename_type->name->names = jl_perm_symsvec(
            8,
            "name",
            "module",
            "names",
    ...
    jl_simplevector_type->name =
            jl_new_typename_in(jl_symbol("SimpleVector"), core);
    jl_simplevector_type->name->wrapper = (jl_value_t *)jl_simplevector_type;
    jl_simplevector_type->name->mt =
            jl_new_method_table(jl_simplevector_type->name->name, core);
    ...
    jl_void_type = jl_new_datatype(
            ...
            0,
            0);
    jl_astaggedvalue(jl_nothing)->header =
            ((uintptr_t)jl_void_type) | GC_OLD_MARKED;
    jl_void_type->instance = jl_nothing;

    jl_datatype_t *type_type = (jl_datatype_t *)jl_type_type;
    jl_typeofbottom_type = jl_new_datatype(
            jl_symbol("TypeofBottom"),
            core,
            type_type,
            jl_emptysvec,
            jl_emptysvec,
    ...
    jl_uniontype_type = jl_new_datatype(
            jl_symbol("Union"),
            core,
            type_type,
            jl_emptysvec,
    ...
    jl_unionall_type = jl_new_datatype(
            jl_symbol("UnionAll"),
            core,
            type_type,
            jl_emptysvec,
    ...
    jl_vararg_type = (jl_unionall_t *)jl_new_abstracttype(
                             (jl_value_t *)vararg_sym, core, jl_any_type, tv)
                             ->name->wrapper;
    jl_vararg_typename =
            ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_vararg_type))
                    ->name;

    jl_anytuple_type = jl_new_datatype(
            jl_symbol("Tuple"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_anytuple_type->parameters = jl_svec(
            1, jl_wrap_vararg((jl_value_t *)jl_any_type, (jl_value_t *)NULL));
    ...
    jl_int32_type = NULL;
    jl_int32_type = jl_new_primitivetype(
            (jl_value_t *)jl_symbol("Int32"),
            core,
            jl_any_type,
            jl_emptysvec,
            32);
    jl_int64_type = NULL;
    jl_int64_type = jl_new_primitivetype(
            (jl_value_t *)jl_symbol("Int64"),
            core,
            jl_any_type,
            jl_emptysvec,
            64);
    ...
    jl_uint8_type = NULL;
    jl_uint8_type = jl_new_primitivetype(
            (jl_value_t *)jl_symbol("UInt8"),
            core,
            jl_any_type,
            jl_emptysvec,
            8);

    jl_ssavalue_type = jl_new_datatype(
            jl_symbol("SSAValue"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_abstractslot_type = jl_new_abstracttype(
            (jl_value_t *)jl_symbol("Slot"), core, jl_any_type, jl_emptysvec);

    jl_slotnumber_type = jl_new_datatype(
            jl_symbol("SlotNumber"),
            core,
    ...
    jl_typedslot_type = jl_new_datatype(
            jl_symbol("TypedSlot"),
            core,
            jl_abstractslot_type,
            jl_emptysvec,
    ...
    jl_bool_type = NULL;
    jl_bool_type = jl_new_primitivetype(
            (jl_value_t *)jl_symbol("Bool"),
            core,
            jl_any_type,
            jl_emptysvec,
            8);
    ...
    jl_function_type = jl_new_abstracttype(
            (jl_value_t *)jl_symbol("Function"),
            core,
            jl_any_type,
            jl_emptysvec);
    jl_builtin_type = jl_new_abstracttype(
            (jl_value_t *)jl_symbol("Builtin"),
            core,
            jl_function_type,
            jl_emptysvec);
    ...
    jl_abstractarray_type = (jl_unionall_t *)jl_new_abstracttype(
                                    (jl_value_t *)jl_symbol("AbstractArray"),
                                    core,
                                    jl_any_type,
                                    tv)
                                    ->name->wrapper;
    ...
    jl_array_any_type = jl_apply_type2(
            (jl_value_t *)jl_array_type,
            (jl_value_t *)jl_any_type,
            jl_box_long(1));

    jl_array_symbol_type = jl_apply_type2(
            (jl_value_t *)jl_array_type,
            (jl_value_t *)jl_sym_type,
            jl_box_long(1));

    jl_array_uint8_type = jl_apply_type2(
            (jl_value_t *)jl_array_type,
            (jl_value_t *)jl_uint8_type,
            jl_box_long(1));

    jl_expr_type = jl_new_datatype(
            jl_symbol("Expr"),
            core,
            jl_any_type,
            jl_emptysvec,
            jl_perm_symsvec(3, "head", "args", "typ"),
            jl_svec(3, jl_sym_type, jl_array_any_type, jl_any_type),
            0,
            1,
            3);

    jl_linenumbernode_type = jl_new_datatype(
    ...
    jl_labelnode_type = jl_new_datatype(
            jl_symbol("LabelNode"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_gotonode_type = jl_new_datatype(
            jl_symbol("GotoNode"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_quotenode_type = jl_new_datatype(
            jl_symbol("QuoteNode"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_newvarnode_type = jl_new_datatype(
            jl_symbol("NewvarNode"),
            core,
            jl_any_type,
            jl_emptysvec,
    ...
    jl_module_type = jl_new_datatype(
            jl_symbol("Module"),
            core,
            jl_any_type,
            jl_emptysvec,
            jl_perm_symsvec(2, "name", "parent"),
            jl_svec(2, jl_sym_type, jl_any_type),
            0,
            1,
            2);

    jl_globalref_type = jl_new_datatype(
            jl_symbol("GlobalRef"),
            core,
            jl_any_type,
            jl_emptysvec,
            jl_perm_symsvec(2, "mod", "name"),
            jl_svec(2, jl_module_type, jl_sym_type),
            0,
            0,
            2);
    ...
    jl_method_instance_type = jl_new_datatype(
            ...
            jl_svec(16,
                    jl_new_struct(
                            jl_uniontype_type, jl_method_type, jl_module_type),
                    jl_any_type,
                    jl_any_type,
                    jl_simplevector_type,
                    jl_any_type,
                    ...
                    jl_any_type, // void*
                    jl_any_type,
                    jl_any_type), // void*, void*
            0,
            1,
            4);

    // all kinds of types share a method table
    jl_unionall_type->name->mt = jl_uniontype_type->name->mt =
            jl_datatype_type->name->mt = jl_type_typename->mt;

    jl_intrinsic_type = jl_new_primitivetype(
            (jl_value_t *)jl_symbol("IntrinsicFunction"),
            core,
            jl_builtin_type,
            jl_emptysvec,
            32);
    ...
    tv = jl_svec1(tvar("T"));
    jl_ref_type = (jl_unionall_t *)jl_new_abstracttype(
                          (jl_value_t *)jl_symbol("Ref"), core, jl_any_type, tv)
                          ->name->wrapper;

    tv = jl_svec1(tvar("T"));
    jl_pointer_type =
            (jl_unionall_t *)jl_new_primitivetype(
                    (jl_value_t *)jl_symbol("Ptr"),
                    core,
                    (jl_datatype_t *)jl_apply_type(
                            (jl_value_t *)jl_ref_type, jl_svec_data(tv), 1),
                    tv,
                    sizeof(void *) * 8)
                    ->name->wrapper;
    jl_pointer_typename =
    ...
    jl_typetype_type = (jl_unionall_t *)jl_new_struct(
            ...
            typetype_tvar,
            jl_apply_type1(
                    (jl_value_t *)jl_type_type, (jl_value_t *)typetype_tvar));

    jl_ANY_flag = (jl_value_t *)tvar("ANY");

    jl_abstractstring_type = jl_new_abstracttype(
            (jl_value_t *)jl_symbol("AbstractString"),
            core,
            jl_any_type,
            jl_emptysvec);
    jl_string_type = jl_new_datatype(
            jl_symbol("String"),
            core,
    ...
    jl_value_t *pointer_void = jl_apply_type1(
            (jl_value_t *)jl_pointer_type, (jl_value_t *)jl_void_type);
