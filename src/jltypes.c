...
static int jl_has_typevars__(jl_value_t *v, int incl_wildcard, jl_value_t **p, size_t np)
{
    ...
    if (jl_typeis(v, jl_tvar_type)) {
        ...
        if (!((jl_tvar_t *)v)->bound) return incl_wildcard;
        return 1;
    }
    if (jl_is_typector(v)) return incl_wildcard;
    jl_svec_t *t;
    int        expect = -1;
    if (jl_is_uniontype(v)) {
        ...
        if (is_unspec((jl_datatype_t *)v)) return 0;
        if (p == NULL) {
            if (incl_wildcard)
...
JL_DLLEXPORT int jl_get_size(jl_value_t *val, size_t *pnt)
{
    if (jl_is_long(val)) {
        ...
        if (slen < 0) jl_errorf("size or dimension is negative: %d", slen);
        *pnt = slen;
        return 1;
    }
    return 0;
...
static int union_elt_morespecific(const void *a, const void *b)
{
    ...
    if (jl_args_morespecific(va, vb)) return -1;
            ...
            if (j != i && temp[i] && temp[j]) {
                if (temp[i] == temp[j] ||
                    (!jl_has_typevars(temp[i]) && !jl_has_typevars(temp[j]) &&
                     !(inside_typedef &&
                       (jl_is_typevar(temp[i]) || jl_is_typevar(temp[j]))) &&
                     (type_eqv_(temp[i], temp[j]) || jl_subtype(temp[i], temp[j], 0)))) {
                    temp[i] = NULL;
                    ndel++;
                }
            }
...
static jl_value_t *jl_type_union_v(jl_value_t **ts, size_t n)
{
    ...
    JL_GC_PUSH1(&types);
    jl_uniontype_t *tu =
        (jl_uniontype_t *)jl_gc_alloc(ptls, sizeof(jl_uniontype_t), jl_uniontype_type);
    tu->types = types;
    jl_gc_wb(tu, types);
    JL_GC_POP();
...
{
    if (env->tvars == jl_emptysvec) return tv->bound;
    if (jl_is_typevar(env->tvars)) return (jl_tvar_t *)env->tvars == tv;
    for (size_t i = 0; i < jl_svec_len(env->tvars); i++) {
        if ((jl_tvar_t *)jl_svecref(env->tvars, i) == tv) return 1;
    }
    return 0;
}
...
{
    if (!allowself && var == val) return;
    for (int i = 0; i < soln->n; i += 2) {
        if (soln->data[i] == var &&
            (soln->data[i + 1] == val ||
             (!jl_is_typevar(val) && type_eqv_(soln->data[i + 1], val))))
            return;
        if (soln->data[i] == val && soln->data[i + 1] == var) return;
    }
    if (soln->n >= MAX_CENV_SIZE) jl_error("type too large");
...
static jl_value_t *jl_type_intersect(jl_value_t *a, jl_value_t *b, cenv_t *penv,
                                     cenv_t *eqc, variance_t var);

static jl_value_t *intersect_union(jl_uniontype_t *a, jl_value_t *b, cenv_t *penv,
                                   cenv_t *eqc, variance_t var)
{
    ...
    if (!jl_is_typevar(b)) {
        for (i = 0; i < l; i++) {
            if (jl_svecref(a->types, i) == b) return b;
        }
    }
    ...
    for (i = 0; i < l; i++) {
        ...
        jl_value_t *ti = jl_type_intersect(jl_svecref(a->types, i), b, penv, eqc, var);
        if (ti == (jl_value_t *)jl_bottom_type) {
            eqc->n  = eq0;
            penv->n = co0;
            ti      = jl_type_intersect(jl_svecref(a->types, i), b, penv, eqc, var);
            if (ti != (jl_value_t *)jl_bottom_type) {
...
Simplification of varargs tuple types:
 ...
 JL_TUPLE_VAR:   tuples of unknown length (e.g., JL_VARARG_BOUND or
JL_VARARG_UNBOUND)
...
In general, lenf >= lenr-1. The lower bound is achieved only for a Vararg of
length 0.
*/
typedef enum { JL_TUPLE_FIXED = 0, JL_TUPLE_VAR = 1 } jl_tuple_lenkind_t;

// Set the parameters for a single tuple
// returns lenf, sets kind and lenkind
static size_t data_vararg_params(jl_value_t **data, size_t lenr, cenv_t *eqc,
                                 jl_vararg_kind_t *kind, jl_tuple_lenkind_t *lenkind)
{
    size_t lenf = lenr;
    int    i;
    if (lenr == 0) {
    ...
    *kind    = jl_vararg_kind(data[lenr - 1]);
    if (*kind == JL_VARARG_NONE || *kind == JL_VARARG_INT) *lenkind = JL_TUPLE_FIXED;
...
static size_t tuple_vararg_params(jl_svec_t *a, cenv_t *eqc, jl_vararg_kind_t *kind,
                                  jl_tuple_lenkind_t *lenkind)
{
...
jl_datatype_t *jl_wrap_vararg(jl_value_t *t, jl_value_t *n)
{
    if (n == NULL) {
        if (t == NULL)
            return (jl_datatype_t *)jl_instantiate_type_with((jl_value_t *)jl_vararg_type,
                                                             NULL, 0);
        jl_value_t *env[2];
        env[0] = jl_tparam0(jl_vararg_type);
        env[1] = t;
        return (jl_datatype_t *)jl_instantiate_type_with((jl_value_t *)jl_vararg_type, env,
                                                         1);
    }
    jl_value_t *env[4];
    env[0] = jl_tparam0(jl_vararg_type);
...
static jl_value_t *intersect_tuple(jl_datatype_t *a, jl_datatype_t *b, cenv_t *penv,
                                   cenv_t *eqc, variance_t var)
{
    ...
    if (alenkind == JL_TUPLE_FIXED && blenkind == JL_TUPLE_FIXED) {
        ...
        if (bkind == JL_VARARG_BOUND)
            extend(jl_tparam1(jl_svecref(bp, blenr - 1)), jl_box_long(alenf - blenr + 1),
                   eqc);
    }
        ...
        if (akind == JL_VARARG_BOUND)
            extend(jl_tparam1(jl_svecref(ap, alenr - 1)), jl_box_long(blenf - alenr + 1),
                   eqc);
    }
    else {
...
static jl_value_t *intersect_tag(jl_datatype_t *a, jl_datatype_t *b, cenv_t *penv,
                                 cenv_t *eqc, variance_t var)
{
    ...
    for (i = 0; i < jl_svec_len(p); i++) {
        ...
        if (jl_is_typevar(ap)) {
            ...
            if (bp == (jl_value_t *)jl_bottom_type && !((jl_tvar_t *)ap)->bound) {
            ...
            ti = jl_type_intersect(ap, bp, penv, eqc, invariant);
            if (ap == (jl_value_t *)jl_bottom_type && !((jl_tvar_t *)bp)->bound) {
                // "Union{}" as a type parameter
                jl_svecset(p, i, ti);
                continue;
            }
            ...
            if (tva || tvb) {
                if (jl_subtype_invariant(ap, bp, 0) || jl_subtype_invariant(bp, ap, 0)) {
                    ti = jl_type_intersect(ap, bp, penv, eqc, invariant);
                }
                else {
...
static jl_value_t *intersect_typevar(jl_tvar_t *a, jl_value_t *b, cenv_t *penv, cenv_t *eqc,
                                     variance_t var)
{
    ...
    if (var == invariant) {
        if (!jl_has_typevars_(b, 0) && !jl_is_typevar(b)) {
            ...
            if (i >= eqc->n) extend((jl_value_t *)a, b, eqc);
            JL_GC_POP();
            return (jl_value_t *)a;
        }
        if (jl_is_typevar(b)) {
        ...
        if (jl_is_typevar(b)) {
            for (i = 0; i < penv->n; i += 2) {
                if (penv->data[i] == b && !jl_is_typevar(penv->data[i + 1])) {
                    jl_value_t *ti =
                        jl_type_intersection((jl_value_t *)a, penv->data[i + 1]);
                    if (ti == (jl_value_t *)jl_bottom_type) {
                        JL_GC_POP();
                        return ti;
                    }
            ...
            for (i = 0; i < eqc->n; i += 2) {
                if (eqc->data[i] == b && !jl_is_typevar(eqc->data[i + 1])) {
                    jl_value_t *ti =
                        jl_type_intersection((jl_value_t *)a, eqc->data[i + 1]);
                    if (ti == (jl_value_t *)jl_bottom_type) {
                        JL_GC_POP();
                        return ti;
                    }
                                     ...
                                     cenv_t *eqc, variance_t var)
{
    ...
    if (!jl_has_typevars(a) && !jl_has_typevars(b)) {
        if (jl_subtype(a, b, 0)) return a;
        if (jl_subtype(b, a, 0)) return b;
    }
    // union
    if (jl_is_uniontype(a)) return intersect_union((jl_uniontype_t *)a, b, penv, eqc, var);
    if (jl_is_uniontype(b)) return intersect_union((jl_uniontype_t *)b, a, penv, eqc, var);
    ...
    // tag
    if (!jl_is_datatype(a) || !jl_is_datatype(b)) return (jl_value_t *)jl_bottom_type;
    ...
    */
    if (var == covariant && sub == (jl_datatype_t *)sub->name->primary &&
        jl_has_typevars_from((jl_value_t *)sub->super,
                             ((jl_datatype_t *)sub->name->primary)->parameters))
        env = approxify_type((jl_datatype_t *)sub->super,
                             ((jl_datatype_t *)sub->name->primary)->parameters);
    else
        env = (jl_value_t *)sub->super;
    super = (jl_datatype_t *)jl_type_intersect((jl_value_t *)env, (jl_value_t *)super, penv,
                                               eqc, var);
    ...
    super = (jl_datatype_t *)jl_instantiate_type_with((jl_value_t *)super, eqc->data,
                                                      eqc->n / 2);
    ...
    if (sub_needs_parameters) {
        for (int e = 0; e < jl_svec_len(env); e += 2) {
            ...
            size_t j;
            for (j = 0; j < n; j++) {
                if (tp == jl_svecref(tc_params, j)) break;
            }
            if (j >= n) {
                JL_GC_POP();
                return (jl_value_t *)jl_bottom_type;
    ...
    for (i = 0; i < n; i++) {
        ...
        for (int e = 0; e < jl_svec_len(env); e += 2) {
            if (jl_svecref(env, e) == tp) {
                elt = jl_type_intersect(elt, jl_svecref(env, e + 1), penv, eqc, invariant);
...
static jl_value_t *meet_tvars(jl_tvar_t *a, jl_tvar_t *b)
{
    ...
    if (ub == (jl_value_t *)jl_bottom_type) return ub;
    JL_GC_PUSH2(&lb, &ub);
...
{
    if (jl_is_typevar(ty)) return (jl_value_t *)meet_tvars(tv, (jl_tvar_t *)ty);
    // if (jl_types_equal((jl_value_t*)tv->ub, ty))
    //    return ty;
    if (jl_subtype((jl_value_t *)tv->ub, ty, 0)) return (jl_value_t *)tv;
    // TODO: should we check type_intersection(tv->ub, ty) instead?
    if (!jl_subtype(ty, (jl_value_t *)tv->ub, 0)) return (jl_value_t *)jl_bottom_type;
    // if (jl_types_equal((jl_value_t*)tv->lb, ty))
    //    return ty;
    if (jl_subtype((jl_value_t *)tv->lb, ty, 0)) {
        if (jl_is_leaf_type(ty) || !jl_is_type(ty)) return ty;
        jl_tvar_t *ntv = jl_new_typevar(underscore_sym, tv->lb, ty);
        // TODO: this would be nice but causes some spurious ambiguity warnings
        // due to typevars in covariant position, which we should simplify out
        // somewhere.
        // ntv->bound = tv->bound;
...
static jl_value_t *meet(jl_value_t *X, jl_value_t *Y, variance_t var)
{
    if (jl_is_typevar(X)) {
        ...
        if (tv == (jl_value_t *)jl_bottom_type) return NULL;
        return tv;
    }
    if (jl_is_typevar(Y)) {
...
static jl_value_t *type_to_static_parameter_value(jl_value_t *t, jl_value_t *tv,
                                                  jl_value_t **tvs, int ntv)
{
    int i;
    for (i = 0; i < ntv; i++) {
        if (tv == tvs[i]) break;
    }
    if (i >= ntv) return t; // don't widen vars not in env
    ...
    if (jl_is_tuple_type(t)) {
        ...
        for (size_t i = 0; i < l; i++) {
            jl_value_t *el =
                type_to_static_parameter_value(jl_svecref(p, i), NULL, NULL, 0);
            jl_svecset(np, i, el);
            if (el != jl_svecref(p, i)) changed = 1;
        }
...
void print_env(cenv_t *soln)
{
    for(int i=0; i < soln->n; i+=2) {
        ...
        jl_printf(JL_STDOUT, "%s@%x=", jl_symbol_name(((jl_tvar_t*)T)->name),
T);
...
static int solve_tvar_constraints(cenv_t *env, cenv_t *soln, jl_value_t **tvs, int ntv)
{
    ...
    while (1) {
        ...
        for (int i = 0; i < soln->n; i += 2) {
            ...
            if (jl_is_typevar(*pS)) *pS = *tvar_lookup(soln, pS);
            if (!jl_is_typevar(*pS)) {
                // detect cycles
                if (jl_has_typevars_from_v(*pS, &soln->data[i], 1)) goto ret_no;
            }
        }
        ...
        // 3. given T, let S´ = intersect(all S s.t. (T=S) or (S=T) ∈ env). add
        // (T=S´) to soln.
        for (int i = 0; i < env->n; i += 2) {
            ...
            if (!jl_is_typevar(S)) {
                ...
                if (pT != &T) {
                    ...
                    if (jl_get_size(S, &lenS) && jl_get_size(*pT, &lenT)) {
                        ...
                        if (meet(*pT, S, covariant) == NULL) goto ret_no;
                    }
                }
                else {
        ...
        */
        for (int i = 0; i < eqc.n; i += 2) {
            ...
            JL_TRY {
                ...
                if (tvar) {
                    *extraroot      = rhs;
                    eqc.data[i + 1] = (jl_value_t *)jl_new_typevar(
                        underscore_sym, ((jl_tvar_t *)rhs)->lb, inst);
                }
            }
            JL_CATCH {
            }
...
static int type_eqv__(jl_value_t *a, jl_value_t *b, int distinguish_tctor)
{
    ...
    if (jl_is_typevar(a)) {
        if (jl_is_typevar(b)) {
            return !distinguish_tctor &&
                   type_eqv_(((jl_tvar_t *)a)->ub, ((jl_tvar_t *)b)->ub) &&
    ...
    for (i = 0; i < jl_svec_len(ap); i++) {
        ...
        if (api == bpi) continue;
        if (!type_eqv__(api, bpi, distinguish_tctor)) return 0;
    }
    return 1;
}

static int type_eqv_(jl_value_t *a, jl_value_t *b) { return type_eqv__(a, b, 0); }

JL_DLLEXPORT int jl_types_equal(jl_value_t *a, jl_value_t *b) { return type_eqv_(a, b); }
...
static int type_le_generic(jl_value_t *a, jl_value_t *b, int useenv)
{
    ...
    for (size_t i = 0; i < l; i += 2) {
        ...
        if (!jl_is_typevar(envi)) return 0;
        if (useenv &&
            ((jl_tvar_t *)envi)->bound != ((jl_tvar_t *)jl_svecref(env, i))->bound)
            return 0;
        for (size_t j = 0; j < l; j += 2) {
            if (i != j) {
                if (envi == jl_svecref(env, j + 1)) return 0;
            }
        }
    }
    return 1;
...
static int valid_type_param(jl_value_t *v)
{
    if (jl_is_tuple(v)) {
        ...
        for (i = 0; i < l; i++) {
            ...
            if (!(pi == (jl_value_t *)jl_sym_type || jl_isbits(pi))) return 0;
        }
        return 1;
    }
...
jl_value_t *jl_apply_type_(jl_value_t *tc, jl_value_t **params, size_t n)
{
    ...
    if (n == 0) {
        if (jl_is_typector(tc)) return (jl_value_t *)((jl_typector_t *)tc)->body;
        return tc;
    }
    size_t         i;
    const char *   tname;
    ...
    if (n > ntp) jl_errorf("too many parameters for type %s", tname);
    jl_value_t **env;
    JL_GC_PUSHARGS(env, 2 * ntp);
    size_t ne = 0;
    for (i = 0; i < ntp; i++) {
        jl_tvar_t *tv = (jl_tvar_t *)jl_svecref(tp, i);
        if (!jl_is_typevar(tv)) continue;
    ...
    if (ne < n) jl_errorf("too many parameters for type %s", tname);
...
static int contains_unions(jl_value_t *type)
{
    ...
    for (i = 0; i < jl_nparams(type); i++) {
        if (contains_unions(jl_tparam(type, i))) return 1;
    }
    return 0;
}
...
static int is_typekey_ordered(jl_value_t **key, size_t n)
{
    ...
    for (i = 0; i < n; i++) {
        jl_value_t *k = key[i];
        if (jl_is_typevar(k)) return 0;
        if (jl_is_type(k) &&
            !(jl_is_datatype(k) &&
              (((jl_datatype_t *)k)->uid || k == ((jl_datatype_t *)k)->name->primary ||
...
static int typekey_compare(jl_datatype_t *tt, jl_value_t **key, size_t n)
{
    ...
    for (j = 0; j < n; j++) {
        ...
        if (tj != kj) {
            ...
            if (!dtt && !dtk && jl_egal(tj, kj)) continue;
            uintptr_t tid = (dtt && ((jl_datatype_t *)tj)->uid ? ((jl_datatype_t *)tj)->uid
                                                               : jl_object_id(tj));
            uintptr_t kid = (dtk && ((jl_datatype_t *)kj)->uid ? ((jl_datatype_t *)kj)->uid
                                                               : jl_object_id(kj));
            if (kid != tid) return kid < tid ? -1 : 1;
        }
    }
    return 0;
}
...
static int typekey_eq(jl_datatype_t *tt, jl_value_t **key, size_t n)
{
    ...
    for (j = 0; j < n; j++) {
        jl_value_t *kj = key[j], *tj = jl_svecref(tt->parameters, j);
        if (tj != kj && !type_eqv__(tj, kj, 1)) return 0;
    }
    return 1;
}
...
static ssize_t lookup_type_idx(jl_typename_t *tn, jl_value_t **key, size_t n, int ordered)
{
    ...
    if (ordered) {
        ...
        for (i = 0; i < cl; i++) {
            ...
            if (tt == NULL) return ~i;
            if (typekey_eq(tt, key, n)) return i;
        }
        return ~cl;
    }
}
...
static int is_cacheable(jl_datatype_t *type)
{
    ...
    if (jl_is_abstracttype(type)) return !jl_has_typevars_((jl_value_t *)type, 1);
    // ... or concrete types
...
jl_value_t *jl_cache_type_(jl_datatype_t *type)
{
    if (is_cacheable(type)) {
        ...
        int ord = is_typekey_ordered(jl_svec_data(type->parameters),
        ...
        if (idx >= 0)
            type = (jl_datatype_t *)jl_svecref(
                ord ? type->name->cache : type->name->linearcache, idx);
        else
...
static jl_svec_t *inst_all(jl_svec_t *p, jl_value_t **env, size_t n, jl_typestack_t *stack,
                           int check);
...
void jl_precompute_memoized_dt(jl_datatype_t *dt)
{
    ...
    dt->isleaftype =
        !dt->abstract || (jl_type_type != NULL && dt->name == jl_type_type->name);
    for (i = 0; i < l; i++) {
        jl_value_t *p                         = jl_tparam(dt, i);
        size_t      d                         = jl_type_depth(p) + 1;
        if (d > dt->depth) dt->depth          = d;
        if (!dt->hastypevars) dt->hastypevars = jl_has_typevars__(p, 0, NULL, 0);
        if (!dt->haswildcard) dt->haswildcard = jl_has_typevars__(p, 1, NULL, 0);
        if (dt->isleaftype)
...
static jl_value_t *inst_datatype(jl_datatype_t *dt, jl_svec_t *p, jl_value_t **iparams,
                                 ...
                                 jl_value_t **env, size_t n)
{
    ...
    if (stack_lkup) return stack_lkup;
    ...
    if (istuple && ntp > 0 && jl_is_vararg_type(iparams[ntp - 1])) {
        ...
        if (jl_is_long(jl_tparam1(va))) {
            ...
            if (nt < 0) jl_errorf("apply_type: Vararg length N is negative: %zd", nt);
            va = jl_tparam0(va);
            if (nt == 0 || !jl_has_typevars(va)) {
                if (ntp == 1) return jl_tupletype_fill(nt, va);
                size_t i, l;
    ...
    if (!istuple) {
        if (jl_is_vararg_type((jl_value_t *)dt) && ntp == 2) {
            if (!jl_is_long(iparams[1]) && !jl_is_typevar(iparams[1])) {
                jl_type_error_rt("apply_type", "Vararg count", (jl_value_t *)jl_long_type,
                                 iparams[1]);
            }
        }
        if (tc != (jl_value_t *)dt) return (jl_value_t *)jl_apply_type_(tc, iparams, ntp);
    }
    ...
    if (p == NULL) {
        ...
        for (unsigned i = 0; i < ntp; i++) jl_svecset(p, i, iparams[i]);
    }
    ...
    if (cacheable && !ndt->abstract) ndt->uid = jl_assign_type_uid();
    ...
    if (istuple) {
        ...
        ndt->super =
            (jl_datatype_t *)inst_type_w_((jl_value_t *)dt->super, env, n, stack, 1);
...
static jl_tupletype_t *jl_apply_tuple_type_v_(jl_value_t **p, size_t np, jl_svec_t *params)
{
    ...
    for (size_t i = 0; i < np; i++) {
        ...
        if (!jl_is_leaf_type(pi)) cacheable = 0;
    }
...
{
    return (jl_datatype_t *)inst_datatype(jl_anytuple_type, p, jl_svec_data(p),
                                          jl_svec_len(p), 1, NULL, NULL, 0);
}
                           ...
                           int check)
{
    ...
    for (i = 0; i < lp; i++) {
        jl_svecset(np, i,
                   (jl_value_t *)inst_type_w_(jl_svecref(p, i), env, n, stack, check));
    }
    JL_GC_POP();
    return np;
}
                                 ...
                                 jl_typestack_t *stack, int check)
{
    ...
    if (jl_is_va_tuple(tt) && ntp == 1) {
        ...
        if (T != NULL && N != NULL && jl_is_long(N)) {
            ...
            if (nt < 0) jl_errorf("size or dimension is negative: %zd", nt);
    ...
    for (i = 0; i < ntp; i++) {
        ...
        iparams[i]      = pi;
        if (ip_heap) jl_gc_wb(ip_heap, pi);
    ...
    jl_value_t *result =
        inst_datatype((jl_datatype_t *)tt, ip_heap, iparams, ntp, cacheable, stack, env, n);
    JL_GC_POP();
    return result;
}
                                ...
                                jl_typestack_t *stack, int check)
{
    ...
    if (!jl_is_datatype(t)) return t;
    jl_datatype_t *tt = (jl_datatype_t *)t;
    jl_svec_t *    tp = tt->parameters;
    if (tp == jl_emptysvec) return (jl_value_t *)t;
    jl_typename_t *tn = tt->name;
    jl_value_t *   tc = tn->primary;
    // don't instantiate "Foo" without parameters inside Foo
    if (t == tc && stack != NULL) return (jl_value_t *)t;
    assert(jl_is_datatype(tc));
    if (tn == jl_tuple_typename) return inst_tuple_w_(t, env, n, stack, check);
    size_t ntp = jl_svec_len(tp);
    ...
    for (i = 0; i < ntp; i++) {
        ...
        if (elt == t) {
            ...
            if (jl_is_typevar(tv) && !jl_is_typevar(iparams[i])) {
                if (!jl_subtype(iparams[i], tv, 0)) {
                    jl_type_error_rt(jl_symbol_name(tt->name->name),
                                     jl_symbol_name(((jl_tvar_t *)tv)->name), tv,
                                     iparams[i]);
                }
            }
            if (!bound) {
            ...
            if (cacheable && jl_has_typevars(iparams[i])) cacheable = 0;
        }
    }
    ...
    jl_value_t *result =
        inst_datatype((jl_datatype_t *)tt, NULL, iparams, ntp, cacheable, stack, env, n);
    JL_GC_POP();
    return result;
}
...
jl_datatype_t *jl_wrap_Type(jl_value_t *t)
{
    ...
    // else
    env[1] = t;
    return (jl_datatype_t *)jl_instantiate_type_with((jl_value_t *)jl_type_type, env, 1);
}
...
void jl_reinstantiate_inner_types(jl_datatype_t *t) // can throw!
{
    ...
    for (j = 0; j < partial_inst.len; j++) {
        ...
        for (i = 0; i < n; i++) env[i * 2 + 1] = jl_svecref(ndt->parameters, i);
...
static int jl_tuple_subtype_(jl_value_t **child, size_t clenr, jl_datatype_t *pdt, int ta,
                             int invariant)
{
    ...
    while (1) {
        if (!cseq)
            cseq = !ta && ci < clenr && clenkind != JL_TUPLE_FIXED &&
                   jl_is_vararg_type(child[ci]);
        if (!pseq)
            pseq =
                pi < plenr && plenkind != JL_TUPLE_FIXED && jl_is_vararg_type(parent[pi]);
        if (cseq && !pseq) break;
        if (ci >= clenf) {
            result = pi >= plenf || (pseq && !invariant);
            break;
        }
        if (pi >= plenf && !pseq) break;
        if (ci < clenr) {
            ce                            = child[ci];
        ...
        if (!jl_subtype_le(ce, pe, ta, invariant)) break;
...
int jl_subtype_le(jl_value_t *a, jl_value_t *b, int ta, int invariant)
{
    ...
    if (!ta && jl_is_uniontype(a)) {
        ...
        for (i = 0; i < l_ap; i++) {
            if (!jl_subtype_le(jl_svecref(ap, i), b, 0, invariant)) return 0;
        }
        return 1;
    }
    ...
    if (!ta && jl_is_type_type(a) && !invariant) {
        ...
        if (jl_is_typevar(tp0a)) {
            ...
            if (jl_subtype_le(ub, b, 1, 0) &&
                ...
                if (jl_subtype_le(lb, b, 1, 0)) return 1;
            }
        }
        else {
            if (jl_subtype_le(tp0a, b, 1, 0)) return 1;
        }
    }
    ...
    if (jl_is_uniontype(b)) {
        ...
        for (i = 0; i < jl_svec_len(bp); i++) {
            if (jl_subtype_le(a, jl_svecref(bp, i), ta, invariant)) return 1;
        }
        if (!ta && jl_is_typevar(a) && ((jl_tvar_t *)a)->ub == jl_bottom_type) return 1;
        return 0;
    }
    ...
    if (jl_is_tuple_type(a)) {
        if (jl_is_tuple_type(b)) {
            return jl_tuple_subtype_(jl_svec_data(((jl_datatype_t *)a)->parameters),
                                     jl_nparams(a), (jl_datatype_t *)b, 0, invariant);
        }
    }
    ...
    if (jl_is_datatype(a) && jl_is_datatype(b)) {
        ...
        while (tta != (jl_datatype_t *)jl_any_type) {
            if (tta->name == ttb->name) {
                if (super && ttb->name == jl_type_type->name &&
                    jl_is_typevar(jl_tparam0(b))) {
                    if (jl_subtype_le(a, jl_tparam0(b), 0, 1)) return 1;
                }
                ...
                for (i = 0; i < l; i++) {
                    ...
                    if (invariant) {
                        if (jl_is_typevar(bpara) && !((jl_tvar_t *)bpara)->bound) {
                            if (!jl_is_typevar(apara)) return 0;
                        }
                    }
                    if (!jl_subtype_le(apara, bpara, 0, 1)) return 0;
                }
                return 1;
            }
            else if (invariant) {
    ...
    if (jl_is_typevar(a)) {
        if (jl_is_typevar(b)) {
            return jl_subtype_le((jl_value_t *)((jl_tvar_t *)a)->ub,
...
static int type_eqv_with_ANY(jl_value_t *a, jl_value_t *b)
{
    ...
    return ((a == (jl_value_t *)jl_any_type && b == jl_ANY_flag) ||
            (b == (jl_value_t *)jl_any_type && a == jl_ANY_flag) || type_eqv_(a, b));
}
...
jl_datatype_t *jl_fix_vararg_bound(jl_datatype_t *tt, int nfix)
{
    ...
    jl_datatype_t *ret =
        (jl_datatype_t *)jl_instantiate_type_with((jl_value_t *)tt, env, 1);
    JL_GC_POP();
    return ret;
}
...
static int jl_tuple_morespecific(jl_datatype_t *cdt, jl_datatype_t *pdt, int invariant)
{
    ...
    while (1) {
        if (!cseq)
            cseq =
                (ci < clenr) && clenkind != JL_TUPLE_FIXED && jl_is_vararg_type(child[ci]);
        if (!pseq)
            pseq =
                (pi < plenr) && plenkind != JL_TUPLE_FIXED && jl_is_vararg_type(parent[pi]);
        if (ci >= clenf && !cseq) return 1;
        if (pi >= plenf && !pseq) return some_morespecific;
        if (ci < clenr) {
            ce                            = child[ci];
            if (jl_is_vararg_type(ce)) ce = jl_tparam0(ce);
        }
        ...
        if (!jl_type_morespecific_(ce, pe, invariant)) {
            ...
            if (jl_is_vararg_type(child[ci]) && type_eqv_with_ANY(ce, pe)) return 1;
        }

        if (some_morespecific && cseq && !pseq) return 1;

        // at this point we know one element is strictly more specific
        if (!(type_eqv_with_ANY(ce, pe) ||
              (jl_is_typevar(pe) && jl_types_equal(ce, ((jl_tvar_t *)pe)->ub)))) {
            some_morespecific = 1;
        ...
        /*
          ...
          sub2ind{N,T<:Integer}(inds::Union{Dims{N},Indices{N}},
          I::AbstractVector{T}...)
        */
...
static int jl_type_morespecific_(jl_value_t *a, jl_value_t *b, int invariant)
{
    ...
    if (jl_is_uniontype(a)) {
        ...
        if (jl_subtype_le(b, a, 0, 0)) {
            // fixes issue #4413
            if (!jl_subtype_le(a, b, 0, invariant)) return 0;
        }
        ...
        for (i = 0; i < l_ap; i++) {
            ...
            if (partially_morespecific(ai, b, invariant) &&
                ...
                if (partially_morespecific(b, a, invariant)) return 0;
                return 1;
            }
        }
        return 0;
    ...
    if (jl_is_uniontype(b)) {
        ...
        jl_svec_t *bp = ((jl_uniontype_t *)b)->types;
        for (i = 0; i < jl_svec_len(bp); i++) {
            if (jl_type_morespecific_(a, jl_svecref(bp, i), invariant)) return 1;
        }
        return 0;
    }
    ...
    if (jl_is_datatype(a) && jl_is_datatype(b)) {
        ...
        while (tta != (jl_datatype_t *)jl_any_type) {
            if (tta->name == ttb->name) {
                if (super) {
                    if (tta->name != jl_type_type->name) return 1;
                }
                if (super && ttb->name == jl_type_type->name &&
                    jl_is_typevar(jl_tparam0(b))) {
                    if (jl_type_morespecific_(a, jl_tparam0(b), 1)) return 1;
                }
                assert(jl_nparams(tta) == jl_nparams(ttb));
                for (i = 0; i < jl_nparams(tta); i++) {
                    jl_value_t *apara = jl_tparam(tta, i);
                    jl_value_t *bpara = jl_tparam(ttb, i);
                    if (!jl_type_morespecific_(apara, bpara, 1)) return 0;
                }
                return 1;
            }
            else if (invariant) {
    ...
    if (jl_is_typevar(a)) {
        if (jl_is_typevar(b)) {
            return jl_type_morespecific_((jl_value_t *)((jl_tvar_t *)a)->ub,
...
int jl_args_morespecific_(jl_value_t *a, jl_value_t *b)
{
    ...
    if (btv) {
        ...
        if (nmsp == msp) return 0;
    }
    if (jl_has_typevars((jl_value_t *)a)) {
        int nmsp = jl_type_morespecific(b, a);
        if (nmsp && msp) return 1;
        if (!btv && jl_types_equal(a, b)) return 1;
        if (jl_type_match_morespecific(b, a) != (jl_value_t *)jl_false) return 0;
    }
    return msp;
}
...
int jl_args_morespecific_fix1(jl_value_t *a, jl_value_t *b, int swap)
{
    ...
    if (swap) return jl_args_morespecific_(b, a);
...
static jl_value_t *type_match_(jl_value_t *child, jl_value_t *parent, cenv_t *env,
                               int morespecific, int invariant);

static jl_value_t *tuple_match(jl_datatype_t *child, jl_datatype_t *parent, cenv_t *env,
                               int morespecific, int invariant)
{
    size_t             ci = 0, pi = 0;
    ...
    while (1) {
        if (!cseq)
            cseq = (ci < clenr) && clenkind != JL_TUPLE_FIXED &&
                   jl_is_vararg_type(jl_tparam(child, ci));
        if (!pseq)
            pseq = (pi < plenr) && plenkind != JL_TUPLE_FIXED &&
                   jl_is_vararg_type(jl_tparam(parent, pi));
        if (!morespecific && cseq && !pseq) return jl_false;
        if (ci >= clenf && !cseq)
            return (mode || pi >= plenf || (pseq && !invariant)) ? jl_true : jl_false;
        if (pi >= plenf && !pseq) return mode ? jl_true : jl_false;
        if (ci < clenr) {
        ...
        if (mode && cseq && !pseq) return jl_true;
                               ...
                               int morespecific, int invariant)
{
    jl_value_t *tmp, *tmp2;
    invariant                          = invariant & type_match_invariance_mask;
    if (jl_is_typector(child)) child   = (jl_value_t *)((jl_typector_t *)child)->body;
    if (jl_is_typector(parent)) parent = (jl_value_t *)((jl_typector_t *)parent)->body;
    size_t i, j;
    ...
    if (jl_is_typevar(parent)) {
        ...
        if (morespecific) {
            if (!jl_type_morespecific_(child, parent, 0)) return jl_false;
        }
        else {
            if (!jl_subtype_le(child, parent, 0, 0)) return jl_false;
        }
        ...
        for (int i = 0; i < env->n; i += 2) {
            if (env->data[i] == (jl_value_t *)parent) {
                ...
                if (jl_is_typevar(pv) && jl_is_typevar(child)) {
                    if (pv == (jl_value_t *)child) return jl_true;
                    return jl_false;
                }
                if (morespecific) {
                    ...
                    else if (!jl_is_typevar(child) &&
                             !jl_type_morespecific_(pv, child, 0)) {
                        return jl_true;
                    }
                    ...
                    if (type_eqv_(child, pv)) return jl_true;
                }
                return jl_false;
            }
        }
    ...
    if (jl_is_typevar(child)) {
        if (!invariant || morespecific) {
            if (morespecific) {
                if (jl_type_morespecific_(child, parent, 0)) return jl_true;
            }
            else {
                if (jl_subtype_le(child, parent, 0, 0)) return jl_true;
            }
        }
        return jl_false;
    }
    if (!invariant && parent == (jl_value_t *)jl_any_type) return jl_true;
    ...
    if (jl_is_uniontype(child)) {
        ...
        if (morespecific) {
                ...
                if (tmp != jl_false) {
                    ...
                    if (tmp2 == jl_false) {
                        ...
                        for (j = 0; j < jl_svec_len(t); j++) {
                            tenv.n = 0;
                            env->n = n;
                            if (type_match_(parent, jl_svecref(t, j), &tenv, 1,
                                            invariant) != jl_false &&
                                type_match_(jl_svecref(t, j), parent, env, 1, invariant) ==
                                    jl_false) {
                                env->n = n;
                                JL_GC_POP();
                                return jl_false;
            ...
            for (i = 0; i < jl_svec_len(t); i++) {
                int n = env->n;
                if (type_match_(jl_svecref(t, i), parent, env, morespecific, invariant) ==
                    jl_false) {
                    env->n = n;
                    return jl_false;
                }
    ...
    if (jl_is_uniontype(parent)) {
        ...
        for (i = 0; i < jl_svec_len(t); i++) {
            env->n = n;
            if (type_match_(child, jl_svecref(t, i), env, morespecific, invariant) !=
                jl_false)
                return jl_true;
        }
        return jl_false;
    ...
    while (tta != (jl_datatype_t *)jl_any_type) {
        if (tta->name == ttb->name) {
            ...
            // for any specific T.
            if (super && morespecific && tta->name != jl_type_type->name) return jl_true;
            assert(jl_nparams(tta) == jl_nparams(ttb));
            for (i = 0; i < jl_nparams(tta); i++) {
                int n = env->n;
                if (type_match_(jl_tparam(tta, i), jl_tparam(ttb, i), env, morespecific,
                                1) == jl_false) {
                    env->n = n;
                    return jl_false;
                }
    ...
    if (((jl_datatype_t *)child)->name == jl_type_type->name &&
        ...
        return type_match_(jl_typeof(jl_tparam0(child)), parent, env, morespecific, 0);
    }
    return jl_false;
}
...
jl_value_t *jl_type_match(jl_value_t *a, jl_value_t *b) { return jl_type_match_(a, b, 0); }
...
JL_DLLEXPORT jl_tvar_t *jl_new_typevar_(jl_sym_t *name, jl_value_t *lb, jl_value_t *ub,
                                        jl_value_t *b)
{
    jl_ptls_t  ptls = jl_get_ptls_states();
...
void jl_init_types(void)
{
    ...
    jl_type_type =
        jl_new_abstracttype((jl_value_t *)jl_symbol("Type"), jl_any_type, jl_emptysvec);
    ...
    jl_datatype_type->name->names =
        jl_svec(17, jl_symbol("name"), jl_symbol("super"), jl_symbol("parameters"),
                jl_symbol("types"), jl_symbol("instance"), jl_symbol("layout"),
                jl_symbol("size"), jl_symbol("ninitialized"), jl_symbol("uid"),
                jl_symbol("abstract"), jl_symbol("mutable"), jl_symbol("llvm::StructType"),
                jl_symbol("llvm::DIType"), jl_symbol("depth"), jl_symbol("hastypevars"),
                jl_symbol("haswildcard"), jl_symbol("isleaftype"));
    jl_datatype_type->types = jl_svec(
        17, jl_typename_type, jl_type_type, jl_simplevector_type, jl_simplevector_type,
    ...
    jl_typename_type->name->mt =
        jl_new_method_table(jl_typename_type->name->name, ptls->current_module);
    jl_typename_type->super       = jl_any_type;
    jl_typename_type->parameters  = jl_emptysvec;
    jl_typename_type->name->names = jl_svec(
        8, jl_symbol("name"), jl_symbol("module"), jl_symbol("names"), jl_symbol("primary"),
        jl_symbol("cache"), jl_symbol("linearcache"), jl_symbol("hash"), jl_symbol("mt"));
    jl_typename_type->types =
        jl_svec(8, jl_sym_type, jl_any_type, jl_simplevector_type, jl_type_type,
    ...
    jl_methtable_type->name->mt =
        jl_new_method_table(jl_methtable_type->name->name, ptls->current_module);
    jl_methtable_type->super       = jl_any_type;
    jl_methtable_type->parameters  = jl_emptysvec;
    jl_methtable_type->name->names = jl_svec(
        8, jl_symbol("name"), jl_symbol("defs"), jl_symbol("cache"), jl_symbol("max_args"),
        jl_symbol("kwsorter"), jl_symbol("module"), jl_symbol(""), jl_symbol(""));
    jl_methtable_type->types = jl_svec(
        8, jl_sym_type, jl_any_type, jl_any_type, jl_any_type /*jl_long*/, jl_any_type,
        jl_any_type /*module*/, jl_any_type /*long*/, jl_any_type /*int32*/);
    ...
    jl_sym_type->name->mt =
        jl_new_method_table(jl_sym_type->name->name, ptls->current_module);
    ...
    jl_simplevector_type->name->mt =
        jl_new_method_table(jl_simplevector_type->name->name, ptls->current_module);
    ...
    jl_uniontype_type = jl_new_datatype(jl_symbol("Union"), jl_type_type, jl_emptysvec,
                                        ...
                                        jl_svec(1, jl_simplevector_type), 0, 0, 1);

    jl_bottom_type = (jl_value_t *)jl_new_struct(jl_uniontype_type, jl_emptysvec);

    jl_tvar_type = jl_new_datatype(
        jl_symbol("TypeVar"), jl_any_type, jl_emptysvec,
        jl_svec(4, jl_symbol("name"), jl_symbol("lb"), jl_symbol("ub"), jl_symbol("bound")),
        jl_svec(4, jl_sym_type, jl_type_type, jl_type_type, jl_any_type), 0, 1, 3);
    ...
    jl_anytuple_type->parameters =
        jl_svec(1, jl_wrap_vararg((jl_value_t *)jl_any_type, (jl_value_t *)NULL));
    // jl_anytuple_type->parameters = jl_svec(1, jl_wrap_vararg((jl_value_t*)NULL,
    // (jl_value_t*)NULL));
    ...
    jl_tvar_t *tttvar = jl_new_typevar(jl_symbol("T"), (jl_value_t *)jl_bottom_type,
                                       (jl_value_t *)jl_any_type);
    ...
    jl_int32_type = NULL;
    jl_int32_type =
        jl_new_bitstype((jl_value_t *)jl_symbol("Int32"), jl_any_type, jl_emptysvec, 32);
    jl_int64_type = NULL;
    jl_int64_type =
        jl_new_bitstype((jl_value_t *)jl_symbol("Int64"), jl_any_type, jl_emptysvec, 64);

    jl_uint8_type = NULL;
    jl_uint8_type =
        jl_new_bitstype((jl_value_t *)jl_symbol("UInt8"), jl_any_type, jl_emptysvec, 8);

    jl_ssavalue_type =
        jl_new_datatype(jl_symbol("SSAValue"), jl_any_type, jl_emptysvec,
                        jl_svec1(jl_symbol("id")), jl_svec1(jl_long_type), 0, 0, 1);

    jl_abstractslot_type =
        jl_new_abstracttype((jl_value_t *)jl_symbol("Slot"), jl_any_type, jl_emptysvec);

    jl_slotnumber_type =
        jl_new_datatype(jl_symbol("SlotNumber"), jl_abstractslot_type, jl_emptysvec,
                        jl_svec1(jl_symbol("id")), jl_svec1(jl_long_type), 0, 0, 1);

    jl_typedslot_type =
        jl_new_datatype(jl_symbol("TypedSlot"), jl_abstractslot_type, jl_emptysvec,
    ...
    jl_bool_type = NULL;
    jl_bool_type =
        jl_new_bitstype((jl_value_t *)jl_symbol("Bool"), jl_any_type, jl_emptysvec, 8);
    jl_false = jl_box8(jl_bool_type, 0);
    jl_true  = jl_box8(jl_bool_type, 1);

    jl_typemap_level_type = jl_new_datatype(
        jl_symbol("TypeMapLevel"), jl_any_type, jl_emptysvec,
        jl_svec(7, jl_symbol("index_arg1"), jl_symbol("arg1"), jl_symbol("index_targ"),
                jl_symbol("targ"), jl_symbol("list"), jl_symbol("any"), jl_symbol("key")),
        jl_svec(7, jl_any_type, jl_any_type, jl_any_type, jl_any_type, jl_any_type,
                jl_any_type, jl_any_type),
        0, 1, 6);

    jl_typemap_entry_type = jl_new_datatype(
        jl_symbol("TypeMapEntry"), jl_any_type, jl_emptysvec,
        jl_svec(9, jl_symbol("next"), jl_symbol("sig"), jl_symbol("tvars"),
                jl_symbol("simplesig"), jl_symbol("guardsigs"), jl_symbol("func"),
                jl_symbol("isleafsig"), jl_symbol("issimplesig"), jl_symbol("va")),
    ...
    jl_function_type =
        jl_new_abstracttype((jl_value_t *)jl_symbol("Function"), jl_any_type, jl_emptysvec);
    jl_builtin_type = jl_new_abstracttype((jl_value_t *)jl_symbol("Builtin"),
                                          jl_function_type, jl_emptysvec);

    tv = jl_svec2(tvar("T"), tvar("N"));
    jl_abstractarray_type =
        jl_new_abstracttype((jl_value_t *)jl_symbol("AbstractArray"), jl_any_type, tv);

    tv                 = jl_svec2(tvar("T"), tvar("N"));
    jl_densearray_type = jl_new_abstracttype(
        (jl_value_t *)jl_symbol("DenseArray"),
        (jl_datatype_t *)jl_apply_type((jl_value_t *)jl_abstractarray_type, tv), tv);

    tv            = jl_svec2(tvar("T"), tvar("N"));
    jl_array_type = jl_new_datatype(
        jl_symbol("Array"),
        (jl_datatype_t *)jl_apply_type((jl_value_t *)jl_densearray_type, tv), tv,
    ...
    jl_array_any_type = (jl_value_t *)jl_apply_type(
        (jl_value_t *)jl_array_type, jl_svec(2, jl_any_type, jl_box_long(1)));

    jl_array_symbol_type = (jl_value_t *)jl_apply_type(
        (jl_value_t *)jl_array_type, jl_svec(2, jl_sym_type, jl_box_long(1)));

    jl_array_uint8_type =
        jl_apply_type((jl_value_t *)jl_array_type, jl_svec2(jl_uint8_type, jl_box_long(1)));

    jl_expr_type =
        jl_new_datatype(jl_symbol("Expr"), jl_any_type, jl_emptysvec,
                        jl_svec(3, jl_symbol("head"), jl_symbol("args"), jl_symbol("typ")),
                        jl_svec(3, jl_sym_type, jl_array_any_type, jl_any_type), 0, 1, 3);

    jl_linenumbernode_type =
        jl_new_datatype(jl_symbol("LineNumberNode"), jl_any_type, jl_emptysvec,
                        jl_svec(1, jl_symbol("line")), jl_svec(1, jl_long_type), 0, 0, 1);

    jl_labelnode_type =
        jl_new_datatype(jl_symbol("LabelNode"), jl_any_type, jl_emptysvec,
                        jl_svec(1, jl_symbol("label")), jl_svec(1, jl_long_type), 0, 0, 1);
    ...
    jl_quotenode_type =
        jl_new_datatype(jl_symbol("QuoteNode"), jl_any_type, jl_emptysvec,
                        jl_svec(1, jl_symbol("value")), jl_svec(1, jl_any_type), 0, 0, 1);

    jl_newvarnode_type = jl_new_datatype(jl_symbol("NewvarNode"), jl_any_type, jl_emptysvec,
                                         jl_svec(1, jl_symbol("slot")),
                                         jl_svec(1, jl_slotnumber_type), 0, 0, 1);

    jl_module_type = jl_new_datatype(jl_symbol("Module"), jl_any_type, jl_emptysvec,
                                     jl_svec(2, jl_symbol("name"), jl_symbol("parent")),
                                     jl_svec(2, jl_sym_type, jl_any_type), 0, 1, 2);

    jl_globalref_type = jl_new_datatype(jl_symbol("GlobalRef"), jl_any_type, jl_emptysvec,
    ...
    jl_method_type = jl_new_datatype(
        jl_symbol("Method"), jl_any_type, jl_emptysvec,
        jl_svec(15, jl_symbol("name"), jl_symbol("module"), jl_symbol("file"),
                jl_symbol("line"), jl_symbol("sig"), jl_symbol("tvars"), jl_symbol("ambig"),
                jl_symbol("specializations"), jl_symbol("lambda_template"),
                jl_symbol("roots"), jl_symbol("invokes"), jl_symbol("called"),
                jl_symbol("isstaged"), jl_symbol("needs_sparam_vals_ducttape"),
                jl_symbol("")),
        jl_svec(15, jl_sym_type, jl_module_type, jl_sym_type, jl_int32_type, jl_type_type,
                jl_any_type,
                jl_any_type, // Union{Array, Void}
                jl_any_type, jl_any_type, jl_array_any_type, jl_any_type, jl_int32_type,
                jl_bool_type, jl_bool_type, jl_bool_type),
        0, 1, 9);

    jl_lambda_info_type = jl_new_datatype(
        jl_symbol("LambdaInfo"), jl_any_type, jl_emptysvec,
        jl_svec(24, jl_symbol("rettype"), jl_symbol("sparam_syms"),
                jl_symbol("sparam_vals"), jl_symbol("specTypes"), jl_symbol("code"),
                jl_symbol("slottypes"), jl_symbol("ssavaluetypes"), jl_symbol("slotnames"),
                jl_symbol("slotflags"), jl_symbol("unspecialized_ducttape"),
                jl_symbol("def"), jl_symbol("constval"), jl_symbol("nargs"),
                jl_symbol("isva"), jl_symbol("inferred"), jl_symbol("pure"),
                jl_symbol("inlineable"), jl_symbol("inInference"), jl_symbol("inCompile"),
                jl_symbol("jlcall_api"), jl_symbol(""), jl_symbol("fptr"), jl_symbol(""),
                jl_symbol("")),
        jl_svec(24, jl_any_type, jl_simplevector_type, jl_simplevector_type, jl_any_type,
                jl_any_type, jl_any_type, jl_any_type, jl_array_any_type,
                jl_array_uint8_type, jl_any_type, jl_method_type, jl_any_type,
                jl_int32_type, jl_bool_type, jl_bool_type, jl_bool_type, jl_bool_type,
                jl_bool_type, jl_bool_type, jl_uint8_type, jl_bool_type, jl_any_type,
                jl_any_type, jl_any_type),
        0, 1, 7);
    ...
    jl_typector_type =
        jl_new_datatype(jl_symbol("TypeConstructor"), jl_type_type, jl_emptysvec,
                        jl_svec(2, jl_symbol("parameters"), jl_symbol("body")),
                        jl_svec(2, jl_simplevector_type, jl_any_type), 0, 0, 2);
    ...
    tv          = jl_svec1(tvar("T"));
    jl_ref_type = jl_new_abstracttype((jl_value_t *)jl_symbol("Ref"), jl_any_type, tv);
    ...
    // Type{T}
    jl_typetype_tvar = jl_new_typevar(jl_symbol("T"), (jl_value_t *)jl_bottom_type,
                                      (jl_value_t *)jl_any_type);
    ...
    jl_value_t *pointer_void =
        jl_apply_type((jl_value_t *)jl_pointer_type, jl_svec1(jl_void_type));
