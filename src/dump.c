...
static int get_next_edge(jl_array_t *list, int i, jl_value_t** invokesig, jl_method_instance_t **caller) JL_NOTSAFEPOINT
{
    jl_value_t *item = jl_array_ptr_ref(list, i);
    if (invokesig != NULL)
        *invokesig = NULL;
    assert(jl_is_method_instance(item));
    // Not an `invoke` call, it's just the MethodInstance
    *caller = (jl_method_instance_t*)item;
    return i + 1;
}

static int set_next_edge(jl_array_t *list, int i, jl_value_t *invokesig, jl_method_instance_t *caller)
{
    assert(!invokesig);
...
static void push_edge(jl_array_t *list, jl_value_t *invokesig, jl_method_instance_t *caller)
{
    assert(!invokesig);
        ...
        if (valid) {
            ...
            for (j = 0; j < jl_array_len(idxs_array); j++) {
                ...
                if (callee && jl_is_method_instance(callee)) {
                    ...
                    assert(invokesig == NULL); (void)invokesig;
