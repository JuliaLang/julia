    ...
    if (!imaging_mode && jl_options.use_precompiled == JL_OPTIONS_USE_PRECOMPILED_YES) {
        ...
        *sysimg_gvars[tls_getter_idx - 1] = (jl_value_t *)jl_get_ptls_states_getter();
        ...
        if (strcmp(cpu_target, "native") == 0) {
            ...
            if (!HasSSSE3)
                jl_error("The current host does not support SSSE3, but the system image "
                         "was compiled for Core2.\n"
                         "Please delete or regenerate sys.{so,dll,dylib}.");
        }
#endif
...
static void jl_serialize_gv_syms(jl_serializer_state *s, jl_sym_t *v)
{
    ...
    if (v->left)
        jl_serialize_gv_syms(s, v->left);
    if (v->right)
        jl_serialize_gv_syms(s, v->right);
}
...
static void jl_serialize_datatype(jl_serializer_state *s, jl_datatype_t *dt)
{
    ...
    if (strncmp(jl_symbol_name(dt->name->name), "#kw#", 4) == 0) {
        /* XXX: yuck, but the auto-generated kw types from the serializer isn't a real type,
                                           ...
                                           jl_value_t **loc)
{
    ...
    if (has_instance) {
        assert(
            s->mode !=
            MODE_MODULE_POSTWORK); // there shouldn't be an instance on a type with uid = 0
                                         ...
                                         jl_value_t **loc)
{
    ...
    if (vtag == (jl_value_t *)jl_simplevector_type || vtag == (jl_value_t *)LongSvec_tag) {
        ...
        if (m->lambda_template)
            jl_gc_wb(m, m->lambda_template);
        ...
        jl_datatype_t *dt = (jl_datatype_t *)jl_deserialize_value(
            s, NULL); // no loc, since if dt is replaced, then dt->instance would be also
        jl_set_typeof(v, dt);
        return v;
    }
    assert(0);
...
static int read_verify_mod_list(ios_t *s)
{
    if (!jl_main_module->uuid) {
        jl_printf(JL_STDERR,
                  "ERROR: Main module uuid state is invalid for module deserialization.\n");
        return 0;
    }
    while (1) {
...
static void jl_save_system_image_to_stream(ios_t *f)
{
    ...
    jl_serialize_gv_others(&s); // serialize things that might not have visible gc roots
                                // roots with GlobalValue references
...
static void jl_recache_types(void)
{
    ...
    while (i < flagref_list.len) {
        ...
        if (t != dt) {
            jl_set_typeof(
                dt,
                (void *)(intptr_t)0x10); // invalidate the old value to help catch errors
            if ((jl_value_t *)dt == o) {
                if (loc)
                    *loc = (jl_value_t *)t;
                if (offs > 0)
        ...
        if (t->instance != v) {
            jl_set_typeof(
                v, (void *)(intptr_t)0x20); // invalidate the old value to help catch errors
            if (v == o) {
                *loc = t->instance;
                if (offs > 0)
                    backref_list.items[offs] = t->instance;
