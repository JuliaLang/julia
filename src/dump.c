...
static void
jl_serialize_value_(jl_serializer_state *s, jl_value_t *v, int as_literal);
static jl_value_t *
jl_deserialize_value(jl_serializer_state *s, jl_value_t **loc);
...
static int module_in_worklist(jl_module_t *mod)
{
    ...
    for (i = 0; i < l; i++) {
        jl_module_t *workmod =
                (jl_module_t *)jl_array_ptr_ref(serializer_worklist, i);
...
static int type_in_worklist(jl_datatype_t *dt)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (type_in_worklist(
                    (jl_datatype_t *)(jl_is_datatype(p) ? p : jl_typeof(p))))
            return 1;
    }
    return 0;
}
...
static void jl_serialize_datatype(jl_serializer_state *s, jl_datatype_t *dt)
{
    ...
    if (!internal &&
        jl_unwrap_unionall(dt->name->wrapper) == (jl_value_t *)dt) {
        ...
        if (jl_unwrap_unionall(dt->name->wrapper) ==
            ...
            tag = 5; // internal, and not in the typename cache (just needs uid
                     // reassigned)
        else
            tag = 10; // anything else that's internal (just needs uid
                      // reassigned and possibly recaching)
    }
        ...
        tag = 10; // external, but definitely new (still needs uid and caching,
                  // but not full unique-ing)
    }
    else {
    ...
    if (strncmp(jl_symbol_name(dt->name->name), "#kw#", 4) == 0) {
        /* XXX: yuck, but the auto-generated kw types from the serializer isn't
         * a real type, so we *must* be very careful */
        assert(tag == 0 || tag == 5 || tag == 6 || tag == 10);
        if (tag == 6) {
            jl_methtable_t *mt = dt->name->mt;
            jl_datatype_t *primarydt = (jl_datatype_t *)jl_unwrap_unionall(
                    jl_get_global(mt->module, mt->name));
            assert(jl_is_datatype(primarydt));
            assert(jl_typeof(primarydt->name->mt->kwsorter) ==
                   (jl_value_t *)dt);
            dt = primarydt;
            tag = 9;
        }
    ...
    write_uint8(
            s->s,
            dt->abstract | (dt->mutabl << 1) | (has_layout << 2) |
                    (has_instance << 3) | (dt->hasfreetypevars << 4) |
                    (dt->isleaftype << 5));
...
{
    // TODO: this accidentally copies QuoteNode(Expr(...)) and
    // QuoteNode(svec(...))
    return jl_is_symbol(v) || jl_is_slot(v) || jl_is_ssavalue(v) ||
           jl_is_uniontype(v) || jl_is_expr(v) || jl_is_newvarnode(v) ||
           jl_is_svec(v) || jl_is_tuple(v) ||
           ((jl_datatype_t *)jl_typeof(v))->instance || jl_is_int32(v) ||
           jl_is_int64(v) || jl_is_bool(v) || jl_is_quotenode(v) ||
...
jl_serialize_value_(jl_serializer_state *s, jl_value_t *v, int as_literal)
{
    ...
    if (s->mode == MODE_AST) {
        ...
        if (jl_is_module(v)) {
            ...
            if (module_in_worklist(m) && !module_in_worklist(m->parent)) {
                // will need to reinsert this into parent bindings, later (in
                // case of any errors during reinsert)
                arraylist_push(&reinit_list, (void *)pos);
    ...
    if (jl_is_svec(v)) {
        ...
        if (s->mode == MODE_AST &&
            jl_globalref_mod(v) == s->tree_enclosing_module) {
        ...
        if (ar->flags.ndims == 1 && ar->elsize < 128) {
            ...
            write_uint16(
                    s->s, (ar->flags.ptrarray << 15) | (ar->elsize & 0x7fff));
        }
        ...
        if (li->max_world == 0 && li->min_world == 0) {
            internal = 1; // not world-tracked
                ...
                module_in_worklist(li->def.method->module)) {
        ...
        while ((jl_value_t *)te != jl_nothing) {
            for (i = 1; i < nf; i++) {
                if (jl_field_size(jl_typemap_entry_type, i) > 0)
                    jl_serialize_value(
                            s, jl_get_nth_field((jl_value_t *)te, i));
            }
            te = te->next;
        }
    }
        ...
        if (t == jl_int64_type && *(int64_t *)data >= S32_MIN &&
            *(int64_t *)data <= S32_MAX) {
            ...
            if (t == jl_typemap_level_type) {
                ...
                // (which will need to be rehashed during deserialization
                // anyhow)
                jl_typemap_level_t *node = (jl_typemap_level_t *)v;
                assert( // make sure this type has the expected ordering
                        offsetof(jl_typemap_level_t, arg1) ==
                                0 * sizeof(jl_value_t *) &&
                        offsetof(jl_typemap_level_t, targ) ==
                                2 * sizeof(jl_value_t *) &&
                        offsetof(jl_typemap_level_t, linear) ==
                                4 * sizeof(jl_value_t *) &&
                        offsetof(jl_typemap_level_t, any) ==
                                5 * sizeof(jl_value_t *) &&
                        offsetof(jl_typemap_level_t, key) ==
                                6 * sizeof(jl_value_t *) &&
            ...
            if (nf == 0 && jl_datatype_size(t) > 0) {
                if (t->name == jl_pointer_typename &&
                    jl_unbox_voidpointer(v) != (void *)-1) {
                    // normalize most pointers to NULL, to help catch memory
                    // errors
                    // but permit MAP_FAILED / INVALID_HANDLE to be stored
                    // unchanged
                    write_int32(s->s, 0);
#ifdef _P64
                    write_int32(s->s, 0);
    ...
    if (backedges) {
        ...
        for (i = 1; i < l; i += 2) {
            ...
            if (caller->max_world == ~(size_t)0) {
                ...
                jl_array_t **edges =
                        (jl_array_t **)ptrhash_bp(&edges_map, (void *)caller);
...
static void collect_backedges(jl_method_instance_t *callee)
{
    ...
    if (backedges) {
        ...
        for (i = 0; i < l; i++) {
            ...
            if (caller->max_world == ~(size_t)0) {
                jl_array_t **edges =
                        (jl_array_t **)ptrhash_bp(&edges_map, caller);
                if (*edges == HT_NOTFOUND)
                    *edges = jl_alloc_vec_any(0);
...
static int jl_collect_methcache_from_mod(jl_typemap_entry_t *ml, void *closure)
{
    ...
    if (module_in_worklist(m->module)) {
        ...
        jl_typemap_visitor(
                m->specializations, jl_collect_backedges_to_mod, closure);
    }
    return 1;
}
...
static void jl_collect_backedges_to(
        jl_method_instance_t *caller,
        jl_array_t *direct_callees,
        ...
        *pcallees = (jl_array_t *)HT_NOTFOUND;
        ...
        for (i = 0; i < l; i++) {
            ...
            if (jl_is_method_instance(c)) {
                jl_collect_backedges_to(
                        (jl_method_instance_t *)c, direct_callees, to_restore);
            }
        }
    }
}
...
static void jl_collect_backedges(jl_array_t *s)
{
    ...
    for (i = 0; i < edges_map.size; i += 2) {
        ...
        if (callees != HT_NOTFOUND &&
            module_in_worklist(caller->def.method->module)) {
            size_t i,
                    l = jl_array_len(
                            callees); // length may change during iteration
            for (i = 0; i < l; i++) { // only consider the initial list
                jl_value_t *c = jl_array_ptr_ref(callees, i);
                if (jl_is_method_instance(c)) {
                    jl_collect_backedges_to(
                            (jl_method_instance_t *)c, callees, &to_restore);
                }
            }
...
static void write_dependency_list(ios_t *s)
{
    ...
    if (!deps)
        deps = (jl_array_t *)jl_get_global(
                jl_base_module, jl_symbol("_require_dependencies"));
    ...
    jl_array_t *udeps =
            deps && unique_func ? (jl_array_t *)jl_apply(uniqargs, 2) : NULL;
...
static jl_value_t *
jl_deserialize_datatype(jl_serializer_state *s, int pos, jl_value_t **loc)
{
    ...
    if (tag == 6 || tag == 7) {
        ...
        if (tag == 7) {
            ...
            dtv = jl_apply_type(
                    dtv, jl_svec_data(parameters), jl_svec_len(parameters));
        }
        else {
    ...
    if (tag == 9) {
        jl_datatype_t *primarydt =
                (jl_datatype_t *)jl_deserialize_value(s, NULL);
    ...
    assert(s->tree_literal_values == NULL && s->mode != MODE_AST &&
           "no new data-types expected during MODE_AST");
    ...
    if (has_layout) {
        ...
        if (layout == 1) {
            dt->layout = ((jl_datatype_t *)jl_unwrap_unionall(
            ...
            size_t fielddesc_size =
                    nf > 0 ? jl_fielddesc_size(fielddesc_type) : 0;
            jl_datatype_layout_t *layout =
                    (jl_datatype_layout_t *)jl_gc_perm_alloc(
    ...
    if (has_instance) {
        assert(dt->uid != 0 &&
               "there shouldn't be an instance on a type with uid = 0");
        dt->instance = jl_deserialize_value(s, &dt->instance);
        jl_gc_wb(dt, dt->instance);
    }
    dt->name =
            (jl_typename_t *)jl_deserialize_value(s, (jl_value_t **)&dt->name);
    jl_gc_wb(dt, dt->name);
    dt->parameters = (jl_svec_t *)jl_deserialize_value(
            s, (jl_value_t **)&dt->parameters);
    jl_gc_wb(dt, dt->parameters);
    dt->super =
            (jl_datatype_t *)jl_deserialize_value(s, (jl_value_t **)&dt->super);
...
static jl_value_t *jl_deserialize_value_(
        jl_serializer_state *s, jl_value_t *vtag, jl_value_t **loc);
static jl_value_t *
jl_deserialize_value(jl_serializer_state *s, jl_value_t **loc)
{
    assert(!ios_eof(s->s));
    ...
    if (tag == BackRef_tag || tag == ShortBackRef_tag) {
        assert(s->tree_literal_values == NULL && s->mode != MODE_AST);
        uintptr_t offs =
                (tag == BackRef_tag) ? read_int32(s->s) : read_uint16(s->s);
        int isflagref = 0;
        isflagref = !!(offs & 1);
        offs >>= 1;
...
static jl_value_t *
jl_deserialize_value_svec(jl_serializer_state *s, jl_value_t *vtag)
{
...
static jl_value_t *
jl_deserialize_value_symbol(jl_serializer_state *s, jl_value_t *vtag)
{
    int usetable = (s->mode != MODE_AST);
    size_t len;
...
static jl_value_t *
jl_deserialize_value_array(jl_serializer_state *s, jl_value_t *vtag)
{
    int usetable = (s->mode != MODE_AST);
    int16_t i, ndims;
    ...
    jl_array_t *a = jl_new_array_for_deserialization(
            (jl_value_t *)NULL, ndims, dims, isunboxed, elsize);
    if (usetable)
...
static jl_value_t *
jl_deserialize_value_expr(jl_serializer_state *s, jl_value_t *vtag)
{
    int usetable = (s->mode != MODE_AST);
    size_t i, len;
...
static jl_value_t *
jl_deserialize_value_method(jl_serializer_state *s, jl_value_t **loc)
{
    int usetable = (s->mode != MODE_AST);
    jl_method_t *m = (jl_method_t *)jl_gc_alloc(
            s->ptls, sizeof(jl_method_t), jl_method_type);
    ...
    m->module =
            (jl_module_t *)jl_deserialize_value(s, (jl_value_t **)&m->module);
    jl_gc_wb(m, m->module);
    m->sparam_syms = (jl_svec_t *)jl_deserialize_value(
            s, (jl_value_t **)&m->sparam_syms);
    ...
    if (m->source)
        jl_gc_wb(m, m->source);
    m->unspecialized = (jl_method_instance_t *)jl_deserialize_value(
            s, (jl_value_t **)&m->unspecialized);
    if (m->unspecialized)
        jl_gc_wb(m, m->unspecialized);
    m->generator = (jl_method_instance_t *)jl_deserialize_value(
...
static jl_value_t *
jl_deserialize_value_method_instance(jl_serializer_state *s, jl_value_t **loc)
{
    int usetable = (s->mode != MODE_AST);
    ...
    li->specTypes = (jl_value_t *)jl_deserialize_value(
            s, (jl_value_t **)&li->specTypes);
    if (li->specTypes)
    ...
    li->sparam_vals = (jl_svec_t *)jl_deserialize_value(
            s, (jl_value_t **)&li->sparam_vals);
    jl_gc_wb(li, li->sparam_vals);
    li->backedges = (jl_array_t *)jl_deserialize_value(
            s, (jl_value_t **)&li->backedges);
    if (li->backedges)
...
static jl_value_t *jl_deserialize_value_module(jl_serializer_state *s)
{
    ...
    if (ref_only) {
        jl_value_t *m_ref = jl_get_global(
                (jl_module_t *)jl_deserialize_value(s, NULL), mname);
        if (usetable)
    ...
    m->parent =
            (jl_module_t *)jl_deserialize_value(s, (jl_value_t **)&m->parent);
    jl_gc_wb(m, m->parent);
    ...
    while (1) {
        ...
        b->owner = (jl_module_t *)jl_deserialize_value(
                s, (jl_value_t **)&b->owner);
        if (b->owner != NULL)
            jl_gc_wb(m, b->owner);
...
static jl_value_t *
jl_deserialize_value_singleton(jl_serializer_state *s, jl_value_t **loc)
{
    if (s->mode == MODE_AST) {
    ...
    jl_datatype_t *dt = (jl_datatype_t *)jl_deserialize_value(
            s, (jl_value_t **)HT_NOTFOUND); // no loc, since if dt is replaced,
                                            // then dt->instance would be also
    jl_set_typeof(v, dt);
    return v;
}

static void
jl_deserialize_struct(jl_serializer_state *s, jl_value_t *v, size_t startfield)
{
    ...
    for (i = startfield; i < nf; i++) {
        if (jl_field_size(dt, i) > 0) {
            if (jl_field_isptr(dt, i)) {
                jl_value_t **fld =
                        (jl_value_t **)(data + jl_field_offset(dt, i));
    ...
    if (dt == jl_typemap_entry_type) {
        if (((jl_typemap_entry_t *)v)->max_world == ~(size_t)0) {
            ...
            ((jl_typemap_entry_t *)v)->min_world =
                    ((jl_typemap_entry_t *)v)->max_world - 1;
        }
    }
}
...
{
    int N = read_int32(s->s);
    int n = N;
    jl_value_t *te = jl_nothing;
    jl_value_t **pn = &te;
    while (n > 0) {
        jl_value_t *v = jl_gc_alloc(
                s->ptls,
                jl_datatype_size(jl_typemap_entry_type),
...
static jl_value_t *jl_deserialize_value_any(
        jl_serializer_state *s, jl_value_t *vtag, jl_value_t **loc)
{
    int usetable = (s->mode != MODE_AST);
    int32_t sz =
            (vtag == (jl_value_t *)SmallDataType_tag ? read_uint8(s->s)
                                                     : read_int32(s->s));
    ...
    if (s->mode == MODE_MODULE && dt == jl_typename_type) {
        ...
        if (ref_only) {
            ...
            jl_datatype_t *dt =
                    (jl_datatype_t *)jl_unwrap_unionall(jl_get_global(m, sym));
        ...
        jl_serializer_state *s, jl_value_t *vtag, jl_value_t **loc)
{
    ...
    if (vtag == (jl_value_t *)jl_simplevector_type ||
        ...
        return deser_symbols[read_uint8(s->s)];
            ...
            vtag == (jl_value_t *)jl_symbol_type ||
        ...
        assert(vtag == (jl_value_t *)jl_datatype_type ||
...
static void jl_insert_methods(jl_array_t *list)
{
    ...
    for (i = 0; i < l; i += 2) {
        ...
        jl_tupletype_t *simpletype =
                (jl_tupletype_t *)jl_array_ptr_ref(list, i + 1);
...
static size_t
lowerbound_dependent_world_set(size_t world, arraylist_t *dependent_worlds)
{
...
static void jl_insert_backedges(jl_array_t *list, arraylist_t *dependent_worlds)
{
    ...
    for (i = 0; i < l; i += 2) {
        jl_method_instance_t *caller =
                (jl_method_instance_t *)jl_array_ptr_ref(list, i);
        ...
        for (j = 0; valid && j < jl_array_len(callees); j++) {
            ...
            for (k = 0; valid && k < jl_array_len(matches); k++) {
                jl_method_t *m = (jl_method_t *)jl_svecref(
                        jl_array_ptr_ref(matches, k), 2);
                if (lowerbound_dependent_world_set(
                            m->min_world, dependent_worlds) != m->min_world) {
                    // intersection has a new method and is now probably no
                    // good, just invalidate everything now
                    valid = 0;
                }
            }
        }
            ...
            for (j = 0; j < jl_array_len(callees); j++) {
                ...
                if (jl_is_method_instance(callee)) {
                    jl_method_instance_add_backedge(
                            (jl_method_instance_t *)callee, caller);
                }
                else {
...
static jl_value_t *read_verify_mod_list(ios_t *s, arraylist_t *dependent_worlds)
{
    if (!jl_main_module->uuid) {
        return jl_get_exceptionf(
                jl_errorexception_type, "Main module uuid state is invalid for "
    ...
    while (1) {
        ...
        if (!m) {
            ...
            if (!require_func)
                require_func =
                        jl_get_global(jl_base_module, jl_symbol("require"));
        ...
        if (!m) {
            return jl_get_exceptionf(
                    ...
                    "Requiring \"%s\" did not define a corresponding module.",
                    name);
        }
        if (!jl_is_module(m)) {
            return jl_get_exceptionf(
                    jl_errorexception_type,
                    "Invalid module path (%s does not name a module).",
                    name);
        }
        if (m->uuid != uuid) {
            return jl_get_exceptionf(
                    jl_errorexception_type,
                    "Module %s uuid did not match cache file.",
                    name);
        }
...
jl_finalize_deserializer(jl_serializer_state *s, arraylist_t *tracee_list)
{
    ...
    while (pos != -1) {
        jl_reinit_item(
                (jl_value_t *)backref_list.items[pos],
                read_int32(s->s),
                tracee_list);
        pos = read_int32(s->s);
    }
...
JL_DLLEXPORT jl_array_t *jl_compress_ast(jl_method_t *m, jl_code_info_t *code)
{
    ...
    jl_serializer_state s = {
        &dest, MODE_AST, m->roots, m->module, jl_get_ptls_states()
    };
    ...
    assert(nsyms >= m->nargs &&
           nsyms < INT32_MAX); // required by generated functions
...
JL_DLLEXPORT jl_code_info_t *jl_uncompress_ast(jl_method_t *m, jl_array_t *data)
{
    ...
    assert(jl_array_len(data) > 2 &&
           ((uint8_t *)data->data)[jl_array_len(data) - 1] == 0);
    size_t i;
    ios_t src;
    ios_mem(&src, 0);
    ...
    int en = jl_gc_enable(0); // Might GC
    jl_serializer_state s = {
        &src, MODE_AST, m->roots, m->module, jl_get_ptls_states()
    };

    jl_code_info_t *code = (jl_code_info_t *)jl_gc_alloc(
    ...
    for (i = 0; i < nf - 5; i++) {
        ...
        jl_value_t **fld =
                (jl_value_t *
                         *)((char *)jl_data_ptr(code) + jl_field_offset(jl_code_info_type, i));
        *fld = jl_deserialize_value(&s, fld);
    }
...
{
    char *tmpfname =
            strcat(strcpy((char *)alloca(strlen(fname) + 8), fname), ".XXXXXX");
    ios_t f;
    if (ios_mkstemp(&f, tmpfname) == NULL) {
        jl_printf(
                JL_STDERR,
                "Cannot open cache file \"%s\" for writing.\n",
                tmpfname);
        return 1;
    }
    ...
    jl_serializer_state s = {
        &f, MODE_MODULE, NULL, NULL, jl_get_ptls_states()
    };
...
#ifndef NDEBUG
// skip the performance optimizations of jl_types_equal and just use subtyping
// directly
// one of these types is invalid - that's why we're doing the recache type
// operation
...
static jl_datatype_t *
jl_recache_type(jl_datatype_t *dt, size_t start, jl_value_t *v)
{
    if (v == NULL)
    ...
    if (dt->uid == 0) {
        ...
        if (jl_svec_len(tt) ==
            0) { // jl_cache_type doesn't work if length(parameters) == 0
            dt->uid = jl_assign_type_uid();
            t = dt;
        }
        else {
    ...
    while (i < flagref_list.len) {
        ...
        if (jl_is_method(o) || jl_is_method_instance(o)) {
            ...
            if (t != dt) {
                ...
                jl_set_typeof(dt, (void *)(intptr_t)0x10); // invalidate the old
                                                           // datatype to help
                                                           // catch errors
                if ((jl_value_t *)dt == o) {
                    if (loc)
            ...
            if (t->instance != v) {
                jl_set_typeof(v, (void *)(intptr_t)0x20); // invalidate the old
                                                          // value to help catch
                                                          // errors
                if (v == o) {
                    *loc = t->instance;
            ...
            // delete this item from the flagref list, so it won't be
            // re-encountered later
            flagref_list.len -= 2;
            if (i >= flagref_list.len)
                break;
            flagref_list.items[i + 0] =
                    flagref_list.items[flagref_list.len + 0];
            flagref_list.items[i + 1] =
                    flagref_list.items[flagref_list.len + 1];
        }
    }
}

static void
jl_update_backref_list(jl_value_t *old, jl_value_t *_new, size_t start)
{
...
static jl_method_t *jl_lookup_method_worldset(
        jl_methtable_t *mt, jl_datatype_t *sig, arraylist_t *dependent_worlds)
{
    ...
    while (1) {
        ...
        world = lowerbound_dependent_world_set(
                _new->min_world, dependent_worlds);
...
static jl_method_t *
jl_recache_method(jl_method_t *m, size_t start, arraylist_t *dependent_worlds)
{
    jl_datatype_t *sig = (jl_datatype_t *)m->sig;
    jl_datatype_t *ftype = jl_first_argument_datatype((jl_value_t *)sig);
    jl_methtable_t *mt = ftype->name->mt;
    jl_set_typeof(m, (void *)(intptr_t)0x30); // invalidate the old value to
                                              // help catch errors
...
static jl_method_instance_t *jl_recache_method_instance(
        jl_method_instance_t *li, size_t start, arraylist_t *dependent_worlds)
{
    ...
    jl_set_typeof(li, (void *)(intptr_t)0x40); // invalidate the old value to
                                               // help catch errors
    jl_svec_t *env = jl_emptysvec;
    jl_value_t *ti = jl_type_intersection_env(
            (jl_value_t *)argtypes, (jl_value_t *)m->sig, &env);
    // assert(ti != jl_bottom_type); (void)ti;
    if (ti == jl_bottom_type)
        env = jl_emptysvec; // the intersection may fail now if the type system
                            // had made an incorrect subtype env in the past
    jl_method_instance_t *_new = jl_specializations_get_linfo(
            m, (jl_value_t *)argtypes, env, jl_world_counter);
...
static void jl_recache_other(arraylist_t *dependent_worlds)
{
    ...
    while (i < flagref_list.len) {
        ...
        int offs = (int)(intptr_t)flagref_list.items[i + 1];
        jl_value_t *_new,
                *o = loc ? *loc : (jl_value_t *)backref_list.items[offs];
        i += 2;
        if (jl_is_method(o)) {
            // lookup the real Method based on the placeholder sig
            _new = (jl_value_t *)jl_recache_method(
                    (jl_method_t *)o, i, dependent_worlds);
        }
        else if (jl_is_method_instance(o)) {
            // lookup the real MethodInstance based on the placeholder specTypes
            _new = (jl_value_t *)jl_recache_method_instance(
                    (jl_method_instance_t *)o, i, dependent_worlds);
        }
        else {
            abort();
        }
...
static jl_value_t *_jl_restore_incremental(ios_t *f)
{
    ...
    qsort(dependent_worlds.items,
          dependent_worlds.len,
          sizeof(size_t),
          size_isgreater);

    jl_serializer_state s = { f, MODE_MODULE, NULL, NULL, ptls };
    jl_array_t *restored =
            (jl_array_t *)jl_deserialize_value(&s, (jl_value_t **)&restored);
    ...
    jl_value_t *external_backedges =
            jl_deserialize_value(&s, &external_backedges);

    arraylist_t *tracee_list = NULL;
    if (jl_newmeth_tracer)
        tracee_list =
                arraylist_new((arraylist_t *)malloc(sizeof(arraylist_t)), 0);
    ...
    jl_insert_methods((jl_array_t *)external_methods); // hook up methods of
                                                       // external generic
                                                       // functions (needs to be
                                                       // after recache types)
    jl_recache_other(&dependent_worlds); // make all of the other objects
                                         // identities correct (needs to be
    ...
    jl_gc_enable(
            en); // subtyping can allocate a lot, not valid before recache-other

    jl_insert_backedges(
            (jl_array_t *)external_backedges,
            &dependent_worlds); // restore external backedges (needs to be last)
    ...
    jl_gc_enable_finalizers(ptls, 1); // make sure we don't run any Julia code
                                      // concurrently before this point
    if (tracee_list) {
        jl_methtable_t *mt;
...
jl_restore_incremental_from_buf(const char *buf, size_t sz)
{
    ios_t f;
