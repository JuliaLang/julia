...
// atomic loads
template <typename T> static inline T jl_atomic_load(volatile T *obj)
{
...
template <typename T> static inline T jl_atomic_load_acquire(volatile T *obj)
{
    T val = *obj;
    jl_signal_fence();
    return val;
...
STATIC_INLINE int8_t jl_gc_state_set(jl_ptls_t ptls, int8_t state, int8_t old_state)
{
    ...
    // non GC-safe region.
    if (old_state && !state) jl_gc_safepoint_(ptls);
    return old_state;
}
...
static inline void jl_mutex_lock_nogc(jl_mutex_t *lock) { jl_mutex_wait(lock, 0); }
...
static inline void jl_mutex_check_type(jl_mutex_t *m) { (void)m; }
