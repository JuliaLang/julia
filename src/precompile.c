...
void write_srctext(ios_t *f, jl_array_t *udeps, int64_t srctextpos, int64_t datastartpos, int64_t checksumpos) {
...
JL_DLLEXPORT void jl_write_compiler_output(void)
{
    ...
    jl_create_system_image(native_code, jl_options.incremental ? worklist : NULL, emit_split, &s, &z, &udeps, &srctextpos, &datastartpos, &checksumpos);
    ...
    if (jl_options.outputji || emit_native)
        write_srctext(s, udeps, srctextpos, datastartpos,  checksumpos);
