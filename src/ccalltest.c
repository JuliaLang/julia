...
// Intel compiler does not currently support complex int (which is GNU
// extension)
...
JL_DLLEXPORT struct1 test_1long_a(jint x1, jint x2, jint x3, struct1 a, float b)
{
    ...
    if (verbose)
        fprintf(stderr,
                "(%" PRIjint ", %" PRIjint ", %" PRIjint ") & %g + %g i & %g\n",
                x1,
                x2,
                x3,
                a.x,
...
JL_DLLEXPORT struct1
test_1long_b(jint x1, double x2, jint x3, struct1 a, float b)
{
    // Unpack a "small" struct { float, double }
    if (verbose)
        fprintf(stderr,
                "(%" PRIjint ", %g, %" PRIjint ") & %g + %g i & %g\n",
                x1,
                x2,
                x3,
                a.x,
...
JL_DLLEXPORT struct1
test_1long_c(jint x1, double x2, jint x3, jint x4, struct1 a, float b)
{
    // Unpack a "small" struct { float, double }
    if (verbose)
        fprintf(stderr,
                "(%" PRIjint ", %g, %" PRIjint ", %" PRIjint
                ") & %g + %g i & %g\n",
                x1,
                x2,
                x3,
...
{
    // The C equivalent of a  ComplexPair{Float32} struct (but without special
    // complex ABI)
    if (verbose)
        fprintf(stderr, "%g + %g i & %g\n", a.x, a.y, b);
    a.x += b * 1;
...
{
    // The C equivalent of a  ComplexPair{Float64} struct (but without special
    // complex ABI)
    if (verbose)
        fprintf(stderr, "%g + %g i & %g\n", a.x, a.y, b);
    a.x += b * 1;
...
JL_DLLEXPORT int128_t test_128(int128_t a, int64_t b)
{
    ...
    if (verbose)
        fprintf(stderr,
                "0x%016" PRIx64 "%016" PRIx64 " & %" PRId64 "\n",
                a.y,
                a.x,
                b);
    a.x += b * 1;
...
JL_DLLEXPORT struct_big test_big_long(jint x1, jint x2, jint x3, struct_big a)
{
    ...
    if (verbose)
        fprintf(stderr,
                "(%" PRIjint ", %" PRIjint ", %" PRIjint ") %" PRIjint
                " %" PRIjint " %c\n",
                x1,
                x2,
                x3,
...
#define test_huge(suffix, reg)                                                 \
    JL_DLLEXPORT struct_huge##suffix test_huge##suffix(                        \
            char a, struct_huge##suffix b, char c)                             \
    {                                                                          \
        if (verbose)                                                           \
            fprintf(stderr, "%c-%c\n", a, c);                                  \
        b.reg *= 39;                                                           \
...
JL_DLLEXPORT int test_long_args_intp(
        int *a1,
        int *a2,
        int *a3,
        int *a4,
...
JL_DLLEXPORT int test_long_args_int(
        int a1,
        int a2,
        int a3,
        int a4,
...
JL_DLLEXPORT float test_long_args_float(
        float a1,
        float a2,
        float a3,
        float a4,
...
JL_DLLEXPORT double test_long_args_double(
        double a1,
        double a2,
        double a3,
        double a4,
...
JL_DLLEXPORT int
test_spill_int4(int *v1, int *v2, int *v3, int *v4, struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int5(
        int *v1, int *v2, int *v3, int *v4, int *v5, struct_spill_pint s)
{
    return *v1 + *v2 + *v3 + *v4 + *v5 + *s.a + *s.b;
}

JL_DLLEXPORT int test_spill_int6(
        int *v1,
        int *v2,
        int *v3,
        int *v4,
...
JL_DLLEXPORT int test_spill_int7(
        int *v1,
        int *v2,
        int *v3,
        int *v4,
...
JL_DLLEXPORT int test_spill_int8(
        int *v1,
        int *v2,
        int *v3,
        int *v4,
...
JL_DLLEXPORT int test_spill_int9(
        int *v1,
        int *v2,
        int *v3,
        int *v4,
...
JL_DLLEXPORT int test_spill_int10(
        int *v1,
        int *v2,
        int *v3,
        int *v4,
        ...
        struct_spill_pint s)
{
    return (*v1 + *v2 + *v3 + *v4 + *v5 + *v6 + *v7 + *v8 + *v9 + *v10 + *s.a +
            *s.b);
}
...
JL_DLLEXPORT float
test_spill_float3(float v1, float v2, float v3, struct_spill_float s)
{
    return v1 + v2 + v3 + s.a + s.b;
}

JL_DLLEXPORT float
test_spill_float4(float v1, float v2, float v3, float v4, struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float5(
        float v1, float v2, float v3, float v4, float v5, struct_spill_float s)
{
    return v1 + v2 + v3 + v4 + v5 + s.a + s.b;
}

JL_DLLEXPORT float test_spill_float6(
        float v1,
        float v2,
        float v3,
        float v4,
...
JL_DLLEXPORT float test_spill_float7(
        float v1,
        float v2,
        float v3,
        float v4,
...
JL_DLLEXPORT float test_spill_float8(
        float v1,
        float v2,
        float v3,
        float v4,
...
JL_DLLEXPORT float test_spill_float9(
        float v1,
        float v2,
        float v3,
        float v4,
...
JL_DLLEXPORT float test_spill_float10(
        float v1,
        float v2,
        float v3,
        float v4,
...
{
    // 64-bit x86 has only level 2 SSE, which does not have a <4 x int32>
    // multiplication,
...
JL_DLLEXPORT struct_aa64_1
test_aa64_i128_2(int64_t v1, __int128 v2, struct_aa64_1 v3)
{
...
JL_DLLEXPORT struct_aa64_2
test_aa64_fp16_2(int v1, float v2, double v3, __fp16 v4)
{
...
JL_DLLEXPORT int64_t test_ppc64_vec1long(
        ...
        int64_t d8,
        int64_t d9,
        struct_huge1_ppc64 vs)
{
    return d1 + d2 + d3 + d4 + d5 + d6 + d7 + d8 + d9 + vs.m + vs.v[0] +
           vs.v[1] + vs.v[2] + vs.v[3];
}
...
JL_DLLEXPORT int64_t test_ppc64_vec1long_vec(
        ...
        float32x4_t vs)
{
    return d1 + d2 + d3 + d4 + d5 + d6 + d7 + d8 + d9 + vs[0] + vs[1] + vs[2] +
           vs[3];
}

JL_DLLEXPORT float32x4_t test_ppc64_vec2(
        int64_t d1,
        float32x4_t a,
        float32x4_t b,
        float32x4_t c,
...
{
    float32x4_t r;
    r[0] = d1 + a[0] + b[0] + c[0] + d[0] + e[0] + f[0] + g[0] + h[0] + i[0] +
           j[0] + k[0] + l[0] + m[0] + n[0];
    r[1] = d1 + a[1] + b[1] + c[1] + d[1] + e[1] + f[1] + g[1] + h[1] + i[1] +
           j[1] + k[1] + l[1] + m[1] + n[1];
    r[2] = d1 + a[2] + b[2] + c[2] + d[2] + e[2] + f[2] + g[2] + h[2] + i[2] +
           j[2] + k[2] + l[2] + m[2] + n[2];
    r[3] = d1 + a[3] + b[3] + c[3] + d[3] + e[3] + f[3] + g[3] + h[3] + i[3] +
           j[3] + k[3] + l[3] + m[3] + n[3];
    return r;
}
...
{
    printf("c_exit_finalizer: %d, %u",
           *(int *)v,
           (unsigned)((uintptr_t)v & (uintptr_t)1));
}
