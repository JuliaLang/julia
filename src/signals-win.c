...
static void JL_NORETURN start_backtrace_fiber(void)
{
    ...
    ptls->bt_size = rec_backtrace_ctx(ptls->bt_data, JL_MAX_BT_SIZE, error_ctx, NULL); // FIXME!
...
#error WIN16 not supported :P
    ...
    if (!ptls->safe_restore) {
        ...
        if (excpt != jl_stackovf_exception) {
            ptls->bt_size = rec_backtrace_ctx(ptls->bt_data, JL_MAX_BT_SIZE, ctxThread, NULL); // FIXME!
        }
    ...
    while (1) {
        if (running && bt_size_cur < bt_size_max) {
            ...
            bt_size_cur += rec_backtrace_ctx((uintptr_t*)bt_data_prof + bt_size_cur,
                bt_size_max - bt_size_cur - 1, &ctxThread, NULL); // FIXME!
