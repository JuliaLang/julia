...
struct ABI_AArch64Layout : AbiLayout {
    ...
    Type *get_llvm_vectype(jl_datatype_t *dt) const
    {
        ...
        // `!dt->mutabl && dt->pointerfree && !dt->haspadding && dt->nfields >
        // 0`
        if (dt->layout == NULL)
            return nullptr;
...
#define jl_is_floattype(v) jl_subtype(v, (jl_value_t *)jl_floatingpoint_type)
    ...
    // Note that it is the fundamental types that are important and not the
    // member
    // types.
    bool isHFAorHVA(
            jl_datatype_t *dt, size_t dsz, size_t &nele, ElementType &ele) const
    {
        ...
        while (size_t nfields = jl_datatype_nfields(dt)) {
            ...
            assert(i < nfields);
            // If there's only one non zero sized member, try again on this
            // member
            if (fieldsz == dsz) {
            ...
            for (; i < nfields; i++) {
                ...
                if (fieldsz == 0)
                    continue;
                jl_datatype_t *fieldtype =
                        (jl_datatype_t *)jl_field_type(dt, i);
                // Check element count.
    ...
    Type *isHFAorHVA(jl_datatype_t *dt, size_t &nele) const
    {
        ...
        // Aggregate
        // with a Fundamental Data Type that is a Floating-Point type and at
        // most
        // four uniquely addressable members.
        // An Homogeneous Short-Vector Aggregate (HVA) is an Homogeneous
        // Aggregate
        // with a Fundamental Data Type that is a Short-Vector type and at most
        // four
        ...
        if (dsz > 64 || !dt->layout || dt->layout->npointers ||
    ...
    bool needPassByRef(jl_datatype_t *dt, AttrBuilder &ab) override
    {
        ...
        // We only check for the total size and not whether it is a composite
        // type
            ...
            size_t *rewrite_len) const
    {
        ...
        if (Type *eltype = isHFAorHVA(dt, *rewrite_len)) {
            ...
            *fpreg = true;
            // Rewrite to [n x eltype] where n is the number of fundamental
            // types.
            return eltype;
        }
        ...
        // C.3
        //   If the argument is an HFA or an HVA then the NSRN is set to 8 and
        //   the
        //   size of the argument is rounded up to the nearest multiple of 8
        //   bytes.
        // C.4
        //   If the argument is an HFA, an HVA, a Quad-precision Floating-point
        //   or
        //   Short Vector Type then the NSAA is rounded up to the larger of 8 or
        //   the Natural Alignment of the argument’s type.
        // C.5
        //   If the argument is a Half- or Single- precision Floating Point
        //   type,
        //   then the size of the argument is set to 8 bytes. The effect is as
        //   if
        //   the argument had been copied to the least significant bits of a
        //   64-bit
        ...
        // pointers. We don't need to worry about floating points here since
        // they
        // are handled above.
        ...
        // C.8
        //   If the argument has an alignment of 16 then the NGRN is rounded up
        //   to
        //   the next even number.
        // C.9
        //   If the argument is an Integral Type, the size of the argument is
        //   equal
        //   to 16 and the NGRN is less than 7, the argument is copied to
        //   x[NGRN]
        //   and x[NGRN+1]. x[NGRN] shall contain the lower addressed
        //   double-word
        //   of the memory representation of the argument. The NGRN is
        //   incremented
        ...
        //   x[NGRN].
        //   The argument is passed as though it had been loaded into the
        //   registers
        //   from a double-word-aligned address with an appropriate sequence of
        //   LDR
        //   instructions loading consecutive registers from memory (the
        //   contents of
        //   any unused parts of the registers are unspecified by this
        //   standard).
        //   The NGRN is incremented by the number of registers used. The
        //   argument
        ...
        assert(jl_datatype_size(dt) <=
               16); // Should be pass by reference otherwise
        *rewrite_len = (jl_datatype_size(dt) + 7) >> 3;
        ...
        // C.14
        //   If the size of the argument is less than 8 bytes then the size of
        //   the
