...
static void write_gctaggedfield(jl_serializer_state *s, jl_datatype_t *ref) JL_NOTSAFEPOINT
{
    ...
    write_uint(s->s, s->incremental ? 0 : GC_OLD);
}

static void write_constreloc(jl_serializer_state *s, jl_datatype_t *ref) JL_NOTSAFEPOINT
{
    assert(!s->incremental);
    arraylist_push(&s->constreloc_list, (void*)(uintptr_t)ios_pos(s->const_data));
    arraylist_push(&s->constreloc_list, (void*)backref_id(s, ref, s->link_ids_gctags));
    write_uint(s->const_data, GC_OLD_MARKED);
}
...
#define JL_ARRAY_ALIGN(jl_value, nbytes) LLT_ALIGN(jl_value, nbytes)
            ...
            if (s->incremental && jl_typeis(v, jl_typemap_entry_type)) {
                jl_typemap_entry_t *newentry = (jl_typemap_entry_t*)&f->buf[reloc_offset];
...
static uintptr_t get_reloc_for_item(uintptr_t reloc_item, size_t reloc_offset, size_t relative_to, ssize_t constdata_offset)
{
    ...
    if (tag == DataRef) {
        ...
        if ((tag == ConstDataRef) == (constdata_offset > 0))
            // constdata_offset's sign reflects which section 'relative_to' is
            // itself relative to so we need to adjust the reloc_base if they disagree
            reloc_base += constdata_offset;
        return reloc_base + reloc_offset - (relative_to & ~(size_t)15);
    }
    else {
        // just write the item reloc_id directly
        assert(relative_to == 0 && "type tag DataTypes must be in DataRef section");
...
static void jl_finish_relocs(char *base, size_t size, arraylist_t *list, int absolute, ssize_t constdata_offset)
{
    for (size_t i = 0; i < list->len; i += 2) {
        ...
        *pv = get_reloc_for_item(item, *pv, absolute ? 0 : pos + sizeof(void*), constdata_offset);
    }
}
...
static void jl_read_reloclist(jl_serializer_state *s, jl_array_t *link_ids, uint8_t bits)
{
    ...
    while (1) {
        ...
        if (bits)
            v = (v | bits) - ((uintptr_t)(pv + 1) & ~(uintptr_t)15);
        *pv = v;
    }
...
static void jl_skip_reloclist(jl_serializer_state *s)
{
    void *current = (void*)(s->relocs->buf + s->relocs->bpos);
    void *endmark = memchr(current, 0, s->relocs->size - s->relocs->bpos);
    assert(endmark);
    s->relocs->bpos += ((char*)endmark - (char*)current) + 1;
    assert(s->relocs->bpos <= s->relocs->size);
}
...
void gc_sweep_sysimg(void)
{
    ...
    for (size_t i = 0; i < 2*nblobs; i+=2) {
        ...
        while (1) {
            ...
            jl_atomic_store_relaxed(&o->valuetag, (jl_atomic_load_relaxed(&o->valuetag) & ~(uintptr_t)15) | GC_OLD);
        }
    }
}
...
static void _jl_write_value(jl_serializer_state *s, jl_value_t *v)
{
    ...
    uintptr_t reloc = get_reloc_for_item(item, 0, 0, 0);
                                           ...
                                           jl_array_t *ext_targets, jl_array_t *edges) JL_GC_DISABLED
{
    ...
    arraylist_new(&s.constreloc_list, 0);
    ...
    ssize_t constdata_offset = ios_pos(f);
    ios_seek(&const_data, 0);
    ios_copyall(f, &const_data);
    size_t constdata_size = s.const_data->size;
    assert(ios_pos(f) - constdata_offset == constdata_size);
    ...
    char *base = &f->buf[0];
    jl_finish_relocs(base + sysimg_offset, sysimg_size, &s.gctags_list, s.incremental, constdata_offset - sysimg_offset);
    jl_finish_relocs(base + sysimg_offset, sysimg_size, &s.relocs_list, 1, constdata_offset - sysimg_offset);
    jl_finish_relocs(base + constdata_offset, constdata_size, &s.constreloc_list, 0, -(constdata_offset - sysimg_offset));
    ...
    arraylist_free(&s.constreloc_list);
        ...
        if (otyp == (jl_value_t*)jl_datatype_type) {
            ...
            if (jl_is_datatype(dt)) {
                ...
                if (newdt == NULL) {
                    ...
                    // force it old
                    jl_atomic_store_relaxed(&jl_astaggedvalue(newdt)->valuetag, (jl_atomic_load_relaxed(&jl_astaggedvalue(newdt)->valuetag) & ~(uintptr_t)15) | GC_OLD);
