...
static const jl_fptr_t id_to_fptrs[] = { NULL,
                                         NULL,
                                         jl_f_throw,
                                         jl_f_is,
                                         jl_f_typeof,
...
enum RefTags { DataRef, ConstDataRef, TagRef, SymbolRef, BindingRef };

// this supports up to 1 GB images and 16 RefTags
// if a larger size is required, will need to add support for writing larger
// relocations in many cases below
...
static void jl_load_sysimg_so(void)
{
    ...
    if (!imaging_mode &&
        ...
        sysimg_gvars_base =
                (uintptr_t *)jl_dlsym(jl_sysimg_handle, "jl_sysimg_gvars_base");
        sysimg_gvars_offsets = (const int32_t *)jl_dlsym(
                jl_sysimg_handle, "jl_sysimg_gvars_offsets");
        sysimg_fvars_base = (const char *)jl_dlsym(
                jl_sysimg_handle, "jl_sysimg_fvars_base");
        sysimg_fvars_offsets = (const int32_t *)jl_dlsym(
                jl_sysimg_handle, "jl_sysimg_fvars_offsets");
        globalUnique = *(size_t *)jl_dlsym(jl_sysimg_handle, "jl_globalUnique");
#ifdef JULIA_ENABLE_THREADING
        size_t tls_getter_idx = *(size_t *)jl_dlsym(
                jl_sysimg_handle, "jl_ptls_states_getter_idx");
        *sysimg_gvars(sysimg_gvars_base, tls_getter_idx - 1) =
                (uintptr_t)jl_get_ptls_states_getter();
        size_t tls_offset_idx =
                *(size_t *)jl_dlsym(jl_sysimg_handle, "jl_tls_offset_idx");
        *sysimg_gvars(sysimg_gvars_base, tls_offset_idx - 1) =
                (uintptr_t)(jl_tls_offset == -1 ? 0 : jl_tls_offset);
#endif
        const char *cpu_target = (const char *)jl_dlsym(
                jl_sysimg_handle, "jl_sysimg_cpu_target");
        if (strcmp(cpu_target, jl_options.cpu_target) != 0)
            jl_error("Julia and the system image were compiled for different "
                     "architectures.\n"
        ...
        if (strcmp(cpu_target, "native") == 0) {
            if (!RUNNING_ON_VALGRIND) {
                uint64_t saved_cpuid = *(uint64_t *)jl_dlsym(
                        jl_sysimg_handle, "jl_sysimg_cpu_cpuid");
                if (saved_cpuid !=
                    (((uint64_t)info[2]) | (((uint64_t)info[3]) << 32)))
                    jl_error("Target architecture mismatch. Please delete or "
                             "regenerate sys.{so,dll,dylib}.");
            }
        }
        else if (strcmp(cpu_target, "core2") == 0) {
            int HasSSSE3 = (info[2] & 1 << 9);
            if (!HasSSSE3)
                jl_error("The current host does not support SSSE3, but the "
                         "system image was compiled for Core2.\n"
                         "Please delete or regenerate sys.{so,dll,dylib}.");
        }
#endif
    ...
    const char *sysimg_data =
            (const char *)jl_dlsym(jl_sysimg_handle, "jl_system_image_data");
...
static void jl_serialize_value_(jl_serializer_state *s, jl_value_t *v)
{
    ...
    assert(item < ((uintptr_t)1 << RELOC_TAG_OFFSET) &&
...
static uintptr_t _backref_id(jl_serializer_state *s, jl_value_t *v)
{
    ...
    if (jl_is_symbol(v)) {
        ...
        if (idx == HT_NOTFOUND) {
            ...
            assert(offset < ((uintptr_t)1 << RELOC_TAG_OFFSET) &&
                   "too many symbols");
    ...
    if (idx == HT_NOTFOUND) {
        ...
        assert(idx != HT_NOTFOUND &&
               "object missed during jl_serialize_value pass");
    }
...
static void
jl_write_module(jl_serializer_state *s, uintptr_t item, jl_module_t *m)
{
    ...
    arraylist_push(
            &s->relocs_list,
            (void *)(reloc_offset + offsetof(jl_module_t, name)));
    arraylist_push(&s->relocs_list, (void *)backref_id(s, m->name));
    newm->parent = NULL;
    arraylist_push(
            &s->relocs_list,
            (void *)(reloc_offset + offsetof(jl_module_t, parent)));
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->owner == m || m != jl_main_module) {
                write_gctaggedfield(
                        s, (uintptr_t)BindingRef << RELOC_TAG_OFFSET);
                tot += sizeof(void *);
                size_t binding_reloc_offset = ios_pos(s->s);
                record_gvar(
                        s,
                        jl_get_llvm_gv((jl_value_t *)b),
                        ((uintptr_t)DataRef << RELOC_TAG_OFFSET) +
                                binding_reloc_offset);
                ...
                size_t flag_offset =
                        offsetof(jl_binding_t, owner) + sizeof(b->owner);
                ios_write(
                        s->s,
                        (char *)b + flag_offset,
                        sizeof(*b) - flag_offset);
    ...
    if (m == jl_main_module) {
        ...
        arraylist_push(
                &s->relocs_list,
                (void *)(reloc_offset + offsetof(jl_module_t, usings._space[0])));
        ...
        if (newm->usings.items == &newm->usings._space[0]) {
            ...
            for (i = 0; i < m->usings.len; i++) {
                ...
                arraylist_push(
                        &s->relocs_list,
                        (void *)backref_id(s, m->usings._space[i]));
            }
        }
        else {
...
#define JL_ARRAY_ALIGN(jl_value, nbytes) LLT_ALIGN(jl_value, nbytes)
            ...
            size_t tsz = JL_ARRAY_ALIGN(
                    sizeof(jl_array_t) + ndimwords * sizeof(size_t),
            ...
            if (!ar->flags.ptrarray) {
                ...
                arraylist_push(
                        &s->relocs_list,
                        (void *)(reloc_offset + offsetof(jl_array_t, data))); // relocation location
                arraylist_push(
                        &s->relocs_list,
                        (void *)(((uintptr_t)ConstDataRef << RELOC_TAG_OFFSET) + data)); // relocation target
                if (ar->elsize == 1)
                    tot += 1;
                ...
                arraylist_push(
                        &s->relocs_list,
                        (void *)(((uintptr_t)DataRef << RELOC_TAG_OFFSET) + item)); // relocation target
                size_t i;
                for (i = 0; i < alen; i++) {
            ...
            write_uint8(
                    s->s,
                    '\0'); // null-terminated strings for easier C-compatibility
        }
            ...
            for (i = 0; i < nf; i++) {
                ...
                if (jl_field_isptr(t, i) > 0) {
                    write_pointerfield(s, *(jl_value_t **)slot);
                        ...
                        t->mutabl && jl_is_cpointer_type(jl_field_type(t, i))) {
                    write_pointer(s->s);
                }
                else if (fsz > 0) {
            ...
            if (jl_is_method(v)) {
                ...
                jl_method_instance_t *newm =
                        (jl_method_instance_t *)&s->s->buf[reloc_offset];
                newm->fptr = NULL;
                ...
                if (jl_is_method(m->def.method)) {
                    ...
                    if (m->jlcall_api == 2) {
                        ...
                        int jlcall_api = jl_jlcall_api(
                                m->functionObjectsDecls.functionObject);
                        assert(jlcall_api);
                        newm->jlcall_api = jlcall_api;
                        // save functionObject pointers
                        int cfunc = jl_assign_functionID(
                                m->functionObjectsDecls.specFunctionObject);
                        int func = jl_assign_functionID(
                                m->functionObjectsDecls.functionObject);
                        assert(reloc_offset < INT32_MAX);
                        if (cfunc != 0) {
                            ios_ensureroom(
                                    s->fptr_record, cfunc * sizeof(void *));
                            ios_seek(
                                    s->fptr_record,
                                    (cfunc - 1) * sizeof(void *));
                        ...
                        if (func != 0) {
                            ios_ensureroom(
                                    s->fptr_record, func * sizeof(void *));
                            ios_seek(
                                    s->fptr_record,
                                    (func - 1) * sizeof(void *));
                            write_uint32(s->fptr_record, reloc_offset);
                ...
                jl_datatype_t *newdt =
                        (jl_datatype_t *)&s->s->buf[reloc_offset];
                newdt->struct_decl = NULL;
                newdt->ditype = NULL;
                if (dt->layout != NULL) {
                    size_t nf = dt->layout->nfields;
                    size_t fieldsize =
                            jl_fielddesc_size(dt->layout->fielddesc_type);
                    int has_padding = dt->layout->npointers && nf;
                    char *flddesc = (char *)dt->layout;
                    size_t fldsize =
                            sizeof(jl_datatype_layout_t) + nf * fieldsize;
                    uintptr_t layout_unaligned =
                            LLT_ALIGN(ios_pos(s->const_data), sizeof(uint32_t));
                    uintptr_t layout =
                            LLT_ALIGN(ios_pos(s->const_data), sizeof(void *));
                    if (has_padding) {
                        ...
                        write_padding(
                                s->const_data,
                                layout_unaligned -
                                        ios_pos(s->const_data)); // realign
                                                                 // stream
                    }
                    else {
                        write_padding(
                                s->const_data,
                                layout - ios_pos(s->const_data)); // realign
                                                                  // stream
                    }
                    ...
                    arraylist_push(
                            &s->relocs_list,
                            (void *)(reloc_offset + offsetof(jl_datatype_t, layout))); // relocation location
                    arraylist_push(
                            &s->relocs_list,
                            (void *)(((uintptr_t)ConstDataRef << RELOC_TAG_OFFSET) + layout)); // relocation target
                    ios_write(s->const_data, flddesc, fldsize);
                }
            }
            else if (jl_typeof(v) == jl_idtable_type) {
                // will need to rehash this, later (after types are fully
                // constructed)
                arraylist_push(&reinit_list, (void *)item);
...
// just write the item reloc_id directly
#ifndef NDEBUG
        assert(reloc_offset == 0 && "offsets for relocations to builtin "
                                    "objects should be precomposed in the "
                                    "reloc_item");
        ...
        case ConstDataRef:
            break;
        case TagRef:
            assert(offset >= 2 && offset < deser_tag.len &&
                   deser_tag.items[offset] && "corrupt relocation item id");
            break;
        case SymbolRef:
...
static inline uintptr_t get_item_for_reloc(
        jl_serializer_state *s, uintptr_t base, size_t size, uint32_t reloc_id)
{
    ...
    case TagRef:
        assert(offset < deser_tag.len && deser_tag.items[offset] &&
...
static void
jl_write_skiplist(ios_t *s, char *base, size_t size, arraylist_t *list)
{
    size_t i;
...
static void jl_update_all_fptrs(jl_serializer_state *s)
{
    ...
    // make these NULL now so we skip trying to restore GlobalVariable pointers
    // later
    ...
    jl_method_instance_t **linfos =
            (jl_method_instance_t **)&s->fptr_record->buf[0];
    for (i = 0; i < sysimg_fvars_max; i++) {
        ...
        if (offset != 0) {
            ...
            if (fvars_base == NULL) {
                ...
                assert(jl_is_method(li->def.method) && li->jlcall_api &&
                       li->jlcall_api != 2);
                linfos[i] = li;
                jl_fptr_to_llvm(
                        (jl_fptr_t)sysimg_fvars(fvars_base, i), li, cfunc);
            }
        }
    }
    if (fvars_base) {
...
static void jl_finalize_serializer(jl_serializer_state *s)
{
    ...
    if (jl_module_init_order != NULL) {
        ...
        for (i = 0; i < l; i++) {
            ...
            assert(ptrhash_get(
                           &backref_table,
                           jl_array_ptr_ref(jl_module_init_order, i)) !=
                   HT_NOTFOUND);
        }
    }
...
static void jl_reinit_item(jl_value_t *v, int how, arraylist_t *tracee_list)
{
    ...
    JL_TRY {
        ...
        case 2: { // reinsert module v into parent (const)
            ...
            if (b->value != NULL) {
                if (!jl_is_module(b->value)) {
                    jl_errorf(
                            "Invalid redefinition of constant %s.",
                            jl_symbol_name(mod->name)); // this also throws
                }
                if (jl_generating_output() && jl_options.incremental) {
                    jl_errorf(
                            "Cannot replace module %s during incremental "
                            "precompile.",
        ...
        case 3: { // rehash MethodTable
            ...
            // TODO: consider reverting this when we can split on Type{...}
            // better
            jl_typemap_rehash(
                    mt->cache, 1); //(mt == jl_type_typename->mt) ? 0 : 1);
            if (tracee_list)
...
static jl_array_t *
jl_finalize_deserializer(jl_serializer_state *s, arraylist_t *tracee_list)
{
...
static void jl_prune_type_cache(jl_svec_t *cache)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (ptrhash_get(&backref_table, ti) != HT_NOTFOUND ||
            ...
            if (singleton &&
                (ptrhash_get(&backref_table, singleton) != HT_NOTFOUND ||
                 jl_get_llvm_gv(singleton) != 0))
                jl_svecset(cache, ins++, ti);
        }
    }
    if (i > ins) {
...
static void jl_save_system_image_to_stream(ios_t *f)
{
    ...
    if (jl_core_module != NULL) {
        jl_array_t *args =
                (jl_array_t *)jl_get_global(jl_core_module, jl_symbol("ARGS"));
        if (args != NULL) {
    ...
    jl_idtable_type =
            jl_base_module
                    ? jl_get_global(jl_base_module, jl_symbol("ObjectIdDict"))
                    : NULL;
    ...
    { // step 1: record values (recursively) that need to go in the image
        ...
        for (i = 0; i < builtin_typenames.len; i++) {
            jl_serialize_value(
                    &s, ((jl_typename_t *)builtin_typenames.items[i])->cache);
            jl_serialize_value(
                    &s,
                    ((jl_typename_t *)builtin_typenames.items[i])->linearcache);
        }
    }
    ...
    { // step 2: build all the sysimg sections
        ...
        // ensure everything in deser_tag are reassociated with their
        // GlobalValue
        uintptr_t i;
        for (i = 0; i < deser_tag.len; i++) {
            ...
            record_gvar(
                    &s,
                    jl_get_llvm_gv(v),
                    ((uintptr_t)TagRef << RELOC_TAG_OFFSET) + i);
        }
    }
...
// Takes in a path of the form "usr/lib/julia/sys.so" (jl_restore_system_image
// should be passed the same string)
JL_DLLEXPORT void jl_preload_sysimg_so(const char *fname)
{
    ...
    if (!is_ji) // .ji extension => load .ji file only
        jl_set_sysimg_so(
                jl_load_dynamic_library(fname, JL_RTLD_LOCAL | JL_RTLD_NOW));
}
...
JL_DLLEXPORT void jl_set_sysimg_so(void *handle)
{
    ...
    void **jl_RTLD_DEFAULT_handle_pointer =
            (void **)jl_dlsym_e(handle, "jl_RTLD_DEFAULT_handle_pointer");
    if (!jl_RTLD_DEFAULT_handle_pointer ||
        (void *)&jl_RTLD_DEFAULT_handle != *jl_RTLD_DEFAULT_handle_pointer)
        jl_error("System image file failed consistency check: maybe opened the "
                 "wrong version?");
    if (jl_options.cpu_target == NULL)
        jl_options.cpu_target =
                (const char *)jl_dlsym(handle, "jl_sysimg_cpu_target");
...
static void jl_restore_system_image_from_stream(ios_t *f)
{
    ...
    jl_gc_set_permalloc_region(
            (void *)sysimg_base, (void *)(sysimg_base + sysimg.size));
    ...
    jl_core_module =
            (jl_module_t *)jl_get_global(jl_main_module, jl_symbol("Core"));
    jl_base_module =
            (jl_module_t *)jl_get_global(jl_main_module, jl_symbol("Base"));
...
// TODO: need to enforce that the alignment of the buffer is suitable for
// vectors
...
static void jl_init_serializer2(int for_serialize)
{
    ...
    void *tags[] = {
        ...
        jl_array_uint8_type,
        jl_symbol_type->name,
        jl_ssavalue_type->name,
        jl_tuple_typename,
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_ref_type))->name,
        jl_pointer_typename,
        jl_simplevector_type->name,
        jl_datatype_type->name,
        jl_uniontype_type->name,
        jl_array_typename,
        jl_expr_type->name,
        jl_typename_type->name,
        jl_type_typename,
        jl_methtable_type->name,
        jl_typemap_level_type->name,
        jl_typemap_entry_type->name,
        jl_tvar_type->name,
        ((jl_datatype_t *)jl_unwrap_unionall(
                 (jl_value_t *)jl_abstractarray_type))
                ->name,
        ((jl_datatype_t *)jl_unwrap_unionall((jl_value_t *)jl_densearray_type))
                ->name,
        jl_vararg_typename,
        jl_void_type->name,
        jl_method_instance_type->name,
        jl_method_type->name,
        jl_module_type->name,
        jl_function_type->name,
        jl_typedslot_type->name,
        jl_abstractslot_type->name,
        jl_slotnumber_type->name,
        jl_unionall_type->name,
        jl_intrinsic_type->name,
        jl_task_type->name,
        jl_labelnode_type->name,
        jl_linenumbernode_type->name,
        jl_builtin_type->name,
        jl_gotonode_type->name,
        jl_quotenode_type->name,
        jl_globalref_type->name,
        jl_typeofbottom_type->name,
        jl_string_type->name,
        jl_abstractstring_type->name,
...
#include "common_symbols1.inc"
    ...
    for (i = 0; tags[i] != NULL; i++) {
        void *v = tags[i];
        if (!for_serialize) {
            // some builtins are only rooted through a type cache or Main.Core
            // binding,
            // but were allocated young, so we force the gc to change their tag
            // here
    ...
    assert(!for_serialize ||
           ptrhash_get(&sertag_table, ptls->root_task) ==
                   (char *)HT_NOTFOUND +
                           ((uintptr_t)TagRef << RELOC_TAG_OFFSET) + 3);
...
#define NBOX_C 1024
    ...
    if (for_serialize) {
        ...
        while (id_to_fptrs[i] != NULL) {
            ptrhash_put(
                    &fptr_to_id, (void *)(uintptr_t)id_to_fptrs[i], (void *)i);
            i += 1;
        }
    }
    ...
    arraylist_push(
            &builtin_typenames, ((jl_datatype_t *)jl_ref_type->body)->name);
