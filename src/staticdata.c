...
// The sysimage is linked into the same binary as libjulia-internal, so its
// symbols are referenced directly. JL_STATIC_SYSIMAGE_HANDLE denotes that image.
...
// Look up an image symbol, statically for the linked-in sysimage
...
// Canonical path of the binary containing the sysimage, or NULL
...
#ifdef JL_LIBRARY_STATIC
    // Only the linked-in sysimage can be used; reject an explicit -J for a
    // different one (`fname` is already canonical, see jl_resolve_sysimg_location)
