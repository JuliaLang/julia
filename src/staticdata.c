...
static jl_image_buf_t get_image_buf(void *handle, int is_pkgimage)
{
    ...
    if (handle != jl_RTLD_DEFAULT_handle) {
        int symbol_found = jl_dlsym(handle, "get_jl_RTLD_DEFAULT_handle_addr", (void **)&get_jl_RTLD_DEFAULT_handle_addr, 0, 1);
    ...
    if (jl_system_image_size == 0 || is_pkgimage) {
        ...
        jl_dlsym(handle, "jl_system_image_size", (void **)&plen, 1, 1);
        jl_dlsym(handle, "jl_system_image_data", (void **)&data, 1, 1);
        jl_dlsym(handle, "jl_image_pointers", (void**)&pointers, 1, 1);
    } else {
