                    ...
                    if (isboxed || (jl_is_datatype(ety) && ((jl_datatype_t*)ety)->layout->npointers > 0)) { // if elements are just bits, don't need a write barrier
                        ...
                        auto is_owned = [&]() {
                            ...
                            if (auto call = dyn_cast<CallInst>(aryv)) {
                                if (jl_alloc::getArrayAllocInfo(info, call)) {
                                    return static_cast<llvm::Value*>(ConstantInt::getFalse(ctx.builder.getContext()));
                                }
                            }
                            ...
                            return ctx.builder.CreateICmpEQ(flags, ConstantInt::get(getInt16Ty(ctx.builder.getContext()), 3));
                        }();
