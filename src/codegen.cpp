...
// returns true if the call has been handled
{
    ...
    //     }
    //     jl_value_t *buf_dt = jl_unwrap_unionall(buf.typ);
    //     if (jl_is_buffer_type(aty_dt) && indices_ok) {
    //         jl_value_t *ety = jl_tparam0(buf_dt);
    //         if (!jl_has_free_typevars(ety)) {
    ...
    //             Value *idx = emit_buffer_index(ctx, buf, buf_ex, idx, boundscheck);
    //             if (!isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
    //                 // no-op
    //             }
    //             else {
    ...
    //                                     emit_bitcast(ctx, decay_derived(ctx, bufv), ctx.types().T_pprjlvalue),
    ...
    //                     Value *offset = emit_arrayoffset(ctx, buf, nd);
    ...
    //                     }
    //                     else {
    //                         Value *ndims = (nd == -1 ? emit_arrayndims(ctx, buf) : ConstantInt::get(getInt16Ty(ctx.builder.getContext()), nd));
