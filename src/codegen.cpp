        ...
        *ret = jl_cgval_t();
        ...
        if (jl_is_buffer_kind_type(buf_dt) && indices_ok) {
            ...
            if (!jl_has_free_typevars(ety)) {
                ...
                if (!lyt.isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
                    ...
                    *ret = ghostValue(ctx, ety);
                ...
                else if (!lyt.isboxed && jl_is_uniontype(ety)) {
                    ...
                    if (lyt.elsize == 0) {
                        ...
                        Type *AT = ArrayType::get(IntegerType::get(ctx.builder.getContext(), 8 * lyt.alignment), (lyt.elsize + lyt.alignment - 1) / lyt.alignment);
                    ...
                    *ret = emit_unionload(ctx, data, ptindex, ety, lyt.elsize, lyt.alignment, ctx.tbaa().tbaa_arraybuf, true, lyt.ntags, ctx.tbaa().tbaa_bufferselbyte);
                }
                else {
                    ...
                    *ret = typed_load(ctx,
                            ...
                            i, ety,
                            lyt.isboxed ? ctx.tbaa().tbaa_ptrarraybuf : ctx.tbaa().tbaa_arraybuf,
                            aliasscope,
                            lyt.isboxed,
                            AtomicOrdering::NotAtomic);
                }
        ...
        if ((jl_is_buffer_type(buf_dt) || jl_is_dynbuffer_type(buf_dt)) && indices_ok) {
            ...
            if (!jl_has_free_typevars(ety)) {
                ...
                if (!lyt.isboxed && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
                    ...
                    if (lyt.isboxed || (jl_is_datatype(ety) && ((jl_datatype_t*)ety)->layout->npointers > 0)) { // if elements are just bits, don't need a write barrier
                    ...
                    if (!lyt.isboxed && jl_is_uniontype(ety)) {
                        Type *AT = ArrayType::get(IntegerType::get(ctx.builder.getContext(), 8 * lyt.alignment), (lyt.elsize + lyt.alignment - 1) / lyt.alignment);
                        Value *data = emit_bitcast(ctx, emit_bufferptr(ctx, buf, buf_ex), AT->getPointerTo());
                        ...
                        ptindex = ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx.builder.getContext()), ptindex, ConstantInt::get(getInt32Ty(ctx.builder.getContext()), 0));
                        ptindex = ctx.builder.CreateInBoundsGEP(getInt8Ty(ctx.builder.getContext()), ptindex, i);
                        jl_aliasinfo_t ai = jl_aliasinfo_t::fromTBAA(ctx, ctx.tbaa().tbaa_bufferselbyte);
                        ai.decorateInst(ctx.builder.CreateStore(tindex, ptindex));
                        if (lyt.elsize > 0 && (!jl_is_datatype(val.typ) || jl_datatype_size(val.typ) > 0)) {
                            // copy data (if any)
                        ...
                        typed_store(ctx,
                                    emit_bufferptr(ctx, buf, buf_ex, lyt.isboxed),
                                    i, val, jl_cgval_t(), ety,
                                    lyt.isboxed ? ctx.tbaa().tbaa_ptrarraybuf : ctx.tbaa().tbaa_arraybuf,
                                    ctx.noalias().aliasscope.current,
                                    data_owner,
                                    lyt.isboxed,
                                    lyt.isboxed ? AtomicOrdering::Release : AtomicOrdering::NotAtomic, // TODO: we should do this for anything with CountTrackedPointers(elty).count > 0
