                            ...
                            ArrayRef<jl_cgval_t> argv, size_t nargs, const jl_cgval_t *modifyop) JL_CANSAFEPOINT
{
    ...
    if (op == StoreKind::Unset) {
        ...
        // Name every reference being cleared, including references after scalar
        // fields in inline elements. `ptr` already skips any lock prefix.
        Value *null_child = Constant::getNullValue(ctx.types().T_prjlvalue);
        if (isboxed) {
            emit_write_barrier(ctx, mem, ptr, ArrayRef<Value*>(null_child));
        }
        else {
