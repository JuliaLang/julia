...
public:
    ...
    std::vector<std::tuple<jl_cgval_t, AllocaInst *, PHINode *, jl_value_t *>> PhiNodes;
...
static void emit_phinode_assign(jl_codectx_t &ctx, ssize_t idx, jl_value_t *r)
{
    ...
    if (jl_is_uniontype(phiType)) {
        ...
        if (dest) {
            ...
            val.Vboxed = ptr_phi;
            ctx.PhiNodes.push_back(std::make_tuple(val, dest, ptr_phi, r));
            ...
            ctx.PhiNodes.push_back(std::make_tuple(val, dest, (PHINode*)NULL, r));
            ctx.SAvalues.at(idx) = val;
            ctx.ssavalue_assigned.at(idx) = true;
            return;
        }
    ...
    ctx.PhiNodes.push_back(std::make_tuple(slot, dest, value_phi, r));
    ...
    */
    ...
    auto undef_value_for_type = [&](jl_value_t *phiType, Type *UndefType) {
        ...
        if (UndefType == T_prjlvalue) {
            ...
            llvm::IRBuilderBase::InsertPoint IP = ctx.builder.saveIP();
            ctx.builder.SetInsertPoint(ctx.ptlsStates);
    ...
    for (auto &tup : ctx.PhiNodes) {
        ...
        jl_value_t *r;
        AllocaInst *dest;
        std::tie(phi_result, dest, VN, r) = tup;
