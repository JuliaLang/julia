...
//       see
//       https://github.com/JuliaLang/julia/pull/11644#issuecomment-112276813
// also, use ELF because RuntimeDyld COFF I686 support didn't exist
// also, use ELF because RuntimeDyld COFF X86_64 doesn't seem to work (fails to
...
JL_DLLEXPORT void __stack_chk_fail()
{
    ...
    abort(); // end with abort, since the compiler destroyed the stack upon
             // entry to this function, there's no going back now
}
#endif
...
// type-based alias analysis nodes.  Indentation of comments indicates
// hierarchy.
/* clang-format off */
...
extern "C" int32_t jl_jlcall_api(const void *function)
{
    ...
    assert(Name.startswith("japi1_") ||  // jlcall abi 1 from JIT
           ...
           Name.startswith("jlcall_") || // jlcall abi 1 from JIT wrapping a
                                         // specsig method
           Name.startswith("jlsysw_"));  // jlcall abi 1 from sysimg wrapping a
                                         // specsig method
    return 1;
}
...
static bool isbits_spec(jl_value_t *jt, bool allow_singleton = true)
{
    return jl_isbits(jt) && jl_is_leaf_type(jt) &&
           (allow_singleton || (jl_datatype_size(jt) > 0) ||
            (jl_datatype_nfields(jt) > 0));
}
...
struct jl_cgval_t {
    Value *V; // may be of type T* or T, or set to NULL if ghost (or if the
              // value has not been initialized yet, for a variable definition)
    Value *TIndex; // if `V` is an unboxed (tagged) Union described by `typ`,
                   // this gives the DataType index (1-based, small int) as an
                   // i8
    ...
    bool isboxed; // whether this value is a jl_value_t* allocated on the heap
                  // with the right type tag
    bool isghost; // whether this value is "ghost"
    bool isimmutable; // V points to something that is definitely immutable
                      // (e.g. single-assignment, but including memory)
    ...
    // bool isvalue() const
    //{
    //    // whether this value is compatible with loading into registers
    //    (`emit_unbox` without an explicit type)
    ...
    jl_cgval_t(
        ...
        :       // general constructor (with pointer type auto-detect)
          V(V), // V is allowed to be NULL in a jl_varinfo_t context, but not
                // during codegen contexts
          TIndex(tindex), constant(NULL), gcroot(gcroot), typ(typ),
          isboxed(isboxed), isghost(false),
    ...
    jl_cgval_t(jl_value_t *typ)
        : // ghost value constructor
          V(NULL),
          TIndex(NULL), constant(((jl_datatype_t *)typ)->instance),
          gcroot(NULL), typ(typ), isboxed(false), isghost(true),
          isimmutable(true), tbaa(nullptr)
    {
    ...
    jl_cgval_t(const jl_cgval_t &v, jl_value_t *typ, Value *tindex)
        : // copy constructor with new type
          V(v.V),
          TIndex(tindex), constant(v.constant), gcroot(v.gcroot), typ(typ),
          isboxed(v.isboxed), isghost(v.isghost), isimmutable(v.isimmutable),
          tbaa(v.tbaa)
    {
    ...
    jl_cgval_t()
        : // undef / unreachable / default constructor
          ...
          TIndex(NULL), constant(NULL), gcroot(NULL), typ(jl_bottom_type),
          isboxed(false), isghost(true), isimmutable(true), tbaa(nullptr)
    {
    }
};
...
struct jl_varinfo_t {
    Instruction *boxroot; // an address, if the var might be in a jl_value_t**
                          // stack slot (marked tbaa_const, if appropriate)
    jl_cgval_t value;     // a stack slot or constant value
    Value *pTIndex;       // i8* stack slot for the value.TIndex tag describing
                          // `value.V`
    ...
    jl_varinfo_t()
        : boxroot(NULL), value(jl_cgval_t()), pTIndex(NULL), dinfo(NULL),
          defFlag(NULL), isSA(false), isVolatile(false), isArgument(false),
...
static jl_returninfo_t get_specsig_function(
        Module *M,
...
static Value *global_binding_pointer(
        ...
        bool assign,
        jl_codectx_t *ctx);
static jl_cgval_t emit_checked_var(
        Value *bp, jl_sym_t *name, jl_codectx_t *ctx, bool isvol, MDNode *tbaa);
static jl_cgval_t emit_sparam(size_t i, jl_codectx_t *ctx);
static Value *emit_condition(
        const jl_cgval_t &condV, const std::string &msg, jl_codectx_t *ctx);
static void allocate_gc_frame(BasicBlock *b0, jl_codectx_t *ctx);
static GlobalVariable *
prepare_global(GlobalVariable *G, Module *M = jl_builderModule);
...
static GlobalVariable *
get_pointer_to_constant(Constant *val, StringRef name, Module &M)
{
    GlobalVariable *gv = new GlobalVariable(
            M, val->getType(), true, GlobalVariable::PrivateLinkage, val, name);
    gv->setUnnamedAddr(GlobalValue::UnnamedAddr::Global);
    return gv;
}

static AllocaInst *
emit_static_alloca(Type *lty, int arraysize, jl_codectx_t *ctx)
{
#if JL_LLVM_VERSION >= 50000
    return new AllocaInst(
            lty,
            0,
            ConstantInt::get(T_int32, arraysize),
            "",
...
static AllocaInst *emit_static_alloca(Type *lty)
{
    ...
    return new AllocaInst(
            ...
            0,
            "",
            /*InsertBefore=*/&*builder.GetInsertBlock()
                    ->getParent()
                    ->getEntryBlock()
                    .getFirstInsertionPt());
#else
...
static inline jl_cgval_t ghostValue(jl_value_t *typ)
{
    ...
    if (jl_is_type_type(typ)) {
        // replace T::Type{T} with T, by assuming that T must be a leaftype of
        // some sort
...
static inline jl_cgval_t
mark_julia_slot(Value *v, jl_value_t *typ, Value *tindex, MDNode *tbaa)
{
...
static inline jl_cgval_t mark_julia_type(
        Value *v,
        bool isboxed,
        jl_value_t *typ,
        jl_codectx_t *ctx,
...
static inline jl_cgval_t
update_julia_type(const jl_cgval_t &v, jl_value_t *typ, jl_codectx_t *ctx)
{
    ...
    if (jl_is_leaf_type(v.typ) && !jl_is_kind(v.typ)) {
        if (jl_is_leaf_type(typ) && !jl_is_kind(typ) &&
            !((jl_datatype_t *)typ)->abstract &&
            !((jl_datatype_t *)v.typ)->abstract) {
...
{
    assert((!vi.boxroot || vi.pTIndex) &&
           "undef check is null pointer for boxed things");
    assert(vi.usedUndef && vi.defFlag && "undef flag codegen corrupted");
    builder.CreateStore(
            ConstantInt::get(T_int1, val), vi.defFlag, vi.isVolatile);
}
...
static void CreateTrap(IRBuilder<> &builder)
{
    ...
    BasicBlock *newBB =
            BasicBlock::Create(builder.getContext(), "after_noret", f);
...
// given a value marked with type `v.typ`, compute the mapping and/or boxing to
// return a value of type `typ`
static jl_cgval_t convert_julia_type(
        const jl_cgval_t &v,
        jl_value_t *typ,
        jl_codectx_t *ctx,
        bool needsroot = true)
{
    ...
    if (jl_is_leaf_type(typ)) {
        ...
        if (v.TIndex) {
            ...
            Value *tindex =
                    builder.CreateAnd(v.TIndex, ConstantInt::get(T_int8, 0x7f));
                ...
                for_each_uniontype_small(
                        ...
                        [&](unsigned idx, jl_datatype_t *jt) {
                            ...
                            if (new_idx) {
                                ...
                                new_tindex = builder.CreateSelect(
                                        cmp,
                                        ConstantInt::get(T_int8, new_idx),
                                        new_tindex);
                                t = true;
                            }
                                ...
                                // since it appeared as a leaftype in the
                                // original type
            ...
            if (!isa<Constant>(new_tindex)) {
                ...
                if (v.ispointer() && v.V && !isa<AllocaInst>(v.V)) {
                    wasboxed = builder.CreateAnd(
                            v.TIndex, ConstantInt::get(T_int8, 0x80));
                    new_tindex = builder.CreateOr(wasboxed, new_tindex);
                    wasboxed = builder.CreateICmpNE(
                            wasboxed, ConstantInt::get(T_int8, 0));

                    // may need to handle compute_box_tindex for some of the
                    // values
                    ...
                    for_each_uniontype_small(
                            ...
                            [&](unsigned idx, jl_datatype_t *jt) {
                                ...
                                if (old_idx == 0) {
                                    if (!union_box_dt) {
                                        BasicBlock *isaBB = BasicBlock::Create(
                                                jl_LLVMContext,
                                                "union_isa",
                                                ctx->f);
                                    ...
                                    // didn't handle this item before, select
                                    // its new union index
                                    Value *cmp = builder.CreateICmpEQ(
                                            maybe_decay_untracked(
                                                    literal_pointer_val(
                                                            (jl_value_t *)jt)),
                                            union_box_dt);
                                    union_box_tindex = builder.CreateSelect(
                                            cmp,
                                            ConstantInt::get(
                                                    T_int8, 0x80 | idx),
                                            union_box_tindex);
                    ...
                    if (union_box_dt) {
                        ...
                        BasicBlock *postBB = BasicBlock::Create(
                                jl_LLVMContext, "post_union_isa", ctx->f);
                        builder.CreateBr(postBB);
                        builder.SetInsertPoint(currBB);
                        Value *wasunknown = builder.CreateICmpEQ(
                                v.TIndex, ConstantInt::get(T_int8, 0x80));
                ...
                if (!skip_box.all()) {
                    ...
                    // box them now (tindex above already selected 0x80 = box
                    // for them)
                    // root the result, and return a new mark_julia_slot over
                    // the result
                    ...
                    if (needsroot) {
                        ...
                        froot = emit_local_root(ctx);
                        Value *newroot = boxv;
                        if (wasboxed || v.gcroot) { // oldbox might be all ghost
                                                    // values (which don't need
                                                    // roots)
                            // store either the old box or the new box into the
                            // gc-root (skip_box ensures these are
                            // mutually-exclusive)
                            // need to clone the value from `v.gcroot` if this
                            // isn't a new box
                            Value *oldroot;
                            if (v.gcroot)
                                ...
                                oldroot = v.V;
                            newroot = builder.CreateSelect(
                                    wasboxed,
                                    emit_bitcast(oldroot, boxv->getType()),
                                    newroot);
                        }
                    ...
                    if (v.V == NULL) {
                        ...
                        slotv = builder.CreateSelect(
                                isboxv,
                                decay_derived(boxv),
                                emit_bitcast(slotv, boxv->getType()));
                        jl_cgval_t newv = jl_cgval_t(
                                slotv, froot, false, typ, new_tindex);
                        newv.tbaa = tbaa;
            ...
            // previous value was unboxed (leaftype), statically compute union
            // tindex
            ...
            if (new_idx) {
                ...
                if (v.V && !v.ispointer()) {
                    // TODO: remove this branch once all consumers of v.TIndex
                    // understand how to handle a non-ispointer value
                    Value *slotv = emit_static_alloca(v.V->getType());
                    builder.CreateStore(v.V, slotv);
                    jl_cgval_t newv =
                            jl_cgval_t(slotv, NULL, false, typ, new_tindex);
...
extern "C" jl_llvm_functions_t jl_compile_linfo(
        ...
        const jl_cgparams_t *params)
{
    ...
    if (params != &jl_default_cgparams /* fast path */ &&
        ...
        jl_error("functions compiled with custom codegen params mustn't be "
                 "cached");
    ...
    // Codegen lock held in this block
    {
        ...
        if (!jl_is_method(li->def.method)) {
            ...
            if (decls.functionObject != NULL || !src || !jl_is_code_info(src) ||
                li->jlcall_api == 2) {
                goto locked_out;
            }
        }
            ...
            // try to infer it for ourself, but first, re-check if it's already
            // compiled.
        ...
        JL_CATCH {
            // something failed! this is very bad, since other WIP may be
            // pointing to this function
            // but there's not much we can do now. try to clear much of the WIP
            // anyways.
            ...
            const char *mname = jl_symbol_name(
                    jl_is_method(li->def.method) ? li->def.method->name
                                                 : anonymous_sym);
        ...
        if (JL_HOOK_TEST(params, module_activation)) {
            JL_HOOK_CALL(
                    ...
                    module_activation,
                    1,
                    jl_box_voidpointer(wrap(m.release())));
        }
        else {
            ...
            if (!toplevel) {
                ...
                // and the runtime doesn't notify us when the code becomes
                // unreachable :(
        ...
        if (JL_DELETE_NON_INLINEABLE &&
            ...
            (li->jlcall_api == 2 ||
             !jl_ast_flag_inlineable((jl_array_t *)li->inferred)) &&
    ...
    if (dump_compiles_stream != NULL) {
        ...
        jl_printf(
                dump_compiles_stream, "%" PRIu64 "\t\"", this_time - last_time);
...
static void
jl_setup_module(Module *m, const jl_cgparams_t *params = &jl_default_cgparams)
{
...
extern "C" JL_DLLEXPORT uint64_t jl_get_llvm_fptr(llvm::Function *llvmf)
{
    uint64_t addr = getAddressForFunction(llvmf);
    if (!addr)
        addr = jl_ExecutionEngine->findUnmangledSymbol(llvmf->getName())
                       .getAddress();
    return addr;
}
...
static jl_method_instance_t *jl_get_unspecialized(jl_method_instance_t *method)
{
    ...
    if (def->unspecialized == NULL) {
        ...
        if (def->unspecialized == NULL) {
            def->unspecialized =
                    jl_get_specialized(def, def->sig, jl_emptysvec);
...
extern "C" jl_generic_fptr_t
jl_generate_fptr(jl_method_instance_t *li, void *_F, size_t world)
{
    ...
    if (jl_is_method(li->def.method)) {
        ...
        if (!F || !jl_can_finalize_function(F)) {
            ...
            if (!unspec)
                unspec = jl_get_unspecialized(li); // get-or-create the
                                                   // unspecialized version to
                                                   // cache the result
            jl_code_info_t *src =
                    unspec->def.method->isstaged
                            ? jl_code_for_staged(unspec)
                            : (jl_code_info_t *)unspec->def.method->source;
            ...
            if (unspec == li) {
                // temporarily clear the decls so that it will compile our
                // unspec version of src
                unspec->functionObjectsDecls.functionObject = NULL;
                unspec->functionObjectsDecls.specFunctionObject = NULL;
            }
            assert(src);
            F = (Function *)jl_compile_linfo(
                        &unspec, src, unspec->min_world, &jl_default_cgparams)
                        .functionObject; // this does not change unspec
            if (unspec == li) {
...
static Function *
jl_cfunction_object(jl_function_t *f, jl_value_t *rt, jl_tupletype_t *argt);
// get the address of a C-callable entry point for a function
extern "C" JL_DLLEXPORT void *
jl_function_ptr(jl_function_t *f, jl_value_t *rt, jl_value_t *argt)
{
    JL_GC_PUSH1(&argt);
    if (jl_is_tuple(argt)) {
        // TODO: maybe deprecation warning, better checking
        argt = (jl_value_t *)jl_apply_tuple_type_v(
                (jl_value_t **)jl_data_ptr(argt), jl_nfields(argt));
    }
    JL_LOCK(&codegen_lock);
...
extern "C" JL_DLLEXPORT void *jl_function_ptr_by_llvm_name(char *name)
{
...
// export a C-callable entry point for a function (dllexport'ed dlsym), with a
// given name
extern "C" JL_DLLEXPORT void
jl_extern_c(jl_function_t *f, jl_value_t *rt, jl_value_t *argt, char *name)
{
    assert(jl_is_tuple_type(argt));
    JL_LOCK(&codegen_lock);
    Function *llvmf = jl_cfunction_object(f, rt, (jl_tupletype_t *)argt);
    // force eager emission of the function (llvm 3.3 gets confused otherwise
    // and tries to do recursive compilation)
    uint64_t Addr = getAddressForFunction(llvmf);
    ...
    GlobalAlias *GA = GlobalAlias::create(
            llvmf->getType()->getElementType(),
            llvmf->getType()->getAddressSpace(),
...
// this is paired with jl_dump_function_ir and jl_dump_function_asm in
// particular ways:
// misuse will leak memory or cause read-after-free
extern "C" JL_DLLEXPORT void *jl_get_llvmf_defn(
        jl_method_instance_t *linfo,
        size_t world,
        bool getwrapper,
        bool optimize,
        const jl_cgparams_t params)
{
    ...
    if (!src || (jl_value_t *)src == jl_nothing) {
        ...
        if (!src && jl_is_method(linfo->def.method))
            src = linfo->def.method->isstaged
                          ? jl_code_for_staged(linfo)
                          : (jl_code_info_t *)linfo->def.method->source;
    }
    if ((jl_value_t *)src == jl_nothing)
        src = NULL;
    ...
    JL_CATCH {
        ...
        JL_UNLOCK(&codegen_lock); // Might GC
        const char *mname = jl_symbol_name(
                jl_is_method(linfo->def.method) ? linfo->def.method->name
                                                : anonymous_sym);
        jl_rethrow_with_add("error compiling %s", mname);
    }
    ...
    Function *specf_decl =
            (Function *)linfo->functionObjectsDecls.specFunctionObject;
    if (specf_decl) {
...
extern "C" JL_DLLEXPORT void *jl_get_llvmf_decl(
        jl_method_instance_t *linfo,
        size_t world,
        bool getwrapper,
        const jl_cgparams_t params)
{
    ...
    if (decls.functionObject == NULL && linfo->jlcall_api == 2 &&
        jl_is_method(linfo->def.method)) {
        // normally we don't generate native code for these functions, so need
        // an exception here
        // This leaks a bit of memory to cache native code that we'll never
        // actually need
        JL_LOCK(&codegen_lock);
...
extern "C" JL_DLLEXPORT const jl_value_t *
jl_dump_function_ir(void *f, bool strip_ir_metadata, bool dump_module)
{
    std::string code;
    ...
    if (!llvmf || (!llvmf->isDeclaration() && !llvmf->getParent()))
        jl_error("jl_dump_function_ir: Expected Function* in a temporary "
                 "Module");
    ...
    if (!llvmf->getParent()) {
        ...
        if (strip_ir_metadata) {
            ...
            for (Module::iterator I = m->begin(), E = m->end(); I != E; ++I) {
                Function *f2 = &*I;
                Function::BasicBlockListType::iterator f2_bb =
                        f2->getBasicBlockList().begin();
                // iterate over all basic blocks in the function
                for (; f2_bb != f2->getBasicBlockList().end(); ++f2_bb) {
                    BasicBlock::InstListType::iterator f2_il =
                            (*f2_bb).getInstList().begin();
                    ...
                    for (; f2_il != (*f2_bb).getInstList().end();) {
                        ...
                        SmallVector<std::pair<unsigned, MDNode *>, 4>::iterator
                                md_iter = MDForInst.begin();

                        // iterate over all metadata kinds and set to NULL to
                        // remove
...
static uint64_t
compute_obj_symsize(const object::ObjectFile *obj, uint64_t offset)
{
    // Scan the object file for the closest symbols above and below offset in
    // the .text section
    uint64_t lo = 0;
    uint64_t hi = 0;
    bool setlo = false;
...
extern "C" JL_DLLEXPORT const jl_value_t *
jl_dump_function_asm(void *f, int raw_mc, const char *asm_variant = "att")
{
    jl_ptls_t ptls = jl_get_ptls_states();
    std::string code;
    llvm::raw_string_ostream stream(code);
    ...
    assert(fptr != 0);
    if (!jl_DI_for_fptr(
                fptr, &symsize, &slide, &section_slide, &object, &context)) {
    ...
    jl_dump_asm_internal(
            fptr, symsize, slide, object, objcontext, stream, asm_variant);
...
const int logdata_blocksize = 32; // target getting nearby lines in the same
                                  // general cache area and reducing calls to
                                  // malloc by chunking
typedef uint64_t logdata_block[logdata_blocksize];
typedef StringMap<std::vector<logdata_block *>> logdata_t;

static void visitLine(
        std::vector<logdata_block *> &vec,
        int line,
        Value *addend,
        const char *name)
{
    ...
    Value *v = ConstantExpr::getIntToPtr(
            ConstantInt::get(T_size, (uintptr_t)&data[line]), T_pint64);
    builder.CreateStore(
            builder.CreateAdd(builder.CreateLoad(v, true, name), addend),
            v,
            true); // not atomic, so this might be an underestimate,
...
static void coverageVisitLine(StringRef filename, int line)
{
    ...
    if (filename == "" || filename == "none" || filename == "no file" ||
        ...
        return;
    visitLine(
            coverageData[filename], line, ConstantInt::get(T_int64, 1), "lcnt");
}
        ...
        int allow_alloc = true)
{
    ...
    if (jl_is_expr(ex)) {
        ...
        if (e->head == call_sym) {
            jl_value_t *f =
                    static_eval(jl_exprarg(e, 0), ctx, sparams, allow_alloc);
            if (f) {
                if (jl_array_dim0(e->args) == 3 && f == jl_builtin_getfield) {
                    m = (jl_module_t *)static_eval(
                            jl_exprarg(e, 1), ctx, sparams, allow_alloc);
                    // Check the tag before evaluating `s` so that a value of
                    // random
                    // type won't be corrupted.
                    if (!m || !jl_is_module(m))
                        return NULL;
                    // Assumes that the module is rooted somewhere.
                    s = (jl_sym_t *)static_eval(
                            jl_exprarg(e, 2), ctx, sparams, allow_alloc);
                    if (s && jl_is_symbol(s)) {
                    ...
                    // here we know we're calling specific builtin functions
                    // that work in world 1.
                    jl_get_ptls_states()->world_age = 1;
                    jl_value_t *result =
                            jl_apply_with_saved_exception_state(v, n + 1, 1);
...
static std::set<int>
assigned_in_try(jl_array_t *stmts, int s, long l, int *pend)
{
    std::set<int> av;
...
static void
mark_volatile_vars(jl_array_t *stmts, std::vector<jl_varinfo_t> &slots)
{
    size_t slength = jl_array_dim0(stmts);
    for (int i = 0; i < (int)slength; i++) {
        ...
        if (jl_is_expr(st)) {
            if (((jl_expr_t *)st)->head == enter_sym) {
                ...
                std::set<int> as = assigned_in_try(
                        stmts, i + 1, jl_unbox_long(jl_exprarg(st, 0)), &last);
                for (int j = 0; j < (int)slength; j++) {
                    if (j < i || j > last) {
                        std::set<int>::iterator it = as.begin();
                        for (; it != as.end(); it++) {
                            if (local_var_occurs(
                                        jl_array_ptr_ref(stmts, j), *it)) {
...
static void
simple_escape_analysis(jl_value_t *expr, bool esc, jl_codectx_t *ctx)
{
    if (jl_is_expr(expr)) {
        esc = true;
...
{
    Instruction *newroot = new AllocaInst(
            T_prjlvalue, 0, "gcroot", /*InsertBefore*/ ctx->ptlsStates);
    if (vi) {
...
static jl_cgval_t
emit_getfield(jl_value_t *expr, jl_sym_t *name, jl_codectx_t *ctx)
{
    ...
    if (jl_is_module(expr)) {
        ...
        Value *bp = global_binding_pointer(
                (jl_module_t *)expr, name, &bnd, false, ctx);
        ...
        if (bnd && bnd->value != NULL) {
            ...
            return mark_julia_type(
                    tbaa_decorate(tbaa_binding, builder.CreateLoad(bp)),
                    true,
    ...
    jl_cgval_t myargs_array[2] = { emit_expr(expr, ctx),
                                   mark_julia_const((jl_value_t *)name) };
    Value *result = emit_jlcall(
            jlgetfield_func,
            ...
            ctx);
    bool needsgcroot =
            true; // !arg1.isimmutable || !jl_is_leaf_type(arg1.typ) ||
                  // !is_datatype_all_pointers((jl_datatype_t*)arg1.typ); //
                  // TODO: probably want this as a llvm pass
    jl_cgval_t ret = mark_julia_type(
            result,
            true,
            jl_any_type,
            ctx,
            needsgcroot); // (typ will be patched up by caller)
    return ret;
}

static Value *emit_bits_compare(
        const jl_cgval_t &arg1, const jl_cgval_t &arg2, jl_codectx_t *ctx)
{
    ...
    if (at->isVectorTy()) {
        ...
        for (unsigned i = 0; i < l; i++) {
            ...
            fld1 = builder.CreateExtractElement(
                    varg1, ConstantInt::get(T_int32, i)),
            fld2 = builder.CreateExtractElement(
                    varg2, ConstantInt::get(T_int32, i)),
            subAns = emit_bits_compare(
                    mark_julia_type(fld1, false, fldty, ctx),
                    mark_julia_type(fld2, false, fldty, ctx),
                    ctx);
    ...
    if (at->isAggregateType()) { // Struct or Array
        ...
        if (sz > 512 && !((jl_datatype_t *)arg1.typ)->layout->haspadding) {
            Value *answer = builder.CreateCall(
...
// emit code for is (===).
static Value *
emit_f_is(const jl_cgval_t &arg1, const jl_cgval_t &arg2, jl_codectx_t *ctx)
{
    ...
    if (arg1.isghost || arg2.isghost) {
        ...
        if (arg1.TIndex)
            return emit_isa(arg1, rt2, NULL, ctx)
                    .first; // rt2 is a singleton type
        if (arg2.TIndex)
            return emit_isa(arg2, rt1, NULL, ctx)
                    .first; // rt1 is a singleton type
    ...
    if (isbits) { // whether this type is unique'd by value
        ...
        Value *same_type = (typ == rt2) ? emit_isa(arg1, typ, NULL, ctx).first
                                        : emit_isa(arg2, typ, NULL, ctx).first;
        BasicBlock *currBB = builder.GetInsertBlock();
        BasicBlock *isaBB = BasicBlock::Create(jl_LLVMContext, "is", ctx->f);
        BasicBlock *postBB =
                BasicBlock::Create(jl_LLVMContext, "post_is", ctx->f);
        builder.CreateCondBr(same_type, isaBB, postBB);
        builder.SetInsertPoint(isaBB);
        Value *bitcmp = emit_bits_compare(
                jl_cgval_t(arg1, typ, NULL), jl_cgval_t(arg2, typ, NULL), ctx);
        builder.CreateBr(postBB);
        builder.SetInsertPoint(postBB);
    ...
    if (rt1 == (jl_value_t *)jl_sym_type || rt2 == (jl_value_t *)jl_sym_type)
        ptr_comparable = 1;
    if (jl_is_mutable_datatype(rt1) ||
        jl_is_mutable_datatype(rt2)) // excludes abstract types
        ptr_comparable = 1;
    if (jl_subtype(rt1, (jl_value_t *)jl_type_type) ||
        jl_subtype(
                rt2,
                (jl_value_t *)
                        jl_type_type)) // need to use typeseq for datatypes
        ptr_comparable = 0;
    if ((jl_is_type_type(rt1) && jl_is_leaf_type(jl_tparam0(rt1))) ||
        (jl_is_type_type(rt2) &&
         jl_is_leaf_type(jl_tparam0(rt2)))) // can compare leaf types by pointer
        ptr_comparable = 1;
    if (ptr_comparable) {
        Value *varg1 =
                arg1.constant ? literal_pointer_val(arg1.constant) : arg1.V;
        Value *varg2 =
                arg2.constant ? literal_pointer_val(arg2.constant) : arg2.V;
        assert(varg1 && varg2 && (arg1.isboxed || arg1.TIndex) &&
               (arg2.isboxed || arg2.TIndex) &&
               "Only boxed types are valid for pointer comparison.");
        return builder.CreateICmpEQ(decay_derived(varg1), decay_derived(varg2));
    }

    JL_FEAT_REQUIRE(ctx, runtime);
    Value *varg1 = mark_callee_rooted(boxed(arg1, ctx));
    Value *varg2 = mark_callee_rooted(
            boxed(arg2, ctx, false)); // potentially unrooted!
    return builder.CreateTrunc(
            builder.CreateCall(prepare_call(jlegal_func), { varg1, varg2 }),
            T_int1);
}

static bool emit_builtin_call(
        jl_cgval_t *ret,
        jl_value_t *f,
        jl_value_t **args,
        size_t nargs,
    ...
    if (f == jl_builtin_is && nargs == 2) {
        ...
        if (v1.constant) {
            if (v2.constant) {
                *ret = mark_julia_type(
                        ConstantInt::get(
                                T_int8, jl_egal(v1.constant, v2.constant)),
                        false,
                        jl_bool_type,
                        ctx);
                JL_GC_POP();
        ...
        // FIXME: v.typ is roughly equiv. to expr_type, but with typeof(T) ==
        // Type{T} instead of DataType in a few cases
        if (v1.typ == (jl_value_t *)jl_datatype_type)
            v1 = update_julia_type(
                    v1,
                    expr_type(args[1], ctx),
                    ctx); // patch up typ if necessary
        if (v2.typ == (jl_value_t *)jl_datatype_type)
            v2 = update_julia_type(
                    v2,
                    expr_type(args[2], ctx),
                    ctx); // patch up typ if necessary
        // emit comparison test
        Value *ans = emit_f_is(v1, v2, ctx);
        *ret = mark_julia_type(
                builder.CreateZExt(ans, T_int8), false, jl_bool_type, ctx);
        JL_GC_POP();
        return true;
    }
        ...
        *ret = emit_typeof(arg1, ctx);
        ...
        jl_value_t *arg = expr_type(args[1], ctx);
        rt1 = arg;
        jl_value_t *ty = expr_type(args[2], ctx);
        rt2 = ty;
        ...
        if (jl_subtype(ty, (jl_value_t *)jl_type_type)) {
            *ret = emit_expr(args[1], ctx);
            ...
            JL_FEAT_REQUIRE(ctx, runtime);
            builder.CreateCall(
                    prepare_call(jltypeassert_func), { rt_val, rt_ty });
            JL_GC_POP();
            return true;
        }
    }
        ...
        if (jl_is_type_type(rt1) && !jl_has_free_typevars(rt1) &&
            ...
            emit_expr(args[2], ctx);
            *ret = mark_julia_type(
                    ConstantInt::get(T_int8, issub), false, jl_bool_type, ctx);
            JL_GC_POP();
            return true;
        }
    }
            ...
            f == jl_builtin__apply && nargs == 2 && ctx->vaStack &&
            slot_eq(args[2], ctx->vaSlot)) {
        // turn Core._apply(f, Tuple) ==> f(Tuple...) using the jlcall calling
        // convention if Tuple is the vaStack allocation
        Value *theF =
                maybe_decay_untracked(boxed(emit_expr(args[1], ctx), ctx));
        ...
        *ret = mark_julia_type(r, true, expr_type(expr, ctx), ctx);
        ...
        if (ctx->source->inferred) {
            ...
            if (jl_is_tuple_type(rt1) && jl_is_leaf_type(rt1) &&
                nargs == jl_datatype_nfields(rt1)) {
        ...
        Value *arg1 =
                boxed(emit_expr(args[1], ctx), ctx, false); // rooted by throw
        ...
        *ret = jl_cgval_t();
        ...
        jl_value_t *aty = expr_type(args[1], ctx);
        rt1 = aty;
        jl_value_t *ity = expr_type(args[2], ctx);
        rt2 = ity;
        ...
        if (jl_is_array_type(aty) && ity == (jl_value_t *)jl_long_type) {
            ...
            if (jl_is_long(ndp)) {
                ...
                if (jl_is_long(args[2])) {
                    ...
                    if (idx > 0 && idx <= ndims) {
                        *ret = mark_julia_type(
                                emit_arraysize(ary, args[1], idx, ctx),
                                false,
                                jl_long_type,
                                ctx);
                        JL_GC_POP();
                    ...
                    Value *idx =
                            emit_unbox(T_size, emit_expr(args[2], ctx), ity);
                    error_unless(
                            builder.CreateICmpSGT(
                                    idx, ConstantInt::get(T_size, 0)),
                            "arraysize: dimension out of range",
                            ctx);
                    BasicBlock *outBB = BasicBlock::Create(
                            jl_LLVMContext, "outofrange", ctx->f);
                    BasicBlock *inBB =
                            BasicBlock::Create(jl_LLVMContext, "inrange");
                    BasicBlock *ansBB =
                            BasicBlock::Create(jl_LLVMContext, "arraysize");
                    builder.CreateCondBr(
                            builder.CreateICmpSLE(
                                    idx, ConstantInt::get(T_size, ndims)),
                            inBB,
                            outBB);
        ...
        if (jl_is_array_type(aty_dt) && indexes_ok) {
            ...
            if (!jl_has_free_typevars(ety)) { // TODO: jn/foreigncall branch has
                                              // a better predicate
                ...
                if (jl_is_long(ndp) || nargs == 2) {
                    ...
                    Value *idx = emit_array_nd_index(
                            ary, args[1], nd, &args[2], nargs - 1, ctx);
                    if (jl_array_store_unboxed(ety) &&
                        ...
                        *ret = typed_load(
                                emit_arrayptr(ary, args[1], ctx),
                                idx,
                                ety,
                                ctx,
                                jl_array_store_unboxed(ety) ? tbaa_arraybuf
                                                            : tbaa_ptrarraybuf);
                    }
                    JL_GC_POP();
                    return true;
        ...
        jl_value_t *aty = expr_type(args[1], ctx);
        rt1 = aty;
        jl_value_t *vty = expr_type(args[2], ctx);
        rt2 = vty;
        bool indexes_ok = true;
        ...
        if (jl_is_array_type(aty_dt) && indexes_ok) {
            ...
            if (!jl_has_free_typevars(ety) &&
                jl_subtype(vty, ety)) { // TODO: jn/foreigncall branch has a
                                        // better predicate
                ...
                if (jl_is_long(ndp) || nargs == 3) {
                    ...
                    ssize_t nd = jl_is_long(ndp) ? jl_unbox_long(ndp) : -1;
                    Value *idx = emit_array_nd_index(
                            ary, args[1], nd, &args[3], nargs - 2, ctx);
                    if (!isboxed && jl_datatype_size(ety) == 0) {
                        ...
                        PHINode *data_owner = NULL; // owner object against
                                                    // which the write barrier
                                                    // must check
                        if (isboxed) { // if not boxed we don't need a write
                                       // barrier
                            assert(ary.isboxed);
                            Value *aryv =
                                    maybe_decay_untracked(boxed(ary, ctx));
                            Value *flags = emit_arrayflags(ary, ctx);
                            // the owner of the data is ary itself except if
                            // ary->how == 3
                            flags = builder.CreateAnd(flags, 3);
                            Value *is_owned = builder.CreateICmpEQ(
                                    flags, ConstantInt::get(T_int16, 3));
                            BasicBlock *curBB = builder.GetInsertBlock();
                            BasicBlock *ownedBB = BasicBlock::Create(
                                    jl_LLVMContext, "array_owned", ctx->f);
                            BasicBlock *mergeBB = BasicBlock::Create(
                                    jl_LLVMContext, "merge_own", ctx->f);
                            builder.CreateCondBr(is_owned, ownedBB, mergeBB);
                            builder.SetInsertPoint(ownedBB);
                            // load owner pointer
                            Value *own_ptr;
                            if (jl_is_long(ndp)) {
                                own_ptr = tbaa_decorate(
                                        tbaa_const,
                        ...
                        typed_store(
                                emit_arrayptr(ary, args[1], ctx, isboxed),
                                idx,
                                v,
                                ety,
                                ctx,
                                ...
                                data_owner,
                                0,
                                false); // don't need to root the box if we had
                                        // to make one since it's being stored
                                        // in the array immediatly
                    }
                    *ret = ary;
                    JL_GC_POP();
                    return true;
        ...
        if (jl_is_quotenode(args[2]) && jl_is_symbol(jl_fieldref(args[2], 0))) {
            *ret = emit_getfield(
                    args[1], (jl_sym_t *)jl_fieldref(args[2], 0), ctx);
            if (ret->typ ==
                (jl_value_t *)
                        jl_any_type) // improve the type, if known from the expr
        ...
        if (ctx->vaStack && slot_eq(args[1], ctx->vaSlot)) {
            ...
            idx = builder.CreateAdd(
                    idx, ConstantInt::get(T_size, ctx->nReqArgs));
            *ret = mark_julia_type(
                    tbaa_decorate(
                            tbaa_value,
                            builder.CreateLoad(
                                    builder.CreateGEP(ctx->argArray, idx))),
                    /*boxed*/ true,
                    expr_type(expr, ctx),
                    ctx,
                    /*needsgcroot*/ false);
        ...
        if (fldt == (jl_value_t *)jl_long_type && jl_is_datatype(utt) &&
            ((jl_datatype_t *)utt)->layout) {
            if ((jl_is_structtype(utt) || jl_is_tuple_type(utt)) &&
                !jl_subtype((jl_value_t *)jl_module_type, (jl_value_t *)stt)) {
                size_t nfields = jl_datatype_nfields(utt);
                jl_cgval_t strct = emit_expr(args[1], ctx);
                // integer index
                size_t idx;
                if (jl_is_long(args[2]) &&
                    (idx = jl_unbox_long(args[2]) - 1) < nfields) {
                    // known index
                    *ret = emit_getfield_knownidx(
                            strct, idx, (jl_datatype_t *)utt, ctx);
                    JL_GC_POP();
                    return true;
                }
                else {
                    // unknown index
                    Value *vidx = emit_unbox(
                            T_size,
                            emit_expr(args[2], ctx),
                            (jl_value_t *)jl_long_type);
                    if (emit_getfield_unknownidx(
                                ret, strct, vidx, (jl_datatype_t *)utt, ctx)) {
                        if (ret->typ ==
                            (jl_value_t *)jl_any_type) // improve the type, if
                                                       // known from the expr
            ...
            if (jl_is_tuple_type(utt) &&
                is_tupletype_homogeneous(((jl_datatype_t *)utt)->types, true)) {
                // For tuples, we can emit code even if we don't know the exact
                // type (e.g. because we don't know the length). This is
                // possible
                // as long as we know that all elements are of the same (leaf)
                // type.
                jl_cgval_t strct = emit_expr(args[1], ctx);
                if (strct.ispointer()) {
                    ...
                    if (!bounds_check_enabled(ctx)) {
                        vidx = builder.CreateSub(
                                vidx, ConstantInt::get(T_size, 1));
                    }
                    else {
                        vidx = emit_bounds_check(
                                ...
                                (jl_value_t *)stt,
                                vidx,
                                emit_datatype_nfields(
                                        emit_typeof_boxed(strct, ctx)),
                                ctx);
                    }
        ...
        if (jl_is_structtype(uty) && uty != jl_module_type && uty->layout) {
            size_t idx = (size_t)-1;
            if (jl_is_quotenode(args[2]) &&
                jl_is_symbol(jl_fieldref(args[2], 0))) {
                idx = jl_field_index(
                        uty, (jl_sym_t *)jl_fieldref(args[2], 0), 0);
            }
            else if (
                    jl_is_long(args[2]) ||
                    (jl_is_quotenode(args[2]) &&
                     jl_is_long(jl_fieldref(args[2], 0)))) {
                ssize_t i;
                if (jl_is_long(args[2]))
        ...
        if (ctx->vaStack && slot_eq(args[1], ctx->vaSlot)) {
            *ret = mark_julia_type(
                    emit_n_varargs(ctx), false, jl_long_type, ctx);
            JL_GC_POP();
            return true;
        }
        jl_value_t *aty = expr_type(args[1], ctx);
        ...
        if (jl_is_type_type(aty)) {
            ...
            if (jl_is_leaf_type(tp0)) {
                ...
                *ret = mark_julia_type(
                        ConstantInt::get(T_size, jl_datatype_nfields(tp0)),
                        false,
                        jl_long_type,
                        ctx);
                JL_GC_POP();
            ...
            if (rt2 == (jl_value_t *)jl_long_type) {
                ...
                Value *fieldtyp = tbaa_decorate(
                        ...
                        builder.CreateLoad(builder.CreateGEP(
                                decay_derived(
                                        emit_bitcast(types_svec, T_pprjlvalue)),
                                idx)));
                *ret = mark_julia_type(
                        fieldtyp, true, expr_type(expr, ctx), ctx);
                JL_GC_POP();
                return true;
            }
        }
        ...
        if (jl_is_datatype(sty) && sty != (jl_value_t *)jl_symbol_type &&
            ((jl_datatype_t *)sty)->name != jl_array_typename &&
            sty != (jl_value_t *)jl_simplevector_type &&
            sty != (jl_value_t *)jl_string_type &&
            // exclude DataType, since each DataType has its own size, not
            // sizeof(DataType).
            // this is issue #8798
            sty != (jl_value_t *)jl_datatype_type) {
            if (jl_is_leaf_type(sty) ||
                (((jl_datatype_t *)sty)->name->names == jl_emptysvec &&
                 jl_datatype_size(sty) > 0)) {
                *ret = mark_julia_type(
                        ConstantInt::get(T_size, jl_datatype_size(sty)),
                        false,
                        jl_long_type,
                        ctx);
                JL_GC_POP();
        ...
        if (fieldidx < 0 || fieldidx >= jl_datatype_nfields(stt)) {
            *ret = mark_julia_const(jl_false);
                ...
                !jl_field_isptr(stt, fieldidx) ||
                fieldidx < stt->ninitialized) {
...
static jl_cgval_t emit_call_function_object(
        jl_method_instance_t *li,
        const jl_cgval_t &theF,
        jl_llvm_functions_t decls,
        jl_value_t **args,
        size_t nargs,
        jl_value_t *callexpr,
        jl_codectx_t *ctx)
{
    ...
    if (decls.specFunctionObject != NULL) {
        ...
        jl_returninfo_t returninfo = get_specsig_function(
                jl_Module, proto->getName(), li->specTypes, jlretty);
        ...
        case jl_returninfo_t::SRet:
            result = emit_static_alloca(
                    cft->getParamType(0)->getContainedType(0), ctx);
            argvals[idx] = decay_derived(result);
            idx++;
            break;
        case jl_returninfo_t::Union:
            result = emit_static_alloca(
                    ArrayType::get(T_int8, returninfo.union_bytes), ctx);
        ...
        for (size_t i = 0; i < nargs + 1; i++) {
            ...
            if (isboxed) {
                assert(at == T_prjlvalue &&
                       (et == T_pjlvalue || et == T_prjlvalue));
        ...
        CallInst *call = builder.CreateCall(
                returninfo.decl, ArrayRef<Value *>(&argvals[0], nfargs));
        ...
        case jl_returninfo_t::Union: {
            ...
            Value *derived = builder.CreateSelect(
                    builder.CreateICmpEQ(
                            builder.CreateAnd(
                                    tindex, ConstantInt::get(T_int8, 0x80)),
                            ConstantInt::get(T_int8, 0)),
                    decay_derived(
                            builder.CreateBitCast(argvals[0], T_pjlvalue)),
                    decay_derived(box));
        ...
        // see if inference has a different / better type for the call than the
        // lambda
...
static jl_cgval_t emit_invoke(jl_expr_t *ex, jl_codectx_t *ctx)
{
    ...
    if (lival.constant) {
        ...
        assert(jl_is_method_instance(li));
        jl_llvm_functions_t decls =
                jl_compile_linfo(&li, NULL, ctx->world, ctx->params);
        if (li->jlcall_api == 2) {
        ...
        if (decls.functionObject) {
            ...
            if (jlcall_api == 1) {
                ...
                jl_cgval_t result = emit_call_function_object(
                        li,
                        fval,
                        decls,
                        &args[1],
    ...
    JL_FEAT_REQUIRE(ctx, runtime);
    jl_cgval_t result = mark_julia_type(
            emit_jlcall(
                    prepare_call(jlinvoke_func),
                    boxed(lival, ctx, false),
                    &args[1],
                    nargs,
                    ctx),
            true,
            expr_type((jl_value_t *)ex, ctx),
            ctx);
    if (result.typ == jl_bottom_type)
        CreateTrap(builder);
    return result;
...
static jl_cgval_t emit_call(jl_expr_t *ex, jl_codectx_t *ctx)
{
    ...
    if (f != NULL) {
        ...
        if (jl_typeis(f, jl_intrinsic_type)) {
            result = emit_intrinsic(
                    (intrinsic) * (uint32_t *)jl_data_ptr(f), args, nargs, ctx);
            if (result.typ == (jl_value_t *)jl_any_type) // the select_value
                                                         // intrinsic may be
                                                         // missing type
                                                         // information
        ...
        if (jl_isa(f, (jl_value_t *)jl_builtin_type)) {
            bool handled = emit_builtin_call(
                    &result, (jl_value_t *)f, args, nargs, ctx, expr);
            if (handled) {
                JL_GC_POP();
                return result;
            }
    ...
    if (f && jl_isa(f, (jl_value_t *)jl_builtin_type)) {
        std::map<jl_fptr_t, Function *>::iterator it =
                builtin_func_map.find(jl_get_builtin_fptr(f));
    ...
    if (!JL_FEAT_TEST(ctx, runtime)) {
        ...
        jl_errorf(
                "generic call to %s requires the runtime language feature",
    ...
    Value *callval =
            emit_jlcall(jlapplygeneric_func, nullptr, args, nargs, ctx);
        ...
        jl_codectx_t *ctx)
{
    ...
    if (assign) {
        ...
        if (b == NULL) {
            ...
            GlobalVariable *bindinggv = new GlobalVariable(
                    *ctx->f->getParent(),
                    T_pjlvalue,
                    false,
            ...
            BasicBlock *have_val = BasicBlock::Create(jl_LLVMContext, "found"),
                       *not_found =
                               BasicBlock::Create(jl_LLVMContext, "notfound");
            BasicBlock *currentbb = builder.GetInsertBlock();
            builder.CreateCondBr(
                    builder.CreateICmpNE(cachedval, initnul),
                    have_val,
                    not_found);
            ctx->f->getBasicBlockList().push_back(not_found);
            builder.SetInsertPoint(not_found);
            Value *bval = builder.CreateCall(
                    prepare_call(jlgetbindingorerror_func),
                    { maybe_decay_untracked(
        ...
        Value *bp, jl_sym_t *name, jl_codectx_t *ctx, bool isvol, MDNode *tbaa)
{
    ...
    undef_var_error_ifnot(
            builder.CreateICmpNE(v, maybe_decay_untracked(V_null)), name, ctx);
...
static jl_cgval_t emit_isdefined(jl_value_t *sym, jl_codectx_t *ctx)
{
    ...
    if (jl_is_slot(sym)) {
        ...
        if (vi.boxroot != NULL) {
            ...
            Value *box_isnull =
                    builder.CreateICmpNE(boxed, maybe_decay_untracked(V_null));
            if (vi.pTIndex) {
...
static jl_cgval_t emit_local(jl_value_t *slotload, jl_codectx_t *ctx)
{
    ...
    if (vi.boxroot == NULL || vi.pTIndex != NULL) {
        if (!vi.isVolatile || vi.value.constant || !vi.value.V) {
            ...
            assert(!volatile_slot->isArrayAllocation() &&
                   "variables not expected to be VLA");
    ...
    if (vi.boxroot != NULL) {
        ...
        if (vi.pTIndex) {
            ...
            if (v.V) { // v.V will be null if it is a union of all ghost values
                ...
                v.V = builder.CreateSelect(
                        load_unbox,
                        emit_bitcast(decay_derived(v.V), boxed->getType()),
                        boxed);
            }
            else
                v.V = boxed;
            ...
            v = mark_julia_type(
                    ...
                    typ,
                    ctx,
                    /*gc-root*/ !vi.isArgument); // if an argument, doesn't need
                                                 // an additional root
            if (vi.usedUndef)
                isnull = box_isnull;
        }
...
static Value *try_emit_union_alloca(
        jl_uniontype_t *ut,
        bool &allunbox,
        size_t &min_align,
        jl_codectx_t *ctx)
{
    ...
    if (nbytes > 0) {
        ...
        // try to pick an Integer type size such that SROA will emit reasonable
        // code
        Type *AT = ArrayType::get(
                IntegerType::get(jl_LLVMContext, 8 * min_align),
...
static Value *compute_box_tindex(
        Value *datatype,
        jl_value_t *supertype,
        jl_value_t *ut,
        jl_codectx_t *ctx)
{
            ...
            [&](unsigned idx, jl_datatype_t *jt) {
                if (jl_subtype((jl_value_t *)jt, supertype)) {
                    ...
                    tindex = builder.CreateSelect(
                            cmp, ConstantInt::get(T_int8, idx), tindex);
                }
            },
            ut,
            counter);
...
static Value *compute_tindex_unboxed(
        const jl_cgval_t &val, jl_value_t *typ, jl_codectx_t *ctx)
{
    if (val.constant)
        return ConstantInt::get(
                T_int8,
                get_box_tindex((jl_datatype_t *)jl_typeof(val.constant), typ));
    if (val.isboxed)
        return compute_box_tindex(
                emit_typeof_boxed(val, ctx), val.typ, typ, ctx);
    assert(val.TIndex);
...
static void emit_assignment(jl_value_t *l, jl_value_t *r, jl_codectx_t *ctx)
{
    if (jl_is_ssavalue(l)) {
        ...
        jl_cgval_t slot = emit_expr(r, ctx); // slot could be a jl_value_t
                                             // (unboxed) or jl_value_t*
                                             // (ispointer)
        if (slot.isboxed || slot.TIndex) {
            // see if inference suggested a different type for the ssavalue than
            // the expression
            // e.g. sometimes the information is inconsistent after inlining
            // getfield on a Tuple
            jl_value_t *ssavalue_types =
                    (jl_value_t *)ctx->source->ssavaluetypes;
        ...
        if (!slot.isboxed && !slot.isimmutable) {
            ...
            if (jl_is_uniontype(jt)) {
                ...
                bool allunbox;
                size_t min_align;
                dest = try_emit_union_alloca(
                        ((jl_uniontype_t *)jt), allunbox, min_align, ctx);
                Value *isboxed = NULL;
                if (slot.ispointer() && slot.V != NULL &&
                    !isa<AllocaInst>(slot.V)) {
                ...
                if (isboxed) {
                    ...
                    if (slot.gcroot) {
                        ...
                        // This might load the wrong object in general, but if
                        // it gets selected, below,
                    ...
                    if (dest) // might be all ghost values
                        dest = builder.CreateSelect(
                                isboxed,
                                decay_derived(box),
                                emit_bitcast(
                                        decay_derived(dest), box->getType()));
                    else
                        dest = box;
                }
                else {
                    assert(allunbox &&
                           "Failed to allocate correct union-type storage.");
                }
    ...
    if (bp != NULL) { // it's a global
        JL_FEAT_REQUIRE(ctx, runtime);
        assert(bnd);
        Value *rval = mark_callee_rooted(
                boxed(emit_expr(r, ctx), ctx, false)); // no root needed since
                                                       // this is about to be
                                                       // assigned to a global
        builder.CreateCall(
                prepare_call(jlcheckassign_func),
                { literal_pointer_val(bnd), rval });
        // Global variable. Does not need debug info because the debugger knows
        // about
        // its memory location.
        return;
    }
    ...
    rval_info =
            convert_julia_type(rval_info, slot_type, ctx, /*needs-root*/ true);
    if (rval_info.typ == jl_bottom_type)
        return;

    // add info to arrayvar list
    if (rval_info.isboxed) {
        // check isboxed in case rval isn't the right type (for example, on a
        // dead branch),
    ...
    if (vi.boxroot) {
        ...
        if (vi.pTIndex && rval_info.TIndex) {
            ...
            isboxed = builder.CreateICmpNE(
                    builder.CreateAnd(
                            rval_info.TIndex, ConstantInt::get(T_int8, 0x80)),
                    ConstantInt::get(T_int8, 0));
            rval = maybe_decay_untracked(V_null);
            if (rval_info.ispointer() && rval_info.V != NULL &&
                !isa<AllocaInst>(rval_info.V) &&
                !(isa<Constant>(isboxed) &&
                  cast<ConstantInt>(isboxed)->isZero())) // might be all ghost
                                                         // values or otherwise
                                                         // definitely not boxed
        ...
        builder.CreateStore(
                maybe_decay_untracked(rval), vi.boxroot, vi.isVolatile);
    }
        ...
        if (!vi.value.constant) { // check that this is not a virtual store
            ...
            if (vi.value.V == NULL) {
                ...
                if (rval_info.isghost) {
                    ...
                    if (rval_info.typ != vi.value.typ && !vi.pTIndex &&
                        !rval_info.TIndex) {
                        // isbits cast-on-assignment is invalid. this branch
                        // should be dead-code.
                        CreateTrap(builder);
                    }
                    else {
                        ...
                        if (vi.pTIndex)
                            builder.CreateStore(
                                    UndefValue::get(
                                            cast<AllocaInst>(vi.value.V)
                                                    ->getAllocatedType()),
                                    vi.value.V);
                        Type *store_ty = julia_type_to_llvm(
                                rval_info.constant
                ...
                // the memcpy intrinsic does not allow to specify different
                // alias tags
                ...
                if (vi.pTIndex == NULL) {
                    ...
                    Value *copy_bytes = ConstantInt::get(
                            T_int32, jl_datatype_size(vi.value.typ));
        ...
        const jl_cgval_t &condV, const std::string &msg, jl_codectx_t *ctx)
{
    ...
    if (isbool) {
        ...
        return builder.CreateXor(
                builder.CreateTrunc(cond, T_int1), ConstantInt::get(T_int1, 1));
    }
    if (condV.isboxed) {
        return builder.CreateICmpEQ(
                boxed(condV, ctx),
...
static Value *
emit_condition(jl_value_t *cond, const std::string &msg, jl_codectx_t *ctx)
{
...
static void emit_stmtpos(jl_value_t *expr, jl_codectx_t *ctx)
{
    ...
    if (head == line_sym || head == meta_sym || head == boundscheck_sym ||
        ...
        if (!jl_is_method(ctx->linfo->def.method)) {
            ...
            Value *world =
                    builder.CreateLoad(prepare_global(jlgetworld_global));
...
static jl_cgval_t emit_expr(jl_value_t *expr, jl_codectx_t *ctx)
{
    ...
    if (jl_is_ssavalue(expr)) {
        ...
        assert(idx >= 0);
        if (!ctx->ssavalue_assigned.at(idx)) {
            ctx->ssavalue_assigned.at(idx) =
                    true;        // (assignment, not comparison test)
            return jl_cgval_t(); // dead code branch
        }
        else {
            return ctx->SAvalues.at(idx); // at this point, SAvalues[idx]
                                          // actually contains the SAvalue
        }
    }
    if (jl_is_globalref(expr)) {
        return emit_getfield(
                (jl_value_t *)jl_globalref_mod(expr),
                jl_globalref_name(expr),
                ctx);
    }
    ...
    if (!jl_is_expr(expr)) {
        ...
        if (needroot &&
            jl_is_method(ctx->linfo->def.method)) { // toplevel exprs and some
    ...
    if (head == isdefined_sym) {
        ...
        if (jl_is_method(ctx->linfo->def.method)) { // don't bother codegen
        ...
        if (bp) {
            Value *mdargs[5] = { name,
                                 literal_pointer_val((jl_value_t *)mod),
                                 bp,
                                 maybe_decay_untracked(bp_owner),
                                 literal_pointer_val(bnd) };
            jl_cgval_t gf = mark_julia_type(
                    builder.CreateCall(
                            prepare_call(jlgenericfunction_func),
                            makeArrayRef(mdargs)),
                    true,
                    jl_function_type,
                    ctx);
        ...
        Value *mdargs[4] = { /*argdata*/ a1,
                             /*code*/ a2,
                             /*module*/ literal_pointer_val(
                                     (jl_value_t *)ctx->module),
        ...
        if (jl_is_symbol(sym)) {
            JL_FEAT_REQUIRE(ctx, runtime);
            jl_binding_t *bnd = NULL;
            (void)global_binding_pointer(mod, sym, &bnd, true, ctx);
            assert(bnd);
            builder.CreateCall(
                    prepare_call(jldeclareconst_func),
                    literal_pointer_val(bnd));
        }
    }
        ...
        if (jl_is_quotenode(arg)) {
            ...
            if (!(jl_is_expr(arg1) || jl_typeis(arg1, jl_array_any_type) ||
                  jl_is_quotenode(arg1))) {
        ...
        return mark_julia_type(
                builder.CreateCall(
                        prepare_call(jlcopyast_func),
                        maybe_decay_untracked(boxed(ast, ctx))),
                true,
                ast.typ,
                ctx);
    }
        ...
        if (jl_is_toplevel_only_expr(expr) &&
            ...
            Value *args[2] = { literal_pointer_val((jl_value_t *)ctx->module),
        ...
        if (head == abstracttype_sym || head == compositetype_sym ||
            ...
            jl_errorf(
                    "unsupported or misplaced expression \"%s\" in function %s",
                    jl_symbol_name(head),
                    ctx->name);
        }
    }
...
static void allocate_gc_frame(BasicBlock *b0, jl_codectx_t *ctx)
{
    ...
    ctx->signalPage = emit_nthptr_recast(
            ctx->ptlsStates,
            nthfield,
            tbaa_const,
...
static void emit_last_age_field(jl_codectx_t *ctx)
{
    ctx->world_age_field = builder.CreateGEP(
            ...
            ConstantInt::get(
                    T_size,
                    offsetof(jl_tls_states_t, world_age) / sizeof(size_t)));
}
        ...
        jl_tupletype_t *sigt)
{
    ...
    jl_method_instance_t *lam =
            jl_get_specialization1((jl_tupletype_t *)sigt, world);
    jl_value_t *astrt = (jl_value_t *)jl_any_type;
    // infer it first, if necessary
    if (lam) {
        name = jl_symbol_name(lam->def.method->name);
        jl_code_info_t *src = NULL;
        if (!lam->inferred) // TODO: this isn't ideal to be unconditionally
                            // calling type inference from here
        ...
        if (lam) {
            ...
            if (astrt != (jl_value_t *)jl_bottom_type &&
                ...
                // occasionally required by the C API (typically error
                // callbacks)
                // and doesn't capture the majority of the case when a function
                // may throw.
                jl_printf(
                        JL_STDERR,
                        "WARNING: cfunction: return type of %s does not "
                        "match\n",
                        name);
            }
        }
    ...
    jl_setup_module(M);
    Function *cw = Function::Create(
            sig.functype, GlobalVariable::ExternalLinkage, funcName.str(), M);
    jl_init_function(cw);
    ...
    ctx.world_age_field =
            builder.CreateSelect(have_tls, ctx.world_age_field, dummy_world);
    Value *last_age = tbaa_decorate(
            tbaa_gcframe, builder.CreateLoad(ctx.world_age_field));
    ...
    Value *age_ok = NULL;
    if (lam) {
        Value *lam_max = builder.CreateLoad(builder.CreateConstInBoundsGEP1_32(
                T_size,
                emit_bitcast(
                        decay_derived(literal_pointer_val((jl_value_t *)lam)),
                        T_psize),
                offsetof(jl_method_instance_t, max_world) / sizeof(size_t)));
        // XXX: age is always OK if we don't have a TLS. This is a hack required
        // due to `@threadcall` abuse.
        ...
        world_v = builder.CreateSelect(
                builder.CreateOr(have_tls, age_ok), world_v, lam_max);
    ...
    if (lam && lam->jlcall_api == 2) {
        ...
        Function *proto =
                (Function *)lam->functionObjectsDecls.specFunctionObject;
        jl_returninfo_t returninfo = get_specsig_function(
                M, proto->getName(), lam->specTypes, lam->rettype);
        FunctionType *cft = returninfo.decl->getFunctionType();
        assert(proto->getFunctionType() == cft);
        proto = cast<Function>(prepare_call(proto));
        if (proto != returninfo.decl) {
        ...
        if (jlfunc_sret || cc == jl_returninfo_t::Union) {
            ...
            if (sig.sret && jlfunc_sret)
                ...
                result = decay_derived(builder.CreateAlloca(
                        cft->getParamType(0)->getContainedType(0)));
        ...
        theFptr = lam ? (Function *)lam->functionObjectsDecls.functionObject
                      : NULL;
        specsig = false;
        jlfunc_sret = false;
        myargs = new AllocaInst(
                T_prjlvalue,
#if JL_LLVM_VERSION >= 50000
                0,
#endif
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (jl_is_abstract_ref_type(jargty)) {
            ...
            if (jargty == (jl_value_t *)jl_any_type) {
                ...
                // must be a jl_value_t* (because it's mutable or contains gc
                // roots)
                inputarg = mark_julia_type(
                        maybe_decay_untracked(emit_bitcast(val, T_prjlvalue)),
                        true,
                        jargty,
                        &ctx);
            }
                ...
                if (type_is_ghost(T)) {
                    ...
                    val = builder.CreateAlignedLoad(val, 1); // make no
                                                             // alignment
                                                             // assumption about
                                                             // pointer from C
            ...
            if (argboxed) {
                ...
                if (sig.byRefList.at(i)) {
                    assert(cast<PointerType>(val->getType())
                                   ->getElementType() == sig.fargt[i]);
                    val = builder.CreateAlignedLoad(
                            val, 1); // unknown alignment from C
                }
                else {
                    bool issigned =
                            jl_signed_type &&
                            jl_subtype(jargty, (jl_value_t *)jl_signed_type);
                ...
                if (isboxed) {
                    ...
                    Value *mem = emit_allocobj(
                            &ctx,
                            jl_datatype_size(jargty),
                            literal_pointer_val((jl_value_t *)jargty));
                    tbaa_decorate(
                            jl_is_mutable(jargty) ? tbaa_mutab : tbaa_immut,
                            builder.CreateAlignedStore(
                                    val,
                                    emit_bitcast(
                                            mem,
                                            val->getType()->getPointerTo()),
                                    16)); // julia's gc gives 16-byte aligned
                                          // addresses
        ...
        if (!specsig) {
            Value *arg = boxed(inputarg, &ctx, false); // don't want a gcroot,
                                                       // since it's about to be
                                                       // put into the jlcall
                                                       // frame anyways
            ...
            Value *arg;
            jl_value_t *spect = jl_nth_slot_type(
                    lam->specTypes, i + 1); // +1 because argt excludes function
            bool isboxed;
            ...
            if (isboxed) {
                ...
                continue; // ghost types are skipped by the specsig method
                          // signature
            }
    ...
    if (specsig) {
        if (lam->jlcall_api == 2) {
            ...
            if (age_ok) {
                funcName << "_gfthunk";
                gf_thunk = Function::Create(
                        theFptr->getFunctionType(),
                        GlobalVariable::InternalLinkage,
                        funcName.str(),
                        M);
            ...
            CallInst *call =
                    builder.CreateCall(call_v, ArrayRef<Value *>(args));
            ...
            case jl_returninfo_t::SRet:
                ...
                break;
            case jl_returninfo_t::Union:
                retval = mark_julia_slot(
                        builder.CreateExtractValue(call, 0),
                        astrt,
                        builder.CreateExtractValue(call, 1),
                        tbaa_stack);
                // note that the value may not be rooted here (on the return
                // path)
                break;
            case jl_returninfo_t::Ghosts:
        ...
        // for jlcall, we need to pass the function object even if it is a
        // ghost.
        // here we reconstruct the function instance from its type (first elt of
        // argt)
        ...
        if (age_ok) {
            ...
            Value *myargs1 =
                    builder.CreateConstInBoundsGEP1_32(T_prjlvalue, myargs, 1);
            ret_jlcall = builder.CreateCall(
                    prepare_call(theFptr), { theF, myargs1, nargs_v });
        ...
        Value *ret = builder.CreateCall(
                prepare_call(jlapplygeneric_func), { myargs, nargs_v });
        if (age_ok) {
            builder.CreateBr(b_after);
    ...
    if (toboxed) {
        ...
        r =
                boxed(retval,
                      &ctx,
                      false); // no gcroot since this is on the return path
    }
        ...
        Type *prt = sig.prt;
        if (sig.sret)
            prt = sig.fargt_sig[0]->getContainedType(
                    0); // sret is a PointerType
        bool issigned = jl_signed_type &&
                        jl_subtype(declrt, (jl_value_t *)jl_signed_type);
        Value *v = julia_to_native(
                sig.lrt, toboxed, declrt, NULL, retval, false, 0, &ctx, NULL);
    ...
    // also need to finish emission of our specsig -> jl_apply_generic converter
    // thunk
    ...
    if (gf_thunk) {
        ...
        for (size_t i = 0; i < nargs + 1; i++) {
            ...
            if (type_is_ghost(et)) {
                ...
                if (isboxed) {
                    ...
                    arg_box =
                            boxed(mark_julia_slot(arg_v, jt, NULL, tbaa_const),
                                  &ctx2,
                                  false);
                }
                else {
                    assert(at == et);
                    arg_box =
                            boxed(mark_julia_type(arg_v, false, jt, &ctx2),
                                  &ctx2,
                                  false);
                }
                (void)at;
            ...
            Value *argn =
                    builder.CreateConstInBoundsGEP1_32(T_prjlvalue, myargs, i);
            builder.CreateStore(maybe_decay_untracked(arg_box), argn);
        }
        assert(AI == gf_thunk->arg_end());
        Value *nargs_v = ConstantInt::get(T_int32, nargs + 1);
        Value *gf_ret = builder.CreateCall(
                prepare_call(jlapplygeneric_func), { myargs, nargs_v });
        jl_cgval_t gf_retbox = mark_julia_type(
                gf_ret, true, jl_any_type, &ctx, /*needsroot*/ false);
        ...
        case jl_returninfo_t::Union: {
            ...
            Value *tindex = compute_box_tindex(
                    gf_ret, (jl_value_t *)jl_any_type, astrt, &ctx);
...
const struct jl_typemap_info cfunction_cache = { 1, &jl_voidpointer_type };

// Get the LLVM Function* for the C-callable entry point for a certain function
// and argument types.
// here argt does not include the leading function type argument
static Function *
jl_cfunction_object(jl_function_t *ff, jl_value_t *declrt, jl_tupletype_t *argt)
{
    ...
    jl_value_t *cfunc_sig =
            NULL; // type signature of the call to cfunction (for caching)
    ...
    if (jl_is_abstract_ref_type(declrt)) {
        ...
        if (jl_is_typevar(declrt))
            jl_error("cfunction: return type Ref should have an element type, "
                     "not Ref{T}");
        if (declrt == (jl_value_t *)jl_any_type)
            jl_error("cfunction: return type Ref{Any} is invalid. Use Any or "
                     "Ptr{Any} instead.");
        if (!jl_is_leaf_type(declrt))
            jl_svecset(cfunc_sig, nargs + 1, declrt); // Ref{Abstract} is the
    ...
    for (i = 0; i < nargs; i++) {
        ...
        if (jl_is_abstract_ref_type(ati)) {
            ...
            if (jl_is_typevar(ati))
                jl_error("cfunction: argument type Ref should have an element "
                         "type, not Ref{T}");
            if (ati != (jl_value_t *)jl_any_type && !jl_is_leaf_type(ati))
                jl_svecset(cfunc_sig, i + 1, ati); // Ref{Abstract} is the same
                                                   // calling convention as
                                                   // Abstract
        }
        if (jl_is_pointer(ati) && jl_is_typevar(jl_tparam0(ati)))
            jl_error("cfunction: argument type Ptr should have an element "
                     "type, Ptr{T}");
    ...
    if (sf == NULL) {
        sf = jl_typemap_insert(
                &jl_cfunction_list,
                (jl_value_t *)jl_cfunction_list.unknown,
                (jl_tupletype_t *)cfunc_sig,
                NULL,
                jl_emptysvec,
                NULL,
...
static Function *gen_jlcall_wrapper(
        jl_method_instance_t *lam,
        const jl_returninfo_t &f,
        const std::string &funcName,
        Module *M)
{
    Function *w = Function::Create(
            jl_func_sig, GlobalVariable::ExternalLinkage, funcName, M);
    jl_init_function(w);
    ...
    case jl_returninfo_t::Ghosts:
        break;
    case jl_returninfo_t::SRet:
        result = builder.CreateAlloca(
                ftype->getParamType(0)->getContainedType(0));
    ...
    for (size_t i = 0; i < nargs; i++) {
        ...
        if (i == 0) {
            ...
            Value *argPtr = builder.CreateGEP(
                    argArray, ConstantInt::get(T_size, i - 1));
            theArg = builder.CreateLoad(argPtr);
        }
        if (lty != NULL && !isboxed) {
            theArg = decay_derived(
                    emit_bitcast(theArg, PointerType::get(lty, 0)));
            if (!lty->isAggregateType()) // keep "aggregate" type values in
                                         // place as pointers
                theArg = builder.CreateAlignedLoad(
                        theArg, julia_alignment(theArg, ty, 0));
        }
    ...
    CallInst *call =
            builder.CreateCall(f.decl, ArrayRef<Value *>(&args[0], nfargs));
    ...
    case jl_returninfo_t::Boxed:
        retval =
                mark_julia_type(call, true, jlretty, &ctx, /*needsroot*/ false);
        break;
    case jl_returninfo_t::Register:
        retval = mark_julia_type(
                call, false, jlretty, &ctx, /*needsroot*/ false);
        break;
    case jl_returninfo_t::SRet:
        retval = mark_julia_slot(result, jlretty, NULL, tbaa_stack);
        break;
    ...
    builder.CreateRet(
            boxed(retval,
                  &ctx,
                  false)); // no gcroot needed since this on the return path
    assert(!ctx.roots);
    return w;
}
        ...
        jl_value_t *jlrettype)
{
    ...
    if (jlrettype == (jl_value_t *)jl_void_type) {
        ...
        bool allunbox;
        union_alloca_type(
                (jl_uniontype_t *)jlrettype,
                allunbox,
                props.union_bytes,
                props.union_align,
                props.union_minalign);
    ...
    if (props.cc == jl_returninfo_t::SRet) {
        attributes = attributes.addAttribute(
                jl_LLVMContext, 1, Attribute::StructRet);
        attributes =
                attributes.addAttribute(jl_LLVMContext, 1, Attribute::NoAlias);
        attributes = attributes.addAttribute(
                jl_LLVMContext, 1, Attribute::NoCapture);
    }
    ...
    for (size_t i = 0; i < jl_nparams(sig); i++) {
        ...
        if (ty->isAggregateType()) { // aggregate types are passed by pointer
            attributes = attributes.addAttribute(
                    jl_LLVMContext, fsig.size() + 1, Attribute::NoCapture);
            attributes = attributes.addAttribute(
                    jl_LLVMContext, fsig.size() + 1, Attribute::ReadOnly);
    ...
    Function *f =
            Function::Create(ftype, GlobalVariable::ExternalLinkage, name, M);
...
get_specsig_di(jl_value_t *sig, DIFile *topfile, DIBuilder &dbuilder)
{
    ...
    return dbuilder.createSubroutineType(
            dbuilder.getOrCreateTypeArray(ditypes));
}
        ...
        const jl_cgparams_t *params)
{
    ...
    ctx.module = jl_is_method(lam->def.method) ? lam->def.method->module
                                               : lam->def.module;
    ctx.linfo = lam;
    ctx.source = src;
    ctx.world = world;
    ctx.name = jl_symbol_name(
            jl_is_method(lam->def.method) ? lam->def.method->name
                                          : anonymous_sym);
    ...
    int n_ssavalues = jl_is_long(src->ssavaluetypes)
                              ? jl_unbox_long(src->ssavaluetypes)
    ...
    if (nreq > 0 && lam->def.method->isva) {
        ...
        va = 1;
        jl_sym_t *vn =
                (jl_sym_t *)jl_array_ptr_ref(src->slotnames, ctx.nargs - 1);
        if (vn != unused_sym)
    ...
    if (va && ctx.vaSlot != -1) {
        ...
        varinfo.isArgument = true;
        varinfo.value =
                mark_julia_type((Value *)NULL, false, jl_tuple_type, &ctx);
    }
    ...
    for (i = 0; i < vinfoslen; i++) {
        ...
        varinfo.usedUndef = (jl_vinfo_usedundef(flags) != 0) ||
                            (!varinfo.isArgument && !src->inferred);
        if (!varinfo.isArgument) {
            jl_value_t *typ = jl_is_array(src->slottypes)
                                      ? jl_array_ptr_ref(src->slottypes, i)
                                      : (jl_value_t *)jl_any_type;
            if (!jl_is_type(typ))
    ...
    bool needsparams = jl_is_method(lam->def.method)
                               ? jl_svec_len(lam->def.method->sparam_syms) !=
    ...
    if (!va && ctx.nargs > 0 && !needsparams &&
        lam->specTypes != (jl_value_t *)jl_anytuple_type && src->inferred) {
    ...
    if (specsig) { // assumes !va and !needsparams
        ...
        returninfo = get_specsig_function(
                M, specName.str(), lam->specTypes, jlrettype);
        f = returninfo.decl;
        ctx.has_sret =
                (returninfo.cc == jl_returninfo_t::SRet ||
        ...
        f = Function::Create(
                needsparams ? jl_func_sig_sparams : jl_func_sig,
    ...
    f->addAttributes(
            ...
            AttributeList::get(
                    f->getContext(), AttributeList::FunctionIndex, *attr));
#else
    f->addAttributes(
            AttributeSet::FunctionIndex,
            AttributeSet::get(
                    f->getContext(), AttributeSet::FunctionIndex, *attr));
#endif
#endif
#if defined(_OS_WINDOWS_) && defined(_CPU_X86_64_)
...
#ifdef USE_POLLY
    if (!jl_has_meta(stmts, polly_sym) ||
    ...
    if (ctx.debug_enabled) {
...
#if JL_LLVM_VERSION >= 40000
        DICompileUnit *CU =
                dbuilder.createCompileUnit(0x01, topfile, "julia", true, "", 0);
#else
        DICompileUnit *CU = dbuilder.createCompileUnit(
                0x01, filename, ".", "julia", true, "", 0);
#endif
        ...
        if (jl_options.debug_level >= 2) {
            ...
            // Go over all arguments and local variables and initialize their
            // debug information
            for (i = 0; i < nreq; i++) {
                jl_sym_t *argname =
                        (jl_sym_t *)jl_array_ptr_ref(src->slotnames, i);
                if (argname == unused_sym)
                    continue;
                jl_varinfo_t &varinfo = ctx.slots[i];
            ...
            if (va && ctx.vaSlot != -1) {
                ctx.slots[ctx.vaSlot].dinfo = dbuilder.createParameterVariable(
                        SP, // Scope (current function will be fill in later)
                        std::string(
                                jl_symbol_name(slot_symbol(ctx.vaSlot, &ctx))) +
                                "...",           // Variable name
                        ctx.has_sret + nreq + 1, // Argument number (1-based)
                        topfile,                 // File
                        toplineno == -1 ? 0 : toplineno, // Line (for now, use
                                                         // lineno of the
                                                         // function)
                        julia_type_to_di(
                                ctx.slots[ctx.vaSlot].value.typ,
                                &dbuilder,
                                false),
                        AlwaysPreserve, // May be deleted if optimized out
                        DIFlagZero);    // Flags (TODO: Do we need any)
            ...
            for (i = 0; i < vinfoslen; i++) {
                ...
                jl_varinfo_t &varinfo = ctx.slots[i];
                if (varinfo.isArgument || s == compiler_temp_sym ||
                    s == unused_sym)
                    continue;
    ...
    */
    ...
    for (i = 0; i < vinfoslen; i++) {
        ...
        assert(!varinfo.boxroot); // variables shouldn't have memory locs
                                  // already
        if (varinfo.value.constant) {
            ...
            bool allunbox;
            size_t align;
            Value *lv = try_emit_union_alloca(
                    (jl_uniontype_t *)jt, allunbox, align, &ctx);
            if (lv) {
            ...
            assert(!isboxed);
            assert(!type_is_ghost(vtype) &&
                   "constants should already be handled");
            ...
            if (ctx.debug_enabled && varinfo.dinfo) {
                assert((Metadata *)varinfo.dinfo->getType() !=
                       jl_pvalue_dillvmt);
        ...
        if (!varinfo.isArgument || // always need a slot if the variable is
                                   // assigned
            specsig || // for arguments, give them stack slots if they aren't in
                       // `argArray` (otherwise, will use that pointer)
            (va && (int)i == ctx.vaSlot &&
             varinfo.escapes) ||           // or it's the va arg tuple
            (s != unused_sym && i == 0)) { // or it is the first argument (which
                                           // isn't in `argArray`)
#if JL_LLVM_VERSION >= 50000
            AllocaInst *av = new AllocaInst(
                    T_prjlvalue,
                    0,
#else
            AllocaInst *av = new AllocaInst(
    ...
    for (i = 0; i < nreq; i++) {
        ...
        if (s == unused_sym || vi.value.constant) {
            ...
            if (specsig) {
                if (type_is_ghost(llvmArgType)) { // this argument is not
                                                  // actually passed
                    ...
                    if (isboxed)
                        maybe_mark_argument_dereferenceable(Arg, argType);
                    theArg = mark_julia_type(
                            Arg, isboxed, argType, &ctx, /*needsgcroot*/ false);
                }
            }
            else {
                if (i == 0) {
                    ...
                    theArg = mark_julia_type(
                            ...
                            &ctx,
                            /*needsgcroot*/ false);
                    if (ctx.debug_enabled && vi.dinfo && !vi.boxroot &&
                        !vi.value.V) {
                        SmallVector<uint64_t, 8> addr;
                        addr.push_back(llvm::dwarf::DW_OP_deref);
                        addr.push_back(llvm::dwarf::DW_OP_plus);
                        addr.push_back((i - 1) * sizeof(void *));
                        if ((Metadata *)vi.dinfo->getType() !=
                            jl_pvalue_dillvmt)
                            addr.push_back(llvm::dwarf::DW_OP_deref);
                        dbuilder.insertDeclare(
                                pargArray,
            ...
            if (vi.boxroot == NULL) {
                ...
                // keep track of original (possibly boxed) value to avoid
                // re-boxing or moving
                vi.value = theArg;
                if (specsig && theArg.V && ctx.debug_enabled && vi.dinfo) {
                    ...
                    if (!parg) {
                        parg = builder.CreateAlloca(
                                theArg.V->getType(), NULL, jl_symbol_name(s));
                ...
                Value *argp = boxed(theArg, &ctx, false); // skip the temporary
                                                          // gcroot since it
                                                          // would be folded to
                                                          // argp anyways
    ...
    if (va && ctx.vaSlot != -1) {
        ...
        if (!vi.escapes) {
            ...
            if (vi.boxroot != NULL) {
                Value *restTuple = builder.CreateCall(
                        prepare_call(jltuple_func),
                        { maybe_decay_untracked(V_null),
                          builder.CreateGEP(
                                  argArray, ConstantInt::get(T_size, nreq - 1)),
                          builder.CreateSub(
                                  argCount,
                ...
                // TODO: Perhaps allow this in the future, but for now since
                // varargs
    ...
    StmtProp cur_prop{ topdebugloc, filename, toplineno, false,
                       true,        false,    false };
    ctx.line = &cur_prop.line;
    if (coverage_mode != JL_LOG_NONE || malloc_log_mode) {
        cur_prop.in_user_code =
                (!jl_is_submodule(ctx.module, jl_base_module) &&
    ...
    for (i = 0; i < stmtslen; i++) {
        ...
        if (jl_is_linenode(stmt) || (expr && expr->head == line_sym)) {
            ...
            if (meta_arg == (jl_value_t *)jl_symbol("push_loc")) {
                ...
                DI_stack.push_back(DbgState{ cur_prop.loc,
                                             SP,
                                             cur_prop.file,
                                             cur_prop.line,
                                             cur_prop.in_user_code });
                ...
                if (ctx.debug_enabled) {
                    ...
                    cur_prop.loc = DebugLoc::get(
                            inlined_func_lineno, 0, SP, inlinedAt);
                }
    ...
    auto handle_label = [&](int lname, bool unconditional) {
        ...
        if (lname == cursor + 1 && cur_bb->begin() == cur_bb->end()) {
            ...
            bb = BasicBlock::Create(
                    jl_LLVMContext, "L" + std::to_string(lname), f);
            if (unconditional) {
                if (!cur_bb->getTerminator())
                    builder.CreateBr(bb);
    ...
    while (cursor != -1) {
        ...
        if (expr && expr->head == return_sym) {
            ...
            retvalinfo = convert_julia_type(
                    retvalinfo, jlrettype, &ctx, /*needs-root*/ false);
            ...
            case jl_returninfo_t::Boxed:
                retval =
                        boxed(retvalinfo,
                              &ctx,
                              false); // skip the gcroot on the return path
                break;
            case jl_returninfo_t::Register:
                ...
                if (retvalinfo.TIndex) {
                    ...
                    if (retvalinfo.V == NULL) {
                        ...
                        if (retvalinfo.ispointer() &&
                            !isa<AllocaInst>(retvalinfo.V)) {
                            // also need to account for the possibility the
                            // return object is boxed
                            ...
                            if (isa<Constant>(isboxed_union))
                                data = cast<ConstantInt>(isboxed_union)
                                                       ->isZero()
                                               ? data
                                               : builder.CreateLoad(
                                                         retvalinfo.gcroot);
                            else
                                data = builder.CreateSelect(
                                        isboxed_union,
                    ...
                    tindex =
                            compute_tindex_unboxed(retvalinfo, jlrettype, &ctx);
                    tindex = builder.CreateOr(
                            tindex, ConstantInt::get(T_int8, 0x80));
                    data = maybe_decay_untracked(
                            boxed(retvalinfo,
                                  &ctx,
                                  false)); // skip the gcroot on the return path
                    sret = NULL;
            ...
            if (sret) {
                if (retvalinfo.ispointer()) {
                    if (returninfo.cc == jl_returninfo_t::SRet) {
                        ...
                        Value *copy_bytes = ConstantInt::get(
                                T_int32, jl_datatype_size(jlrettype));
                        builder.CreateMemCpy(
                                sret,
                    ...
                    builder.CreateStore(
                            emit_unbox(store_ty, retvalinfo, retvalinfo.typ),
                            sret);
                }
            }
...
#ifdef _OS_WINDOWS_
            BasicBlock *cond_resetstkoflw_blk =
                    BasicBlock::Create(jl_LLVMContext, "cond_resetstkoflw", f);
            BasicBlock *resetstkoflw_blk =
                    BasicBlock::Create(jl_LLVMContext, "resetstkoflw", f);
...
std::pair<MDNode *, MDNode *> tbaa_make_child(
        const char *name, MDNode *parent = nullptr, bool isConstant = false)
{
    ...
    MDNode *n =
            mbuilder->createTBAAStructTagNode(scalar, scalar, 0, isConstant);
    return std::make_pair(n, scalar);
}

static GlobalVariable *
global_to_llvm(const std::string &cname, void *addr, Module *m)
{
    GlobalVariable *gv = new GlobalVariable(
            *m, T_pjlvalue, true, GlobalVariable::ExternalLinkage, NULL, cname);
    add_named_global(gv, addr);
    return gv;
}
llvm::SmallVector<std::pair<jl_value_t **, GlobalVariable *>, 16> gv_for_global;
static GlobalVariable *
global_jlvalue_to_llvm(const std::string &cname, jl_value_t **addr, Module *m)
{
...
static Function *
jlcall_func_to_llvm(const std::string &cname, jl_fptr_t addr, Module *m)
{
    Function *f =
            Function::Create(jl_func_sig, Function::ExternalLinkage, cname, m);
    add_named_global(f, addr);
    return f;
}

extern "C" void
jl_fptr_to_llvm(jl_fptr_t fptr, jl_method_instance_t *lam, int specsig)
{
    if (imaging_mode) {
        if (!specsig) {
            lam->fptr = fptr; // in imaging mode, it's fine to use the fptr, but
                              // we don't want it in the shadow_module
        }
    }
    else {
        // this assigns a function pointer (from loading the system image), to
        // the function object
        std::stringstream funcName;
        if (specsig)
        ...
        if (specsig) { // assumes !va
            Function *f = get_specsig_function(
                                  shadow_output,
                                  funcName.str(),
                                  lam->specTypes,
                                  lam->rettype)
            ...
            if (lam->jlcall_api != 1) { // jl_func_sig_sparams -- don't bother
                                        // emitting the FunctionObject (since
                                        // can't be used right now)
                assert(lam->fptr == NULL);
                lam->fptr = fptr;
            }
            else {
                Function *f = jlcall_func_to_llvm(
                        funcName.str(), fptr, shadow_output);
                if (lam->functionObjectsDecls.functionObject == NULL) {
...
static void init_julia_llvm_meta(void)
{
    ...
    tbaa_ptrarraybuf =
            tbaa_make_child("jtbaa_ptrarraybuf", tbaa_data_scalar).first;
    ...
    tbaa_arraysize =
            tbaa_make_child("jtbaa_arraysize", tbaa_array_scalar).first;
    tbaa_arraylen = tbaa_make_child("jtbaa_arraylen", tbaa_array_scalar).first;
    tbaa_arrayflags =
            tbaa_make_child("jtbaa_arrayflags", tbaa_array_scalar).first;
...
static void init_julia_llvm_env(Module *m)
{
    ...
    // This type is used to create undef Values for use in struct declarations
    // to skip indices
    ...
    jl_value_dillvmt = dbuilder.createStructType(
            ...
            "jl_value_t",
            julia_h,
            71,    // At the time of this writing. Not sure if it's worth it to
                   // keep this in sync
    ...
    diargs.push_back(
            julia_type_to_di((jl_value_t *)jl_int32_type, &dbuilder, false));
    ...
    Type *vaelts[] = { T_pint8
                       ...
                       T_int16 };
    static_assert(
            sizeof(jl_array_flags_t) == sizeof(int16_t),
            "Size of jl_array_flags_t is not the same as int16_t");
    Type *jl_array_llvmt = StructType::create(
            jl_LLVMContext,
    ...
    jltls_states_var = new GlobalVariable(
            *m,
            T_ppjlvalue,
            false,
            GlobalVariable::ExternalLinkage,
    ...
    jltls_states_func = Function::Create(
            FunctionType::get(jltls_states_var->getType(), false),
            Function::ExternalLinkage,
            "jl_get_ptls_states",
            m);
    ...
    jltls_states_func = Function::Create(
            FunctionType::get(PointerType::get(T_ppjlvalue, 0), false),
            Function::ExternalLinkage,
            "jl_get_ptls_states",
            m);
    add_named_global(jltls_states_func, jl_get_ptls_states_getter());
    if (imaging_mode) {
        PointerType *pfunctype =
    ...
    jlerror_func = Function::Create(
            FunctionType::get(T_void, args1, false),
            Function::ExternalLinkage,
            "jl_error",
            m);
    ...
    jlthrow_func = Function::Create(
            FunctionType::get(T_void, args1_, false),
            Function::ExternalLinkage,
            "jl_throw",
            m);
    ...
    jlundefvarerror_func = Function::Create(
            FunctionType::get(T_void, args1_, false),
            Function::ExternalLinkage,
    ...
    jlboundserrorv_func = Function::Create(
            FunctionType::get(T_void, args2_boundserrorv, false),
            Function::ExternalLinkage,
    ...
    args2_boundserror.push_back(
            PointerType::get(T_jlvalue, AddressSpace::CalleeRooted));
    args2_boundserror.push_back(T_size);
    jlboundserror_func = Function::Create(
            FunctionType::get(T_void, args2_boundserror, false),
            Function::ExternalLinkage,
            "jl_bounds_error_int",
            m);
    ...
    args3_vboundserror.push_back(T_size);
    jlvboundserror_func = Function::Create(
            FunctionType::get(T_void, args3_vboundserror, false),
            Function::ExternalLinkage,
    ...
    args3_uboundserror.push_back(
            PointerType::get(T_int8, AddressSpace::Derived));
    args3_uboundserror.push_back(T_prjlvalue);
    args3_uboundserror.push_back(T_size);
    jluboundserror_func = Function::Create(
            FunctionType::get(T_void, args3_uboundserror, false),
            Function::ExternalLinkage,
    ...
    jlnew_func = Function::Create(
            jl_func_sig, Function::ExternalLinkage, "jl_new_structv", m);
    ...
    memcmp_func = Function::Create(
            FunctionType::get(T_int32, args_memcmp, false),
            Function::ExternalLinkage,
            "memcmp",
            m);
    ...
    jltypeerror_func = Function::Create(
            FunctionType::get(T_void, te_args, false),
            Function::ExternalLinkage,
            "jl_type_error_rt",
            m);
    ...
    args_2ptrs.push_back(
            PointerType::get(T_jlvalue, AddressSpace::CalleeRooted));
    jlcheckassign_func = Function::Create(
            FunctionType::get(T_void, args_2ptrs, false),
            Function::ExternalLinkage,
    ...
    jldeclareconst_func = Function::Create(
            FunctionType::get(T_void, args_1binding, false),
            Function::ExternalLinkage,
            "jl_declare_constant",
            m);
    ...
    args_2ptrs_.push_back(T_prjlvalue);
    jlgetbindingorerror_func = Function::Create(
            FunctionType::get(T_pjlvalue, args_2ptrs_, false),
            Function::ExternalLinkage,
            "jl_get_binding_or_error",
            m);
    add_named_global(jlgetbindingorerror_func, &jl_get_binding_or_error);

    jlboundp_func = Function::Create(
            FunctionType::get(T_int32, args_2ptrs_, false),
            Function::ExternalLinkage,
            "jl_boundp",
            m);
    ...
    builtin_func_map[jl_f_typeof] =
            jlcall_func_to_llvm("jl_f_typeof", &jl_f_typeof, m);
    builtin_func_map[jl_f_sizeof] =
            jlcall_func_to_llvm("jl_f_sizeof", &jl_f_sizeof, m);
    ...
    builtin_func_map[jl_f__apply] =
            jlcall_func_to_llvm("jl_f__apply", &jl_f__apply, m);
    ...
    builtin_func_map[jl_f_throw] =
            jlcall_func_to_llvm("jl_f_throw", &jl_f_throw, m);
    builtin_func_map[jl_f_tuple] =
            jlcall_func_to_llvm("jl_f_tuple", &jl_f_tuple, m);
    builtin_func_map[jl_f_svec] =
            jlcall_func_to_llvm("jl_f_svec", &jl_f_svec, m);
    builtin_func_map[jl_f_applicable] =
            jlcall_func_to_llvm("jl_f_applicable", &jl_f_applicable, m);
    builtin_func_map[jl_f_invoke] =
            jlcall_func_to_llvm("jl_f_invoke", &jl_f_invoke, m);
    builtin_func_map[jl_f_invoke_kwsorter] = jlcall_func_to_llvm(
            "jl_f_invoke_kwsorter", &jl_f_invoke_kwsorter, m);
    ...
    builtin_func_map[jl_f_nfields] =
            jlcall_func_to_llvm("jl_f_nfields", &jl_f_nfields, m);
    builtin_func_map[jl_f__expr] =
            jlcall_func_to_llvm("jl_f__expr", &jl_f__expr, m);
    ...
    jldepwarnpi_func = Function::Create(
            FunctionType::get(T_void, argsdepwarnpi, false),
            Function::ExternalLinkage,
            "jl_depwarn_partial_indexing",
            m);
    add_named_global(jldepwarnpi_func, &jl_depwarn_partial_indexing);

    std::vector<Type *> args_1ptr(0);
    args_1ptr.push_back(T_prjlvalue);
    queuerootfun = Function::Create(
            FunctionType::get(T_void, args_1ptr, false),
            Function::ExternalLinkage,
            "jl_gc_queue_root",
            m);
    ...
    jlapplygeneric_func = Function::Create(
            FunctionType::get(T_prjlvalue, agargs, false),
            Function::ExternalLinkage,
            "jl_apply_generic",
            m);
    ...
    jlinvoke_func = Function::Create(
            FunctionType::get(T_prjlvalue, invokeargs, false),
            Function::ExternalLinkage,
            "jl_invoke",
            m);
    ...
    args_topeval.push_back(T_prjlvalue);
    jltopeval_func = Function::Create(
            FunctionType::get(T_pjlvalue, args_topeval, false),
            Function::ExternalLinkage,
            "jl_toplevel_eval",
            m);
    ...
    jlcopyast_func = Function::Create(
            FunctionType::get(T_prjlvalue, args_copyast, false),
            Function::ExternalLinkage,
            "jl_copy_ast",
            m);
    ...
    jlnsvec_func = Function::Create(
            FunctionType::get(T_pjlvalue, args5, true),
            Function::ExternalLinkage,
            "jl_svec",
            m);
    ...
    mdargs.push_back(T_prjlvalue);
    mdargs.push_back(T_prjlvalue);
    jlmethod_func = Function::Create(
            FunctionType::get(T_void, mdargs, false),
            Function::ExternalLinkage,
            "jl_method_def",
            m);
    ...
    jlgenericfunction_func = Function::Create(
            FunctionType::get(T_prjlvalue, funcdefargs, false),
            Function::ExternalLinkage,
...
#ifdef _OS_WINDOWS_
    resetstkoflw_func = Function::Create(
            FunctionType::get(T_int32, false),
            Function::ExternalLinkage,
            "_resetstkoflw",
            m);
    add_named_global(resetstkoflw_func, &_resetstkoflw);
#if defined(_CPU_X86_64_)
    juliapersonality_func = Function::Create(
            FunctionType::get(T_int32, true),
            Function::ExternalLinkage,
            "__julia_personality",
            m);
...
#if defined(_CPU_X86_64_)
#if defined(_COMPILER_MINGW_)
    Function *chkstk_func = Function::Create(
            FunctionType::get(T_void, false),
            Function::ExternalLinkage,
            "___chkstk_ms",
            m);
    ...
    jlleave_func = Function::Create(
            FunctionType::get(T_void, lhargs, false),
            Function::ExternalLinkage,
            "jl_pop_handler",
            m);
    ...
    args_2vals_callee_rooted.push_back(
            PointerType::get(T_jlvalue, AddressSpace::CalleeRooted));
    jlegal_func = Function::Create(
            FunctionType::get(T_int32, args_2vals_callee_rooted, false),
            Function::ExternalLinkage,
            "jl_egal",
            m);
    ...
    args_2vals_tracked.push_back(T_prjlvalue);
    jlisa_func = Function::Create(
            FunctionType::get(T_int32, args_2vals_tracked, false),
            Function::ExternalLinkage,
            "jl_isa",
            m);
    add_named_global(jlisa_func, &jl_isa);

    jlsubtype_func = Function::Create(
            FunctionType::get(T_int32, args_2vals_tracked, false),
            Function::ExternalLinkage,
            "jl_subtype",
            m);
    ...
    jltypeassert_func = Function::Create(
            FunctionType::get(T_void, args_2vals_tracked, false),
            Function::ExternalLinkage,
            "jl_typeassert",
            m);
    ...
    applytype_args.push_back(T_prjlvalue);
    applytype_args.push_back(
            PointerType::get(T_prjlvalue, AddressSpace::Derived));
    jlapplytype_func = Function::Create(
            FunctionType::get(T_pjlvalue, applytype_args, false),
            Function::ExternalLinkage,
    ...
    alloc_pool_args.push_back(T_int32);
    jlalloc_pool_func = Function::Create(
            FunctionType::get(T_prjlvalue, alloc_pool_args, false),
            Function::ExternalLinkage,
            "jl_gc_pool_alloc",
            m);
    ...
    jlalloc_big_func = Function::Create(
            FunctionType::get(T_prjlvalue, alloc_big_args, false),
            Function::ExternalLinkage,
            "jl_gc_big_alloc",
            m);
    ...
    jldlsym_func = Function::Create(
            FunctionType::get(T_pvoidfunc, dlsym_args, false),
            Function::ExternalLinkage,
            "jl_load_and_lookup",
            m);
    ...
    jlnewbits_func = Function::Create(
            FunctionType::get(T_pjlvalue, newbits_args, false),
            Function::ExternalLinkage,
            "jl_new_bits",
            m);
    ...
    jlgetnthfieldchecked_func = Function::Create(
            FunctionType::get(T_prjlvalue, getnthfld_args, false),
            Function::ExternalLinkage,
            "jl_get_nth_field_checked",
            m);
    add_named_global(jlgetnthfieldchecked_func, *jl_get_nth_field_checked);

    diff_gc_total_bytes_func = Function::Create(
            FunctionType::get(T_int64, false),
            Function::ExternalLinkage,
    ...
    jlarray_data_owner_func = Function::Create(
            FunctionType::get(T_prjlvalue, array_owner_args, false),
            Function::ExternalLinkage,
            "jl_array_data_owner",
            m);
...
#if JL_LLVM_VERSION >= 50000
                    .addAttribute(
                            jlarray_data_owner_func->getContext(),
                            AttributeList::FunctionIndex,
                            Attribute::ReadOnly)
                    .addAttribute(
                            jlarray_data_owner_func->getContext(),
    ...
    gcroot_flush_func = Function::Create(
            FunctionType::get(T_void, false),
            Function::ExternalLinkage,
            "julia.gcroot_flush");
    ...
    add_named_global(
            pointer_from_objref_func, (void *)NULL, /*dllimport*/ false);

    except_enter_func = Function::Create(
            FunctionType::get(T_int32, false),
            Function::ExternalLinkage,
            "julia.except_enter");
    ...
    jl_globalPM->add(new TargetLibraryInfoWrapperPass(
            Triple(jl_TargetMachine->getTargetTriple())));
...
static inline void
checkARMArchFeature(std::string &cpu, StringMap<bool> &HostFeatures)
{
#if defined(_CPU_ARM_)
    if (cpu == "generic") {
...
#else
    // These two are allowed on 32bit. Allow them on 64bits too for consistency
    // since
    // they basically mean "generic" on aarch64.
    // In particular, "armv8-a" is the generic value for the GCC `-march`
    // option.
...
extern "C" void *jl_init_llvm(void)
{
    ...
    cl::ParseCommandLineOptions(
            sizeof(argv_tailmerge) / sizeof(argv_tailmerge[0]),
            argv_tailmerge,
            "disable-tail-merge\n");
#if defined(_OS_WINDOWS_) && defined(_CPU_X86_64_)
    const char *const argv_copyprop[] = {
        "", "-disable-copyprop"
    }; // llvm bug 21743
    cl::ParseCommandLineOptions(
            sizeof(argv_copyprop) / sizeof(argv_copyprop[0]),
            argv_copyprop,
            "disable-copyprop\n");
#endif
...
// options.PrintMachineCode = true; //Print machine code produced during JIT
// compiling
#if defined(_OS_WINDOWS_) && !defined(_CPU_X86_64_)
...
#else
            .setOptLevel(
                    jl_options.opt_level == 0 ? CodeGenOpt::None
    ...
    jl_TargetMachine = eb.selectTarget(TheTriple, "", TheCPU, targetFeatures);
...
extern "C" void jl_init_codegen(void)
{
    ...
    BOX_F(float32, float32);
    BOX_F(float64, float64);
    BOX_F(char, char);
    UBOX_F(ssavalue, size);

    box8_func = boxfunc_llvm(
            ft2arg(T_pjlvalue, T_pjlvalue, T_int8), "jl_box8", &jl_box8, m);
    box16_func = boxfunc_llvm(
            ft2arg(T_pjlvalue, T_pjlvalue, T_int16), "jl_box16", &jl_box16, m);
    box32_func = boxfunc_llvm(
            ft2arg(T_pjlvalue, T_pjlvalue, T_int32), "jl_box32", &jl_box32, m);
    box64_func = boxfunc_llvm(
            ft2arg(T_pjlvalue, T_pjlvalue, T_int64), "jl_box64", &jl_box64, m);
...
{
#if JL_LLVM_VERSION >= 50000
    cast<Instruction>(((Value *)v))
            ->getParent()
            ->getParent()
            ->print(llvm::dbgs(), nullptr, true);
#else
