...
static const int jl_gc_sizeclasses[JL_GC_N_POOLS] = {
    ...
    // the following tables are computed for maximum packing efficiency via the
    // formula:
...
STATIC_INLINE int JL_CONST_FUNC jl_gc_szclass(size_t sz)
{
    ...
    const int N = 0;
#elif defined(_CPU_ARM_) || defined(_CPU_PPC_)
    if (sz <= 8) return (sz + 3) / 4 - 1;
    const int N = 1;
#else
    if (sz <= 12) return (sz + 3) / 4 - 1;
    const int N = 2;
#endif
    if (sz <= 256) return (sz + 15) / 16 + N;
    if (sz <= 496) return 16 - 16376 / 4 / LLT_ALIGN(sz, 16 * 4) + 16 + N;
    if (sz <= 1008) return 16 - 16376 / 2 / LLT_ALIGN(sz, 16 * 2) + 24 + N;
...
STATIC_INLINE void jl_gc_wb_buf(void *parent, void *bufptr,
                                size_t minsz) // parent isa jl_value_t*
{
...
jl_array_t *jl_new_array_for_deserialization(jl_value_t *atype, uint32_t ndims,
                                             size_t *dims, int isunboxed, int elsz);
...
void jl_dump_native(const char *bc_fname, const char *obj_fname, const char *sysimg_data,
                    size_t sysimg_len);
...
STATIC_INLINE void jl_free_aligned(void *p) { _aligned_free(p); }
#else
STATIC_INLINE void *jl_malloc_aligned(size_t sz, size_t align)
{
#if defined(_P64) || defined(__APPLE__)
    if (align <= 16) return malloc(sz);
#endif
    void *ptr;
...
{
#if defined(_P64) || defined(__APPLE__)
    if (align <= 16) return realloc(d, sz);
#endif
...
STATIC_INLINE void jl_free_aligned(void *p) { free(p); }
#endif
...
jl_typemap_entry_t *jl_typemap_entry_assoc_exact(jl_typemap_entry_t *mn, jl_value_t **args,
                                                 size_t n);
STATIC_INLINE jl_typemap_entry_t *jl_typemap_assoc_exact(union jl_typemap_t ml_or_cache,
...
int jl_typemap_intersection_visitor(union jl_typemap_t a, int offs,
