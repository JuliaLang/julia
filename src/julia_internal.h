...
STATIC_INLINE jl_value_t *jl_gc_permobj(size_t sz, void *ty)
{
    ...
    unsigned align =
            (sz == 0 ? sizeof(void *)
                     : (allocsz <= sizeof(void *) * 2 ? sizeof(void *) * 2
                                                      : 16));
    jl_taggedvalue_t *o = (jl_taggedvalue_t *)jl_gc_perm_alloc(
            allocsz, 0, align, sizeof(void *) % align);
...
// Returns a int32 where the high 16 bits are a lower bound of the number of
// non-pointer fields
// at the beginning of the type and the low 16 bits are a lower bound on the
// number of non-pointer
// fields at the end of the type. This field only exists for a layout that has
// at least one
// pointer fields.
...
jl_code_info_t *
jl_type_infer(jl_method_instance_t **li, size_t world, int force);
jl_generic_fptr_t
jl_generate_fptr(jl_method_instance_t *li, void *F, size_t world);
...
jl_llvm_functions_t
jl_compile_for_dispatch(jl_method_instance_t **li, size_t world);
JL_DLLEXPORT int jl_compile_hint(jl_tupletype_t *types);
jl_code_info_t *jl_new_code_info_from_ast(jl_expr_t *ast);

STATIC_INLINE jl_value_t *
jl_compile_method_internal(jl_generic_fptr_t *fptr, jl_method_instance_t *meth)
{
    ...
    if (__unlikely(fptr->fptr == NULL || fptr->jlcall_api == 0)) {
        ...
        if (meth->jlcall_api == 2)
            return jl_assume(meth->inferred_const);
        // if it hasn't been inferred, try using the unspecialized meth cache
        // instead
        if (!meth->inferred) {
            fptr->fptr = meth->unspecialized_ducttape;
            fptr->jlcall_api = 1;
            if (!fptr->fptr) {
                if (jl_is_method(meth->def.method) &&
                    !meth->def.method->isstaged &&
                    meth->def.method->unspecialized) {
                    fptr->fptr = meth->def.method->unspecialized->fptr;
                    fptr->jlcall_api =
                            meth->def.method->unspecialized->jlcall_api;
                    if (fptr->jlcall_api == 2) {
                        return jl_assume(meth->def.method->unspecialized
                                                 ->inferred_const);
                    }
                }
            }
...
STATIC_INLINE jl_value_t *jl_call_fptr_internal(
        const jl_generic_fptr_t *fptr,
...
STATIC_INLINE jl_value_t *jl_call_method_internal(
        jl_method_instance_t *meth, jl_value_t **args, uint32_t nargs)
{
...
JL_DLLEXPORT jl_value_t *
jl_apply_2va(jl_value_t *f, jl_value_t **args, uint32_t nargs);
...
STATIC_INLINE void
jl_gc_wb_binding(jl_binding_t *bnd, void *val) // val isa jl_value_t*
{
    if (__unlikely(
                jl_astaggedvalue(bnd)->bits.gc == 3 &&
                (jl_astaggedvalue(val)->bits.gc & 1) == 0))
        gc_queue_binding(bnd);
}

STATIC_INLINE void
jl_gc_wb_buf(void *parent, void *bufptr, size_t minsz) // parent isa jl_value_t*
{
...
jl_value_t *
jl_get_exceptionf(jl_datatype_t *exception_type, const char *fmt, ...);

JL_DLLEXPORT void jl_typeassert(jl_value_t *x, jl_value_t *t);

#define JL_CALLABLE(name)                                                      \
    JL_DLLEXPORT jl_value_t *name(                                             \
...
jl_value_t *jl_type_intersection_env_s(
        jl_value_t *a, jl_value_t *b, jl_svec_t **penv, int *issubty);
jl_value_t *
jl_type_intersection_env(jl_value_t *a, jl_value_t *b, jl_svec_t **penv);
...
jl_value_t *jl_call_scm_on_ast(
        const char *funcname, jl_value_t *expr, jl_module_t *inmodule);
...
jl_method_instance_t *jl_method_lookup(
        ...
        size_t world);
jl_value_t *
jl_gf_invoke(jl_tupletype_t *types, jl_value_t **args, size_t nargs);
...
jl_array_t *jl_new_array_for_deserialization(
        jl_value_t *atype,
        uint32_t ndims,
        size_t *dims,
        int isunboxed,
...
#ifdef LIBOSXUNWIND
size_t
rec_backtrace_ctx_dwarf(uintptr_t *data, size_t maxsize, bt_context_t *ctx);
#endif
JL_DLLEXPORT jl_value_t *jl_get_backtrace(void);
JL_DLLEXPORT jl_value_t *jl_apply_with_saved_exception_state(
        jl_value_t **args, uint32_t nargs, int drop_exceptions);
...
JL_DLLEXPORT jl_value_t *
jl_pointerset(jl_value_t *p, jl_value_t *x, jl_value_t *align, jl_value_t *i);
...
JL_DLLEXPORT jl_value_t *
jl_fma_float(jl_value_t *a, jl_value_t *b, jl_value_t *c);
JL_DLLEXPORT jl_value_t *
jl_muladd_float(jl_value_t *a, jl_value_t *b, jl_value_t *c);
...
JL_DLLEXPORT jl_value_t *
jl_select_value(jl_value_t *isfalse, jl_value_t *a, jl_value_t *b);
...
STATIC_INLINE void *
jl_realloc_aligned(void *p, size_t sz, size_t oldsz, size_t align)
{
    (void)oldsz;
...
STATIC_INLINE void *
jl_realloc_aligned(void *d, size_t sz, size_t oldsz, size_t align)
{
#if defined(_P64) || defined(__APPLE__)
    if (align <= 16)
        return realloc(d, sz);
...
jl_typemap_entry_t *jl_typemap_assoc_by_type(
        ...
        int8_t offs,
        size_t world);
static jl_typemap_entry_t *const INEXACT_ENTRY =
        (jl_typemap_entry_t *)(uintptr_t)-1;
...
jl_typemap_entry_t *jl_typemap_entry_assoc_exact(
        jl_typemap_entry_t *mn, jl_value_t **args, size_t n, size_t world);
...
int jl_typemap_visitor(
        union jl_typemap_t a, jl_typemap_visitor_fptr fptr, void *closure);
...
struct typemap_intersection_env {
    ...
    jl_value_t
            *va; // the tparam0 for the vararg in type, if applicable (or NULL)
    // output values
    jl_value_t *ti; // intersection type
    jl_svec_t *env; // intersection env (initialize to null to perform
                    // intersection without an environment)
    int issubty;    // if `a <: b` is true in `intersect(a,b)`
};
int jl_typemap_intersection_visitor(
        union jl_typemap_t a,
        int offs,
...
extern jl_sym_t *amp_sym;
extern jl_sym_t *module_sym;
extern jl_sym_t *colons_sym;
