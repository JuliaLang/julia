...
static void jl_rec_backtrace(jl_task_t *t) JL_NOTSAFEPOINT
{
    ...
    if (context == NULL && (!t->copy_stack && t->started && t->stkbuf != NULL)) {
      ...
      if (jl_ptr_demangle_available()) {
        ...
        if (valid_longjmp_target(mc->gregs[REG_ESP], mc->gregs[REG_EIP]))
            context = &c;
      #elif defined(_CPU_X86_64_)
        ...
        if (valid_longjmp_target(mc->gregs[REG_RSP], mc->gregs[REG_RIP]))
            context = &c;
      #elif defined(_CPU_ARM_)
        ...
        mc->arm_pc = mc->arm_lr;
        if (valid_longjmp_target(mc->arm_sp, mc->arm_pc))
            context = &c;
      #elif defined(_CPU_AARCH64_)
        ...
        mc->pc = mc->regs[30];
        if (valid_longjmp_target(mc->sp, mc->pc))
            context = &c;
      #else
