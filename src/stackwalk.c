...
/*
  stackwalk.c
  utilities for walking the stack and looking up information about code
  addresses
*/
#include "julia.h"
...
size_t jl_unw_stepn(bt_cursor_t *cursor, uintptr_t *ip, uintptr_t *sp, size_t maxsize)
{
    ...
    if (!jl_setjmp(buf, 0)) {
        ...
        while (1) {
            ...
            if (!jl_unw_step(cursor, &ip[n], sp ? &sp[n] : &nullsp)) break;
            n++;
        }
        n++;
#if !defined(_OS_WINDOWS_)
...
size_t rec_backtrace_ctx(uintptr_t *data, size_t maxsize, bt_context_t *context)
{
    size_t      n = 0;
    bt_cursor_t cursor;
    if (!jl_unw_init(&cursor, context)) return 0;
    ...
    if (jl_unw_init(&cursor, &context)) {
        ...
        do {
            ...
            n = jl_unw_stepn(&cursor, (uintptr_t *)jl_array_data(ip) + offset,
                             returnsp ? (uintptr_t *)jl_array_data(sp) + offset : NULL,
                             maxincr);
            offset += maxincr;
        } while (n > maxincr);
...
static int jl_unw_init(bt_cursor_t *cursor, bt_context_t *Context)
{
    ...
    BOOL result = StackWalk64(IMAGE_FILE_MACHINE_I386, GetCurrentProcess(), hMainThread,
                              ...
                              JuliaFunctionTableAccess64, JuliaGetModuleBase64, NULL);
    jl_in_stackwalk = 0;
    return result;
#else
    *cursor = *Context;
...
static int readable_pointer(LPCVOID pointer)
{
    ...
    if (VirtualQuery(pointer, &mInfo, sizeof(MEMORY_BASIC_INFORMATION)) == 0) return 0;
...
// Might be called from unmanaged thread.
    ...
    *sp = (uintptr_t)cursor->stackframe.AddrStack.Offset;
    if (*ip == 0 || *ip == ((uintptr_t)0) - 1) {
        if (!readable_pointer((LPCVOID)*sp)) return 0;
    ...
    *sp = (uintptr_t)cursor->Rsp;
    ...
    if (!FunctionEntry) {          // assume this is a NO_FPO RBP-based function
        ...
        if (!readable_pointer((LPCVOID)cursor->Rsp)) return 0;
...
{
    unw_word_t reg;
    if (unw_get_reg(cursor, UNW_REG_IP, &reg) < 0) return 0;
    *ip = reg;
    if (unw_get_reg(cursor, UNW_REG_SP, &reg) < 0) return 0;
    *sp = reg;
...
size_t rec_backtrace_ctx_dwarf(uintptr_t *data, size_t maxsize, bt_context_t *context)
{
    size_t      n;
    bt_cursor_t cursor;
    if (!jl_unw_init_dwarf(&cursor, context)) return 0;
    n = jl_unw_stepn(&cursor, data, NULL, maxsize);
    return n > maxsize ? maxsize : n;
}
#endif
                              ...
                              size_t ip, int skipC, int skipInline)
{
    ...
    jl_getFunctionInfo(func_name, file_name, line_num, inlinedat_file,
inlinedat_line, outer_linfo,
...
JL_DLLEXPORT void jlbacktrace(void)
{
    ...
    for (i = 0; i < n; i++) jl_gdblookup(ptls->bt_data[i] - 1);
}
