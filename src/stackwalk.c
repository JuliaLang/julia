...
NOINLINE size_t rec_backtrace(jl_bt_element_t *bt_data, size_t maxsize, int skip, int no_gcstack) JL_NOTSAFEPOINT
{
    ...
    if (!jl_unw_init(&cursor, &context) || maxsize == 0)
        return 0;
    jl_gcframe_t *pgcstack = no_gcstack ? NULL : jl_pgcstack;
    size_t bt_size = 0;
...
JL_DLLEXPORT jl_record_backtrace_result_t jl_record_backtrace(jl_task_t *t, jl_bt_element_t *bt_data, size_t max_bt_size, int all_tasks_profiler) JL_NOTSAFEPOINT
{
    ...
    if (t == ct) {
        result.bt_size = rec_backtrace(bt_data, max_bt_size, 0, 0);
        result.tid = tid;
        return result;
    }
    ...
    if (context) {
        if (all_tasks_profiler) {
            bt_size = rec_backtrace_ctx(bt_data, max_bt_size, context, NULL);
        }
        else {
