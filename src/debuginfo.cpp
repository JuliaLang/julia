...
extern "C" JL_DLLEXPORT void jl_lock_profile(void)
{
    uv_rwlock_rdlock(&threadsafe);
}

extern "C" JL_DLLEXPORT void jl_unlock_profile(void)
{
...
JL_DLLEXPORT void jl_register_fptrs(uint64_t sysimage_base, const jl_sysimg_fptrs_t *fptrs,
...
JL_DLLEXPORT int jl_getFunctionInfo(jl_frame_t **frames_out, size_t pointer, int skipC, int noInline) JL_NOTSAFEPOINT
{
    // This function is not allowed to reference any TLS variables if noInline
    // since it can be called from an unmanaged thread on OSX.
