...
#else
            if (Section->isText(isText) || !isText)
                continue;
            Section->getAddress(SectionAddr);
            Section->getAddress(SectionLoadAddr);
#endif
...
// GOAL: Assign errorobj
            ...
            *obj         = (llvm::object::MachOObjectFile *)origerrorobj;
            ...
            // First find the uuid of the object file (we'll use this to make sure we find
            // the
            ...
            // we can't have nice things (easily). For now we only support .dSYM files in
            // the same directory
            // as the shared library. In the future we may use DBGCopyFullDSYMURLForUUID
            // from CoreFoundation to make
...
// On Linux systems we need to mmap another copy because of the permissions on the mmap'ed
// shared library.
// On Windows we need to mmap another copy since reading the in-memory copy seems to return
