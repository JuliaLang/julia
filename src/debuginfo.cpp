...
extern "C" void jl_init_debuginfo() { uv_rwlock_init(&threadsafe); }
                                     ...
                                     uint8_t *UnwindData)
{
    ...
    if (!catchjmp[0]) {
        ...
        *(DWORD *)&UnwindData[8] =
            (DWORD)(catchjmp - Section); // relative location of catchjmp
    ...
    if (0) {
        ...
        if (mod_size &&
            ...
            if (len > MAX_SYM_NAME) len = MAX_SYM_NAME;
...
{
    if (triggered_linfos.empty()) return;
    if (jl_linfo_tracer) {
...
public:
                                       ...
                                       const EmittedFunctionDetails &Details)
    {
        ...
        if (linfo_it != linfo_in_flight.end()) {
            ...
            if (((Function *)linfo->functionObjectsDecls.functionObject)
                    ->getName()
                    .equals(sName))
...
#ifndef KEEP_BODIES
        if (!jl_generating_output()) const_cast<Function *>(&F)->deleteBody();
#endif
...
#endif
    {
        ...
        auto getLoadAddress = [&](const StringRef &sName) -> uint64_t {
            ...
            if (search == loadedSections.end()) return 0;
        ...
        *(DWORD *)&UnwindData[SectionLoadOffset + 8] = (DWORD)(
        ...
        for (const auto &sym_size : symbols) {
            ...
            if (linfo_it != linfo_in_flight.end()) {
                linfo = linfo_it->second;
                if (linfo->compile_traced) triggered_linfos.push_back(linfo);
                linfo_in_flight.erase(linfo_it);
                if (((Function *)linfo->functionObjectsDecls.functionObject)
                        ->getName()
                        .equals(sName))
            ...
            if (linfo) linfomap[Addr] = std::make_pair(Size, linfo);
            if (first) {
                ObjectInfo tmp = {
                    &debugObj, (size_t)SectionSize,
        ...
        for (const object::SymbolRef &sym_iter : obj.symbols()) {
            ...
            if (!Addr && sName[0] == '_') {
                Addr =
                    ((MCJIT *)jl_ExecutionEngine)->getSymbolAddress(sName.substr(1), true);
            }
            if (!Addr) continue;
#endif
...
extern "C" char *jl_demangle(const char *name)
{
...
static int lookup_pointer(DIContext *context, jl_frame_t **frames, size_t pointer,
                          int demangle, int noInline)
{
    ...
    for (int i = 0; i < n_frames; i++) {
        ...
        if (!frame->func_name || !func_name.compare(0, 7, "jlcall_") ||
            !func_name.compare(0, 7, "jlcapi_")) {
            frame->fromC = 1;
        }
    }
    return n_frames;
...
extern "C" void jl_register_fptrs(uint64_t sysimage_base, void **fptrs,
                                  jl_lambda_info_t **linfos, size_t n)
{
...
bool jl_dylib_DI_for_fptr(size_t pointer, const llvm::object::ObjectFile **obj,
                          llvm::DIContext **context, int64_t *slide, int64_t *section_slide,
                          bool onlySysImg, bool *isSysImg, void **saddr, char **name,
                          char **filename)
{
    *obj           = NULL;
    *context       = NULL;
...
// Assigning fname, fbase, msize
    ...
    if (isvalid) {
        ...
        if (isSysImg) *isSysImg = insysimage;
        ...
        if (SymFromAddr(GetCurrentProcess(), dwAddress, &dwDisplacement64, pSymbol)) {
            ...
            if (name) jl_copy_str(name, pSymbol->Name);
            if (saddr) *saddr = (void *)(uintptr_t)pSymbol->Address;
        }
        else if (saddr) {
            *saddr = NULL;
        }
        ...
        if (filename && !*filename) jl_copy_str(filename, fname);
    ...
    if (dladdr_success && dlinfo.dli_fname) {
...
#endif
        const char *fname;
        if (saddr) *saddr       = dlinfo.dli_saddr;
        ...
        if (name) jl_copy_str(name, dlinfo.dli_sname);
        if (filename) jl_copy_str(filename, dlinfo.dli_fname);
...
// GOAL: Assign errorobj
            ...
            MemoryBuffer *membuf = MemoryBuffer::getMemBuffer(
                StringRef((const char *)fbase, msize), "", false);
            llvm::object::ObjectFile *origerrorobj =
                llvm::object::ObjectFile::createObjectFile(membuf);
#endif
            if (!origerrorobj) {
            ...
            *obj         = (llvm::object::MachOObjectFile *)origerrorobj;
            ...
            // First find the uuid of the object file (we'll use this to make sure we
            // find the
            ...
            // On OS X debug symbols are not contained in the dynamic library and
            // that's why
            // we can't have nice things (easily). For now we only support .dSYM files
            // in the same directory
            // as the shared library. In the future we may use
            // DBGCopyFullDSYMURLForUUID from CoreFoundation to make
...
#else
            llvm::object::ObjectFile *errorobj =
                llvm::object::ObjectFile::createObjectFile(dsympath);
#endif

#else // ifndef _OS_DARWIN_

// On Linux systems we need to mmap another copy because of the permissions on
// the mmap'ed shared library.
// On Windows we need to mmap another copy since reading the in-memory copy
// seems to return object_error:unexpected_eof
#ifdef LLVM35
            auto errorobj = llvm::object::ObjectFile::createObjectFile(fname);
#else
            llvm::object::ObjectFile *errorobj =
                llvm::object::ObjectFile::createObjectFile(fname);
#endif
...
#else
                *context =
                    DIContext::getDWARFContext(const_cast<object::ObjectFile *>(*obj));
#endif
                    *slide = -(int64_t)fbase;
#ifdef _OS_DARWIN_
                }
                else {
                    // If we're here the, the dsym does not match the dylib. Use the
                    // original
                    // object instead. For consistency (and to make sure we get a sensible
                    // size
                    // for the memory buffer), we also use a fresh copy mapped from
                    // the file rather than reusing the one in memory. We may want to
                    // revisit
                                   ...
                                   int noInline)
{
    ...
    if (!jl_dylib_DI_for_fptr(pointer, &object, &context, &slide, &section_slide, skipC,
                              &isSysImg, &saddr, &frame0->func_name, &frame0->file_name)) {
        frame0->fromC = 1;
        return 1;
    }
    ...
    if (isSysImg && sysimg_fvars) {
        ...
        unw_proc_info_t pip;
        if (!saddr &&
            unw_get_proc_info_by_ip(unw_local_addr_space, pointer, &pip, NULL) == 0)
        ...
        if (!saddr) {
            ...
            if (fn) saddr        = (void *)(ImageBase + fn->BeginAddress);
        }
#endif
        if (saddr) {
                   ...
                   )
{
    ...
    *slide    = 0;
    ...
    if (fit != fmap.end() && fptr < fit->first + fit->second.lengthAdr) {
        if (symsize) *symsize = fit->second.lengthAdr;
    ...
    if (fit != objmap.end() && fptr < fit->first + fit->second.SectionSize) {
        if (symsize) *symsize             = 0;
        if (section_slide) *section_slide = fit->second.slide;
...
#ifdef USE_MCJIT
extern "C" JL_DLLEXPORT jl_value_t *jl_get_dobj_data(uint64_t fptr)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    // Used by Gallium.jl
    ...
    jl_gc_safe_leave(ptls, gc_state);
    if (object == NULL) return jl_nothing;
...
extern "C" JL_DLLEXPORT uint64_t jl_get_section_start(uint64_t fptr)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    // Used by Gallium.jl
    int8_t gc_state = jl_gc_safe_enter(ptls);
...
#if defined(LLVM37) &&                                                                     \
    (defined(_OS_LINUX_) || (defined(_OS_DARWIN_) && defined(LLVM_SHLIB)))
...
/*
 * We use a custom unwinder, so we need to make sure that when registering
 * dynamic
 * frames, we do so with our unwinder rather than with the system one. If LLVM
 * is
...
template <typename T> static T parse_leb128(const uint8_t *&Addr, const uint8_t *End)
{
...
template <typename U, typename T> static U safe_trunc(T t)
{
...
void register_eh_frames(uint8_t *Addr, size_t Size)
{
    ...
    processFDEs((char *)Addr, Size, [&](const char *Entry) {
        ...
        if (cie_addr != cur_cie) encoding = parseCIE(cie_addr, Addr + Size);
        ...
        if (start < start_ip) start_ip      = start;
        if (end_ip < (start + size)) end_ip = start + size;
...
public:
    ...
    {
        return JMM->allocateGlobal(Size, Alignment);
    }
    virtual void deallocateFunctionBody(void *Body)
    {
        return JMM->deallocateFunctionBody(Body);
    }
    virtual uint8_t *startExceptionTable(const Function *F, uintptr_t &ActualSize)
    {
        return JMM->startExceptionTable(F, ActualSize);
    }
    virtual void endExceptionTable(const Function *F, uint8_t *TableStart,
                                   uint8_t *TableEnd, uint8_t *FrameRegister)
    {
        return JMM->endExceptionTable(F, TableStart, TableEnd, FrameRegister);
    }
    virtual void deallocateExceptionTable(void *ET)
    {
        return JMM->deallocateExceptionTable(ET);
    }
    ...
    {
        uint8_t *mem =
            JMM->allocateCodeSection(Size + 48, Alignment, SectionID, SectionName);
        mem[Size] = 0;
        return mem;
    }
    virtual uint8_t *allocateDataSection(uintptr_t Size, unsigned Alignment,
                                         ...
                                         bool IsReadOnly)
    {
        return JMM->allocateDataSection(Size, Alignment, SectionID, SectionName,
                                        IsReadOnly);
    }
#else
    virtual uint8_t *allocateCodeSection(uintptr_t Size, unsigned Alignment,
    ...
    {
        return JMM->applyPermissions(ErrMsg);
    }
    virtual void registerEHFrames(StringRef SectionData)
    {
        return JMM->registerEHFrames(SectionData);
    }
};
JITMemoryManager *createJITMemoryManagerWin() { return new JITMemoryManagerWin(); }
#endif
