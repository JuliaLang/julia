...
static TargetData<1> arg_target_data(const TargetData<1> &arg, bool require_host)
{
    ...
    if (res.name == "native") {
        ...
        // llvm getHostCPUFeatures is not implemented for riscv
        // assume g(macfd)c for now
        std::string features = jl_get_cpu_features_llvm() + "+m,+a,+c,+f,+d";
        append_ext_features(res.ext_features, features);
    }
    else {
...
{
    // we currently assume g(macfd)c, which has 32 and 64-bits fma
    return jl_true;
}
