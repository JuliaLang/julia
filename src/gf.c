...
JL_DLLEXPORT jl_value_t *jl_invoke(jl_lambda_info_t *meth, jl_value_t **args,
                                   uint32_t nargs)
{
...
JL_DLLEXPORT int8_t jl_is_in_pure_context(void) { return in_pure_callback; }
...
const struct jl_typemap_info lambda_cache = {0, &jl_lambda_info_type};
...
JL_DLLEXPORT jl_lambda_info_t *jl_get_specialized(jl_method_t *m, jl_tupletype_t *types,
                                                  jl_svec_t *sp);

// get or create the LambdaInfo for a specialization
JL_DLLEXPORT jl_lambda_info_t *
jl_specializations_get_linfo(jl_method_t *m, jl_tupletype_t *type, jl_svec_t *sparams)
{
    JL_LOCK(&m->writelock);
    jl_typemap_entry_t *sf = jl_typemap_assoc_by_type(m->specializations, type, NULL, 1,
                                                      /*subtype*/ 0, /*offs*/ 0);
    if (sf && jl_is_lambda_info(sf->func.value) &&
        ((jl_lambda_info_t *)sf->func.value)->code != jl_nothing) {
    ...
    jl_typemap_insert(&m->specializations, (jl_value_t *)m, type, jl_emptysvec, NULL,
                      jl_emptysvec, (jl_value_t *)li, 0, &tfunc_cache, NULL);
    JL_UNLOCK(&m->writelock);
    JL_GC_POP();
    return li;
}
...
{
    jl_typemap_entry_t *sf = jl_typemap_assoc_by_type(m->specializations, type, NULL, 2,
                                                      /*subtype*/ 0, /*offs*/ 0);
    if (!sf) return jl_nothing;
    return sf->func.value;
}

JL_DLLEXPORT jl_value_t *jl_methtable_lookup(jl_methtable_t *mt, jl_tupletype_t *type)
{
    jl_typemap_entry_t *sf = jl_typemap_assoc_by_type(mt->defs, type, NULL, 2,
                                                      /*subtype*/ 0, /*offs*/ 0);
    if (!sf) return jl_nothing;
    return sf->func.value;
}
...
static jl_function_t *jl_new_generic_function_with_supertype(jl_sym_t *     name,
                                                             jl_module_t *  module,
                                                             jl_datatype_t *st, int iskw);
jl_value_t *jl_mk_builtin_func(const char *name, jl_fptr_t fptr)
{
    jl_sym_t *  sname = jl_symbol(name);
    jl_value_t *f =
        jl_new_generic_function_with_supertype(sname, jl_core_module, jl_builtin_type, 0);
    ...
    jl_typemap_insert(&mt->cache, (jl_value_t *)mt, jl_anytuple_type, jl_emptysvec, NULL,
                      jl_emptysvec, (jl_value_t *)li, 0, &lambda_cache, NULL);
    return f;
}
...
jl_lambda_info_t *jl_type_infer(jl_lambda_info_t *li, int force)
{
    ...
    if (li->def != NULL) mod = li->def->module;
    static int inInference   = 0;
    int        lastIn        = inInference;
    inInference              = 1;
    if (jl_typeinf_func != NULL &&
        (force || (mod != jl_gf_mtable(jl_typeinf_func)->module &&
                   (mod != jl_core_module || !lastIn)))) { // avoid any potential
                                                           // recursion in calling
                                                           // jl_typeinf_func on
                                                           // itself
        ...
        assert(li->def ||
               li->inInference ==
                   0);            // if this is toplevel expr, make sure inference finished
...
JL_DLLEXPORT void jl_set_lambda_rettype(jl_lambda_info_t *li, jl_value_t *rettype)
{
    ...
    assert(!li->inferred ||
           li->functionObjectsDecls.functionObject ==
               NULL);            // protect against some double-infer dataflow mistakes
                ...
                if (jl_is_datatype(b->value)) {
                    ...
                    if (tn->module == m && tn->name == b->name) {
                        ...
                        if (mt != NULL && (jl_value_t *)mt != jl_nothing) {
                            ...
                            jl_typemap_visitor(mt->defs, get_method_unspec_list,
                                               (void *)unspec);
                        }
                    }
                }
...
JL_DLLEXPORT void jl_set_typeinf_func(jl_value_t *f)
{
    ...
    for (i = 0, l = jl_array_len(unspec); i < l; i++) {
        ...
        if (!li->inferred) jl_type_infer(li, 1);
    }
    JL_GC_POP();
}
...
static int very_general_type(jl_value_t *t)
{
    return (t && (t == (jl_value_t *)jl_any_type || t == (jl_value_t *)jl_type_type ||
                  (jl_is_typevar(t) && ((jl_tvar_t *)t)->ub == (jl_value_t *)jl_any_type)));
}
...
jl_value_t *jl_nth_slot_type(jl_tupletype_t *sig, size_t i)
{
    ...
    if (i < len - 1) return jl_tparam(sig, i);
...
static jl_tupletype_t *join_tsig(jl_tupletype_t *tt, jl_tupletype_t *sig)
{
        ...
        if (jl_is_type_type(elt)) {
            ...
            if (jl_subtype(kind, decl_i, 0)) {
                if (!jl_subtype((jl_value_t *)jl_type_type, decl_i, 0)) {
                    ...
                    // cache something different since that doesn't necessarily actually
                    // apply
                    //
    ...
    if (newparams) tt = jl_apply_tuple_type(newparams);
    JL_GC_POP();
    return tt;
}

static jl_value_t *ml_matches(union jl_typemap_t ml, int offs, jl_tupletype_t *type,
                              int lim, int include_ambiguous);
...
static void jl_cacheable_sig(
    ...
    jl_tupletype_t *decl, jl_method_t *definition,

    jl_svec_t **const newparams, int *const need_guard_entries, int *const makesimplesig)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_type_type(elt) && jl_is_tuple_type(jl_tparam0(elt)) &&
            (!jl_subtype(decl_i, (jl_value_t *)jl_type_type, 0) ||
             is_kind(decl_i))) {                       // Type{Tuple{...}}
        ...
        if (decl_i == jl_ANY_flag) {
            ...
            */
            ...
            if (i < jl_nparams(decl)) {
                ...
                if (jl_is_vararg_type(declt)) declt = jl_tparam0(declt);
                jl_value_t *di =
                    jl_type_intersection(declt, (jl_value_t *)jl_typetype_type);
                assert(di != (jl_value_t *)jl_bottom_type);
                if (is_kind(di))
                    // issue #11355: DataType has a UID and so takes precedence in the
                    // cache
                ...
                // can give `T=Type` instead of `T=Type{Type{Type{...`.   /* make
                // editors happy:}}} */
            }
            else {
                jl_svecset(*newparams, i, (jl_value_t *)jl_typetype_type);
...
JL_DLLEXPORT int jl_is_cacheable_sig(jl_tupletype_t *type, jl_tupletype_t *decl,
                                     jl_method_t *definition)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_vararg_type(elt)) // varargs are always considered compilable
            continue;
        if (is_kind(elt)) // kind slots always need guard entries (checking for
                          // subtypes of Type)
            continue;
        if (decl_i == jl_ANY_flag) {
            // don't specialize on slots marked ANY
            if (elt != (jl_value_t *)jl_any_type) return 0;
            continue;
        }
        ...
        if (jl_is_type_type(elt) && jl_is_tuple_type(jl_tparam0(elt)) &&
            (!jl_subtype(decl_i, (jl_value_t *)jl_type_type, 0) ||
             is_kind(decl_i))) { // Type{Tuple{...}}
            if (elt != (jl_value_t *)jl_anytuple_type_type) return 0;
            continue;
        }
        ...
        if (notcalled_func && (decl_i == (jl_value_t *)jl_any_type ||
            ...
            // and attempt to despecialize types marked Function, Callable, or Any
            // when called with a subtype of Function but is not called
            if (elt != (jl_value_t *)jl_function_type) return 0;
            continue;
        }
        else if (jl_is_type_type(elt) && jl_is_type_type(jl_tparam0(elt)) &&
                 // give up on specializing static parameters for
            ...
            */
            ...
            /*
              ...
              Type{TC} nor TypeConstructor is more specific.
            */
            if (elt != (jl_value_t *)jl_typetype_type) return 0;
            continue;
        }
...
static jl_lambda_info_t *
cache_method(jl_methtable_t *mt, union jl_typemap_t *cache, jl_value_t *parent,
             ...
             jl_typemap_entry_t *m, jl_svec_t *sparams)
{
    ...
    int makesimplesig      = 0;
    jl_cacheable_sig(type, tt, decl, definition, (jl_svec_t **)&newparams,
                     &need_guard_entries, &makesimplesig);
    ...
    if (!definition->isstaged && jl_nparams(type) > mt->max_args &&
        ...
        //
        // note: this also protects the work join_tsig did to correct `types` for
        // the
    ...
    if (need_guard_entries) {
        temp = ml_matches(mt->defs, 0, type, -1,
                          0); // TODO: use MAX_UNSPECIALIZED_CONFLICTS?
        int guards = 0;
        if (temp == jl_false) {
            cache_with_orig = 1;
        }
        ...
        if (!cache_with_orig && guards > 0) {
            ...
            for (i = 0, l = jl_array_len(temp); i < l; i++) {
                ...
                if (((jl_method_t *)jl_svecref(m, 2)) != definition) {
                    ...
                    //        jl_emptysvec, NULL, jl_emptysvec, /*guard*/NULL,
                    //        jl_cachearg_offset(mt), &lambda_cache, NULL);
                }
            }
        }
    }
    ...
    if (origtype == type) {
        ...
        if (makesimplesig) {
            ...
            // (the only case this applies to currently due to the above logic is
            // jl_function_type)
...
static jl_lambda_info_t *jl_mt_assoc_by_type(jl_methtable_t *mt, jl_datatype_t *tt,
...
static int check_ambiguous_visitor(jl_typemap_entry_t *             oldentry,
                                   struct typemap_intersection_env *closure0)
{
    ...
    if (!jl_args_morespecific((jl_value_t *)(closure->after ? type : sig),
        ...
        jl_typemap_entry_t *l =
            jl_typemap_assoc_by_type(map, (jl_tupletype_t *)isect, NULL, 0, 0, 0);
...
static void method_overwrite(jl_typemap_entry_t *newentry, jl_method_t *oldvalue)
{
    ...
    if (oldmod != newmod) jl_printf(s, " in module %s", jl_symbol_name(newmod->name));
    ...
    if (jl_typeof(pml->unknown) == (jl_value_t *)jl_typemap_level_type) {
        ...
        if (cache->arg1.values != (void *)jl_nothing) {
            ...
            for (i = 0; i < l; i++) {
                ...
                if (pl->unknown != jl_nothing) {
                    invalidate_conflicting(pl, type, (jl_value_t *)cache->arg1.values,
                                           shadowed);
                }
            }
        }
        if (cache->targ.values != (void *)jl_nothing) {
            ...
            for (i = 0; i < l; i++) {
                union jl_typemap_t *pl = &d[i];
                if (pl->unknown != jl_nothing) {
                    invalidate_conflicting(pl, type, (jl_value_t *)cache->targ.values,
                                           shadowed);
                }
            }
        }
    ...
    while (l != (void *)jl_nothing) {
        ...
        for (i = 0; i < n; i++) {
            if (d[i] == (jl_value_t *)l->func.linfo->def) {
                replaced = jl_type_intersection(type, (jl_value_t *)l->sig) !=
...
static void update_max_args(jl_methtable_t *mt, jl_tupletype_t *type)
{
    ...
    if (jl_va_tuple_kind(type) == JL_VARARG_UNBOUND) na--;
    if (na > mt->max_args) mt->max_args = na;
}

void jl_method_table_insert(jl_methtable_t *mt, jl_method_t *method,
                            jl_tupletype_t *simpletype)
{
    ...
    jl_typemap_entry_t *newentry =
        jl_typemap_insert(&mt->defs, (jl_value_t *)mt, type, tvars, simpletype,
                          jl_emptysvec, (jl_value_t *)method, 0, &method_defs, &oldvalue);
    if (oldvalue) {
...
{
    jl_ptls_t   ptls     = jl_get_ptls_states();
    jl_value_t *fargs[3] = {(jl_value_t *)jl_methoderror_type, (jl_value_t *)f, args};
    if (fargs[0]) {
        jl_throw(jl_apply_generic(fargs, 3));
    }
    else {
        jl_printf((JL_STREAM *)STDERR_FILENO, "A method error occurred before the "
                                              "base MethodError type was defined. "
                                              "Aborting...\n");
                                           ...
                                           int cache, int inexact)
{
    jl_typemap_entry_t *entry =
        jl_typemap_assoc_by_type(mt->cache, types, NULL, 0, 1, jl_cachearg_offset(mt));
    if (entry) return entry->func.linfo;
    JL_LOCK(&mt->writelock);
    ...
    jl_lambda_info_t *sf = jl_mt_assoc_by_type(mt, types, cache, inexact);
    if (cache) {
        JL_UNLOCK(&mt->writelock);
    }
    else {
...
jl_lambda_info_t *jl_method_lookup(jl_methtable_t *mt, jl_value_t **args, size_t nargs,
                                   int cache)
{
    jl_typemap_entry_t *entry =
        jl_typemap_assoc_exact(mt->cache, args, nargs, jl_cachearg_offset(mt));
    if (entry) return entry->func.linfo;
...
{
    // one unspecialized version of a function can be shared among all cached
    // specializations
    jl_method_t *def = method->def;
    if (method->unspecialized_ducttape != NULL) return method->unspecialized_ducttape;
    if (method->sparam_syms != jl_emptysvec) {
        ...
        if (def->needs_sparam_vals_ducttape) {
            method->unspecialized_ducttape =
                jl_get_specialized(def, method->specTypes, method->sparam_vals);
            jl_gc_wb(method, method->unspecialized_ducttape);
            method->unspecialized_ducttape->unspecialized_ducttape =
                method->unspecialized_ducttape;
        }
        JL_UNLOCK(&def->writelock);
        if (method->unspecialized_ducttape != NULL) return method->unspecialized_ducttape;
...
jl_lambda_info_t *jl_compile_for_dispatch(jl_lambda_info_t *li)
{
    ...
    if (li->functionObjectsDecls.functionObject != NULL) return li;
    if (li->def) {
        ...
        if (li->inInference || li->inCompile) {
            ...
            if (li->code == jl_nothing || (!li->inferred && li->def != NULL &&
    ...
    if (li->functionObjectsDecls.functionObject ==
        NULL) { // check again, because jl_type_infer may have compiled it
        jl_compile_linfo(li);
    }
    if (li->def) JL_UNLOCK(&li->def->writelock);
    return li;
}
...
jl_lambda_info_t *jl_get_specialization1(jl_tupletype_t *types)
{
    ...
    for (i = 0; i < jl_nparams(types); i++) {
        ...
        if (ti == (jl_value_t *)jl_datatype_type || jl_is_tuple_type(ti)) {
            ...
            if (matches == jl_false) return NULL;
            break;
        }
    }
    ...
    if (sf == NULL || sf->code == NULL || jl_has_call_ambiguities(types, sf->def)) {
        sf = NULL;
    }
    JL_GC_POP();
    return sf;
...
JL_DLLEXPORT int jl_compile_hint(jl_tupletype_t *types)
{
    ...
    if (li->functionObjectsDecls.functionObject == NULL && li->jlcall_api != 2) {
        assert(!li->inCompile);
        if (li->inInference) return 0;
        if (li->code == jl_nothing || !li->inferred) jl_type_infer(li, 0);
        if (li->inInference || li->code == jl_nothing) return 0;
        jl_compile_linfo(li);
    }
    return 1;
}
...
int jl_has_call_ambiguities(jl_tupletype_t *types, jl_method_t *m)
{
    ...
    for (size_t i = 0; i < jl_array_len(m->ambig); i++) {
        ...
        if (jl_type_intersection((jl_value_t *)mambig->sig, (jl_value_t *)types) !=
            (jl_value_t *)jl_bottom_type) {
            return 1;
        }
    }
...
jl_tupletype_t *jl_argtype_with_function(jl_function_t *f, jl_tupletype_t *types)
{
    ...
    for (i = 0; i < l; i++) jl_svecset(tt, i + 1, jl_tparam(types, i));
...
static int tupletype_any_bottom(jl_value_t *sig)
{
    ...
    for (i = 0; i < l; i++) {
        if (jl_svecref(types, i) == jl_bottom_type) return 1;
    }
    return 0;
}
...
static int _compile_all_tvar_union(jl_tupletype_t *methsig, jl_svec_t *tvars)
{
    ...
    for (i = 0; i < tvarslen; i++) {
        ...
        env[2 * i + 1] = jl_bottom_type; // initialize the list with Union{}, since
                                         // T<:Union{} is always a valid option
    }

    for (i = 0; i < tvarslen; /* incremented by inner loop */) {
        jl_value_t *sig;
        JL_TRY {
            sig = (jl_value_t *)jl_instantiate_type_with((jl_value_t *)methsig, env,
                                                         tvarslen);
        }
        JL_CATCH {
            goto getnext; // sigh, we found an invalid type signature. should we warn
                          // the user?
        }
...
static int _compile_all_union(jl_tupletype_t *sig, jl_svec_t *tvars)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (jl_is_uniontype(ty)) {
            ...
            if (l == 0) return 1; // why does this method exist?
            ++count_unions;
        }
    }

    if (count_unions == 0) return _compile_all_tvar_union(sig, tvars);
    ...
    while (!incr) {
        ...
        if (!_compile_all_tvar_union(methsig, tvars)) complete = 0;
    }
...
static void _compile_all_deq(jl_array_t *found)
{
    ...
    for (found_i = 0; found_i < found_l; found_i++) {
        ...
        // type infer a copy of the template, to avoid modifying the template code
        // itself
        if (m->isstaged)
            linfo = templ;
        else
        ...
        if (linfo->jlcall_api == 2) continue;

        // infer this function now, if necessary
        if (!linfo->inferred || linfo->code == jl_nothing) jl_type_infer(linfo, 1);

        // keep track of whether all possible signatures have been cached (and thus
        // whether it can skip trying to compile the template function)
        // this is necessary because many intrinsics try to call static_eval and
        // thus are not compilable unspecialized
        int complete = _compile_all_union(ml->sig, ml->tvars);
        if (complete) {
            if (templ->fptr == NULL)
                // indicate that this method doesn't need to be compiled, because it was
                // fully covered above
...
static int _compile_all_enq(jl_typemap_entry_t *ml, void *env)
{
    ...
    if (m->lambda_template->functionObjectsDecls.functionObject == NULL &&
        m->lambda_template->jlcall_api != 2 && m->lambda_template->fptr == NULL) {
            ...
            if (b->owner == m && b->value && b->constp) {
                if (jl_is_datatype(b->value)) {
                    ...
                    if (tn->module == m && tn->name == b->name) {
                        jl_methtable_t *mt = tn->mt;
                        if (mt != NULL && (jl_value_t *)mt != jl_nothing) {
                            jl_typemap_visitor(mt->defs, _precompile_enq_spec,
                                               (void *)unspec);
                        }
                    }
                }
...
// STATIC_INLINE jl_typemap_entry_t *sig2_match_fast(jl_value_t **args,
// jl_typemap_entry_t *entry1, jl_typemap_entry_t *entry2, size_t n)
//{
//    // NOTE: This function is a huge performance hot spot!!
...
static void flush_from_cache(jl_typemap_entry_t *entry)
{
    int i;
    for (i = 0; i < N_CALL_CACHE; i++) {
        if (call_cache[i] == entry) call_cache[i] = NULL;
    }
}
...
JL_DLLEXPORT jl_value_t *jl_apply_generic(jl_value_t **args, uint32_t nargs)
{
    ...
    if (traceen) show_call(args[0], &args[1], nargs - 1);
#endif
    ...
    */
    ...
    //        else if (sig_match_fast(args, jl_svec_data(entry1->sig->parameters),
    //        0, nargs))
    //            entry = entry1;
    //    }
    //    else if (entry2 && nargs == jl_svec_len(entry2->sig->parameters)) {
    //        if (sig_match_fast(args, jl_svec_data(entry2->sig->parameters), 0,
    //        nargs))
    //            entry = entry2;
    //    }
    //
    ...
    //        jl_value_t *F = args[0];
    //        mt = jl_gf_mtable(F);
    //        entry = jl_typemap_assoc_exact(mt->cache, args, nargs,
    //        jl_cachearg_offset(mt));
    //        if (entry && entry->isleafsig && entry->simplesig ==
    //        (void*)jl_nothing && entry->guardsigs == jl_emptysvec) {
    ...
    //            !sig_match_fast(args, jl_svec_data(entry->sig->parameters),
    //            nargs)) {
    ...
    //            if (!entry || nargs != jl_svec_len(entry->sig->parameters) ||
    //                    !sig_match_fast(args,
    //                    jl_svec_data(entry->sig->parameters), nargs)) {
    ...
    if (i == 4) {
        ...
        mt            = jl_gf_mtable(F);
        entry = jl_typemap_assoc_exact(mt->cache, args, nargs, jl_cachearg_offset(mt));
        if (entry && entry->isleafsig && entry->simplesig == (void *)jl_nothing &&
    ...
    if (entry) {
        ...
        if (mfunc == NULL) {
#ifdef JL_TRACE
            if (error_en) show_call(F, args, nargs);
#endif
...
JL_DLLEXPORT jl_value_t *jl_gf_invoke_lookup(jl_datatype_t *types)
{
    jl_methtable_t *    mt = ((jl_datatype_t *)jl_tparam0(types))->name->mt;
    jl_typemap_entry_t *entry =
        jl_typemap_assoc_by_type(mt->defs, types, /*don't record env*/ NULL,
                                 /*exact match*/ 0, /*subtype*/ 1, /*offs*/ 0);
    if (!entry) return jl_nothing;
...
jl_value_t *jl_gf_invoke(jl_tupletype_t *types0, jl_value_t **args, size_t nargs)
{
    ...
    if (tm) {
        ...
        if (method->invokes.unknown != NULL)
            tm = jl_typemap_assoc_exact(method->invokes, args, nargs,
                                        jl_cachearg_offset(mt));
        if (tm) {
            mfunc = tm->func.linfo;
        }
            ...
            if (entry->tvars != jl_emptysvec) {
                jl_value_t *ti = jl_lookup_match((jl_value_t *)tt, (jl_value_t *)entry->sig,
                                                 &tpenv, entry->tvars);
            ...
            if (func->invokes.unknown == NULL) func->invokes.unknown = jl_nothing;

            mfunc =
                cache_method(mt, &func->invokes, entry->func.value, sig, tt, entry, tpenv);
        }
                                                             ...
                                                             jl_datatype_t *st, int iskw)
{
    ...
    free(prefixed);
    jl_datatype_t *ftype =
        jl_new_datatype(tname, st, jl_emptysvec, jl_emptysvec, jl_emptysvec, 0, 0, 0);
    JL_GC_PUSH1(&ftype);
    ftype->name->mt->name = name;
    jl_gc_wb(ftype->name->mt, name);
    ftype->name->module = module;
    jl_gc_wb(ftype->name, module);
    ftype->name->mt->module = module;
    jl_gc_wb(ftype->name->mt, module);
...
JL_DLLEXPORT jl_function_t *jl_get_kwsorter(jl_typename_t *tn)
{
    ...
    if (!mt->kwsorter) {
        JL_LOCK(&mt->writelock);
        if (!mt->kwsorter) {
            mt->kwsorter = jl_new_generic_function_with_supertype(tn->name, mt->module,
                                                                  jl_function_type, 1);
...
JL_DLLEXPORT jl_svec_t *jl_match_method(jl_value_t *type, jl_value_t *sig, jl_svec_t *tvars)
{
    jl_svec_t * env = jl_emptysvec;
    jl_value_t *ti  = NULL;
    JL_GC_PUSH2(&env, &ti);
...
static int ml_matches_visitor(jl_typemap_entry_t *             ml,
                              struct typemap_intersection_env *closure0)
{
    ...
    */
    ...
    if (closure->lim >= 0) {
        ...
        for (i = 0; i < len; i++) {
            ...
            // Instead we should have computed the intersection as (UnionAll S
            // Type{T{S}}),
            ...
            if (jl_is_leaf_type(prior_ti) && !jl_has_typevars(closure->match.ti) &&
                !jl_has_typevars(prior_ti) && jl_subtype(closure->match.ti, prior_ti, 0)) {
                skip = 1;
                break;
            }
        }
    ...
    if (!skip) {
        ...
        */
        ...
        // over-approximated.
        if (matched_all_typevars &&
            jl_types_equal(closure->match.ti, closure->match.type) &&
        ...
        if (meth->ambig != jl_nothing && (!closure->include_ambiguous || done)) {
            ...
            for (size_t j = 0; j < jl_array_len(meth->ambig); j++) {
                ...
                env                 = jl_emptysvec;
                jl_value_t *mti     = jl_type_intersection_matching(
                    (jl_value_t *)closure->match.type, (jl_value_t *)mambig->sig, &env,
                    mambig->tvars);
                    ...
                    if (closure->include_ambiguous) {
                        ...
                        if (k >= len) {
                            ...
                            jl_array_ptr_1d_push(
                                (jl_array_t *)closure->t,
                                (jl_value_t *)jl_svec(3, mti, env, mambig));
                            len++;
                        }
                    }
                    else {
                        // the current method doesn't match if there is an intersection with
                        // an
                        // ambiguous method that covers our intersection with this one.
                        jl_value_t *ambi = jl_type_intersection_matching(
                            (jl_value_t *)ml->sig, (jl_value_t *)mambig->sig, &env,
                            mambig->tvars);
...
static jl_value_t *ml_matches(union jl_typemap_t defs, int offs, jl_tupletype_t *type,
                              int lim, int include_ambiguous)
{
    size_t      l  = jl_svec_len(type->parameters);
    jl_value_t *va = NULL;
...
{
    assert(jl_nparams(types) > 0);
    if (jl_tparam0(types) == jl_bottom_type) return (jl_value_t *)jl_alloc_vec_any(0);
    assert(jl_is_datatype(jl_tparam0(types)));
    jl_methtable_t *mt = ((jl_datatype_t *)jl_tparam0(types))->name->mt;
    if (mt == NULL) return (jl_value_t *)jl_alloc_vec_any(0);
    return ml_matches(mt->defs, 0, types, lim, include_ambiguous);
}

static jl_mutex_t typeinf_lock;

JL_DLLEXPORT void jl_typeinf_begin(void) { JL_LOCK(&typeinf_lock); }

JL_DLLEXPORT void jl_typeinf_end(void) { JL_UNLOCK(&typeinf_lock); }
