...
jl_invoke(jl_method_instance_t *meth, jl_value_t **args, uint32_t nargs)
{
    if (meth->jlcall_api) {
...
JL_DLLEXPORT void
jl_register_method_tracer(void (*callback)(jl_method_instance_t *tracee))
{
...
JL_DLLEXPORT void
jl_register_newmeth_tracer(void (*callback)(jl_method_t *tracee))
{
...
JL_DLLEXPORT void
jl_register_linfo_tracer(void (*callback)(jl_method_instance_t *tracee))
{
...
const struct jl_typemap_info method_defs = { 0, &jl_method_type };
const struct jl_typemap_info lambda_cache = { 0, &jl_method_instance_type };
const struct jl_typemap_info tfunc_cache = { 1, &jl_any_type };
...
JL_DLLEXPORT jl_method_instance_t *jl_specializations_get_linfo(
        jl_method_t *m, jl_value_t *type, jl_svec_t *sparams, size_t world)
{
    ...
    // pick an initial world that is likely to be valid both before and after
    // inference
...
JL_DLLEXPORT jl_value_t *
jl_specializations_lookup(jl_method_t *m, jl_tupletype_t *type, size_t world)
{
    jl_typemap_entry_t *sf = jl_typemap_assoc_by_type(
            m->specializations,
            type,
...
JL_DLLEXPORT jl_value_t *
jl_methtable_lookup(jl_methtable_t *mt, jl_tupletype_t *type, size_t world)
{
    jl_typemap_entry_t *sf = jl_typemap_assoc_by_type(
...
void jl_mk_builtin_func(jl_datatype_t *dt, const char *name, jl_fptr_t fptr)
{
    ...
    if (dt == NULL) {
        jl_value_t *f = jl_new_generic_function_with_supertype(
                sname, jl_core_module, jl_builtin_type, 0);
...
// if successful, also updates the li argument to describe the validity of this
// src
// if inference doesn't occur (or can't finish), returns NULL instead
jl_code_info_t *
jl_type_infer(jl_method_instance_t **pli, size_t world, int force)
{
    JL_TIMING(INFERENCE);
    ...
    li->inInference = 1;
    jl_svec_t *linfo_src_rettype =
            (jl_svec_t *)jl_apply_with_saved_exception_state(fargs, 3, 0);
    ptls->world_age = last_age;
    li->inInference = 0;

    jl_code_info_t *src = NULL;
    if (linfo_src_rettype && jl_is_svec(linfo_src_rettype) &&
        jl_svec_len(linfo_src_rettype) == 3 &&
...
int jl_is_rettype_inferred(jl_method_instance_t *li)
{
    if (!li->inferred)
        return 0;
    if (jl_is_code_info(li->inferred) &&
        !((jl_code_info_t *)li->inferred)->inferred)
        return 0;
    return 1;
}
...
static int set_max_world2(jl_typemap_entry_t *entry, void *closure0)
{
    ...
    // entry->max_world should be <= closure->replaced->max_world and >=
    // closure->world
...
{
    struct set_world *closure = (struct set_world *)closure0;
    // entry->min_world should be >= closure->replaced->min_world and >=
    // closure->world
    if (entry->func.linfo == closure->replaced) {
...
static void update_world_bound(
        jl_method_instance_t *replaced,
        jl_typemap_visitor_fptr fptr,
        size_t world)
{
        ...
        size_t max_world)
{
    ...
    assert(li->inInference && "shouldn't be caching an inference result for a "
                              "MethodInstance that wasn't being inferred");
    if (min_world != li->min_world || max_world != li->max_world) {
        if (!jl_is_method(li->def.method)) {
            ...
            if (!isinferred && li->min_world >= min_world &&
                li->max_world <= max_world) {
                // expand the current (uninferred) entry to cover the full
                // inferred range
                // only update the specializations though, since the method
                ...
                // clip applicability of old method instance (uninferred or
                // inferred)
                // to make it easier to find the inferred method
                // (even though the real applicability was unchanged)
                // there are 6(!) regions here to consider + boundary conditions
                // for each
                if (li->max_world >= min_world && li->min_world <= max_world) {
                    // there is a non-zero overlap between [li->min, li->max]
                    // and [min, max]
                    // there are now 4 regions left to consider
                    // TODO: also take into account li->def.method->world range
                    // when computing preferred division
                    if (li->max_world > max_world) {
                        ...
                        assert(min_world > 1 &&
                               "logic violation: min(li->min_world) == 1 (by "
                               "construction), so min(min_world) == 2");
                        update_world_bound(li, set_max_world2, min_world - 1);
                    }
                    else {
                        // old inferred li is fully covered by new inference
                        // result, so just delete it
                        assert(isinferred);
                        update_world_bound(
                                li, set_max_world2, li->min_world - 1);
                    }
                }

                // build a new entry to describe the new (inferred)
                // applicability
                li = jl_get_specialized(
                        li->def.method, li->specTypes, li->sparam_vals);
                li->min_world = min_world;
                li->max_world = max_world;
                jl_typemap_insert(
                        &li->def.method->specializations,
                        li->def.value,
                        (jl_tupletype_t *)li->specTypes,
                        NULL,
                        jl_emptysvec,
                        (jl_value_t *)li,
                        0,
...
static int get_spec_unspec_list(jl_typemap_entry_t *l, void *closure)
{
    if (jl_is_method_instance(l->func.value) &&
        !jl_is_rettype_inferred(l->func.linfo))
...
{
    jl_typemap_visitor(
            def->func.method->specializations, get_spec_unspec_list, closure);
    return 1;
}

void jl_foreach_mtable_in_module(
        jl_module_t *m, void (*visit)(jl_methtable_t *mt, void *env), void *env)
{
    size_t i;
        ...
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->owner == m && b->value && b->constp) {
                ...
                if (jl_is_datatype(v)) {
                    ...
                    if (child != m && child->parent == m &&
                        child->name == b->name) {
                        // this is the original/primary binding for the
                        // submodule
...
static void reset_mt_caches(jl_methtable_t *mt, void *env)
{
    ...
    if (mt->defs.unknown !=
        jl_nothing) // make sure not to reset builtin functions
...
JL_DLLEXPORT void jl_set_typeinf_func(jl_value_t *f)
{
    ...
    jl_foreach_mtable_in_module(
            jl_main_module, reset_mt_caches, (void *)unspec);
    size_t i, l;
    for (i = 0, l = jl_array_len(unspec); i < l; i++) {
        jl_method_instance_t *li =
                (jl_method_instance_t *)jl_array_ptr_ref(unspec, i);
...
// after intersection, the argument tuple type needs to be corrected to reflect
// the signature match
// that occurred, if the arguments contained a Type but the signature matched on
// the kind
        ...
        if (jl_is_type_type(elt)) {
            ...
            if (jl_subtype(kind, decl_i)) {
                if (!jl_subtype((jl_value_t *)jl_type_type, decl_i)) {
                    ...
                    // representations of any type. If we matched this method
                    // because
                    // it matched the leaf type UnionAll, then don't cache
                    // something
...
static void jl_cacheable_sig(
        jl_tupletype_t
                *const type, // the specialized type signature for type lambda
        ...
        int *const makesimplesig)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if ((tt != type &&
             elt != jl_tparam(tt, i)) || // if join_tsig made a swap
            jl_is_kind(elt)) { // might see a kind if called at compile-time
            // kind slots always need guard entries (checking for subtypes of
            // Type)
        ...
        if (jl_is_type_type(elt) && jl_is_tuple_type(jl_tparam0(elt)) &&
            ...
            (!jl_subtype(decl_i, (jl_value_t *)jl_type_type) ||
             jl_is_kind(decl_i))) {                    // Type{Tuple{...}}
        ...
        int notcalled_func =
                (i > 0 && i <= 8 && !(definition->called & (1 << (i - 1))) &&
        ...
        if (decl_i == jl_ANY_flag) {
            ...
            *need_guard_entries = 1;
                ...
                notcalled_func && (decl_i == (jl_value_t *)jl_any_type ||
                                   ...
                                   (jl_is_uniontype(decl_i) &&
                                    ((((jl_uniontype_t *)decl_i)->a ==
                                              (jl_value_t *)jl_function_type &&
                                      ((jl_uniontype_t *)decl_i)->b ==
                                              (jl_value_t *)jl_type_type) ||
                                     (((jl_uniontype_t *)decl_i)->b ==
                                              (jl_value_t *)jl_function_type &&
                                      ((jl_uniontype_t *)decl_i)->a ==
                                              (jl_value_t *)jl_type_type))))) {
            // and attempt to despecialize types marked Function, Callable, or
            // Any
            ...
            *need_guard_entries = 1;
                ...
                jl_is_type_type(elt) && jl_is_type_type(jl_tparam0(elt)) &&
                // give up on specializing static parameters for
                // Type{Type{Type{...}}}
            ...
            */
            ...
            if (i < jl_nparams(ud)) {
                ...
                jl_value_t *di = jl_type_intersection(
                        declt, (jl_value_t *)jl_typetype_type);
                assert(di != (jl_value_t *)jl_bottom_type);
                if (jl_is_kind(di))
                    // issue #11355: DataType has a UID and so takes precedence
                    // in the cache
                    jl_svecset(*newparams, i, (jl_value_t *)jl_typetype_type);
                else
                    jl_svecset(*newparams, i, di);
                // TODO: recompute static parameter values, so in extreme cases
                // we
            ...
            *need_guard_entries = 1;
                ...
                jl_is_type_type(elt) && very_general_type(decl_i) &&
                !jl_has_free_typevars(decl_i)) {
            /*
              ...
              be alternate representations of any type. Extensionally, TC ==
              TC.body,
...
{
    // compute whether this type signature is a possible return value from
    // jl_cacheable_sig
    // return jl_cacheable_sig(type, NULL, definition->sig, definition, NULL,
    // NULL);
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_vararg_type(elt)) // varargs are always considered compilable
            continue;
        if (jl_is_kind(elt)) // kind slots always need guard entries (checking
                             // for subtypes of Type)
            continue;
        if (decl_i == jl_ANY_flag) {
            // don't specialize on slots marked ANY
        ...
        if (jl_is_type_type(elt)) { // if join_tsig would make a swap
            ...
            // needs to be corrected to the leaf type 'kind'
            jl_value_t *kind = jl_typeof(jl_tparam0(elt));
            if (kind != (jl_value_t *)jl_tvar_type &&
                jl_subtype(kind, decl_i)) {
                if (!jl_subtype((jl_value_t *)jl_type_type, decl_i))
                    return 0;
            }
        }
        ...
        int allow_exec)
{
    ...
    if (!definition->isstaged && jl_nparams(type) > mt->max_args &&
        ...
        //
        // note: this also protects the work join_tsig did to correct `types`
        // for the
        ...
        if (all_are_subtypes) {
            ...
            if (jl_is_type_type(lasttype) &&
                jl_is_type_type(jl_tparam0(lasttype)))
                lasttype = (jl_value_t *)jl_type_type;
            jl_svecset(
                    limited, i, jl_wrap_vararg(lasttype, (jl_value_t *)NULL));
        }
        else {
            ...
            if (nsp > 0) {
                ...
                lastdeclt = (jl_value_t *)jl_instantiate_type_with(
                        (jl_value_t *)lastdeclt, jl_svec_data(env), nsp);
            }
    ...
    if (need_guard_entries) {
        temp = ml_matches(
                ...
                world,
                &min_valid,
                &max_valid); // TODO: use MAX_UNSPECIALIZED_CONFLICTS?
        int guards = 0;
        if (temp == jl_false) {
            cache_with_orig = 1;
        }
            ...
            for (i = 0; i < l; i++) {
                ...
                if (unmatched_tvars || guards > MAX_UNSPECIALIZED_CONFLICTS) {
                    // if distinguishing a guard entry from the generalized
                    // signature
            ...
            for (i = 0, l = jl_array_len(temp); i < l; i++) {
                ...
                if (other != definition) {
                    jl_svecset(
                            guardsigs,
                            guards,
                            (jl_tupletype_t *)jl_svecref(m, 0));
                    guards++;
                    // jl_typemap_insert(cache, parent,
                    // (jl_tupletype_t*)jl_svecref(m, 0),
                    //        NULL, jl_emptysvec, /*guard*/NULL,
                    //        jl_cachearg_offset(mt), &lambda_cache,
                    //        other->min_world, other->max_world, NULL);
                }
            }
        }
    }
    ...
    newmeth = jl_specializations_get_linfo(
            definition, (jl_value_t *)type, sparams, world);
    ...
    if (cache_with_orig) {
        ...
        type = origtype; // restore `type` to be the `origtype` backup (discard
                         // computed simplified `type`)
    ...
    if (origtype == type) {
        type = NULL; // don't need `type` anymore: it's equivalent to the
                     // `origtype`
        if (makesimplesig) {
            // reduce the complexity of rejecting this entry in the cache
            // by replacing non-simple types with jl_any_type to build a new
            // `type`
            // (the only case this applies to currently due to the above logic
            // is jl_function_type)
...
/*
  ...
  for example, (AbstractArray, AbstractMatrix) and (AbstractMatrix,
  AbstractArray) are ambiguous.
  however, (AbstractArray, AbstractMatrix, Foo) and (AbstractMatrix,
  AbstractArray, Bar) are fine
...
static int check_ambiguous_visitor(
        jl_typemap_entry_t *oldentry, struct typemap_intersection_env *closure0)
{
    struct ambiguous_matches_env *closure =
            container_of(closure0, struct ambiguous_matches_env, match);
    ...
    if (closure->match.issubty) {
        ...
        msp = jl_type_morespecific_no_subtype(
                (jl_value_t *)type, (jl_value_t *)sig);
    }
    else {
        if (jl_subtype((jl_value_t *)sig, (jl_value_t *)type))
            ...
            msp = jl_type_morespecific_no_subtype(
                    (jl_value_t *)sig, (jl_value_t *)type);
    }
    ...
    if (!msp || closure->after) {
        ...
        if (closure->shadowed == NULL) {
            ...
            jl_array_ptr_1d_push(
                    (jl_array_t *)closure->shadowed, oldentry->func.value);
        }
    }
    return 1;
}
...
static jl_value_t *
check_ambiguous_matches(union jl_typemap_t defs, jl_typemap_entry_t *newentry)
{
    jl_tupletype_t *type = newentry->sig;
    jl_tupletype_t *ttypes =
            (jl_tupletype_t *)jl_unwrap_unionall((jl_value_t *)type);
...
static void
method_overwrite(jl_typemap_entry_t *newentry, jl_method_t *oldvalue)
{
    // method overwritten
...
static void invalidate_method_instance(
        jl_method_instance_t *replaced, size_t max_world, int depth)
{
    ...
    if (replaced->max_world > max_world) {
        ...
        assert(replaced->min_world - 1 <= max_world &&
               "attempting to set invalid world constraints");
        ...
        if (backedges) {
            ...
            for (i = 0; i < l; i++) {
                jl_method_instance_t *replaced =
                        (jl_method_instance_t *)jl_array_ptr_ref(backedges, i);
...
static int invalidate_backedges(
        jl_typemap_entry_t *oldentry, struct typemap_intersection_env *closure0)
{
    struct invalidate_conflicting_env *closure =
            container_of(closure0, struct invalidate_conflicting_env, match);
    if (oldentry->max_world > closure->max_world) {
        ...
        if (backedges) {
            size_t i, l = jl_array_len(backedges);
            jl_method_instance_t **replaced =
                    (jl_method_instance_t **)jl_array_data(backedges);
            for (i = 0; i < l; i++) {
...
JL_DLLEXPORT void jl_method_instance_add_backedge(
        jl_method_instance_t *callee, jl_method_instance_t *caller)
{
    assert(callee->min_world <= caller->min_world &&
           callee->max_world >= caller->max_world);
...
JL_DLLEXPORT void jl_method_table_add_backedge(
        jl_methtable_t *mt, jl_value_t *typ, jl_value_t *caller)
{
...
JL_DLLEXPORT void jl_method_table_insert(
        jl_methtable_t *mt, jl_method_t *method, jl_tupletype_t *simpletype)
{
    ...
    if (oldvalue) {
        ...
        if (mt->backedges) {
            jl_value_t **backedges =
                    (jl_value_t **)jl_array_data(mt->backedges);
            size_t i, na = jl_array_len(mt->backedges);
            size_t ins = 0;
            for (i = 1; i < na; i += 2) {
                jl_value_t *backedgetyp = backedges[i - 1];
                if (!jl_has_empty_intersection(
                            backedgetyp, (jl_value_t *)type)) {
                    jl_method_instance_t *backedge =
                            (jl_method_instance_t *)backedges[i];
    ...
    if (oldvalue) {
        ...
        if (jl_is_method(oldvalue)) {
            jl_typemap_intersection_visitor(
                    ((jl_method_t *)oldvalue)->specializations, 0, &env.match);
        }
        else {
            ...
            for (i = 0; i < n; i++) {
                jl_typemap_intersection_visitor(
                        d[i]->specializations, 0, &env.match);
            }
        }
    }
...
void JL_NORETURN
jl_method_error_bare(jl_function_t *f, jl_value_t *args, size_t world)
{
    if (jl_methoderror_type) {
        ...
        struct jl_method_error {
            ...
            jl_value_t *args;
            size_t world;
        } *pe = (struct jl_method_error *)e, ee = { f, args, world };
        *pe = ee;
        jl_throw(e);
    }
    else {
        jl_printf((JL_STREAM *)STDERR_FILENO, "A method error occurred before "
                                              "the base MethodError type was "
                                              "defined. Aborting...\n");
        jl_static_show((JL_STREAM *)STDERR_FILENO, (jl_value_t *)f);
        jl_printf((JL_STREAM *)STDERR_FILENO, " world %u\n", (unsigned)world);
        jl_static_show((JL_STREAM *)STDERR_FILENO, args);
...
void JL_NORETURN
jl_method_error(jl_function_t *f, jl_value_t **args, size_t na, size_t world)
{
...
jl_method_instance_t *jl_method_lookup_by_type(
        jl_methtable_t *mt,
        ...
        size_t world)
{
    jl_typemap_entry_t *entry = jl_typemap_assoc_by_type(
            mt->cache, types, NULL, 0, 1, jl_cachearg_offset(mt), world);
    if (entry) {
        jl_method_instance_t *linfo = (jl_method_instance_t *)entry->func.value;
        assert(linfo->min_world <= entry->min_world &&
               linfo->max_world >= entry->max_world &&
               "typemap consistency error: MethodInstance doesn't apply to "
               "full range of its entry");
        return linfo;
    }
    JL_LOCK(&mt->writelock);
    ...
    if (jl_is_leaf_type((jl_value_t *)types))
        cache = 1;
    jl_method_instance_t *sf =
            jl_mt_assoc_by_type(mt, types, cache, inexact, allow_exec, world);
    if (cache) {
        JL_UNLOCK(&mt->writelock);
    }
    else {
...
JL_DLLEXPORT int
jl_method_exists(jl_methtable_t *mt, jl_tupletype_t *types, size_t world)
{
    return jl_method_lookup_by_type(mt, types, 0, 0, 1, world) != NULL;
}

jl_method_instance_t *jl_method_lookup(
        jl_methtable_t *mt,
        jl_value_t **args,
        size_t nargs,
        int cache,
        size_t world)
{
    jl_typemap_entry_t *entry = jl_typemap_assoc_exact(
            mt->cache, args, nargs, jl_cachearg_offset(mt), world);
    if (entry)
        return entry->func.linfo;
    JL_LOCK(&mt->writelock);
    entry = jl_typemap_assoc_exact(
            mt->cache, args, nargs, jl_cachearg_offset(mt), world);
    if (entry) {
        JL_UNLOCK(&mt->writelock);
        return entry->func.linfo;
...
// -1 for no limit.
JL_DLLEXPORT jl_value_t *jl_matching_methods(
        jl_tupletype_t *types,
        int lim,
        int include_ambiguous,
        size_t world,
...
jl_llvm_functions_t
jl_compile_for_dispatch(jl_method_instance_t **pli, size_t world)
{
    jl_method_instance_t *li = *pli;
    if (li->jlcall_api == 2)
...
jl_get_specialization1(jl_tupletype_t *types, size_t world)
{
    JL_TIMING(METHOD_LOOKUP_COMPILE);
    if (!jl_is_leaf_type((jl_value_t *)types) ||
        jl_has_free_typevars((jl_value_t *)types))
        return NULL;
    ...
    for (i = 0; i < jl_nparams(args); i++) {
        ...
        if (ti == (jl_value_t *)jl_datatype_type || jl_is_tuple_type(ti)) {
            jl_value_t *matches = jl_matching_methods(
                    types, 1, 0, world, &min_valid, &max_valid);
            if (matches == jl_false)
                return NULL;
            break;
        }
    ...
    jl_method_instance_t *sf =
            jl_method_lookup_by_type(mt, types, 1, 1, 1, world);
...
JL_DLLEXPORT int jl_has_call_ambiguities(jl_tupletype_t *types, jl_method_t *m)
{
    ...
    for (size_t i = 0; i < jl_array_len(m->ambig); i++) {
        ...
        if (!jl_has_empty_intersection(
                    (jl_value_t *)mambig->sig, (jl_value_t *)types))
            return 1;
    }
    return 0;
}
...
jl_tupletype_t *
jl_argtype_with_function(jl_function_t *f, jl_tupletype_t *types)
{
...
static void __attribute__((unused)) enable_trace(int x)
{
    trace_en = x;
}
...
STATIC_INLINE int
sig_match_fast(jl_value_t **args, jl_value_t **sig, size_t i, size_t n)
{
...
void call_cache_stats()
{
    ...
    jl_safe_printf(
            "cache occupied: %d / %d; pick_which stats: {%d, %d, %d, %d}\n",
            count,
            N_CALL_CACHE,
            pick_which_stat[0],
            pick_which_stat[1],
...
STATIC_INLINE jl_method_instance_t *jl_lookup_generic_(
        jl_value_t **args, uint32_t nargs, uint32_t callsite, size_t world)
{
#ifdef JL_GF_PROFILE
    ncalls++;
#endif
    ...
    */
    ...
    // them is less likely
    uint32_t cache_idx[4] = { (callsite) & (N_CALL_CACHE - 1),
                              (callsite >> 8) & (N_CALL_CACHE - 1),
                              (callsite >> 16) & (N_CALL_CACHE - 1),
                              (callsite >> 24 | callsite << 8) &
                                      (N_CALL_CACHE - 1) };
    jl_typemap_entry_t *entry = NULL;
    ...
    for (i = 0; i < 4; i++) {
        ...
        if (entry && nargs == jl_svec_len(entry->sig->parameters) &&
            sig_match_fast(
                    args, jl_svec_data(entry->sig->parameters), 0, nargs) &&
    ...
    if (i == 4) {
        ...
        entry = jl_typemap_assoc_exact(
                mt->cache, args, nargs, jl_cachearg_offset(mt), world);
        if (entry && entry->isleafsig &&
            entry->simplesig == (void *)jl_nothing &&
    ...
    if (traceen)
        jl_printf(
                JL_STDOUT,
                " at %s:%d\n",
                jl_symbol_name(mfunc->def.method->file),
                mfunc->def.method->line);
#endif
    return mfunc;
}

jl_method_instance_t *jl_lookup_generic(
        jl_value_t **args, uint32_t nargs, uint32_t callsite, size_t world)
{
...
{
    jl_method_instance_t *mfunc = jl_lookup_generic_(
            args,
            nargs,
...
jl_value_t *
jl_gf_invoke(jl_tupletype_t *types0, jl_value_t **args, size_t nargs)
{
    ...
    types = (jl_datatype_t *)jl_argtype_with_function(
            gf, (jl_tupletype_t *)types0);
    jl_methtable_t *mt = jl_gf_mtable(gf);
    jl_typemap_entry_t *entry =
            (jl_typemap_entry_t *)jl_gf_invoke_lookup(types, world);
    ...
    if (method->invokes.unknown != NULL)
        tm = jl_typemap_assoc_exact(
                method->invokes, args, nargs, jl_cachearg_offset(mt), world);
    if (tm) {
        mfunc = tm->func.linfo;
    }
    else {
        ...
        if (tm) {
            ...
            if (jl_is_unionall(entry->sig)) {
                jl_value_t *ti = jl_type_intersection_env(
                        (jl_value_t *)tt, (jl_value_t *)entry->sig, &tpenv);
...
JL_DLLEXPORT jl_value_t *jl_get_invoke_lambda(
        jl_methtable_t *mt,
        jl_typemap_entry_t *entry,
        jl_tupletype_t *tt,
        size_t world)
{
    ...
    jl_typemap_entry_t *tm = NULL;
    if (method->invokes.unknown != NULL) {
        tm = jl_typemap_assoc_by_type(
                method->invokes, tt, NULL, 0, 1, jl_cachearg_offset(mt), world);
        if (tm) {
    ...
    jl_method_instance_t *mfunc = cache_method(
            mt,
            &func->invokes,
            entry->func.value,
            sig,
...
jl_function_t *jl_new_generic_function_with_supertype(
        jl_sym_t *name, jl_module_t *module, jl_datatype_t *st, int iskw)
{
...
JL_DLLEXPORT jl_function_t *jl_get_kwsorter(jl_value_t *ty)
{
    ...
    if (!mt->kwsorter) {
        JL_LOCK(&mt->writelock);
        if (!mt->kwsorter) {
            mt->kwsorter = jl_new_generic_function_with_supertype(
                    tn->name, mt->module, jl_function_type, 1);
...
{
    return jl_new_generic_function_with_supertype(
            name, module, jl_function_type, 0);
}
...
static int ml_matches_visitor(
        jl_typemap_entry_t *ml, struct typemap_intersection_env *closure0)
{
    struct ml_matches_env *closure =
            container_of(closure0, struct ml_matches_env, match);
    int i;
    if (closure->world !=
        0) { // use zero as a flag value for returning all matches
        // ignore method table entries that have been replaced in the current
        // world
    ...
    */
    ...
    if (!skip) {
        /*
          ...
          might have argument type `Tuple{}` and signature type
          `Tuple{Vararg{T}}`,
        ...
        */
        ...
        if (closure0->issubty) {
            ...
            // this method is excluded by the static-parameters-must-have-values
            // rule
            if (!matched_all_tvars(
                        (jl_value_t *)ml->sig,
                        jl_svec_data(env),
                        jl_svec_len(env)))
        ...
        // if we reach a definition that fully covers the arguments but there
        // are
        // ambiguities, then this method might not actually match, so we
        // shouldn't
        // add it to the results.
        if (return_this_match && meth->ambig != jl_nothing &&
            (!closure->include_ambiguous || done)) {
            jl_svec_t *env = NULL;
            jl_value_t *mti = NULL;
            JL_GC_PUSH2(&env, &mti);
            for (size_t j = 0; j < jl_array_len(meth->ambig); j++) {
                jl_method_t *mambig =
                        (jl_method_t *)jl_array_ptr_ref(meth->ambig, j);
                env = jl_emptysvec;
                mti = jl_type_intersection_env(
                ...
                if (mti != (jl_value_t *)jl_bottom_type) {
                    if (closure->include_ambiguous) {
                        ...
                        int k;
                        for (k = 0; k < len; k++) {
                            if ((jl_value_t *)mambig ==
                                jl_svecref(jl_array_ptr_ref(closure->t, k), 2))
                                break;
                        }
                        if (k >= len) {
                            if (len == 0) {
                        ...
                        // the current method definitely never matches if the
                        // intersection with this method
                        // is also fully covered by an ambiguous method's
                        // signature
        ...
        if (return_this_match) {
            ...
            closure->matc =
                    jl_svec(3, closure->match.ti, closure->match.env, meth);
            if (len == 0) {
                ...
                jl_array_ptr_1d_push(
                        (jl_array_t *)closure->t, (jl_value_t *)closure->matc);
            }
        }
        if (done)
            return 0;
