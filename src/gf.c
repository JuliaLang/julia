...
JL_DLLEXPORT void jl_add_codeinst_to_jit(jl_code_instance_t *codeinst, jl_code_info_t *src)
{
    ...
    if (jl_generating_output() && jl_is_method(codeinst->def->def.method) && jl_atomic_load_relaxed(&codeinst->inferred) == jl_nothing) {
        jl_value_t *compressed = jl_compress_ir(codeinst->def->def.method, src);
