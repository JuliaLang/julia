...
JL_DLLEXPORT jl_value_t *jl_methtable_lookup(jl_methtable_t *mt, jl_tupletype_t *type)
{
    jl_typemap_entry_t *sf =
        jl_typemap_assoc_by_type(mt->defs, type, NULL, 2, /*subtype*/ 0, /*offs*/ 0);
    if (!sf)
        return jl_nothing;
    return sf->func.value;
}
...
jl_lambda_info_t *jl_type_infer(jl_lambda_info_t *li, int force)
{
    ...
    if (jl_typeinf_func != NULL &&
        (force || (mod != jl_gf_mtable(jl_typeinf_func)->module &&
                   (mod != jl_core_module || !lastIn)))) { // avoid any potential recursion
                                                           // in calling jl_typeinf_func on
                                                           // itself
...
// after intersection, the argument tuple type needs to be corrected to reflect the
// signature match
        ...
        if (decl_i == jl_ANY_flag) {
            ...
            */
            ...
            if (i < jl_nparams(decl)) {
                ...
                // can give `T=Type` instead of `T=Type{Type{Type{...`.   /* make editors
                // happy:}}} */
            }
            else {
                jl_svecset(*newparams, i, (jl_value_t *)jl_typetype_type);
                                     ...
                                     jl_method_t *definition)
{
    ...
    for (i = 0; i < np; i++) {
        ...
        if (jl_is_vararg_type(elt)) // varargs are always considered compilable
            continue;
        if (is_kind(elt)) // kind slots always need guard entries (checking for subtypes of
                          // Type)
            continue;
        if (decl_i == jl_ANY_flag) {
            // don't specialize on slots marked ANY
             ...
             jl_typemap_entry_t *m, jl_svec_t *sparams)
{
    ...
    if (need_guard_entries) {
        temp =
            ml_matches(mt->defs, 0, type, -1, 0); // TODO: use MAX_UNSPECIALIZED_CONFLICTS?
        int guards = 0;
        if (temp == jl_false) {
            cache_with_orig = 1;
        }
    ...
    if (cache_with_orig) {
        ...
        type = origtype;    // restore `type` to be the `origtype` backup (discard computed
                            // simplified `type`)
...
/*
  ...
  for example, (AbstractArray, AbstractMatrix) and (AbstractMatrix, AbstractArray) are
  ambiguous.
  however, (AbstractArray, AbstractMatrix, Foo) and (AbstractMatrix, AbstractArray, Bar) are
  fine
...
void JL_NORETURN jl_method_error_bare(jl_function_t *f, jl_value_t *args)
{
    ...
    if (fargs[0]) {
        ...
        jl_printf((JL_STREAM *)STDERR_FILENO, "A method error occurred before the base "
                                              "MethodError type was defined. "
                                              "Aborting...\n");
...
static int _compile_all_tvar_union(jl_tupletype_t *methsig, jl_svec_t *tvars)
{
    ...
    for (i = 0; i < tvarslen; /* incremented by inner loop */) {
        ...
        JL_CATCH {
            goto getnext; // sigh, we found an invalid type signature. should we warn the
                          // user?
        }
        ...
        if (jl_is_leaf_type(sig)) {
            if (jl_compile_hint((jl_tupletype_t *)sig)) {
                if (!jl_has_typevars((jl_value_t *)sig))
...
static void _compile_all_deq(jl_array_t *found)
{
    ...
    for (found_i = 0; found_i < found_l; found_i++) {
        if (found_i % (1 + found_l / 300) == 0 ||
            found_i == found_l - 1) // show 300 progress steps, to show progress without
        ...
        // keep track of whether all possible signatures have been cached (and thus whether
        // it can skip trying to compile the template function)
        // this is necessary because many intrinsics try to call static_eval and thus are
        // not compilable unspecialized
...
// STATIC_INLINE jl_typemap_entry_t *sig2_match_fast(jl_value_t **args, jl_typemap_entry_t
// *entry1, jl_typemap_entry_t *entry2, size_t n)
//{
//    // NOTE: This function is a huge performance hot spot!!
...
JL_DLLEXPORT jl_value_t *jl_apply_generic(jl_value_t **args, uint32_t nargs)
{
    ...
    */
    ...
    //        else if (sig_match_fast(args, jl_svec_data(entry1->sig->parameters), 0,
    //        nargs))
    //            entry = entry1;
    //    }
