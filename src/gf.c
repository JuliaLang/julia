...
static uint64_t typeinf_reentrances;
...
JL_DLLEXPORT void jl_typeinf_begin(void)
{
    ...
    if (typeinf_reentrances++ == 0 && jl_atomic_load_relaxed(&jl_measure_compile_time_enabled)) {
...
{
    JL_LOCK_NOGC(&typeinf_timing_mutex);
    if (--typeinf_reentrances == 0 && inference_is_measuring_compile_time) {
