...
static void drop_all_methcache(jl_methcache_t *mc) JL_CANSAFEPOINT
{
    ...
    // Deletion barriers for the two dropped caches.
    jl_gc_wb(mc, (void*)&mc->cache, NULL);
    jl_gc_wb(mc, (void*)&mc->leafcache, NULL);
...
static void _invalidate_backedges(jl_method_instance_t *replaced_mi, jl_code_instance_t *replaced_ci, size_t max_world, int depth) {
    ...
    if (!replaced_ci) {
        ...
        jl_gc_wb(replaced_mi, (void*)&replaced_mi->backedges, NULL);
    ...
    for (i = 0; i < na; i += 2) {
        ...
        *env->isect = *env->isect2 = NULL;
        if (missing) {
            ...
            // remove this entry (cf. `jl_eqtable_pop`); deletion barriers for both slots
...
static int erase_method_backedges(jl_typemap_entry_t *def, void *closure) JL_CANSAFEPOINT
{
    ...
    if (jl_is_svec(specializations)) {
        ...
        for (i = 0; i < l; i++) {
            jl_method_instance_t *mi = (jl_method_instance_t*)jl_svecref(specializations, i);
            if ((jl_value_t*)mi != jl_nothing) {
                jl_gc_wb(mi, (void*)&mi->backedges, NULL);
                mi->backedges = 0;
            }
        }
    }
