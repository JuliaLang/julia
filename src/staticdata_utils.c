...
static int jl_collect_methcache_from_mod(jl_typemap_entry_t *ml, void *closure) JL_CANSAFEPOINT
{
    ...
    if (!jl_object_in_image((jl_value_t*)m->module)) {
        if (s) {
            ...
            if (cert != jl_nothing) {
                // Save the final dispatch bits. A later activation may have cleared
                // LATEST_ONLY after this one recorded its bits, and replay does not
...
static const int JI_FORMAT_VERSION = 17; // extext_methods carries (method, activation cert) pairs
