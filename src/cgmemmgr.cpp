...
{
    return (size > jl_page_size * 256 ? LLT_ALIGN(size, jl_page_size) : jl_page_size * 256);
}
...
{
#ifdef _OS_WINDOWS_
    char *mem = (char *)VirtualAlloc(NULL, size + jl_page_size, MEM_COMMIT, PAGE_READWRITE);
...
#ifdef _OS_WINDOWS_
enum class Prot : int { RW = PAGE_READWRITE, RX = PAGE_EXECUTE, RO = PAGE_READONLY };
...
static void protect_page(void *ptr, size_t size, Prot flags)
{
    ...
    if (!VirtualProtect(ptr, size, (DWORD)flags, &old_prot)) {
        jl_safe_printf("Cannot protect page @%p of size %u to 0x%x (err 0x%x)\n", ptr,
                       (unsigned)size, (unsigned)flags, (unsigned)GetLastError());
        abort();
    }
}
#else  // _OS_WINDOWS_
...
static bool check_fd_or_close(int fd)
{
    ...
    void *ptr = mmap(nullptr, jl_page_size, PROT_READ | PROT_EXEC, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED) {
        close(fd);
        return false;
    }
...
{
    void *addr = MapViewOfFile((HANDLE)id, FILE_MAP_ALL_ACCESS, 0, 0, size);
...
static void *alloc_shared_page(size_t size, size_t *id, bool exec)
{
    ...
    HANDLE hdl = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, file_mode, 0, size, NULL);
    *id        = (size_t)hdl;
...
// file system.
    ...
    if (check_fd_or_close(fd)) return fd;
#endif
#ifdef _OS_FREEBSD_
...
// `shm_open` can't be mapped exec on mac
#ifndef _OS_DARWIN_
    do {
        snprintf(shm_name, sizeof(shm_name), "julia-codegen-%d-%d", (int)pid, rand());
    ...
    snprintf(shm_name, sizeof(shm_name), "/tmp/julia-codegen-%d-XXXXXX", (int)pid);
    fd = mkstemp(shm_name);
    if (check_fd_or_close(fd)) {
        unlink(shm_name);
        return fd;
...
static void *create_shared_map(size_t size, size_t id)
{
    void *addr = mmap(nullptr, size, PROT_READ | PROT_WRITE, MAP_SHARED, anon_hdl, id);
...
static void *alloc_shared_page(size_t size, size_t *id, bool exec)
{
    ...
    *id        = off;
    if (__unlikely(off + size > map_size)) {
        ...
        while (off + size > map_size) map_size += map_size_inc;
...
struct Block {
    ...
    Block(Block &&other) : ptr(other.ptr), total(other.total), avail(other.avail)
    {
        other.ptr   = nullptr;
    ...
    void *alloc(size_t size, size_t align)
    {
        ...
        if (aligned_avail < size) return nullptr;
...
public:
    void *alloc(size_t size, size_t align)
    {
        ...
        for (int i = 0; i < nblocks && blocks[i].ptr; i++) {
            if (void *ptr = blocks[i].alloc(size, align)) return ptr;
...
struct SplitPtrBlock : public Block {
    ...
    SplitPtrBlock(SplitPtrBlock &&other) : SplitPtrBlock() { swap(other); }
};
...
template <bool exec> class ROAllocator {
protected:
    static constexpr int nblocks = 8;
    ...
    virtual void *get_wr_ptr(SplitPtrBlock &block, void *rt_ptr, size_t size,
                             size_t align)         = 0;
...
template <bool exec> class DualMapAllocator : public ROAllocator<exec> {
protected:
    void *get_wr_ptr(SplitPtrBlock &block, void *rt_ptr, size_t, size_t) override
    {
        assert((char *)rt_ptr >= block.ptr && (char *)rt_ptr < (block.ptr + block.total));
        if (!(block.state & SplitPtrBlock::WRInit)) {
            block.wr_ptr = (uintptr_t)create_shared_map(block.total, block.wr_ptr);
    ...
    void finalize_block(SplitPtrBlock &block, bool reset)
    {
        ...
        if (block.state & SplitPtrBlock::InitAlloc) {
            ...
            assert(!(block.state & (SplitPtrBlock::WRReady | SplitPtrBlock::WRInit)));
...
public:
    DualMapAllocator() { assert(anon_hdl != -1); }
...
#ifdef _OS_LINUX_
template <bool exec> class SelfMemAllocator : public ROAllocator<exec> {
    SmallVector<Block, 16> temp_buff;

protected:
    void *get_wr_ptr(SplitPtrBlock &block, void *rt_ptr, size_t size, size_t align) override
    {
    ...
    void finalize_block(SplitPtrBlock &block, bool reset)
    {
        if (!(block.state & SplitPtrBlock::Alloc)) return;
        if (block.state & SplitPtrBlock::InitAlloc) {
...
public:
    SelfMemAllocator() : ROAllocator<exec>(), temp_buff() { assert(self_mem_fd != -1); }
    void finalize() override
    {
        for (auto &block : this->blocks) {
            finalize_block(block, false);
        ...
        for (auto &alloc : this->allocations) {
            if (alloc.rt_addr == alloc.wr_addr) continue;
...
public:
    RTDyldMemoryManagerJL()
        : SectionMemoryManager(), pending_eh(), rw_alloc(), ro_alloc(), exe_alloc(),
    ...
    template <typename DL, typename Alloc> void mapAddresses(DL &Dyld, Alloc &&allocator)
    {
        for (auto &alloc : allocator->allocations) {
            if (alloc.rt_addr == alloc.wr_addr || alloc.relocated) continue;
            alloc.relocated = true;
            Dyld.mapSectionAddress(alloc.wr_addr, (uintptr_t)alloc.rt_addr);
        }
    }
    template <typename DL> void mapAddresses(DL &Dyld)
    {
        if (!ro_alloc) return;
...
#ifdef _OS_WINDOWS_
    template <typename Alloc> void *lookupWriteAddressFor(void *rt_addr, Alloc &&allocator)
    {
        for (auto &alloc : allocator->allocations) {
    ...
    {
        if (!ro_alloc) return rt_addr;
        if (void *ptr = lookupWriteAddressFor(rt_addr, ro_alloc)) return ptr;
        if (void *ptr = lookupWriteAddressFor(rt_addr, exe_alloc)) return ptr;
        return rt_addr;
    }
#endif // _OS_WINDOWS_
};

uint8_t *RTDyldMemoryManagerJL::allocateCodeSection(uintptr_t Size, unsigned Alignment,
                                                    unsigned  SectionID,
                                                    StringRef SectionName)
{
    ...
    code_allocated = true;
    if (exe_alloc) return (uint8_t *)exe_alloc->alloc(Size, Alignment);
    return SectionMemoryManager::allocateCodeSection(Size, Alignment, SectionID,
                                                     SectionName);
}

uint8_t *RTDyldMemoryManagerJL::allocateDataSection(uintptr_t Size, unsigned Alignment,
                                                    unsigned  SectionID,
                                                    StringRef SectionName, bool isReadOnly)
{
    if (!isReadOnly) return (uint8_t *)rw_alloc.alloc(Size, Alignment);
    if (ro_alloc) return (uint8_t *)ro_alloc->alloc(Size, Alignment);
...
bool RTDyldMemoryManagerJL::finalizeMemory(std::string *ErrMsg)
{
    ...
    if (ro_alloc) {
        ...
        assert(exe_alloc);
        exe_alloc->finalize();
        for (auto &frame : pending_eh) register_eh_frames(frame.addr, frame.size);
        pending_eh.clear();
        return false;
    }
    else {
...
void RTDyldMemoryManagerJL::registerEHFrames(uint8_t *Addr, uint64_t LoadAddr, size_t Size)
{
...
void RTDyldMemoryManagerJL::deregisterEHFrames(uint8_t *Addr, uint64_t LoadAddr,
                                               size_t Size)
{
