...
static jl_value_t *
eval_body(jl_array_t *stmts, interpreter_state *s, int start, int toplevel);

jl_value_t *jl_eval_module_expr(jl_module_t *m, jl_expr_t *ex);
int jl_is_toplevel_only_expr(jl_value_t *e);

jl_value_t *jl_interpret_toplevel_expr_in(
        jl_module_t *m,
        jl_value_t *e,
        jl_code_info_t *src,
        jl_svec_t *sparam_vals)
...
static jl_value_t *
do_call(jl_value_t **args, size_t nargs, interpreter_state *s)
{
    jl_value_t **argv;
...
static jl_value_t *
do_invoke(jl_value_t **args, size_t nargs, interpreter_state *s)
{
    jl_value_t **argv;
...
static int equiv_type(jl_datatype_t *dta, jl_datatype_t *dtb)
{
    if (!(jl_typeof(dta) == jl_typeof(dtb) &&
          ...
          jl_egal((jl_value_t *)dta->name->names,
                  (jl_value_t *)dtb->name->names) &&
    ...
    while (jl_is_unionall(a)) {
        ...
        if (!jl_egal(ua->var->lb, ub->var->lb) ||
            !jl_egal(ua->var->ub, ub->var->ub) ||
    ...
    for (i = 0; i < nf; i++) {
        ...
        if (jl_has_free_typevars(ta)) {
            if (!jl_has_free_typevars(tb) || !jl_egal(ta, tb))
                ...
                jl_has_free_typevars(tb) || jl_typeof(ta) != jl_typeof(tb) ||
...
static void check_can_assign_type(jl_binding_t *b, jl_value_t *rhs)
{
    if (b->constp && b->value != NULL && jl_typeof(b->value) != jl_typeof(rhs))
        jl_errorf(
                "invalid redefinition of constant %s", jl_symbol_name(b->name));
}
...
void jl_set_datatype_super(jl_datatype_t *tt, jl_value_t *super)
{
    if (!jl_is_datatype(super) || !jl_is_abstracttype(super) ||
        ...
        jl_errorf(
                "invalid subtyping in definition of %s",
...
{
    return jl_is_long(src->ssavaluetypes) ? jl_unbox_long(src->ssavaluetypes)
                                          : jl_array_len(src->ssavaluetypes);
}
...
static jl_value_t *eval(jl_value_t *e, interpreter_state *s)
{
    ...
    if (jl_is_ssavalue(e)) {
        ...
        if (src == NULL || id >= jl_source_nssavalues(src) || id < 0 ||
            s->locals == NULL)
    ...
    if (jl_is_symbol(e)) { // bare symbols appear in toplevel exprs not wrapped
                           // in `thunk`
    ...
    if (ex->head == isdefined_sym) {
        ...
        if (jl_is_symbol(fname)) {
            ...
            jl_value_t *gf =
                    jl_generic_function_def(fname, modu, bp, bp_owner, b);
        ...
        if (inside_typedef)
            jl_error("cannot eval a new abstract type definition while "
        ...
        b->value = temp;
        if (temp == NULL ||
            !equiv_type(dt, (jl_datatype_t *)jl_unwrap_unionall(temp))) {
        ...
        if (inside_typedef)
            jl_error("cannot eval a new bits type definition while defining "
                     "another type");
        jl_value_t *name = args[0];
        ...
        if (!jl_is_long(vnb))
            jl_errorf(
                    "invalid declaration of bits type %s",
                    jl_symbol_name((jl_sym_t *)name));
        ssize_t nb = jl_unbox_long(vnb);
        if (nb < 1 || nb >= (1 << 23) || (nb & 7) != 0)
            jl_errorf(
                    "invalid number of bits in type %s",
                    jl_symbol_name((jl_sym_t *)name));
        ...
        if (inside_typedef)
            jl_error("cannot eval a new data type definition while defining "
                     "another type");
        jl_value_t *name = args[0];
        jl_value_t *para = eval(args[1], s);
        jl_value_t *temp = NULL;
        ...
        if (para == (jl_value_t *)jl_emptysvec &&
            jl_is_datatype_make_singleton(dt)) {
        ...
        jl_throw(args[0]);
            ...
            ex->head == boundscheck_sym || ex->head == inbounds_sym ||
            ex->head == fastmath_sym || ex->head == simdloop_sym ||
            ex->head == meta_sym) {
        return jl_nothing;
    }
    jl_errorf(
            "unsupported or misplaced expression %s", jl_symbol_name(ex->head));
    return (jl_value_t *)jl_nothing;
}
...
eval_body(jl_array_t *stmts, interpreter_state *s, int start, int toplevel)
{
    ...
    while (1) {
        if (i >= ns)
            jl_error("`body` expression must terminate in `return`. Use "
                     "`block` instead.");
        if (toplevel)
        ...
        if (jl_is_gotonode(stmt)) {
            ...
            if (head == return_sym) {
                ...
                if (cond == jl_false) {
                    ...
                    jl_type_error_rt(
                            "toplevel", "if", (jl_value_t *)jl_bool_type, cond);
                }
            }
...
#ifdef _OS_WINDOWS_
                    if (jl_get_ptls_states()->exception_in_transit ==
...
jl_value_t *
jl_interpret_call(jl_method_instance_t *lam, jl_value_t **args, uint32_t nargs)
{
    if (lam->jlcall_api == 2)
    ...
    jl_value_t **locals;
    JL_GC_PUSHARGS(
            locals, jl_source_nslots(src) + jl_source_nssavalues(src) + 2);
