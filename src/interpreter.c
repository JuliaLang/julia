...
static jl_value_t *eval_body(jl_array_t *stmts, interpreter_state *s, int start,
                             int toplevel);

jl_value_t *jl_eval_module_expr(jl_expr_t *ex);
int jl_is_toplevel_only_expr(jl_value_t *e);

jl_value_t *jl_interpret_toplevel_expr(jl_value_t *e) { return eval(e, NULL); }
...
static jl_value_t *do_call(jl_value_t **args, size_t nargs, interpreter_state *s)
{
    ...
    size_t i;
    for (i = 0; i < nargs; i++) argv[i] = eval(args[i], s);
...
static jl_value_t *do_invoke(jl_value_t **args, size_t nargs, interpreter_state *s)
{
    ...
    size_t i;
    for (i = 1; i < nargs; i++) argv[i - 1] = eval(args[i], s);
...
jl_value_t *jl_eval_global_var(jl_module_t *m, jl_sym_t *e)
{
    ...
    if (v == NULL) jl_undefined_var_error(e);
    return v;
}
...
static int equiv_type(jl_datatype_t *dta, jl_datatype_t *dtb)
{
    return (jl_typeof(dta) == jl_typeof(dtb) &&
            ...
            dta->parameters == jl_emptysvec && dta->name->name == dtb->name->name &&
            jl_egal((jl_value_t *)dta->types, (jl_value_t *)dtb->types) &&
            dta->abstract == dtb->abstract && dta->mutabl == dtb->mutabl &&
            dta->size == dtb->size && dta->ninitialized == dtb->ninitialized &&
...
static void check_can_assign_type(jl_binding_t *b)
{
    if (b->constp && b->value != NULL && !jl_is_datatype(b->value))
        jl_errorf("invalid redefinition of constant %s", jl_symbol_name(b->name));
}
...
void jl_set_datatype_super(jl_datatype_t *tt, jl_value_t *super)
{
    if (!jl_is_datatype(super) || !jl_is_abstracttype(super) ||
        ...
        jl_subtype(super, (jl_value_t *)jl_vararg_type, 0) || jl_is_tuple_type(super) ||
        jl_subtype(super, (jl_value_t *)jl_type_type, 0) ||
        super == (jl_value_t *)jl_builtin_type) {
        jl_errorf("invalid subtyping in definition of %s", jl_symbol_name(tt->name->name));
    }
    tt->super = (jl_datatype_t *)super;
    jl_gc_wb(tt, tt->super);
}

static int jl_linfo_nslots(jl_lambda_info_t *li) { return jl_array_len(li->slotflags); }

static int jl_linfo_nssavalues(jl_lambda_info_t *li)
{
    return jl_is_long(li->ssavaluetypes) ? jl_unbox_long(li->ssavaluetypes)
                                         : jl_array_len(li->ssavaluetypes);
}
...
static jl_value_t *eval(jl_value_t *e, interpreter_state *s)
{
    ...
    if (jl_is_globalref(e)) {
        ...
        if (v == NULL) jl_undefined_var_error(s);
        return v;
    }
    if (jl_is_quotenode(e)) return jl_fieldref(e, 0);
    jl_module_t *modu =
        (lam == NULL || lam->def == NULL) ? ptls->current_module : lam->def->module;
    if (jl_is_symbol(e)) { // bare symbols appear in toplevel exprs not wrapped in `thunk`
        jl_value_t *v = jl_get_global(modu, (jl_sym_t *)e);
        if (v == NULL) jl_undefined_var_error((jl_sym_t *)e);
        return v;
    }
    if (!jl_is_expr(e)) return e;
    ...
    if (ex->head == call_sym) {
        ...
        assert(n > 0);
        if (s->sparam_vals) return jl_svecref(s->sparam_vals, n - 1);
        if (n <= jl_svec_len(lam->sparam_vals)) {
            jl_value_t *sp = jl_svecref(lam->sparam_vals, n - 1);
            if (!jl_is_typevar(sp)) return sp;
        }
        ...
        if (jl_is_symbol(fname)) {
            ...
            if (jl_expr_nargs(ex) == 1) return gf;
        }
        ...
        if (inside_typedef)
            jl_error("cannot eval a new abstract type definition while defining "
                     "another type");
...
#endif
        dt = jl_new_datatype((jl_sym_t *)name, NULL, (jl_svec_t *)para, (jl_svec_t *)temp,
                             NULL, 0, args[5] == jl_true ? 1 : 0, jl_unbox_long(args[6]));
        ...
        JL_TRY {
            ...
            for (size_t i = 0; i < jl_svec_len(dt->types); i++) {
                ...
                if (!jl_is_type(elt) && !jl_is_typevar(elt))
                    jl_type_error_rt(jl_symbol_name(dt->name->name), "type definition",
        ...
        if (temp == NULL || !equiv_type(dt, (jl_datatype_t *)temp)) {
            ...
            // TODO: remove all old ctors and set temp->name->ctor_factory =
            // dt->name->ctor_factory
        }
        ...
        if (nargs == 0) jl_error("malformed \"error\" expression");
        if (jl_is_string(args[0])) jl_errorf("syntax: %s", jl_string_data(args[0]));
        jl_throw(args[0]);
    }
    else if (ex->head == boundscheck_sym || ex->head == inbounds_sym ||
             ex->head == fastmath_sym || ex->head == simdloop_sym || ex->head == meta_sym) {
        return jl_nothing;
    }
                             ...
                             int toplevel)
{
    ...
    while (1) {
        ...
        if (jl_is_gotonode(stmt)) {
            ...
            if (head == return_sym) {
                ...
                if (jl_is_ssavalue(sym)) {
                    ...
                    if (jl_is_globalref(sym)) {
                        ...
                        m = (s == NULL || s->lam == NULL || s->lam->def == NULL)
                                ? ptls->current_module
                                : s->lam->def->module;
                    }
                    assert(jl_is_symbol(sym));
                ...
                if (toplevel) jl_lineno = jl_unbox_long(jl_exprarg(stmt, 0));
            ...
            if (toplevel) jl_lineno = jl_linenode_line(stmt);
            // TODO: interpreted function line numbers
        }
