...
static jl_value_t *eval(jl_value_t *e, jl_value_t **locals, jl_lambda_info_t *lam)
{
    ...
    if (jl_typeis(e,jl_assignnode_type)) {
        ...
        jl_value_t *rhs = eval(jl_fieldref(e,1), locals, lam);
        if (jl_is_gensym(sym)) {
                             ...
                             int start, int toplevel)
{
    ...
    while (1) {
        ...
        if (jl_typeis(stmt, jl_gotoifnotnode_type)) {
            jl_value_t *cond = eval(jl_fieldref(stmt,0), locals, lam);
            if (cond == jl_false) {
                i = jl_unbox_long(jl_fieldref(stmt,1))-1;
                continue;
            }
...
jl_value_t *jl_interpret_toplevel_thunk(jl_lambda_info_t *lam)
{
    ...
    jl_value_t **locals;
    JL_GC_PUSHARGS(locals, jl_array_len(lam->slotnames) + jl_array_len(lam->gensymtypes));
