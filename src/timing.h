...
#define JL_TIMING_OWNERS                                                       \
    X(ROOT)                                                                    \
    , X(GC), X(LOWERING), X(PARSING), X(INFERENCE), X(CODEGEN),                \
            X(METHOD_LOOKUP_SLOW), X(METHOD_LOOKUP_FAST), X(LLVM_OPT),         \
            X(LLVM_MODULE_FINISH), X(LLVM_EMIT), X(METHOD_LOOKUP_COMPILE),     \
            X(TYPE_CACHE_LOOKUP), X(TYPE_CACHE_INSERT), X(STAGED_FUNCTION),    \
            X(MACRO_INVOCATION), X(AST_COMPRESS), X(AST_UNCOMPRESS),           \
            X(SYSIMG_LOAD), X(SYSIMG_DUMP), X(NATIVE_DUMP),
...
_jl_timing_block_init(jl_timing_block_t *block, int owner)
{
    uint64_t t = rdtscp();
    block->owner = owner;
    block->total = 0;
