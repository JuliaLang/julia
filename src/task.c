...
static void fill(struct _probe_data *p) { boundlow(p); }
...
static void _probe_arch(void)
{
    ...
    mangle_pointers = !(s[4] > ptls->stack_lo && s[4] < ptls->stack_hi);
#elif defined(__linux__) && defined(__x86_64__)
    jl_ptls_t ptls  = jl_get_ptls_states();
    char **   s     = (char **)p.ref_probe;
    mangle_pointers = !(s[6] > ptls->stack_lo && s[6] < ptls->stack_hi);
#else
    mangle_pointers = 0;
#endif
...
{
    if (t->state == done_sym || t->state == failed_sym) return;
...
static void NOINLINE restore_stack(jl_ptls_t ptls, jl_task_t *t, jl_jmp_buf *where, char *p)
{
...
// TODO: early free of t->stkbuf
    ...
    if (task_done_hook_func == NULL) {
        task_done_hook_func =
            (jl_function_t *)jl_get_global(jl_base_module, jl_symbol("task_done_hook"));
    }
    if (task_done_hook_func != NULL) {
...
{
    jl_ptls_t ptls  = jl_get_ptls_states();
    ptls->stackbase = (char *)(((uintptr_t)__stk + sizeof(*__stk)) &
                               -16); // also ensures stackbase is 16-byte aligned
...
{
    if (t == ptls->current_task) return;
#ifdef ENABLE_TIMINGS
    jl_timing_block_t *blk = ptls->current_task->timing_stack;
    if (blk) jl_timing_block_stop(blk);
#endif
...
#error ASM_COPY_STACKS not supported on this cpu architecture
    ...
    if (blk) jl_timing_block_start(blk);
#endif
}
...
JL_DLLEXPORT jl_value_t *jl_switchto(jl_task_t *t, jl_value_t *arg)
{
    ...
    if (t->state == done_sym || t->state == failed_sym ||
        (t->stkbuf == (void *)(intptr_t)-1)) {
        if (t->exception != jl_nothing) jl_throw(t->exception);
        return t->result;
    }
    if (ptls->in_finalizer) jl_error("task switch not allowed from inside gc finalizer");
    if (in_pure_callback) jl_error("task switch not allowed from inside staged function");
    ...
    if (other_defer_signal && !defer_signal) jl_sigint_safepoint(ptls);
    return val;
}
...
void JL_NORETURN throw_internal(jl_value_t *e)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    ptls->io_wait  = 0;
    if (ptls->safe_restore) jl_longjmp(*ptls->safe_restore, 1);
...
JL_DLLEXPORT void jl_throw(jl_value_t *e)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    assert(e != NULL);
    if (!ptls->safe_restore) record_backtrace();
    throw_internal(e);
}
...
JL_DLLEXPORT void jl_rethrow_other(jl_value_t *e) { throw_internal(e); }
...
{
    _probe_arch();
    jl_task_type = jl_new_datatype(
        jl_symbol("Task"), jl_any_type, jl_emptysvec,
        jl_svec(13, jl_symbol("parent"), jl_symbol("storage"), jl_symbol("state"),
                jl_symbol("consumers"), jl_symbol("donenotify"), jl_symbol("result"),
                jl_symbol("exception"), jl_symbol("backtrace"), jl_symbol("code"),
                jl_symbol("ctx"), jl_symbol("bufsz"), jl_symbol("stkbuf"),
                jl_symbol("ssize")),
        jl_svec(13, jl_any_type, jl_any_type, jl_sym_type, jl_any_type, jl_any_type,
    ...
    // jl_unprotect_stack_func = jl_new_closure(jl_unprotect_stack,
    // (jl_value_t*)jl_emptysvec, NULL);
}
...
{
    jl_ptls_t ptls     = jl_get_ptls_states();
    ptls->current_task = (jl_task_t *)jl_gc_alloc(ptls, sizeof(jl_task_t), jl_task_type);
#ifdef COPY_STACKS
...
JL_DLLEXPORT int jl_is_task_started(jl_task_t *t) { return t->started; }
