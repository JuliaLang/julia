...
private:
    ...
    /// If present, the annotation is an MDNode attached to an instruction in the loop's
    /// latch.
    bool hasSIMDLoopMetadata(Loop *L) const;

    /// If Phi is part of a reduction cycle of FAdd or FMul, mark the ops as permitting
    /// reassociation/commuting.
...
// Loop does not have a LoopID yet, so give it one.
    ...
    // Mark memory references so that Loop::isAnnotatedParallel will return true for this
    // loop.
