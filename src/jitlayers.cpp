...
CISymbolPtr *JuliaOJIT::linkCISymbol(jl_code_instance_t *CI)
{
    ...
    // linkCallTarget relinks a CI whose entry has a different invoke_api; its old
    // names no longer belong to CISymbols
    auto &CISym = CISymbols[CI];
    if (CISym.invoke)
        CISymbolNames.erase(CISym.invoke);
    if (CISym.specptr)
        CISymbolNames.erase(CISym.specptr);
    CISym = {API, InvokeSym, SpecSym};
    if (InvokeSym)
