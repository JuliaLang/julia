...
// Except for parts of this file which were copied from LLVM, under the UIUC
// license (marked below).

// this defines the set of optimization passes defined for Julia at various
// optimization levels
void addOptimizationPasses(legacy::PassManagerBase *PM, int opt_level)
{
    ...
    PM->add(createTargetTransformInfoWrapperPass(
            jl_TargetMachine->getTargetIRAnalysis()));
...
// effectiveness of the optimization, but should retain correctness.
    ...
    PM->add(createLowerSimdLoopPass()); // Annotate loop marked with "simdloop"
                                        // as LLVM parallel loop
    ...
    if (jl_options.opt_level >= 3)
        PM->add(createInstructionCombiningPass()); // Clean up after SLP loop
                                                   // vectorizer
...
jl_add_optimization_passes(LLVMPassManagerRef PM, int opt_level)
{
...
{
    static void *atomic_hdl =
            jl_load_dynamic_library_e("libatomic", JL_RTLD_LOCAL);
...
// TODO: hook up RegisterJITEventListener, instead of hard-coding the GDB and
// JuliaListener targets
template <typename ObjSetT, typename LoadResult>
void JuliaOJIT::DebugObjectRegistrar::operator()(
        ...
        const LoadResult &LOS)
{
    ...
    for (; oit != Objects.end(); ++oit, ++lit) {
        ...
        OwningBinary<object::ObjectFile> SavedObject =
                LO->getObjectForDebug(*Object);
        ...
        if (!SavedObject.getBinary()) {
            ...
            auto NewObj =
                    ObjectFile::createObjectFile(NewBuffer->getMemBufferRef());
            assert(NewObj);
            SavedObject = OwningBinary<object::ObjectFile>(
        ...
        for (auto &Symbol : Object->symbols()) {
            ...
            // (which would present a lazy-initializer functor interface
            // instead)
...
void JuliaOJIT::DebugObjectRegistrar::NotifyGDB(
        OwningBinary<object::ObjectFile> &DebugObj)
{
    const char *Buffer =
    ...
    if (!JITCodeEntry) {
        jl_printf(
                JL_STDERR,
                "WARNING: Allocation failed when registering a JIT entry!\n");
    }
    else {
...
JuliaOJIT::JuliaOJIT(TargetMachine &TM)
    : TM(TM), DL(TM.createDataLayout()), ObjStream(ObjBufferSV),
      MemMgr(createRTDyldMemoryManager()),
      ObjectLayer(DebugObjectRegistrar(*this)),
      CompileLayer(ObjectLayer, [this](Module &M) {
          JL_TIMING(LLVM_OPT);
          PM.run(M);
          ...
          if (!Obj) {
              ...
              llvm::report_fatal_error(
                      ...
                      "'\n"
                      "The module's content was printed above. Please file a "
                      "bug report");
          }
      ...
      })
{
    addOptimizationPasses(
            &PM, jl_generating_output() ? 0 : jl_options.opt_level);
...
void JuliaOJIT::addModule(std::unique_ptr<Module> M)
{
    ...
    auto Resolver = orc::createLambdaResolver(
            [&](const std::string &Name) {
                ...
                // Step 0: ObjectLinkingLayer has checked whether it is in the
                // current module
    ...
    auto modset = CompileLayer.addModuleSet(
            std::move(Ms), MemMgr, std::move(Resolver));
...
// and it decides whether the definition needs a Comdat section and adds the
// appropriate declaration
// TODO: consider moving this into jl_add_to_shadow or jl_dump_shadow? the JIT
// doesn't care, so most calls are now no-ops
...
static T *addComdat(T *G)
{
    ...
    if (imaging_mode && !G->isDeclaration()) {
        ...
        if (G->getParent() == shadow_output) {
            ...
            // Add unwind exception personalities to functions to handle async
            // exceptions
            assert(!juliapersonality_func ||
                   juliapersonality_func->getParent() == shadow_output);
...
static void jl_merge_module(Module *dest, std::unique_ptr<Module> src)
{
    ...
    for (Module::alias_iterator I = src->alias_begin(), E = src->alias_end();
         I != E;) {
        GlobalAlias *sG = &*I;
        GlobalValue *dG = dest->getNamedValue(sG->getName());
        ++I;
        if (dG) {
            if (!dG->isDeclaration()) { // aliases are always definitions, so
                                        // this test is reversed from the above
                                        // two
                sG->replaceAllUsesWith(dG);
                sG->eraseFromParent();
                continue;
    ...
    if (sNMD) {
        ...
        for (NamedMDNode::op_iterator I = sNMD->op_begin(), E = sNMD->op_end();
             I != E;
             ++I) {
            dNMD->addOperand(*I);
        }
...
static void jl_add_to_ee(std::unique_ptr<Module> m)
{
    ...
    ArrayType *atype = ArrayType::get(
            T_uint32, 3); // want 4-byte alignment of 12-bytes of data
    (new GlobalVariable(
             *m,
             atype,
             false,
...
static void jl_merge_recursive(Module *m, Module *collector)
{
    // probably not many unresolved declarations, but be sure to iterate over
    // their Names,
...
void jl_finalize_module(Module *m, bool shadow)
{
    ...
    // in the newer JITs, the shadow module is separate from the execution
    // module
    if (shadow)
        jl_add_to_shadow(m);
}

// helper function for adding a DLLImport (dlsym) address to the execution
// engine
// (for values created locally or in the sysimage, jl_emit_and_add_to_shadow is
...
//
// if in imaging_mode, emit a GlobalVariable with the same name and an
// initializer to the shadow_module
...
// to the execution engine to make it valid for the current session (with the
// current value)
void *jl_emit_and_add_to_shadow(GlobalVariable *gv, void *gvarinit)
{
    PointerType *T = cast<PointerType>(
            gv->getType()->getElementType()); // pointer is the only supported
                                              // type here
...
{
    void *p =
            (void *)(intptr_t)jl_ExecutionEngine->getPointerToGlobalIfAvailable(
                    gv);
    assert(p);
    return p;
}
...
static void emit_offset_table(
        Module *mod, const std::vector<GlobalValue *> &vars, StringRef name)
{
    assert(!vars.empty());
    addComdat(GlobalAlias::create(
            GlobalVariable::ExternalLinkage, name + "_base", vars[0]));
    auto vbase = ConstantExpr::getPtrToInt(vars[0], T_size);
    size_t nvars = vars.size();
    std::vector<Constant *> offsets(nvars);
    for (size_t i = 0; i < nvars; i++) {
        auto ptrdiff = ConstantExpr::getSub(
                ConstantExpr::getPtrToInt(vars[i], T_size), vbase);
        offsets[i] = sizeof(void *) == 8
                             ? ConstantExpr::getTrunc(ptrdiff, T_uint32)
                             : ptrdiff;
    }
    ArrayType *vars_type = ArrayType::get(T_uint32, nvars);
    addComdat(new GlobalVariable(
            *mod,
            vars_type,
            true,
...
static void
jl_gen_llvm_globaldata(Module *mod, const char *sysimg_data, size_t sysimg_len)
{
    ...
    Constant *feature_string =
            ConstantDataArray::getString(jl_LLVMContext, jl_options.cpu_target);
    addComdat(new GlobalVariable(
            *mod,
    ...
    GlobalValue *jlRTLD_DEFAULT_var =
            mod->getNamedValue("jl_RTLD_DEFAULT_handle");
    addComdat(new GlobalVariable(
            *mod,
    ...
    if (sysimg_data) {
        Constant *data = ConstantDataArray::get(
                jl_LLVMContext,
                ArrayRef<uint8_t>(
                        (const unsigned char *)sysimg_data, sysimg_len));
        addComdat(new GlobalVariable(
                          *mod,
                          data->getType(),
                          false,
...
// takes the running content that has collected in the shadow module and dump it
// to disk
// this builds the object file portion of the sysimage files for fast startup
extern "C" void jl_dump_native(
        const char *bc_fname,
...
// make sure to emit the native object format, even if FORCE_ELF was set in
// codegen
#if defined(_OS_WINDOWS_)
...
#endif
                    // Use small model so that we can use signed 32bits offset
                    // in the function and GV tables
                    CodeModel::Small,
                    CodeGenOpt::Aggressive // -O3 TODO: respect command -O0
                                           // flag?
                    ));
    ...
    if (unopt_bc_fname) {
        // call output handler directly to avoid special case handling of `-`
        // filename
        int FD;
        std::error_code EC =
                sys::fs::openFileForWrite(unopt_bc_fname, FD, sys::fs::F_None);
        unopt_bc_OS.reset(new raw_fd_ostream(FD, true));
        std::string err;
        if (EC)
            err = "ERROR: failed to open --output-unopt-bc file '" +
                  std::string(unopt_bc_fname) + "': " + EC.message();
        if (!err.empty())
            jl_safe_printf("%s\n", err.c_str());
        else {
    ...
    if (bc_fname) {
        ...
        int FD;
        std::error_code EC =
                sys::fs::openFileForWrite(bc_fname, FD, sys::fs::F_None);
        bc_OS.reset(new raw_fd_ostream(FD, true));
        std::string err;
        if (EC)
            err = "ERROR: failed to open --output-bc file '" +
                  std::string(bc_fname) + "': " + EC.message();
        if (!err.empty())
            jl_safe_printf("%s\n", err.c_str());
        else {
    ...
    if (obj_fname) {
        ...
        int FD;
        std::error_code EC =
                sys::fs::openFileForWrite(obj_fname, FD, sys::fs::F_None);
        obj_OS.reset(new raw_fd_ostream(FD, true));
        std::string err;
        if (EC)
            err = "ERROR: failed to open --output-o file '" +
                  std::string(obj_fname) + "': " + EC.message();
        if (!err.empty())
            jl_safe_printf("%s\n", err.c_str());
        else {
            if (TM->addPassesToEmitFile(
                        ...
                        *obj_OS.get(),
                        ...
                        false)) {
                jl_safe_printf("ERROR: target does not support generation of "
                               "object files\n");
            }
        }
    }
...
public:
    ...
    Pass *
    createPrinterPass(raw_ostream &O, const std::string &Banner) const override
    {
...
static RegisterPass<JuliaPipeline<2>>
        Y("julia", "Runs the entire julia pipeline (at -O2)", false, false);
