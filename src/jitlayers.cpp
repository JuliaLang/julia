...
// PM->add(createCFGSimplificationPass());    // Merge & remove BBs
    ...
    PM->add(createLowerSimdLoopPass()); // Annotate loop marked with "simdloop" as
                                        // LLVM parallel loop
...
{
    static void *atomic_hdl = jl_load_dynamic_library_e("libatomic", JL_RTLD_LOCAL);
    static const char *const atomic_prefix = "__atomic_";
    if (!atomic_hdl) return 0;
                                                 ...
                                                 const LoadResult &LOS)
{
    ...
    for (; oit != Objects.end(); ++oit, ++lit) {
        ...
        if (!SavedObject.getBinary()) {
            ...
            assert(NewObj);
            SavedObject =
                OwningBinary<object::ObjectFile>(std::move(*NewObj), std::move(NewBuffer));
        }
        else {
        ...
        ORCNotifyObjectEmitted(JuliaListener.get(), *Object,
                               *SavedObjects.back().getBinary(), *LO, JIT.MemMgr);

        // record all of the exported symbols defined in this object
        // in the primary hash table for the enclosing JIT
        for (auto &Symbol : Object->symbols()) {
            auto Flags = Symbol.getFlags();
            if (Flags & object::BasicSymbolRef::SF_Undefined) continue;
            if (!(Flags & object::BasicSymbolRef::SF_Exported)) continue;
...
JuliaOJIT::JuliaOJIT(TargetMachine &TM)
    : TM(TM), DL(TM.createDataLayout()), ObjStream(ObjBufferSV),
      MemMgr(createRTDyldMemoryManager()), ObjectLayer(DebugObjectRegistrar(*this)),
      CompileLayer(ObjectLayer, [this](Module &M) {
          ...
          if (!Obj) {
              ...
              llvm::report_fatal_error("FATAL: Unable to compile LLVM Module: '" + Buf +
                                       "'\n"
                                       "The module's content was printed "
                                       "above. Please file a bug report");
#else
...
void *JuliaOJIT::getPointerToGlobalIfAvailable(StringRef S)
{
    ...
    if (pos != GlobalSymbolTable.end()) return pos->second;
    return nullptr;
}
...
void JuliaOJIT::addModule(std::unique_ptr<Module> M)
{
    ...
    for (Module::iterator I = M->begin(), E = M->end(); I != E;) {
        ...
        if (F->isDeclaration()) {
            if (F->use_empty())
                F->eraseFromParent();
            else if (!(isIntrinsicFunction(F) || findUnmangledSymbol(F->getName()) ||
    ...
    auto Resolver = orc::createLambdaResolver(
        [&](const std::string &Name) {
            ...
            // Step 0: ObjectLinkingLayer has checked whether it is in the current
            // module
    ...
    auto modset = CompileLayer.addModuleSet(std::move(Ms), MemMgr, std::move(Resolver));
    // Force LLVM to emit the module so that we can register the symbols
    // in our lookup table.
    CompileLayer.emitAndFinalize(modset);
}

void JuliaOJIT::removeModule(ModuleHandleT H) { CompileLayer.removeModuleSet(H); }
...
const DataLayout &JuliaOJIT::getDataLayout() const { return DL; }

const Triple &JuliaOJIT::getTargetTriple() const { return TM.getTargetTriple(); }
...
static void jl_merge_module(Module *dest, std::unique_ptr<Module> src)
{
    ...
    for (Module::alias_iterator I = src->alias_begin(), E = src->alias_end(); I != E;) {
        ...
        ++I;
        if (dG) {
            if (!dG->isDeclaration()) { // aliases are always definitions, so this
                                        // test is reversed from the above two
                sG->replaceAllUsesWith(dG);
                sG->eraseFromParent();
                continue;
            }
...
// to finalizing a function, look up its name in the `module_for_fname` map of
// unfinalized functions
...
static void jl_finalize_function(const std::string &F, Module *collector)
{
    ...
    if (m) {
        // probably not many unresolved declarations, but be sure iterate over their
        // Names,
...
void jl_finalize_module(Module *m, bool shadow)
{
    ...
    for (Module::iterator I = m->begin(), E = m->end(); I != E; ++I) {
        Function *F                                             = &*I;
        if (!F->isDeclaration()) module_for_fname[F->getName()] = m;
    }
#endif
...
void *jl_emit_and_add_to_shadow(GlobalVariable *gv, void *gvarinit)
{
    PointerType *T = cast<PointerType>(
        gv->getType()->getElementType()); // pointer is the only supported type here
...
GlobalVariable *jl_emit_sysimg_slot(Module *m, Type *typ, const char *name, uintptr_t init,
                                    size_t &idx)
{
    assert(imaging_mode);
    ...
    GlobalVariable *gv =
        new GlobalVariable(*m, typ, false, GlobalVariable::InternalLinkage,
...
#else
    void *p =
        jl_ExecutionEngine->getPointerToGlobal(shadow_output->getNamedValue(gv->getName()));
#endif
    assert(p);
    return p;
}
...
#else
    // on the old jit, the shadow_module is the same as the execution
    // engine_module
                                   ...
                                   const char *sysimg_data, size_t sysimg_len)
{
    ...
    addComdat(new GlobalVariable(
        *mod, gvars_type, true, GlobalVariable::ExternalLinkage,
        MapValue(ConstantArray::get(gvars_type, ArrayRef<Constant *>(jl_sysimg_gvars)),
                 VMap),
        "jl_sysimg_gvars"));
    ArrayType *fvars_type = ArrayType::get(T_pvoidfunc, jl_sysimg_fvars.size());
    addComdat(new GlobalVariable(
        *mod, fvars_type, true, GlobalVariable::ExternalLinkage,
        MapValue(ConstantArray::get(fvars_type, ArrayRef<Constant *>(jl_sysimg_fvars)),
                 VMap),
        "jl_sysimg_fvars"));
    addComdat(new GlobalVariable(*mod, T_size, true, GlobalVariable::ExternalLinkage,
    ...
    addComdat(new GlobalVariable(*mod, feature_string->getType(), true,
                                 GlobalVariable::ExternalLinkage, feature_string,
    ...
    if (strcmp(jl_options.cpu_target, "native") == 0) {
        ...
        addComdat(new GlobalVariable(
            *mod, T_uint64, true, GlobalVariable::ExternalLinkage,
...
extern "C" void jl_dump_native(const char *bc_fname, const char *obj_fname,
                               const char *sysimg_data, size_t sysimg_len)
{
    assert(imaging_mode);
...
// No DataLayout pass needed anymore.
    ...
    if (bc_fname) {
        ...
        std::string err;
        if (EC)
            err = "ERROR: failed to open --output-bc file '" + std::string(bc_fname) +
                  "': " + EC.message();
#else
        std::string                        err;
    ...
    if (obj_fname) {
        ...
        std::string err;
        if (EC)
            err = "ERROR: failed to open --output-o file '" + std::string(obj_fname) +
                  "': " + EC.message();
#else
        std::string err;
...
#endif
            if (TM->addPassesToEmitFile(PM, *obj_FOS.get(), TargetMachine::CGFT_ObjectFile,
                                        false)) {
                jl_safe_printf(
                    "ERROR: target does not support generation of object files\n");
            }
        }
    }
...
extern "C" int32_t jl_assign_functionID(void *function)
{
    ...
    jl_sysimg_fvars.push_back(ConstantExpr::getBitCast(
        shadow_output->getNamedValue(((Function *)function)->getName()), T_pvoidfunc));
...
extern "C" int32_t jl_get_llvm_gv(jl_value_t *p)
{
    ...
    if (it == jl_value_to_llvm.end()) return 0;
...
GlobalVariable *jl_get_global_for(const char *cname, void *addr, Module *M)
{
    ...
    GlobalVariable *gv = new GlobalVariable(
        *M, T_pjlvalue, false, GlobalVariable::ExternalLinkage, NULL, gvname.str());
