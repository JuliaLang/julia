...
(define (expand-ncat e (hvncat '((top hvncat))))
                 ...
                 (map (lambda (x)
                        (if (and (pair? x) (memv (car x) (list 'row 'nrow)))
                            (if (eq? (car x) 'row)
                                ...
                                (if (= d (1+ (cadr x)))
                                    (get-shape (cddr x) is-row-first (1- d))
                                    (get-shape (list x) is-row-first (1- d))))
                            (get-shape (list x) is-row-first (1- d))))
                      a)))
             (if (pair? (car ashape))
  ...
  (define (get-dims a is-row-first d)
    (if (and (pair? (car a)) (memv (caar a) (list 'row 'nrow)))
        (if (eq? (caar a) 'nrow)
            ...
            (if is-row-first
                (case d
                  ((1) (list (length (cdar a))))
                  ((2) (list (length (cdar a)) (length a)))
                  (else
                   (cons (length a) (get-dims (list (car a)) is-row-first (1- d)))))
                (error "cannot mix space and ;; separators in an array expression, except to wrap a line")))
        (if (= d 1)
  ...
  (let ((d (cadr e))
        (a (cddr e)))
    (let ((rows (reverse (get-rows a))))
      (let ((aflat (apply append rows))
            (is-row-first (any (lambda (x)
                                 (> (length x) 1))
                               rows)))
        ...
        (expand-forms
          (let ((dims `(tuple ,@(reverse (get-dims a is-row-first d))))
                (shape (get-shape a is-row-first d)))
                (set! shape (cons (list (car shape)) (cdr shape)))
            (if (is-balanced shape)
                ...
                (let ((callshape
                      (cons 'tuple (reverse (map (lambda (x)
                                                   (cons 'tuple x))
                                                 shape)))))
                  `(call ,@hvncat ,callshape ,(tf is-row-first) ,@aflat)))))))))
