    ...
    (cond ((and (length= e 2) (or (symbol? name) (globalref? name)))
          ...
          ((eq? (car name) 'call)
           (if (and (eq? (cadr name) '*) (number? (caddr name)))
               (error (string "invalid function definition; \"" (deparse (caddr name))
                              "\" is not a valid function argument name in \"" (deparse name) "\"")))
  ...
  (let ((tryb   (cadr e))
    ...
    (cond ((and (length> e 4) (not (equal? (caddddr e) '(false))))
           (if (has-unmatched-symbolic-goto? tryb)
               (error "`goto` out of a `try` block is not permitted with `finally`"))
           (if (and (not (equal? catchb '(false)))
                    (has-unmatched-symbolic-goto? catchb))
               (error "`goto` out of a `catch` block is not permitted with `finally`"))
           (if (and (length> e 5)
                    (has-unmatched-symbolic-goto? (cons 'block (cdddddr e))))
               (error "`goto` out of an `else` block is not permitted with `finally`"))
