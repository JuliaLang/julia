...
(define-macro (seq-use a b)
  `(seq-use- ,a (lambda () ,b)))

;; returns 'read, 'write, 'maybe-read, 'maybe-write, 'both, or #f if unused
(define (var-use-style- e var)
  ;; combine two use styles sequentially
  (define (seq-use- a b-thk)
    (if (memq a '(read write both))
        a
        (let ((b (b-thk)))
          (cond ((eq? a b) a)
                ((not a)   b)
                ((not b)   a)
                ((eq? a 'maybe-read)
                 (if (eq? b 'read) 'read 'both))
                ((eq? a 'maybe-write)
                 (if (eq? b 'write) 'write 'both))
                (else (assert #f))))))
  (define (maybe a)
    (cond ((eq? a 'read) 'maybe-read)
          ((eq? a 'write) 'maybe-write)
          (else a)))
  ;; combine two use styles alternately
  (define (alt-use a b)
    (cond ((eq? a b) a)
          ((not a) (maybe b))
          ((not b) (maybe a))
          ((eq? a 'write)
           (if (eq? b 'maybe-write) b 'both))
          ((eq? a 'read)
           (if (eq? b 'maybe-read) b 'both))
          ((eq? a 'maybe-write)
           (if (eq? b 'write) a 'both))
          ((eq? a 'maybe-read)
           (if (eq? b 'read) a 'both))
          (else (assert #f))))
  (cond ((eq? e var) 'read)
        ((or (atom? e) (quoted? e)) #f)
        ((memq (car e) '(ssavalue decl const import using export local)) #f)
        ((eq? (car e) 'lambda) #f)
        ((eq? (car e) '=)
         (seq-use (var-use-style- (caddr e) var)
                  (and (eq? (cadr e) var) 'write)))
        ((eq? (car e) 'method)
         (if (length> e 2)
             (seq-use (and (eq? (cadr e) var) 'write)
                      (var-use-style (cons 'block (cddr e)) var))
             #f))
        ((eq? (car e) 'scope-block)
         (let ((nested (var-use-style- (cadr e) var)))
           (if (memq nested '(read maybe-read both))
               nested
               #f)))
        ((memq (car e) '(if elseif))
         (seq-use (var-use-style- (cadr e) var)
                  (let ((then (var-use-style- (caddr e) var)))
                    (if (null? (cdddr e))
                        (maybe then)
                        (alt-use then
                                 (var-use-style- (cadddr e) var))))))
        ((eq? (car e) 'trycatch)
         (alt-use (maybe (var-use-style (cadr e) var))
                  (maybe (var-use-style (caddr e) var))))
        ((eq? (car e) 'tryfinally)
         (seq-use (maybe (var-use-style (cadr e) var))
                  (var-use-style (caddr e) var)))
        ((memq (car e) '(break symbolicgoto))
         'both)
        ((eq? (car e) 'return)
         (seq-use (var-use-style (cadr e) var)
                  'both))
        ((eq? (car e) '_while)
         (seq-use (var-use-style- (cadr e) var)
                  (maybe (var-use-style- (caddr e) var))))
        (else
         (let loop ((exprs (cdr e)))
           (if (null? exprs)
               #f
               (seq-use (var-use-style- (car exprs) var)
                        (loop (cdr exprs))))))))

(define (var-use-style e var)
  (let ((s (var-use-style- e var)))
    (cond ((eq? s 'maybe-read) 'read)
          ((eq? s 'maybe-write) 'write)
          ((eq? s 'both)
           (error (string "Scope of variable \"" var "\" is ambiguous. Please explicitly declare it global or local.")))
          (else s))))
  ...
  (cond ((symbol? e)
        ...
        ((eq? (car e) 'scope-block)
         (let* ((blok            (cadr e)) ;; body of scope-block expression
                ...
                (outside-func?   (null? argnames))
                (toplevel?       (and outside-func? (eq? e (lam:body lam))))
                (implicit-locals
                 (filter (if toplevel?
                ...
                (implicit-locals
                 (if (and outside-func? (not toplevel?))
                     (filter (lambda (v) (eq? (var-use-style blok v) 'write))
                             implicit-locals)
                     implicit-locals))
