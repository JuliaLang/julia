            ...
            ((call new splatnew foreigncall cfunction new_opaque_closure)
             (let* ((args
                     (cond ((eq? (car e) 'foreigncall)
                            ...
                            ;;       the 1st is either a tuple of args or a single normal arg
                            (append (if (tuple-call? (cadr e))
                                        (list `(call (core tuple) ,@(compile-args (cddr (cadr e)) break-labels)))
                           ...
                           ((and (length> e 2)
                            ...
                            (append (list (cadr e))
                                    (if (tuple-call? (caddr e))
                                        (list `(call (core tuple) ,@(compile-args (cddr (caddr e)) break-labels)))
