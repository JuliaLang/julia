  ...
  (table
   ...
   (lambda (e)
     (if (pair? (cdr e))
         ...
         (let* ((label-expr (cadr e))
                (label (cond ((eq? label-expr '_) '_)
                             ((and (pair? label-expr)
                                   (memq (car label-expr) '(quote inert))
                                   (symbol? (cadr label-expr)))
                              (cadr label-expr))
                             (else (error "invalid break label: expected _ or :symbol")))))
           (if (length> e 2)
   ...
   (lambda (e)
     (if (pair? (cdr e))
         ...
         ;; Note: QuoteNode becomes (inert sym) when converted from Julia to flisp
         (let* ((label-expr (cadr e))
                (name (cond ((eq? label-expr '_) '_)
                            ((and (pair? label-expr)
                                  (memq (car label-expr) '(quote inert))
                                  (symbol? (cadr label-expr)))
                             (cadr label-expr))
                            (else (error "invalid continue label: expected _ or :symbol")))))
   ...
   (lambda (e)
     ...
     ;; For labeled loops, add an extra break-block for name-cont inside loop-cont
     ;; so that (continue name) works while keeping loop-cont for ordinary continue
     (let* ((name (cadr e))
            (body (expand-forms (caddr e)))
            (is-loop (and (pair? body) (eq? (car body) 'break-block) (eq? (cadr body) 'loop-exit)))
            (cont-name (symbol (string name "-cont"))))
       ;; Helper to add an extra break-block name-cont inside the outermost loop-cont
       ;; in-outer-loop tracks whether we're inside the labeled loop's loop-exit (vs a nested loop)
       (define (add-cont-label e)
         (cond ((atom? e) e)
               ((and (pair? e) (eq? (car e) 'break-block) (eq? (cadr e) 'loop-cont))
                    `(break-block loop-cont (break-block ,cont-name ,(add-cont-label (caddr e)))))
               ((and (pair? e) (eq? (car e) 'break-block) (eq? (cadr e) 'loop-exit))
                ;; Do not descent into nested loops
                e)
               ((pair? e)
                (cons (add-cont-label (car e)) (add-cont-label (cdr e))))
               (else e)))
       ;; If body is a loop, add the extra cont label
       (if is-loop
           `(symbolicblock ,name (break-block loop-exit ,(add-cont-label (caddr body))))
           `(symbolicblock ,name ,body))))
