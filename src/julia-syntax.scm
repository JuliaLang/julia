...
(define (expand-ncat e
                     (hvncat '((top hvncat))))
  (define (get-dims ashape is-row-first d)
    (if (null? ashape)
        '()
        (if (= d 2)
            (if is-row-first
                (list (caar (cdaadr ashape)) (caaadr ashape))
                (list (car ashape) (caaadr ashape)))
            (cons (car ashape) (get-dims (caadr ashape) is-row-first (1- d))))))
  (define (get-shape a d)
    (let ((ashape (map (lambda (x)
      ...
      (expand-forms
        (let ((is-row-first
              ...
              (rows (map (lambda (x)
                          ...
                          aflat))
              (shape (get-shape a d)))
          (let ((dims (cons 'tuple (reverse (get-dims shape (eq is-row-first '(true)) d)))))
