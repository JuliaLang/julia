...
(define (compile-body e vi lam)
  (let ((code '())            ;; statements (emitted in reverse order)
        ...
        (finally-handler #f)  ;; `(var label map handlers)` where `map` is a list of `(tag . action)`.
                              ...
                              ;; `handlers` is the list of active handlers including the `finally`.
        (handler-goto-fixups '())  ;; `goto`s that might need `leave` exprs added
        (handlers '()))       ;; list of active exception handlers by ssa name
    (define (emit c)
    ...
    (define (leave-finally-block action (need-goto #t))
      (let* ((tags (caddr finally-handler))
        ...
        (if need-goto
            (let ((enclosing-handlers (cadddr finally-handler)))
              (emit `(leave ,@(list-upto handlers enclosing-handlers) ,(car enclosing-handlers)))
      ...
      (if (pair? handlers)
          (let ((tmp (cond ((and (simple-atom? x) (or (not (ssavalue? x)) (not finally-handler))) #f)
            ...
            (if finally-handler
                ...
                (begin (emit `(leave ,@handlers))
                       (actually-return (or tmp x))))
            (or tmp x))
          (actually-return x)))
    (define (emit-break labl)
      (let ((lvl (caddr labl)))
        (if (and finally-handler (pair? (list-upto (cadddr finally-handler) lvl)))
            (leave-finally-block `(break ,labl))
            (begin
              (let ((handlers-between (list-upto handlers lvl)))
                (if (pair? handlers-between)
                    (emit `(leave ,@handlers-between))))
              (emit `(goto ,(cadr labl)))))))
            ...
            ((break-block)
             (let ((endl (make-label)))
               (begin0 (compile (caddr e)
                                (cons (list (cadr e) endl handlers)
                                      break-labels)
                                value #f)
                       (mark-label endl)))
            ...
            ((label symboliclabel)
             (if (eq? (car e) 'symboliclabel)
                 (if (has? label-level (cadr e))
                     ...
                     (put! label-level (cadr e) handlers)))
            ...
            ((symbolicgoto)
             (let* ((m (get label-map (cadr e) #f))
               ...
               (set! handler-goto-fixups
                     (cons (list code handlers (cadr e)) handler-goto-fixups))
               '(null)))

            ;; exception handlers are lowered using
            ;; (enter L) - push handler with catch block at label L
            ;; (leave s₁ s₂ ...) - pop exception handlers entered at the given statement numbers
            ((trycatch tryfinally)
             (let ((catch (make-label))
                   ...
                   (ssaname (make-ssavalue)))
               (emit `(= ,ssaname (enter ,catch)))
               (set! handlers (cons ssaname handlers))
               (if finally (begin (set! my-finally-handler (list finally endl '() handlers))
...
(define (renumber-lambda lam)
  ...
  (let stmtloop ((stmts (cdr (lam:body lam)))
                 (n     1))
    (if (null? stmts)
        (set! nssavalues (- n 1))
        (let ((e (car stmts)))
          (if (and (assignment? e) (ssavalue? (cadr e)))
              (begin (put! ssavalue-table (cadr (cadr e)) n)
                     (set-car! stmts (caddr e))))
          (stmtloop (cdr stmts) (+ n 1)))))
  ...
  (let ((body (renumber-slots (lam:body lam)))
        (vi   (lam:vinfo lam)))
    (renumber-labels! body (label-to-idx-map body))
    (listify-lambda
     `(lambda ,(cadr lam)
