          ...
          (let ((e (car stmts)))
            (cond ((atom? e) (emit e))
                  ...
                  ((eq? (car e) 'label)
                   ...
                   (set! grtable (table)))
                  ((eq? (car e) 'latestworld)
                   (set! grtable (table))
                   (emit e))
                  ((and (assignment? e) (ssavalue? (cadr e)))
                   (let* ((lhs-i (cadr (cadr e)))
                          (rhs (caddr e))
                          (idx (and (ssavalue? rhs) (get ssavtable (cadr rhs) #f))))
                     (if idx
                         ;; if both lhs and rhs are ssavalues, merge them
                         (put! ssavtable lhs-i idx)
                         (if (globalref? rhs)
                             (let ((oldgr (get grtable rhs #f)))
                               ;; if this globalref is already in the IR, use it
                               (if oldgr (put! ssavtable lhs-i oldgr)
                                   (begin
                                     (put! grtable rhs i)
                                     (put! ssavtable lhs-i i)
                                     (emit rhs))))
                             (begin
                               (put! ssavtable lhs-i i)
                               (emit rhs))))))
