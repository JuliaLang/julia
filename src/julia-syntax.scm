   ...
   (let* ((attrs ())
          (fields (let ((n 0))
                    (map (lambda (x)
                         ...
                         fields)))
          (attrs (reverse attrs))
          (meta        (map cadr (filter meta? defs)))
          (defs        (filter (lambda (x) (not (or (effect-free? x) (eq? (car x) 'string) (meta? x)))) defs))
       ...
       (scope-block
        (block
         ...
         (= ,name (call (core _structtype) (thismodule) (inert ,name) (call (core svec) ,@params)
                        ...
                        ,mut ,min-initialized ,(make-bool (memq 'quietparams meta))))
