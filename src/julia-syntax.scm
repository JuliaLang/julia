  ...
  (let ((vi     (car (lam:vinfo lam)))
    ...
    (define (visit e)
      ...
      (cond ((atom? e) (if (symbol? e) (mark-used e))
            ...
            ((eq? (car e) 'scope-block)
             (visit (cadr e)))
            ((memq (car e) '(block call new splatnew _do_while spawn-block))
...
(define (valid-ir-rvalue? lhs e)
  (or (ssavalue? lhs)
      ...
      (and (symbol? lhs) (pair? e)
           (memq (car e) '(new splatnew the_exception isdefined call invoke foreigncall cfunction gc_preserve_begin copyast syncregion)))))
      ...
      (if (or (not (pair? e)) (memq (car e) '(null true false ssavalue quote inert top core copyast the_exception $
          ...
          (case (car e)
            ...
            ((_do_while)
             ...
             (if value (compile '(null) break-labels value tail)))
            ((spawn-block)
             (let* ((tasktoken (make-ssavalue))
                    (retval    (new-mutable-var))
                    (excp      (make-ssavalue))
                    (endl (make-label))
                    (topl (make-label))
                    (syncnode (cadr e))
                    (thunk (compile (caddr e) break-labels #t #f))
                    )
               ;; emit ssavalue for Task
               (emit `(= ,tasktoken (call (core Task))))
               (emit `(detach ,syncnode ,tasktoken ,topl ,endl))
               (mark-label topl)
               ;; retval captures the result of either the try or catch
               (compile (expand-forms
                   `(trycatch (= ,retval ,thunk) (block
                        ;; get the exception
                        (= ,excp (the_exception))
                        ;; propagate the exception
                        (call (core setfield) ,tasktoken (inert exception) ,excp)
                        (= ,retval ,excp)
                    ))) break-labels #f #f)
               ;; consume retval
               (emit `(reattach ,syncnode ,tasktoken, retval ,endl))
               (mark-label endl)
               tasktoken))
            ((break-block)
             (let ((endl (make-label)))
  ...
  (let* ((stuff (compact-ir (lam:body lam) file line))
    ...
    (define (renumber-stuff e)
      (cond ((symbol? e)
            ...
            ((eq? (car e) 'detach)
             `(detach ,(renumber-stuff (cadr e)) ,(renumber-stuff (caddr e)) ,(get label-table (cadddr e)) ,(get label-table (caddddr e))))
            ((eq? (car e) 'reattach)
             `(reattach ,(renumber-stuff (cadr e)) ,(renumber-stuff (caddr e)) ,(renumber-stuff (cadddr e)) ,(get label-table (caddddr e))))
            ((eq? (car e) 'lambda)
