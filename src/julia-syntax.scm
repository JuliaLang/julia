  ...
  (cond
    ((symbol? var)
     (let* ((vi (get locals var #f))
            (cv (var-info-for var (cadr (lam:vinfo lam))))
      ...
      (cond
       ((symbol? e)
        ...
        (let ((vi (get locals e #f))
              (cv (var-info-for e (cadr (lam:vinfo lam)))))
          (cond ((eq? e fname) e)
                ((memq e (lam:sp lam)) e)
                (cv
        ...
        (case (car e)
          ...
          ((isdefined) ;; convert isdefined expr to function for closure converted variables
           (let* ((sym (cadr e))
                  ...
                  (cv (and (symbol? sym) (var-info-for sym (cadr (lam:vinfo lam))))))
             (cond ((eq? sym fname) e)
                        ...
                        (typedef  ;; expression to define the type
                         (let* ((fieldtypes (map (lambda (v)
                                                   (cond ((is-var-boxed? v lam) '(core Box))
                                                         ((is-var-nospecialize? v lam) (vinfo:type (var-info-for v (car (lam:vinfo lam)))))
                                                         (else (make-ssavalue))))
                                                 capt-vars))
