...
(define (expand-for lhss itrs body elsebody)
  ...
  (table
   ...
   (lambda (e)
     (let ((ranges (if (eq? (car (cadr e)) 'block)
                       (cdr (cadr e))
                       (list (cadr e))))
           (elsebody  (if (length> e 3) (cadddr e) '(null))))
       (expand-forms (expand-for (map cadr ranges) (map caddr ranges) (caddr e) elsebody))))
  ...
  (cond ((symbol? e) (let ((r (assq e renames)))
        ...
        ((eq? (car e) 'break-block)
         `(break-block ,(cadr e) ;; ignore type symbol of break-block expression
                       ...
                       ,(resolve-scopes- (cadddr e) env implicitglobals lam renames #f sp))) ;; elsebody of break-block expression
          ...
          (case (car e)
            ...
            ((break-block)
             (let ((endl (make-label))
               ...
               (compile (caddr e)
                        (cons (list (cadr e) endl handler-level catch-token-stack valvar tail)
                              break-labels)
                        #f #f)
            ...
            ((break)
             (let ((labl (assq (caddr e) break-labels)))
               (if (not labl)
                   ...
                   (let ((endl (cadr labl))
                         (hlevel (caddr labl))
                         (catch-token-stack (cadddr labl))
                         (valtarget (cadddr (cdr labl)))
                         (blocktail (cadddr (cddr labl)))
                         (valexpr (cadr e)))
