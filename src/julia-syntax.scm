   ...
   (lambda (e)
     ...
     (cond
      ...
      ((and (pair? lhs)
            ...
            (pair? (cadr lhs))
            (eq? (car (cadr lhs)) 'call))
       ; rewrite (:(=), (:curly, (:call, name, args...), sparams...), body)
       ;      as (:(function), (:call (:curly, (:curly, name, sparams...), args...), body)
       ; TODO: deprecate the old form (that this is currently making use of) and use the new form exclusively
       (expand-forms `(function (call (curly ,(cadr (cadr lhs)) ,@(cddr lhs)) ,@(cddr (cadr lhs))) ,@(cddr e))))
      ((and (pair? lhs)
