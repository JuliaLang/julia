      ...
      (or (and (eq? (caaddr e) 'generator)
               (let ((ranges (cddr (caddr e))))
                 ...
                 (and (every (lambda (x) (and (pair? x) (eq? (car x) '=)))
                      ...
                      (lower-1d-comprehension (cadr e) (cadr (caddr e)) (car ranges)))))
          `(call (top collect) ,(cadr e) ,(caddr e)))))))

(define (lower-1d-comprehension ty expr itr-expr)
  (let ((result (gensy))
        (isz (make-ssavalue))
        (idx (gensy))
        ...
        (itr (make-ssavalue)))
    `(scope-block
      (block (local ,idx) (local ,result)
             ,.(map (lambda (r) `(local ,r))
                    (lhs-vars (cadr itr-expr)))
             (= ,itr ,(caddr itr-expr))
             (= ,isz (call (top iteratorsize) ,itr))
             (if (call (core isa) ,isz (top HasShape))
                 (= ,result (call (curly (core Array) ,ty 1) (call (top size) ,itr)))
                 (if (call (core isa) ,isz (top HasLength))
                     (= ,result (call (curly (core Array) ,ty 1) (call (top length) ,itr)))
                     (= ,result (call (curly (core Array) ,ty 1) 0))))
             (= ,idx 1)
             (for (= ,(cadr itr-expr) ,itr)
