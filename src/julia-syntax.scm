             ...
             (let ((var-exprs (map (lambda (v)
                                     (let ((cv (assq v (cadr (lam:vinfo lam)))))
                                       (if cv
                                           ...
                                           v)))
                                   cvs)))
               `(new_opaque_closure ,(cadr e) (call (core apply_type) Union) (core Any)
                      ,(convert-lambda lam2 (car (lam:args lam2)) #f '() (symbol-to-idx-map cvs))
                      ,@var-exprs))))
          ((method)
             ...
             (let* ((args
                     (cond ((eq? (car e) 'foreigncall)
                           ...
                           ((eq? (car e) 'new_opaque_closure)
                             (let ((lambda (linearize (caddddr e))))
                                (append
                                  (compile-args (list-head (cdr e) 3) break-labels)
                                  (list lambda)
                                  (compile-args (cdddddr e) break-labels))))
