...
(define (expand-ncat e (hvncat '((top hvncat))))
           ...
           (let ((ashape
                 (map (lambda (x)
                        (if (or (not (is-row x))
                            ...
                            (get-shape (list x) is-row-first (1- d))
                            (get-shape (if (eq? (car x) 'nrow)
                                           (cddr x)
                                           (cdr x))
                                       is-row-first (1- d))))
                      a)))
  ...
  (define (get-dims a is-row-first d)
    (if (= d 1)
        (list (length a))
        (if (or (not (is-row (car a)))
                (and (eq? (caar a) 'nrow) (> d (1+ (cadar a))))
                (and (eq? (caar a) 'row) (> d 2)))
            (cons (length a) (get-dims (list (car a)) is-row-first (1- d)))
            (if (eq? (caar a) 'nrow)
                (if (and is-row-first (= d 2))
                    (list (car (get-dims (cddar a) is-row-first (1- d))) (length (cddar a)))
                    (cons (length a) (get-dims (cddar a) is-row-first (1- d))))
                (list (length (cdar a)) (if (= d 2) (length a) '()))))))
  (define (is-balanced shape)
    ; determine whether there are exactly the same number of elements along each axis
    (= 0 (sum (map (lambda (x y)
                     (sum (map (lambda (z)
                                 (- z y))
                               x)))
                   shape (cons (list (car shape)) (map car (cdr shape)))))))
             ...
             (if (is-row x)
                 (if (eq? (car x) 'nrow)
                     (let* ((raflat (append (hasrows-flatten (cddr x))))
                            (r (or (car raflat) (car y))))
                       (if (and (= (cadr x) 2) r)
                           (error "cannot mix space and ;; separators in an array expression, except to wrap a line"))
                       (cons r (append (cdr raflat) (cdr y))))
                     (cons #t (append (reverse (cdr x)) (cdr y))))
              ...
              (let ((shape (get-shape a is-row-first d)))
                (if (is-balanced shape)
                    (let ((dims `(tuple ,@(reverse (get-dims a is-row-first d)))))
                      `(call ,@hvncat ,dims ,(tf is-row-first) ,@aflat)) ; balanced call
                    `(call ,@hvncat ,(tuplize shape) ,(tf is-row-first) ,@aflat)))))))) ; all else
