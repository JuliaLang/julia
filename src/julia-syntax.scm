...
(define expand-table
  (table
   ...
   'opaque_closure
   (lambda (e)
     (let* ((meth      (caddr (expand-forms (cadr e)))) ;; `method` expr
            (named?    (eq? (car meth) 'method))
            (meth      (if named? meth (caddr meth)))
            (name      (if named? (cadr meth) '(null)))
            (lam       (cadddr meth))
            ...
            (ex
             (if (length= argtypes 0)
              `(_opaque_closure ,name ,(expand-forms `(curly (core Tuple))) (false) 0 ,functionloc ,lam)
               (let* ((vssa (make-ssavalue))
                      ...
                      (argtype   (foldl (lambda (var ex) `(call (core UnionAll) ,var ,ex))
                                        ...
                                        (reverse tvars))))
                    `(_opaque_closure ,name ,argtype (call (core isa) ,vssa (core TypeofVararg)) ,(length argtypes) ,functionloc ,lam)))))
       (if named? `(= ,name ,ex) ex)))
   'block
   (lambda (e)
             ...
             (let ((var-exprs (map (lambda (v)
                                     (let ((cv (assq v (cadr (lam:vinfo lam)))))
                                       (if cv
                                           ...
                                           v)))
                                   cvs)))
               `(new_opaque_closure ,(caddr e) ,isva (call (core apply_type) Union) (core Any)
                      (opaque_closure_method ,(cadr e) ,nargs ,functionloc ,(convert-lambda lam2 (car (lam:args lam2)) #f '() (symbol-to-idx-map cvs)))
                      ,@var-exprs))))
          ((method)
