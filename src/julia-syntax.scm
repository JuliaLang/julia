...
(define (find-symbolic-label-refs e tbl)
  (if (or (not (pair? e)) (quoted? e))
      '()
      (if (memq (car e) '(symbolicgoto symbolicawait))
          (put! tbl (cadr e) #t)
...
(define (resolve-scopes- e scope (sp '()) (loc #f))
  (cond ((symbol? e)
        ...
        ((eq? (car e) 'global)
         ...
         e)
        ((eq? (car e) 'symbolicawait)
          `(symbolicawait ,(cadr e) ,(resolve-scopes- (caddr e) scope sp loc) ,(cadddr e)))
  ...
  (let* ((stuff (compact-ir (lam:body lam)
    ...
    (define (renumber-stuff e)
      (cond ((eq? e UNUSED) (error "Attempted to use slot marked unused"))
            ...
            ((eq? (car e) 'await)
             `(await ,(get label-table (cadr e)) ,@(map renumber-stuff (cddr e))))
            ((eq? (car e) 'gotoifnot)
