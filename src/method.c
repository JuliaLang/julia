...
void jl_add_scanned_method(jl_module_t *m, jl_method_t *meth)
{
    ...
    if (m->scanned_methods == jl_nothing) {
        jl_write(m, (void**)&(m->scanned_methods), (jl_value_t*)jl_alloc_vec_any(0));
    }
                                              ...
                                              int binding_effects)
{
    ...
    if (e->head == jl_cfunction_sym) {
        ...
        if (typ == (jl_value_t*)jl_voidpointer_type) {
            ...
            jl_write(a, (void**)&a, jl_toplevel_eval(module, *(jl_value_t**)a));
        }
...
jl_code_info_t *jl_new_code_info_from_ir(jl_expr_t *ir)
{
    ...
    jl_write(li, (void**)&(li->debuginfo), jl_linetable_to_debuginfo(codelocs_any, linetable));

    assert(jl_is_expr(bodyex));
    jl_array_t *body = bodyex->args;
    jl_write(li, (void**)&(li->code), body);
    size_t n = jl_array_nrows(body);
    jl_value_t **bd = (jl_value_t**)jl_array_ptr_data((jl_array_t*)li->code);
    jl_write(li, (void**)&(li->ssaflags), jl_alloc_array_1d(jl_array_uint32_type, n));
    ...
    jl_write(li, (void**)&(li->slotnames), jl_alloc_array_1d(jl_array_symbol_type, nslots));
    jl_write(li, (void**)&(li->slotflags), jl_alloc_array_1d(jl_array_uint8_type, nslots));
    jl_write(li, (void**)&(li->ssavaluetypes), jl_box_long(nssavalue));
...
JL_DLLEXPORT void jl_method_set_source(jl_method_t *m, jl_code_info_t *src)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (jl_is_expr(st) && ((jl_expr_t*)st)->head == jl_meta_sym) {
            ...
            if (nargs >= 1 && jl_exprarg(st, 0) == (jl_value_t*)jl_nospecialize_sym) {
                ...
                jl_write(m, (void**)&(m->generator), jl_toplevel_eval(m->module, gexpr));
                st = jl_nothing;
            }
    ...
    jl_write(src, (void**)&(src->code), copy);
    jl_write(m, (void**)&(m->slot_syms), jl_compress_argnames(src->slotnames));
    if (gen_only) {
        m->source = NULL;
    }
    else {
        jl_write(m, (void**)&(m->debuginfo), src->debuginfo);
        jl_write(m, (void**)&(m->source), (jl_value_t*)src);
        jl_write(m, (void**)&(m->source), (jl_value_t*)jl_compress_ir(m, NULL));
    }
    JL_GC_POP();
}
    ...
    int nargs, jl_value_t *functionloc, jl_code_info_t *ci, int isva, int isinferred)
{
    ...
    if (isinferred) {
        jl_write(m, (void**)&(m->slot_syms), jl_compress_argnames(ci->slotnames));
    } else {
...
static void prepare_method_for_roots(jl_method_t *m, uint64_t modid)
{
    ...
    if (!m->root_blocks && modid != 0) {
        jl_write(m, (void**)&(m->root_blocks), jl_alloc_array_1d(jl_array_uint64_type, 0));
    }
}
