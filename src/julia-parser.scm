           ...
           (let ((firstch (read-char (ts:port s))))
               (if (and (not (eqv? firstch #\\))
                   ...
                   (let ((b (open-output-string)))
                     (let loop ((c firstch) (allowed-hex 0) (first? #t) (only-x? #t))
                       (cond
                         ((eqv? c #\') #f)
                         ((eof-object? c)
                         ...
                         ((eqv? c #\\)
                          (let* ((c           (not-eof-1 (read-char (ts:port s))))
                                 (only-x?     (and only-x? (= c #\x)))
                                 (allowed-hex (case c (#\x 2)
                                                      (#\u 4)
                                                      (#\U 8)
                                                      (else 0))))
                            (or first? only-x?
                            ...
                            (write-char #\\ b)
                            (write-char c b)
                            (loop (read-char (ts:port s)) allowed-hex #f only-x?)))
                         ((and (> allowed-hex 0) (char-hex? c))
                          (write-char c b)
                          (loop (read-char (ts:port s)) (- allowed-hex 1) #f only-x?))
                         (first?
                          (write-char c b)
                          (loop (read-char (ts:port s)) 0 #f #f))
