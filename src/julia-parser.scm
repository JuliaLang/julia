  ...
  (let loop ((ex    (parse-pipe< s))
    ...
    (let ((t (peek-token s)))
      (cond ((is-prec-comparison? t)
             (begin (take-token s)
                    (if first
                        (loop (cons (list 'comparison (dep-check s ex t) t (dep-check s (parse-pipe< s) t))
                                    #f)
                              #f)
                        (loop (cons (append (car ex) (list t (dep-check s (parse-pipe< s) t)))
                                    #f)
                              #f))))
            (first (car ex))
            ((length= (car ex) 4)
...
(define (parse-pipe< s) ;;(parse-RtoL s parse-pipe> is-prec-pipe<? #f parse-pipe<))
  (let* ((ex (parse-pipe> s))
    ...
    (if (is-prec-pipe<? t)
        (begin (take-token s)
               (cons (list 'call t (dep-check s ex t) (dep-check s (parse-pipe< s) t))
                     #f))
        ex)))
...
(define (parse-pipe> s) ;;(parse-LtoR s parse-range is-prec-pipe>?))
  (let loop ((ex (parse-range s))
    ...
    (if (is-prec-pipe>? t)
        (begin (take-token s)
               (loop (cons (list 'call t (dep-check s ex t) (dep-check s (parse-range s) t))
                           #f)
                     (peek-token s)))
        ex)))
...
(define (parse-range s)
  (let loop ((ex     (parse-expr s))
    ...
    (let* ((t   (peek-token s))
      ...
      (cond ((and first? (eq? t '|..|))
             (take-token s)
             (cons `(call ,t ,(dep-check s ex '|..|) ,(dep-check s (parse-expr s) '|..|))
                   #f))
                 ...
                 (let ((argument
                        (cond ((closing-token? (peek-token s))
                               (error  (string "missing last argument in \""
                                               (deparse (car ex)) ":\" range expression ")))
                              ((newline? (peek-token s))
                               (error "line break in \":\" expression"))
                              (else
                               (dep-check s (parse-expr s) t)))))
                   (if (and (not (ts:space? s))
                            (or (eq? argument '<) (eq? argument '>)))
                       (error (string "\":" argument "\" found instead of \""
                                      argument ":\"")))
                   (if first?
                       (loop (cons (list t (dep-check s ex t) argument) #f)
                             #f)
                       (loop (cons (append (car ex) (list argument)) #f)
                             #t)))))
            ((eq? t '...)
             (take-token s)
             (cons (list '... (dep-check s ex '...))
                   #f))
            (else ex)))))
      ...
      (if (not (ops t))
          ...
          (let ((spc (ts:space? s)))
            ...
            (cond ((and space-sensitive spc (memq t unary-and-binary-ops)
                   ...
                   p-ex)
                  ((memq t chain-ops)
                   (loop (cons (list* 'call t (dep-check s p-ex t)
                                      (parse-chain s (lambda (s) (dep-check s (down s) t)) t))
                               t)))
                  (else
                   (loop (cons (list 'call t (dep-check s p-ex t) (dep-check s (down s) t))
                               t)))))))))

(define (parse-expr s)     (parse-with-chains s parse-shift         is-prec-plus? '(+ ++)))

(define (deprecated-precedence s inner outer)
  (syntax-deprecation s (string "call to `" inner "` inside call to `" outer "`")
                      (string "parenthesized call to `" inner "`")))

;; check whether the relative precedence of (cdr p) and op is deprecated, and warn if so
;; returns (car p), which is the parsed subexpression.
(define (dep-check s p op)
  (let ((ex    (car p))
        (inner (cdr p)))
    (if (and inner
             (not (eq? inner op))
             (or (and (memq inner '(& |\||))
                 ...
                 (and (eq? inner '*)
                      (is-prec-bitshift? op))))
        (deprecated-precedence s inner op))
    ex))

(define (parse-shift s)    #;(parse-LtoR        s parse-term          is-prec-bitshift?)
  (let loop ((p-ex (parse-term s))
             (t  (peek-token s))
             (warn1 #f))
    (let ((ex (car p-ex))
          (warn (cdr p-ex)))
      (if (is-prec-bitshift? t)
          (begin (if warn (deprecated-precedence s warn t))
                 (take-token s)
                 (let ((nxt (parse-term s)))
                   (loop (cons (list 'call t ex (car nxt)) (cdr nxt))
                         (peek-token s)
                         (cdr nxt))))
          (begin (if warn1 (deprecated-precedence s warn1 (cadr ex)))
                 p-ex)))))

(define (parse-term s)     (parse-with-chains s parse-rational      is-prec-times? '(*)))
(define (parse-rational s) (cons (parse-LtoR  s parse-unary-subtype is-prec-rational?) #f))
  ...
  (with-bindings
   ...
   (without-whitespace-newline
    (let ((doargs (if (memv (peek-token s) '(#\newline #\;))
                      '()
                      (parse-comma-separated s (lambda (s) (car (parse-range s)))))))
      `(-> (tuple ,@doargs)
                     ...
                     (begin
                       ...
                       (let ((nxt (peek-token s)))
                         (if (or (memq nxt '(= in ∈))
                             ...
                             #t)))
                     #f))
         (lhs (car (parse-pipe< s)))
         (t   (peek-token s)))
    (cond ((memq t '(= in ∈))
           (take-token s)
           (let* ((rhs (dep-check s (parse-pipe< s) t))
                  (t   (peek-token s)))
