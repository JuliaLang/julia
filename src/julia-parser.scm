...
(define (parse-imports s word)
  (let ((imports (parse-comma-separated s (lambda (s) (parse-import s word)))))
    (if (and (not (eqv? word 'importall))
             (eqv? (peek-token s) 'from))
        (begin (take-token s)
               (let ((from (parse-import s 'from)))
                 (map (lambda (x)
                        (cons (car x) (append (cdr from) (cdr x))))
                      imports)))
        imports)))
