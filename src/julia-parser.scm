...
(define-macro (ts:pos s)        `(aref ,s 5))
(define (ts:space? s)           (aref s (if (ts:pbtok s) 4 2)))
(define (ts:save-pos! s)        (aset! s 5 (io.pos (ts:port s))))
(define (ts:swap-pos! s)
  (let ((p (aref s 5)) (q (aref s 6)))
       (aset! s 5 q)
       (aset! s 6 p)))
(define (ts:update-pos! s)
  (begin (aset! s 6 (aref s 5))
...
(define (require-token s)
  (let ((t (or (ts:pbtok s) (ts:last-tok s)
        (begin (ts:save-pos! s) (next-token (ts:port s) s)))))
    (if (eof-object? t)
