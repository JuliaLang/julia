...
static void JL_NORETURN jl_dlerror(const char *fmt, const char *sym)
{
    ...
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL,
                   GetLastError(), MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_US), reason,
    ...
    */
    if (modname == NULL) {
        ...
        if (!GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                                ...
                                (LPCWSTR)(&jl_load_dynamic_library), (HMODULE *)&handle)) {
...
JL_DLLEXPORT void *jl_dlsym(void *handle, const char *symbol)
{
    ...
    if (!ptr) jl_dlerror("could not load symbol \"%s\":\n%s", symbol);
    return ptr;
}
...
{
    if (jl_dlsym_e(jl_exe_handle, f_name)) return (const char *)1;
    if (jl_dlsym_e(jl_dl_handle, f_name)) return (const char *)2;
    if (jl_dlsym_e(jl_kernel32_handle, f_name)) return "kernel32";
    if (jl_dlsym_e(jl_ntdll_handle, f_name)) return "ntdll";
...
#error This version of MSVC has not been tested.
        ...
        return "msvcrt";
#endif
    if (jl_dlsym_e(jl_winsock_handle, f_name)) return "ws2_32";
