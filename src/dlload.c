...
static void *
jl_load_dynamic_library_(const char *modname, unsigned flags, int throw_err)
{
    char path[PATHBUF];
    int i;
    uv_stat_t stbuf;
...
#else
        Dl_info info;
        if (!dladdr((void *)(uintptr_t)&jl_load_dynamic_library, &info) ||
            !info.dli_fname)
            jl_error("could not load base module");
    ...
    */
    if (!abspath && jl_base_module != NULL) {
        jl_array_t *DL_LOAD_PATH = (jl_array_t *)jl_get_global(
                jl_base_module, jl_symbol("DL_LOAD_PATH"));
        if (DL_LOAD_PATH != NULL) {
            size_t j;
            for (j = 0; j < jl_array_len(DL_LOAD_PATH); j++) {
                char *dl_path =
                        jl_string_data(jl_array_ptr_data(DL_LOAD_PATH)[j]);
                ...
                for (i = 0; i < n_extensions; i++) {
                    ...
                    if (dl_path[len - 1] == PATHSEPSTRING[0])
                        snprintf(
                                path, PATHBUF, "%s%s%s", dl_path, modname, ext);
                    else
                        snprintf(
                                path,
                                PATHBUF,
...
JL_DLLEXPORT void *
jl_load_dynamic_library_e(const char *modname, unsigned flags)
{
...
#error This version of MSVC has not been tested.
    ...
    // We could also loop over all libraries that have been used so far, but,
    // again,
    // explicit is preferred over implicit
    return NULL;
    // oops, we didn't find it. NULL defaults to searching
    // jl_RTLD_DEFAULT_handle,
    // which defaults to jl_dl_handle, where we won't find it, and will throw
    // the
    // appropriate error.
}
#endif
