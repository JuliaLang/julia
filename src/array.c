                               ...
                               int isunboxed, int elsz)
{
    ...
    for (i = 0; i < ndims; i++) {
        ...
        if (prod > (wideint_t)MAXINTVAL) jl_error("invalid Array dimensions");
        nel = prod;
    }

    if (isunboxed) {
        wideint_t prod = (wideint_t)elsz * (wideint_t)nel;
        if (prod > (wideint_t)MAXINTVAL) jl_error("invalid Array size");
        tot = prod;
        if (elsz == 1) {
    ...
    if (tot <= ARRAY_INLINE_NBYTES) {
        ...
        if (!isunboxed) memset(data, 0, tot);
    }
    a->flags.pooled = tsz <= GC_MAX_SZCLASS;

    a->data                                                     = data;
    if (JL_ARRAY_IMPL_NUL && elsz == 1) ((char *)data)[tot - 1] = '\0';
    ...
    if (ndims == 1) {
        ...
        for (i = 0; i < ndims; i++) adims[i] = dims[i];
    }
...
static inline jl_array_t *_new_array(jl_value_t *atype, uint32_t ndims, size_t *dims)
{
    ...
    if (isunboxed) elsz = jl_datatype_size(el_type);
                                         ...
                                         int own_buffer)
{
    ...
    if (__unlikely(ndims == 1)) return jl_ptr_to_array_1d(atype, data, nel, own_buffer);
    jl_value_t *el_type = jl_tparam0(atype);
...
{
    size_t l = 1;
    for (size_t i = 0; i < jl_array_ndims(a); i++) l *= jl_array_dim(a, i);
    return l;
}
#endif
...
JL_CALLABLE(jl_f_arraysize)
{
    ...
    if (dno < 1) jl_error("arraysize: dimension out of range");
    if (dno > nd) return jl_box_long(1);
                             ...
                             const char *fname)
{
    ...
    size_t nd = jl_array_ndims(a);
    for (k = 0; k < nidxs; k++) {
        if (!jl_is_long(args[k])) jl_type_error(fname, (jl_value_t *)jl_long_type, args[k]);
        size_t ii = jl_unbox_long(args[k]) - 1;
        i += ii * stride;
        size_t d = k >= nd ? 1 : jl_array_dim(a, k);
        if (k < nidxs - 1 && ii >= d) jl_bounds_error_v((jl_value_t *)a, args, nidxs);
        stride *= d;
    }
    for (; k < nd; k++) stride *= jl_array_dim(a, k);
    if (i >= stride) jl_bounds_error_v((jl_value_t *)a, args, nidxs);
    return i;
}
...
{
    if (a->flags.ptrarray) return ((jl_value_t **)jl_array_data(a))[i] != NULL;
    return 1;
}
...
JL_DLLEXPORT void jl_arrayset(jl_array_t *a, jl_value_t *rhs, size_t i)
{
    ...
    if (el_type != (jl_value_t *)jl_any_type) {
        if (!jl_subtype(rhs, el_type, 1)) jl_type_error("arrayset", el_type, rhs);
    }
    if (!a->flags.ptrarray) {
...
{
    if (i >= jl_array_len(a)) jl_bounds_error_int((jl_value_t *)a, i + 1);
    char *ptail = (char *)a->data + i * a->elsize;
    if (a->flags.ptrarray) memset(ptail, 0, a->elsize);
}
...
{
    if (a->flags.isshared) {
        if (a->flags.how != 3) jl_error("cannot resize array with shared data");
        assert(a->offset == 0);
...
STATIC_INLINE void jl_array_grow_at_beg(jl_array_t *a, size_t idx, size_t inc, size_t n)
{
    ...
    if (a->offset >= inc) {
        ...
        if (inc > (a->maxsize - n) / 2 - (a->maxsize - n) / 20) {
            ...
            while (n + 2 * inc > newlen - a->offset) newlen *= 2;
            newlen           = limit_overallocation(a, n, newlen, 2 * inc);
            size_t newoffset = (newlen - newnrows) / 2;
            if (!array_resize_buffer(a, newlen)) data = (char *)a->data + oldoffsnb;
            ...
            if (idx > 0 && newdata < data) memmove(newdata, data, nb1);
...
STATIC_INLINE void jl_array_grow_at_end(jl_array_t *a, size_t idx, size_t inc, size_t n)
{
    ...
    if (__unlikely((n + inc) > a->maxsize - a->offset)) {
        ...
        while ((n + inc) > newlen - a->offset) newlen *= 2;
...
JL_DLLEXPORT void jl_array_grow_at(jl_array_t *a, ssize_t idx, size_t inc)
{
    ...
    if (idx < 0 || idx > n) jl_bounds_error_int((jl_value_t *)a, idx + 1);
    if (idx + 1 < n / 2) {
...
static size_t jl_array_limit_offset(jl_array_t *a, size_t offset)
{
    ...
    // and growing at end
    if (offset >= 13 * a->maxsize / 20) offset = 17 * (a->maxsize - a->nrows) / 100;
#ifdef _P64
...
STATIC_INLINE void jl_array_del_at_beg(jl_array_t *a, size_t idx, size_t dec, size_t n)
{
    ...
    if (__unlikely(newoffs != offset) || idx > 0) {
        ...
        if (elsz == 1) nbtotal++;
        if (idx > 0) memmove(newdata, olddata, nb1);
...
STATIC_INLINE void jl_array_del_at_end(jl_array_t *a, size_t idx, size_t dec, size_t n)
{
    ...
    size_t elsz = a->elsize;
    size_t last = idx + dec;
    if (n > last) memmove(data + idx * elsz, data + last * elsz, (n - last) * elsz);
    n -= dec;
    if (elsz == 1) data[n] = 0;
    a->nrows               = n;
#ifdef STORE_ARRAY_LEN
...
JL_DLLEXPORT void jl_array_del_at(jl_array_t *a, ssize_t idx, size_t dec)
{
    size_t n    = jl_array_nrows(a);
    size_t last = idx + dec;
    if (__unlikely(idx < 0)) jl_bounds_error_int((jl_value_t *)a, idx + 1);
    if (__unlikely(last > n)) jl_bounds_error_int((jl_value_t *)a, last);
    // The unsharing needs to happen before we modify the buffer
    if (__unlikely(a->flags.isshared)) array_try_unshare(a);
    if (idx < n - last) {
...
{
    size_t n = jl_array_nrows(a);
    if (__unlikely(dec > n)) jl_bounds_error_int((jl_value_t *)a, dec);
    if (__unlikely(a->flags.isshared)) array_try_unshare(a);
...
{
    size_t n = jl_array_nrows(a);
    if (__unlikely(n < dec)) jl_bounds_error_int((jl_value_t *)a, 0);
    if (__unlikely(a->flags.isshared)) array_try_unshare(a);
...
JL_DLLEXPORT jl_array_t *jl_array_copy(jl_array_t *ary)
{
    ...
    jl_array_t *new_ary = _new_array_(jl_typeof(ary), jl_array_ndims(ary), &ary->nrows,
                                      !ary->flags.ptrarray, elsz);
...
JL_DLLEXPORT jl_array_t *jl_array_cconvert_cstring(jl_array_t *a)
{
    ...
    if (!jl_has_implicit_byte(a)) a = jl_array_copy(a);
