        ...
        void **symaddr = nullptr)
{
    ...
    if (sym_iter == symMap->end()) {
        // MCJIT forces this to have external linkage eventually, so we would
        // clobber
        ...
        bool runtime_lib)
{
    ...
    builder.CreateCondBr(
            builder.CreateICmpNE(llvmf_orig, initnul), ccall_bb, dlsym_lookup);
    ...
    Value *llvmf = builder.CreateCall(
            prepare_call(builder, jldlsym_func),
            { libname, stringConstPtr(builder, f_name), libptrgv });
        ...
        Function *f)
{
    ...
    GlobalVariable *llvmgv;
    bool runtime_lib =
            runtime_sym_gvs(f_lib, f_name, f->getParent(), libptrgv, llvmgv);
    libptrgv = prepare_global(libptrgv);
    llvmgv = prepare_global(llvmgv);
    return runtime_sym_lookup(
            funcptype, f_lib, f_name, f, libptrgv, llvmgv, runtime_lib);
}
        ...
        bool runtime_lib)
{
    ...
    Function *plt = Function::Create(
            functype, GlobalVariable::ExternalLinkage, fname, M);
    ...
    GlobalVariable *got = new GlobalVariable(
            *M,
            T_pvoidfunc,
            false,
            GlobalVariable::ExternalLinkage,
    ...
    *(void **)jl_emit_and_add_to_shadow(got) = symaddr;
    ...
    Value *ptr = runtime_sym_lookup(
            funcptype, f_lib, f_name, plt, libptrgv, llvmgv, runtime_lib);
    auto store = builder.CreateAlignedStore(
            builder.CreateBitCast(ptr, T_pvoidfunc), got, sizeof(void *));
    store->setAtomic(AtomicOrdering::Release);
...
// Known failures includes vararg (not needed here) and sret.
    ...
    got = global_proto(got); // exchange got for the permanent global before
                             // jl_finalize_module destroys it
        ...
        const char *f_name)
{
    ...
    bool runtime_lib =
            runtime_sym_gvs(f_lib, f_name, LM, libptrgv, llvmgv, &symaddr);
    PointerType *funcptype = PointerType::get(functype, 0);
...
// --- ABI Implementations ---
// Partially based on the LDC ABI implementations licensed under the BSD
// 3-clause license
...
// Determine if object of bitstype ty maps to a native x86 SIMD type (__m128,
// __m256, or __m512) in C
...
static Value *llvm_type_rewrite(
        Value *v,
        Type *target_type,
        bool issigned, /* determines whether an integer value should be zero or
                          sign extended */
        jl_codectx_t *ctx)
{
    ...
    if (from_type == T_void || isa<UndefValue>(v))
        return UndefValue::get(target_type); // convert undef (unreachable) ->
                                             // undef (target_type)

    assert(from_type->isPointerTy() ==
           target_type->isPointerTy()); // expect that all ABIs consider all
                                        // pointers to be equivalent
...
static Value *
runtime_apply_type(jl_value_t *ty, jl_unionall_t *unionall, jl_codectx_t *ctx)
{
    ...
    return builder.CreateCall(
            prepare_call(jlapplytype_func), makeArrayRef(args));
}

static void typeassert_input(
        const jl_cgval_t &jvinfo,
        jl_value_t *jlto,
        jl_unionall_t *jlto_env,
        int argn,
        bool addressOf,
        jl_codectx_t *ctx)
{
    if (jlto != (jl_value_t *)jl_any_type && !jl_subtype(jvinfo.typ, jlto)) {
        if (!addressOf && jlto == (jl_value_t *)jl_voidpointer_type) {
            ...
            // due to Ref{T} conversion behavior in input
            ...
            if (!jlto_env || !jl_has_typevar_from_unionall(jlto, jlto_env)) {
                ...
                jl_cgval_t jlto_runtime = mark_julia_type(
                        runtime_apply_type(jlto, jlto_env, ctx),
                        true,
                        jl_any_type,
                        ctx);
                Value *vx = boxed(jvinfo, ctx);
                Value *istype = builder.CreateICmpNE(
                        builder.CreateCall(
                                prepare_call(jlisa_func),
                                { vx, boxed(jlto_runtime, ctx) }),
                        ConstantInt::get(T_int32, 0));
                BasicBlock *failBB =
                        BasicBlock::Create(jl_LLVMContext, "fail", ctx->f);
                BasicBlock *passBB =
                        BasicBlock::Create(jl_LLVMContext, "pass", ctx->f);
                builder.CreateCondBr(istype, passBB, failBB);

                builder.SetInsertPoint(failBB);
                emit_type_error(
                        mark_julia_type(vx, true, jl_any_type, ctx),
                        boxed(jlto_runtime, ctx),
                        msg.str(),
                        ctx);
                builder.CreateUnreachable();
        ...
        bool *needStackRestore)
{
    ...
    if (!jl_is_cpointer_type(jlto) || !to->isPointerTy()) {
        emit_error(
                "ccall: & on argument was not matched by Ptr{T} argument type",
                ctx);
    ...
    if (jvinfo.isboxed) {
        if (!jl_is_abstracttype(ety)) {
            if (jl_is_mutable_datatype(ety)) {
                ...
                return data_pointer(jvinfo, ctx, to);
                    ...
                    jl_is_immutable_datatype(ety) &&
                    jlto != (jl_value_t *)jl_voidpointer_type) {
                // yes copy
                Value *nbytes;
                AllocaInst *ai;
                ...
                builder.CreateMemCpy(
                        ai,
                        data_pointer(jvinfo, ctx, T_pint8),
                        nbytes,
                        sizeof(void *)); // minimum gc-alignment in julia is
                                         // pointer size
        ...
        *needStackRestore = true;
        ...
        BasicBlock *mutableBB =
                BasicBlock::Create(jl_LLVMContext, "is-mutable", ctx->f);
        BasicBlock *immutableBB =
                BasicBlock::Create(jl_LLVMContext, "is-immutable", ctx->f);
        BasicBlock *afterBB =
                BasicBlock::Create(jl_LLVMContext, "after", ctx->f);
        ...
        builder.CreateMemCpy(
                ...
                data_pointer(jvinfo, ctx, T_pint8),
                nbytes,
                sizeof(void *)); // minimum gc-alignment in julia is pointer
                                 // size
...
static void interpret_symbol_arg(
        native_sym_arg_t &out,
        jl_value_t *arg,
        jl_codectx_t *ctx,
        const char *fname,
        bool llvmcall)
    ...
    if (ptr == NULL) {
        ...
        if (!jl_is_cpointer_type(ptr_ty)) {
            emit_cpointercheck(
                    arg1,
                    !strcmp(fname, "ccall")
                            ? "ccall: first argument not a pointer or valid "
                              "constant expression"
                            : "cglobal: first argument not a pointer or valid "
                              "constant expression",
                    ctx);
        }
        ...
        bool compiletime = false)
{
    ...
    JL_TRY {
        ...
        constant = jl_interpret_toplevel_expr_in(
                ctx->module, ex, ctx->source, ctx->linfo->sparam_vals);
...
static jl_cgval_t
emit_cglobal(jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    if (sym.jl_ptr != NULL) {
        ...
        if (imaging_mode)
            jl_printf(
                    JL_STDERR,
                    "WARNING: literal address used in cglobal for %s; code "
                    "cannot be statically compiled\n",
                    sym.f_name);
    }
    else {
        if (imaging_mode) {
            res = runtime_sym_lookup(
                    (PointerType *)lrt, sym.f_lib, sym.f_name, ctx->f);
        }
        else {
            ...
            if (symaddr == NULL) {
                ...
                if (sym.f_lib != NULL) {
#ifdef _OS_WINDOWS_
                    assert(sym.f_lib != JL_EXE_LIBNAME &&
                           sym.f_lib != JL_DL_LIBNAME);
#endif
                    msg << " in library ";
                    msg << sym.f_lib;
                }
            ...
            // putting anything complicated here: just JIT the address of the
            // cglobal
...
public:
    ...
    void CloneFunctionBody(Function *F)
    {
        ...
        llvm::CloneFunctionInto(
                NewF, F, VMap, true, Returns, "", NULL, NULL, this);
    ...
    {
        Function *NewF =
                (llvm::Function *)MapValue(F, VMap, RF_None, NULL, this);
    ...
    Value *materialize(Value *V) override
    {
        ...
        if (F) {
            if (isIntrinsicFunction(F)) {
                return destModule->getOrInsertFunction(
            ...
            GlobalVariable *oldGV =
                    destModule->getGlobalVariable(GV->getName());
            if (oldGV != NULL)
                return oldGV;
            ...
            if (GV->isDeclaration())
                return newGV;
            if (!GV->getName().empty()) {
                uint64_t addr = jl_ExecutionEngine->getGlobalValueAddress(
                        GV->getName());
                if (addr != 0) {
            ...
            if (GV->hasInitializer()) {
                Value *C = MapValue(
                        GV->getInitializer(), VMap, RF_None, NULL, this);
...
static jl_cgval_t
emit_llvmcall(jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    at = try_eval(
            args[3],
            ctx,
            "error statically evaluating llvmcall argument tuple",
            true);
    rt = try_eval(
            args[2],
            ctx,
            "error statically evaluating llvmcall return type",
            true);
    ir = try_eval(
            args[1], ctx, "error statically evaluating llvm IR argument", true);
    int i = 1;
    if (jl_is_tuple(ir)) {
        // if the IR is a tuple, we expect (declarations, ir)
        if (jl_nfields(ir) != 2)
            jl_error("Tuple as first argument to llvmcall must have exactly "
                     "two children");
    ...
    if (!isString && !isPtr) {
        jl_error("IR passed to llvmcall must be a string or pointer to an LLVM "
                 "Function");
    }
     ...
     * If the argument type is immutable (including bitstype), we pass the
     * loaded llvm value
    ...
    for (size_t i = 0; i < nargt; ++i) {
        ...
        Value *v = julia_to_native(
                t, toboxed, tti, NULL, arg, false, i, ctx, NULL);
        bool issigned =
                jl_signed_type && jl_subtype(tti, (jl_value_t *)jl_signed_type);
    ...
    if (isString) {
        ...
        Module *m = NULL;
        bool failed = parseAssemblyInto(
                llvm::MemoryBufferRef(ir_string, "llvmcall"), *jl_Module, Err);
        if (!failed)
            m = jl_Module;
        if (m == NULL) {
    ...
    CallInst *inst =
            builder.CreateCall(f, ArrayRef<Value *>(&argvals[0], nargt));
    if (isString)
    ...
    if (inst->getType() != rettype) {
        jl_error("Return type of llvmcall'ed function does not match declared "
                 "return type");
    }
...
static jl_cgval_t mark_or_box_ccall_result(
        Value *result,
        bool isboxed,
        jl_value_t *rt,
        jl_unionall_t *unionall,
        bool static_rt,
        jl_codectx_t *ctx)
{
    if (!static_rt) {
        ...
        // TODO: is this leaf check actually necessary, or is it structurally
        // guaranteed?
        emit_leafcheck(
                runtime_dt, "ccall: return type must be a leaf DataType", ctx);
#if JL_LLVM_VERSION >= 40000
        const DataLayout &DL = jl_data_layout;
#else
        const DataLayout &DL = jl_ExecutionEngine->getDataLayout();
...
public:
    std::vector<Type *> fargt; // vector of llvm output types
                               // (julia_struct_to_llvm) for arguments (vararg
                               // is the last item, if applicable)
    std::vector<Type *>
            fargt_sig; // vector of ABI coercion types for call signature
    std::vector<bool> fargt_isboxed; // vector of whether the llvm output type
                                     // is a Julia-box for each argument (vararg
                                     // is the last item, if applicable)
    Type *fargt_vasig = NULL;        // ABI coercion type for vararg list
    std::vector<bool> byRefList; // vector of "byref" parameters (vararg is the
                                 // last item, if applicable)
#if JL_LLVM_VERSION >= 50000
    AttributeList attributes; // vector of function call site attributes (vararg
                              // is the last item, if applicable)
#else
    AttributeSet attributes; // vector of function call site attributes (vararg
                             // is the last item, if applicable)
#endif
    Type *lrt;     // input parameter of the llvm return type (from
                   // julia_struct_to_llvm)
    bool retboxed; // input parameter indicating whether lrt is jl_value_t*
    Type *prt;     // out parameter of the llvm return type for the function
                   // signature
    int sret;      // out parameter for indicating whether return value has been
                   // moved to the first argument position
    std::string err_msg;
    ...
    size_t nargs; // number of actual arguments (can be different from the size
                  // of at when varargs)
    size_t isVa;
    ...
    function_sig_t(
            ...
            CallingConv::ID cc,
            bool llvmcall)
        : fargt_vasig(NULL), lrt(lrt), retboxed(retboxed), prt(NULL), sret(0),
          cc(cc), llvmcall(llvmcall), functype(NULL), at(at), rt(rt),
          unionall_env(unionall_env), nargs(nargs), isVa(isVa)
    {
        ...
        if (type_is_ghost(lrt)) {
            ...
            if (!jl_is_datatype(rt) || ((jl_datatype_t *)rt)->layout == NULL ||
...
#if !defined(_OS_WINDOWS_) // llvm used to use the old mingw ABI, skipping this
                           // marking works around that difference
                ...
                paramattrs.push_back(
                        AttributeSet::get(jl_LLVMContext, 1, retattrs));
#endif
                fargt_sig.push_back(
                        PointerType::get(lrt, AddressSpace::Derived));
                sret = 1;
                prt = lrt;
            }
            else {
        ...
        for (i = 0; i < nargt;) {
            ...
            if (jl_is_abstract_ref_type(tti)) {
                if (jl_is_typevar(jl_tparam0(tti)))
                    jl_error("ccall: argument type Ref should have an element "
                             "type, not Ref{T}");
                ...
                if (jl_is_primitivetype(tti)) {
                    ...
                    if (jl_datatype_size(bt) < 4) {
                        if (jl_signed_type &&
                            jl_subtype(tti, (jl_value_t *)jl_signed_type))
            ...
            Type *pat;
            if (!jl_is_datatype(tti) ||
                ((jl_datatype_t *)tti)->layout == NULL || jl_is_array_type(tti))
            ...
            do { // for each arg for which this type applies, add the
                 // appropriate LLVM parameter attributes
                if (i < nargs) { // if vararg, the last declared arg type may
                                 // not have a corresponding arg value
#if JL_LLVM_VERSION >= 50000
                    paramattrs.push_back(std::move(ab));
#else
                    AttributeSet params =
                            AttributeSet::get(jl_LLVMContext, i + sret + 1, ab);
                    paramattrs.push_back(params);
#endif
                }
                i++;
            } while (current_isVa &&
                     i < nargs); // if is this is the vararg, loop to the end
        }
...
static const std::string verify_ccall_sig(
        size_t nargs,
        jl_value_t *&rt,
        jl_value_t *at,
        jl_unionall_t *unionall_env,
    ...
    if (unionall_env == NULL) {
        ...
        if (!static_rt && sparam_vals != NULL && jl_svec_len(sparam_vals) > 0) {
            rt = jl_instantiate_type_in_env(
                    rt, unionall_env, jl_svec_data(sparam_vals));
    ...
    if (!retboxed && static_rt) {
        if (!jl_is_leaf_type(rt)) {
            if (jl_is_cpointer_type(rt))
                return "ccall: return type Ptr should have an element type "
                       "(not Ptr{_<:T})";
...
static jl_cgval_t emit_ccall(jl_value_t **args, size_t nargs, jl_codectx_t *ctx)
{
    ...
    if (jl_is_abstract_ref_type(rt)) {
                ...
                for (i = 0; jl_is_unionall(ua); i++) {
                    if (ua->var == (jl_tvar_t *)ref) {
                        ...
                        if (runtime_sp.constant) {
                            ...
                            error_unless(
                                    notany,
                                    "ccall: return type Ref{Any} is invalid. "
                                    "use Ptr{Any} instead.",
                                    ctx);
                            always_error = false;
                        }
        ...
        if (always_error) {
            emit_error(
                    "ccall: return type Ref{Any} is invalid. use Ptr{Any} "
                    "instead.",
                    ctx);
            JL_GC_POP();
            return jl_cgval_t();
    ...
    auto _is_libjulia_func = [&](uintptr_t ptr, const char *name) {
        if ((uintptr_t)fptr == ptr)
            return true;
        return (!f_lib || f_lib == JL_DL_LIBNAME) && f_name &&
               !strcmp(f_name, name);
    };
#define is_libjulia_func(name) _is_libjulia_func((uintptr_t) & (name), #name)
    ...
    static const auto ptls_getter =
            jl_dlsym_e(jl_dlopen(nullptr, 0), "jl_get_ptls_states");
#else
    ...
    if (is_libjulia_func(jl_array_ptr)) {
        ...
        return mark_or_box_ccall_result(
                emit_bitcast(emit_arrayptr(ary, ctx), lrt),
                retboxed,
                rt,
                unionall,
                static_rt,
        ...
        if (!retboxed) {
            return mark_or_box_ccall_result(
                    emit_bitcast(
                            emit_pointer_from_objref(
                                    emit_bitcast(ary, T_prjlvalue)),
                            lrt),
                    retboxed,
                    rt,
                    unionall,
            ...
            return mark_or_box_ccall_result(
                    maybe_decay_untracked(emit_bitcast(ary, lrt)),
                    retboxed,
                    rt,
                    unionall,
                    static_rt,
...
// TODO
        ...
        static auto pauseinst = InlineAsm::get(
                FunctionType::get(T_void, false), "pause", "~{memory}", true);
        builder.CreateCall(pauseinst);
        JL_GC_POP();
        return ghostValue(jl_void_type);
#elif defined(_CPU_AARCH64_) || (defined(_CPU_ARM_) && __ARM_ARCH >= 7)
        static auto wfeinst = InlineAsm::get(
                FunctionType::get(T_void, false), "wfe", "~{memory}", true);
        ...
        return ghostValue(jl_void_type);
#elif defined(_CPU_AARCH64_) || (defined(_CPU_ARM_) && __ARM_ARCH >= 7)
        static auto sevinst = InlineAsm::get(
                FunctionType::get(T_void, false), "sev", "~{memory}", true);
        ...
        Value *ptid = builder.CreateGEP(
                ptls_i16, ConstantInt::get(T_size, tid_offset / 2));
        return mark_or_box_ccall_result(
        ...
        defer_sig =
                builder.CreateAdd(defer_sig, ConstantInt::get(T_sigatomic, 1));
        ...
        error_unless(
                builder.CreateICmpNE(
                        defer_sig, ConstantInt::get(T_sigatomic, 0)),
                "sigatomic_end called in non-sigatomic region",
                ctx);
        defer_sig =
                builder.CreateSub(defer_sig, ConstantInt::get(T_sigatomic, 1));
        builder.CreateStore(defer_sig, pdefer_sig);
        BasicBlock *checkBB =
                BasicBlock::Create(jl_LLVMContext, "check", ctx->f);
        ...
        builder.CreateLoad(
                builder.CreateConstGEP1_32(ctx->signalPage, -1), true);
        ...
        if (f && (jl_is_type_type((jl_value_t *)frt) &&
                  !jl_has_free_typevars(jl_tparam0(frt)))) {
            ...
            if (!fargt) {
                ...
                fargt = (jl_value_t *)jl_apply_tuple_type_v(
                        (jl_value_t **)jl_data_ptr(fargt), jl_nfields(fargt));
            }
            ...
            if (fargt) {
                ...
                Value *llvmf = NULL;
                JL_TRY {
                    llvmf = jl_cfunction_object(
                            (jl_function_t *)f, frt, (jl_tupletype_t *)fargt);
                }
                JL_CATCH {
                    llvmf = NULL;
                }
                if (llvmf) {
                    ...
                    // make sure to emit any side-effects that may have been
        ...
        JL_GC_POP();
            ...
            is_libjulia_func(jl_array_isassigned) &&
        ...
        if (jl_is_array_type(aryty)) {
            ...
            if (jl_isbits(ety)) {
            ...
            else if (!jl_has_free_typevars(ety)) { // TODO: jn/foreigncall
                                                   // branch has a better
                                                   // predicate
                ...
                Value *idx = emit_unbox(
                        T_size,
                        emit_expr(idxex, ctx),
                        (jl_value_t *)jl_ulong_type);
                Value *arrayptr = emit_bitcast(
                        emit_arrayptr(aryv, aryex, ctx), T_ppjlvalue);
                Value *slot_addr = builder.CreateGEP(arrayptr, idx);
                Value *load = tbaa_decorate(
                        tbaa_arraybuf, builder.CreateLoad(slot_addr));
                Value *res = builder.CreateZExt(
                        builder.CreateICmpNE(load, V_null), T_int32);
                JL_GC_POP();
                return mark_or_box_ccall_result(
                        res, retboxed, rt, unionall, static_rt, ctx);
            }
        }
    }
        ...
        assert(nargt == 1);
        auto obj =
                emit_pointer_from_objref(boxed(emit_expr(args[4], ctx), ctx));
        auto strp = builder.CreateConstGEP1_32(
                emit_bitcast(obj, T_pint8), sizeof(void *));
        JL_GC_POP();
        return mark_or_box_ccall_result(
                strp, retboxed, rt, unionall, static_rt, ctx);
    }

    // emit arguments
    jl_cgval_t *argv =
            (jl_cgval_t *)alloca(sizeof(jl_cgval_t) * (nargs - 3) / 2);
    SmallVector<Value *, 16> gc_uses;
    ...
    for (i = 4; i < nargs + 1; i += 2) {
        ...
        Value *gcuse = arg_root.gcroot ? builder.CreateLoad(arg_root.gcroot)
                                       : arg_root.V;
        if (gcuse)
    ...
    jl_cgval_t retval = sig.emit_a_ccall(
            symarg, nargt, addressOf, argv, gc_uses, static_rt, ctx);
    JL_GC_POP();
    return retval;
}
        ...
        jl_codectx_t *ctx)
{
    ...
    // save place before arguments, for possible insertion of temp arg area
    // saving code.
    BasicBlock::InstListType &instList =
            builder.GetInsertBlock()->getInstList();
    ...
    for (size_t ai = 0; ai < nargs; ai++) {
        ...
        Type *pargty = ai + sret < fargt_sig.size() ? fargt_sig.at(ai + sret)
                                                    : fargt_vasig;
        jl_cgval_t &arg = argv[ai];

        // if we know the function sparams, try to fill those in now
        // so that the julia_to_native type checks are more likely to be doable
        // (e.g. leaf types) at compile-time
        ...
        if (ctx->spvals_ptr == NULL && !toboxed && unionall_env &&
            ...
            jargty_in_env = jl_instantiate_type_in_env(
                    jargty_in_env,
                    unionall_env,
                    jl_svec_data(ctx->linfo->sparam_vals));
        ...
        if (!addressOf.at(ai)) {
            if (jl_is_abstract_ref_type(jargty)) {
                if (!jl_is_cpointer_type(arg.typ)) {
                    emit_cpointercheck(
                            arg,
                            "ccall: argument to Ref{T} is not a pointer",
                            ctx);
    ...
    if (sret) {
        ...
        if (jl_isbits(rt)) {
            ...
            assert(jl_datatype_size(rt) > 0 &&
                   "sret shouldn't be a singleton instance");
            result = emit_allocobj(
                    ctx,
                    jl_datatype_size(rt),
    ...
    if (llvmcall) {
        ...
        Type *funcptype = PointerType::get(functype, 0);
        llvmf = literal_static_pointer_val(
                (void *)(uintptr_t)symarg.fptr, funcptype);
        if (imaging_mode)
            jl_printf(
                    JL_STDERR,
                    "WARNING: literal address used in ccall for %s; code "
                    "cannot be statically compiled\n",
                    symarg.f_name);
    }
    else {
        ...
        if (imaging_mode) {
            ...
            if (functype->isVarArg())
                llvmf = runtime_sym_lookup(
                        funcptype, symarg.f_lib, symarg.f_name, ctx->f);
            else
                llvmf = emit_plt(
                        functype, attributes, cc, symarg.f_lib, symarg.f_name);
        }
        else {
            void *symaddr =
                    jl_dlsym_e(jl_get_library(symarg.f_lib), symarg.f_name);
            if (symaddr == NULL) {
                std::stringstream msg;
...
#ifdef _OS_WINDOWS_
                    assert(symarg.f_lib != JL_EXE_LIBNAME &&
    ...
    if (needStackRestore) {
        ...
        builder.CreateCall(
                Intrinsic::getDeclaration(jl_Module, Intrinsic::stackrestore),
                stacksave);
    }
    if (0) { // Enable this to turn on SSPREQ (-fstack-protector) on the
             // function containing this ccall
    ...
    if (retboxed) {
        ...
        Type *jlrt = julia_type_to_llvm(rt, &jlretboxed); // compute the real
                                                          // "julian" return
                                                          // type and compute
                                                          // whether it is boxed
        if (jlretboxed) {
        ...
        if (type_is_ghost(jlrt)) {
            return ghostValue(rt);
                ...
                jl_is_datatype(rt) &&
                jl_is_datatype_singleton((jl_datatype_t *)rt)) {
            ...
            jlretboxed = false; // trigger mark_or_box_ccall_result to build the
                                // runtime box
        }
        else if (lrt != prt) {
            assert(jlrt == lrt || !lrt->isStructTy()); // julia_type_to_llvm and
                                                       // julia_struct_to_llvm
                                                       // should be returning
                                                       // the same StructType
    ...
    return mark_or_box_ccall_result(
            result, jlretboxed, rt, unionall_env, static_rt, ctx);
}
