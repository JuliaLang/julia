...
void jl_profile_task(void)
{
    ...
    // TODO:
    // 1. Retry less aggressively (and possibly not even retry at all).
    // 2. Can we take the address of a sentinel C function (e.g. `dummy_stack_trace`) and record it
    //    as a backtrace entry?
    // 3. The "task is done" seems like a much more common reason for failure, than `jl_record_backtrace`
    //    failing to get a backtrace.
    // 4. Try a workload which spawns a lot of tasks which die quickly (what's the "loss due to done tasks" rate?)

    const int n_max_random_attempts = (1 << 16);
    ...
    // sequentially select a task that is not done
    // if we failed to find a task randomly
    if (t == NULL) {
        for (int i = 0; i < tasks->len; i++) {
            t = (jl_task_t*)tasks->items[i];
            assert(t == NULL || jl_is_task(t));
            if (t == NULL) {
                continue;
            }
