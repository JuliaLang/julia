...
jl_gc_find_taggedvalue_pool(char *p, size_t *osize_p)
{
    if (!page_metadata(p))
        // Not in the pool
        return NULL;
...
/*
 ...
 First, reproduce it with GC_VERIFY. It does change the allocation profile so if
 the error
 is rare enough this may not be straightforward. If the backtracking goes well
 you should know
 which object and which of its slots was written to without being caught by the
 write
 barrier. Most times this allows you to take a guess. If this type of object is
 modified
 by C code directly, look for missing jl_gc_wb() on pointer updates. Be aware
 that there are
 innocent looking functions which allocate (and thus trigger marking) only on
 special cases.

 If you cant find it, you can try the following :
 - Ensure that should_timeout() is deterministic instead of clock based.
 - Once you have a completly deterministic program which crashes on gc_verify,
 the addresses
   should stay constant between different runs (with same binary, same
 environment ...).
   Do not forget to turn off ASLR (linux: echo 0 >
 /proc/sys/kernel/randomize_va_space).
   At this point you should be able to run under gdb and use a hw watch to look
 for writes
   at the exact addr of the slot (use something like watch *slot_addr if
 *slot_addr == val).
 - If it went well you are now stopped at the exact point the problem is
 happening.
   Backtraces in JIT'd code wont work for me (but I'm not sure they should) so
 in that
...
static void gc_clear_mark_pagetable1(pagetable1_t *pagetable1, int bits)
{
    for (int pg_i = 0; pg_i < REGION1_PG_COUNT / 32; pg_i++) {
        ...
        if (line) {
            for (int j = 0; j < 32; j++) {
                if ((line >> j) & 1) {
                    gc_clear_mark_pagetable0(
                            pagetable1->meta0[pg_i * 32 + j], bits);
                }
            }
        }
    }
...
static void gc_clear_mark_pagetable(int bits)
{
    for (int pg_i = 0; pg_i < (REGION2_PG_COUNT + 31) / 32; pg_i++) {
        ...
        if (line) {
            for (int j = 0; j < 32; j++) {
                if ((line >> j) & 1) {
                    gc_clear_mark_pagetable1(
                            memory_map.meta1[pg_i * 32 + j], bits);
                }
            }
        }
    }
...
static void gc_verify_track(jl_ptls_t ptls)
{
    ...
    do {
        ...
        if (lostval_parents.len == 0) {
            jl_printf(JL_STDERR, "Could not find the missing link. We missed a "
                                 "toplevel root. This is odd.\n");
            break;
        }
        ...
        if (lostval_parent ==
            ...
            jl_printf(
                    JL_STDERR,
                    "%p was written a reference to %p that was not recorded\n",
                    lostval_parent,
                    lostval);
...
void gc_verify(jl_ptls_t ptls)
{
    ...
    for (int i = 0; i < clean_len + bits_save[GC_OLD].len; i++) {
        jl_taggedvalue_t *v =
                (jl_taggedvalue_t *)
                        bits_save[i >= clean_len ? GC_OLD : GC_CLEAN]
    ...
    while ((char *)v <= lim) {
        ...
        if (!in_freelist) {
            ...
            if (dt != (jl_value_t *)jl_buff_tag &&
                // the following are used by the deserializer to invalidate
                // objects
                v->header != 0x10 &&
                v->header != 0x20 && v->header != 0x30 && v->header != 0x40 &&
    ...
    for (int t_i = 0; t_i < jl_n_threads; t_i++) {
        ...
        for (int i = 0; i < JL_GC_N_POOLS; i++) {
            ...
            while (next) {
                ...
                // TODO: verify they are ordered and on the right byte
                // boundaries
                if (gc_page_data(next) != gc_page_data(last)) {
                    ...
                    if (gc_page_data(next) != allocating) {
                        // when not currently allocating on this page,
                        // fl_begin_offset should be correct
...
static void gc_scrub_range(char *low, char *high)
{
    ...
    for (char **stack_p = ((char **)high) - 1; stack_p > (char **)low;
         stack_p--) {
        char *p = *stack_p;
        size_t osize;
        jl_taggedvalue_t *tag = jl_gc_find_taggedvalue_pool(p, &osize);
        if (osize <= sizeof(jl_taggedvalue_t) || !tag ||
            gc_marked(tag->bits.gc))
            continue;
...
void objprofile_count(void *ty, int old, int sz)
{
    ...
    if ((intptr_t)ty <= 0x10) {
        ty = (void *)jl_buff_tag;
            ...
            ty != (void *)jl_buff_tag && ty != jl_malloc_tag &&
...
static void
gc_stats_pagetable1(pagetable1_t *pagetable1, unsigned *p1, unsigned *p0)
{
    for (int pg_i = 0; pg_i < REGION1_PG_COUNT / 32; pg_i++) {
        uint32_t line =
                pagetable1->allocmap0[pg_i] | pagetable1->freemap0[pg_i];
        if (line) {
            for (int j = 0; j < 32; j++) {
                if ((line >> j) & 1) {
                    (*p1)++;
        ...
        if (line) {
            for (int j = 0; j < 32; j++) {
                if ((line >> j) & 1) {
                    (*p2)++;
                    gc_stats_pagetable1(
                            memory_map.meta1[pg_i * 32 + j], p1, p0);
                }
            }
        }
    }
...
void jl_print_gc_stats(JL_STREAM *s)
{
    ...
    if (gc_num.pause > 0) {
        jl_printf(
                s,
                "gc time  \t%.5f sec (%2.1f%%) in %d (%d full) collections\n",
        ...
        jl_printf(
                s,
                "\t\t(%2d%% mark, %2d%% sweep, %2d%% finalizers)\n",
    ...
    jl_printf(
            s,
            "page table max utilization : %u (%.1f%%) - %u (%.1f%%) - %u "
            "(%.1f%%)\n",
            p2,
            p2 * 100.0 / REGION2_PG_COUNT,
            p1,
            p1 * 100.0 / REGION1_PG_COUNT / p2,
            p0,
...
void gc_time_sysimg_end(uint64_t t0)
{
    ...
    jl_printf(
            JL_STDOUT, "GC sweep sysimg end %.2f ms\n", sweep_pool_sec * 1000);
}
...
void gc_time_mark_pause(
        int64_t t0, int64_t scanned_bytes, int64_t perm_scanned_bytes)
{
...
void gc_time_sweep_pause(
        uint64_t gc_end_t,
        int64_t actual_allocd,
        int64_t live_bytes,
        int64_t estimate_freed,
        int sweep_full)
{
    ...
    jl_printf(
            JL_STDOUT,
            "GC sweep pause %.2f ms live %" PRId64 " kB "
            "(freed %" PRId64 " kB EST %" PRId64 " kB "
            "[error %" PRId64 "] = %d%% of allocd b %" PRIu64 ") "
            "(%.2f ms in post_mark) %s | next in %" PRId64 " kB\n",
            jl_ns2ms(sweep_pause),
            live_bytes / 1024,
            gc_num.freed / 1024,
            estimate_freed / 1024,
            gc_num.freed - estimate_freed,
            pct,
            gc_num.since_sweep / 1024,
...
static size_t
pool_stats(jl_gc_pool_t *p, size_t *pwaste, size_t *np, size_t *pnold)
{
    jl_taggedvalue_t *v;
    ...
    *pnold = nold;
    if (npgs != 0) {
        jl_printf(
                JL_STDOUT,
                "%4d : %7d/%7d objects (%3d%% old), %5d pages, %5d kB, %5d kB "
                "waste\n",
                p->osize,
                nused,
                nused + nfree,
...
void gc_stats_all_pool(void)
{
    ...
    jl_printf(
            JL_STDOUT,
            "%d objects (%d%% old), %d kB (%d%% old) total allocated, %d total "
            "fragments (%d%% overhead), in %d pages\n",
            no,
            (nold * 100) / no,
            nb / 1024,
            (noldbytes * 100) / nb,
...
void gc_stats_big_obj(void)
{
    ...
    jl_printf(
            JL_STDOUT,
            "%d kB (%d%% old) in %d large objects (%d%% old)\n",
            (nbytes + nbytes_old) / 1024,
            nbytes + nbytes_old ? (nbytes_old * 100) / (nbytes + nbytes_old)
                                : 0,
            nused + nused_old,
...
NOINLINE void
gc_mark_loop_unwind(jl_ptls_t ptls, gc_mark_sp_t sp, int pc_offset)
{
    jl_jmp_buf *old_buf = ptls->safe_restore;
    jl_jmp_buf buf;
    ptls->safe_restore = &buf;
    ...
    while (sp.pc < top) {
        ...
        if (pc == gc_mark_label_addrs[GC_MARK_L_marked_obj]) {
            gc_mark_marked_obj_t *data =
                    gc_repush_markdata(&sp, gc_mark_marked_obj_t);
            if ((char *)data > data_top) {
                jl_safe_printf("Mark stack unwind overflow -- ABORTING !!!\n");
                break;
            }
            jl_safe_printf(
                    "%p: Root object: %p :: %p (bits: %d)\n        of type ",
                    (void *)data,
                    (void *)data->obj,
                    (void *)data->tag,
                    (int)data->bits);
            ...
            jl_fielddesc8_t *desc =
                    (jl_fielddesc8_t *)jl_dt_layout_fields(vt->layout);
            jl_safe_printf(
                    "%p:  %s Object (8bit) %p :: %p -- [%d, %d)\n        of "
                    "type ",
                    (void *)data,
                    prefix,
                    (void *)data->parent,
            ...
            jl_fielddesc16_t *desc =
                    (jl_fielddesc16_t *)jl_dt_layout_fields(vt->layout);
            jl_safe_printf(
                    "%p:  %s Object (16bit) %p :: %p -- [%d, %d)\n        of "
                    "type ",
                    (void *)data,
                    prefix,
                    (void *)data->parent,
            ...
            jl_fielddesc32_t *desc =
                    (jl_fielddesc32_t *)jl_dt_layout_fields(vt->layout);
            jl_safe_printf(
                    "%p:  %s Object (32bit) %p :: %p -- [%d, %d)\n        of "
                    "type ",
                    (void *)data,
                    prefix,
                    (void *)data->parent,
            ...
            jl_safe_printf(
                    "%p:  %s Module (bindings) %p (bits %d) -- [%p, %p)\n",
                    (void *)data,
                    prefix,
                    (void *)data->parent,
                    (int)data->bits,
