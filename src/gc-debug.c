...
static void gc_verify_track(jl_ptls_t ptls)
{
    do {
        ...
        if (lostval_parents.len == 0) {
            jl_printf(JL_STDERR, "Could not find the missing link. We missed a toplevel "
                                 "root. This is odd.\n");
            break;
        }
...
void jl_print_gc_stats(JL_STREAM *s)
{
    ...
    int i = 0;
    while (i < REGION_COUNT && regions[i].pages)
        i++;
...
void gc_stats_all_pool(void)
{
    ...
    jl_printf(JL_STDOUT, "%d objects (%d%% old), %d kB (%d%% old) total allocated, %d "
                         "total fragments (%d%% overhead), in %d pages\n",
