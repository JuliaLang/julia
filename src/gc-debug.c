...
jl_gc_pagemeta_t *jl_gc_page_metadata(void *data) { return page_metadata(data); }

region_t *jl_gc_find_region(void *ptr) { return find_region(ptr); }
...
JL_DLLEXPORT jl_taggedvalue_t *jl_gc_find_taggedvalue_pool(char *p, size_t *osize_p)
{
    ...
    // In the page header
    if (p < page_begin) return NULL;
    size_t ofs    = p - page_begin;
    int    pg_idx = page_index(r, page_begin);
    // Check if this is a free page
    if (!(r->allocmap[pg_idx / 32] & (uint32_t)(1 << (pg_idx % 32)))) return NULL;
    ...
    if (tag + osize > gc_page_data(p) + GC_PAGE_SZ) return NULL;
    if (osize_p) *osize_p = osize;
...
void add_lostval_parent(jl_value_t *parent)
{
    for (int i = 0; i < lostval_parents_done.len; i++) {
        if ((jl_value_t *)lostval_parents_done.items[i] == parent) return;
    }
    for (int i = 0; i < lostval_parents.len; i++) {
        if ((jl_value_t *)lostval_parents.items[i] == parent) return;
    }
...
static void clear_mark(int bits)
{
    ...
    for (int h = 0; h < REGION_COUNT; h++) {
        ...
        if (!region->pages) break;
        for (int pg_i = 0; pg_i < region->pg_cnt / 32; pg_i++) {
            uint32_t line = region->allocmap[pg_i];
            if (line) {
                for (int j = 0; j < 32; j++) {
                    if ((line >> j) & 1) {
                        jl_gc_pagemeta_t *pg = page_metadata(
                            region->pages[pg_i * 32 + j].data + GC_PAGE_OFFSET);
...
static void gc_debug_alloc_setnext(jl_alloc_num_t *num)
{
    ...
    if (!num->interv || next > num->max || interv > next) next = UINT64_MAX;
    num->next                                                  = next;
}
...
static void gc_debug_alloc_init(jl_alloc_num_t *num, const char *name)
{
    ...
    if (num->interv == 0) num->interv = 1;
    num->next                         = num->min;
}

static int gc_debug_alloc_check(jl_alloc_num_t *num)
{
    if (++num->num < num->next) return 0;
    gc_debug_alloc_setnext(num);
    return 1;
}

int gc_debug_check_pool(void) { return gc_debug_alloc_check(&jl_gc_debug_env.pool); }

int gc_debug_check_other(void) { return gc_debug_alloc_check(&jl_gc_debug_env.other); }
...
{
    gc_debug_print_status();
    if (!jl_gc_debug_env.wait_for_debugger) return;
...
{
    if (!gc_debug_alloc_check(&jl_gc_debug_env.print)) return;
    gc_debug_print_status();
}
...
static void gc_scrub_range(char *stack_lo, char *stack_hi)
{
    ...
    for (char **stack_p = (char **)stack_lo; stack_p > (char **)stack_hi; stack_p--) {
        char *            p = *stack_p;
        size_t            osize;
        jl_taggedvalue_t *tag = jl_gc_find_taggedvalue_pool(p, &osize);
        if (osize <= sizeof(jl_taggedvalue_t) || !tag || gc_marked(tag->bits.gc)) continue;
...
void gc_scrub(char *stack_hi) { gc_scrub_range(gc_stack_lo, stack_hi); }
#else
void gc_debug_critical_error(void) {}
...
static void objprofile_print(htable_t nums, htable_t sizes)
{
    for (int i = 0; i < nums.size; i += 2) {
        if (nums.table[i + 1] != HT_NOTFOUND) {
            ...
            if (sz > 2e9) {
                jl_printf(JL_STDERR, " %6d : %*.1f GB of (%*p) ", num, 6,
                          ((double)sz) / 1024 / 1024 / 1024, ptr_hex_width, ty);
            }
            else if (sz > 2e6) {
                jl_printf(JL_STDERR, " %6d : %*.1f MB of (%*p) ", num, 6,
                          ((double)sz) / 1024 / 1024, ptr_hex_width, ty);
            }
            else if (sz > 2e3) {
                jl_printf(JL_STDERR, " %6d : %*.1f kB of (%*p) ", num, 6,
                          ((double)sz) / 1024, ptr_hex_width, ty);
            }
            else {
                jl_printf(JL_STDERR, " %6d : %*d  B of (%*p) ", num, 6, (int)sz,
                          ptr_hex_width, ty);
            }
...
STATIC_INLINE double jl_ns2ms(int64_t t) { return t / (double)1e6; }

STATIC_INLINE double jl_ns2s(int64_t t) { return t / (double)1e9; }

static uint64_t gc_premark_end;
static uint64_t gc_postmark_end;
void            gc_settime_premark_end(void) { gc_premark_end = jl_hrtime(); }
void            gc_settime_postmark_end(void) { gc_postmark_end = jl_hrtime(); }
#endif
...
void gc_time_pool_end(int sweep_full)
{
    ...
    jl_printf(JL_STDOUT, "GC sweep pools end %.2f ms at %.1f GB/s "
                         "(skipped %.2f %% of %" PRId64 ", swept %" PRId64 " pgs, "
                         "%" PRId64 " freed with %" PRId64 " lazily) %s\n",
              sweep_pool_sec * 1000, sweep_speed,
              (total_pages ? ((double)skipped_pages * 100) / total_pages : 0), total_pages,
              total_pages - skipped_pages, freed_pages, lazy_freed_pages,
...
void gc_time_mark_pause(int64_t t0, int64_t scanned_bytes, int64_t perm_scanned_bytes)
{
    ...
    jl_printf(JL_STDOUT, "GC mark pause %.2f ms | "
              ...
              jl_ns2ms(gc_premark_end - t0), (scanned_bytes + perm_scanned_bytes) / 1024,
              scanned_bytes / 1024, perm_scanned_bytes / 1024, last_remset_len,
              remset_nptr);
}
...
void gc_debug_init(void)
{
    ...
    if (env && strcmp(env, "0") != 0) jl_gc_debug_env.always_full = 1;
...
#ifdef GC_VERIFY
    for (int i = 0; i < 4; i++) arraylist_new(&bits_save[i], 0);
...
void gc_stats_all_pool(void)
{
    ...
    jl_printf(JL_STDOUT, "%d objects (%d%% old), %d kB (%d%% old) total "
                         "allocated, %d total fragments (%d%% overhead), in %d "
                         "pages\n",
              no, (nold * 100) / no, nb / 1024, (noldbytes * 100) / nb, tw, (tw * 100) / nb,
              tp);
}
