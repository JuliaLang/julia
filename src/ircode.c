...
typedef struct {
    ...
    jl_method_t *method;
    jl_value_t *edges; // a SimpleVector, or the compact edge list of an image CodeInstance
    jl_ptls_t ptls;
...
static void literal_val_id(rle_reference *rr, jl_ircode_state *s, jl_value_t *v) JL_CANSAFEPOINT
{
    ...
    size_t nedges = jl_is_svec(s->edges) ? jl_svec_len(s->edges) : 0; // compact image edge lists are never used as encoding targets
...
// Compressed IR may refer to an image object by its (image, offset) position.
// Objects that get uniqued at load (types, methods, modules, symbols, singletons, ...)
// are excluded: which copy wins depends on load order, so in another process
// the offset may point at a copy that was replaced.
...
static void jl_encode_as_indexed_root(jl_ircode_state *s, jl_value_t *v) JL_CANSAFEPOINT
{
    ...
    // Not when building a sysimage: this IR is written into a new image with a different
    // layout, so the offsets would be wrong. Package images are fine, because their
    // dependency files are checked to be byte-identical when loaded.
...
JL_DLLEXPORT jl_code_info_t *jl_uncompress_ir(jl_method_t *m, jl_code_instance_t *metadata, jl_string_t *data)
{
    ...
    if (metadata) {
        ...
        if (new_debuginfo != NULL)
            jl_di_materialize_all(new_debuginfo); // callers read this tree with raw field reads
