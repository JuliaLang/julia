...
typedef struct _jl_method_t {
    ...
    jl_svec_t *tvars;
    // list of potentially-ambiguous methods (nothing = none, Vector{Any} of Methods
    // otherwise)
    jl_value_t *ambig;
...
typedef struct _jl_lambda_info_t {
    ...
    struct _jl_lambda_info_t *unspecialized_ducttape; // if template can't be compiled due
                                                      // to intrinsics, an un-inferred
                                                      // executable copy may get stored here
    ...
    int8_t
              jlcall_api; // the c-abi for fptr; 0 = jl_fptr_t, 1 = jl_fptr_sparam_t, 2 = constval
