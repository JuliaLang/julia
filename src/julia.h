...
// it forms a roughly tree-shaped structure, consisting of nodes of
// TypeMapLevels
// which split the tree when possible, for example based on the key into the
// tuple type at `offs`
// when key is a leaftype, (but only when the tree has enough entries for this
// to be
...
typedef jl_value_t *(*jl_fptr_sparam_t)(
        jl_svec_t *, jl_value_t *, jl_value_t **, uint32_t);
typedef jl_value_t *(*jl_fptr_linfo_t)(
        struct _jl_method_instance_t *, jl_value_t **, uint32_t, jl_svec_t *);
...
typedef struct _jl_method_t {
    ...
    // list of potentially-ambiguous methods (nothing = none, Vector{Any} of
    // Methods otherwise)
    jl_value_t *ambig;

    // table of all argument types for which we've inferred or compiled this
    // code
    union jl_typemap_t specializations;

    jl_svec_t *sparam_syms; // symbols giving static parameter names
    jl_value_t *source; // original code template (jl_code_info_t, but may be
                        // compressed), null for builtins
    struct _jl_method_instance_t
            *unspecialized; // unspecialized executable method instance, or null
    struct _jl_method_instance_t
            *generator; // executable code-generating function if isstaged
    ...
    int32_t nargs;
    int32_t called; // bit flags: whether each of the first 8 arguments is
                    // called
    uint8_t isva;
    uint8_t isstaged;
    uint8_t pure;
...
typedef struct _jl_method_instance_t {
    ...
    jl_svec_t *sparam_vals;          // static parameter values, indexed by
                                     // def.method->sparam_syms
    jl_array_t *backedges;
    jl_value_t *inferred; // inferred jl_code_info_t, or value of the function
                          // if jlcall_api == 2, or null
    jl_value_t *inferred_const; // inferred constant return value, or null
    size_t min_world;
    size_t max_world;
    uint8_t inInference;    // flags to tell if inference is running on this
                            // function
    uint8_t jlcall_api;     // the c-abi for fptr; 0 = jl_fptr_t, 1 =
                            // jl_fptr_sparam_t, 2 = constval
    uint8_t compile_traced; // if set will notify callback if this linfo is
                            // compiled
    jl_fptr_t fptr; // jlcall entry point with api specified by jlcall_api
    jl_fptr_t unspecialized_ducttape; // if template can't be compiled due to
                                      // intrinsics, an un-inferred fptr may get
                                      // stored here, jlcall_api = 1
...
// UnionAll type (iterated union over all values of a variable in certain
// bounds)
...
// of a type and storing all data common to different instantiations of the
// type,
...
// in little-endian, isptr is always the first bit, avoiding the need for a
// branch in computing isptr
typedef struct {
    uint8_t isptr : 1;
    uint8_t size : 7;
...
typedef struct {
    ...
    uint32_t npointers : 20;     // number of pointer fields, top 4 bits are
                                 // exponent (under-approximation)
...
typedef struct _jl_typemap_entry_t {
    ...
    int8_t isleafsig; // isleaftype(sig) & !any(isType, sig) : unsorted and very
                      // fast
    int8_t issimplesig; // all(isleaftype | isAny | isType | isVararg, sig) :
                        // sorted and fast
...
typedef struct _jl_methtable_t {
    ...
    jl_module_t *module;  // used for incremental serialization to locate
                          // original binding
    jl_array_t *backedges;
    jl_mutex_t writelock;
} jl_methtable_t;
...
#define JL_GC_PUSH5(arg1, arg2, arg3, arg4, arg5)                              \
    void *__gc_stkf[] = {                                                      \
        (void *)11, jl_pgcstack, arg1, arg2, arg3, arg4, arg5                  \
    };                                                                         \
    jl_pgcstack = (jl_gcframe_t *)__gc_stkf;

#define JL_GC_PUSH6(arg1, arg2, arg3, arg4, arg5, arg6)                        \
    void *__gc_stkf[] = { (void *)13, jl_pgcstack, arg1, arg2,                 \
                          arg3,       arg4,        arg5, arg6 };               \
    jl_pgcstack = (jl_gcframe_t *)__gc_stkf;
...
STATIC_INLINE void jl_gc_wb(void *parent, void *ptr)
{
    ...
    if (__unlikely(
                jl_astaggedvalue(parent)->bits.gc == 3 &&
...
#define jl_array_data_owner(a)                                                 \
    (*((jl_value_t *                                                           \
                *)((char *)a + jl_array_data_owner_offset(jl_array_ndims(a)))))
...
#define jl_exprarg(e, n)                                                       \
    (((jl_value_t **)jl_array_data(((jl_expr_t *)(e))->args))[n])
...
// struct type info
#define jl_field_name(st, i)                                                   \
    (jl_sym_t *)jl_svecref(((jl_datatype_t *)st)->name->names, (i))
...
#define jl_dt_layout_fields(d)                                                 \
    ((const char *)(d) + sizeof(jl_datatype_layout_t))
...
static inline int jl_field_isptr(jl_datatype_t *st, int i)
{
    const jl_datatype_layout_t *ly = st->layout;
    assert(i >= 0 && (size_t)i < ly->nfields);
    return ((const jl_fielddesc8_t
                     *)(jl_dt_layout_fields(ly) + (i << (ly->fielddesc_type + 1))))
            ->isptr;
}
...
#define jl_is_tuple(v)                                                         \
    (((jl_datatype_t *)jl_typeof(v))->name == jl_tuple_typename)
...
#define jl_is_mutable_datatype(t)                                              \
    (jl_is_datatype(t) && (((jl_datatype_t *)t)->mutabl))
#define jl_is_immutable(t) (!((jl_datatype_t *)t)->mutabl)
#define jl_is_immutable_datatype(t)                                            \
    (jl_is_datatype(t) && (!((jl_datatype_t *)t)->mutabl))
...
#define jl_is_slot(v)                                                          \
    (jl_typeis(v, jl_slotnumber_type) || jl_typeis(v, jl_typedslot_type))
...
STATIC_INLINE int jl_is_primitivetype(void *v)
{
    return (jl_is_datatype(v) && jl_is_immutable(v) &&
            ((jl_datatype_t *)(v))->layout && jl_datatype_nfields(v) == 0 &&
...
STATIC_INLINE int jl_is_structtype(void *v)
{
    return (jl_is_datatype(v) &&
            (jl_field_count(v) > 0 || jl_datatype_size(v) == 0) &&
...
{
    return (!d->abstract && jl_datatype_size(d) == 0 && d != jl_sym_type &&
            d->name != jl_array_typename && d->uid != 0 &&
...
{
    return (jl_is_datatype(t) &&
            ((jl_datatype_t *)(t))->name ==
                    ((jl_datatype_t *)jl_pointer_type->body)->name);
}
...
STATIC_INLINE int jl_is_abstract_ref_type(jl_value_t *t)
{
    return (jl_is_datatype(t) &&
            ((jl_datatype_t *)(t))->name ==
                    ((jl_datatype_t *)jl_ref_type->body)->name);
}
...
{
    return (jl_is_datatype(v) &&
            ((jl_datatype_t *)(v))->name ==
                    ((jl_datatype_t *)jl_type_type->body)->name);
}
...
JL_DLLEXPORT int
jl_subtype_env(jl_value_t *x, jl_value_t *y, jl_value_t **env, int envsz);
...
jl_new_typename_in(jl_sym_t *name, jl_module_t *inmodule);
...
JL_DLLEXPORT jl_value_t *
jl_apply_type(jl_value_t *tc, jl_value_t **params, size_t n);
...
STATIC_INLINE jl_vararg_kind_t jl_vararg_kind(jl_value_t *v)
{
    ...
    if (jl_is_typevar(lenv) && lenv != (jl_value_t *)v1 &&
        lenv != (jl_value_t *)v2)
...
JL_DLLEXPORT jl_array_t *
jl_reshape_array(jl_value_t *atype, jl_array_t *data, jl_value_t *dims);
JL_DLLEXPORT jl_array_t *
jl_ptr_to_array_1d(jl_value_t *atype, void *data, size_t nel, int own_buffer);
JL_DLLEXPORT jl_array_t *jl_ptr_to_array(
        jl_value_t *atype, void *data, jl_value_t *dims, int own_buffer);

JL_DLLEXPORT jl_array_t *jl_alloc_array_1d(jl_value_t *atype, size_t nr);
JL_DLLEXPORT jl_array_t *
jl_alloc_array_2d(jl_value_t *atype, size_t nr, size_t nc);
JL_DLLEXPORT jl_array_t *
...
JL_DLLEXPORT void
jl_arrayset(jl_array_t *a, jl_value_t *v, size_t i);      // 0-indexed
...
JL_DLLEXPORT void
jl_array_ptr_1d_push2(jl_array_t *a, jl_value_t *b, jl_value_t *c);
...
JL_DLLEXPORT void
jl_module_use(jl_module_t *to, jl_module_t *from, jl_sym_t *s);
JL_DLLEXPORT void
...
JL_DLLEXPORT jl_value_t *
jl_eqtable_get(jl_array_t *h, void *key, jl_value_t *deflt);
...
JL_DLLEXPORT void JL_NORETURN
jl_exceptionf(jl_datatype_t *ty, const char *fmt, ...);
...
JL_DLLEXPORT void JL_NORETURN
jl_type_error(const char *fname, jl_value_t *expected, jl_value_t *got);
...
JL_DLLEXPORT void JL_NORETURN
jl_bounds_error_v(jl_value_t *v, jl_value_t **idxs, size_t nidxs);
JL_DLLEXPORT void JL_NORETURN jl_bounds_error_int(jl_value_t *v, size_t i);
JL_DLLEXPORT void JL_NORETURN
jl_bounds_error_tuple_int(jl_value_t **v, size_t nv, size_t i);
JL_DLLEXPORT void JL_NORETURN
jl_bounds_error_unboxed_int(void *v, jl_value_t *vt, size_t i);
JL_DLLEXPORT void JL_NORETURN
jl_bounds_error_ints(jl_value_t *v, size_t *idxs, size_t nidxs);
...
// front end interface
JL_DLLEXPORT jl_value_t *jl_parse_input_line(
        const char *str, size_t len, const char *filename, size_t filename_len);
JL_DLLEXPORT jl_value_t *
jl_parse_string(const char *str, size_t len, int pos0, int greedy);
JL_DLLEXPORT int jl_parse_depwarn(int warn);
JL_DLLEXPORT jl_value_t *jl_load_file_string(
        const char *text, size_t len, char *filename, jl_module_t *inmodule);
...
typedef void *jl_uv_libhandle; // compatible with dlopen (void*) / LoadLibrary
                               // (HMODULE)
JL_DLLEXPORT jl_uv_libhandle
jl_load_dynamic_library(const char *fname, unsigned flags);
JL_DLLEXPORT jl_uv_libhandle
jl_load_dynamic_library_e(const char *fname, unsigned flags);
...
JL_DLLEXPORT jl_code_info_t *
jl_uncompress_ast(jl_method_t *m, jl_array_t *data);
...
#ifdef _OS_WINDOWS_
#define JL_CATCH                                                               \
    else for (i__ca = 1, jl_eh_restore_state(&__eh); i__ca; i__ca = 0) if (    \
...
#else
#define JL_CATCH                                                               \
    else for (i__ca = 1, jl_eh_restore_state(&__eh); i__ca; i__ca = 0)
#endif
...
// julia options -----------------------------------------------------------
// NOTE: This struct needs to be kept in sync with JLOptions type in
// base/options.jl
typedef struct {
    int8_t quiet;
...
typedef struct {
    ...
    // module setup: prepare a module for code emission (data layout, DWARF
    // version, ...)
...
typedef struct {
    ...
    int dynamic_alloc;     // is the compiler allowed to allocate dynamically
                           // (requires runtime)?
...
#define JULIA_DEFINE_FAST_TLS()                                                \
    ...
    {                                                                          \
        static __attribute__((                                                 \
                tls_model("local-exec"))) __thread jl_tls_states_t tls_states; \
        return &tls_states;                                                    \
    }                                                                          \
    __attribute__((constructor)) void jl_register_ptls_states_getter(void)     \
    {                                                                          \
        /* We need to make sure this function is called before any reference   \
         * to */                                                               \
        /* TLS variables. */                                                   \
