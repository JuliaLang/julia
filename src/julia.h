...
STATIC_INLINE int jl_array_ndimwords(uint32_t ndims) { return (ndims < 3 ? 0 : ndims - 2); }
...
typedef jl_value_t *(*jl_fptr_linfo_t)(struct _jl_lambda_info_t *, jl_value_t **, uint32_t,
                                       jl_svec_t *);
...
typedef struct _jl_method_t {
    ...
    jl_tupletype_t *sig;
    // bound type variables (static parameters). redundant with
    // TypeMapEntry->tvars
    jl_svec_t *tvars;
    ...
    int8_t  isstaged;
    // if there are intrinsic calls, sparams are probably required to compile
    // successfully,
    // and so unspecialized will be created for each linfo instead of using
    // linfo->def->template
...
typedef struct _jl_lambda_info_t {
    ...
    struct _jl_lambda_info_t *unspecialized_ducttape; // if template can't be
                                                      // compiled due to
                                                      // intrinsics, an
                                                      // un-inferred executable
                                                      // copy may get stored here
    jl_method_t *def;     // method this is specialized from, (null if this is a
                          // toplevel thunk)
...
STATIC_INLINE int jl_is_bitstype(void *v)
{
    return (jl_is_datatype(v) && jl_is_immutable(v) && ((jl_datatype_t *)(v))->layout &&
            jl_datatype_nfields(v) == 0 && ((jl_datatype_t *)(v))->size > 0);
}

STATIC_INLINE int jl_is_structtype(void *v)
{
    return (jl_is_datatype(v) &&
            (jl_field_count(v) > 0 || ((jl_datatype_t *)(v))->size == 0) &&
...
{
    return (!d->abstract && d->size == 0 && d != jl_sym_type &&
...
{
    return (jl_is_datatype(t) && ((jl_datatype_t *)(t))->name == jl_ref_type->name);
}
...
STATIC_INLINE jl_value_t *jl_is_ref_type(jl_value_t *t)
{
    ...
    while (dt != jl_any_type && dt->name != dt->super->name) {
        if (dt->name == jl_ref_type->name) return (jl_value_t *)dt;
        dt = dt->super;
    }
    return 0;
}
...
{
    return (jl_is_datatype(v) && ((jl_datatype_t *)(v))->name == jl_type_type->name);
}
...
JL_DLLEXPORT jl_method_t *jl_new_method(jl_lambda_info_t *definition, jl_sym_t *name,
                                        jl_tupletype_t *sig, jl_svec_t *tvars,
                                        int isstaged);
...
{
    return (jl_is_datatype(v) && ((jl_datatype_t *)(v))->name == jl_vararg_type->name);
}

STATIC_INLINE jl_vararg_kind_t jl_vararg_kind(jl_value_t *v)
{
    if (!jl_is_vararg_type(v)) return JL_VARARG_NONE;
    jl_value_t *lenv = jl_tparam1(v);
    if (jl_is_long(lenv)) return JL_VARARG_INT;
...
JL_DLLEXPORT jl_value_t *jl_eqtable_get(jl_array_t *h, void *key, jl_value_t *deflt);
...
STATIC_INLINE int jl_vinfo_sa(uint8_t vi) { return (vi & 16) != 0; }

STATIC_INLINE int jl_vinfo_usedundef(uint8_t vi) { return (vi & 32) != 0; }
...
static inline void jl_lock_frame_push(jl_mutex_t *lock)
{
    ...
    // For early bootstrap
    if (__unlikely(!ptls->current_task)) return;
...
#else
static inline void jl_lock_frame_push(jl_mutex_t *lock) { (void)lock; }
static inline void                                jl_lock_frame_pop(void) {}
...
#define JL_CATCH                                                                           \
              ...
              i__ca = 0) if (((jl_get_ptls_states()->exception_in_transit ==               \
                               jl_stackovf_exception) &&                                   \
                              _resetstkoflw()) ||                                          \
                             1)
#else
#define JL_CATCH else for (i__ca = 1, jl_eh_restore_state(&__eh); i__ca; i__ca = 0)
#endif
...
JL_DLLEXPORT void jl_safe_printf(const char *str, ...) _JL_FORMAT_ATTR(printf, 1, 2);
