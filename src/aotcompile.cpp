            ...
            if (it != compiled_functions.end()) {
                ...
                if (proto.private_linkage) {
                    it->second.TSM.withModuleDo([&](Module &M) {
                        cast<Function>(M.getNamedValue(decls.specFunctionObject))->addFnAttr(Attribute::InlineHint);
                    });
                }
            }
        ...
        if (preal_decl.empty()) {
            ...
            if (!proto.specsig) {
                ...
                if (proto.decl->hasFnAttribute(Attribute::InlineHint))
                    pinvoke->addFnAttr(Attribute::InlineHint);
        ...
        if (!preal_decl.empty()) {
            ...
            if (Function *specfun = cast<Function>(mod->getNamedValue(preal_decl))) {
                if (proto.decl != specfun) {
                    ...
                    if (proto.decl->hasFnAttribute(Attribute::InlineHint))
                        specfun->addFnAttr(Attribute::InlineHint);
                    proto.decl->eraseFromParent();
                    proto.decl = specfun;
                }
            }
