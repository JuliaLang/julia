...
// {
//     jl_element_type_layout_t flags = jl_element_type_layout(jl_buffer_eltype(b));
//     assert(flags.union_max > 1); // is bits union
//     return ((char*)jl_buffer_data(b)) + (jl_buffer_len(b) * flags.elsize);
// }
...
JL_DLLEXPORT jl_buffer_t *jl_new_buffer(jl_value_t *btype, size_t len)
{
    ...
    else if (flags.union_max == 1 && flags.elsize == 1) {
    ...
    if (tot <= ARRAY_INLINE_NBYTES) {
        ...
        data = (char*)b + doffs + sizeof(size_t);
    }
    else {
...
JL_DLLEXPORT jl_value_t *jl_bufref(jl_buffer_t *b, size_t i)
{
    ...
    jl_element_type_layout_t flags = jl_element_type_layout(ety);
    char *data = (char*)(jl_buffer_data(b));
    if (flags.union_max != 0 && jl_is_datatype(ety) && jl_datatype_size(ety) == 0) {
        return ((jl_datatype_t*)ety)->instance;
    }
    else if (flags.union_max != 0) {
        if (jl_is_uniontype(ety)) {
            // isbits union selector bytes are always stored directly after the last array element
            uint8_t sel = ((uint8_t*)data + (len * flags.elsize))[i];
            ety = jl_nth_union_component(ety, sel);
            if (jl_is_datatype_singleton((jl_datatype_t*)ety))
                return ((jl_datatype_t*)ety)->instance;
        }
        jl_value_t *r = undefref_check((jl_datatype_t*)ety, jl_new_bits(ety, data + (i * flags.elsize)));
...
JL_DLLEXPORT int jl_buffer_isassigned(jl_buffer_t *b, size_t i)
{
    ...
    if (flags.union_max == 0) {
        ...
        jl_datatype_t *elty = (jl_datatype_t*)eltype;
         ...
         jl_value_t **elem = (jl_value_t**)(jl_buffer_data(b) + i * flags.elsize);
