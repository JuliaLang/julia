...
static jl_value_t *
jl_vexceptionf(jl_datatype_t *exception_type, const char *fmt, va_list args)
{
    ...
    if (ok < 0) { // vasprintf failed
        msg = jl_cstr_to_string(
                "internal error: could not display error message");
    }
    else {
...
jl_exceptionf(jl_datatype_t *exception_type, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
...
JL_DLLEXPORT void JL_NORETURN jl_too_few_args(const char *fname, int min)
{
    jl_exceptionf(
            ...
            "%s: too few arguments (expected %d)",
            fname,
            min);
}
...
JL_DLLEXPORT void JL_NORETURN jl_too_many_args(const char *fname, int max)
{
    jl_exceptionf(
            jl_argumenterror_type,
            "%s: too many arguments (expected %d)",
            fname,
            max);
}

JL_DLLEXPORT void JL_NORETURN jl_type_error_rt(
        const char *fname, const char *context, jl_value_t *ty, jl_value_t *got)
{
    jl_value_t *ctxt = NULL;
    JL_GC_PUSH2(&ctxt, &got);
    ctxt = jl_pchar_to_string((char *)context, strlen(context));
    jl_value_t *ex =
            jl_new_struct(jl_typeerror_type, jl_symbol(fname), ctxt, ty, got);
    jl_throw(ex);
}
...
JL_DLLEXPORT void JL_NORETURN
jl_bounds_error_unboxed_int(void *data, jl_value_t *vt, size_t i)
{
...
{
    jl_datatype_t *eof_error = (jl_datatype_t *)jl_get_global(
            jl_base_module, jl_symbol("EOFError"));
...
JL_DLLEXPORT jl_value_t *jl_get_keyword_sorter(jl_value_t *f)
{
    ...
    if (mt->kwsorter == NULL)
        jl_errorf(
                "function %s does not accept keyword arguments",
        ...
        jl_value_t **args, uint32_t nargs, int drop_exceptions)
{
    ...
    JL_CATCH {
        if (!drop_exceptions) {
            jl_printf(JL_STDERR, "Internal error: encountered unexpected error "
                                 "in runtime:\n");
...
JL_DLLEXPORT jl_nullable_float64_t
jl_try_substrtod(char *str, size_t offset, size_t len)
{
    char *p;
...
JL_DLLEXPORT jl_nullable_float32_t
jl_try_substrtof(char *str, size_t offset, size_t len)
{
    char *p;
    char *bstr = str + offset;
    char *pend = bstr + len;
    ...
    if (errno == ERANGE &&
        (out == 0 || out == HUGE_VALF || out == -HUGE_VALF)) {
        hasvalue = 0;
    }
    else if (p == bstr) {
        hasvalue = 0;
...
static size_t
jl_static_show_x(JL_STREAM *out, jl_value_t *v, struct recur_list *depth);
    ...
    if ((uintptr_t)vt < 4096U) {
        ...
        if (jl_is_method(li->def.method)) {
            ...
            if (li->specTypes) {
                ...
                n += jl_show_svec(
                        out,
                        ((jl_datatype_t *)jl_unwrap_unionall(li->specTypes))
                                ->parameters,
        ...
        while (jl_is_uniontype(v)) {
            // tail-recurse on b to flatten the printing of the Union structure
            // in the common case
        ...
        // show type-var bounds only if they aren't going to be printed by
        // UnionAll later
        ...
        if (showbounds &&
            (ub != (jl_value_t *)jl_any_type || lb != jl_bottom_type)) {
            // show type-var upper bound if it is defined, or if we showed the
            // lower bound
        ...
        if (av->flags.ptrarray) {
            // print arrays with newlines, unless the elements are probably
            // small
            for (j = 0; j < tlen; j++) {
            ...
            for (; i < tlen; i++) {
                if (!istuple) {
                    n += jl_printf(
                            out,
                            "%s",
                            jl_symbol_name((jl_sym_t *)jl_svecref(
                                    vt->name->names, i)));
                    n += jl_printf(out, "=");
                }
...
JL_DLLEXPORT size_t jl_static_show_func_sig(JL_STREAM *s, jl_value_t *type)
{
    ...
    size_t n = 0;
    if (jl_nparams(ftype) == 0 ||
        ftype == ((jl_datatype_t *)ftype)->name->wrapper) {
        n += jl_printf(
                s,
                "%s",
                jl_symbol_name(((jl_datatype_t *)ftype)->name->mt->name));
    }
    else {
        n += jl_printf(s, "(::");
...
void jl_depwarn_partial_indexing(size_t n)
{
    static jl_value_t *depwarn_func = NULL;
    if (!depwarn_func && jl_base_module) {
        depwarn_func = jl_get_global(
                jl_base_module, jl_symbol("partial_linear_indexing_warning"));
    }
    if (!depwarn_func) {
        jl_safe_printf(
                "WARNING: Partial linear indexing is deprecated. Use "
                "`reshape(A, Val{%zd})` to make the dimensionality of the "
                "array match "
