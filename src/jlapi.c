                                     ...
                                     const char *image_relative_path)
{
    ...
    if (image_relative_path != NULL) jl_options.image_file = image_relative_path;
...
JL_DLLEXPORT jl_value_t *jl_eval_string(const char *str)
{
    ...
    JL_TRY {
        ...
        jl_value_t *ast = jl_parse_input_line(str, strlen(str), filename, strlen(filename));
        JL_GC_PUSH1(&ast);
...
JL_DLLEXPORT jl_value_t *jl_exception_occurred(void)
{
    ...
    return ptls->exception_in_transit == jl_nothing ? NULL : ptls->exception_in_transit;
}
...
{
    if (!jl_is_datatype(v)) return NULL;
...
JL_DLLEXPORT void *jl_array_eltype(jl_value_t *a) { return jl_tparam0(jl_typeof(a)); }
...
JL_DLLEXPORT const char *jl_string_ptr(jl_value_t *s) { return jl_string_data(s); }
...
JL_DLLEXPORT jl_value_t *jl_call(jl_function_t *f, jl_value_t **args, int32_t nargs)
{
    ...
    JL_TRY {
        ...
        argv[0] = (jl_value_t *)f;
        for (int i = 1; i < nargs + 1; i++) argv[i] = args[i - 1];
...
JL_DLLEXPORT void jl_yield(void)
{
    ...
    if (yieldfunc != NULL) jl_call0(yieldfunc);
}
...
JL_DLLEXPORT void jl_sigatomic_begin(void) { JL_SIGATOMIC_BEGIN(); }

JL_DLLEXPORT void jl_sigatomic_end(void)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (ptls->defer_signal == 0) jl_error("sigatomic_end called in non-sigatomic region");
    JL_SIGATOMIC_END();
}
...
JL_DLLEXPORT int jl_ver_major(void) { return JULIA_VERSION_MAJOR; }

JL_DLLEXPORT int jl_ver_minor(void) { return JULIA_VERSION_MINOR; }

JL_DLLEXPORT int jl_ver_patch(void) { return JULIA_VERSION_PATCH; }

JL_DLLEXPORT int jl_ver_is_release(void) { return JULIA_VERSION_IS_RELEASE; }

JL_DLLEXPORT const char *jl_ver_string(void) { return JULIA_VERSION_STRING; }
...
JL_DLLEXPORT jl_value_t *(jl_valueof)(jl_taggedvalue_t *v) { return jl_valueof(v); }

JL_DLLEXPORT jl_value_t *(jl_typeof)(jl_value_t *v) { return jl_typeof(v); }
...
JL_DLLEXPORT void(jl_cpu_pause)(void) { jl_cpu_pause(); }

JL_DLLEXPORT void(jl_cpu_wake)(void) { jl_cpu_wake(); }
