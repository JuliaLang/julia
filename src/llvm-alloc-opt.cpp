...
void Optimizer::moveToStack(CallInst *orig_inst, size_t sz, bool has_ref)
{
    ...
    auto new_inst = cast<Instruction>(prolog_builder.CreateBitCast(ptr, JuliaType::get_pjlvalue_ty(prolog_builder.getContext(), buff->getType()->getPointerAddressSpace())));
...
bool AllocOpt::doInitialization(Module &M)
{
    ...
    lifetime_start = Intrinsic::getDeclaration(&M, Intrinsic::lifetime_start, { Type::getInt8PtrTy(M.getContext(), M.getDataLayout().getAllocaAddrSpace()) });
    lifetime_end = Intrinsic::getDeclaration(&M, Intrinsic::lifetime_end, { Type::getInt8PtrTy(M.getContext(), M.getDataLayout().getAllocaAddrSpace()) });
