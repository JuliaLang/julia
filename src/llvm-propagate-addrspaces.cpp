...
public:
    ...
    Value *LiftPointer(
            Value *V, Type *LocTy = nullptr, Instruction *InsertPt = nullptr);
...
Value *PropagateJuliaAddrspaces::LiftPointer(
        Value *V, Type *LocTy, Instruction *InsertPt)
{
    ...
    for (Value *V : llvm::reverse(Stack)) {
        ...
        Type *NewRetTy = cast<PointerType>(GEP->getType())
                                 ->getElementType()
                                 ->getPointerTo(getValueAddrSpace(CurrentV));
        ...
        NewGEP->setOperand(
                GetElementPtrInst::getPointerOperandIndex(), CurrentV);
        LiftingMap[GEP] = NewGEP;
        CurrentV = NewGEP;
    }
    if (LocTy &&
        cast<PointerType>(CurrentV->getType())->getElementType() != LocTy) {
...
void PropagateJuliaAddrspaces::visitStoreInst(StoreInst &SI)
{
    ...
    if (!isSpecialAS(AS))
        return;
    Value *Replacement = LiftPointer(
            SI.getPointerOperand(), SI.getValueOperand()->getType(), &SI);
    if (!Replacement)
        return;
...
void PropagateJuliaAddrspaces::visitMemTransferInst(MemTransferInst &MTI)
{
    ...
    if (isSpecialAS(DestAS)) {
        Value *Replacement = LiftPointer(
                Dest,
                cast<PointerType>(Dest->getType())->getElementType(),
                &MTI);
        if (Replacement)
            Dest = Replacement;
    }
    ...
    if (isSpecialAS(SrcAS)) {
        Value *Replacement = LiftPointer(
                Src, cast<PointerType>(Src->getType())->getElementType(), &MTI);
        if (Replacement)
            Src = Replacement;
    }
    ...
    Value *TheFn = Intrinsic::getDeclaration(
            MTI.getModule(),
            MTI.getIntrinsicID(),
            { Dest->getType(), Src->getType(), MTI.getOperand(2)->getType() });
...
static RegisterPass<PropagateJuliaAddrspaces>
        X("PropagateJuliaAddrspaces",
          "Propagate (non-)rootedness information",
          false,
          false);
