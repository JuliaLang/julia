...
;; "Clean up" lambda expressions generated by the first lowering pass so that
;; they can be translated by the julia code.
(define (cleanup-lambdas e)
  (if (not (pair? e))
    e
    (let ((e2 (map cleanup-lambdas e)))
      (if (eq? (car e) 'lambda)
        `(lambda (vect ,@(cadr e2)) (vect ,@(caddr e2)) ,(cadddr e))
        e2))))

;; Lowering testing: call first pass only
(define (jl-expand-forms e)
  (error-wrap (lambda () (cleanup-lambdas (expand-forms e)))))
