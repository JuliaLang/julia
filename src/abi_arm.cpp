...
#define jl_is_floattype(v) jl_subtype(v, (jl_value_t *)jl_floatingpoint_type)
    ...
    // Check whether a type contained by a candidate homogeneous aggregate is
    // valid
    // fundamental type.
    //
    ...
    size_t isLegalHA(jl_datatype_t *dt, Type *&base) const
    {
        ...
        if (jl_is_structtype(dt)) {
            ...
            if (jl_datatype_size(dt) > 64 || dt->layout->npointers ||
                dt->layout->haspadding)
                return 0;
            ...
            bool *need_rewrite) const
    {
        ...
        //   Any result whose type would satisfy the conditions for a VFP CPRC
        //   is
        ...
        // - A double-word sized Fundamental Data Type (e.g., long long, double
        // and
        //   64-bit containerized vectors) is returned in r0 and r1.
        // - A word-sized Fundamental Data Type (eg., int, float) is returned in
        // r0.
        // NOTE: assuming "fundamental type" == jl_is_primitivetype, might need
        // exact def
        ...
        // NOTE: we don't check for jl_is_structtype below, because at this
        // point
        ...
        *need_rewrite = true;
        ...
        // format
        //   is as if the result had been stored in memory at a word-aligned
        //   address
        //   and then loaded into r0 with an LDR instruction. Any bits in r0
        //   that
        ...
        //   (§5.5, rule A.4). The memory to be used for the result may be
        //   modified
    ...
    // If the argument should be passed in general purpose registers, we may
    // need
