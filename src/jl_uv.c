...
void jl_wake_libuv(void) { uv_async_send(&signal_async); }
...
static void jl_uv_call_close_callback(jl_value_t *val)
{
    jl_value_t *args[2];
    args[0] =
        jl_get_global(jl_base_relative_to(((jl_datatype_t *)jl_typeof(val))->name->module),
...
JL_DLLEXPORT void jl_uv_closeHandle(uv_handle_t *handle)
{
    ...
    if (handle == (uv_handle_t *)JL_STDIN) JL_STDIN   = (JL_STREAM *)STDIN_FILENO;
    if (handle == (uv_handle_t *)JL_STDOUT) JL_STDOUT = (JL_STREAM *)STDOUT_FILENO;
    if (handle == (uv_handle_t *)JL_STDERR) JL_STDERR = (JL_STREAM *)STDERR_FILENO;
    // also let the client app do its own cleanup
    if (handle->type != UV_FILE && handle->data)
        jl_uv_call_close_callback((jl_value_t *)handle->data);
    if (handle == (uv_handle_t *)&signal_async) return;
    free(handle);
}
...
JL_DLLEXPORT int jl_init_pipe(uv_pipe_t *pipe, int writable, int readable, int julia_only)
{
    int flags = 0;
    if (writable) flags |= UV_PIPE_WRITABLE;
    if (readable) flags |= UV_PIPE_READABLE;
    if (!julia_only) flags |= UV_PIPE_SPAWN_SAFE;
...
JL_DLLEXPORT void jl_close_uv(uv_handle_t *handle)
{
    if (handle->type == UV_FILE) {
        ...
        jl_uv_closeHandle(handle); // synchronous (ok since the callback is known to
                                   // not interact with any global state)
        return;
    }
    ...
    if (handle->type == UV_NAMED_PIPE || handle->type == UV_TCP) {
        ...
        if (uv_is_writable((uv_stream_t *)handle)) {
            // attempt graceful shutdown of writable streams to give them a chance to
            // flush first
             ...
             *    in the process of closing this stream (since there's no other call
             * to
             *    uv_shutdown).
    ...
    if (!uv_is_closing((uv_handle_t *)handle)) {
        ...
        if (handle->type == UV_TTY) uv_tty_set_mode((uv_tty_t *)handle, UV_TTY_MODE_NORMAL);
...
JL_DLLEXPORT void jl_uv_associate_julia_struct(uv_handle_t *handle, jl_value_t *data)
{
    handle->data = data;
}
...
JL_DLLEXPORT int jl_spawn(char *name, char **argv, uv_loop_t *loop, uv_process_t *proc,
                          jl_value_t *julia_struct, uv_handle_type stdin_type,
                          uv_pipe_t *stdin_pipe, uv_handle_type stdout_type,
                          uv_pipe_t *stdout_pipe, uv_handle_type stderr_type,
                          uv_pipe_t *stderr_pipe, int flags, char **env, char *cwd,
                          uv_exit_cb cb)
{
...
JL_DLLEXPORT uv_loop_t *jl_global_event_loop(void) { return jl_io_loop; }
...
JL_DLLEXPORT int jl_fs_sendfile(int src_fd, int dst_fd, int64_t in_offset, size_t len)
{
    uv_fs_t req;
    JL_SIGATOMIC_BEGIN();
    int ret = uv_fs_sendfile(jl_io_loop, &req, dst_fd, src_fd, in_offset, len, NULL);
    uv_fs_req_cleanup(&req);
    JL_SIGATOMIC_END();
    return ret;
}
...
JL_DLLEXPORT int jl_fs_write(int handle, const char *data, size_t len, int64_t offset)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (ptls->safe_restore) return write(handle, data, len);
    uv_fs_t  req;
    uv_buf_t buf[1];
...
JL_DLLEXPORT int jl_tcp_getsockname(uv_tcp_t *handle, uint16_t *port, void *host,
                                    uint32_t *family)
{
    int                     namelen;
...
JL_DLLEXPORT int jl_tcp_getpeername(uv_tcp_t *handle, uint16_t *port, void *host,
                                    uint32_t *family)
{
    int                     namelen;
    struct sockaddr_storage addr;
...
JL_DLLEXPORT int jl_udp_bind(uv_udp_t *handle, uint16_t port, uint32_t host, uint32_t flags)
{
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(struct sockaddr_in));
    addr.sin_port        = port;
...
JL_DLLEXPORT int jl_udp_bind6(uv_udp_t *handle, uint16_t port, void *host, uint32_t flags)
{
    struct sockaddr_in6 addr;
    memset(&addr, 0, sizeof(struct sockaddr_in6));
    addr.sin6_port = port;
...
JL_DLLEXPORT int jl_udp_send(uv_udp_t *handle, uint16_t port, uint32_t host, void *data,
                             uint32_t size, uv_udp_send_cb cb)
{
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(struct sockaddr_in));
    addr.sin_port        = port;
...
JL_DLLEXPORT int jl_udp_send6(uv_udp_t *handle, uint16_t port, void *host, void *data,
                              uint32_t size, uv_udp_send_cb cb)
{
    struct sockaddr_in6 addr;
    memset(&addr, 0, sizeof(struct sockaddr_in6));
...
JL_DLLEXPORT int jl_getaddrinfo(uv_loop_t *loop, const char *host, const char *service,
                                jl_function_t *cb, uv_getaddrinfo_cb uvcb)
{
...
JL_DLLEXPORT void jl_sockaddr_set_port(struct sockaddr_storage *addr, uint16_t port)
{
    if (addr->ss_family == AF_INET)
...
JL_DLLEXPORT int jl_uv_unix_fd_is_watched(int fd, uv_poll_t *handle, uv_loop_t *loop)
{
    if (fd >= loop->nwatchers) return 0;
    if (loop->watchers[fd] == NULL) return 0;
    if (handle && loop->watchers[fd] == &handle->io_watcher) return 0;
    return 1;
}
...
{
#ifdef _OS_WINDOWS_
    if (jl_ispty((uv_pipe_t *)handle)) return UV_TTY;
#endif
    return handle->type;
}
...
JL_DLLEXPORT int jl_tty_set_mode(uv_tty_t *handle, int mode)
{
    ...
    if (mode) mode_enum     = UV_TTY_MODE_RAW;
...
JL_DLLEXPORT int jl_uv_handle(uv_stream_t *handle) { return uv__stream_fd(handle); }
#else
JL_DLLEXPORT HANDLE jl_uv_handle(uv_stream_t *handle)
{
    switch (handle->type) {
    case UV_TTY: return ((uv_tty_t *)handle)->handle;
    case UV_NAMED_PIPE: return ((uv_pipe_t *)handle)->handle;
