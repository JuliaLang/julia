...
JL_DLLEXPORT void jl_close_uv(uv_handle_t *handle)
{
    if (handle->type == UV_FILE) {
        ...
        jl_uv_closeHandle(handle); // synchronous (ok since the callback is known to not
                                   // interact with any global state)
        return;
    }
    ...
    if (handle->type == UV_NAMED_PIPE || handle->type == UV_TCP) {
        ...
        if (uv_is_writable((uv_stream_t *)handle)) {
            // attempt graceful shutdown of writable streams to give them a chance to flush
            // first
