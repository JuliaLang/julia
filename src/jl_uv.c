...
{
    jl_value_t *args[2];
    args[0] = jl_get_global(
            jl_base_relative_to(
                    ((jl_datatype_t *)jl_typeof(val))->name->module),
...
// getters and setters
JL_DLLEXPORT void *jl_uv_process_data(uv_process_t *p)
{
    return p->data;
}
JL_DLLEXPORT void *jl_uv_buf_base(const uv_buf_t *buf)
{
    return buf->base;
}
JL_DLLEXPORT size_t jl_uv_buf_len(const uv_buf_t *buf)
{
    return buf->len;
}
JL_DLLEXPORT void jl_uv_buf_set_base(uv_buf_t *buf, char *b)
{
    buf->base = b;
}
JL_DLLEXPORT void jl_uv_buf_set_len(uv_buf_t *buf, size_t n)
{
    buf->len = n;
}
JL_DLLEXPORT void *jl_uv_connect_handle(uv_connect_t *connect)
{
    return connect->handle;
}
JL_DLLEXPORT void *jl_uv_getaddrinfo_data(uv_getaddrinfo_t *req)
{
    return req->data;
}
JL_DLLEXPORT uv_file jl_uv_file_handle(jl_uv_file_t *f)
{
    return f->file;
}
JL_DLLEXPORT void *jl_uv_req_data(uv_req_t *req)
{
    return req->data;
}
JL_DLLEXPORT void jl_uv_req_set_data(uv_req_t *req, void *data)
{
    req->data = data;
}
JL_DLLEXPORT void *jl_uv_handle_data(uv_handle_t *handle)
{
    return handle->data;
}
JL_DLLEXPORT void *jl_uv_write_handle(uv_write_t *req)
{
    return req->handle;
}
...
JL_DLLEXPORT int
jl_init_pipe(uv_pipe_t *pipe, int writable, int readable, int julia_only)
{
    int flags = 0;
    if (writable)
...
static void jl_proc_exit_cleanup(
...
JL_DLLEXPORT void jl_close_uv(uv_handle_t *handle)
{
    if (handle->type == UV_PROCESS && ((uv_process_t *)handle)->pid != 0) {
        ...
        assert(handle->data ==
               NULL); // make sure Julia has forgotten about it already
    ...
    if (handle->type == UV_FILE) {
        ...
        jl_uv_closeHandle(handle); // synchronous (ok since the callback is
                                   // known to not interact with any global
                                   // state)
        return;
    }
    ...
    if (handle->type == UV_NAMED_PIPE || handle->type == UV_TCP) {
        ...
        if (uv_is_writable((uv_stream_t *)handle)) {
            // attempt graceful shutdown of writable streams to give them a
            // chance to flush first
            uv_shutdown_t *req = (uv_shutdown_t *)malloc(sizeof(uv_shutdown_t));
            req->data = 0;
            /*
             * We are explicitly ignoring the error here for the following
             * reason:
             * There is only two scenarios in which this returns an error:
             * a) In case the stream is already shut down, in which case we're
             * likely
             *    in the process of closing this stream (since there's no other
             * call to
             *    uv_shutdown).
             * b) In case the stream is already closed, in which case uv_close
             * would
...
JL_DLLEXPORT void
jl_uv_associate_julia_struct(uv_handle_t *handle, jl_value_t *data)
{
    handle->data = data;
}
...
JL_DLLEXPORT int jl_spawn(
        char *name,
        char **argv,
        uv_loop_t *loop,
        uv_process_t *proc,
        jl_value_t *julia_struct,
        uv_handle_type stdin_type,
        uv_pipe_t *stdin_pipe,
        uv_handle_type stdout_type,
        uv_pipe_t *stdout_pipe,
        uv_handle_type stderr_type,
...
JL_DLLEXPORT int
jl_fs_sendfile(int src_fd, int dst_fd, int64_t in_offset, size_t len)
{
    uv_fs_t req;
    JL_SIGATOMIC_BEGIN();
    int ret = uv_fs_sendfile(
            jl_io_loop, &req, dst_fd, src_fd, in_offset, len, NULL);
    uv_fs_req_cleanup(&req);
    JL_SIGATOMIC_END();
    return ret;
}
...
JL_DLLEXPORT int
jl_fs_write(int handle, const char *data, size_t len, int64_t offset)
{
    jl_ptls_t ptls = jl_get_ptls_states();
...
JL_DLLEXPORT void jl_uv_puts(uv_stream_t *stream, const char *str, size_t n)
{
    assert(stream);
    static_assert(
            offsetof(uv_stream_t, type) == offsetof(ios_t, bm) &&
                    sizeof(((uv_stream_t *)0)->type) ==
                            sizeof(((ios_t *)0)->bm),
...
// NOTE: These function expects port/host to be in network byte-order (Big
// Endian)
JL_DLLEXPORT int
jl_tcp_bind(uv_tcp_t *handle, uint16_t port, uint32_t host, unsigned int flags)
{
...
JL_DLLEXPORT int
jl_tcp_bind6(uv_tcp_t *handle, uint16_t port, void *host, unsigned int flags)
{
...
JL_DLLEXPORT int jl_tcp_getsockname(
        uv_tcp_t *handle, uint16_t *port, void *host, uint32_t *family)
{
    int namelen;
...
JL_DLLEXPORT int jl_tcp_getpeername(
        uv_tcp_t *handle, uint16_t *port, void *host, uint32_t *family)
{
    int namelen;
    struct sockaddr_storage addr;
...
JL_DLLEXPORT int
jl_udp_bind(uv_udp_t *handle, uint16_t port, uint32_t host, uint32_t flags)
{
    struct sockaddr_in addr;
    memset(&addr, 0, sizeof(struct sockaddr_in));
    addr.sin_port = port;
...
JL_DLLEXPORT int
jl_udp_bind6(uv_udp_t *handle, uint16_t port, void *host, uint32_t flags)
{
    struct sockaddr_in6 addr;
    memset(&addr, 0, sizeof(struct sockaddr_in6));
    addr.sin6_port = port;
...
JL_DLLEXPORT int
jl_uv_interface_addresses(uv_interface_address_t **ifAddrStruct, int *count)
{
    return uv_interface_addresses(ifAddrStruct, count);
}

JL_DLLEXPORT int
jl_uv_interface_address_is_internal(uv_interface_address_t *addr)
{
    return addr->is_internal;
}

JL_DLLEXPORT struct sockaddr_in *
jl_uv_interface_address_sockaddr(uv_interface_address_t *ifa)
{
    return &ifa->address.address4;
}

JL_DLLEXPORT int jl_getaddrinfo(
        uv_loop_t *loop,
        const char *host,
        const char *service,
        jl_function_t *cb,
...
JL_DLLEXPORT struct sockaddr *
jl_sockaddr_from_addrinfo(struct addrinfo *addrinfo)
{
...
JL_DLLEXPORT unsigned int
jl_sockaddr_host6(struct sockaddr_in6 *addr, char *host)
{
...
JL_DLLEXPORT void
jl_sockaddr_set_port(struct sockaddr_storage *addr, uint16_t port)
{
    if (addr->ss_family == AF_INET)
...
JL_DLLEXPORT int jl_tcp4_connect(
        uv_tcp_t *handle, uint32_t host, uint16_t port, uv_connect_cb cb)
{
    struct sockaddr_in addr;
...
JL_DLLEXPORT int
jl_tcp6_connect(uv_tcp_t *handle, void *host, uint16_t port, uv_connect_cb cb)
{
    struct sockaddr_in6 addr;
    uv_connect_t *req = (uv_connect_t *)malloc(sizeof(uv_connect_t));
    req->data = 0;
...
JL_DLLEXPORT int jl_connect_raw(
        uv_tcp_t *handle, struct sockaddr_storage *addr, uv_connect_cb cb)
{
    uv_connect_t *req = (uv_connect_t *)malloc(sizeof(uv_connect_t));
    req->data = 0;
    return uv_tcp_connect(req, handle, (struct sockaddr *)addr, cb);
...
JL_DLLEXPORT int
jl_uv_unix_fd_is_watched(int fd, uv_poll_t *handle, uv_loop_t *loop)
{
...
JL_DLLEXPORT int jl_ispty(uv_pipe_t *pipe)
{
    if (pipe->type != UV_NAMED_PIPE)
        return 0;
    size_t len = 0;
    if (uv_pipe_getpeername(pipe, NULL, &len) != UV_ENOBUFS)
        return 0;
...
JL_DLLEXPORT int jl_queue_work(
        ...
        int notify_idx)
{
    struct work_baton *baton =
            (struct work_baton *)malloc(sizeof(struct work_baton));
