...
/*
  ...
  - Type variables can be restricted to range over only concrete types.
    This is done by returning false if such a variable's lower bound is not
  concrete.
  - Diagonal rule: a type variable is concrete if it occurs more than once in
    covariant position, and never in invariant position. This sounds like a
  syntactic
...
typedef struct _varbinding {
    ...
    // here for reference in case that var is popped from the environment before
    // this one.
    ...
    int8_t concrete; // 1 if another variable has a constraint forcing this one
                     // to be concrete
    // in covariant position, we need to try constraining a variable in
    // different ways:
    // 0 - unconstrained
    // 1 - less than
    // 2 - greater than
    // 3 - inexpressible - occurs when the var has non-trivial overlap with
    // another type,
    //                     and we would need to return `intersect(var,other)`.
    //                     in this case
    //                     we choose to over-estimate the intersection by
    //                     returning the var.
    int8_t constraintkind;
    int depth0; // # of invariant constructors nested around the UnionAll type
                // for this var
...
typedef struct {
    ...
    jl_value_t **envout; // for passing caller the computed bounds of right-side
                         // variables
    ...
    int emptiness_only; // true iff intersection only needs to test for
                        // emptiness
} jl_stenv_t;
...
static void restore_env(jl_stenv_t *e, jl_value_t *root, jl_savedenv_t *se)
{
    ...
    if (e->envout && e->envidx < e->envsz)
        memset(&e->envout[e->envidx],
               0,
               (e->envsz - e->envidx) * sizeof(void *));
}
...
static int obviously_egal(jl_value_t *a, jl_value_t *b)
{
    ...
    if (jl_is_uniontype(a)) {
        return obviously_egal(
                       ((jl_uniontype_t *)a)->a, ((jl_uniontype_t *)b)->a) &&
               obviously_egal(
                       ((jl_uniontype_t *)a)->b, ((jl_uniontype_t *)b)->b);
    }
    if (jl_is_unionall(a)) {
        return ((jl_unionall_t *)a)->var == ((jl_unionall_t *)b)->var &&
               obviously_egal(
                       ((jl_unionall_t *)a)->body, ((jl_unionall_t *)b)->body);
    }
    if (jl_is_typevar(a))
        return 0;
...
{
    if (a == b || a == (jl_value_t *)jl_any_type ||
    ...
    if (jl_is_datatype(a) && jl_is_datatype(b)) {
        ...
        if (istuple) {
            ...
            if (jl_is_va_tuple(ad)) {
                ...
                // note: some disjoint types (e.g. tuples of different lengths)
        ...
        for (i = 0; i < np; i++) {
            ...
            if (jl_is_type(ai)) {
                if (jl_is_type(bi)) {
                    if (istuple &&
                        (ai == jl_bottom_type || bi == jl_bottom_type))
                        ; // TODO: this can return 1 if and when Tuple{Union{}}
                          // === Union{}
...
static int in_union(jl_value_t *u, jl_value_t *x)
{
    ...
    if (!jl_is_uniontype(u))
        return 0;
    return in_union(((jl_uniontype_t *)u)->a, x) ||
           in_union(((jl_uniontype_t *)u)->b, x);
}
...
static jl_value_t *pick_union_element(jl_value_t *u, jl_stenv_t *e, int8_t R)
{
    ...
    do {
        ...
        state->depth++;
        if (ui == 0) {
            state->more = state->depth; // memorize that this was the deepest
                                        // available choice
...
// compare the current component of `u` to `t`. `R==1` means `u` came from the
// right side.
static int subtype_union(
        jl_value_t *t, jl_uniontype_t *u, jl_stenv_t *e, int8_t R, int param)
{
...
static int subtype_ufirst(jl_value_t *x, jl_value_t *y, jl_stenv_t *e)
{
    ...
    if (jl_is_typevar(x) && jl_is_uniontype(y))
        return (x == ((jl_uniontype_t *)y)->a ||
                x == ((jl_uniontype_t *)y)->b ||
...
static jl_value_t *
intersect_ufirst(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int depth);
...
static int var_lt(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int param)
{
    ...
    if (jl_is_typevar(a)) {
        ...
        if (aa && !aa->right && in_union(bb->lb, a) &&
            bb->depth0 != aa->depth0 && var_outside(e, b, (jl_tvar_t *)a)) {
            // an "exists" var cannot equal a "forall" var inside it unless the
            // forall
...
static int var_gt(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int param)
{
    ...
    if (!((bb->ub == (jl_value_t *)jl_any_type && !jl_is_type(a) &&
           !jl_is_typevar(a)) ||
          subtype_ufirst(a, bb->ub, e)))
        return 0;
...
// compare UnionAll type `u` to `t`. `R==1` if `u` came from the right side of A
// <: B.
static int subtype_unionall(
        jl_value_t *t, jl_unionall_t *u, jl_stenv_t *e, int8_t R, int param)
{
    jl_varbinding_t *btemp = e->vars;
    // if the var for this unionall (based on identity) already appears
    // somewhere
    // in the environment, rename to get a fresh var.
    while (btemp != NULL) {
        if (btemp->var == u->var ||
            // outer var can only refer to inner var if bounds changed
            (btemp->lb != btemp->var->lb &&
             jl_has_typevar(btemp->lb, u->var)) ||
            (btemp->ub != btemp->var->ub &&
    ...
    jl_varbinding_t vb = { u->var, u->var->lb, u->var->ub,  R, NULL, 0,      0,
                           0,      0,          e->invdepth, 0, NULL, e->vars };
        ...
        if (e->envidx < e->envsz) {
            ...
            if (!vb.occurs_inv && vb.lb != jl_bottom_type)
                val = is_leaf_bound(vb.lb)
                              ? vb.lb
                              : (jl_value_t *)jl_new_typevar(
                                        u->var->name, jl_bottom_type, vb.lb);
            else if (vb.lb == vb.ub)
                val = vb.lb;
            ...
            // TODO: substitute the value (if any) of this variable into
            // previous envout entries
        }
    }
    else {
    ...
    // handle the "diagonal dispatch" rule, which says that a type var occurring
    // more
    // than once, and only in covariant position, is constrained to concrete
    // types. E.g.
    //  ( Tuple{Int, Int}    <: Tuple{T, T} where T) but
    // !( Tuple{Int, String} <: Tuple{T, T} where T)
    // Then check concreteness by checking that the lower bound is not an
    // abstract type.
    ...
    while (btemp != NULL) {
        ...
        if (vi != (jl_value_t *)vb.var && btemp->var->ub != vi &&
            jl_has_typevar(vi, vb.var)) {
...
// unwrap <=2 layers of UnionAlls, leaving the vars in *p1 and *p2 and returning
// the body
static jl_value_t *
unwrap_2_unionall(jl_value_t *t, jl_tvar_t **p1, jl_tvar_t **p2)
{
    if (jl_is_unionall(t)) {
...
static int
subtype_tuple(jl_datatype_t *xd, jl_datatype_t *yd, jl_stenv_t *e, int param)
{
    ...
    size_t i = 0, j = 0;
    int vx = 0, vy = 0,
        vvx = (lx > 0 && jl_is_vararg_type(jl_tparam(xd, lx - 1)));
        ...
        if (vx && vy) {
            ...
            if (jl_is_datatype(xi)) {
                ...
                if (jl_is_typevar(xl)) {
                    ...
                    if (xlv && jl_is_long(xlv->lb) &&
                        jl_unbox_long(xlv->lb) == 0)
                        break;
                }
            }
            if (jl_is_datatype(yi)) {
                ...
                if (jl_is_typevar(yl)) {
                    ...
                    if (ylv && jl_is_long(ylv->lb) &&
                        jl_unbox_long(ylv->lb) == 0)
                        break;
                }
            }
        }
        if (xi == lastx &&
            ((yi == lasty && !jl_has_free_typevars(xi) &&
              !jl_has_free_typevars(yi)) ||
             (yi == lasty && !vx && vy && jl_is_leaf_type(xi)))) {
            // fast path for repeated elements
        }
        else if (
                e->Runions.depth == 0 && e->Lunions.depth == 0 &&
                !jl_has_free_typevars(xi) && !jl_has_free_typevars(yi)) {
...
// (as opposed to being outside any type constructor, or comparing variable
// bounds).
...
static int subtype(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int param)
{
    ...
    if (jl_is_uniontype(y)) {
        ...
        int ui = 1;
        if (jl_is_typevar(x)) {
            // The `convert(Type{T},T)` pattern, where T is a Union, required
            // changing priority
            // of unions and vars: if matching `typevar <: union`, first try to
            // match the whole
            // union against the variable before trying to take it apart to see
            // if there are any
            ...
            state->depth++;
            if (ui == 0)
                state->more = state->depth; // memorize that this was the
                                            // deepest available choice
        }
        if (ui == 1)
    ...
    if (jl_is_datatype(x) && jl_is_datatype(y)) {
        ...
        if (jl_is_type_type(x) && !jl_is_type_type(y)) {
            ...
            if (!jl_is_typevar(tp0)) {
                // TODO this is not strictly correct, but we don't yet have any
                // other way for
                // e.g. the argument `Int` to match a `::DataType` slot. Most
                // correct would be:
                // Int isa DataType, Int isa Type{Int}, Type{Int} more specific
                // than DataType,
                // !(Type{Int} <: DataType), !isleaftype(Type{Int}), because
                // non-DataTypes can
        ...
        while (xd != jl_any_type && xd->name != yd->name) {
            if (xd->super == NULL)
                jl_errorf(
                        "circular type parameter constraint in definition of "
                        "%s",
...
jl_subtype_env(jl_value_t *x, jl_value_t *y, jl_value_t **env, int envsz)
{
    jl_stenv_t e;
    if (envsz == 0 &&
        (y == (jl_value_t *)jl_any_type || x == jl_bottom_type || x == y))
        return 1;
...
// returns true if the intersection of `t` and `Type` is non-empty and not a
// kind
// this is sufficient to determine if `isa(x, T)` can instead simply check for
// `typeof(x) <: T`
    ...
    if (jl_is_type(x)) {
        ...
        if (!jl_has_free_typevars(x)) {
            ...
            if (jl_has_intersect_type_not_kind(t2)) {
                JL_GC_PUSH1(&x);
                x = (jl_value_t *)jl_wrap_Type(
                        x); // TODO jb/subtype avoid jl_wrap_Type
...
static jl_value_t *
intersect(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int param);

static jl_value_t *intersect_union(
        jl_value_t *x, jl_uniontype_t *u, jl_stenv_t *e, int8_t R, int param)
{
    if (param == 2 ||
        (!jl_has_free_typevars(x) && !jl_has_free_typevars((jl_value_t *)u))) {
        jl_value_t *a = NULL, *b = NULL, *save = NULL;
        jl_savedenv_t se;
    ...
    // try all possible choices in covariant position; union them all together
    // at the top level
...
static jl_value_t *
set_var_to_const(jl_varbinding_t *bb, jl_value_t *v, jl_varbinding_t *othervar)
{
...
static int
var_occurs_inside(jl_value_t *v, jl_tvar_t *var, int inside, int want_inv);

static jl_value_t *
intersect_var(jl_tvar_t *b, jl_value_t *a, jl_stenv_t *e, int8_t R, int param)
{
    jl_varbinding_t *bb = lookup(e, b);
    if (bb == NULL)
        return R ? intersect_ufirst(a, b->ub, e, 0)
                 : intersect_ufirst(b->ub, a, e, 0);
    ...
    jl_savedenv_t se;
    if (param == 2) {
        jl_value_t *ub = R ? intersect_ufirst(a, bb->ub, e, d)
                           : intersect_ufirst(bb->ub, a, e, d);
        JL_GC_PUSH2(&ub, &root);
        save_env(e, &root, &se);
        ...
        if (ub != (jl_value_t *)b) {
            if (jl_has_free_typevars(ub)) {
                // constraint X == Ref{X} is unsatisfiable. also check variables
                // set equal to X.
                ...
                jl_varbinding_t *btemp = e->vars;
                while (btemp != NULL) {
                    if (btemp->lb == (jl_value_t *)b &&
                        btemp->ub == (jl_value_t *)b &&
        ...
        return R ? intersect_ufirst(a, bb->ub, e, d)
                 : intersect_ufirst(bb->ub, a, e, d);
    }
    ...
    if (ub == a) {
        if (bb->lb == jl_bottom_type ||
            // if the var has an equality constraint then make sure bounds stay
            // consistent.
            // TODO: try to use this check in more cases
            bb->ub != bb->lb ||
            try_subtype_in_env(bb->lb, ub, e)) {
            bb->ub = ub;
            return (jl_value_t *)b;
        }
        return ub;
    ...
    JL_GC_PUSH2(&root, &ub);
    save_env(e, &root, &se);
    jl_value_t *ii = R ? intersect_ufirst(a, bb->lb, e, d)
                       : intersect_ufirst(bb->lb, a, e, d);
...
// invariant constructors. `inside` means we are currently inside a constructor
// of the
// requested kind.
static int
var_occurs_inside(jl_value_t *v, jl_tvar_t *var, int inside, int want_inv)
{
    if (v == (jl_value_t *)var) {
        ...
        return var_occurs_inside(
                       ((jl_uniontype_t *)v)->a, var, inside, want_inv) ||
               var_occurs_inside(
                       ((jl_uniontype_t *)v)->b, var, inside, want_inv);
    }
...
static jl_value_t *
finish_unionall(jl_value_t *res, jl_varbinding_t *vb, jl_stenv_t *e)
{
        ...
        if (jl_has_typevar(btemp->lb, vb->var)) {
            ...
            if (varval) {
                ...
                btemp->lb = vb->lb;
                    ...
                    btemp->depth0 == vb->depth0 &&
                    !jl_has_typevar(vb->lb, btemp->var) &&
                    !jl_has_typevar(vb->ub, btemp->var) &&
                    jl_has_typevar(btemp->ub, vb->var)) {
                // if our variable is T, and some outer variable has constraint
                // S = Ref{T},
                // move the `where T` outside `where S` instead of putting it
                // here. issue #21243.
                if (btemp->innervars == NULL)
                    btemp->innervars = jl_alloc_array_1d(jl_array_any_type, 0);
                if (newvar != vb->var) {
                    btemp->lb = jl_substitute_var(
                            btemp->lb, vb->var, (jl_value_t *)newvar);
    ...
    // if `v` still occurs, re-wrap body in `UnionAll v` or eliminate the
    // UnionAll
    if (jl_has_typevar(res, vb->var)) {
        if (varval) {
            JL_TRY {
                // you can construct `T{x} where x` even if T's parameter is
                // actually
                // limited. in that case we might get an invalid instantiation
                // here.
...
static jl_value_t *intersect_unionall(
        jl_value_t *t, jl_unionall_t *u, jl_stenv_t *e, int8_t R, int param)
{
    ...
    if (res != jl_bottom_type) {
        if (vb.concrete || vb.occurs_inv > 1 ||
...
// check n = (length of vararg type v)
static int
intersect_vararg_length(jl_value_t *v, ssize_t n, jl_stenv_t *e, int8_t R)
{
    jl_tvar_t *va_p1 = NULL, *va_p2 = NULL;
    jl_value_t *tail = unwrap_2_unionall(v, &va_p1, &va_p2);
    assert(jl_is_datatype(tail));
...
static jl_value_t *
intersect_tuple(jl_datatype_t *xd, jl_datatype_t *yd, jl_stenv_t *e, int param)
{
    size_t lx = jl_nparams(xd), ly = jl_nparams(yd);
    if (lx == 0 && ly == 0)
    ...
    while (1) {
        ...
        if (xi == NULL || yi == NULL) {
            ...
            if (vx && intersect_vararg_length(xi, ly + 1 - lx, e, 0))
                res = (jl_value_t *)jl_apply_tuple_type_v(
                        jl_svec_data(params), j);
            if (vy && intersect_vararg_length(yi, lx + 1 - ly, e, 1))
                res = (jl_value_t *)jl_apply_tuple_type_v(
                        jl_svec_data(params), i);
            break;
        }
        if (vx && !vy)
        ...
        if (vx && vy) {
            // {A^n...,Vararg{T,N}} ∩ {Vararg{S,M}} = {(A∩S)^n...,Vararg{T∩S,N}}
            // plus N = M-n
        ...
        if (ii == jl_bottom_type) {
            if (vx && vy) {
                int len = i > j ? i : j;
                if ((xb && jl_is_long(xb->lb) &&
                     lx - 1 + jl_unbox_long(xb->lb) != len) ||
                    (yb && jl_is_long(yb->lb) &&
                     ly - 1 + jl_unbox_long(yb->lb) != len)) {
                    res = jl_bottom_type;
                }
                else if (
                        param == 2 &&
                        jl_is_unionall(xi) != jl_is_unionall(yi)) {
                    res = jl_bottom_type;
                }
                else {
                    if (xb)
...
static jl_value_t *intersect_sub_datatype(
        jl_datatype_t *xd, jl_datatype_t *yd, jl_stenv_t *e, int R, int param)
{
    jl_value_t *isuper =
    ...
    super_pattern = jl_rewrap_unionall(
            (jl_value_t *)((jl_datatype_t *)jl_unwrap_unionall(wrapper))->super,
            wrapper);
    ...
    jl_value_t *ii = jl_bottom_type;
    {
        ...
        if (subtype_in_env(isuper, super_pattern, &tempe)) {
            ...
            int i;
            for (i = 0; i < envsz; i++) {
                // if a parameter is not constrained by the supertype, use the
                // original
                // parameter value from `x`. this is detected by the value in
                // `env` being
                // the exact typevar from the type's `wrapper`, or a free
                // typevar.
                jl_value_t *ei = env[i];
...
static jl_value_t *
intersect_invariant(jl_value_t *x, jl_value_t *y, jl_stenv_t *e)
{
    ...
    if (jl_is_typevar(x) && jl_is_typevar(y) &&
        (jl_is_typevar(ii) || !jl_is_type(ii)))
        return ii;
    if (ii == jl_bottom_type) {
...
static jl_value_t *
intersect_type_type(jl_value_t *x, jl_value_t *y, jl_stenv_t *e, int8_t R)
{
    ...
    if (!jl_is_kind(y))
        return jl_bottom_type;
    if (y == (jl_value_t *)jl_typeofbottom_type &&
        ((jl_tvar_t *)p0)->lb == jl_bottom_type)
    ...
    // NOTE: we cannot express e.g. DataType ∩ (UnionAll T<:Integer Type{T}), so
    returning `x`
    ...
    if (jl_is_typevar(x)) {
        if (jl_is_typevar(y)) {
            ...
            if (xx && yy && var_outside(e, (jl_tvar_t *)x, (jl_tvar_t *)y)) {
                // to preserve variable identities correctly, always accumulate
                // bounds
                // on the outer variable, return the outer variable, and set the
                // inner
            ...
            if (param == 2) {
                ...
                if (xx && yy && xx->depth0 != yy->depth0) {
                    record_var_occurrence(yy, e, param);
                    return subtype_in_env(yy->ub, yy->lb, e) ? y
                                                             : jl_bottom_type;
                }
                if (xub == xlb && jl_is_typevar(xub)) {
                    if (y == xub) {
                ...
                if (!jl_is_type(xlb) && !jl_is_typevar(xlb)) {
                    ...
                    if (ylb == jl_bottom_type &&
                        yub == (jl_value_t *)jl_any_type)
                        return xlb;
                    return jl_bottom_type;
                }
                if (!(subtype_in_env(xlb, yub, e) &&
                      subtype_in_env(ylb, xub, e)))
                    return jl_bottom_type;
            ...
            if (jl_is_unionall(a)) {
                ...
                if (jl_is_unionall(ua->body)) {
                    ...
                    if (jl_has_typevar(ub->var->ub, ua->var) ||
                        ...
                        b = intersect_unionall(
                                x, (jl_unionall_t *)y, e, 1, param);
                    }
                }
            }
            free(se.buf);
        ...
        if (xd->name == yd->name) {
            ...
            if (jl_is_vararg_type(x)) {
                ...
                jl_value_t *xp1 = jl_tparam0(xd), *xp2 = jl_tparam1(xd),
                           *yp1 = jl_tparam0(yd), *yp2 = jl_tparam1(yd);
                // in Vararg{T1} <: Vararg{T2}, need to check subtype twice to
                // simulate the possibility of multiple arguments, which is
                // needed
                // to implement the diagonal rule correctly.
                if (intersect(xp1, yp1, e, param == 0 ? 1 : param) ==
                    jl_bottom_type)
                    return jl_bottom_type;
...
static jl_value_t *
intersect_types(jl_value_t *x, jl_value_t *y, int emptiness_only)
{
    jl_stenv_t e;
    ...
    if (jl_subtype_env(a, b, env, szb)) {
        ...
        *ans = intersect_all(a, b, &e);
        if (*ans == jl_bottom_type)
            goto bot;
        // TODO: don't yet use the types returned by `intersect`, since it
        // returns
...
JL_DLLEXPORT jl_svec_t *
jl_env_from_type_intersection(jl_value_t *a, jl_value_t *b)
{
...
static int type_morespecific_(
        jl_value_t *a, jl_value_t *b, int invariant, jl_typeenv_t *env);
        ...
        jl_typeenv_t *env)
{
    ...
    while (1) {
        ...
        // Tuple{..., T} not more specific than Tuple{..., Vararg{S}} if S is
        // diagonal
        if (eqv && i == clen - 1 && clen == plen && !cva && pva &&
...
static int args_morespecific_fix1(
        jl_value_t *a, jl_value_t *b, int swap, jl_typeenv_t *env)
{
...
static int partially_morespecific(
        jl_value_t *a, jl_value_t *b, int invariant, jl_typeenv_t *env)
{
        ...
        jl_value_t *a, jl_value_t *b, int invariant, jl_typeenv_t *env)
{
    ...
    if (jl_is_tuple_type(a) && jl_is_tuple_type(b)) {
        ...
        if (ans != -1)
            return ans;
        return tuple_morespecific(
                (jl_datatype_t *)a, (jl_datatype_t *)b, invariant, env);
    }
    ...
    if (jl_is_uniontype(a)) {
        if (sub_msp(b, a, env))
            ...
            return 1;
        // Union a is more specific than b if some element of a is more specific
        // than b,
        // and b is not more specific than any element of a.
        jl_uniontype_t *u = (jl_uniontype_t *)a;
        if (partially_morespecific(u->a, b, invariant, env) &&
            !type_morespecific_(b, u->a, invariant, env)) {
            if (partially_morespecific(b, a, invariant, env))
                return 0;
            return 1;
        }
        if (partially_morespecific(u->b, b, invariant, env) &&
            !type_morespecific_(b, u->b, invariant, env)) {
            if (partially_morespecific(b, a, invariant, env))
                return 0;
            return 1;
        }
    ...
    if (!invariant) {
        if ((jl_datatype_t *)a == jl_any_type || a == jl_ANY_flag)
            return 0;
        if ((jl_datatype_t *)b == jl_any_type || b == jl_ANY_flag)
            return 1;
    }
        ...
        while (tta != jl_any_type) {
            if (tta->name == ttb->name) {
                if (super) {
                    ...
                    if (jl_is_typevar(tp0)) {
                        if (sub_msp((jl_value_t *)jl_any_type,
                                    ((jl_tvar_t *)tp0)->ub,
                                    env))
                            return 1;
                    }
                ...
                int ascore = 0, bscore = 0, ascore1 = 0, bscore1 = 0, adiag = 0,
                    bdiag = 0;
                for (size_t i = 0; i < jl_nparams(tta); i++) {
                    ...
                    if (type_morespecific_(apara, bpara, 1, env))
                        ...
                        bscore += 1;
                    if (jl_is_typevar(bpara) && !jl_is_typevar(apara) &&
                        !jl_is_type(apara))
                        ascore1 = 1;
                    else if (
                            jl_is_typevar(apara) && !jl_is_typevar(bpara) &&
                            !jl_is_type(bpara))
                        bscore1 = 1;
    ...
    if (jl_is_typevar(a)) {
        if (jl_is_typevar(b)) {
            return ((type_morespecific_(
                             (jl_value_t *)((jl_tvar_t *)a)->ub,
                             (jl_value_t *)((jl_tvar_t *)b)->ub,
                             0,
                             env) &&
                     !type_morespecific_(
                             (jl_value_t *)((jl_tvar_t *)a)->lb,
                             (jl_value_t *)((jl_tvar_t *)b)->lb,
                             0,
                             env)) ||
                    (type_morespecific_(
                             (jl_value_t *)((jl_tvar_t *)b)->lb,
        ...
        return type_morespecific_(
                (jl_value_t *)((jl_tvar_t *)a)->ub, b, 0, env);
    }
    if (jl_is_typevar(b)) {
        if (!jl_is_type(a))
            return 1;
        ...
        return type_morespecific_(
                a, (jl_value_t *)((jl_tvar_t *)b)->ub, 0, env);
    }
