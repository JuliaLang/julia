...
JL_DLLEXPORT int32_t jl_nb_available(ios_t *s) { return (int32_t)(s->size - s->bpos); }
...
JL_DLLEXPORT int32_t jl_stat(const char *path, char *statbuf)
{
    ...
    if (ret == 0) memcpy(statbuf, req.ptr, sizeof(uv_stat_t));
...
JL_DLLEXPORT uint64_t jl_ios_get_nbyte_int(ios_t *s, const size_t n)
{
    ...
    do {
        ...
        if (space == ret && ret < n) jl_eof_error();
    } while (ret < n);
    uint64_t x   = 0;
    uint8_t *buf = (uint8_t *)&s->buf[s->bpos];
    if (n == 8) {
        // expecting loop unrolling optimization
        for (size_t i = 0; i < 8; i++) x |= (uint64_t)buf[i] << (i << 3);
    }
    else if (n >= 4) {
        // expecting loop unrolling optimization
        for (size_t i = 0; i < 4; i++) x |= (uint64_t)buf[i] << (i << 3);
        for (size_t i = 4; i < n; i++) x |= (uint64_t)buf[i] << (i << 3);
    }
    else {
        for (size_t i = 0; i < n; i++) x |= (uint64_t)buf[i] << (i << 3);
    }
    s->bpos += n;
    return x;
}
...
JL_DLLEXPORT int jl_cpu_cores(void)
{
    ...
    GAPC gapc = (GAPC)jl_dlsym_e(jl_kernel32_handle, "GetActiveProcessorCount");
...
JL_DLLEXPORT uint64_t jl_hrtime(void) { return uv_hrtime(); }
...
JL_DLLEXPORT int32_t jl_get_zero_subnormals(int8_t isZero) { return 0; }

JL_DLLEXPORT int32_t jl_set_zero_subnormals(int8_t isZero) { return isZero; }
...
#else
JL_DLLEXPORT long                                  jl_getpagesize(void) { return sysconf(_SC_PAGESIZE); }
#endif
...
{
    if (ty->layout == NULL) jl_error("non-leaf type doesn't have an alignment");
    return ty->layout->alignment;
}

// Takes a handle (as returned from dlopen()) and returns the absolute path to
// the image loaded
JL_DLLEXPORT const char *jl_pathname_for_handle(void *handle)
{
    ...
    for (int32_t i = _dyld_image_count(); i >= 0; i--) {
        ...
        // If the handle is the same as what was passed in (modulo mode bits),
        // return this image name
        if (((intptr_t)handle & (-4)) == ((intptr_t)probe_lib & (-4))) return image_name;
    }
                                                     ...
                                                     _In_opt_ PVOID a)
{
    ...
    // XXX: change to jl_arrayset if array storage allocation for Array{String,1}
    // changes:
...
JL_DLLEXPORT void jl_raise_debugger(void)
{
    ...
    if (IsDebuggerPresent() == 1) DebugBreak();
#else
    raise(SIGTRAP);
#endif // _OS_WINDOWS_
}

JL_DLLEXPORT jl_sym_t *jl_get_UNAME(void) { return jl_symbol(JL_BUILD_UNAME); }

JL_DLLEXPORT jl_sym_t *jl_get_ARCH(void) { return jl_symbol(JL_BUILD_ARCH); }
