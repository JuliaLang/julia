...
(define-mac (pattern-set form) ;. pats)
  (define (filter p l)
    (if (null? l)
	'()
	(if (p (car l))
  ...
  (let ((pats (cdr form))
	(pred (lambda (x)
		(and (pair? x) (= (length x) 3)
		     ...
		     (pair? (cadr x))))))
    (let ((pls (filter pred pats))
	  (others (filter (lambda (x) (not (pred x))) pats)))
      (let ((heads (delete-duplicates (map (lambda (p) (car (cadr p))) pls)))
	    (ht    (gensym)))
...
(define-mac (pattern-lambda form) ;pat body)
  (define (delete-duplicates l)
    (if (null? l)
	'()
	(if (memq (car l) (cdr l))
