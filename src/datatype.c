...
static jl_datatype_t *jl_incompleteunion_type = NULL;
...
JL_DLLEXPORT void jl_init_incomplete_types(void)
{
    ...
    jl_incompleteunion_type = (jl_datatype_t*)jl_get_global(jl_core_module, jl_symbol("IncompleteUnion"));
...
STATIC_INLINE int jl_is_incompleteunion(jl_value_t *v) JL_NOTSAFEPOINT {
    return jl_incompleteunion_type != NULL && jl_typeis(v, jl_incompleteunion_type);
}
...
STATIC_INLINE int jl_is_incomplete(jl_value_t *v) JL_NOTSAFEPOINT {
    return jl_is_incompletestruct(v) || jl_is_incompleteunion(v) ||
           jl_is_incompleteunionall(v) || jl_is_incompletetypeapp(v);
}

// Field accessors for IncompleteStruct
// Layout: name(0), super(1), parameters(2), fieldnames(3), fieldtypes(4), fieldattrs(5), mutabl(6), abstract(7)
...
// For Bool fields, use jl_fieldref and compare to jl_true (jl_unbox_bool can crash on boxed bools)
#define jl_incompletestruct_mutabl(is)     (jl_fieldref((jl_value_t*)(is), 6) == jl_true)
#define jl_incompletestruct_abstract(is)   (jl_fieldref((jl_value_t*)(is), 7) == jl_true)
#define jl_incompletestruct_min_initialized(is) ((int)jl_unbox_long(jl_fieldref((jl_value_t*)(is), 8)))

// Field accessors for IncompleteUnion
#define jl_incompleteunion_types(iu)       ((jl_svec_t*)jl_get_nth_field_noalloc((jl_value_t*)(iu), 0))
...
static int contains_specific_incomplete(jl_value_t *t, jl_value_t *target)
{
    if (t == target)
        return 1;
    ...
    if (jl_is_incompleteunion(t)) {
        jl_svec_t *types = jl_incompleteunion_types(t);
        size_t n = jl_svec_len(types);
        for (size_t i = 0; i < n; i++) {
            if (contains_specific_incomplete(jl_svecref(types, i), target))
                return 1;
        }
        return 0;
    }
...
static jl_value_t *resolve_type_refs(jl_value_t *t, htable_t *subst_map, htable_t *resolving_set)
{
    ...
    if (jl_is_incompletestruct(t)) {
        jl_value_t *dt = ptrhash_get(subst_map, t);
        if (dt != HT_NOTFOUND)
            return dt;
        jl_error("unresolved incomplete type reference");
    }

    // IncompleteUnion -> resolve each member, create real Union
    if (jl_is_incompleteunion(t)) {
        // Check for cycles (self-referential unions)
        if (ptrhash_get(resolving_set, t) != HT_NOTFOUND)
            jl_error("self-referential type alias not allowed");
        ptrhash_put(resolving_set, t, t);
        jl_svec_t *types = jl_incompleteunion_types(t);
        size_t n = jl_svec_len(types);
        JL_GC_PUSH1(&types);
        jl_value_t **resolved = (jl_value_t**)alloca(n * sizeof(jl_value_t*));
        for (size_t i = 0; i < n; i++) {
            resolved[i] = resolve_type_refs(jl_svecref(types, i), subst_map, resolving_set);
        }
        jl_value_t *result = jl_type_union(resolved, n);
        JL_GC_POP();
        return result;
    }
    ...
    if (jl_is_incompletetypeapp(t)) {
        ...
        jl_value_t *result = jl_apply_type(head, resolved, n);
        JL_GC_POP();
        return result;
    }
    ...
    if (jl_is_unionall(t)) {
        ...
        if (body == ua->body)
            return t;
        return jl_type_unionall(ua->var, body);
    }
    ...
    if (jl_is_datatype(t)) {
        ...
        if (!changed)
            return t;
        return jl_apply_type((jl_value_t*)dt->name->wrapper, resolved, n);
    }
    ...
    JL_TRY {
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (!jl_is_incompletestruct(incomplete)) {
                jl_type_error("resolve_incompletes", (jl_value_t*)jl_incompletestruct_type, incomplete);
            }
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (datatypes[i]->name->wrapper == NULL) {
                datatypes[i]->name->wrapper = (jl_value_t*)datatypes[i];
                jl_gc_wb(datatypes[i]->name, datatypes[i]);
                for (int j = np - 1; j >= 0; j--) {
                    jl_tvar_t *tv = (jl_tvar_t*)jl_svecref(params, j);
                    datatypes[i]->name->wrapper = jl_new_struct(jl_unionall_type, tv, datatypes[i]->name->wrapper);
                    jl_gc_wb(datatypes[i]->name, datatypes[i]->name->wrapper);
                }
                results[i] = datatypes[i]->name->wrapper;
            }
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (super != jl_nothing && super != NULL) {
                ...
                jl_value_t *resolved_super = resolve_type_refs(super, &subst_map, &resolving_set);
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            datatypes[i]->isconcretetype = 0;  // Will be set properly after layout
        }
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            jl_svec_t *ftypes = jl_alloc_svec(nf);
            JL_GC_PUSH2(&ftypes, &is_types);  // Root both to survive GC during resolve_type_refs
        ...
        for (size_t i = 0; i < n; i++) {
            ...
            if (datatypes[i]->types != NULL) {
                jl_compute_field_offsets(datatypes[i]);
            }
            ...
            if (!mutabl) {
                size_t nf = jl_svec_len(datatypes[i]->types);
                int mayinlinealloc = 1;
                for (size_t j = 0; j < nf && mayinlinealloc; j++) {
                    jl_value_t *fld = jl_svecref(datatypes[i]->types, j);
                    // Check if this field references ANY type in the recursive block
                    for (size_t k = 0; k < n && mayinlinealloc; k++) {
                        if (references_typename(fld, datatypes[k]->name, 1, 1)) {
                            mayinlinealloc = 0;
                        }
                    }
                }
                datatypes[i]->name->mayinlinealloc = mayinlinealloc;
            }
            ...
            jl_maybe_allocate_singleton_instance(datatypes[i]);
        }
    }
    JL_CATCH {
