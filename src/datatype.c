...
#define BOX_FUNC(typ,c_type,pfx)                                        \
    ...
    {                                                                   \
        ...
        jl_value_t *v = jl_gc_alloc(ct->ptls, LLT_ALIGN(sizeof(x), sizeof(void*)), \
                                    jl_##typ##_type, JL_alloc_box_unknown_reason); \
...
JL_DLLEXPORT jl_value_t *jl_box_char(uint32_t x)
{
    ...
    jl_value_t *v = jl_gc_alloc(ct->ptls, sizeof(void*), jl_char_type, JL_alloc_box_unknown_reason);
...
JL_DLLEXPORT jl_value_t *jl_new_struct(jl_datatype_t *type, ...)
{
    ...
    va_start(args, type);
    jl_value_t *jv = jl_gc_alloc(ct->ptls, jl_datatype_size(type), type, JL_alloc_new_object);
...
JL_DLLEXPORT jl_value_t *jl_new_structt(jl_datatype_t *type, jl_value_t *tup)
{
    ...
    // TODO(PR): is that right?
    jl_value_t *jv = jl_gc_alloc(ct->ptls, size, type, JL_alloc_new_object);
    if (type->smalltag) // TODO: do we need this?
        jl_set_typetagof(jv, type->smalltag, 0);
    if (nf == 0)
        return jv;
