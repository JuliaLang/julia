        ...
        int ninitialized)
{
    ...
    if (super != NULL) 
        jl_write(t, (void**)&(t->super), super);
    else
        t->super = super;
    jl_write(t, (void**)&(t->parameters), parameters);
    
    if (ftypes != NULL)
        jl_write(t, (void**)&(t->types), ftypes);
    else
        t->types = ftypes;
    t->name = NULL;
    ...
    jl_write(t, (void**)&(t->name), tn);
    jl_write(t->name, (void**)&(t->name->names), fnames);
...
inline void set_nth_field(jl_datatype_t *st, jl_value_t *v, size_t i, jl_value_t *rhs, int isatomic) JL_NOTSAFEPOINT
{
    ...
    if (jl_field_isptr(st, i)) {
        ...
        if (hasptr)
            jl_gc_multi_wb_post(v, rhs); // rhs is immutable
    }
}
...
inline jl_value_t *swap_bits(jl_value_t *ty, char *v, uint8_t *psel, jl_value_t *parent, jl_value_t *rhs, enum atomic_kind isatomic)
{
    ...
    jl_value_t *r;
    if (hasptr)
        jl_gc_multi_wb_pre(parent, rhs); // rhs is immutable
    if (isatomic && !needlock) {
    ...
    if (hasptr)
        jl_gc_multi_wb_post(parent, rhs); // rhs is immutable
    if (__unlikely(r == NULL))
        jl_throw(jl_undefref_exception);
    return r;
}
...
inline jl_value_t *modify_bits(jl_value_t *ty, char *p, uint8_t *psel, jl_value_t *parent, jl_value_t *op, jl_value_t *rhs, enum atomic_kind isatomic)
{
    ...
    while (1) {
        ...
        if (isatomic && !needlock) {
            assert(yty == rty);
            if (hasptr)
                jl_gc_multi_wb_pre(parent, y); // y is immutable
