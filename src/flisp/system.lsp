...
(define (__start argv)
  ...
  (if (pair? (cdr argv))
      (begin (set! *argv* (cdr argv))
             (set! *interactive* #f)
             (if (or (equal? "-e" (cadr argv)) (equal? "--eval" (cadr argv)))
                 (if (length> argv 2)
                     (begin (eval-string (string "(begin " (string.join (cddr argv) " ") ")"))
                            (io.flush *output-stream*))
                     (begin (princ "ERROR: -e/--eval needs at least one expression\n")
                            (exit 1)))
                 (__script (cadr argv))))
      (begin (set! *argv* argv)
	     (set! *interactive* #t)
