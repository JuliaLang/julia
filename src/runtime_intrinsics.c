...
jl_pointerref(jl_value_t *p, jl_value_t *i, jl_value_t *align)
{
    ...
    if (ety == (jl_value_t *)jl_any_type) {
        jl_value_t **pp =
                (jl_value_t *
                         *)(jl_unbox_long(p) + (jl_unbox_long(i) - 1) * sizeof(void *));
        return *pp;
    }
    else {
...
JL_DLLEXPORT jl_value_t *
jl_pointerset(jl_value_t *p, jl_value_t *x, jl_value_t *i, jl_value_t *align)
{
...
JL_DLLEXPORT jl_value_t *jl_cglobal(jl_value_t *v, jl_value_t *ty)
{
    ...
    jl_value_t *rt = v == (jl_value_t *)jl_void_type
                             ? (jl_value_t *)jl_voidpointer_type
                             : // a common case
                             (jl_value_t *)jl_apply_type1(
                                     (jl_value_t *)jl_pointer_type, ty);
...
#define SELECTOR_FUNC(intrinsic)                                               \
    ...
    static inline intrinsic##_t select_##intrinsic(                            \
            unsigned sz, const select_##intrinsic##_t list)                    \
    {                                                                          \
...
#define un_iintrinsic_ctype(OP, name, nbits, c_type)                           \
    static inline void jl_##name##nbits(                                       \
            unsigned runtime_nbits, void *pa, void *pr)                        \
    {                                                                          \
        c_type a = *(c_type *)pa;                                              \
        *(c_type *)pr = OP(a);                                                 \
    }
...
#define bi_intrinsic_ctype(OP, name, nbits, c_type)                            \
    static void jl_##name##nbits(                                              \
            unsigned runtime_nbits, void *pa, void *pb, void *pr)              \
    {                                                                          \
        c_type a = *(c_type *)pa;                                              \
        c_type b = *(c_type *)pb;                                              \
...
#define ter_intrinsic_ctype(OP, name, nbits, c_type)                           \
    static void jl_##name##nbits(                                              \
            unsigned runtime_nbits, void *pa, void *pb, void *pc, void *pr)    \
    {                                                                          \
        c_type a = *(c_type *)pa;                                              \
        c_type b = *(c_type *)pb;                                              \
        c_type c = *(c_type *)pc;                                              \
...
#define un_iintrinsic_fast(LLVMOP, OP, name, u)                                \
    un_iintrinsic_ctype(OP, name, 8, u##int##8_t) un_iintrinsic_ctype(         \
            OP, name, 16, u##int##16_t)                                        \
            un_iintrinsic_ctype(OP, name, 32, u##int##32_t)                    \
                    un_iintrinsic_ctype(                                       \
                            OP,                                                \
                            name,                                              \
                            64,                                                \
                            u##int##64_t) static const select_intrinsic_1_t    \
                            name##_list = {                                    \
                                LLVMOP,        jl_##name##8,  jl_##name##16,   \
                                jl_##name##32, jl_##name##64,                  \
                            };                                                 \
...
#define uu_iintrinsic_fast(LLVMOP, OP, name, u)                                \
    uu_iintrinsic_ctype(OP, name, 8, u##int##8_t) uu_iintrinsic_ctype(         \
            OP, name, 16, u##int##16_t)                                        \
            uu_iintrinsic_ctype(OP, name, 32, u##int##32_t)                    \
                    uu_iintrinsic_ctype(                                       \
                            ...
                            name,                                              \
                            64,                                                \
                            u##int##64_t) static const select_intrinsic_u1_t   \
                            name##_list = {                                    \
                                LLVMOP,        jl_##name##8,  jl_##name##16,   \
                                jl_##name##32, jl_##name##64,                  \
                            };                                                 \
...
static inline jl_value_t *jl_iintrinsic_1(
        ...
        jl_value_t *(*lambda1)(
                jl_value_t *, void *, unsigned, unsigned, const void *),
        const void *list)
{
    ...
    if (osize2 > isize || isize2 > isize) {
        /* if needed, round type up to a real c-type and set/clear the unused
         * bits */
        void *pa2;
        pa2 = alloca(osize2);
...
static inline jl_value_t *jl_intrinsiclambda_ty1(
        jl_value_t *ty,
        void *pa,
        unsigned osize,
        unsigned osize2,
        const void *voidlist)
{
    ...
    intrinsic_1_t op =
            select_intrinsic_1(osize2, (const intrinsic_1_t *)voidlist);
    op(osize * host_char_bit, pa, jl_data_ptr(newv));
    return newv;
}

static inline jl_value_t *jl_intrinsiclambda_u1(
        jl_value_t *ty,
        void *pa,
        unsigned osize,
        unsigned osize2,
        const void *voidlist)
{
    ...
    jl_value_t *newv = jl_gc_alloc(ptls, jl_datatype_size(ty), ty);
    intrinsic_u1_t op =
            select_intrinsic_u1(osize2, (const intrinsic_u1_t *)voidlist);
    ...
    if (osize > sizeof(unsigned)) {
        ...
        memset((char *)jl_data_ptr(newv) + sizeof(unsigned),
               0,
...
static inline jl_value_t *jl_intrinsic_cvt(
        jl_value_t *ty,
        jl_value_t *a,
        const char *name,
        intrinsic_cvt_t op,
        intrinsic_cvt_check_t check_op)
{
    ...
    jl_value_t *newv = jl_gc_alloc(ptls, jl_datatype_size(ty), ty);
    op(aty == (jl_value_t *)jl_bool_type ? 1 : isize * host_char_bit,
       pa,
       osize * host_char_bit,
       jl_data_ptr(newv));
    if (ty == (jl_value_t *)jl_bool_type)
...
#define un_fintrinsic_withtype(OP, name)                                       \
    un_fintrinsic_ctype(OP, jl_##name##32, float) un_fintrinsic_ctype(         \
            OP, jl_##name##64, double)                                         \
            JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *ty, jl_value_t *a)  \
    {                                                                          \
...
static inline jl_value_t *jl_fintrinsic_1(
        jl_value_t *ty,
        jl_value_t *a,
        const char *name,
        fintrinsic_op1 *floatop,
        fintrinsic_op1 *doubleop)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (!jl_is_primitivetype(jl_typeof(a)))
        jl_errorf("%s: value is not a primitive type", name);
    ...
    default:
        jl_errorf(
                "%s: runtime floating point intrinsics are not implemented for "
...
#define bi_iintrinsic_cnvtb_fast(LLVMOP, OP, name, u, cvtb)                    \
    bi_intrinsic_ctype(OP, name, 8, u##int##8_t) bi_intrinsic_ctype(           \
            OP, name, 16, u##int##16_t)                                        \
            bi_intrinsic_ctype(OP, name, 32, u##int##32_t) bi_intrinsic_ctype( \
                    OP,                                                        \
                    name,                                                      \
                    64,                                                        \
                    u##int##64_t) static const select_intrinsic_2_t            \
                    name##_list = {                                            \
                        LLVMOP,        jl_##name##8,  jl_##name##16,           \
                        jl_##name##32, jl_##name##64,                          \
                    };                                                         \
...
#define bool_iintrinsic_fast(LLVMOP, OP, name, u)                              \
    bool_intrinsic_ctype(OP, name, 8, u##int##8_t) bool_intrinsic_ctype(       \
            OP, name, 16, u##int##16_t)                                        \
            bool_intrinsic_ctype(OP, name, 32, u##int##32_t)                   \
                    bool_intrinsic_ctype(                                      \
                            ...
                            name,                                              \
                            64,                                                \
                            u##int##64_t) static const select_intrinsic_cmp_t  \
                            name##_list = {                                    \
                                LLVMOP,        jl_##name##8,  jl_##name##16,   \
                                jl_##name##32, jl_##name##64,                  \
                            };                                                 \
...
#define checked_iintrinsic(name, u, lambda_checked)                            \
    ...
    {                                                                          \
        return jl_iintrinsic_2(                                                \
                a, b, #name, u##signbitbyte, lambda_checked, name##_list, 0);  \
    }
    ...
    checked_intrinsic_ctype(CHECK_OP, OP, name, 8, u##int##8_t)                           \
            checked_intrinsic_ctype(CHECK_OP, OP, name, 16, u##int##16_t)                 \
                    checked_intrinsic_ctype(                                              \
                            CHECK_OP, OP, name, 32, u##int##32_t)                         \
                            checked_intrinsic_ctype(                                      \
                                    CHECK_OP,                                             \
                                    OP,                                                   \
                                    name,                                                 \
                                    64,                                                   \
                                    u##int##64_t) static const select_intrinsic_checked_t \
                                    name##_list = {                                       \
                                        LLVMOP,        jl_##name##8,                      \
                                        jl_##name##16, jl_##name##32,                     \
                                        jl_##name##64,                                    \
                                    };                                                    \
...
static inline jl_value_t *jl_iintrinsic_2(
        ...
        char (*getsign)(void *, unsigned),
        jl_value_t *(*lambda2)(
                jl_value_t *, void *, void *, unsigned, unsigned, const void *),
        const void *list,
        int cvtb)
{
    ...
    if (sz2 > szb) {
        /* round type up to the appropriate c-type and set/clear/truncate the
         * unused bits */
        void *pb2 = alloca(sz2);
        memcpy(pb2, pb, szb);
...
static inline jl_value_t *jl_intrinsiclambda_2(
        jl_value_t *ty,
        void *pa,
        void *pb,
        unsigned sz,
...
static inline jl_value_t *jl_intrinsiclambda_cmp(
        jl_value_t *ty,
        void *pa,
        void *pb,
        unsigned sz,
...
{
    intrinsic_cmp_t op =
            select_intrinsic_cmp(sz2, (const intrinsic_cmp_t *)voidlist);
    int cmp = op(sz * host_char_bit, pa, pb);
    return cmp ? jl_true : jl_false;
}

static inline jl_value_t *jl_intrinsiclambda_checked(
        jl_value_t *ty,
        void *pa,
        void *pb,
        unsigned sz,
        ...
        const void *voidlist)
{
    ...
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_value_t *newv =
            jl_gc_alloc(ptls, ((jl_datatype_t *)tuptyp)->size, tuptyp);

    intrinsic_checked_t op = select_intrinsic_checked(
            sz2, (const intrinsic_checked_t *)voidlist);
...
static inline jl_value_t *jl_intrinsiclambda_checkeddiv(
        jl_value_t *ty,
        void *pa,
        void *pb,
        unsigned sz,
...
#define bi_fintrinsic(OP, name)                                                \
    bi_intrinsic_ctype(OP, name, 32, float) bi_intrinsic_ctype(                \
            OP, name, 64, double)                                              \
            JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)   \
    {                                                                          \
        jl_ptls_t ptls = jl_get_ptls_states();                                 \
...
#define bool_fintrinsic(OP, name)                                              \
    bool_intrinsic_ctype(OP, name, 32, float) bool_intrinsic_ctype(            \
            OP, name, 64, double)                                              \
            JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)   \
    {                                                                          \
        jl_value_t *ty = jl_typeof(a);                                         \
...
#define ter_fintrinsic(OP, name)                                               \
    ter_intrinsic_ctype(OP, name, 32, float)                                   \
            ter_intrinsic_ctype(OP, name, 64, double)                          \
                    JL_DLLEXPORT jl_value_t *jl_##name(                        \
                            jl_value_t *a, jl_value_t *b, jl_value_t *c)       \
    {                                                                          \
        jl_ptls_t ptls = jl_get_ptls_states();                                 \
        jl_value_t *ty = jl_typeof(a);                                         \
...
#define div(a, b) a / b
                                bi_iintrinsic_fast(LLVMSDiv, div, sdiv_int, ) bi_iintrinsic_fast(
                                        LLVMUDiv, div, udiv_int, u)
#define rem(a, b) a % b
                                        bi_iintrinsic_fast(LLVMSRem, rem, srem_int, ) bi_iintrinsic_fast(
                                                LLVMURem, rem, urem_int, u)
...
#define fpislt(a, b)                                                           \
    sizeof(a) == sizeof(float) ? fpislt32(a, b) : fpislt64(a, b)
                ...
                bool_fintrinsic(eq, eq_float) bool_fintrinsic(
                        ne, ne_float) bool_fintrinsic(lt, lt_float)
                        bool_fintrinsic(le, le_float) bool_fintrinsic(
                                fpiseq, fpiseq) bool_fintrinsic(fpislt, fpislt)
...
#define not_op(a) ~a
        ...
        cvt_iintrinsic(LLVMTrunc, trunc_int) cvt_iintrinsic(LLVMSExt, sext_int)
                cvt_iintrinsic(LLVMZExt, zext_int) cvt_iintrinsic(
                        LLVMSItoFP, sitofp) cvt_iintrinsic(LLVMUItoFP, uitofp)
                        cvt_iintrinsic(LLVMFPtoSI, fptosi) cvt_iintrinsic(
                                LLVMFPtoUI, fptoui)
...
cvt_iintrinsic_checked(LLVMTrunc, check_trunc_sint, checked_trunc_sint) static unsigned check_trunc_uint(
        unsigned isize, unsigned osize, void *pa)
{
    return !all_eq(
            (char *)pa + osize,
            isize - osize,
            0); // TODO: assumes little-endian
}
...
#define check_uadd_int(a, b)                                                   \
    ...
    a >= -b
        checked_iintrinsic_fast(
                LLVMAdd_uov, check_uadd_int, add, checked_uadd_int, u)
#define check_ssub_int(a, b) check_sadd_int(a, -b)
                checked_iintrinsic_fast(
                        LLVMSub_sov, check_ssub_int, sub, checked_ssub_int, )
#define check_usub_int(a, b)                                                   \
    /* this test checks for (a - b) < 0 ==> overflow */                        \
    a < b
                        checked_iintrinsic_fast(LLVMSub_uov, check_usub_int, sub, checked_usub_int, u) checked_iintrinsic_slow(
                                LLVMMul_sov,
                                checked_smul_int, ) checked_iintrinsic_slow(LLVMMul_uov, checked_umul_int, u)

                                checked_iintrinsic_div(LLVMDiv_sov, checked_sdiv_int, ) checked_iintrinsic_div(
                                        LLVMDiv_uov,
                                        checked_udiv_int,
                                        u) checked_iintrinsic_div(LLVMRem_sov, checked_srem_int, )
                                        checked_iintrinsic_div(
                                                LLVMRem_uov,
                                                checked_urem_int,
                                                u)
...
#define copysign_float(a, b) fp_select2(a, b, copysign)
                                                        ...
                                                        un_fintrinsic(abs_float, abs_float) bi_fintrinsic(
                                                                copysign_float,
                                                                copysign_float) un_fintrinsic(ceil_float, ceil_llvm)
                                                                un_fintrinsic(floor_float, floor_llvm) un_fintrinsic(
                                                                        trunc_float,
                                                                        trunc_llvm)
                                                                        un_fintrinsic(
                                                                                rint_float,
                                                                                ...
                                                                                un_fintrinsic(
                                                                                        ...
                                                                                        JL_DLLEXPORT
        jl_value_t *jl_select_value(
                jl_value_t *isfalse, jl_value_t *a, jl_value_t *b)
{
