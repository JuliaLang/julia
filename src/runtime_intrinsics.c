...
// This is in implementation of the Julia intrinsic functions against boxed
// types
// excluding the c interface (ccall, cglobal, llvmcall)
//
// this file assumes a little-endian processor, although that isn't too hard to
// fix
// it also assumes two's complement negative numbers, which might be a bit
// harder to fix
//
...
JL_DLLEXPORT jl_value_t *jl_pointerref(jl_value_t *p, jl_value_t *i, jl_value_t *align)
{
    ...
    if (ety == (jl_value_t *)jl_any_type) {
        ...
        if (!jl_is_datatype(ety)) jl_error("pointerref: invalid pointer");
        size_t nb =
            LLT_ALIGN(jl_datatype_size(ety), ((jl_datatype_t *)ety)->layout->alignment);
                                       ...
                                       jl_value_t *align)
{
    ...
    if (ety == (jl_value_t *)jl_any_type) {
        ...
        *pp = x;
        ...
        if (!jl_is_datatype(ety)) jl_error("pointerset: invalid pointer");
        size_t nb =
            LLT_ALIGN(jl_datatype_size(ety), ((jl_datatype_t *)ety)->layout->alignment);
        char *pp = (char *)jl_unbox_long(p) + (jl_unbox_long(i) - 1) * nb;
        if (jl_typeof(x) != ety) jl_error("pointerset: type mismatch in assign");
        jl_assign_bits(pp, x);
    }
    return p;
}
...
#define SELECTOR_FUNC(intrinsic)                                                           \
    ...
    static inline intrinsic##_t select_##intrinsic(unsigned                     sz,        \
                                                   const select_##intrinsic##_t list)      \
    {                                                                                      \
...
#define un_iintrinsic_fast(LLVMOP, OP, name, u)                                            \
    un_iintrinsic_ctype(OP, name, 8, u##int##8_t) un_iintrinsic_ctype(OP, name, 16,        \
                                                                      u##int##16_t)        \
        un_iintrinsic_ctype(OP, name, 32, u##int##32_t) un_iintrinsic_ctype(               \
            OP, name, 64, u##int##64_t) static const select_intrinsic_1_t name##_list = {  \
...
#define uu_iintrinsic_fast(LLVMOP, OP, name, u)                                            \
    uu_iintrinsic_ctype(OP, name, 8, u##int##8_t) uu_iintrinsic_ctype(OP, name, 16,        \
                                                                      u##int##16_t)        \
        uu_iintrinsic_ctype(OP, name, 32, u##int##32_t) uu_iintrinsic_ctype(               \
            OP, name, 64, u##int##64_t) static const select_intrinsic_u1_t name##_list = { \
            LLVMOP, jl_##name##8, jl_##name##16, jl_##name##32, jl_##name##64,             \
    };                                                                                     \
...
static inline jl_value_t *jl_iintrinsic_1(
    ...
    const void *list)
{
    if (!jl_is_bitstype(jl_typeof(a))) jl_errorf("%s: value is not a bitstype", name);
    if (!jl_is_bitstype(ty)) jl_errorf("%s: type is not a bitstype", name);
    ...
    if (isize2 > osize2) osize2 = isize2;
...
#define cvt_iintrinsic(LLVMOP, name) cvt_iintrinsic_checked(LLVMOP, NULL, name)

static inline jl_value_t *jl_intrinsic_cvt(jl_value_t *ty, jl_value_t *a, const char *name,
                                           intrinsic_cvt_t       op,
                                           intrinsic_cvt_check_t check_op)
{
    jl_ptls_t   ptls = jl_get_ptls_states();
    jl_value_t *aty  = jl_typeof(a);
    if (!jl_is_bitstype(aty)) jl_errorf("%s: value is not a bitstype", name);
    if (!jl_is_bitstype(ty)) jl_errorf("%s: type is not a bitstype", name);
    void *   pa    = jl_data_ptr(a);
    unsigned isize = jl_datatype_size(aty);
    unsigned osize = jl_datatype_size(ty);
    if (check_op && check_op(isize, osize, pa)) jl_throw(jl_inexact_exception);
    jl_value_t *newv = jl_gc_alloc(ptls, ((jl_datatype_t *)ty)->size, ty);
                                          ...
                                          fintrinsic_op1 *floatop, fintrinsic_op1 *doubleop)
{
    ...
    default:
        jl_errorf("%s: runtime floating point intrinsics are not implemented for "
                  "bit sizes other than 32 and 64",
                  name);
    }
    return newv;
}
...
#define bi_iintrinsic_cnvtb_fast(LLVMOP, OP, name, u, cvtb)                                \
    bi_intrinsic_ctype(OP, name, 8, u##int##8_t) bi_intrinsic_ctype(                       \
        OP, name, 16, u##int##16_t) bi_intrinsic_ctype(OP, name, 32, u##int##32_t)         \
        bi_intrinsic_ctype(OP, name, 64,                                                   \
                           u##int##64_t) static const select_intrinsic_2_t name##_list = { \
            LLVMOP, jl_##name##8, jl_##name##16, jl_##name##32, jl_##name##64,             \
    };                                                                                     \
...
#define bool_iintrinsic_fast(LLVMOP, OP, name, u)                                          \
    bool_intrinsic_ctype(OP, name, 8, u##int##8_t)                                         \
        bool_intrinsic_ctype(OP, name, 16, u##int##16_t)                                   \
            bool_intrinsic_ctype(OP, name, 32, u##int##32_t) bool_intrinsic_ctype(         \
                OP, name, 64,                                                              \
                u##int##64_t) static const select_intrinsic_cmp_t name##_list = {          \
                LLVMOP, jl_##name##8, jl_##name##16, jl_##name##32, jl_##name##64,         \
    };                                                                                     \
...
#define checked_iintrinsic(name, u)                                                        \
    JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)                       \
    {                                                                                      \
        return jl_iintrinsic_2(a, b, #name, u##signbitbyte, jl_intrinsiclambda_checked,    \
                               name##_list, 0);                                            \
    }
        ...
        checked_intrinsic_ctype(CHECK_OP, OP, name, 16, u##int##16_t)                      \
            checked_intrinsic_ctype(CHECK_OP, OP, name, 32, u##int##32_t)                  \
                checked_intrinsic_ctype(                                                   \
                    CHECK_OP, OP, name, 64,                                                \
                    u##int##64_t) static const select_intrinsic_checked_t name##_list = {  \
                    LLVMOP, jl_##name##8, jl_##name##16, jl_##name##32, jl_##name##64,     \
    };                                                                                     \
    ...
    const void *list, int cvtb)
{
    ...
    if (tyb != ty) {
        if (!cvtb) jl_errorf("%s: types of a and b must match", name);
        if (!jl_is_bitstype(tyb)) jl_errorf("%s: b is not a bitstypes", name);
    }
    if (!jl_is_bitstype(ty)) jl_errorf("%s: a is not a bitstypes", name);
    ...
    if (sz2 > szb) {
        /* round type up to the appropriate c-type and set/clear/truncate the unused
         * bits */
        void *pb2 = alloca(sz2);
        memcpy(pb2, pb, szb);
                                                     ...
                                                     const void *voidlist)
{
    ...
    if (ovflw) jl_throw(jl_overflow_exception);
    if (ty == (jl_value_t *)jl_bool_type)
        return *(uint8_t *)jl_data_ptr(newv) & 1 ? jl_true : jl_false;
    return newv;
}
...
#define bi_fintrinsic(OP, name)                                                            \
    bi_intrinsic_ctype(OP, name, 32, float) bi_intrinsic_ctype(OP, name, 64, double)       \
        JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)                   \
    {                                                                                      \
        jl_ptls_t   ptls = jl_get_ptls_states();                                           \
        jl_value_t *ty   = jl_typeof(a);                                                   \
        if (jl_typeof(b) != ty) jl_error(#name ": types of a and b must match");           \
        if (!jl_is_bitstype(ty)) jl_error(#name ": values are not bitstypes");             \
        ...
        default:                                                                           \
            jl_error(#name ": runtime floating point intrinsics are not "                  \
                           "implemented for bit sizes other than 32 and 64");              \
        }                                                                                  \
        return newv;                                                                       \
    }

#define bool_fintrinsic(OP, name)                                                          \
    bool_intrinsic_ctype(OP, name, 32, float) bool_intrinsic_ctype(OP, name, 64, double)   \
        JL_DLLEXPORT jl_value_t *jl_##name(jl_value_t *a, jl_value_t *b)                   \
    {                                                                                      \
        jl_value_t *ty = jl_typeof(a);                                                     \
        if (jl_typeof(b) != ty) jl_error(#name ": types of a and b must match");           \
...
#define ter_fintrinsic(OP, name)                                                           \
    ter_intrinsic_ctype(OP, name, 32, float) ter_intrinsic_ctype(OP, name, 64, double)     \
...
#define div(a, b) a / b
                bi_iintrinsic_fast(LLVMSDiv, div,
                                   sdiv_int, ) bi_iintrinsic_fast(LLVMUDiv, div, udiv_int,
                                                                  u)
#define rem(a, b) a % b
                    bi_iintrinsic_fast(LLVMSRem, rem,
                                       srem_int, ) bi_iintrinsic_fast(LLVMURem, rem,
                                                                      urem_int, u)
#define smod(a, b) ((a < 0) == (b < 0)) ? a % b : (b + (a % b)) % b
                        bi_iintrinsic_fast(jl_LLVMSMod, smod, smod_int, )
#define frem(a, b) fp_select2(a, b, fmod)

                            un_fintrinsic(neg_float, neg_float) bi_fintrinsic(
                                add, add_float) bi_fintrinsic(sub, sub_float)
                                bi_fintrinsic(mul, mul_float) bi_fintrinsic(
                                    div, div_float) bi_fintrinsic(frem, rem_float)

// ternary operators //
#define fma(a, b, c) sizeof(a) == sizeof(float) ? fmaf(a, b, c) : fma(a, b, c)
#define muladd(a, b, c) a *b + c
                                    ter_fintrinsic(fma,
                                                   fma_float) ter_fintrinsic(muladd,
                                                                             muladd_float)
...
#define lt(a, b) a < b
                                                bool_iintrinsic_fast(LLVMICmpSLT, lt,
                                                                     slt_int, )
                                                    bool_iintrinsic_fast(LLVMICmpULT, lt,
                                                                         ult_int, u)
#define le(a, b) a <= b
                                                        bool_iintrinsic_fast(LLVMICmpSLE,
                                                                             le, sle_int, )
                                                            bool_iintrinsic_fast(
                                                                LLVMICmpULE, le, ule_int, u)
...
fpiseq_n(float, 32) fpiseq_n(double, 64)
...
#define fpislt_n(c_type, nbits)                                                            \
    ...
    {                                                                                      \
        ...
        if (ua.d >= 0 && ua.d < ub.d) return 1;                                            \
        if (ua.d < 0 && ua.ud > ub.ud) return 1;                                           \
        return 0;                                                                          \
    }
    fpislt_n(float, 32) fpislt_n(double, 64)
#define fpislt(a, b) sizeof(a) == sizeof(float) ? fpislt32(a, b) : fpislt64(a, b)

        bool_fintrinsic(eq, eq_float) bool_fintrinsic(ne, ne_float)
            bool_fintrinsic(lt, lt_float) bool_fintrinsic(le, le_float)
                bool_fintrinsic(fpiseq, fpiseq) bool_fintrinsic(fpislt, fpislt)
...
#define lshr_op(a, b) (b >= 8 * sizeof(a)) ? 0 : a >> b
                                    bi_iintrinsic_cnvtb_fast(LLVMLShr, lshr_op, lshr_int, u,
                                                             1)
#define ashr_op(a, b)                                                                      \
    /* if ((signed)a > 0) [in two's complement] ? ... : ...) */                            \
    (a >> (host_char_bit * sizeof(a) - 1)) ? ~(b >= 8 * sizeof(a) ? 0 : (~a) >> b)         \
                                           : (b >= 8 * sizeof(a) ? 0 : a >> b)
                                        bi_iintrinsic_cnvtb_fast(LLVMAShr, ashr_op,
                                                                 ashr_int, u, 1)
...
#define not_op(a) ~a
    ...
    // conversions
    cvt_iintrinsic(LLVMTrunc, trunc_int) cvt_iintrinsic(LLVMSExt, sext_int)
        cvt_iintrinsic(LLVMZExt, zext_int) cvt_iintrinsic(LLVMSItoFP, sitofp)
            cvt_iintrinsic(LLVMUItoFP, uitofp) cvt_iintrinsic(LLVMFPtoSI, fptosi)
...
#define fpcvt(pr, a)                                                                       \
    if (osize == 32)                                                                       \
        ...
        *(double *)pr = a;                                                                 \
        ...
        jl_error("fptrunc/fpext: runtime floating point intrinsics are not "               \
                 "implemented for bit sizes other than 32 and 64");
                            ...
                            JL_DLLEXPORT jl_value_t *jl_fptoui_auto(jl_value_t *a)
{
    ...
    case 4: ty = jl_uint32_type; break;
    case 8: ty = jl_uint64_type; break;
    default:
        jl_error("fptoui: runtime floating point intrinsics are not implemented "
...
JL_DLLEXPORT jl_value_t *jl_fptosi_auto(jl_value_t *a)
{
    jl_datatype_t *ty;
    switch (jl_datatype_size(jl_typeof(a))) {
    case 4: ty = jl_int32_type; break;
    case 8: ty = jl_int64_type; break;
    default:
        jl_error("fptoui: runtime floating point intrinsics are not implemented "
                 "for bit sizes other than 32 and 64");
    }
...
{
    return !all_eq((char *)pa + osize, isize - osize,
                   signbitbyte(pa, isize)); // TODO: assumes little-endian
}
cvt_iintrinsic_checked(LLVMTrunc, check_trunc_sint,
                       checked_trunc_sint) static unsigned check_trunc_uint(unsigned isize,
                                                                            unsigned osize,
                                                                            void *   pa)
{
...
{
    jl_value_t *ty = jl_typeof(a);
    if (!jl_is_bitstype(ty)) jl_error("check_top_bit: value is not a bitstype");
    if (signbitbyte(jl_data_ptr(a), jl_datatype_size(ty))) jl_throw(jl_inexact_exception);
    return a;
}

// checked arithmetic
#define check_sadd_int(a, b)                                                               \
    /* this test is a reduction of (b > 0) ? (a + b > typemax(a)) : (a + b <               \
     * typemin(a)) ==> overflow                                                            \
     * where (a - a) == (typeof(a))0 */                                                    \
    (b > 0) ? (a > ~((a - a + 1) << (8 * sizeof(a) - 1)) - b)                              \
            : (a < ((a - a + 1) << (8 * sizeof(a) - 1)) - b)
...
#define check_usub_int(a, b)                                                               \
    ...
    a < b
            checked_iintrinsic_fast(LLVMSub_uov, check_usub_int, sub, checked_usub_int,
                                    u) checked_iintrinsic_slow(LLVMMul_sov,
                                                               checked_smul_int, )
                checked_iintrinsic_slow(LLVMMul_uov, checked_umul_int,
                                        u) checked_iintrinsic_slow(LLVMDiv_sov,
                                                                   checked_sdiv_int, )
                    checked_iintrinsic_slow(LLVMDiv_uov, checked_udiv_int,
                                            u) checked_iintrinsic_slow(LLVMRem_sov,
                                                                       checked_srem_int, )
...
#define sqrt_float(pr, a)                                                                  \
    if (a < 0) jl_throw(jl_domain_exception);                                              \
    *pr = fp_select(a, sqrt)
                                                ...
                                                un_fintrinsic(trunc_float, trunc_llvm)
                                                    un_fintrinsic(rint_float, rint_llvm)
                                                        un_fintrinsic(sqrt_float, sqrt_llvm)
                                                            ...
                                                            JL_DLLEXPORT
    jl_value_t *jl_powi_llvm(jl_value_t *a, jl_value_t *b)
{
    jl_ptls_t   ptls = jl_get_ptls_states();
    jl_value_t *ty   = jl_typeof(a);
    if (!jl_is_bitstype(ty)) jl_error("powi_llvm: a is not a bitstype");
    ...
    switch (sz) {
    /* choose the right size c-type operation */
    case 4: *(float *)pr  = powf(*(float *)pa, (float)jl_unbox_int32(b)); break;
    case 8: *(double *)pr = pow(*(double *)pa, (double)jl_unbox_int32(b)); break;
    default:
        jl_error("powi_llvm: runtime floating point intrinsics are not implemented "
                 "for bit sizes other than 32 and 64");
    }
    return newv;
}
