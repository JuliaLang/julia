...
{
    // sign bit of a signed number, respecting the logical bit width
...
#define SELECTOR_FUNC(intrinsic) \
    ...
    static inline intrinsic##_t select_##intrinsic(unsigned sz, unsigned bitwidth, const select_##intrinsic##_t list) JL_NOTSAFEPOINT \
    { \
        intrinsic##_t thunk = bitwidth == sz * host_char_bit ? list[select_by_size(sz)] : NULL; \
                            ...
                            jl_value_t *(*lambda1)(jl_value_t*, void*, unsigned, unsigned, const void*), const void *list)
{
    ...
    if (osize2 > isize || isize2 > isize) {
        ...
        memcpy(pa2, pa, isize);
        memset((char*)pa2 + isize, getsign(pa, isize, nbits), osize2 - isize);
        pa = pa2;
    }
...
{
    unsigned nbits = jl_datatype_nbits((jl_datatype_t*)ty);
    intrinsic_1_t op = select_intrinsic_1(osize2, nbits, (const intrinsic_1_t*)voidlist);
...
static inline jl_value_t *jl_intrinsiclambda_u1(jl_value_t *ty, void *pa, unsigned osize, unsigned osize2, const void *voidlist)
{
    jl_task_t *ct = jl_current_task;
    unsigned nbits = jl_datatype_nbits((jl_datatype_t*)ty);
    intrinsic_u1_t op = select_intrinsic_u1(osize2, nbits, (const intrinsic_u1_t*)voidlist);
    uint64_t cnt = op(nbits, pa);
                            ...
                            const void *list, int cvtb)
{
    ...
    if (sz2 > sz) {
        ...
        void *pa2 = alloca(sz2);
        memcpy(pa2, pa, sz);
        memset((char*)pa2 + sz, getsign(pa, sz, nbits), sz2 - sz);
        pa = pa2;
    }
    if (sz2 > szb) {
        ...
        void *pb2 = alloca(sz2);
        memcpy(pb2, pb, szb);
        memset((char*)pb2 + szb, cvtb ? 0 : getsign(pb, szb, nbits), sz2 - szb);
        pb = pb2;
    }
...
static inline jl_value_t *jl_intrinsiclambda_2(jl_value_t *ty, void *pa, void *pb, unsigned sz, unsigned sz2, const void *voidlist)
{
    void *pr = alloca(sz2);
    unsigned nbits = jl_datatype_nbits((jl_datatype_t*)ty);
    intrinsic_2_t op = select_intrinsic_2(sz2, nbits, (const intrinsic_2_t*)voidlist);
    op(nbits, pa, pb, pr);
    return jl_new_bits(ty, pr);
}
...
{
    unsigned nbits = jl_datatype_nbits((jl_datatype_t*)ty);
    intrinsic_cmp_t op = select_intrinsic_cmp(sz2, nbits, (const intrinsic_cmp_t*)voidlist);
    int cmp = op(nbits, pa, pb);
...
static inline jl_value_t *jl_intrinsiclambda_checked(jl_value_t *ty, void *pa, void *pb, unsigned sz, unsigned sz2, const void *voidlist)
{
    ...
    unsigned nbits = jl_datatype_nbits((jl_datatype_t*)ty);
    intrinsic_checked_t op = select_intrinsic_checked(sz2, nbits, (const intrinsic_checked_t*)voidlist);
    int ovflw = op(nbits, pa, pb, jl_data_ptr(newv));
