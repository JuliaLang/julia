...
pub unsafe fn mmtk_scan_gcstack<EV: SlotVisitor<JuliaVMSlot>>(
    ...
    if !s.is_null() {
        ...
        loop {
            ...
            // bit set is a tagged pointer (a tag in the low bits, payload in
            // the remaining ones, e.g. a foreign runtime's immediate value)
            // and is not a heap reference. See gc_is_tagged_pointer in
            // gc-common.h for the representation contract.
