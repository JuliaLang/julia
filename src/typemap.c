                                           ...
                                           int8_t tparam, int8_t offs, jl_value_t *parent)
{
    if (jl_is_datatype(ty)) {
        ...
        if (!uid || is_kind(ty) || jl_has_typevars(ty))
            // be careful not to put non-leaf types or DataType/TypeConstructor in the cache
            // here,
                 ...
                 (jl_is_uniontype(ty) ||       // punt on Union{...} right now
                  ...
                  (jl_is_type_type(ty) &&
                   (jl_is_typevar(jl_tparam0(ty))
                    ...
                    : jl_subtype(
                          t, ty,
                          0))) // `t` is a leaftype, so intersection test becomes subtype
...
int sigs_eq(jl_value_t *a, jl_value_t *b, int useenv)
{
    ...
    // useenv == 1 : subtyping + ensure typevars correspond + fail if bound != bound in some
    // typevar match
    // useenv == 2 : ignore typevars (because UnionAll getting lost in intersection can
    // cause jl_types_equal to fail in the wrong direction for some purposes)
                                      ...
                                      jl_value_t **                 overwritten)
{
    ...
    for (i = 0, l = jl_field_count(type); i < l && newrec->issimplesig; i++) {
        ...
        else if (!jl_is_leaf_type(
                     decl)) // anything else can go through the general subtyping test
