...
#define MAX_METHLIST_COUNT                                                     \
    12                    // this can strongly affect the sysimg size and speed!
...
// compute whether the specificity of this type is equivalent to Any in the sort
// order
...
static int
sig_match_by_type_leaf(jl_value_t **types, jl_tupletype_t *sig, size_t n)
{
    size_t i;
    for (i = 0; i < n; i++) {
        ...
        if (jl_is_type_type(
                    a)) // decl is not Type, because it wouldn't be leafsig
...
static int sig_match_by_type_simple(
        jl_value_t **types,
        size_t n,
        jl_tupletype_t *sig,
        size_t lensig,
        int va)
{
    ...
    for (i = 0; i < lensig; i++) {
        ...
        jl_value_t *a = types[i];
        jl_value_t *unw =
                jl_is_unionall(decl) ? ((jl_unionall_t *)decl)->body : decl;
        if (jl_is_type_type(unw)) {
            jl_value_t *tp0 = jl_tparam0(unw);
            if (jl_is_type_type(a)) {
                if (jl_is_typevar(tp0)) {
                    // in the case of Type{_}, the types don't have to match
                    // exactly.
                    ...
                    if (!(jl_typeof(jl_tparam0(a)) == jl_typeof(tp0) &&
                          jl_types_equal(jl_tparam0(a), tp0)))
                        return 0;
                }
            }
            else if (
                    !jl_is_kind(a) || !jl_is_typevar(tp0) ||
            ...
            if (jl_is_type_type(a)) // decl is not Type, because it would be
                                    // caught above
                a = jl_typeof(jl_tparam0(a));
            if (!jl_types_equal(a, decl))
                return 0;
...
static inline int sig_match_simple(
        jl_value_t **args, size_t n, jl_value_t **sig, int va, size_t lensig)
{
        ...
        if (jl_is_type_type(unw) && jl_is_type(a)) {
            ...
            if (jl_is_typevar(tp0)) {
                ...
                if (a != tp0) {
                    ...
                    if (jl_is_datatype(da) && jl_is_datatype(dt) &&
                        da->name != dt->name)
                        return 0;
...
static inline union jl_typemap_t mtcache_hash_lookup(
        const struct jl_ordereddict_t *a,
        jl_value_t *ty,
        int8_t tparam,
        int8_t offs)
{
    ...
    if (idx > 0) {
        ...
        if (jl_typeof(ml.unknown) == (jl_value_t *)jl_typemap_level_type) {
            ...
            assert(jl_typeof(ml.unknown) ==
                   (jl_value_t *)jl_typemap_entry_type);
            t = jl_field_type(
                    jl_unwrap_unionall((jl_value_t *)ml.leaf->sig), offs);
            if (tparam)
                t = jl_tparam0(t);
        }
        if (t != ty)
...
static void mtcache_rehash(
        struct jl_ordereddict_t *pa,
        size_t newlen,
        jl_value_t *parent,
        int8_t tparam,
        int8_t offs)
...
static union jl_typemap_t *mtcache_hash_bp(
        struct jl_ordereddict_t *pa,
        jl_value_t *ty,
        int8_t tparam,
        int8_t offs,
        jl_value_t *parent)
{
    if (jl_is_datatype(ty)) {
        ...
        if (!uid || jl_is_kind(ty) || jl_has_free_typevars(ty))
            // be careful not to put non-leaf types or DataType/UnionAll in the
            // cache here,
            // since they should have a lower priority and need to go into the
            // sorted list
            return NULL;
        ...
        while (1) {
            ...
            if (idx == 0) {
                ...
                if (idx > jl_max_int(pa->indexes))
                    mtcache_rehash(
                            pa,
                            jl_array_len(pa->indexes),
                            parent,
                            tparam,
                            offs);
                jl_intset(pa->indexes, slot, idx);
                return &((union jl_typemap_t *)jl_array_data(
                        pa->values))[idx - 1];
            }
            union jl_typemap_t *pml =
                    &((union jl_typemap_t *)jl_array_data(pa->values))[idx - 1];
            if (pml->unknown == jl_nothing)
                return pml;
            jl_value_t *t;
            if (jl_typeof(pml->unknown) ==
                (jl_value_t *)jl_typemap_level_type) {
                t = pml->node->key;
            }
            else {
                assert(jl_typeof(pml->unknown) ==
                       (jl_value_t *)jl_typemap_entry_type);
                t = jl_field_type(
                        jl_unwrap_unionall((jl_value_t *)pml->leaf->sig), offs);
                if (tparam)
                    t = jl_tparam0(t);
            }
            if (t == ty)
                return pml;
            mtcache_rehash(
                    pa, jl_array_len(pa->indexes) * 2, parent, tparam, offs);
        }
    }
    return NULL;
}
...
static int jl_typemap_array_visitor(
        struct jl_ordereddict_t *a, jl_typemap_visitor_fptr fptr, void *closure)
{
...
// calls fptr on each jl_typemap_entry_t in cache in sort order, until fptr
// return false
static int jl_typemap_node_visitor(
        jl_typemap_entry_t *ml, jl_typemap_visitor_fptr fptr, void *closure)
{
...
int jl_typemap_visitor(
        union jl_typemap_t cache, jl_typemap_visitor_fptr fptr, void *closure)
{
...
// predicate to fast-test if this type is a leaf type that can exist in the
// cache
// and does not need a more expensive linear scan to find all intersections
int is_cache_leaf(jl_value_t *ty)
{
    return (jl_is_datatype(ty) && ((jl_datatype_t *)ty)->uid != 0 &&
            !jl_is_kind(ty));
}

static int jl_typemap_intersection_array_visitor(
        struct jl_ordereddict_t *a,
        jl_value_t *ty,
        int tparam,
        int offs,
...
static int jl_typemap_intersection_node_visitor(
        jl_typemap_entry_t *ml, struct typemap_intersection_env *closure)
{
    ...
    if (jl_typeof(map.unknown) == (jl_value_t *)jl_typemap_level_type) {
        ...
        if (ty) {
            if (cache->targ.values != (void *)jl_nothing) {
                jl_value_t *typetype =
                        jl_is_type_type(ty) ? jl_tparam0(ty) : NULL;
                if (typetype && !jl_has_free_typevars(typetype)) {
                    if (is_cache_leaf(typetype)) {
                        // direct lookup of leaf types
                        union jl_typemap_t ml = mtcache_hash_lookup(
                                &cache->targ, typetype, 1, offs);
                        if (ml.unknown != jl_nothing) {
                    ...
                    // first, fast-path: optimized pre-intersection test to see
                    // if `ty` could intersect with any Type
                    if (typetype ||
                        !jl_has_empty_intersection(
                                (jl_value_t *)jl_type_type, ty))
                        if (!jl_typemap_intersection_array_visitor(
                                    &cache->targ, ty, 1, offs, closure))
                            return 0;
                }
            }
            if (cache->arg1.values != (void *)jl_nothing) {
                if (is_cache_leaf(ty)) {
                    // direct lookup of leaf types
                    union jl_typemap_t ml =
                            mtcache_hash_lookup(&cache->arg1, ty, 0, offs);
                    if (ml.unknown != jl_nothing) {
                        if (!jl_typemap_intersection_visitor(
                                    ml, offs + 1, closure))
                            return 0;
        ...
        if (!jl_typemap_intersection_node_visitor(map.node->linear, closure))
            return 0;
        return jl_typemap_intersection_visitor(
                map.node->any, offs + 1, closure);
    }
    else {
...
*/
static jl_typemap_entry_t *jl_typemap_assoc_by_type_(
        ...
        size_t world)
{
    ...
    for (; ml != (void *)jl_nothing; ml = ml->next) {
        ...
        size_t lensig =
                jl_field_count(jl_unwrap_unionall((jl_value_t *)ml->sig));
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            ...
            if (ml->simplesig != (void *)jl_nothing) {
                ...
                int isva = lensimplesig > 0 &&
                           jl_is_vararg_type(
                                   jl_tparam(ml->simplesig, lensimplesig - 1));
                if (lensig == n || (isva && lensimplesig <= n + 1))
                    ismatch = sig_match_by_type_simple(
                            jl_svec_data(types->parameters),
                            n,
                            ml->simplesig,
                            lensimplesig,
                            isva);
            ...
            if (ismatch == 0)
                ...
                ismatch = sig_match_by_type_leaf(
                        jl_svec_data(types->parameters), ml->sig, lensig);
                ...
                if (ismatch) {
                    ...
                    for (i = 0, l = jl_svec_len(*penv); i < l; i++) {
                        if (jl_is_typevar(jl_svecref(*penv, i))) {
                            if (inexact) {
                                // "inexact" means the given type is
                                // compile-time,
                    ...
                    if (inexact) {
                        // the compiler might attempt jl_get_specialization on
                        // e.g.
                        // convert(::Type{Type{Int}}, ::DataType), which is
                        // concrete but might not
                        // equal the run time type. in this case ti would be
...
static jl_typemap_entry_t *jl_typemap_lookup_by_type_(
        jl_typemap_entry_t *ml, jl_tupletype_t *types, size_t world)
{
    for (; ml != (void *)jl_nothing; ml = ml->next) {
        if (world < ml->min_world || world > ml->max_world)
            continue;
        ...
        assert(na > 0 && nb > 0);
        if (!jl_is_vararg_type(jl_tparam(a, na - 1)) &&
            !jl_is_vararg_type(jl_tparam(b, nb - 1))) {
            if (na != nb)
                continue;
        }
        if (na > 1 && nb > 1) {
...
jl_typemap_entry_t *jl_typemap_assoc_by_type(
        union jl_typemap_t ml_or_cache,
        jl_tupletype_t *types,
        jl_svec_t **penv,
        int8_t inexact,
        int8_t subtype,
        ...
        size_t world)
{
    if (jl_typeof(ml_or_cache.unknown) == (jl_value_t *)jl_typemap_level_type) {
        ...
        // called object is the primary key for constructors, otherwise first
        // argument
        jl_value_t *ty = NULL;
        ...
        if (!subtype) {
            if (ty && jl_is_any(ty))
                return jl_typemap_assoc_by_type(
                        cache->any,
                        types,
                        penv,
                        inexact,
                        ...
                        offs + 1,
                        world);
            if (isva) // in lookup mode, want to match Vararg exactly, not as a
                      // subtype
                ty = NULL;
        }
        if (ty) {
            if (jl_is_type_type(ty)) {
                ...
                if (cache->targ.values != (void *)jl_nothing &&
                    jl_is_datatype(a0)) {
                    union jl_typemap_t ml =
                            mtcache_hash_lookup(&cache->targ, a0, 1, offs);
                    if (ml.unknown != jl_nothing) {
                        jl_typemap_entry_t *li = jl_typemap_assoc_by_type(
                                ml,
                                types,
                                penv,
                                inexact,
            ...
            if (cache->arg1.values != (void *)jl_nothing &&
                ...
                if (ml.unknown != jl_nothing) {
                    jl_typemap_entry_t *li = jl_typemap_assoc_by_type(
                            ml, types, penv, inexact, subtype, offs + 1, world);
                    if (li)
                        return li;
                }
            }
            if (!subtype && is_cache_leaf(ty))
                return NULL;
        }
        // Always check the list (since offs doesn't always start at 0)
        if (subtype) {
            jl_typemap_entry_t *li = jl_typemap_assoc_by_type_(
                    cache->linear, types, inexact, penv, world);
            if (li)
                return li;
            return jl_typemap_assoc_by_type(
                    cache->any, types, penv, inexact, subtype, offs + 1, world);
        }
        else {
        ...
        return subtype ? jl_typemap_assoc_by_type_(
                                 ml_or_cache.leaf, types, inexact, penv, world)
                       : jl_typemap_lookup_by_type_(
                                 ml_or_cache.leaf, types, world);
    }
}

jl_typemap_entry_t *jl_typemap_entry_assoc_exact(
        jl_typemap_entry_t *ml, jl_value_t **args, size_t n, size_t world)
{
    // some manually-unrolled common special cases
    while (ml->simplesig == (void *)jl_nothing &&
           ml->guardsigs == jl_emptysvec && ml->isleafsig) {
        // use a tight loop for a long as possible
        if (world >= ml->min_world && world <= ml->max_world) {
            if (n == jl_field_count(ml->sig) &&
                jl_typeof(args[0]) == jl_tparam(ml->sig, 0)) {
                if (n == 1)
                    return ml;
                if (n == 2) {
                    ...
                    if (sig_match_leaf(
                                args, jl_svec_data(ml->sig->parameters), n))
                        return ml;
                }
            }
        }
        ...
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            if (ml->simplesig != (void *)jl_nothing) {
                ...
                if (lensig == n || (isva && lensimplesig <= n + 1)) {
                    if (!sig_match_simple(
                                args,
                                n,
                                jl_svec_data(ml->simplesig->parameters),
                                isva,
                                lensimplesig))
                        continue;
                }
            ...
            if (ml->isleafsig) {
                ...
                if (!sig_match_simple(
                            args,
                            n,
                            jl_svec_data(ml->sig->parameters),
                            ml->va,
                            lensig))
                    continue;
            }
            ...
            if (ml->guardsigs != jl_emptysvec) {
                for (i = 0, l = jl_svec_len(ml->guardsigs); i < l; i++) {
                    ...
                    // expensive subtype check, since guard entries added for
                    // ANY might be
                    // abstract. this fixed issue #12967.
                    if (jl_tuple_isa(
                                args,
                                n,
                                (jl_tupletype_t *)jl_svecref(
                                        ml->guardsigs, i))) {
                        goto nomatch;
                    }
                }
...
jl_typemap_entry_t *jl_typemap_level_assoc_exact(
        ...
        size_t world)
{
    if (n > offs) {
        ...
        if (ty == (jl_value_t *)jl_datatype_type &&
            cache->targ.values != (void *)jl_nothing) {
            union jl_typemap_t ml_or_cache =
                    mtcache_hash_lookup(&cache->targ, a1, 1, offs);
            jl_typemap_entry_t *ml = jl_typemap_assoc_exact(
                    ml_or_cache, args, n, offs + 1, world);
            if (ml)
                return ml;
        }
        if (cache->arg1.values != (void *)jl_nothing) {
    ...
    if (cache->linear != (jl_typemap_entry_t *)jl_nothing) {
        jl_typemap_entry_t *ml =
                jl_typemap_entry_assoc_exact(cache->linear, args, n, world);
        if (ml)
            return ml;
    }
        ...
        const struct jl_typemap_info *tparams)
{
    ...
    if (count > MAX_METHLIST_COUNT) {
        pml->node =
                jl_method_convert_list_to_cache(pml->leaf, key, offs, tparams);
...
{
    union jl_typemap_t *pml =
            mtcache_hash_bp(cache, key, tparam, offs, (jl_value_t *)parent);
    if (pml)
        jl_typemap_insert_generic(
                pml,
                (jl_value_t *)cache->values,
                newrec,
                key,
                offs + 1,
                tparams);
        ...
        const struct jl_typemap_info *tparams)
{
    ...
    // Don't put Varargs in the optimized caches (too hard to handle in lookup
    // and bp)
    if (t1 && !isva) {
    ...
    jl_typemap_list_insert_(
            &cache->linear, (jl_value_t *)cache, newrec, tparams);
}
        ...
        jl_value_t **overwritten)
{
    ...
    if ((jl_value_t *)simpletype == jl_nothing) {
        jl_typemap_entry_t *ml = jl_typemap_assoc_by_type(
                *cache, type, NULL, 1, 0, offs, min_world);
        if (ml && ml->simplesig == (void *)jl_nothing) {
            if (newvalue == ml->func.value) // no change. TODO: involve world in
                                            // computation!
                return ml;
            if (overwritten != NULL)
    ...
    newrec->issimplesig = !jl_is_unionall(
            type); // a TypeVar environment needs a complex matching test
    newrec->isleafsig =
            newrec->issimplesig &&
            !newrec->va; // entirely leaf types don't need to be sorted
    JL_GC_PUSH1(&newrec);
    ...
    for (i = 0, l = jl_field_count(ttype); i < l && newrec->issimplesig; i++) {
        ...
        if (decl == (jl_value_t *)jl_datatype_type)
            newrec->isleafsig =
                    0; // Type{} may have a higher priority than DataType
        else if (decl == (jl_value_t *)jl_unionall_type)
            newrec->isleafsig =
                    0; // Type{} may have a higher priority than UnionAll
        else if (jl_is_type_type(decl))
            newrec->isleafsig = 0; // Type{} may need special processing to
                                   // compute the match
        else if (jl_is_vararg_type(decl))
            newrec->isleafsig =
                    0; // makes iteration easier when the endpoints are the same
        else if (decl == (jl_value_t *)jl_any_type)
            newrec->isleafsig = 0;       // Any needs to go in the general cache
        else if (!jl_is_leaf_type(decl)) // anything else can go through the
                                         // general subtyping test
            newrec->isleafsig = newrec->issimplesig = 0;
    }
    // TODO: assert that guardsigs == jl_emptysvec && simplesig == jl_nothing if
    // isleafsig and optimize with that knowledge?
        ...
        const struct jl_typemap_info *tparams)
{
    ...
    while (l != (void *)jl_nothing) {
        if (!l->isleafsig) { // quickly ignore all of the leafsig entries (these
                             // were handled by caller)
            if (jl_type_morespecific(
                        (jl_value_t *)newrec->sig, (jl_value_t *)l->sig)) {
                if (l->simplesig == (void *)jl_nothing ||
                    newrec->simplesig != (void *)jl_nothing ||
                    !jl_types_equal(
                            (jl_value_t *)l->sig, (jl_value_t *)newrec->sig)) {
                    // might need to insert multiple entries for a lookup
                    // differing only by their simplesig
                    // when simplesig contains a kind
                    // TODO: make this test more correct or figure out a better
                    // way to compute this
                    break;
                }
            }
    ...
    *pl = newrec;
    ...
    if (has_unions((jl_value_t *)newrec->sig)) {
        ...
        while (item != (void *)jl_nothing) {
            ...
            while (l != newrec->next) {
                if (jl_type_morespecific(
                            (jl_value_t *)item->sig, (jl_value_t *)l->sig)) {
