...
static int sig_match_by_type_leaf(jl_value_t **types, jl_tupletype_t *sig, size_t n)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (!jl_types_equal(a, decl)) return 0;
    }
    return 1;
}
                                    ...
                                    size_t lensig, int va)
{
        ...
        if (jl_is_type_type(decl)) {
            ...
            if (jl_is_type_type(a)) {
                if (tp0 == (jl_value_t *)jl_typetype_tvar) {
                    ...
                    if (!jl_subtype(jl_tparam0(a), ((jl_tvar_t *)tp0)->ub, 0)) return 0;
                }
                else {
                    if (!jl_types_equal(jl_tparam0(a), tp0)) return 0;
                }
            }
            else if (!is_kind(a) || !jl_is_typevar(tp0) ||
    ...
    if (va) {
        ...
        if (jl_vararg_kind(decl) == JL_VARARG_INT) {
            if (n - i != jl_unbox_long(jl_tparam1(decl))) return 0;
        }
        jl_value_t *t = jl_tparam0(decl);
        for (; i < n; i++) {
            if (!jl_subtype(types[i], t, 0)) return 0;
        }
        return 1;
    }
    return 1;
...
static inline int sig_match_simple(jl_value_t **args, size_t n, jl_value_t **sig, int va,
                                   size_t lensig)
{
        ...
        if (decl == (jl_value_t *)jl_any_type) {
            ...
            */
            ...
            if (tp0 == (jl_value_t *)jl_typetype_tvar) {
                ...
                if (!jl_subtype(a, ((jl_tvar_t *)tp0)->ub, 0)) return 0;
            }
            else {
                if (a != tp0 && !jl_types_equal(a, tp0)) return 0;
            }
        }
        else {
            return 0;
    ...
    if (va) {
        ...
        jl_value_t *t = jl_tparam0(decl);
        for (; i < n; i++) {
            if (!jl_subtype(args[i], t, 1)) return 0;
        }
        return 1;
    }
    return 1;
...
static jl_array_t *jl_alloc_int_1d(size_t np, size_t len)
{
    ...
    if (np < 0xFF) {
        ...
        if (int16 == NULL) int16 = jl_apply_array_type(jl_uint16_type, 1);
        ty                       = int16;
    }
    else {
        assert(np < 0x7FFFFFFF);
        static jl_value_t *int32 = NULL;
        if (int32 == NULL) int32 = jl_apply_array_type(jl_uint32_type, 1);
        ty                       = int32;
    }
...
static inline union jl_typemap_t mtcache_hash_lookup(const struct jl_ordereddict_t *a,
                                                     jl_value_t *ty, int8_t tparam,
                                                     int8_t offs)
{
    ...
    if (idx > 0) {
        ...
        if (ml.unknown == jl_nothing) return ml;
        jl_value_t *t;
        ...
        if (t != ty) ml.unknown = jl_nothing;
    }
    return ml;
}

static void mtcache_rehash(struct jl_ordereddict_t *pa, size_t newlen, jl_value_t *parent,
                           int8_t tparam, int8_t offs)
{
    ...
    for (i = 1; i <= nval; i++) {
        ...
        if (ml.unknown == jl_nothing) continue;
        jl_value_t *t;
        if (jl_typeof(ml.unknown) == (jl_value_t *)jl_typemap_level_type) {
            t = ml.node->key;
        }
                                           ...
                                           int8_t tparam, int8_t offs, jl_value_t *parent)
{
    if (jl_is_datatype(ty)) {
        ...
        if (!uid || is_kind(ty) || jl_has_typevars(ty))
            // be careful not to put non-leaf types or DataType/TypeConstructor in the
            // cache here,
            // since they should have a lower priority and need to go into the sorted
            // list
            return NULL;
        ...
        while (1) {
            ...
            if (pml->unknown == jl_nothing) return pml;
            jl_value_t *t;
...
static int jl_typemap_array_visitor(struct jl_ordereddict_t *a,
                                    jl_typemap_visitor_fptr fptr, void *closure)
{
                                   ...
                                   void *closure)
{
    while (ml != (void *)jl_nothing) {
        if (!fptr(ml, closure)) return 0;
        ml = ml->next;
    }
    return 1;
}

int jl_typemap_visitor(union jl_typemap_t cache, jl_typemap_visitor_fptr fptr,
                       void *closure)
{
...
static int jl_typemap_intersection_array_visitor(struct jl_ordereddict_t *a, jl_value_t *ty,
                                                 int tparam, int offs,
        ...
        if (ty == (jl_value_t *)jl_any_type || // easy case: Any always matches
            (tparam ?                          // need to compute `ty <: Type{t}`
                 (jl_is_uniontype(ty) ||       // punt on Union{...} right now
                  jl_typeof(t) ==
                      ty || // deal with kinds (e.g. ty == DataType && t == Type{t})
                  (jl_is_type_type(ty) &&
                   (jl_is_typevar(jl_tparam0(ty))
                        ? jl_subtype(t, ((jl_tvar_t *)jl_tparam0(ty))->ub, 0)
                        : // deal with ty == Type{<:T}
                        jl_subtype(t, jl_tparam0(ty),
                                   0))))     // deal with ty == Type{T{#<:T}}
                    : jl_subtype(t, ty, 0))) // `t` is a leaftype, so intersection
                                             // test becomes subtype
                                                ...
                                                struct typemap_intersection_env *closure)
{
    ...
    while (ml != (void *)jl_nothing) {
        if (closure->type == (jl_value_t *)ml->sig) {
            ...
            if (closure->env) closure->env = ml->tvars;
            ...
            if (cache->targ.values != (void *)jl_nothing) {
                ...
                if (typetype && !jl_has_typevars(typetype)) {
                    if (is_cache_leaf(typetype)) {
                        ...
                        union jl_typemap_t ml =
                            mtcache_hash_lookup(&cache->targ, typetype, 1, offs);
                    ...
                    if (typetype ||
                        jl_type_intersection((jl_value_t *)jl_type_type, ty) !=
                            jl_bottom_type)
                        if (!jl_typemap_intersection_array_visitor(&cache->targ, ty, 1,
                                                                   offs, closure))
                            return 0;
                }
            }
...
int sigs_eq(jl_value_t *a, jl_value_t *b, int useenv)
{
    ...
    // useenv == 1 : subtyping + ensure typevars correspond + fail if bound !=
    // bound in some typevar match
    // useenv == 2 : ignore typevars (because UnionAll getting lost in
    // intersection can cause jl_types_equal to fail in the wrong direction for
    // some purposes)
...
*/
static jl_typemap_entry_t *jl_typemap_assoc_by_type_(jl_typemap_entry_t *ml,
                                                     jl_tupletype_t *types, int8_t inexact,
                                                     jl_svec_t **penv)
{
    ...
    while (ml != (void *)jl_nothing) {
        ...
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            ...
            if (ismatch == 0)
                ...
                ismatch = sig_match_by_type_leaf(jl_svec_data(types->parameters), ml->sig,
                                                 lensig);
                ...
                ismatch = jl_type_match((jl_value_t *)types, (jl_value_t *)ml->sig) !=
                          (jl_value_t *)jl_false;
            }
            else {
                ...
                jl_value_t *ti = jl_lookup_match((jl_value_t *)types, (jl_value_t *)ml->sig,
                                                 penv, ml->tvars);
                resetenv = 1;
                ...
                if (ismatch) {
                    ...
                    if (inexact) {
                        ...
                        // convert(::Type{Type{Int}}, ::DataType), which is concrete but
                        // might not
                        ...
                        JL_GC_POP();
                        if (!ismatch) return INEXACT_ENTRY;
                    }
                }
            }
            ...
            if (ismatch) {
                ...
                if (ismatch) return ml;
            }
            if (resetenv) *penv = jl_emptysvec;
        }
        ml = ml->next;
    }
    return NULL;
...
static jl_typemap_entry_t *jl_typemap_lookup_by_type_(jl_typemap_entry_t *ml,
                                                      jl_tupletype_t *types, int8_t useenv)
{
    while (ml != (void *)jl_nothing) {
        // TODO: more efficient
...
jl_typemap_entry_t *jl_typemap_assoc_by_type(union jl_typemap_t ml_or_cache,
                                             jl_tupletype_t *types, jl_svec_t **penv,
                                             int8_t subtype_inexact__sigseq_useenv,
                                             int8_t subtype, int8_t offs)
{
    if (jl_typeof(ml_or_cache.unknown) == (jl_value_t *)jl_typemap_level_type) {
        ...
        if (!subtype) {
            if (ty && jl_is_any(ty))
                return jl_typemap_assoc_by_type(cache->any, types, penv,
                                                subtype_inexact__sigseq_useenv, subtype,
                                                offs + 1);
        ...
        if (ty) {
                ...
                if (cache->targ.values != (void *)jl_nothing && jl_is_datatype(a0)) {
                    ...
                    if (ml.unknown != jl_nothing) {
                        jl_typemap_entry_t *li = jl_typemap_assoc_by_type(
                            ml, types, penv, subtype_inexact__sigseq_useenv, subtype,
                            offs + 1);
                        if (li) return li;
                    }
                }
            ...
            if (cache->arg1.values != (void *)jl_nothing && jl_is_datatype(ty)) {
                ...
                if (ml.unknown != jl_nothing) {
                    jl_typemap_entry_t *li = jl_typemap_assoc_by_type(
                        ml, types, penv, subtype_inexact__sigseq_useenv, subtype, offs + 1);
                    if (li) return li;
                }
            }
        ...
        if (subtype) {
            jl_typemap_entry_t *li = jl_typemap_assoc_by_type_(
                cache->linear, types, subtype_inexact__sigseq_useenv, penv);
            if (li) return li;
            return jl_typemap_assoc_by_type(
                cache->any, types, penv, subtype_inexact__sigseq_useenv, subtype, offs + 1);
        }
        else {
            return jl_typemap_lookup_by_type_(cache->linear, types,
                                              subtype_inexact__sigseq_useenv);
        }
    }
    else {
        return subtype ? jl_typemap_assoc_by_type_(ml_or_cache.leaf, types,
                                                   subtype_inexact__sigseq_useenv, penv)
                       : jl_typemap_lookup_by_type_(ml_or_cache.leaf, types,
                                                    subtype_inexact__sigseq_useenv);
    }
}

jl_typemap_entry_t *jl_typemap_entry_assoc_exact(jl_typemap_entry_t *ml, jl_value_t **args,
                                                 size_t n)
{
    ...
    while (ml->simplesig == (void *)jl_nothing && ml->guardsigs == jl_emptysvec &&
        ...
        if (n == jl_field_count(ml->sig) && jl_typeof(args[0]) == jl_tparam(ml->sig, 0)) {
            if (n == 1) return ml;
            if (n == 2) {
                if (jl_typeof(args[1]) == jl_tparam(ml->sig, 1)) return ml;
            }
            else if (n == 3) {
                ...
                if (sig_match_leaf(args, jl_svec_data(ml->sig->parameters), n)) return ml;
            }
        }
        ml = ml->next;
        if (ml == (void *)jl_nothing) return NULL;
    }
        ...
        if (lensig == n || (ml->va && lensig <= n + 1)) {
            if (ml->simplesig != (void *)jl_nothing) {
                ...
                int    isva         = lensimplesig > 0 &&
                           jl_is_vararg_type(jl_tparam(ml->simplesig, lensimplesig - 1));
                if (lensig == n || (isva && lensimplesig <= n + 1)) {
                    if (!sig_match_simple(args, n, jl_svec_data(ml->simplesig->parameters),
                                          isva, lensimplesig))
                        goto nomatch;
                }
                else {
            ...
            if (ml->isleafsig) {
                ...
                if (!sig_match_simple(args, n, jl_svec_data(ml->sig->parameters), ml->va,
                                      lensig))
                    goto nomatch;
            }
            else {
                if (!jl_tuple_subtype(args, n, ml->sig, 1)) goto nomatch;
            }
            ...
            if (ml->guardsigs != jl_emptysvec) {
                for (i = 0, l = jl_svec_len(ml->guardsigs); i < l; i++) {
                    ...
                    if (jl_tuple_subtype(
                            args, n, (jl_tupletype_t *)jl_svecref(ml->guardsigs, i), 1)) {
                        goto nomatch;
                    }
                }
            }
...
jl_typemap_entry_t *jl_typemap_level_assoc_exact(jl_typemap_level_t *cache,
                                                 jl_value_t **args, size_t n, int8_t offs)
{
    if (n > offs) {
...
static void jl_typemap_level_insert_(jl_typemap_level_t *cache, jl_typemap_entry_t *newrec,
...
static jl_typemap_level_t *jl_method_convert_list_to_cache(jl_typemap_entry_t *ml,
                                                           jl_value_t *key, int8_t offs)
{
...
static void jl_typemap_insert_generic(union jl_typemap_t *pml, jl_value_t *parent,
                                      jl_typemap_entry_t *newrec, jl_value_t *key,
                                      int8_t offs, const struct jl_typemap_info *tparams)
{
    if (jl_typeof(pml->unknown) == (jl_value_t *)jl_typemap_level_type) {
...
static int jl_typemap_array_insert_(struct jl_ordereddict_t *cache, jl_value_t *key,
                                    ...
                                    const struct jl_typemap_info *tparams)
{
    ...
    if (pml)
        jl_typemap_insert_generic(pml, (jl_value_t *)cache->values, newrec, key, offs + 1,
                                  tparams);
    return pml != NULL;
}
                                     ...
                                     int8_t offs, const struct jl_typemap_info *tparams)
{
    ...
    if (t1 && jl_is_any(t1))
        return jl_typemap_insert_generic(&cache->any, (jl_value_t *)cache, newrec,
                                         (jl_value_t *)jl_any_type, offs + 1, tparams);
    // Don't put Varargs in the optimized caches (too hard to handle in lookup and
    // bp)
    if (t1 && !isva) {
        ...
        if (t1 != (jl_value_t *)jl_typetype_type && jl_is_type_type(t1)) {
            ...
            if (jl_typemap_array_insert_(&cache->targ, a0, newrec, (jl_value_t *)cache, 1,
                                         offs, tparams))
                return;
        }
        if (jl_typemap_array_insert_(&cache->arg1, t1, newrec, (jl_value_t *)cache, 0, offs,
                                     tparams))
            return;
    }
                                      ...
                                      jl_value_t **                 overwritten)
{
    ...
    if ((jl_value_t *)simpletype == jl_nothing) {
        ...
        if (ml && ml->simplesig == (void *)jl_nothing) {
            if (overwritten != NULL) *overwritten = ml->func.value;
            ...
            if (newvalue) jl_gc_wb(ml, newvalue);
            // sigatomic end
            return ml;
        }
    }
    if (overwritten != NULL) *overwritten = NULL;
    ...
    newrec->issimplesig =
        (tvars == jl_emptysvec); // a TypeVar environment needs an complex matching test
    newrec->isleafsig =
    ...
    for (i = 0, l = jl_field_count(type); i < l && newrec->issimplesig; i++) {
        ...
        else if (!jl_is_leaf_type(decl)) // anything else can go through the general
                                         // subtyping test
...
static int has_unions(jl_tupletype_t *type)
{
    ...
    for (i = 0; i < jl_nparams(type); i++) {
        ...
        if (jl_is_uniontype(t) || (jl_is_vararg_type(t) && jl_is_uniontype(jl_tparam0(t))))
            return 1;
    }
    return 0;
}
        ...
        if (!l->isleafsig) { // quickly ignore all of the leafsig entries (these
            ...
            if (jl_args_morespecific((jl_value_t *)newrec->sig, (jl_value_t *)l->sig)) {
                if (l->simplesig == (void *)jl_nothing ||
                    ...
                    // might need to insert multiple entries for a lookup differing only
                    // by their simplesig
