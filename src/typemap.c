    ...
    while (ml != (void*)jl_nothing) {
        if (closure->type == (jl_value_t*)ml->sig) {
            ...
            if (closure->env) {
                ...
                ti = jl_type_intersection_matching(closure->type, (jl_value_t*)ml->sig, &closure->env);
            }
            else {
...
// determine if param appears in some position in types,
// which will force it to have a value for all leaf subtypes of `types`
int jl_is_param_bound(jl_value_t *types, jl_tvar_t *param)
{
    while (jl_is_unionall(types)) {
        jl_tvar_t *var = ((jl_unionall_t*)types)->var;
        if (var == param) {
            return 1;
        }
        types = ((jl_unionall_t*)types)->body;
    }
    if (jl_is_tuple_type(types)) {
        size_t i, n = jl_field_count(types);
        for (i = 0; i < n; i++) {
            jl_value_t *a = jl_tparam(types, i);
            if (jl_is_param_bound(a, param)) {
                return 1;
            }
        }
    }
    else if (jl_is_datatype(types) &&
            ((jl_datatype_t*)types)->name == jl_vararg_typename) {
        return (jl_is_param_bound(jl_tparam0(types), param));
    }
    return 0;
}
...
//
// as a subtype, pass in an `env` pointer to additionally constrain
// that all concrete subtypes of `types` must constrain all `tvars` in the return value
