...
void *mach_segv_listener(void *arg)
{
    (void)arg;
    while (1) {
        int ret = mach_msg_server(
                exc_server, 2048, segv_port, MACH_MSG_TIMEOUT_NONE);
...
static void allocate_segv_handler()
{
    ...
    ret = mach_port_insert_right(
            self, segv_port, segv_port, MACH_MSG_TYPE_MAKE_SEND);
    ...
    for (int16_t tid = 0; tid < jl_n_threads; tid++) {
        attach_exception_port(
                pthread_mach_thread_np(jl_all_tls_states[tid]->system_id), 0);
    }
}
...
static void jl_call_in_state(
        jl_ptls_t ptls2, x86_thread_state64_t *state, void (*fptr)(void))
{
...
static void
jl_throw_in_thread(int tid, mach_port_t thread, jl_value_t *exception)
{
    ...
    if (!ptls2->safe_restore) {
        ...
        ptls2->bt_size = rec_backtrace_ctx(
                ptls2->bt_data, JL_MAX_BT_SIZE, (bt_context_t *)&state);
        ...
        mach_msg_type_number_t code_count)
{
    ...
    if (thread == mach_profiler_thread) {
        return profiler_segv_handler(
                exception_port, thread, task, exception, code, code_count);
    }
#endif
    int16_t tid;
#ifdef JULIA_ENABLE_THREADING
    ...
    if (msync((void *)(fault_addr & ~(jl_page_size - 1)), 1, MS_ASYNC) ==
        ...
        if (is_addr_on_stack(ptls2, (void *)fault_addr)) {
            excpt = jl_stackovf_exception;
                ...
                msync((void *)(fault_addr & ~(jl_page_size - 1)),
                      1,
                      MS_ASYNC) != 0) {
            ...
            if (!(exc_state.__err & WRITE_FAULT))
                return KERN_INVALID_ARGUMENT; // rethrow the SEGV since it
                                              // wasn't an error with writing to
                                              // read-only memory
...
static void attach_exception_port(thread_port_t thread, int segv_only)
{
    ...
    ret = thread_set_exception_ports(
            thread, mask, segv_port, EXCEPTION_DEFAULT, MACHINE_THREAD_STATE);
...
static void jl_thread_suspend_and_get_state(int tid, unw_context_t **ctx)
{
    ...
    ret = thread_get_state(
            tid_port, x86_THREAD_STATE64, (thread_state_t)&state, &count);
        ...
        mach_msg_type_number_t code_count)
{
    ...
    kern_return_t ret = thread_set_state(
            thread, x86_THREAD_STATE64, (thread_state_t)&state, count);
    HANDLE_MACH_ERROR("thread_set_state", ret);
    ...
    while (1) {
        ...
        for (i = jl_n_threads; i-- > 0;) {
...
#ifdef LIBOSXUNWIND
            /*
             *  Unfortunately compact unwind info is incorrectly generated for
             * quite a number of
             *  libraries by quite a large number of compilers. We can fall back
             * to DWARF unwind info
             *  in some cases, but in quite a number of cases (especially
             * libraries not compiled in debug
             *  mode, only the compact unwind info may be available). Even more
             * unfortunately, there is no
             *  way to detect such bogus compact unwind info (other than
             * noticing the resulting segfault).
             *  What we do here is ugly, but necessary until the compact unwind
             * info situation improves.
             *  We try to use the compact unwind info and if that results in a
             * segfault, we retry with DWARF info.
             *  Note that in a small number of cases this may result in bogus
             * stack traces, but at least the topmost
             *  entry will always be correct, and the number of cases in which
             * this is an issue is rather small.
             *  Other than that, this implementation is not incorrect as the
             * other thread is paused while we are profiling
             *  and during stack unwinding we only ever read memory, but never
             * write it.
             */

            forceDwarf = 0;
            unw_getcontext(&profiler_uc); // will resume from this point if the
                                          // next lines segfault at any point
            ...
            if (forceDwarf == 0) {
                ...
                jl_safe_printf("WARNING: profiler attempt to access an invalid "
                               "memory location\n");
            }
            ...
            if (running) {
                // Reset the alarm
                kern_return_t ret = clock_alarm(
                        clk, TIME_RELATIVE, timerprof, profile_port);
...
JL_DLLEXPORT int jl_profile_start_timer(void)
{
    ...
    if (!profile_started) {
        ...
        ret = host_get_clock_service(
                mach_host_self(), SYSTEM_CLOCK, (clock_serv_t *)&clk);
        ...
        pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
        if (pthread_create(
                    &profiler_thread, &attr, mach_profile_listener, NULL) !=
            0) {
            jl_error("pthread_create failed");
        }
        pthread_attr_destroy(&attr);
