...
namespace {
    ...
    struct Optimizer {
        ...
        Optimizer(ArraylenOpt *pass) : pass(pass) {}
    ...
    struct ArraylenOpt : public FunctionPass, public JuliaPassContext {
        static char ID;
        Optimizer optimizer;
        ArraylenOpt() : FunctionPass(ID), optimizer(this) {
    ...
    char ArraylenOpt::ID = 0;
    static RegisterPass<ArraylenOpt> X("ArraylenOpt", "1D array length propagation and array length unsignedness",
...
{
    return new ArraylenOpt();
}

extern "C" JL_DLLEXPORT void LLVMExtraAddArraylenOptPass_impl(LLVMPassManagerRef PM)
{
    unwrap(PM)->add(createArraylenOptPass());
}