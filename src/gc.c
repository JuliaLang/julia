...
// Note that FOR_EACH_HEAP processes the master thread last.
// This is important for handilng roots that were queued while marking.
...
#define pools (jl_thread_heap->norm_pools)
...
static inline gcval_t *reset_page(pool_t *p, gcpage_t *pg, gcval_t *fl)
{
    ...
    pg->pool_n = p - jl_all_heaps[pg->thread_n]->norm_pools;
