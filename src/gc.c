...
NOINLINE uintptr_t gc_get_stack_ptr(void) { return (uintptr_t)jl_get_frame_addr(); }
...
static void jl_gc_run_finalizers_in_list(jl_ptls_t ptls, arraylist_t *list)
{
    ...
    for (size_t i = 2; i < len; i += 2) run_finalizer(ptls, items[i], items[i + 1]);
    JL_GC_POP();
}
...
static inline int gc_setmark_big(jl_ptls_t ptls, jl_taggedvalue_t *o, int mark_mode)
{
    if (gc_verifying) {
        o->bits.gc = mark_mode;
        return 0;
    ...
    if (mark_reset_age && !gc_marked(bits)) {
        ...
        if (gc_old(bits)) mark_mode = GC_OLD_MARKED;
...
static inline int gc_setmark_pool_(jl_ptls_t ptls, jl_taggedvalue_t *o, int mark_mode,
                                   region_t *r)
{
#ifdef MEMDEBUG
...
static inline int gc_setmark_pool(jl_ptls_t ptls, jl_taggedvalue_t *o, int mark_mode)
{
...
JL_DLLEXPORT jl_value_t *jl_gc_big_alloc(jl_ptls_t ptls, size_t sz)
{
    ...
    if (v == NULL) jl_throw(jl_memory_exception);
...
static void sweep_big(jl_ptls_t ptls, int sweep_full)
{
    ...
    if (sweep_full) {
        ...
        if (ptls->heap.big_objects) ptls->heap.big_objects->prev = last_next;
        *last_next                                               = ptls->heap.big_objects;
        ptls->heap.big_objects                                   = big_objects_marked;
        if (ptls->heap.big_objects) ptls->heap.big_objects->prev = &ptls->heap.big_objects;
...
static NOINLINE jl_taggedvalue_t *add_page(jl_gc_pool_t *p)
{
    ...
    if (data == NULL) jl_throw(jl_memory_exception);
...
static jl_taggedvalue_t **sweep_page(jl_gc_pool_t *p, jl_gc_pagemeta_t *pg,
                                     jl_taggedvalue_t **pfl, int sweep_full, int osize)
{
    char *            data      = pg->data;
...
static void gc_sweep_pool(int sweep_full)
{
    ...
    jl_taggedvalue_t ***pfl = (jl_taggedvalue_t ***)alloca(jl_n_threads * JL_GC_N_POOLS *
                                                           sizeof(jl_taggedvalue_t **));

    // update metadata of pages that were pointed to by freelist or newpages from
    // a pool
...
{
#ifdef JL_DEBUG_BUILD
    if (v == gc_findval) jl_raise_debugger();
#endif
    assert(v != NULL);
    jl_taggedvalue_t *o = jl_astaggedvalue(v);
    verify_val(v);
    int bits = o->bits.gc;
    if (!gc_marked(bits)) return !gc_old(push_root(ptls, (jl_value_t *)v, d, bits));
    return !gc_old(bits);
}
...
NOINLINE static int gc_mark_module(jl_ptls_t ptls, jl_module_t *m, int d)
{
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            gc_setmark_buf(ptls, b, jl_astaggedvalue(m)->bits.gc, sizeof(jl_binding_t));
            ...
            if (b->globalref != NULL) refyoung |= gc_push_root(ptls, b->globalref, d);
        }
    }
...
static void gc_mark_stack(jl_ptls_t ptls, jl_value_t *ta, jl_gcframe_t *s, intptr_t offset,
                          int d)
{
    while (s != NULL) {
...
static void gc_mark_task_stack(jl_ptls_t ptls, jl_task_t *ta, int d)
{
    ...
    if (stkbuf) {
#ifdef COPY_STACKS
        gc_setmark_buf(ptls, ta->stkbuf, jl_astaggedvalue(ta)->bits.gc, ta->bufsz);
#else
        // stkbuf isn't owned by julia for the root task
        if (ta != ptls2->root_task) {
            gc_setmark_buf(ptls, ta->stkbuf, jl_astaggedvalue(ta)->bits.gc, ta->ssize);
        }
#endif
    }
...
// mark v and recurse on its children (or store them on the mark stack when
// recursion depth becomes too high)
...
static int push_root(jl_ptls_t ptls, jl_value_t *v, int d, int bits)
{
    ...
    if (vt == (jl_value_t *)jl_simplevector_type) {
        ...
        if (d >= MAX_MARK_DEPTH) goto queue_the_root;
        ...
        if (a->flags.how == 2 && todo) {
            objprofile_count(jl_malloc_tag, o->bits.gc == GC_OLD_MARKED, array_nbytes(a));
        ...
        if (a->flags.how == 3) {
            ...
            (void)val_buf;
            gc_setmark_buf(ptls, (char *)a->data - a->offset * a->elsize, o->bits.gc,
                           array_nbytes(a));
        }
        if (a->flags.ptrarray && a->data != NULL) {
            ...
            if (l > 100000 && d > MAX_MARK_DEPTH - 10) {
                ...
                for (size_t i = 0; i < l; i++) {
                    ...
                    if (elt != NULL) {
                        verify_parent2("array", v, &((jl_value_t **)data)[i], "elem(%d)",
                                       (int)i);
                        refyoung |= gc_push_root(ptls, elt, d);
                    }
    ...
    // this check should not be needed but it helps catching corruptions early
        ...
        // TODO check if there is a perf improvement for objects with a lot of
        // fields
        ...
        for (int i = 0; i < nf; i++) {
            if (jl_field_isptr(dt, i)) {
                nptr++;
                jl_value_t **slot = (jl_value_t **)((char *)v + jl_field_offset(dt, i));
                jl_value_t * fld  = *slot;
                if (fld) {
...
void pre_mark(jl_ptls_t ptls)
{
    ...
    for (i = 0; i < jl_n_threads; i++) {
        ...
        // running.
        if (ptls2->current_module) gc_push_root(ptls, ptls2->current_module, 0);
    ...
    if (jl_an_empty_vec_any != NULL) gc_push_root(ptls, jl_an_empty_vec_any, 0);
    if (jl_module_init_order != NULL) gc_push_root(ptls, jl_module_init_order, 0);
    ...
    for (i = 0; i < N_CALL_CACHE; i++)
        if (call_cache[i]) gc_push_root(ptls, call_cache[i], 0);
...
JL_DLLEXPORT uint64_t jl_gc_total_hrtime(void) { return gc_num.total_time; }
JL_DLLEXPORT jl_gc_num_t jl_gc_num(void) { return gc_num; }
...
static void _jl_gc_collect(jl_ptls_t ptls, int full, char *stack_hi)
{
    ...
    if (!prev_sweep_full) promoted_bytes += perm_scanned_bytes - last_perm_scanned_bytes;
    // 5. next collection decision
    int not_freed_enough = estimate_freed < (7 * (actual_allocd / 10));
    int nptr             = 0;
    for (int i = 0; i < jl_n_threads; i++) nptr += jl_all_tls_states[i]->heap.remset_nptr;
    int      large_frontier =
        nptr * sizeof(void *) >= default_collect_interval; // many pointers in the
                                                           // intergen frontier => "quick"
                                                           // mark is not quick
    int sweep_full;
    int recollect = 0;
    if ((full || large_frontier ||
        ...
        recollect                           = full;
        if (large_frontier) gc_num.interval = last_long_collect_interval;
    ...
    if (sweep_full) perm_scanned_bytes = 0;
    scanned_bytes                      = 0;
    // 5. start sweeping
    sweep_weak_refs();
...
void jl_gc_init(void)
{
    ...
    if (maxmem > max_collect_interval) max_collect_interval = maxmem;
#endif
}
...
JL_DLLEXPORT void *jl_gc_counted_malloc(size_t sz)
{
    ...
    gc_num.malloc++;
    void *b = malloc(sz);
    if (b == NULL) jl_throw(jl_memory_exception);
    return b;
}
...
// The caller should have acquired `gc_perm_lock`
    ...
    if (__unlikely(sz > gc_perm_size)) {
#ifdef _OS_WINDOWS_
        void *pool = VirtualAlloc(NULL, GC_PERM_POOL_SIZE + JL_SMALL_BYTE_ALIGNMENT,
                                  MEM_COMMIT, PAGE_READWRITE);
        if (__unlikely(pool == NULL)) return NULL;
        ...
        if (__unlikely(pool == MAP_FAILED)) return NULL;
#endif
