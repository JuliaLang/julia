...
NOINLINE static void gc_mark_task(jl_ptls_t ptls, jl_task_t *ta, int d)
{
    ...
    if (ta->backtrace)
        gc_push_root(ptls, ta->backtrace, d);
    if (ta->start)
        gc_push_root(ptls, ta->start, d);
    if (ta->result)
        gc_push_root(ptls, ta->result, d);
...
// mark v and recurse on its children (or store them on the mark stack when recursion depth
// becomes too high)
