...
struct ReadClobberInfo {
    bool HasReadInSourceBlock = false;
};
                                                        ...
                                                        ReadClobberInfo &RCI)
{
    ...
    while (!Worklist.empty()) {
        ...
        // For source block, any read that aliases our write blocks sinking
        if (MABB == CurrentBB) {
            // Check if this read aliases our write location
            ModRefInfo MRI = BatchAA.getModRefInfo(MemInst, WriteLoc);
            if (isRefSet(MRI)) {
                // This read depends on our write - can't sink
                RCI.HasReadInSourceBlock = true;
                return nullptr;
            }
