...
static uintptr_t NOINLINE hash_svec(jl_svec_t *v, htable_t *object_table) JL_NOTSAFEPOINT
{
    ...
    for (i = 0; i < l; i++) {
        ...
        uintptr_t u = (x == NULL) ? 0 : jl_object_id_(jl_typetagof(x), x, object_table);
        h = bitmix(h, u);
    }
    return h;
}
...
static uintptr_t type_object_id_(jl_value_t *v, jl_varidx_t *env, htable_t *object_table) JL_NOTSAFEPOINT
{
    ...
    if (tv == jl_uniontype_type) {
        return bitmix(bitmix(jl_object_id_(jl_typetagof((jl_value_t*)tv), (jl_value_t*)tv, object_table),
...
static uintptr_t immut_id_(jl_datatype_t *dt, jl_value_t *v, uintptr_t h, htable_t *object_table) JL_NOTSAFEPOINT
{
    ...
    for (f = 0; f < nf; f++) {
        ...
        if (jl_field_isptr(dt, f)) {
            ...
            u = (f == NULL) ? 0 : jl_object_id_(jl_typetagof(f), f, object_table);
        }
        else {
...
static uintptr_t NOINLINE jl_object_id__cold(uintptr_t tv, jl_value_t *v, htable_t *object_table) JL_NOTSAFEPOINT
{
...
JL_DLLEXPORT inline uintptr_t jl_object_id_(uintptr_t tv, jl_value_t *v, htable_t *object_table) JL_NOTSAFEPOINT
{
...
JL_DLLEXPORT uintptr_t jl_object_id(jl_value_t *v) JL_NOTSAFEPOINT
{
    ...
    uintptr_t hash_val = jl_object_id_(jl_typetagof(v), v, &object_table);
...
JL_CALLABLE(jl_f_svec)
{
    ...
    for (i = 0; i < nargs; i++) {
        jl_gc_write_fresh(t, jl_svec_data(t)[i], jl_value_t, args[i]);
    }
    return (jl_value_t*)t;
}
