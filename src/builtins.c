...
static void JL_NORETURN jl_vexceptionf(jl_datatype_t *exception_type, const char *fmt,
                                       va_list args)
{
                                               ...
                                               jl_value_t *ty, jl_value_t *got)
{
    ...
    jl_value_t *ex = jl_new_struct(jl_typeerror_type, jl_symbol(fname), ctxt, ty, got);
    jl_throw(ex);
}
...
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (__unlikely(n <= 0)) return;
    jl_handler_t *eh   = ptls->current_task->eh;
    while (--n > 0) eh = eh->prev;
...
static int NOINLINE compare_svec(jl_svec_t *a, jl_svec_t *b)
{
    ...
    if (l != jl_svec_len(b)) return 0;
    for (size_t i = 0; i < l; i++) {
        if (!jl_egal(jl_svecref(a, i), jl_svecref(b, i))) return 0;
    }
    return 1;
}
...
JL_DLLEXPORT int jl_egal(jl_value_t *a, jl_value_t *b)
{
    ...
    if (ta != (jl_value_t *)jl_typeof(b)) return 0;
    if (jl_is_svec(a)) return compare_svec((jl_svec_t *)a, (jl_svec_t *)b);
    ...
    if (sz == 0) return 1;
    size_t nf = jl_datatype_nfields(dt);
    if (nf == 0) return bits_equal(jl_data_ptr(a), jl_data_ptr(b), sz);
...
{
    JL_NARGS(is, 2, 2);
    if (args[0] == args[1]) return jl_true;
...
JL_CALLABLE(jl_f_issubtype)
{
    ...
    if (!jl_is_typevar(args[0])) JL_TYPECHK(subtype, type, args[0]);
    if (!jl_is_typevar(args[1])) JL_TYPECHK(subtype, type, args[1]);
...
{
    if (!jl_subtype(x, t, 1)) jl_type_error("typeassert", t, x);
}
...
JL_CALLABLE(jl_f_typeassert)
{
    ...
    if (!jl_subtype(args[0], args[1], 1)) jl_type_error("typeassert", args[1], args[0]);
    return args[0];
}
...
JL_CALLABLE(jl_f__apply)
{
    ...
    if (nargs == 2) {
        if (f == jl_builtin_svec) {
            if (jl_is_svec(args[1])) return args[1];
    ...
    for (i = 1; i < nargs; i++) {
        ...
        if (jl_is_svec(ai)) {
            ...
            for (j = 0; j < al; j++) newargs[n++] = jl_svecref(t, j);
        }
            ...
            for (j = 0; j < al; j++) {
                ...
                if (arg_heap) jl_gc_wb(arg_heap, newargs[n - 1]);
            }
        }
        else {
            ...
            if (aai->flags.ptrarray) {
                for (j = 0; j < al; j++) {
                    ...
                    // #11772
                    if (__unlikely(arg == NULL)) jl_throw(jl_undefref_exception);
                    newargs[n++] = arg;
                }
            }
            else {
...
jl_value_t *jl_toplevel_eval_in_warn(jl_module_t *m, jl_value_t *ex, int delay_warn)
{
    ...
    if (m == NULL) m           = jl_main_module;
    if (jl_is_symbol(ex)) return jl_eval_global_var(m, (jl_sym_t *)ex);
    jl_value_t * v           = NULL;
    ...
    if (!delay_warn && jl_options.incremental && jl_generating_output()) {
        if (m != last_m) {
            ...
            jl_printf(JL_STDERR, "\n  ** incremental compilation may be broken for "
                                 "this module **\n\n");
        }
    ...
    JL_TRY {
        jl_warn_on_eval =
            delay_warn &&
            (jl_warn_on_eval || m != last_m); // compute whether a warning was suppressed
    ...
    if (nargs != 2) {
        ...
        if (!jl_is_module(args[0])) {
            ...
            if (jl_is_long(args[1])) {
                ...
                if (idx >= jl_datatype_nfields(vt)) return jl_false;
            }
            else {
                JL_TYPECHK(isdefined, symbol, args[1]);
                idx = jl_field_index(vt, (jl_sym_t *)args[1], 0);
                if ((int)idx == -1) return jl_false;
            }
...
JL_CALLABLE(jl_f_tuple)
{
    ...
    if (nargs < jl_page_size / sizeof(jl_value_t *)) {
        ...
        for (i = 0; i < nargs; i++) types[i] = jl_typeof(args[i]);
        tt                                   = jl_inst_concrete_tupletype_v(types, nargs);
    }
    else {
        jl_svec_t *types = jl_alloc_svec_uninit(nargs);
        JL_GC_PUSH1(&types);
        for (i = 0; i < nargs; i++) jl_svecset(types, i, jl_typeof(args[i]));
...
JL_CALLABLE(jl_f_getfield)
{
    ...
    if (!jl_is_datatype(vt)) jl_type_error("getfield", (jl_value_t *)jl_datatype_type, v);
    jl_datatype_t *st = (jl_datatype_t *)vt;
    size_t         idx;
    if (jl_is_long(args[1])) {
        idx = jl_unbox_long(args[1]) - 1;
        if (idx >= jl_datatype_nfields(st)) jl_bounds_error(args[0], args[1]);
    }
    else {
        JL_TYPECHK(getfield, symbol, args[1]);
    ...
    if (fval == NULL) jl_throw(jl_undefref_exception);
    return fval;
}
...
JL_CALLABLE(jl_f_setfield)
{
    ...
    if (!jl_is_datatype(vt)) jl_type_error("setfield!", (jl_value_t *)jl_datatype_type, v);
    jl_datatype_t *st = (jl_datatype_t *)vt;
    if (!st->mutabl) jl_errorf("type %s is immutable", jl_symbol_name(st->name->name));
    size_t idx;
    if (jl_is_long(args[1])) {
        idx = jl_unbox_long(args[1]) - 1;
        if (idx >= jl_datatype_nfields(st)) jl_bounds_error(args[0], args[1]);
...
JL_CALLABLE(jl_f_fieldtype)
{
    ...
    if (st == jl_module_type) jl_error("cannot assign variables in other modules");
...
JL_CALLABLE(jl_f_nfields)
{
    ...
    jl_value_t *x             = args[0];
    if (!jl_is_datatype(x)) x = jl_typeof(x);
    return jl_box_long(jl_field_count(x));
}

// conversion -----------------------------------------------------------------

JL_DLLEXPORT void *(jl_symbol_name)(jl_sym_t *s) { return jl_symbol_name(s); }

// WARNING: THIS FUNCTION IS NEVER CALLED BUT INLINE BY CCALL
JL_DLLEXPORT void *jl_array_ptr(jl_array_t *a) { return a->data; }
JL_DLLEXPORT jl_value_t *jl_value_ptr(jl_value_t *a) { return a; }
...
JL_DLLEXPORT jl_nullable_float64_t jl_try_substrtod(char *str, size_t offset, size_t len)
{
    ...
    if (__unlikely(tofree)) free(tofree);
...
JL_CALLABLE(jl_f_applicable)
{
    ...
    return jl_method_lookup(jl_gf_mtable(args[0]), args, nargs, 1) != NULL ? jl_true
                                                                           : jl_false;
}
...
{
    if (tv == (jl_value_t *)jl_sym_type) return ((jl_sym_t *)v)->hash;
    if (tv == (jl_value_t *)jl_simplevector_type) return hash_svec((jl_svec_t *)v);
    jl_datatype_t *dt = (jl_datatype_t *)tv;
    if (dt == jl_datatype_type) {
        jl_datatype_t *dtv = (jl_datatype_t *)v;
        return bitmix(~dtv->name->hash, hash_svec(dtv->parameters));
    }
    if (dt == jl_typename_type) return ((jl_typename_t *)v)->hash;
                                ...
                                struct recur_list *depth)
{
    ...
    if ((uintptr_t)vt < 4096U) {
        ...
        if (nlsep && tlen > 1) n += jl_printf(out, "\n  ");
        for (j = 0; j < tlen; j++) {
            if (av->flags.ptrarray) {
        ...
        if (!istuple) n += jl_static_show_x(out, (jl_value_t *)vt, depth);
        n += jl_printf(out, "(");
        ...
        if (nb > 0 && tlen == 0) {
            ...
            n += jl_printf(out, "0x");
            for (int i = nb - 1; i >= 0; --i) n += jl_printf(out, "%02" PRIx8, data[i]);
        }
        else {
            for (size_t i = 0; i < tlen; i++) {
                if (!istuple) {
                    n += jl_printf(out, "%s", jl_symbol_name((jl_sym_t *)jl_svecref(
                                                  vt->name->names, i)));
                ...
                if (jl_field_isptr(vt, i)) {
                    ...
                    n += jl_static_show_x_(out, (jl_value_t *)fld_ptr,
                                           (jl_datatype_t *)jl_field_type(vt, i), depth);
                }
                if (istuple && tlen == 1)
                    n += jl_printf(out, ",");
                else if (i != tlen - 1)
...
static size_t jl_static_show_x(JL_STREAM *out, jl_value_t *v, struct recur_list *depth)
{
    ...
    while (p) {
        if (p->v == v) return jl_printf(out, "<circular reference @-%u>", dist);
        dist++;
        p = p->prev;
    }
...
JL_DLLEXPORT size_t jl_static_show_func_sig(JL_STREAM *s, jl_value_t *type)
{
    ...
    if (ftype == NULL) return jl_static_show(s, type);
    size_t n = 0;
