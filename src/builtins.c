...
static int NOINLINE
compare_fields(jl_value_t *a, jl_value_t *b, jl_datatype_t *dt)
{
...
static uintptr_t bits_hash(void *b, size_t sz)
{
    switch (sz) {
    case 1:
        return int32hash(*(int8_t *)b);
    case 2:
        return int32hash(*(int16_t *)b);
    case 4:
        return int32hash(*(int32_t *)b);
#ifdef _P64
    case 8:
        return int64hash(*(int64_t *)b);
#else
    case 8:
        return int64to32hash(*(int64_t *)b);
#endif
    default:
#ifdef _P64
...
static uintptr_t jl_object_id_(jl_value_t *tv, jl_value_t *v)
{
    ...
    for (size_t f = 0; f < nf; f++) {
        ...
        if (jl_field_isptr(dt, f)) {
            ...
            assert(jl_is_datatype(fieldtype) && !fieldtype->abstract &&
                   !fieldtype->mutabl);
...
JL_CALLABLE(jl_f_issubtype)
{
    ...
    if (jl_is_typevar(a))
        a = ((jl_tvar_t *)a)->ub; // TODO should we still allow this?
    if (jl_is_typevar(b))
        b = ((jl_tvar_t *)b)->ub;
    ...
    for (i = 1; i < nargs; i++) {
        if (jl_is_svec(args[i])) {
            ...
            if (jl_append_any_func == NULL) {
                jl_append_any_func = (jl_function_t *)jl_get_global(
                        jl_top_module, jl_symbol("append_any"));
                if (jl_append_any_func == NULL) {
            ...
            args[0] = f;
            jl_value_t *result =
                    jl_apply(jl_array_ptr_data(argarr), jl_array_len(argarr));
            JL_GC_POP();
            return result;
        }
    }
...
JL_DLLEXPORT jl_value_t *jl_toplevel_eval_in(jl_module_t *m, jl_value_t *ex)
{
    ...
    if (jl_options.incremental && jl_generating_output()) {
        if (m != last_m) {
            jl_printf(
                    JL_STDERR,
                    "WARNING: eval from module %s to %s:    \n",
                    jl_symbol_name(m->name),
                    jl_symbol_name(last_m->name));
            jl_static_show(JL_STDERR, ex);
            jl_printf(JL_STDERR, "\n  ** incremental compilation may be broken "
                                 "for this module **\n\n");
        }
    }
    JL_TRY {
...
JL_CALLABLE(jl_f_isdefined)
{
    ...
    if (nargs != 2) {
        ...
        jl_depwarn(
                "`isdefined(:symbol)` is deprecated, "
        ...
        if (!jl_is_module(args[0])) {
            ...
            if (!jl_is_datatype(vt)) {
                jl_type_error(
                        "isdefined", (jl_value_t *)jl_datatype_type, args[0]);
            }
            size_t idx;
...
JL_CALLABLE(jl_f_tuple)
{
    ...
    if (nargs < jl_page_size / sizeof(jl_value_t *)) {
        jl_value_t **types =
                (jl_value_t **)alloca(nargs * sizeof(jl_value_t *));
        for (i = 0; i < nargs; i++)
...
static jl_value_t *get_fieldtype(jl_value_t *t, jl_value_t *f)
{
    ...
    if (!jl_is_datatype(st))
        jl_type_error(
                "fieldtype", (jl_value_t *)jl_datatype_type, (jl_value_t *)st);
    int field_index;
    if (jl_is_long(f)) {
...
JL_CALLABLE(jl_f_apply_type)
{
    ...
    if (args[0] == (jl_value_t *)jl_anytuple_type) {
        for (i = 1; i < nargs; i++) {
            ...
            if (jl_is_vararg_type(pi)) {
                ...
                jl_type_error_rt(
                        "Tuple", "parameter", (jl_value_t *)jl_type_type, pi);
            }
        }
        ...
        // substituting typevars (a valid_type_param check here isn't
        // sufficient).
    ...
    jl_type_error(
            "Type{...} expression", (jl_value_t *)jl_unionall_type, args[0]);
}
...
static void jl_check_type_tuple(jl_value_t *t, jl_sym_t *name, const char *ctx)
{
    if (!jl_is_tuple_type(t))
        jl_type_error_rt(
                jl_symbol_name(name), ctx, (jl_value_t *)jl_type_type, t);
}
...
JL_CALLABLE(jl_f_applicable)
{
    ...
    return jl_method_lookup(jl_gf_mtable(args[0]), args, nargs, 1, world) !=
                           NULL
                   ? jl_true
                   : jl_false;
}
...
JL_CALLABLE(jl_f_invoke)
{
    ...
    if (jl_is_tuple(args[1])) {
        jl_depwarn(
                "`invoke(f, (types...), ...)` is deprecated, "
                "use `invoke(f, Tuple{types...}, ...)` instead",
                (jl_value_t *)jl_symbol("invoke"));
        argtypes = (jl_value_t *)jl_apply_tuple_type_v(
                (jl_value_t **)jl_data_ptr(argtypes), jl_nfields(argtypes));
    }
    else {
    ...
    jl_value_t *res =
            jl_gf_invoke((jl_tupletype_t *)argtypes, &args[1], nargs - 1);
    JL_GC_POP();
    return res;
}
...
jl_expr_t *jl_exprn(jl_sym_t *head, size_t n)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_array_t *ar =
            n == 0 ? (jl_array_t *)jl_an_empty_vec_any : jl_alloc_vec_any(n);
    JL_GC_PUSH1(&ar);
    jl_expr_t *ex =
            (jl_expr_t *)jl_gc_alloc(ptls, sizeof(jl_expr_t), jl_expr_type);
    ex->head = head;
    ex->args = ar;
...
static size_t array_nd_index(
        jl_array_t *a, jl_value_t **args, size_t nidxs, const char *fname)
{
    size_t i = 0;
    size_t k, stride = 1;
    size_t nd = jl_array_ndims(a);
...
static void
add_intrinsic_properties(enum intrinsic f, unsigned nargs, void (*pfunc)(void))
{
...
void jl_init_intrinsic_functions(void)
{
    ...
    jl_mk_builtin_func(
            jl_intrinsic_type, "IntrinsicFunction", jl_f_intrinsic_call);
...
void jl_init_primitives(void)
{
    ...
    jl_value_t *ikws = jl_new_generic_function_with_supertype(
            itn->name, jl_core_module, jl_builtin_type, 1);
    itn->mt->kwsorter = ikws;
    jl_gc_wb(itn->mt, ikws);
    jl_mk_builtin_func(
            (jl_datatype_t *)jl_typeof(ikws),
            jl_symbol_name(jl_gf_name(ikws)),
