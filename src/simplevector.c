...
JL_DLLEXPORT jl_svec_t *jl_svec(size_t n, ...)
{
    ...
    for (size_t i = 0; i < n; i++) jl_svecset(jv, i, va_arg(args, jl_value_t *));
    va_end(args);
    return jv;
}
...
JL_DLLEXPORT jl_svec_t *jl_svec1(void *a)
{
    ...
    jl_svec_t *v = (jl_svec_t *)jl_gc_alloc(ptls, sizeof(void *) * 2, jl_simplevector_type);
...
{
    jl_ptls_t  ptls = jl_get_ptls_states();
    jl_svec_t *v = (jl_svec_t *)jl_gc_alloc(ptls, sizeof(void *) * 3, jl_simplevector_type);
...
JL_DLLEXPORT jl_svec_t *jl_alloc_svec_uninit(size_t n)
{
    ...
    if (n == 0) return jl_emptysvec;
    jl_svec_t *jv =
        (jl_svec_t *)jl_gc_alloc(ptls, (n + 1) * sizeof(void *), jl_simplevector_type);
...
JL_DLLEXPORT jl_svec_t *jl_alloc_svec(size_t n)
{
    if (n == 0) return jl_emptysvec;
    jl_svec_t *jv = jl_alloc_svec_uninit(n);
    for (size_t i = 0; i < n; i++) jl_svecset(jv, i, NULL);
    return jv;
}
...
JL_DLLEXPORT jl_svec_t *jl_svec_copy(jl_svec_t *a)
{
    ...
    for (i = 0; i < n; i++) jl_svecset(c, i, jl_svecref(a, i));
    return c;
}
...
JL_DLLEXPORT jl_svec_t *jl_svec_fill(size_t n, jl_value_t *x)
{
    ...
    for (size_t i = 0; i < n; i++) jl_svecset(v, i, x);
    return v;
}
