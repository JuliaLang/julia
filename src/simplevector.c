...
jl_svec_t *jl_perm_symsvec(size_t n, ...)
{
    if (n == 0)
        return jl_emptysvec;
    jl_svec_t *jv = (jl_svec_t *)jl_gc_permobj(
            (n + 1) * sizeof(void *), jl_simplevector_type);
...
JL_DLLEXPORT jl_svec_t *jl_svec1(void *a)
{
    ...
    jl_svec_t *v = (jl_svec_t *)jl_gc_alloc(
            ptls, sizeof(void *) * 2, jl_simplevector_type);
...
JL_DLLEXPORT jl_svec_t *jl_svec2(void *a, void *b)
{
    jl_ptls_t ptls = jl_get_ptls_states();
    jl_svec_t *v = (jl_svec_t *)jl_gc_alloc(
            ptls, sizeof(void *) * 3, jl_simplevector_type);
...
JL_DLLEXPORT jl_svec_t *jl_alloc_svec_uninit(size_t n)
{
    ...
    if (n == 0)
        return jl_emptysvec;
    jl_svec_t *jv = (jl_svec_t *)jl_gc_alloc(
            ptls, (n + 1) * sizeof(void *), jl_simplevector_type);
    jl_svec_set_len_unsafe(jv, n);
    return jv;
}
