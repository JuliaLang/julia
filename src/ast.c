...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, jl_module_t *mod, int formonly);
...
static jl_value_t *scm_to_julia(fl_context_t *fl_ctx, value_t e, jl_module_t *mod)
{
    ...
    JL_GC_PUSH1(&v);
    JL_TRY {
        v = scm_to_julia_(fl_ctx, e, mod, 0);
    }
    JL_CATCH {
...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, jl_module_t *mod, int formonly)
{
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (sym == line_sym && (n == 1 || n == 2)) {
            jl_value_t *linenum = scm_to_julia_(fl_ctx, car_(e), mod, formonly);
            jl_value_t *file = jl_nothing;
            JL_GC_PUSH2(&linenum, &file);
            if (n == 2)
                file = scm_to_julia_(fl_ctx, car_(cdr_(e)), mod, formonly);
        ...
        JL_GC_PUSH1(&ex);
        if (sym == goto_sym) {
            ex = scm_to_julia_(fl_ctx, car_(e), mod, formonly);
        ...
        else if (sym == globalref_sym && !formonly) {
            ex = scm_to_julia_(fl_ctx, car_(e), mod, formonly);
            temp = scm_to_julia_(fl_ctx, car_(cdr_(e)), mod, formonly);
            assert(jl_is_module(ex));
            assert(jl_is_symbol(temp));
            temp = jl_module_globalref((jl_module_t*)ex, (jl_sym_t*)temp);
        }
        else if (sym == top_sym && !formonly) {
        ...
        else if (sym == thunk_sym && !formonly) {
            ex = scm_to_julia_(fl_ctx, car_(e), mod, formonly);
