...
value_t fl_defined_julia_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (ptls->current_module == NULL) return fl_ctx->F;
    jl_sym_t *    var = jl_symbol(symbol_name(fl_ctx, args[0]));
    jl_binding_t *b   = (jl_binding_t *)ptrhash_get(&ptls->current_module->bindings, var);
...
value_t fl_invoke_julia_macro(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    jl_ptls_t ptls = jl_get_ptls_states();
    if (nargs < 1) argcount(fl_ctx, "invoke-julia-macro", nargs, 1);
...
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (jl_ast_ctx_using || jl_ast_ctx_freed) return;
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (!eo) {
            ...
            if (sym == top_sym) {
                ...
                temp = jl_module_globalref(jl_base_relative_to(ptls->current_module),
                                           (jl_sym_t *)scmv);
                JL_GC_POP();
                return temp;
            }
        ...
        JL_GC_POP();
        if (sym == list_sym) return (jl_value_t *)((jl_expr_t *)ex)->args;
...
static value_t julia_to_scm(fl_context_t *fl_ctx, jl_value_t *v)
{
    ...
    {
        temp = fl_list2(fl_ctx, jl_ast_ctx(fl_ctx)->error_sym,
                        cvalue_static_cstring(fl_ctx, "expression too large"));
    }
    return temp;
}
...
static value_t julia_to_scm_(fl_context_t *fl_ctx, jl_value_t *v)
{
    if (jl_is_symbol(v)) return symbol(fl_ctx, jl_symbol_name((jl_sym_t *)v));
    if (v == jl_true) return jl_ast_ctx(fl_ctx)->true_sym;
    if (v == jl_false) return jl_ast_ctx(fl_ctx)->false_sym;
    if (v == jl_nothing) return fl_cons(fl_ctx, jl_ast_ctx(fl_ctx)->null_sym, fl_ctx->NIL);
    if (jl_is_expr(v)) {
    ...
    if (jl_is_long(v) && fits_fixnum(jl_unbox_long(v))) return fixnum(jl_unbox_long(v));
    if (jl_is_ssavalue(v)) jl_error("SSAValue objects should not occur in an AST");
    if (jl_is_slot(v)) jl_error("Slot objects should not occur in an AST");
    value_t opaque = cvalue(fl_ctx, jl_ast_ctx(fl_ctx)->jvtype, sizeof(void *));
                                             ...
                                             const char *filename, size_t filename_len)
{
    ...
    value_t           e =
        fl_applyn(fl_ctx, 2, symbol_value(symbol(fl_ctx, "jl-parse-string")), s, files);
...
JL_DLLEXPORT jl_value_t *jl_parse_string(const char *str, size_t len, int pos0, int greedy)
{
    ...
    fl_context_t *    fl_ctx = &ctx->fl;
    value_t           s      = cvalue_static_cstrn(fl_ctx, str, len);
    value_t p = fl_applyn(fl_ctx, 3, symbol_value(symbol(fl_ctx, "jl-parse-one-string")), s,
...
{
    jl_ptls_t ptls = jl_get_ptls_states();
    if (in_pure_callback) jl_error("cannot use include inside a generated function");
    jl_ast_context_t *ctx    = jl_ast_ctx_enter();
    fl_context_t *    fl_ctx = &ctx->fl;
    value_t           f, ast;
    ...
    if (content != NULL) {
        ...
        ast = fl_applyn(fl_ctx, 2, symbol_value(symbol(fl_ctx, "jl-parse-string-stream")),
                        t, f);
        fl_free_gc_handles(fl_ctx, 1);
    }
    else {
    ...
    JL_TRY {
        ...
        while (iscons(ast)) {
            value_t expansion;
            {
                JL_TIMING(LOWERING);
                expansion =
                    fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, "jl-expand-to-thunk")),
                              car_(ast));
            }
    ...
    if (err) {
        if (jl_loaderror_type == NULL)
            ...
            jl_rethrow_other(
                jl_new_struct(jl_loaderror_type, form, result, ptls->exception_in_transit));
    }
    JL_GC_POP();
    return result;
}
...
JL_DLLEXPORT int jl_is_rest_arg(jl_value_t *ex)
{
    ...
    if (((jl_expr_t *)atype)->head == dots_sym) return 1;
    if (atype->head != call_sym || jl_array_len(atype->args) < 3 ||
        jl_array_len(atype->args) > 4)
        return 0;
...
int jl_has_meta(jl_array_t *body, jl_sym_t *sym)
{
    ...
    for (i = 0; i < l; i++) {
        ...
        if (jl_is_expr((jl_value_t *)stmt) && stmt->head == meta_sym) {
            ...
            for (i = 0; i < l; i++)
                if (jl_array_ptr_ref(stmt->args, i) == (jl_value_t *)sym) return 1;
        }
    }
    return 0;
}
