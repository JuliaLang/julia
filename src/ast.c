...
STATIC_INLINE void jl_ast_context_list_insert(
        jl_ast_context_list_t **head, jl_ast_context_list_t *node)
{
...
value_t
fl_defined_julia_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    jl_ast_context_t *ctx = jl_ast_ctx(fl_ctx);
...
value_t
fl_current_module_counter(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    jl_ast_context_t *ctx = jl_ast_ctx(fl_ctx);
    assert(ctx->module);
...
value_t
fl_invoke_julia_macro(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (jl_is_expr(lno) && ((jl_expr_t *)lno)->head == line_sym) {
        ...
        margs[1] = jl_new_struct(
                jl_linenumbernode_type, jl_box_long(0), jl_nothing);
    }
    ...
    JL_TRY {
        ...
        mfunc = jl_method_lookup(
                jl_gf_mtable(margs[0]), margs, nargs, 1, world);
        if (mfunc == NULL) {
    ...
    JL_CATCH {
        JL_GC_POP();
        value_t opaque =
                cvalue(fl_ctx, jl_ast_ctx(fl_ctx)->jvtype, sizeof(void *));
    ...
    // `jl_expand` or `jl_parse_eval_all` so the preserved array is rooted
    // there.
    assert(result != NULL);
...
value_t fl_julia_scalar(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    if (fl_isnumber(fl_ctx, args[0]) || fl_isstring(fl_ctx, args[0]))
        return fl_ctx->T;
            ...
            iscvalue(args[0]) &&
            fl_ctx->jl_sym == cv_type((cvalue_t *)ptr(args[0]))) {
...
static void jl_init_ast_ctx(jl_ast_context_t *ast_ctx)
{
    ...
    if (fl_load_system_image_str(
                fl_ctx,
                (char *)flisp_system_image,
    ...
    jl_ast_context_t *ctx = jl_ast_ctx(fl_ctx);
    ctx->jvtype =
            define_opaque_type(fl_ctx->jl_sym, sizeof(void *), NULL, NULL);
...
{
    // Make `--lisp` sigatomic in order to avoid triggering the sigint
    // safepoint.
    ...
    fl_applyn(
            fl_ctx,
            1,
            symbol_value(symbol(fl_ctx, "__start")),
...
JL_DLLEXPORT void fl_profile(const char *fname)
{
    ...
    fl_applyn(
            fl_ctx,
            1,
            symbol_value(symbol(fl_ctx, "profile-e")),
            symbol(fl_ctx, fname));
    jl_ast_ctx_leave(ctx);
}
...
static jl_sym_t *scmsym_to_julia(fl_context_t *fl_ctx, value_t s)
{
    ...
    if (fl_isgensym(fl_ctx, s)) {
        char gsname[16];
        char *n = uint2str(
                &gsname[1], sizeof(gsname) - 1, ((gensym_t *)ptr(s))->id, 10);
        *(--n) = '#';
        return jl_symbol(n);
    }
...
static jl_value_t *scm_to_julia_(fl_context_t *fl_ctx, value_t e, int eo)
{
    ...
    if (iscons(e) || e == fl_ctx->NIL) {
        ...
        if (!eo) {
            if (sym == line_sym && (n == 1 || n == 2)) {
                ...
                jl_value_t *temp =
                        jl_new_struct(jl_linenumbernode_type, linenum, file);
                JL_GC_POP();
                return temp;
            }
            ...
            if (sym == top_sym) {
                ...
                temp = jl_module_globalref(
                        jl_base_relative_to(ctx->module), (jl_sym_t *)scmv);
                JL_GC_POP();
                return temp;
            }
            if (sym == core_sym) {
            ...
            if (sym == globalref_sym) {
                ...
                temp = jl_module_globalref(
                        (jl_module_t *)scmv, (jl_sym_t *)temp);
                JL_GC_POP();
                return temp;
            }
            if (sym == newvar_sym) {
        ...
        for (i = 0; i < n; i++) {
            ...
            jl_array_ptr_set(
                    ((jl_expr_t *)ex)->args,
                    i,
                    scm_to_julia_(fl_ctx, car_(e), eo));
            e = cdr_(e);
        }
        if (sym == lambda_sym)
    ...
    if (iscvalue(e) &&
        cv_class((cvalue_t *)ptr(e)) == jl_ast_ctx(fl_ctx)->jvtype) {
...
static value_t
julia_to_list2(fl_context_t *fl_ctx, jl_value_t *a, jl_value_t *b)
{
...
static value_t julia_to_scm_(fl_context_t *fl_ctx, jl_value_t *v)
{
    ...
    if (jl_typeis(v, jl_labelnode_type))
        return julia_to_list2(
                fl_ctx, (jl_value_t *)label_sym, jl_fieldref(v, 0));
    ...
    if (jl_typeis(v, jl_gotonode_type))
        return julia_to_list2(
                fl_ctx, (jl_value_t *)goto_sym, jl_fieldref(v, 0));
    if (jl_typeis(v, jl_quotenode_type))
        return julia_to_list2(
                fl_ctx, (jl_value_t *)inert_sym, jl_fieldref(v, 0));
    if (jl_typeis(v, jl_newvarnode_type))
        return julia_to_list2(
                fl_ctx, (jl_value_t *)newvar_sym, jl_fieldref(v, 0));
    if (jl_typeis(v, jl_globalref_type)) {
        jl_module_t *m = jl_globalref_mod(v);
        jl_sym_t *sym = jl_globalref_name(v);
        if (m == jl_core_module)
            return julia_to_list2(
                    fl_ctx, (jl_value_t *)core_sym, (jl_value_t *)sym);
        value_t args =
                julia_to_list2(fl_ctx, (jl_value_t *)m, (jl_value_t *)sym);
        fl_gc_handle(fl_ctx, &args);
        ...
        const char *str, size_t len, const char *filename, size_t filename_len)
{
    ...
    value_t e = fl_applyn(
            fl_ctx,
            2,
            symbol_value(symbol(fl_ctx, "jl-parse-string")),
            s,
            files);
    jl_value_t *res =
            e == fl_ctx->FL_EOF ? jl_nothing : scm_to_julia(fl_ctx, e, 0);
    jl_ast_ctx_leave(ctx);
    return res;
}
...
jl_parse_string(const char *str, size_t len, int pos0, int greedy)
{
    ...
    value_t p = fl_applyn(
            fl_ctx,
            3,
            symbol_value(symbol(fl_ctx, "jl-parse-one-string")),
            s,
            fixnum(pos0),
            greedy ? fl_ctx->T : fl_ctx->F);
        ...
        jl_module_t *inmodule)
{
    ...
    if (content != NULL) {
        ...
        ast = fl_applyn(
                fl_ctx,
                2,
                symbol_value(symbol(fl_ctx, "jl-parse-string-stream")),
                t,
                f);
        fl_free_gc_handles(fl_ctx, 1);
    }
        ...
        ast = fl_applyn(
                fl_ctx, 1, symbol_value(symbol(fl_ctx, "jl-parse-file")), f);
    }
    fl_free_gc_handles(fl_ctx, 1);
    if (ast == fl_ctx->F) {
        jl_ast_ctx_leave(ctx);
        ...
        while (iscons(ast)) {
            value_t expansion;
            {
                ...
                expansion = fl_applyn(
                        fl_ctx,
                        1,
                        symbol_value(symbol(fl_ctx, "jl-expand-to-thunk")),
                        car_(ast));
            }
...
static int jl_parse_depwarn_(fl_context_t *fl_ctx, int warn)
{
    value_t prev = fl_applyn(
            fl_ctx,
            1,
            symbol_value(symbol(fl_ctx, "jl-parser-depwarn")),
...
static int jl_parse_deperror(fl_context_t *fl_ctx, int err)
{
    value_t prev = fl_applyn(
            fl_ctx,
            1,
            symbol_value(symbol(fl_ctx, "jl-parser-deperror")),
        ...
        const char *funcname, jl_value_t *expr, jl_module_t *inmodule)
{
    ...
    value_t e =
            fl_applyn(fl_ctx, 1, symbol_value(symbol(fl_ctx, funcname)), arg);
...
JL_DLLEXPORT int jl_is_operator(char *sym)
{
    ...
    int res = fl_applyn(
                      fl_ctx,
                      1,
                      symbol_value(symbol(fl_ctx, "operator?")),
...
JL_DLLEXPORT int jl_operator_precedence(char *sym)
{
    jl_ast_context_t *ctx = jl_ast_ctx_enter();
    fl_context_t *fl_ctx = &ctx->fl;
    int res = numval(fl_applyn(
            fl_ctx,
            1,
            symbol_value(symbol(fl_ctx, "operator-precedence")),
            symbol(fl_ctx, sym)));
    jl_ast_ctx_leave(ctx);
    return res;
}
