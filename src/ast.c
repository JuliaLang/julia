...
static jl_value_t *jl_expand_macros(jl_value_t *expr, jl_module_t *inmodule, struct macroctx_stack *macroctx, int onelevel, int throw_load_error);
...
static jl_value_t *jl_invoke_julia_macro(jl_array_t *args, jl_module_t *inmodule, jl_module_t **ctx, int throw_load_error)
{
    jl_ptls_t ptls = jl_get_ptls_states();
...
static jl_value_t *jl_expand_macros(jl_value_t *expr, jl_module_t *inmodule, struct macroctx_stack *macroctx, int onelevel, int throw_load_error)
{
    ...
    if (e->head == quote_sym && jl_expr_nargs(e) == 1) {
        ...
        expr = jl_expand_macros(expr, inmodule, macroctx, onelevel, throw_load_error);
        JL_GC_POP();
        return expr;
    }
    if (e->head == hygienicscope_sym && jl_expr_nargs(e) == 2) {
        ...
        jl_value_t *a2 = jl_expand_macros(a, inmodule, &newctx, onelevel, throw_load_error);
        if (a != a2)
    ...
    if (e->head == macrocall_sym) {
        ...
        newctx.parent = macroctx;
        jl_value_t *result = jl_invoke_julia_macro(e->args, inmodule, &newctx.m, throw_load_error);
        ...
        if (!onelevel)
            result = jl_expand_macros(result, inmodule, wrap ? &newctx : macroctx, onelevel, throw_load_error);
        if (wrap) {
    ...
    if (e->head == do_sym && jl_expr_nargs(e) == 2 && jl_is_expr(jl_exprarg(e, 0)) &&
        ...
        jl_value_t *ret = jl_expand_macros((jl_value_t*)mc2, inmodule, macroctx, onelevel, throw_load_error);
        JL_GC_POP();
        return ret;
    }
    ...
    for (i = 0; i < jl_array_len(e->args); i++) {
        ...
        jl_value_t *a2 = jl_expand_macros(a, inmodule, macroctx, onelevel, throw_load_error);
        if (a != a2)
...
JL_DLLEXPORT jl_value_t *jl_macroexpand(jl_value_t *expr, jl_module_t *inmodule)
{
    ...
    expr = jl_expand_macros(expr, inmodule, NULL, 0, 0);
...
JL_DLLEXPORT jl_value_t *jl_macroexpand1(jl_value_t *expr, jl_module_t *inmodule)
{
    ...
    JL_GC_PUSH1(&expr);
    expr = jl_copy_ast(expr);
    expr = jl_expand_macros(expr, inmodule, NULL, 1, 0);
    expr = jl_call_scm_on_ast("jl-expand-macroscope", expr, inmodule);
    JL_GC_POP();
    return expr;
}
                                            ...
                                            const char *file, int line)
{
    ...
    JL_GC_PUSH1(&expr);
    expr = jl_copy_ast(expr);
    expr = jl_expand_macros(expr, inmodule, NULL, 0, 1);
