...
static value_t fl_defined_julia_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    return (b != NULL && b->imported == BINDING_IMPORT_NONE) ? fl_ctx->T : fl_ctx->F;
}
...
static value_t fl_nothrow_julia_global(fl_context_t *fl_ctx, value_t *args, uint32_t nargs)
{
    ...
    while (b && b->imported != BINDING_IMPORT_NONE) {
    ...
    if (!b)
        return fl_ctx->F;
    return  (b->constp ? b->restriction : jl_atomic_load_relaxed(&b->value)) != NULL ? fl_ctx->T : fl_ctx->F;
}
