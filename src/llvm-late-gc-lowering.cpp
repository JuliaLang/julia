...
static std::pair<Value*,int> FindBaseValue(const State &S, Value *V, bool UseCache = true) {
    ...
    while (true) {
        ...
        if (isa<BitCastInst>(CurrentV))
            ...
            if (callee && callee->getName().starts_with("julia.gc_loaded")) {
    ...
    for (BasicBlock &BB : F) {
        ...
        for (auto it = BB.rbegin(); it != BB.rend(); ++it) {
            ...
            if (CallInst *CI = dyn_cast<CallInst>(&I)) {
                ...
                else if (callee && callee->getName().starts_with("julia.gc_loaded")) {
                    continue;
                }
                else {
                ...
                if (callee) {
                    ...
                    if (callee == pointer_from_objref_func || callee == gc_preserve_begin_func ||
                        ...
                        callee->getName() == "memcmp" || callee->getName().starts_with("julia.gc_loaded")) {
                        continue;
                    }
