...
restart_switch:
        ...
        case opt_compiled_modules:
            ...
            else if (!strcmp(optarg,"nonew"))
                jl_options.use_compiled_modules = JL_OPTIONS_USE_COMPILED_MODULES_NONEW;
            else
                jl_errorf("julia: invalid argument to --compiled-modules={yes|no|nonew} (%s)", optarg);
            break;
        case opt_pkgimages:
            pkgimage_explicit = 1;
            if (!strcmp(optarg,"yes"))
