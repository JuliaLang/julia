...
JL_DLLEXPORT const char *jl_get_default_sysimg_path(void)
{
    ...
    snprintf(
            path,
            sizeof(system_image_path) - existing_length,
            "_%" PRIx64 "%s",
            jl_cpuid_tag(),
            shlib_ext);
#endif
...
static const char opts[] =
        ...
        " --precompiled={yes|no}    Use precompiled code from system image if "
        "available\n"
        " --compilecache={yes|no}   Enable/disable incremental precompilation "
        "of modules\n"
        ...
        " -L, --load <file>         Load <file> immediately on all "
        "processors\n\n"

        // parallel options
        " -p, --procs {N|auto}      Integer value N launches N additional "
        ...
        // interactive options
        " -i                        Interactive mode; REPL runs and "
        ...
        " --compile={yes|no|all|min}Enable or disable JIT compiler, or request "
        "exhaustive compilation\n"
        " -C, --cpu-target <target> Limit usage of cpu features up to "
        "<target>; set to \"help\" to see the available options\n"
        " -O, --optimize={0,1,2,3}  Set the optimization level (default level "
        "is 2 if unspecified or 3 if used without a level)\n"
        " -g, -g <level>            Enable / Set the level of debug info "
        "generation"
#ifdef JL_DEBUG_BUILD
        " (default level for julia-debug is 2 if unspecified or if used "
        "without a level)\n"
#else
        " (default level is 1 if unspecified or 2 if used without a level)\n"
#endif
        " --inline={yes|no}         Control whether inlining is permitted "
        "(overrides functions declared as @inline)\n"
        " --check-bounds={yes|no}   Emit bounds checks always or never "
        "(ignoring declarations)\n"
#ifdef USE_POLLY
        " --polly={yes|no}          Enable or disable the polyhedral optimizer "
        "Polly (overrides @polly declaration)\n"
#endif
        " --math-mode={ieee,fast}   Disallow or enable unsafe floating point "
        "optimizations (overrides @fastmath declaration)\n\n"

        // error and warning options
        " --depwarn={yes|no|error}  Enable or disable syntax and method "
        "deprecation warnings (\"error\" turns warnings into errors)\n\n"

        // compiler output options
        " --output-o name           Generate an object file (including system "
        "image data)\n"
        " --output-ji name          Generate a system image data file (.ji)\n"
        " --output-unopt-bc name    Generate unoptimized LLVM bitcode (.bc)\n"
        " --output-bc name          Generate LLVM bitcode (.bc)\n"
        " --output-incremental=no   Generate an incremental output file "
...
JL_DLLEXPORT void jl_parse_opts(int *argcp, char ***argvp)
{
    ...
    static const struct option longopts[] = {
        ...
        // with the required arguments defined in base/client.jl
        // `process_options()`
    ...
    // If CPUID specific binaries are enabled, this varies between runs, so
    // initialize
    ...
    restart_switch:
                ...
                for (const struct option *o = longopts; o->val; o++) {
                    if (optopt == o->val) {
                        if (o->has_arg == optional_argument) {
                            ...
                            jl_errorf(
                                    "option `-%c/--%s` is missing an argument",
                                    o->val,
                                    o->name);
                        }
                        else {
        ...
        case opt_use_precompiled:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --precompiled={yes|no} "
                        "(%s)",
                        optarg);
            break;
        case opt_use_compilecache:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --compilecache={yes|no} "
                        "(%s)",
                        optarg);
            break;
        case 'C': // cpu-target
        ...
        case opt_color:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --color={yes|no} (%s)",
                        optarg);
            break;
        case opt_history_file:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --history-file={yes|no} "
                        "(%s)",
                        optarg);
            break;
        case opt_startup_file:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --startup-file={yes|no} "
                        "(%s)",
                        optarg);
            break;
        case opt_compile:
        ...
        case opt_code_coverage:
            if (optarg != NULL) {
                if (!strcmp(optarg, "user"))
                    ...
                    jl_errorf(
                            "julia: invalid argument to --code-coverage (%s)",
                            optarg);
                break;
            }
            else {
        ...
        case opt_track_allocation:
            if (optarg != NULL) {
                if (!strcmp(optarg, "user"))
                    ...
                    jl_errorf(
                            "julia: invalid argument to --track-allocation "
                            "(%s)",
                            optarg);
                break;
            }
        ...
        case opt_check_bounds:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --check-bounds={yes|no} "
                        "(%s)",
                        optarg);
            break;
        case opt_output_bc:
        ...
        case opt_incremental:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to "
                        "--output-incremental={yes|no} (%s)",
                        optarg);
            break;
        case opt_depwarn:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf(
                        "julia: invalid argument to --depwarn={yes|no|error} "
                        "(%s)",
                        optarg);
            break;
        case opt_inline:
