...
static const char opts[]  =
    ...
    " --compiled-modules={yes*|no|existing|strict}  Enable or disable incremental precompilation of modules\n"
    " --pkgimages={yes*|no|existing}                Enable or disable usage of native code caching in the form of pkgimages ($)\n\n"
    ...
    // parallel options
    " -t, --threads {auto|N[,auto|M]}               Enable N[+M] threads; N threads are assigned to the `default` threadpool,\n"
    "                                               and if M is specified, M threads are assigned to the `interactive`\n"
    "                                               threadpool; \"auto\" tries to infer a useful default number of threads\n"
    "                                               to use but the exact behavior might change in the future. Currently sets N\n"
    "                                               to the number of CPUs assigned to this Julia process based on the OS-specific\n"
    "                                               affinity assignment interface if supported (Linux and Windows) or to the\n"
    "                                               number of CPU threads if not supported (MacOS) or if process affinity is not\n"
    "                                               configured, and sets M to 1.\n"
    " --gcthreads=N[,M]                             Use N threads for the mark phase of GC and M (0 or 1) threads for the \n"
    "                                               concurrent sweeping phase of GC. N is set to half of the number of compute\n"
    "                                               threads and M is set to 0 if unspecified.\n"
    " -p, --procs {N|auto}                          Integer value N launches N additional local worker processes \"auto\"\n"
    "                                               launches as many workers as the number of local CPU threads (logical cores)\n"
    ...
    " -C, --cpu-target <target>                     Limit usage of CPU features up to <target>; set to `help` to see the\n"
    "                                               available options\n"
...
#else
    " -g, --debug-info=[{0,1*,2}]                   Set the level of debug info generation (level 2 if `-g` is used\n"
    "                                               without a level) ($)\n"
#endif
    " --inline={yes*|no}                            Control whether inlining is permitted, including overriding\n"
    "                                               @inline declarations\n"
    " --check-bounds={yes|no|auto*}                 Emit bounds checks always, never, or respect @inbounds declarations ($)\n"
#ifdef USE_POLLY
    " --polly={yes*|no}                             Enable or disable the polyhedral optimizer Polly (overrides\n"
    "                                               @polly declaration)\n"
#endif
    ...
    " --code-coverage=@<path>                       Count executions but only in files that fall under the given file\n"
    "                                               path/directory. The `@` prefix is required to select this option.\n"
    "                                               A `@` with no path will track the current directory.\n"

    " --code-coverage=tracefile.info                Append coverage information to the LCOV tracefile (filename supports\n"
    "                                               format tokens)\n"
// TODO: These TOKENS are defined in `runtime_ccall.cpp`. A more verbose `--help` should include that list here.
    " --track-allocation[={none*|user|all}]         Count bytes allocated by each source line (omitting setting is equivalent\n"
    "                                               to `user`)\n"
    " --track-allocation=@<path>                    Count bytes but only in files that fall under the given file path/directory.\n"
    "                                               The `@` prefix is required to select this option. A `@` with no path will\n"
    "                                               track the current directory.\n"
    " --bug-report=KIND                             Launch a bug report session. It can be used to start a REPL, run a script,\n"
    "                                               or evaluate expressions. It first tries to use BugReporting.jl installed\n"
    "                                               in current environment and fallbacks to the latest compatible BugReporting.jl\n"
    "                                               if not. For more information, see --bug-report=help.\n\n"
...
static const char opts_hidden[]  =
    ...
    " --trace-compile={stderr,name}                 Print precompile statements for methods compiled during execution\n"
    "                                               or save to a path\n"
