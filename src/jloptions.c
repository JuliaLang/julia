...
static const char opts[] =
    ...
    "available\n"
    " --compilecache={yes|no}   Enable/disable incremental precompilation of "
    "modules\n"
    ...
    " -O, --optimize={0,1,2,3}  Set the optimization level (default 2 if "
    "unspecified or 3 if specified as -O)\n"
    ...
    " --output-incremental=no   Generate an incremental output file (rather "
    "than complete)\n\n"
    ...
    "Deprecated options:\n"
    " -F                        Load ~/.juliarc (deprecated, use "
    "--startup-file=yes)\n"
    " -f, --no-startup          Don't load ~/.juliarc (deprecated, use "
    "--startup-file=no)\n"
    " -P, --post-boot <expr>    Evaluate <expr>, but don't disable interactive "
    "mode (deprecated, use -i -e instead)\n"
    " --no-history-file         Don't load history file (deprecated, use "
    "--history-file=no)\n";
...
JL_DLLEXPORT void jl_parse_opts(int *argcp, char ***argvp)
{
    ...
    restart_switch:
        ...
        case 'E': // print
            ...
            break;
        case 'P': // post-boot
            jl_printf(JL_STDOUT, "WARNING: julia -P/--post-boot option is "
        ...
        case opt_use_compilecache:
            if (!strcmp(optarg, "yes"))
                ...
                jl_errorf("julia: invalid argument to --compilecache={yes|no} (%s)",
                          optarg);
            break;
        case 'C': // cpu-target
        ...
        case 'p': // procs
            ...
            break;
        case opt_machinefile: jl_options.machinefile = strdup(optarg); break;
        case opt_color:
            if (!strcmp(optarg, "yes"))
        ...
        case opt_no_history_file:
            jl_printf(JL_STDOUT, "WARNING: julia --no-history-file option is "
                                 "deprecated, use --history-file=no instead.\n");
            jl_options.historyfile = JL_OPTIONS_HISTORYFILE_OFF;
            break;
        case opt_startup_file:
            ...
            break;
        case 'f':
            jl_printf(JL_STDOUT, "WARNING: julia -f/--no-startup option is "
                                 "deprecated, use --startup-file=no instead.\n");
            jl_options.startupfile = JL_OPTIONS_STARTUPFILE_OFF;
            break;
        case 'F':
            jl_printf(JL_STDOUT, "WARNING: julia -F option is deprecated, use "
                                 "--startup-file=yes instead.\n");
            jl_options.startupfile = JL_OPTIONS_STARTUPFILE_ON;
            break;
        case opt_compile:
            if (!strcmp(optarg, "yes"))
        ...
        case opt_math_mode:
            ...
            break;
        case opt_worker: jl_options.worker  = strdup(optarg); break;
        case opt_bind_to: jl_options.bindto = strdup(optarg); break;
