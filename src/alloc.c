                                               ...
                                               jl_svec_t *env)
{
    ...
    JL_TRY {
        ...
        // need to eval macros in the right module, but not give a warning for the `eval`
        // call unless that results in a call to `eval`
...
void jl_compute_field_offsets(jl_datatype_t *st)
{
    ...
    if (homogeneous && lastty != NULL && jl_is_tuple_type(st)) {
        // Some tuples become LLVM vectors with stronger alignment than what was calculated
        // above.
