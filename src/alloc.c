...
static jl_value_t *jl_new_bits_internal(jl_value_t *dt, void *data, size_t *len)
{
    ...
    if (nb == 0) return jl_new_struct_uninit(bt);
...
{
    jl_datatype_t *st = (jl_datatype_t *)jl_typeof(v);
    if (i >= jl_datatype_nfields(st)) jl_bounds_error_int(v, i + 1);
    size_t offs = jl_field_offset(st, i);
    if (jl_field_isptr(st, i)) {
...
JL_DLLEXPORT jl_value_t *jl_new_struct_uninit(jl_datatype_t *type)
{
    ...
    if (size > 0) memset(jl_data_ptr(jv), 0, size);
    return jv;
}
                ...
                if (jl_is_symbol(s) && jl_is_globalref(fe)) {
                    ...
                    if (f == jl_builtin_getfield) {
                        ...
                        if (jl_is_globalref(me) ||
                            (jl_is_symbol(me) &&
                             jl_binding_resolved_p(lam->def->module, (jl_sym_t *)me))) {
                            jl_value_t *m =
                                jl_static_eval(me, NULL, lam->def->module, lam, 0, 0);
                            if (m && jl_is_module(m))
...
{
    jl_ptls_t         ptls = jl_get_ptls_states();
    jl_lambda_info_t *li   = (jl_lambda_info_t *)jl_gc_alloc(ptls, sizeof(jl_lambda_info_t),
                                                           jl_lambda_info_type);
    li->code                                    = NULL;
    li->slotnames                               = NULL;
    li->slotflags                               = NULL;
...
static jl_lambda_info_t *jl_instantiate_staged(jl_method_t *generator, jl_tupletype_t *tt,
                                               jl_svec_t *env)
{
    ...
    JL_TRY {
        ...
        jl_array_ptr_set(body->args, 1,
                         jl_call_staged(sparam_vals, func, jl_svec_data(tt->parameters),
                                        jl_nparams(tt)));
        ...
        // need to eval macros in the right module, but not give a warning for the
        // `eval` call unless that results in a call to `eval`
        func = (jl_lambda_info_t *)jl_toplevel_eval_in_warn(generator->module,
                                                            (jl_value_t *)ex, 1);
        ...
        for (i = 0, l = jl_array_len(stmts); i < l; i++) {
            jl_array_ptr_set(stmts, i,
                             jl_resolve_globals(jl_array_ptr_ref(stmts, i), func));
        }
...
JL_DLLEXPORT void jl_method_init_properties(jl_method_t *m)
{
    ...
    for (i = 1; i < li->nargs && i <= 8; i++) {
        ...
        if (jl_array_uint8_ref(li->slotflags, i) & 64) called |= (1 << (i - 1));
    }
    m->called = called;
}
...
jl_method_t *jl_new_method(jl_lambda_info_t *definition, jl_sym_t *name,
                           jl_tupletype_t *sig, jl_svec_t *tvars, int isstaged)
{
    ...
    m->name                            = name;
    m->sig                             = sig;
    if (jl_svec_len(tvars) == 1) tvars = (jl_svec_t *)jl_svecref(tvars, 0);
    m->tvars                           = tvars;
    JL_GC_PUSH1(&m);
    ...
    if (reused) definition = jl_copy_lambda(definition);
    ...
    if (reused) {
        ...
        for (i = 0, l = jl_array_len(stmts); i < l; i++) {
            jl_array_ptr_set(stmts, i,
                             jl_resolve_globals(jl_array_ptr_ref(stmts, i), definition));
        }
...
static jl_sym_t *symtab_lookup(jl_sym_t *volatile *ptree, const char *str, size_t len,
...
JL_DLLEXPORT jl_sym_t *jl_symbol(const char *str) { return _jl_symbol(str, strlen(str)); }
...
{
    jl_datatype_t *dt = jl_new_datatype((jl_sym_t *)name, super, parameters, jl_emptysvec,
                                        jl_emptysvec, 1, 0, 0);
    return dt;
}
...
static jl_datatype_layout_t *jl_get_layout(uint32_t nfields, uint32_t alignment,
                                           int haspadding, jl_fielddesc32_t desc[])
{
    ...
    if (nfields > 0) {
        ...
        for (size_t i = 0; i < nfields; i++) {
            if (desc[i].size > max_size) max_size = desc[i].size;
        }
...
{
    if (!jl_is_vecelement_type(t)) return 0;
    ...
    if (mask) return 0; // nfields has more than two 1s
    ...
    size_t alignment = 1;
    for (; size > alignment; alignment *= 2) continue;
    return alignment;
}
...
void jl_compute_field_offsets(jl_datatype_t *st)
{
    ...
    for (size_t i = 0; i < nfields; i++) {
        ...
        if (jl_isbits(ty) && jl_is_leaf_type(ty) && ((jl_datatype_t *)ty)->layout) {
            ...
            // Should never happen
            if (__unlikely(fsz > max_size)) jl_throw(jl_overflow_exception);
            al            = ((jl_datatype_t *)ty)->layout->alignment;
            desc[i].isptr = 0;
            if (((jl_datatype_t *)ty)->layout->haspadding) haspadding = 1;
        }
        else {
            fsz                      = sizeof(void *);
            if (fsz > MAX_ALIGN) fsz = MAX_ALIGN;
            al                       = fsz;
            desc[i].isptr            = 1;
        }
        if (al != 0) {
            size_t alsz                   = LLT_ALIGN(sz, al);
            if (sz & (al - 1)) haspadding = 1;
            sz                            = alsz;
            if (al > alignm) alignm       = al;
        }
        homogeneous &= lastty == NULL || lastty == ty;
        lastty         = ty;
        desc[i].offset = sz;
        desc[i].size   = fsz;
        if (__unlikely(max_offset - sz < fsz)) jl_throw(jl_overflow_exception);
        sz += fsz;
    }
...
{
    jl_datatype_t *bt = jl_new_datatype((jl_sym_t *)name, super, parameters, jl_emptysvec,
                                        jl_emptysvec, 0, 0, 0);
    uint32_t nbytes                = (nbits + 7) / 8;
    uint32_t alignm                = next_power_of_two(nbytes);
    if (alignm > MAX_ALIGN) alignm = MAX_ALIGN;
    bt->size                       = nbytes;
...
#endif
    jl_typector_t *tc =
        (jl_typector_t *)jl_gc_alloc(ptls, sizeof(jl_typector_t), jl_typector_type);
    tc->parameters = p;
    tc->body       = body;
...
#define SIBOX_FUNC(typ, c_type, nw)                                                          \
    ...
    {                                                                                        \
        ...
        if ((u##c_type)idx < (u##c_type)NBOX_C) return boxed_##typ##_cache[idx];             \
        jl_value_t *v             = jl_gc_alloc(ptls, nw * sizeof(void *), jl_##typ##_type); \
        *(c_type *)jl_data_ptr(v) = x;                                                       \
        return v;                                                                            \
    }
#define UIBOX_FUNC(typ, c_type, nw)                                                          \
    ...
    {                                                                                        \
        jl_ptls_t ptls = jl_get_ptls_states();                                               \
        if (x < NBOX_C) return boxed_##typ##_cache[x];                                       \
        jl_value_t *v             = jl_gc_alloc(ptls, nw * sizeof(void *), jl_##typ##_type); \
        *(c_type *)jl_data_ptr(v) = x;                                                       \
        return v;                                                                            \
    }
...
JL_DLLEXPORT jl_value_t *jl_box_int8(int8_t x) { return boxed_int8_cache[(uint8_t)x]; }
static jl_value_t *                         boxed_uint8_cache[256];
JL_DLLEXPORT jl_value_t *jl_box_uint8(uint8_t x) { return boxed_uint8_cache[x]; }
...
JL_CALLABLE(jl_f__expr)
{
    ...
    JL_GC_PUSH1(&ar);
    for (size_t i  = 0; i < nargs - 1; i++) jl_array_ptr_set(ar, i, args[i + 1]);
    jl_expr_t * ex = (jl_expr_t *)jl_gc_alloc(ptls, sizeof(jl_expr_t), jl_expr_type);
