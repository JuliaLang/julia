...
JL_DLLEXPORT jl_module_t *jl_new_module(jl_sym_t *name)
{
    ...
    jl_module_t *m = (jl_module_t *)jl_gc_alloc(
            ptls, sizeof(jl_module_t), jl_module_type);
    JL_GC_PUSH1(&m);
    assert(jl_is_symbol(name));
    m->name = name;
    m->parent = NULL;
    m->istopmod = 0;
    static unsigned int mcounter; // simple counter backup, in case hrtime is
                                  // not incrementing
...
static jl_binding_t *new_binding(jl_sym_t *name)
{
    ...
    assert(jl_is_symbol(name));
    jl_binding_t *b =
            (jl_binding_t *)jl_gc_alloc_buf(ptls, sizeof(jl_binding_t));
    b->name = name;
    b->value = NULL;
    b->owner = NULL;
    b->globalref = NULL;
...
JL_DLLEXPORT jl_binding_t *jl_get_binding_wr(jl_module_t *m, jl_sym_t *var)
{
    ...
    if (*bp != HT_NOTFOUND) {
        if ((*bp)->owner == NULL) {
            ...
            jl_printf(
                    JL_STDERR,
                    "WARNING: imported binding for %s overwritten in module "
                    "%s\n",
                    jl_symbol_name(var),
...
JL_DLLEXPORT jl_module_t *
jl_get_module_of_binding(jl_module_t *m, jl_sym_t *var)
{
...
// and overwriting.
JL_DLLEXPORT jl_binding_t *
jl_get_binding_for_method_def(jl_module_t *m, jl_sym_t *var)
{
    jl_binding_t **bp = (jl_binding_t **)ptrhash_bp(&m->bindings, var);
    jl_binding_t *b = *bp;
    ...
    if (b != HT_NOTFOUND) {
        if (b->owner != m && b->owner != NULL) {
            ...
            if (b2 == NULL)
                jl_errorf(
                        "invalid method definition: imported function %s.%s "
                        "does not exist",
            ...
            if (!b->imported && (b2->value == NULL || !jl_is_type(b2->value))) {
                jl_errorf(
                        "error in method definition: function %s.%s must be "
...
static void
module_import_(jl_module_t *to, jl_module_t *from, jl_sym_t *s, int explici);
...
static jl_binding_t *
jl_get_binding_(jl_module_t *m, jl_sym_t *var, modstack_t *st)
{
    modstack_t top = { m, st };
    modstack_t *tmp = st;
    while (tmp != NULL) {
    ...
    if (b == HT_NOTFOUND || b->owner == NULL) {
        ...
        for (int i = (int)m->usings.len - 1; i >= 0; --i) {
            ...
            jl_binding_t *tempb =
                    (jl_binding_t *)ptrhash_get(&imp->bindings, var);
            if (tempb != HT_NOTFOUND && tempb->exportp) {
                ...
                if (owner != NULL && tempb->owner != b->owner &&
                    !(tempb->constp && tempb->value && b->constp &&
                      b->value == tempb->value)) {
                    jl_printf(
                            JL_STDERR,
                            "WARNING: both %s and %s export \"%s\"; uses of it "
                            "in module %s must be qualified\n",
                            jl_symbol_name(owner->name),
                            jl_symbol_name(imp->name),
                            jl_symbol_name(var),
                            jl_symbol_name(m->name));
                    // mark this binding resolved, to avoid repeating the
                    // warning
    ...
    if (b == NULL) {
        ...
        if (bto != HT_NOTFOUND) {
            if (bto == b) {
                ...
                jl_printf(
                        JL_STDERR,
                        "WARNING: ignoring conflicting import of %s.%s into "
                        "%s\n",
                        jl_symbol_name(from->name),
                        jl_symbol_name(s),
                        jl_symbol_name(to->name));
                ...
                jl_printf(
                        JL_STDERR,
                        "WARNING: import of %s.%s into %s conflicts with an "
                        "existing identifier; ignored.\n",
                        jl_symbol_name(from->name),
                        jl_symbol_name(s),
                        jl_symbol_name(to->name));
            }
...
JL_DLLEXPORT void
jl_module_import(jl_module_t *to, jl_module_t *from, jl_sym_t *s)
{
    ...
    for (size_t i = 1; i < from->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->exportp && (b->owner == from || b->imported)) {
                ...
                jl_binding_t **tobp =
                        (jl_binding_t **)ptrhash_bp(&to->bindings, var);
                if (*tobp != HT_NOTFOUND && (*tobp)->owner != NULL &&
                    ...
                    jl_printf(
                            JL_STDERR,
                            "WARNING: using %s.%s in module %s conflicts with "
...
void jl_binding_deprecation_warning(jl_binding_t *b)
{
    if (b->deprecated && jl_options.depwarn) {
        ...
        if (jl_options.depwarn != JL_OPTIONS_DEPWARN_ERROR)
            jl_printf(
                    JL_STDERR, "  likely near %s:%d\n", jl_filename, jl_lineno);

        if (jl_options.depwarn == JL_OPTIONS_DEPWARN_ERROR) {
            if (b->owner)
                jl_errorf(
                        "deprecated binding: %s.%s",
                        jl_symbol_name(b->owner->name),
                        jl_symbol_name(b->name));
            else
    ...
    if (b->constp && b->value != NULL) {
        if (!jl_egal(rhs, b->value)) {
            if (jl_typeof(rhs) != jl_typeof(b->value) ||
                jl_is_type(rhs) /*|| jl_is_function(rhs)*/ ||
                jl_is_module(rhs)) {
                jl_errorf(
...
JL_DLLEXPORT void jl_declare_constant(jl_binding_t *b)
{
    if (b->value != NULL && !b->constp) {
        jl_errorf(
                "cannot declare %s constant; it already has a value",
                jl_symbol_name(b->name));
    }
    b->constp = 1;
}
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if ((b->exportp || (imported && b->imported) ||
                ...
                jl_array_ptr_set(
                        a, jl_array_dim0(a) - 1, (jl_value_t *)b->name);
            }
        }
    }
    JL_GC_POP();
...
JL_DLLEXPORT uint64_t jl_module_uuid(jl_module_t *m)
{
    return m->uuid;
}
