...
uint32_t jl_module_next_counter(jl_module_t *m) { return ++(m->counter); }
...
JL_DLLEXPORT void jl_set_istopmod(uint8_t isprimary)
{
    ...
    if (isprimary) jl_top_module   = ptls->current_module;
}

JL_DLLEXPORT uint8_t jl_istopmod(jl_module_t *mod) { return mod->istopmod; }
    ...
    if (b != HT_NOTFOUND) {
        if (b->owner != m && b->owner != NULL) {
            ...
            if (!b->imported && (b2->value == NULL || !jl_is_type(b2->value))) {
                jl_errorf("error in method definition: function %s.%s must be "
...
static jl_binding_t *jl_get_binding_(jl_module_t *m, jl_sym_t *var, modstack_t *st)
{
            ...
            if (tempb != HT_NOTFOUND && tempb->exportp) {
                ...
                if (owner != NULL && tempb->owner != b->owner &&
                    !(tempb->constp && tempb->value && b->constp &&
                      b->value == tempb->value)) {
                    jl_printf(JL_STDERR, "WARNING: both %s and %s export \"%s\"; uses of "
                                         "it in module %s must be qualified\n",
    ...
    if (b->owner != m) return jl_get_binding_(b->owner, var, &top);
    return b;
}
...
JL_DLLEXPORT jl_binding_t *jl_get_binding_or_error(jl_module_t *m, jl_sym_t *var)
{
    ...
    if (b == NULL) jl_undefined_var_error(var);
    ...
    if (b == NULL) {
        ...
        if (bto != HT_NOTFOUND) {
            if (bto == b) {
                ...
                jl_printf(JL_STDERR, "WARNING: import of %s.%s into %s conflicts with "
                                     "an existing identifier; ignored.\n",
                          jl_symbol_name(from->name), jl_symbol_name(s),
                          jl_symbol_name(to->name));
            }
            else {
...
JL_DLLEXPORT void jl_module_using(jl_module_t *to, jl_module_t *from)
{
    ...
    for (size_t i = 0; i < to->usings.len; i++) {
        if (from == to->usings.items[i]) return;
    }
        ...
        if (table[i] != HT_NOTFOUND) {
            ...
            if (b->exportp && (b->owner == from || b->imported)) {
                ...
                if (*tobp != HT_NOTFOUND && (*tobp)->owner != NULL &&
                    ...
                    jl_printf(JL_STDERR, "WARNING: using %s.%s in module %s conflicts "
...
JL_DLLEXPORT jl_value_t *jl_module_names(jl_module_t *m, int all, int imported)
{
    ...
    for (i = 1; i < m->bindings.size; i += 2) {
        if (table[i] != HT_NOTFOUND) {
            ...
            if ((b->exportp ||
                 ((imported || b->owner == m) && (all || m == jl_main_module))) &&
...
int jl_is_submodule(jl_module_t *child, jl_module_t *parent)
{
    while (1) {
        ...
        if (child == NULL || child == child->parent) return 0;
        child = child->parent;
    }
}
