...
retry:
    ...
    jl_gc_wb_fresh_pre(new_bpart, new_bpart->restriction);
    new_bpart->restriction = resolution.binding_or_const;
    jl_gc_wb_fresh_post(new_bpart, new_bpart->restriction);
    ...
    while (!new_bpart) {
        ...
        if (need_backdate) {
            ...
            while (1) {
                ...
                jl_gc_wb_fresh_pre(backdate_bpart, backdate_bpart->restriction);
                ...
                jl_gc_wb_fresh_post(backdate_bpart, backdate_bpart->restriction);
...
static jl_globalref_t *jl_new_globalref(jl_module_t *mod, jl_sym_t *name, jl_binding_t *b)
{
    ...
    jl_gc_wb_fresh_pre(g, g->mod);
    g->mod = mod;
    jl_gc_wb_fresh_post(g, g->mod);
    jl_gc_wb_fresh_pre(g, g->name);
    g->name = name;
    jl_gc_wb_fresh_post(g, g->name);
    jl_gc_wb_fresh_pre(g, g->binding);
    g->binding = b;
    jl_gc_wb_fresh_post(g, g->binding);
    return g;
}
    ...
    jl_binding_partition_t *old_bpart, jl_value_t *restriction_val, size_t kind, size_t new_world)
{
    ...
    jl_gc_wb_fresh_pre(new_bpart, new_bpart->next);
    jl_atomic_store_relaxed(&new_bpart->next, old_bpart);
    jl_gc_wb_fresh_post(new_bpart, new_bpart->next);
