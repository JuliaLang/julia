...
struct implicit_search_resolution {
    ...
    size_t max_world;
    int saw_cycle;
    // Not semantic, but used for debug printing. If non-null, the unique module from which this binding was imported
...
{
    to_update->min_world = MAX(to_update->min_world, resolution.min_world);
    to_update->max_world = MIN(to_update->max_world, resolution.max_world);
...
struct implicit_search_resolution jl_resolve_implicit_import(jl_binding_t *b, modstack_t *st, size_t world, int trust_cache)
{
    // First check if we've hit a cycle in this resolution
    {
        ...
        for (; tmp != NULL; tmp = tmp->prev) {
            if (tmp->b == b) {
                return (struct implicit_search_resolution){ PARTITION_FAKE_KIND_CYCLE, NULL, 0, ~(size_t)0, 1, NULL };
            }
        }
    }
    ...
    impstate = depimpstate = (struct implicit_search_resolution){ PARTITION_KIND_GUARD, NULL, min_world, max_world, 0, NULL };
    ...
    for (; i >= 0 && impstate.ultimate_kind != PARTITION_KIND_FAILED; --i) {
        ...
        min_world = MAX(min_world, data.min_world);
        max_world = MIN(max_world, data.max_world);
        ...
        while (tempbpart && jl_bkind_is_some_explicit_import(jl_binding_kind(tempbpart))) {
            max_world = MIN(max_world, jl_atomic_load_relaxed(&tempbpart->max_world));
            min_world = MAX(min_world, tempbpart->min_world);
        ...
        size_t tembppart_min_world = tempbpart ? MAX(tempbpart->min_world, gap.min_world) : gap.min_world;

        max_world = MIN(max_world, tembppart_max_world);
        min_world = MAX(min_world, tembppart_min_world);
        ...
        struct implicit_search_resolution imp_resolution = { PARTITION_KIND_GUARD, NULL, min_world, max_world, 0, NULL };
        if (!tempbpart_valid) {
            ...
            if (kind == PARTITION_KIND_IMPLICIT_GLOBAL || kind == PARTITION_KIND_GLOBAL || kind == PARTITION_KIND_DECLARED || kind == PARTITION_KIND_BACKDATED_CONST) {
    ...
    if (impstate.ultimate_kind == PARTITION_KIND_GUARD && depimpstate.ultimate_kind != PARTITION_KIND_GUARD) {
        depimpstate.min_world = MAX(depimpstate.min_world, min_world);
        depimpstate.max_world = MIN(depimpstate.max_world, max_world);
        return depimpstate;
    }
    impstate.min_world = MAX(impstate.min_world, min_world);
    impstate.max_world = MIN(impstate.max_world, max_world);
    return impstate;
}
