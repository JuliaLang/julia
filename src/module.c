...
static int eq_bindings(jl_binding_partition_t *owner, jl_binding_t *alias, size_t world)
{
    ...
    while (jl_bpart_is_some_import(alias_bpart)) {
        alias = (jl_binding_t*)alias_bpart->restriction;
        alias_bpart = jl_get_binding_partition(alias, world);
    }
    if (jl_bpart_is_some_constant(owner) && jl_bpart_is_some_constant(alias_bpart) && owner->restriction && alias_bpart->restriction == owner->restriction)
        return 1;
...
static jl_binding_t *jl_resolve_owner(jl_binding_t *b/*optional*/, jl_module_t *m, jl_sym_t *var, modstack_t *st)
{
    ...
    if (bpart->kind == BINDING_KIND_GUARD) {
        ...
        bpart->restriction = (jl_value_t*)b2;
        if (b2->deprecated) {
...
JL_DLLEXPORT jl_value_t *jl_get_binding_type(jl_module_t *m, jl_sym_t *var)
{
    ...
    if (jl_bpart_is_some_guard(bpart))
        return jl_nothing;
    if (jl_bpart_is_some_constant(bpart))
        return jl_typeof(bpart->restriction);
    return bpart->restriction;
}
...
static void module_import_(jl_module_t *to, jl_module_t *from, jl_sym_t *asname, jl_sym_t *s, int explici)
{
    ...
    if (b == NULL) {
        ...
        if (btopart->kind == BINDING_KIND_GUARD || btopart->kind == BINDING_KIND_IMPLICIT) {
            ...
            btopart->restriction = (jl_value_t*)b;
        ...
        if (b->exportp && (bpart->kind == BINDING_KIND_GLOBAL || bpart->kind == BINDING_KIND_IMPORTED)) {
            ...
            if (tob) {
                ...
                while (jl_bpart_is_some_import(tobpart)) {
                    tob = (jl_binding_t*)tobpart->restriction;
...
static int should_append_binding(jl_module_t *m, jl_binding_t *b, int non_public, int imported, int usings_explicit, int generated, int deprecated)
{
    ...
    jl_binding_partition_t *bpart = jl_get_binding_partition(b, jl_current_task->world_age);
    if (b->deprecated && !deprecated)
        return 0;
    if (isgenerated && !generated)
        return 0;
    if (bpart->kind == BINDING_KIND_CONST_IMPORT) // CONST_IMPORT is bit of a hybrid that's gated by the `imported` flag.
        return imported;
    if (bpart->kind == BINDING_KIND_GLOBAL || jl_bpart_is_some_constant(bpart)) {
        ...
        return non_public || b->publicp || main_public;
    } else if (bpart->kind == BINDING_KIND_IMPORTED) {
        return imported;
    } else if (bpart->kind == BINDING_KIND_EXPLICIT) {
        return usings_explicit;
    }
    return 0;
}
...
void append_module_names(jl_array_t* a, jl_module_t *m, int non_public, int imported, int usings_explicit, int generated, int deprecated)
{
    ...
    for (size_t i = 0; i < jl_svec_len(table); i++) {
        ...
        if ((void*)b == jl_nothing)
            break;
        if (should_append_binding(m, b, non_public, imported, usings_explicit, generated, deprecated)) {
