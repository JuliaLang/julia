...
extern void jl_input_line_callback_no_readline(char *input);
extern char cloud_buf[1000];
extern int cloud_buf_size;
...
static void ajax_send_message(struct mg_connection *conn,
  ...
  cloud_buf_size = 0;
  jl_input_line_callback_no_readline(text);
  sleep(1);
  int inputlen = strlen(text);
  cloud_buf[cloud_buf_size] = '\0';

  printf("length of result = %d, result = %s\n", cloud_buf_size, cloud_buf);

  text[inputlen] = ' ';
  memcpy(text + inputlen + 1, cloud_buf, cloud_buf_size);
  text[inputlen + cloud_buf_size + 1] = '\0';
