...
# Count Core.Box mentions in the closure-converted tree for `src`.
function acp_count_boxes(src::AbstractString)
    ...
    n = Ref(0)
    walk(e) = begin
        if JuliaSyntax.kind(e) == JuliaSyntax.K"core" && e.name_val == "Box"
            n[] += 1
