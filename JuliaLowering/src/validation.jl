...
function _calldecl_positionals(vcx, params_meta, kw_kind)
    ...
    for p in params
        if kind(p) === kw_kind
            require_assign = true
            if p == params[end]
                ok &= vst1_param_and_default(vcx, p; kw_kind, allow_val_splat=true)
            else
                ok &= vst1_param_and_default(vcx, p; kw_kind, allow_val_splat=false)
            end
        elseif kind(p) === K"..."
...
vst1_param_and_default(vcx, st; kw_kind, allow_val_splat) = @stm st begin
