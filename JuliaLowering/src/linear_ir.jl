...
function compile(ctx::LinearIRContext, ex, needs_value, in_tail_pos)
    ...
    elseif k == K"symbolic_block"
        ...
        name = ex[1].name_val
        is_anonymous = (name === :_)
        # Check for duplicate labels (skip for anonymous `_` blocks which shadow)
        if !is_anonymous && (haskey(ctx.symbolic_jump_targets, name) || name in ctx.symbolic_block_labels)
