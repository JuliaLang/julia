    ...
    return @stm st begin
        ...
        ([K"=" l r], when=(is_eventually_call(l))) -> let
            ...
            if has_if_generated(r)
                ...
                r2 = @ast _ st [K"_generated_body" [K"syntaxquote" gen] rec(f_ctx, nongen)]
            else
                r2 = rec(f_ctx, r)
            end
            @ast _ st [K"function" rec(ctx, l) r2]
        end
        ...
        [K"->" l r] -> let
            ...
            @ast _ st [K"->" rec(ctx, l) r2]
        end
        [K"macro" l r] -> let
            l = apply_arglist_meta(l, collect_body_arg_meta(r))
            @ast _ st [K"macro" rec(ctx, l) rec(with(ctx; toplevel=false), r)]
        end
