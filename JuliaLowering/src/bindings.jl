...
mutable struct BindingInfo
    ...
    # Typed shared container for a boxed capture (unified/capture_analysis.jl):
    # `nothing` keeps `Core.Box`; a Type (e.g. `Base.RefValue{Int}`) replaces it
    # when the value type is provable at lowering time and undef is impossible.
    box_type::Any
...
function BindingInfo(id::IdTag, name::AbstractString, kind::Symbol, node_id::Integer;
    ...
    BindingInfo(id, name, kind, node_id, mod, type, lambda_id, is_const, is_ssa,
                is_internal, is_ambiguous_local, unboxed, nothing, is_nospecialize,
