...
mutable struct BindingInfo
    ...
    # Part-3 mutable-struct capture merge (closure_conversion.jl
    # analyze_merged_captures!): when nonzero, this still-shared variable's
    # container is merged into its unique capturing closure as an untyped
    # MUTABLE FIELD (the closure type becomes a mutable struct) instead of a
    # separate Core.Box. `merged_undef` marks the maybe-undef flavor: the
    # field is ordered last, left uninitialized by `new`, and conditionally
    # initialized from the local at creation.
    merged_into::IdTag
    merged_undef::Bool
...
function BindingInfo(id::IdTag, name::AbstractString, kind::Symbol, node_id::Integer;
                     ...
                     unboxed::Bool = false,
                     merged_into::IdTag = zero(IdTag),
                     merged_undef::Bool = false,
    ...
    BindingInfo(id, name, kind, node_id, mod, type, lambda_id, is_const, is_ssa,
                is_internal, is_ambiguous_local, unboxed, merged_into,
                merged_undef, is_nospecialize,
...
function Base.show(io::IO, binfo::BindingInfo)
    ...
    binfo.merged_into != 0   && print(io, ", merged_into=", binfo.merged_into,
                                      binfo.merged_undef ? " (maybe-undef)" : "")
