...
---------------------
  ...
  * Keyword & optional function arguments
  ...
  * New infix operator `in` (e.g. `x in S`), and corresponding function
    `in(x,S)`, replacing `contains(S,x)` function
    ([#2703](https://github.com/JuliaLang/julia/issues/2703)).

  * New variable bindings on each for loop and comprehension iteration
  ...
  * A semicolon is now allowed after an `import` or `using` statement
...
---------------------
  ...
  * Multimedia I/O API (display, writemime, etcetera)
    ([#3932](https://github.com/JuliaLang/julia/issues/3932)).

  * MPFR-based `BigFloat`
    ([#2814](https://github.com/JuliaLang/julia/issues/2814)),
    and many new `BigFloat` operations.

  * New half-precision IEEE floating-point type, `Float16`
  ...
  * `Collections.PriorityQueue` type and `Collections.heap` functions
    ([#2920](https://github.com/JuliaLang/julia/issues/2920)).

  * `quadgk` 1d-integration routine
    ([#3140](https://github.com/JuliaLang/julia/issues/3140)).

  * `erfinv` and `erfcinv` functions
...
--------------------
  ...
  * `delete!(d::Dict, key)` has now been split
    ...
    and does not throw an exception if `key` was not found
    ([#3439](https://github.com/JuliaLang/julia/issues/3439)).

  * Linear-algebra factorization routines (`lu`, `chol`, etc.)
    now return `Factorization` objects
    (and `lud`, `chold`, etc. are deprecated)
  ...
  * Transitive comparison of floats with rationals
    ([#3102](https://github.com/JuliaLang/julia/issues/3102)).

  * Fast primality testing (da670c4).

  * `sum` and `cumsum` now use pairwise summation for better accuracy
  ...
  * `combinations`, `permutations`, and `partitions`
    ...
    and `integer_partitions` has been renamed to `partitions`
    ([#3989](https://github.com/JuliaLang/julia/issues/3989),
     [#4055](https://github.com/JuliaLang/julia/issues/4055)).

  * `isreadable`/`iswritable` functions added for more IO types
    ([#3872](https://github.com/JuliaLang/julia/issues/3872)).

  * Much faster and improved `readdlm` and `writedlm`
  ...
  * Faster `matchall`
    ...
    and a number of string and regex improvements.
  ...
  * Support optional RTLD flags in `dlopen`
  ...
  * New `sinpi(x)` and `cospi(x)` functions
    to compute sine and cosine of `pi*x` more accurately
...
---------------------
  ...
  * `!` was added to the name of many mutating functions,
    e.g., `push` was renamed `push!`
    ([#907](https://github.com/JuliaLang/julia/issues/907)).

  * `ref` renamed to `getindex`, and `assign` to `setindex!`
  ...
  * `logb` and `ilogb` renamed to `exponent`
  ...
  * `each_line, `each_match`, `begins_with`, `ends_with`,
    `parse_float`, `parse_int`, and `seek_end`
    replaced by: `eachline`, `eachmatch`, and so on (`_` was removed)
  ...
  * `diagmm` and `diagmm!` replaced by `scale` and `scale!`
...
--------------------------------
  ...
  * `julia-release-*` executables renamed to `julia-*`,
    and `libjulia-release` renamed to `libjulia`
