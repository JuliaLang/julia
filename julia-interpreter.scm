...
(make-builtin 'tuplelen "(Tuple,)-->Int"
	      (lambda (t) (j-box int32-type (tuple-length t))))
...
(define-macro (si b n)
  (let ((g (gensym)))
    `(let ((,g ,n))
	   ...
	   (- (bitwise-and (- ,g) ,(- (arithmetic-shift 1 b) 1)))))))
; low-level intrinsics, designed around the idea that primitive operations
; like add_int take any-length, matching bitstring arguments.
; the operations are the same regardless of bitstring length or signedness,
; except when the signed vs. unsigned operations are truly different, such as
; unsigned division. hence the unbox() operations don't need to mention
; signedness, because their job is to feed inputs to these operations.
; the box() operations do deal with signedness, to allow values to be tagged
; with their proper meanings.
(make-builtin 'boxui8  "" (lambda (n) (vector uint8-type  (ui 8  n))))
(make-builtin 'boxsi8  "" (lambda (n) (vector int8-type   (si 8  n))))
(make-builtin 'boxui16 "" (lambda (n) (vector uint16-type (ui 16 n))))
(make-builtin 'boxsi16 "" (lambda (n) (vector int16-type  (si 16 n))))
(make-builtin 'boxui32 "" (lambda (n) (vector uint32-type (ui 32 n))))
(make-builtin 'boxsi32 "" (lambda (n) (vector int32-type  (si 32 n))))
(make-builtin 'boxui64 "" (lambda (n) (vector uint64-type (ui 64 n))))
(make-builtin 'boxsi64 "" (lambda (n) (vector int64-type  (si 64 n))))
(make-builtin 'boxf32  "" (lambda (n) (vector float32-type n)))
(make-builtin 'boxf64  "" (lambda (n) (vector float64-type n)))
(make-builtin 'unbox8  "" j-unbox)
(make-builtin 'unbox16 "" j-unbox)
(make-builtin 'unbox32 "" j-unbox)
(make-builtin 'unbox64 "" j-unbox)
(make-builtin 'neg_int  "(Int,)-->Int"    -)
(make-builtin 'add_int  "(Int,Int)-->Int" +)
(make-builtin 'sub_int  "(Int,Int)-->Int" -)
...
(make-builtin 'mul_float "(Float,Float)-->Float" *)
(make-builtin 'div_float "(Float,Float)-->Float"
	      (lambda (x y) (exact->inexact (/ x y))))
(define (j-eq x y) (if (= x y) julia-true julia-false))
(define (j-lt x y) (if (< x y) julia-true julia-false))
(make-builtin 'eq_int   "(Int,Int)-->Bool" j-eq)
(make-builtin 'slt_int  "(Int,Int)-->Bool" j-lt)
(make-builtin 'ult_int  "(Int,Int)-->Bool" j-lt)
(make-builtin 'eq_float "(Float,Float)-->Bool" j-eq)
(make-builtin 'lt_float "(Float,Float)-->Bool" j-lt)
(make-builtin 'ne_float "(Float,Float)-->Bool"
...
(make-builtin 'sext16   "" (lambda (x) (si 16 (to-int x))))
(make-builtin 'zext16   "" (lambda (x) (ui 16 (to-int x))))
(make-builtin 'sext32   "" (lambda (x) (si 32 (to-int x))))
(make-builtin 'zext32   "" (lambda (x) (ui 32 (to-int x))))
(make-builtin 'sext64   "" (lambda (x) (si 64 (to-int x))))
(make-builtin 'zext64   "" (lambda (x) (ui 64 (to-int x))))
(make-builtin 'trunc8   "" (lambda (x) (si  8 (to-int x))))
(make-builtin 'trunc16  "" (lambda (x) (si 16 (to-int x))))
(make-builtin 'trunc32  "" (lambda (x) (si 32 (to-int x))))
(make-builtin 'fptoui8  "" (lambda (x) (ui  8 (to-int x))))
(make-builtin 'fptosi8  "" (lambda (x) (si  8 (to-int x))))
(make-builtin 'fptoui16 "" (lambda (x) (ui 16 (to-int x))))
(make-builtin 'fptosi16 "" (lambda (x) (si 16 (to-int x))))
(make-builtin 'fptoui32 "" (lambda (x) (ui 32 (to-int x))))
(make-builtin 'fptosi32 "" (lambda (x) (si 32 (to-int x))))
(make-builtin 'fptoui64 "" (lambda (x) (ui 64 (to-int x))))
(make-builtin 'fptosi64 "" (lambda (x) (si 64 (to-int x))))
(make-builtin 'uitofp32 "" exact->inexact)
(make-builtin 'sitofp32 "" exact->inexact)
(make-builtin 'uitofp64 "" exact->inexact)
(make-builtin 'sitofp64 "" exact->inexact)
(make-builtin 'fptrunc32 "" exact->inexact)
(make-builtin 'fpext64   "" exact->inexact)
