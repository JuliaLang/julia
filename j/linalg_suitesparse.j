    ...
    quote
        ...
        function _jl_umfpack_symbolic{Tv<:Float64,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti})
            ...
            status = ccall(dlsym(_jl_libSuiteSparse, $f_sym_r),
                           Ti,
                           (Ti, Ti, Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
                           convert($inttype, S.m), convert($inttype, S.n), S.colptr, S.rowval, S.nzval, Symbolic, C_NULL, C_NULL)
        ...
        function _jl_umfpack_symbolic{Tv<:Complex128,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti})
            ...
            status = ccall(dlsym(_jl_libSuiteSparse, $f_sym_c),
                           Ti,
                           (Ti, Ti, Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Float64}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
                           convert($inttype, S.m), convert($inttype, S.n), S.colptr, S.rowval, real(S.nzval), imag(S.nzval), Symbolic, C_NULL, C_NULL)
            if status != _jl_UMFPACK_OK; error("Error in symoblic factorization"); end
            return Symbolic
        end
    ...
    quote
        ...
        function _jl_umfpack_numeric{Tv<:Float64,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti}, Symbolic)
            ...
            status = ccall(dlsym(_jl_libSuiteSparse, $f_num_r),
                           Ti,
                           (Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Void}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
        ...
        function _jl_umfpack_numeric{Tv<:Complex128,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti}, Symbolic)
            ...
            status = ccall(dlsym(_jl_libSuiteSparse, $f_num_c),
                           Ti,
                           (Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Float64}, Ptr{Void}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
                           S.colptr, S.rowval, real(S.nzval), imag(S.nzval), Symbolic[1], Numeric, C_NULL, C_NULL)
            if status != _jl_UMFPACK_OK; error("Error in numeric factorization"); end
            return Numeric
        end
...
macro _jl_umfpack_solve_macro(f_sol_r, f_sol_c, inttype)
    quote
        ...
        function _jl_umfpack_solve{Tv<:Float64,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti}, b::Vector{Tv}, Numeric)
            x = similar(b)
            status = ccall(dlsym(_jl_libSuiteSparse, $f_sol_r),
                           Ti,
                           (Ti, Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
        ...
        function _jl_umfpack_solve{Tv<:Complex128,Ti<:$inttype}(S::SparseMatrixCSC{Tv,Ti}, b::Vector{Tv}, Numeric)
            xr = similar(b, Float64)
            xi = similar(b, Float64)
            status = ccall(dlsym(_jl_libSuiteSparse, $f_sol_c),
                           Ti,
                           (Ti, Ptr{Ti}, Ptr{Ti}, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Float64}, Ptr{Void}, Ptr{Float64}, Ptr{Float64}),
                           convert(Ti, _jl_UMFPACK_A), S.colptr, S.rowval, real(S.nzval), imag(S.nzval), xr, xi, real(b), imag(b), Numeric[1], C_NULL, C_NULL)
            if status != _jl_UMFPACK_OK; error("Error in solve"); end
...
function (\){Tv<:Union(Float64,Complex128), Ti<:Union(Int64,Int32)}(S::SparseMatrixCSC{Tv,Ti}, b::Vector{Tv})
