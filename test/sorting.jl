...
for R in numTypes, T in numTypes
    @test searchsorted(R[1, 1, 2, 2, 3, 3], convert(T,0)) == 1:0
    @test searchsorted(R[1, 1, 2, 2, 3, 3], convert(T,1)) == 1:2
    @test searchsorted(R[1, 1, 2, 2, 3, 3], convert(T,2)) == 3:4
    @test searchsorted(R[1, 1, 2, 2, 3, 3], convert(T,4)) == 7:6
    @test searchsorted(R[1, 1, 2, 2, 3, 3], 2.5) == 5:4

    @test searchsorted(1:3, convert(T,0)) == 1:0
    @test searchsorted(1:3, convert(T,1)) == 1:1
    @test searchsorted(1:3, convert(T,2)) == 2:2
    @test searchsorted(1:3, convert(T,4)) == 4:3

    @test searchsorted(R[1:10], convert(T,1), by=(x -> x >= 5)) == 1:4
    @test searchsorted(R[1:10], convert(T,10), by=(x -> x >= 5)) == 5:10
    @test searchsorted(R[1:5, 1:5, 1:5], convert(T,1), 6, 10, Base.Order.Forward) == 6:6
    @test searchsorted(ones(R, 15), convert(T,1), 6, 10, Base.Order.Forward) == 6:10
end
