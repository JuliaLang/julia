...
for arr in (identity, as_sub)
    ...
    @test_throws broadcast_getindex(M, eye(Int, 2).+1,arr([1, -1])) BoundsError
    @test_throws broadcast_getindex(M, eye(Int, 2).+1,arr([1, 2]), [2]) BoundsError
    ...
    @test A == diagm(10:12)
    @test_throws broadcast_setindex!(A, 7, [1,-1], [1 2]) BoundsError
