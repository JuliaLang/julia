...
# issue #4718
@test collect(IterTools.filter(x->x[1], zip([true, false, true, false],"abcd"))) == [(true,'a'),(true,'c')]
...
@test eltype(IterTools.filter(isodd, 1:5)) == Int

# typed `collect`
@test collect(Float64, IterTools.filter(isodd, [1,2,3,4]))[1] === 1.0
...
for itr in [product(1:0),
            ...
            lexproduct(1:2, 1:0),
            lexproduct(1:0, 1:2),
            lexproduct(1:0, 1:1, 1:2),
            lexproduct(1:1, 1:0, 1:2),
            lexproduct(1:1, 1:2 ,1:0)]
    @test isempty(itr)
...
@test collect(lexproduct(1:2))           == [(i,)      for i=1:2]
@test collect(lexproduct(1:2, 3:4))      == [(i, j)    for j=3:4, i=1:2]
@test collect(lexproduct(1:2, 3:4, 5:6)) == [(i, j, k) for k=5:6, j=3:4, i=1:2 ]
...
let
    ...
    for itr in Any[countfrom(1), IterTools.filter(i->0, 1:10)]
...
let f1 = IterTools.filter(i->i>0, 1:10)
