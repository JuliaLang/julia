...
struct T24Linear{T,N,dims} <: AbstractArray{T,N}
...
const struct TSlow{T,N} <: AbstractArray{T,N}
...
let
    struct TestThrowNoGetindex{T} <: AbstractVector{T} end
...
struct UnimplementedFastArray{T, N} <: AbstractArray{T, N} end
Base.linearindexing(::UnimplementedFastArray) = Base.LinearFast()

struct UnimplementedSlowArray{T, N} <: AbstractArray{T, N} end
Base.linearindexing(::UnimplementedSlowArray) = Base.LinearSlow()

struct UnimplementedArray{T, N} <: AbstractArray{T, N} end
...
struct TSlowNIndexes{T,N} <: AbstractArray{T,N}
    data::Array{T,N}
end
