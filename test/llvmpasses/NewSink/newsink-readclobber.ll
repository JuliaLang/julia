...
ok:
  ; Read here doesn't prevent sinking since it's not on error path
  ; But wait - our pass checks if loads alias with the store location
  ; The load in ok path would alias, so this actually prevents sinking
...
; Test: Intervening non-aliasing write between our store and a read
; Store to %a should NOT be sunk because there's a read from %a after
; the intervening store to %b (which doesn't alias %a)
; This tests that we follow through ALL MemoryDefs, not just aliasing ones
