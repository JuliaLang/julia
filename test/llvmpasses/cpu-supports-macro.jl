...
# CPU-model expansion: haswell's feature list is a single query.
...
# Assert true (sse2 is part of the x86-64 baseline): folds to no-op.
...
function assert_holds(x::Int)
    @assert Base.@cpu_supports x86_64 sse2
    return x * 2
end
emit(assert_holds, Int)

# Assert false: the intrinsic folds unknown feature names to false.
...
function assert_fails(x::Int)
    @assert Core.Intrinsics.cpu_supports(:not_a_real_feature)
    return x * 10000
end
