...
let N=9000
    ...
    # warmup
    parapply(my_matmult, (A,x,b2), 1:N, numthreads=1)
  
    # run with 1 thread (serial)
    println("\nmy_matmult - 1 thread")
    @time parapply(my_matmult, (A,x,b2), 1:N, numthreads=1)

    # run with 2 threads (parallel)
    println("my_matmult - 2 threads")
    @time parapply(my_matmult,(A,x,b3), 1:N, numthreads=2)
...
function pmedian_filter(im::Matrix, filterSize=3; numthreads=2)
    ...
    parapply(pmedian_filter_core, (im, out, K), 1:N[2], numthreads = numthreads)
    out
end
