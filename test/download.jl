...
mktempdir() do temp_dir
    ...
    @test download("http://httpbin.julialang.org/ip", file) == file
    @test isfile(file)
    ...
    @test download("http://httpbin.julialang.org/status/404", file) == file
    @test isfile(file)
    @test !isempty(read(file))
    ...
    @test download("http://httpbin.julialang.org/status/200", empty_file) == empty_file
    ...
    @test_throws ProcessFailedException download("http://httpbin.julialang.org/status/404", missing_file)
    ...
    if Sys.iswindows()
        ...
        download_powershell("https://httpbin.julialang.org/get?test='^'", metachar_file)
