...
status,len,point,buffer = Grisu.fastdtoa(float16(1.0), Grisu.PRECISION, 3, buffer)
@test status
@test 3 >= len-1
@test "1" == trimrep(buffer)
@test 1 == point
...
status,len,point,buffer = Grisu.fastdtoa(float16(1.5), Grisu.PRECISION, 10, buffer)
if status
  @test 10 >= len-1
  @test "15" == trimrep(buffer)
  @test 1 == point
