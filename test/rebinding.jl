...
# untyped runtime read with `effect_free` false), but a store to the former can succeed
# (throwing only if the binding is re-declared under it, #62154) while a store to the
# latter always throws. Deleting the binding must still invalidate code that stores to
# it: `binding_access_key` carries the binding identity for `DECLARED`.
...
let m = DeprecatedDeclaredDelete
    ...
    @test Base.infer_exception_type(m.store_depdecl, (Int,)) == Union{TypeError, ErrorException}
    ...
    @test Base.binding_kind(Base.lookup_binding_partition(Base.get_world_counter(), GlobalRef(m, :depdecl))) ==
        Base.PARTITION_KIND_GUARD
    @test ci.max_world != typemax(UInt)
    @test Base.invokelatest(Base.infer_exception_type, m.store_depdecl, (Int,)) == ErrorException
end
