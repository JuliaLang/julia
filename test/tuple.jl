...
@testset "conversion and construction" begin
    ...
    @test_throws MethodError(convert, (NTuple, (1, 1.0))) convert(NTuple, (1, 1.0))
    let T = Tuple{Vararg{T}} where T<:Integer, v = (1.0, 2, 0x3)
        @test_throws MethodError(convert, (T, (2, 0x3))) convert(T, v)
    end
    let T = Tuple{T, Vararg{T}} where T<:Integer, v = (1.0, 2, 0x3)
        @test_throws MethodError(convert, (Tuple{Vararg{T}} where T<:Integer, (2, 0x3))) convert(T, v)
    end
