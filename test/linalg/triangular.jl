...
@test LowerTriangular(randn(3, 3)) |> t -> [size(t, i) for i = 1:3] == [size(Array(t), i) for i = 1:3]
...
for elty1 in (Float32, Float64, BigFloat, Complex64, Complex128, Complex{BigFloat}, Int)
    ...
    for (t1, uplo1) in ((UpperTriangular, :U),
        ...
        @test convert(Matrix, A1) == Array(A1)

        # full!
        @test full!(copy(A1)) == Array(A1)

        # fill!
        @test full!(fill!(copy(A1), 1)) == Array(t1(ones(size(A1)...)))
        ...
        if uplo1 == :L
            @test tril(A1,0)  == A1
            @test tril(A1,-1) == LowerTriangular(tril(Array(A1),-1))
            @test tril(A1,1)  == t1(tril(tril(Array(A1),1)))
            ...
            @test triu(A1,0)  == A1
            @test triu(A1,1)  == UpperTriangular(triu(Array(A1),1))
            @test triu(A1,-1) == t1(triu(triu(Array(A1),-1)))
            @test_throws ArgumentError triu!(A1,n+1)
        ...
        let vrange = 1:n-1, viewA1 = t1(view(A1.data, vrange, vrange))
            # transpose
            @test Array(A1.') == Array(A1).'
            @test Array(viewA1.') == Array(viewA1).'
            # ctranspose
            @test Array(A1') == Array(A1)'
            @test Array(viewA1') == Array(viewA1)'
            # transpose!
        ...
        # diag
        @test diag(A1) == diag(Array(A1))

        # real
        @test Array(real(A1)) == real(Array(A1))
        @test Array(imag(A1)) == imag(Array(A1))
        @test Array(abs.(A1)) == abs.(Array(A1))
        ...
        let vrange = 1:n-1, viewA1 = t1(view(A1.data, vrange, vrange))
            # copy
            @test copy(A1) == copy(Array(A1))
            @test copy(viewA1) == copy(Array(viewA1))
            # copy!
            B = similar(A1)
            copy!(B, A1)
            @test B == A1
        ...
        if (elty1 == Float64 || elty1 == Complex128) && (t1 == UpperTriangular || t1 == LowerTriangular)
            @test expm(Array(logm(A1))) ≈ Array(A1)
        end
        ...
        # Binary operations
        @test A1*0.5 == Array(A1)*0.5
        @test 0.5*A1 == 0.5*Array(A1)
        @test A1/0.5 == Array(A1)/0.5
        @test 0.5\A1 == 0.5\Array(A1)

        # inversion
        @test inv(A1) ≈ inv(lufact(Array(A1)))
        inv(Array(A1)) # issue #11298
        @test isa(inv(A1), t1)
        # make sure the call to LAPACK works right
        if elty1 <: BlasFloat
            @test Base.LinAlg.inv!(copy(A1)) ≈ inv(lufact(Array(A1)))
        end

        # Determinant
        @test_approx_eq_eps det(A1) det(lufact(Array(A1))) sqrt(eps(real(float(one(elty1)))))*n*n
        @test_approx_eq_eps logdet(A1) logdet(lufact(Array(A1))) sqrt(eps(real(float(one(elty1)))))*n*n
        ...
        if !(elty1 in (BigFloat, Complex{BigFloat})) # Not handled yet
            ...
            if (t1 == UpperTriangular || t1 == LowerTriangular) && elty1 != Int # Cannot really handle degenerate eigen space and Int matrices will probably have repeated eigenvalues.
                @test_approx_eq_eps vecs*diagm(vals)/vecs Array(A1) sqrt(eps(float(real(one(vals[1])))))*(norm(A1, Inf)*n)^2
            end
        end
        ...
        if elty1 <:BlasFloat
            ...
            @test cond(A1,2) == cond(Array(A1),2)
        end
        ...
        for eltyB in (Float32, Float64, BigFloat, Complex64, Complex128, Complex{BigFloat})
            ...
            if !(eltyB in (BigFloat, Complex{BigFloat})) # rand does not support BigFloat and Complex{BigFloat} as of Dec 2015
                ...
                @test Base.LinAlg.A_mul_B!(Tri,copy(A1)) ≈ Tri*Array(A1)
            end

            # Triangular-dense Matrix/vector multiplication
            @test A1*B[:,1] ≈ Array(A1)*B[:,1]
            @test A1*B ≈ Array(A1)*B
            @test A1.'B[:,1] ≈ Array(A1).'B[:,1]
            @test A1'B[:,1] ≈ Array(A1)'B[:,1]
            @test A1.'B ≈ Array(A1).'B
            @test A1'B ≈ Array(A1)'B
            @test A1*B.' ≈ Array(A1)*B.'
            @test A1*B' ≈ Array(A1)*B'
            @test B*A1 ≈ B*Array(A1)
            @test B[:,1].'A1 ≈ B[:,1].'Array(A1)
            @test B[:,1]'A1 ≈ B[:,1]'Array(A1)
            @test B.'A1 ≈ B.'Array(A1)
            @test B'A1 ≈ B'Array(A1)
            @test B*A1.' ≈ B*Array(A1).'
...
for eltya in (Float32, Float64, Complex64, Complex128, BigFloat, Int)
    ...
    for eltyb in (Float32, Float64, Complex64, Complex128)
        ...
        b = convert(Matrix{eltyb}, eltya <: Complex ? Array(Atri)*ones(n, 2) : Array(Atri)*ones(n, 2))
        x = Array(Atri) \ b
