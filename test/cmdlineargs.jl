...
for precomp in ("yes", "no")
    ...
    @test contains(bt, "in _include")
    if is_windows() && Sys.WORD_SIZE == 32 && precomp == "yes"
        # fixme, issue #17251
        @test_broken contains(bt, "in _include(::String) at $(joinpath(".","loading.jl"))")
    else
        @test contains(bt, "in _include(::String) at $(joinpath(".","loading.jl"))")
    end
