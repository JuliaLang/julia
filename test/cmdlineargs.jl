...
let exename = `$(Base.julia_cmd()) --sysimage-native-code=yes --startup-file=no`
    ...
    for nc in ("0", "-2", "x", "2x", " ")
        v = readchomperrors(setenv(`$exename -i -E 'Sys.CPU_LOGICAL_CORES'`, "JULIA_CPU_LOGICAL_CORES" => nc))
        @test v[1]
        @test v[2] == "1"
        @test v[3] == "WARNING: couldn't parse `JULIA_CPU_LOGICAL_CORES` environment variable. Defaulting Sys.CPU_LOGICAL_CORES to 1."
    end
