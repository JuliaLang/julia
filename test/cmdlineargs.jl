...
let exename = `$(Base.julia_cmd()) --startup-file=no --color=no`
    ...
    mktempdir() do dir
        ...
        rm(memfile)
        @test popfirst!(got) == "        0 g(x) = x + 123456" broken=coverage_enabled
        @test popfirst!(got) == "        - function f(x)" broken=coverage_enabled
        @test popfirst!(got) == "        -     []" broken=coverage_enabled
        if Sys.WORD_SIZE == 64
            # P64 pools with 64 bit tags
            @test popfirst!(got) == "       16     Base.invokelatest(g, 0)" broken=coverage_enabled
        ...
        if Sys.WORD_SIZE == 64
            @test popfirst!(got) == "       32     []" broken=coverage_enabled
        else
            @test popfirst!(got) == "       16     []" broken=coverage_enabled
        end
        @test popfirst!(got) == "        - end" broken=coverage_enabled
        @test popfirst!(got) == "        - f(1.23)" broken=coverage_enabled
