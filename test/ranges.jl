...
# overflow in length
@test_throws length(0:typemax(Int)) OverflowError
@test_throws length(typemin(Int):typemax(Int)) OverflowError
@test_throws length(-1:typemax(Int)-1) OverflowError
...
let r = typemin(Int64):2:typemax(Int64), s = typemax(Int64):-2:typemin(Int64)
    ...
    @test_throws length(r) OverflowError

    @test first(s) == typemax(Int64)
    @test last(s) == (typemin(Int64)+1)
    @test_throws length(s) OverflowError
end
