...
# copy of the other dict.jl test file, amended for ODict, partially commented out
# for stuff that didn't seem to matter.
...
@testset "ODict" begin
    ...
    @testset "eltype, keytype and valtype" begin
        ...
        td = ODict{AbstractString,Float64}()
        @test eltype(td) == Pair{AbstractString,Float64}
        @test keytype(td) == AbstractString
        @test valtype(td) == Float64
        @test keytype(ODict{AbstractString,Float64}) === AbstractString
        @test valtype(ODict{AbstractString,Float64}) === Float64
    end
    # test rethrow of error in ctor
    @test_throws DomainError ODict((sqrt(p[1]), sqrt(p[2])) for p in zip(-1:2, -1:2))
end

let x = ODict(3=>3, 5=>5, 8=>8, 6=>6)
    pop!(x, 5)
    for k in keys(x)
        ODict{Int,Int}(x)
        @test k in [3, 8, 6]
...
@testset "typeof" begin
    ...
    d3 = ODict((1 => 2, 3 => 4))
    @test d == d2 == d3
    @test typeof(d) == typeof(d2) == typeof(d3) == ODict{Int,Int}
    ...
    d3 = ODict((1 => 2, 3 => "b"))
    @test d == d2 == d3
    @test typeof(d) == typeof(d2) == typeof(d3) == ODict{Int,Any}
    ...
    d3 = ODict((1 => 2, "a" => 4))
    @test d == d2 == d3
    @test typeof(d) == typeof(d2) == typeof(d3) == ODict{Any,Int}
    ...
    d3 = ODict((1 => 2, "a" => "b"))
    @test d == d2 == d3
    @test typeof(d) == typeof(d2) == typeof(d3) == ODict{Any,Any}
end

@test_throws ArgumentError first(ODict())
@test first(ODict(:f=>2)) == (:f=>2)

@testset "constructing ODicts from iterators" begin
    d = @inferred ODict(i=>i for i=1:3)
    @test isa(d, ODict{Int,Int})
    @test d == ODict(1=>1, 2=>2, 3=>3)
    d = ODict(i==1 ? (1=>2) : (2.0=>3.0) for i=1:2)
    @test isa(d, ODict{Real,Real})
    @test d == ODict{Real,Real}(2.0=>3.0, 1=>2)
end

@testset "type of ODict constructed from varargs of Pairs" begin
    @test ODict(1=>1, 2=>2.0) isa ODict{Int,Real}
    @test ODict(1=>1, 2.0=>2) isa ODict{Real,Int}
    @test ODict(1=>1.0, 2.0=>2) isa ODict{Real,Real}

    for T in (Nothing, Missing)
        @test ODict(1=>1, 2=>T()) isa ODict{Int,Union{Int,T}}
        @test ODict(1=>T(), 2=>2) isa ODict{Int,Union{Int,T}}
        @test ODict(1=>1, T()=>2) isa ODict{Union{Int,T},Int}
        @test ODict(T()=>1, 2=>2) isa ODict{Union{Int,T},Int}
    end
end

@test_throws KeyError ODict("a"=>2)[Base.secret_table_token]

@testset "issue #1821" begin
    d = ODict{String, Vector{Int}}()
    d["a"] = [1, 2]
...
@testset "equality" for eq in (isequal, ==)
    @test  eq(ODict(), ODict())
    @test  eq(ODict(1 => 1), ODict(1 => 1))
    @test !eq(ODict(1 => 1), ODict())
    @test !eq(ODict(1 => 1), ODict(1 => 2))
    @test !eq(ODict(1 => 1), ODict(2 => 1))

    # Generate some data to populate dicts to be compared
    data_in = [ (rand(1:1000), randstring(2)) for _ in 1:1001 ]

    # Populate the first dict
    d1 = ODict{Int, AbstractString}()
    for (k, v) in data_in
        d1[k] = v
    end
    data_in = collect(d1)
    ...
    @test eq(ODict(), sizehint!(ODict(),96))

    # ODictionaries of different types
    @test !eq(ODict(1 => 2), ODict("dog" => "bone"))
    @test eq(ODict{Int,Int}(), ODict{AbstractString,AbstractString}())
end

@testset "equality special cases" begin
    @test ODict(1=>0.0) == ODict(1=>-0.0)
    @test !isequal(ODict(1=>0.0), ODict(1=>-0.0))

    @test ODict(0.0=>1) != ODict(-0.0=>1)
    @test !isequal(ODict(0.0=>1), ODict(-0.0=>1))

    @test ODict(1=>NaN) != ODict(1=>NaN)
    @test isequal(ODict(1=>NaN), ODict(1=>NaN))

    @test ODict(NaN=>1) == ODict(NaN=>1)
    @test isequal(ODict(NaN=>1), ODict(NaN=>1))

    @test ismissing(ODict(1=>missing) == ODict(1=>missing))
    @test isequal(ODict(1=>missing), ODict(1=>missing))
    d = ODict(1=>missing)
    @test ismissing(d == d)
    d = ODict(1=>[missing])
    @test ismissing(d == d)
    ...
    @test ODict(missing=>1) == ODict(missing=>1)
    @test isequal(ODict(missing=>1), ODict(missing=>1))
end
...
@testset "get!" begin # (get with default values assigned to the given location)
    ...
    @test d == ODict(8=>19, 19=>2, 42=>4)
end

@testset "getkey" begin
   h = ODict(1=>2, 3 => 6, 5=>10)
   @test getkey(h, 1, 7) == 1
   @test getkey(h, 4, 6) == 6
   @test getkey(h, "1", 8) == 8
end
...
@testset "show" begin
    for d in (ODict("\n" => "\n", "1" => "\n", "\n" => "2"),
              ODict(string(i) => i for i = 1:30),
              ODict(reshape(1:i^2,i,i) => reshape(1:i^2,i,i) for i = 1:24),
              ODict(String(Char['α':'α'+i;]) => String(Char['α':'α'+i;]) for i = (1:10)*10),
              ODict("key" => zeros(0, 0)))
...
@testset "issue #9463" begin
    ...
    Base.show(io, MIME("text/plain"), ODict(Alpha()=>1))
...
@testset "issue #2540" begin
    d = ODict{Any,Any}(ODict(x => 1 for x in ['a', 'b', 'c']))
    @test d == ODict('a'=>1, 'b'=>1, 'c'=> 1)
end

@testset "issue #2629" begin
    d = ODict{AbstractString,AbstractString}(ODict(a=>"foo" for a in ["a","b","c"]))
    @test d == ODict("a"=>"foo","b"=>"foo","c"=>"foo")
end
...
@testset "issue #8877" begin
    a = ODict("foo" => 0.0, "bar" => 42.0)
    b = ODict("フー" => 17, "バー" => 4711)
    @test typeof(merge(a, b)) === ODict{String,Float64}
end
...
@testset "Issue 12451" begin
    @test_throws ArgumentError ODict(0)
    @test_throws ArgumentError ODict([1])
    @test_throws ArgumentError ODict([(1,2),0])
end

# test ODict constructor's argument checking (for an iterable of pairs or tuples)
# make sure other errors can propagate when the nature of the iterator is not the problem
@test_throws InexactError ODict(convert(Int,1.5) for i=1:1)
...
@testset "issue #15077" begin
    let badKeys = [
        ...
        local d = ODict{AbstractString,Int}()
...
# #18213
ODict(1 => rand(2,3), 'c' => "asdf") # just make sure this does not trigger a deprecation
...
@testset "issue #19995, hash of dicts" begin
    @test hash(ODict(ODict(1=>2) => 3, ODict(4=>5) => 6)) != hash(ODict(ODict(4=>5) => 3, ODict(1=>2) => 6))
    a = ODict(ODict(3 => 4, 2 => 3) => 2, ODict(1 => 2, 5 => 6) => 1)
    b = ODict(ODict(1 => 2, 2 => 3, 5 => 6) => 1, ODict(3 => 4) => 2)
    @test hash(a) != hash(b)
end
...
@testset "issue #18708 error type for dict constructor" begin
    @test_throws UndefVarError ODict(x => y for x in 1:10)
end
...
@testset "issue #19179 throwing error in dict constructor" begin
    @test_throws Error19179 ODict(i => throw(Error19179()) for i in 1:10)
end

# issue #18090
let
    d = ODict(i => i^2 for i in 1:10_000)
...
@testset "ODict merge" begin
    d1 = ODict("A" => 1, "B" => 2)
    d2 = ODict("B" => 3.0, "C" => 4.0)
    @test @inferred merge(d1, d2) == ODict("A" => 1, "B" => 3, "C" => 4)
    # merge with combiner function
    @test @inferred mergewith(+, d1, d2) == ODict("A" => 1, "B" => 5, "C" => 4)
    @test @inferred mergewith(*, d1, d2) == ODict("A" => 1, "B" => 6, "C" => 4)
    @test @inferred mergewith(-, d1, d2) == ODict("A" => 1, "B" => -1, "C" => 4)
    @test @inferred mergewith(NonFunctionCallable(), d1, d2) == ODict("A" => 1, "B" => 5, "C" => 4)
    @test foldl(mergewith(+), [d1, d2]; init=ODict{Union{},Union{}}()) ==
        ODict("A" => 1, "B" => 5, "C" => 4)
    # backward compatibility
    @test @inferred merge(+, d1, d2) == ODict("A" => 1, "B" => 5, "C" => 4)
end
...
@testset "ODict merge!" begin
    ...
    @test d1 == ODict("A" => 1, "B" => 3, "C" => 4)
    # merge! with combiner function
    @inferred mergewith!(+, d1, d2)
    @test d1 == ODict("A" => 1, "B" => 6, "C" => 8)
    @inferred mergewith!(*, d1, d2)
    @test d1 == ODict("A" => 1, "B" => 18, "C" => 32)
    @inferred mergewith!(-, d1, d2)
    @test d1 == ODict("A" => 1, "B" => 15, "C" => 28)
    ...
    # backward compatibility
    merge!(+, d1, d2)
    @test d1 == ODict("A" => 1, "B" => 21, "C" => 36)
end

@testset "ODict reduce merge" begin
    function check_merge(i::Vector{<:ODict}, o)
        r1 = reduce(merge, i)
        r2 = merge(i...)
        t = typeof(o)
        @test r1 == o
    ...
    check_merge([ODict(1=>2), ODict(1.0=>2.0)], ODict(1.0=>2.0))
    check_merge([ODict(1=>2), ODict(2=>Complex(1.0, 1.0))],
      ODict(2=>Complex(1.0, 1.0), 1=>Complex(2.0, 0.0)))
    check_merge([ODict(1=>2), ODict(3=>4)], ODict(3=>4, 1=>2))
    check_merge([ODict(3=>4), ODict(:a=>5)], ODict(:a => 5, 3 => 4))
end
...
@testset "find" begin
    @test findall(isequal(1), ODict(:a=>1, :b=>2)) == [:a]
    @test sort(findall(isequal(1), ODict(:a=>1, :b=>1))) == [:a, :b]
    @test isempty(findall(isequal(1), ODict()))
    @test isempty(findall(isequal(1), ODict(:a=>2, :b=>3)))

    @test findfirst(isequal(1), ODict(:a=>1, :b=>2)) == :a
    @test findfirst(isequal(1), ODict(:a=>1, :b=>1, :c=>3)) in (:a, :b)
    @test findfirst(isequal(1), ODict()) === nothing
    @test findfirst(isequal(1), ODict(:a=>2, :b=>3)) === nothing
end

#== seemingly not related to ODict:

@testset "ODict printing with limited rows" begin
    local buf
    buf = IOBuffer()
...
@testset "copy!" begin
    ...
    for a = ([3=>4], [0x3=>0x4], [3=>4, 5=>6, 7=>8], Pair{UInt,UInt}[3=>4, 5=>6, 7=>8])
        @test s === copy!(s, ODict(a)) == ODict(a)
        if length(a) == 1 # current limitation of Base.ImmutableDict
            @test s === copy!(s, Base.ImmutableDict(a[])) == ODict(a[])
        end
    end
end
