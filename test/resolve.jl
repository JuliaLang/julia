...
vr(lb) = VersionRange("$lb-*")
vr(lb, ub) = VersionRange("$lb-$ub")

function get_verspec(r)
    length(r) == 1 && return VersionSpec()
    VersionSpec([k == length(r) ? vr(r[k]) : vr(r[k], r[k+1]) for k = 2:2:length(r)])
end
