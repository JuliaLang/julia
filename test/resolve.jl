...
function deps_from_data(deps_data)
    deps = Dict{UTF8String,Dict{VersionNumber,Available}}()
    for d in deps_data
        ...
        if !haskey(deps[p], vn)
            deps[p][vn] = Available("$(p)_$(vn)_sha1", Dict{UTF8String,VersionSet}())
        end
        isempty(r) && continue
        rp = r[1]
        if length(r) > 1
...
function reqs_from_data(reqs_data)
    reqs = Dict{UTF8String,VersionSet}()
    for r in reqs_data
        p = r[1]
...
function sanity_tst(deps_data, expected_result; pkgs=[])
    ...
    #println("deps=$deps")
    #println()
    result = sanity_check(deps, Set(UTF8String[pkgs...]))
