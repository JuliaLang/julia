...
# Test that the compiler knows it can remove a tuple call generated by the
# _apply special case in inlining
function foo_splats(a, b)
    tuple(a..., b...)
    nothing
end
let ci = code_typed(foo_splats, Tuple{Tuple{Int64}, Tuple{Int64}})[1].first
    @test length(ci.code) == 1 && ci.code[1].head == :return
end

# Test that boundcheck elimination works as advertised
function foo_boundscheck(i)
    @boundscheck (i == 3 || error())
    i
end
bar_boundcheck(i) = @inbounds foo_boundscheck(i)
let ci = code_typed(bar_boundcheck, Tuple{Int64})[1].first
    # TODO: This shouldn't be necessary anymore once we cfg simplify be default
    ...
    Core.Compiler.verify_ir(ir)
    @test length(ir.stmts) == 1 && isa(ir.stmts[1], Core.Compiler.ReturnNode)
end
