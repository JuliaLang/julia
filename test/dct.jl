...
for i = 1:length(X)
    @assert_approx_eq Xdct[i] true_Xdct[i]
    @assert_approx_eq Xdct![i] true_Xdct[i]
    @assert_approx_eq Xdct_1[i] true_Xdct_1[i]
    @assert_approx_eq Xdct!_1[i] true_Xdct_1[i]
    @assert_approx_eq Xdct_2[i] true_Xdct_2[i]
    @assert_approx_eq Xdct!_2[i] true_Xdct_2[i]

    @assert_approx_eq pXdct[i] true_Xdct[i]
    @assert_approx_eq pXdct![i] true_Xdct[i]
    @assert_approx_eq pXdct_1[i] true_Xdct_1[i]
    @assert_approx_eq pXdct!_1[i] true_Xdct_1[i]
    @assert_approx_eq pXdct_2[i] true_Xdct_2[i]
    @assert_approx_eq pXdct!_2[i] true_Xdct_2[i]

    @assert_approx_eq Xidct[i] X[i]
    @assert_approx_eq Xidct![i] X[i]
    @assert_approx_eq Xidct_1[i] X[i]
    @assert_approx_eq Xidct!_1[i] X[i]
    @assert_approx_eq Xidct_2[i] X[i]
    @assert_approx_eq Xidct!_2[i] X[i]

    @assert_approx_eq pXidct[i] X[i]
    @assert_approx_eq pXidct![i] X[i]
    @assert_approx_eq pXidct_1[i] X[i]
    @assert_approx_eq pXidct!_1[i] X[i]
    @assert_approx_eq pXidct_2[i] X[i]
    @assert_approx_eq pXidct!_2[i] X[i]

    @assert_approx_eq sXdct[i] true_Xdct[i]
    @assert_approx_eq psXdct[i] true_Xdct[i]
    @assert_approx_eq sXdct![i] true_Xdct[i]
    @assert_approx_eq psXdct![i] true_Xdct[i]
end
