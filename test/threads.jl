...
let code = """
    ...
    victim = Task(() -> (ran[] = 1))
    victim.sticky = false
    # default pool (2 threads): the multiqueue, where the wake is count-gated
...
# A searcher enqueues work under its own slot, so the wake is suppressed, then
# releases the slot on entering the sleep transition and unwinds from the
# post-publish recheck. It holds no slot when it unwinds, so the sleep
# transition's handler must hand the suppressed wake on.
let code = """
    using Base.Threads
    ran = Atomic{Int}(0)
    victim = Task(() -> (ran[] = 1))
