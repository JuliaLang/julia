...
let code = code_lowered(FieldConvert)[1].code
    local fc_global_ssa, sp1_ssa, apply_type_ssa, field_type_ssa,
        ...
        new_ssa
    @test code[(sp1_ssa = 1;)] == Expr(:static_parameter, 1)
    @test code[(apply_type_ssa = 2;)] == Expr(:call, GlobalRef(Core, :apply_type), GlobalRef(@__MODULE__, :FieldConvert), GlobalRef(@__MODULE__, :FieldTypeA), Core.SSAValue(sp1_ssa))
    @test code[(field_type_ssa = 3;)] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(apply_type_ssa), 1)
    @test code[9] == Expr(:(=), Core.SlotNumber(10), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(field_type_ssa), Core.SlotNumber(10)))
    @test code[(slot_read_1 = 10;)] == Core.SlotNumber(10)
    @test code[(field_type2_ssa = 11;)] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(apply_type_ssa), 2)
    @test code[17] == Expr(:(=), Core.SlotNumber(9), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(field_type2_ssa), Core.SlotNumber(9)))
    @test code[(slot_read_2 = 18;)] == Core.SlotNumber(9)
    @test code[(field_type4_ssa = 19;)] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(apply_type_ssa), 4)
    @test code[25] == Expr(:(=), Core.SlotNumber(8), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(field_type4_ssa), Core.SlotNumber(8)))
    @test code[(slot_read_3 = 26;)] == Core.SlotNumber(8)
    @test code[(field_type5_ssa = 27;)] == Expr(:call, GlobalRef(Core, :fieldtype), Core.SSAValue(apply_type_ssa), 5)
    @test code[33] == Expr(:(=), Core.SlotNumber(7), Expr(:call, GlobalRef(Base, :convert), Core.SSAValue(field_type5_ssa), Core.SlotNumber(7)))
    @test code[(slot_read_4 = 34;)] == Core.SlotNumber(7)
    @test code[(new_ssa = 35;)] == Expr(:new, Core.SSAValue(apply_type_ssa), Core.SSAValue(slot_read_1), Core.SSAValue(slot_read_2), Core.SlotNumber(4), Core.SSAValue(slot_read_3), Core.SSAValue(slot_read_4))
    @test code[36] == Core.ReturnNode(Core.SSAValue(new_ssa))
end
