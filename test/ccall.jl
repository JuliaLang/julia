...
let A = [1]
    ...
    finalizer(A, cglobal((:finalizer_cptr, libccalltest), Cvoid))
    finalize(A)
    @test ccall((:get_c_int, libccalltest), Cint, ()) == -1
end

# Pointer finalizer at exit (PR #19911)
let result = read(`$(Base.julia_cmd()) --startup-file=no -e "A = Ref{Cint}(42); finalizer(A, cglobal((:c_exit_finalizer, \"$libccalltest\"), Cvoid))"`, String)
