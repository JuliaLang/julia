...
foo(::Type{Array{S,N}}, A::MyArray{T,N}) where {S<:AbstractFloat,N,T<:AbstractFloat} =
    copyto!(Array{S}(uninitialized, unsize(A)), A.data)
foo(::Type{Array{S,N}}, A::AbstractArray{T,N}) where {S<:AbstractFloat,N,T<:AbstractFloat} =
    copyto!(Array{S}(uninitialized, size(A)), A)
end
...
@testset "detect_unbound_args in Base and Core" begin
    ...
    let need_to_handle_undef_sparam =
        ...
        pop!(need_to_handle_undef_sparam, which(Base.promote_rule, Tuple{Base.DefaultPromotion, Type{Union{Nothing, S}} where S, Type{T} where T}))
        pop!(need_to_handle_undef_sparam, which(Base.promote_rule, Tuple{Base.DefaultPromotion, Type{Union{Missing, S}} where S, Type{T} where T}))
        pop!(need_to_handle_undef_sparam, which(Base.promote_rule, Tuple{Base.ExactPromotion, Type{Union{Nothing, S}} where S, Type{T} where T}))
        pop!(need_to_handle_undef_sparam, which(Base.promote_rule, Tuple{Base.ExactPromotion, Type{Union{Missing, S}} where S, Type{T} where T}))
