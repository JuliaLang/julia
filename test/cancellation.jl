...
Sys.isunix() && @testset "^C escalation ladder in the REPL (pty), $(isempty(tflags) ? "default threads" : join(tflags, " "))" for tflags in ([], ["-t2"])
    ...
    exe = joinpath(Sys.BINDIR, Base.julia_exename())
    p = run(detach(setenv(`$exe -i -q --startup-file=no --color=no $tflags`, env)),
    ...
    # On timeout, SIGQUIT the child so its all-tasks backtrace dump lands in
    # the reported transcript tail. The generous default absorbs first-use
    # JIT of the error-display path on slow hosts.
    function dump_child_tasks()
    ...
    function expect(needle::String; timeout::Real=120.0)
        ...
        if status !== :ok
            was_alive = process_running(p)
            dump_child_tasks()
            @error "expect timed out" needle was_alive exitcode=(was_alive ? nothing : p.exitcode) termsignal=(was_alive ? nothing : p.termsignal)
            # println, not an @error field: the logger truncates long values,
            # and the transcript tail carries the child's full task dump
            println(stderr, "-- transcript tail --\n", snapshot()[max(1, cursor[]):end], "\n-- end tail --")
        end
        @test status == :ok
    end
    ...
    function expect_all(needles::String...; timeout::Real=120.0)
        last_end = Ref(cursor[])
        status = timedwait(timeout; pollint=0.05) do
            s = snapshot()
            stop = cursor[]
        ...
        if status !== :ok
            was_alive = process_running(p)
            dump_child_tasks()
            @error "expect_all timed out" needles was_alive exitcode=(was_alive ? nothing : p.exitcode) termsignal=(was_alive ? nothing : p.termsignal)
            println(stderr, "-- transcript tail --\n", snapshot()[max(1, cursor[]):end], "\n-- end tail --")
        else
            cursor[] = last_end[]
        end
    ...
    for episode in 1:2
        ...
        # Rung offers print from the (serialized) signal listener thread ~1s
        # after the press; on a loaded host that thread may first be busy
        # with e.g. a backtrace dump, so the timeouts are generous safety
        # margins, not latency assertions.
        expect("Press ^C again to also stop waiting for external resources"; timeout=45.0)
        if episode == 1
            ...
            expect("signal ("; timeout=45.0) # the backtrace dump header
        end
        # The dispatch pass runs directly on the adopted signal thread, so
        # the full graded ladder is available regardless of the child's
        # thread topology (there is no listener to starve): every press
        # after an offer climbs exactly one rung. The endgame's messages
        # (abandonment warning, error display, fresh prompt) interleave
        # nondeterministically, hence expect_all.
        kill(p, Base.SIGINT) # press 2: ABANDON_EXTERNAL
        expect("No longer waiting for external resources")
        expect("Press ^C again to forcibly abandon"; timeout=45.0)
...
Sys.isunix() && @testset "^C in the REPL (pty)" begin
    ...
    kill(p, Base.SIGINT)
    expect("failed to acknowledge SIGINT"; timeout=45.0)
    kill(p, Base.SIGINT)
