...
@testset "Unary operators" begin
    b1 = bitrand(n1, n2)
    @check_bit_operation broadcast(flipbits, b1)  BitMatrix
    ...
    b0 = falses(0)
    @check_bit_operation broadcast(flipbits, b0)  BitVector
...
@testset "count & find" begin
    ...
    for i = 0:(v1-1)
        ...
        @test Base.findfirstnot(flipbits.(b1 >> i)) == i+1
    end

    for i = 3:(v1-1), j = 2:i
        submask = b1 << (v1-j+1)
        @test findnext((b1 >> i) .| submask, j) == i+1
        @test findnextnot((flipbits.(b1 >> i)) .⊻ submask, j) == i+1
    end
...
@testset "map over bitarrays" begin
    for l = [0, 1, 63, 64, 65, 127, 128, 129, 255, 256, 257, 6399, 6400, 6401]
        b1 = bitrand(l)
        b2 = bitrand(l)
        @test map(flipbits, b1) == map(x->flipbits(x), b1) == broadcast(flipbits, b1)
        ...
        @testset "map! for length $l" begin
            ...
            @test map!(flipbits, b, b1) == map!(x->flipbits(x), b, b1) == broadcast(flipbits, b1) == b
            @test map!(!, b, b1) == map!(x->!x, b, b1) == broadcast(flipbits, b1) == b
    ...
    @testset "Issue #17970" begin
        ...
        @test C17970::BitArray{1} == map(flipbits, A17970)
    end
end
