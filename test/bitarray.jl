...
m2 = randi(n2)
b2 = bitrand(T, m1, m2)
@check_bit_operation copy_to BitArray{T} (b1, b2)
@check_bit_operation ref BitArray{T} (b1, 1:m1, m2:n2)
b2 = bitrand(T, m1, m2)
@check_bit_operation assign BitArray{T} (b1, b2, 1:m1, n2-m2+1:n2)
...
for p1 = [randi(v1) 1 63 64 65 191 192 193]
    for p2 = [randi(v1) 1 63 64 65 191 192 193]
        for n = 0 : min(v1 - p1 + 1, v1 - p2 + 1)
            b1 = bitrand(T, v1)
            b2 = bitrand(T, v1)
            @check_bit_operation copy_to BitArray{T} (b1, p1, b2, p2, n)
        end
    end
end
...
b1 = bitrand(T, n1, n2)
@check_bit_operation (~) Array{T} (b1,)
@check_bit_operation (-) Array{T} (b1,)
@check_bit_operation sign BitArray{T} (b1,)
@check_bit_operation real BitArray{T} (b1,)
@check_bit_operation imag BitArray{T} (b1,)
@check_bit_operation conj BitArray{T} (b1,)
...
b1 = bitrand(T, n1, n2)
b2 = bitrand(T, n1, n2)
@check_bit_operation (&) BitArray{T} (b1, b2)
@check_bit_operation (|) BitArray{T} (b1, b2)
@check_bit_operation ($) BitArray{T} (b1, b2)
@check_bit_operation (-) Array{T} (b1, b2)
@check_bit_operation (.*) BitArray{T} (b1, b2)
@check_bit_operation (./) Array{Float64} (b1, b2)
@check_bit_operation (.^) Array{Float64} (b1, b2)

b2 = bitones(T, n1, n2)
@check_bit_operation div Array{T} (b1, b2)
@check_bit_operation mod Array{T} (b1, b2)
...
b1 = bitrand(T, s1, s2, s3, s4)
for d = 1 : 4
    j = randi(size(b1, d))
    #for j = 1 : size(b1, d)
        @check_bit_operation slicedim BitArray{T} (b1, d, j)
    #end
    @check_bit_operation flipdim BitArray{T} (b1, d)
end

b1 = bitrand(T, n1, n2)
for k = 1 : 4
    @check_bit_operation rotl90 BitArray{T} (b1, k)
end
...
b1 = bitrand(T, n1, n2)
@check_bit_operation findn_nzs (Array{Int}, Array{Int}, BitArray{T}) (b1,)
...
m1 = randi(s1)
m2 = randi(s3)
@check_bit_operation max BitArray{T} (b1, (), (m1, m2))
@check_bit_operation min BitArray{T} (b1, (), (m1, m2))
@check_bit_operation sum Array{Int} (b1, (m1, m2))
@check_bit_operation prod BitArray{T} (b1, (m1, m2))
...
b1 = bitrand(T, v1)
@check_bit_operation transpose BitArray{T} (b1,)
...
b1 = bitrand(T, s1, s2, s3, s4)
p = randperm(4)
@check_bit_operation permute BitArray{T} (b1, p)
...
b1 = bitrand(T, v1)
b2 = bitrand(T, v1)
@check_bit_operation hcat BitArray{T} (b1, b2)
for m = 1 : v1 - 1
    @check_bit_operation vcat BitArray{T} (b1[1:m], b1[m+1:end])
end
...
b3 = bitrand(T, n1, n2)
b4 = bitrand(T, 1, n2)
@check_bit_operation hcat BitArray{T} (b1, b2, b3)
@check_bit_operation vcat BitArray{T} (b1, b4, b3)

b1 = bitrand(T, s1, s2, s3, s4)
b2 = bitrand(T, s1, s3, s3, s4)
b3 = bitrand(T, s1, s2, s3, s1)
@check_bit_operation cat BitArray{T} (2, b1, b2)
@check_bit_operation cat BitArray{T} (4, b1, b3)
@check_bit_operation cat BitArray{T} (6, b1, b1)
